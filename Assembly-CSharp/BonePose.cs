...
public class BonePose {
	...
	public void InitializeBone(Transform bx) {
		...
		m_bone = bx;
		var localPosition = m_bone.localPosition;
		tx = localPosition.x;
		ty = localPosition.y;
		tz = localPosition.z;
		var localEulerAngles = m_bone.localEulerAngles;
	...
	public bool IsSamePose(BonePose bn, float tolerance) {
		var num1 = Mathf.Abs(tx - bn.tx);
		var num2 = Mathf.Abs(ty - bn.ty);
		var num3 = Mathf.Abs(tz - bn.tz);
		var num4 = Mathf.Abs(rx - bn.rx);
		var num5 = Mathf.Abs(ry - bn.ry);
		var num6 = Mathf.Abs(rz - bn.rz);
		var num7 = Mathf.Abs(sx - bn.sx);
		var num8 = Mathf.Abs(sy - bn.sy);
		var num9 = Mathf.Abs(sz - bn.sz);
		return num1 < (double)tolerance && num2 < (double)tolerance && num3 < (double)tolerance &&
		       num4 < (double)tolerance && num5 < (double)tolerance && num6 < (double)tolerance &&
		       num7 < (double)tolerance && num8 < (double)tolerance && num9 < (double)tolerance;
	}
	...
	public void Deform(float weight, BoneDeformMemento memento) {
		if (m_bone == null)
			return;
		var localPosition = m_bone.localPosition;
		var tx = weight * this.tx;
		var ty = weight * this.ty;
		var tz = weight * this.tz;
		var rx = weight * this.rx;
		var ry = weight * this.ry;
		var rz = weight * this.rz;
		localPosition.x += tx;
		localPosition.y += ty;
		localPosition.z += tz;
	...
	private Transform FindRecursive(Transform obj, string name) {
		var recursive1 = obj.Find(name);
		if (recursive1 != null)
			return recursive1;
		foreach (Transform transform in obj) {
			var recursive2 = FindRecursive(transform, name);
			if (recursive2 != null)
				return recursive2;
		}
