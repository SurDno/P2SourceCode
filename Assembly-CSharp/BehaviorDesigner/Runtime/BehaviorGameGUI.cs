  ...
  public class BehaviorGameGUI : MonoBehaviour
  {
    ...
    public void OnGUI()
    {
      ...
      for (int index1 = 0; index1 < behaviorTrees.Count; ++index1)
      {
        ...
        Vector2 guiPoint = GUIUtility.ScreenToGUIPoint(Camera.main.WorldToScreenPoint(behaviorTreeClient.behavior.transform.position));
        GUIContent content = new GUIContent(text);
        Vector2 vector2 = GUI.skin.label.CalcSize(content);
        vector2.x += 14f;
        vector2.y += 5f;
        GUI.Box(new Rect(guiPoint.x - vector2.x / 2f, (float) (Screen.height - (double) guiPoint.y + vector2.y / 2.0), vector2.x, vector2.y), content);
      }
    }
  }
}
