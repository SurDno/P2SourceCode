...
public class BehaviorSource {
	...
	[SerializeField] [FormerlySerializedAs("mTaskData")]
	...
	public BehaviorSource(IBehaviorTree owner) {
		Initialize(owner);
	}

	public void Initialize(IBehaviorTree owner) {
		this.owner = owner;
	}
	...
	public bool CheckForSerialization(bool force, BehaviorSource behaviorSource, string context) {
		var behaviorSource1 = behaviorSource == null ? this : behaviorSource;
		...
		if (taskData != null && !string.IsNullOrEmpty(taskData.XmlData)) {
			...
			var optimizationService = ServiceCache.OptimizationService;
	...
	public void SetVariable(string name, SharedVariable sharedVariable) {
		...
		if (sharedVariableIndex != null && sharedVariableIndex.TryGetValue(name, out index)) {
			...
			if (!variable.GetType().Equals(typeof(SharedVariable)) &&
			    !variable.GetType().Equals(sharedVariable.GetType()))
				Debug.LogError(string.Format(
					"Error: Unable to set SharedVariable {0} - the variable type {1} does not match the existing type {2}",
					name, variable.GetType(), sharedVariable.GetType()));
			else
