...
public class VariableSynchronizer : MonoBehaviour {
	[SerializeField] private List<SynchronizedVariable> synchronizedVariables = new();
	...
	public void Awake() {
		for (var index = synchronizedVariables.Count - 1; index > -1; --index) {
			var synchronizedVariable = synchronizedVariables[index];
			synchronizedVariable.sharedVariable =
			...
			if (synchronizedVariable.sharedVariable == null)
				...
				switch (synchronizedVariable.synchronizationType) {
					case SynchronizationType.BehaviorDesigner:
						var targetComponent = synchronizedVariable.targetComponent as BehaviorTree;
					...
					case SynchronizationType.Property:
						var property = synchronizedVariable.targetComponent.GetType()
						...
						if (synchronizedVariable.setVariable) {
							...
							if (getMethod == null)
								...
								synchronizedVariable.getDelegate =
									CreateGetDelegate(synchronizedVariable.targetComponent, getMethod);
						} else {
							...
							if (setMethod == null)
								...
								synchronizedVariable.setDelegate =
									CreateSetDelegate(synchronizedVariable.targetComponent, setMethod);
						}
					...
					case SynchronizationType.Animator:
						...
						var propertyType = synchronizedVariable.sharedVariable.GetType().GetProperty("Value")
							.PropertyType;
					...
					case SynchronizationType.PlayMaker:
						var typeWithinAssembly1 =
							TaskUtility.GetTypeWithinAssembly(
								"BehaviorDesigner.Runtime.VariableSynchronizer_PlayMaker");
						if (typeWithinAssembly1 != null) {
							var method1 = typeWithinAssembly1.GetMethod("Start");
							if (method1 != null)
								switch ((int)method1.Invoke(null, new object[1] {
									...
									case 2:
										str =
											"the BehaviorTree Designer SharedVariable is not the same type as the PlayMaker NamedVariable";
										break;
									default:
										var method2 = typeWithinAssembly1.GetMethod("Tick");
										if (method2 != null)
											synchronizedVariable.thirdPartyTick =
												(Action<SynchronizedVariable>)Delegate.CreateDelegate(
													typeof(Action<SynchronizedVariable>), method2);
										break;
								}
					...
					case SynchronizationType.uFrame:
						var typeWithinAssembly2 =
							TaskUtility.GetTypeWithinAssembly("BehaviorDesigner.Runtime.VariableSynchronizer_uFrame");
						if (typeWithinAssembly2 != null) {
							var method3 = typeWithinAssembly2.GetMethod("Start");
							if (method3 != null)
								switch ((int)method3.Invoke(null, new object[1] {
									...
									case 2:
										str =
											"the BehaviorTree Designer SharedVariable is not the same type as the uFrame property";
										break;
									default:
										var method4 = typeWithinAssembly2.GetMethod("Tick");
										if (method4 != null)
											synchronizedVariable.thirdPartyTick =
												(Action<SynchronizedVariable>)Delegate.CreateDelegate(
													typeof(Action<SynchronizedVariable>), method4);
										break;
								}
			...
			if (!string.IsNullOrEmpty(str)) {
				Debug.LogError(string.Format("Unable to synchronize {0}: {1}", synchronizedVariable.sharedVariable.Name,
					str));
	...
	public void Tick() {
		for (var index = 0; index < synchronizedVariables.Count; ++index) {
			var synchronizedVariable = synchronizedVariables[index];
			switch (synchronizedVariable.synchronizationType) {
				case SynchronizationType.BehaviorDesigner:
					if (synchronizedVariable.setVariable) {
						synchronizedVariable.sharedVariable.SetValue(
							synchronizedVariable.targetSharedVariable.GetValue());
						break;
					}
				...
				case SynchronizationType.Animator:
					if (synchronizedVariable.setVariable)
						switch (synchronizedVariable.animatorParameterType) {
							case AnimatorParameterType.Bool:
								synchronizedVariable.sharedVariable.SetValue(
									synchronizedVariable.animator.GetBool(synchronizedVariable.targetID));
								continue;
							case AnimatorParameterType.Float:
								synchronizedVariable.sharedVariable.SetValue(
									synchronizedVariable.animator.GetFloat(synchronizedVariable.targetID));
								continue;
							case AnimatorParameterType.Integer:
								synchronizedVariable.sharedVariable.SetValue(
									synchronizedVariable.animator.GetInteger(synchronizedVariable.targetID));
								continue;
							default:
								continue;
						}
					...
					switch (synchronizedVariable.animatorParameterType) {
						case AnimatorParameterType.Bool:
							synchronizedVariable.animator.SetBool(synchronizedVariable.targetID,
								(bool)synchronizedVariable.sharedVariable.GetValue());
							break;
						case AnimatorParameterType.Float:
							synchronizedVariable.animator.SetFloat(synchronizedVariable.targetID,
								(float)synchronizedVariable.sharedVariable.GetValue());
							break;
						case AnimatorParameterType.Integer:
							synchronizedVariable.animator.SetInteger(synchronizedVariable.targetID,
	...
	private static Func<object> CreateGetDelegate(object instance, MethodInfo method) {
		...
		return Expression.Lambda<Func<object>>(Expression.TypeAs(methodCallExpression, typeof(object)),
			Array.Empty<ParameterExpression>()).Compile();
	}

	private static Action<object> CreateSetDelegate(object instance, MethodInfo method) {
		var instance1 = Expression.Constant(instance);
		var parameterExpression = Expression.Parameter(typeof(object), "p");
		var unaryExpression = Expression.Convert(parameterExpression, method.GetParameters()[0].ParameterType);
		return Expression
			.Lambda<Action<object>>(Expression.Call(instance1, method, unaryExpression), parameterExpression).Compile();
	}
