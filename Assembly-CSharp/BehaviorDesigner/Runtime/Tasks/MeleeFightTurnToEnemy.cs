...
public class MeleeFightTurnToEnemy : Action, IStub, ISerializeDataWrite, ISerializeDataRead {
	...
	public override TaskStatus OnUpdate() {
		...
		var forward = owner.Enemy.transform.position - transform.position;
		forward = new Vector3(forward.x, 0.0f, forward.z);
		forward.Normalize();
		return Quaternion.Angle(transform.rotation, Quaternion.LookRotation(forward)) < (double)AngleDelta
			? TaskStatus.Success
			: TaskStatus.Running;
	}
