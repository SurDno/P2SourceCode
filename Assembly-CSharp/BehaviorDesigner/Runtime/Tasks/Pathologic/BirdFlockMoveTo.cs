...
public class BirdFlockMoveTo : Action, IStub, ISerializeDataWrite, ISerializeDataRead {
	...
	[Tooltip("Adds to target")] [DataReadProxy] [DataWriteProxy] [CopyableProxy()] [SerializeField]
	...
	public override TaskStatus OnUpdate() {
		if (behavior == null) {
			...
			if (behavior == null) {
				Debug.LogWarning(
					gameObject.name + ": doesn't contain " + typeof(EngineBehavior).Name + " unity component",
					gameObject);
				return TaskStatus.Failure;
			}
		}

		var direction = Offset.Value + Target.Value - gameObject.transform.position;
