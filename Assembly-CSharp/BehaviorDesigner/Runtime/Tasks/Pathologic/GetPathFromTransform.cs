...
public class GetPathFromTransform : Action, IStub, ISerializeDataWrite, ISerializeDataRead {
	...
	public override void OnStart() {
		success = false;
		var component1 = Target.Value.GetComponent<PathPart>();
		if (component1 != null) {
			Result.Value = component1.PointsList;
			success = true;
		} else {
			var component2 = Target.Value.GetComponent<PatrolPath>();
			if (component2 != null) {
				var presetPath = component2.GetPresetPath(PointIndex.Value, InversePath.Value);
				if (presetPath != null) {
