...
public class Play3DOneShot : Action, IStub, ISerializeDataWrite, ISerializeDataRead {
	...
	[Tooltip("0 for 2D sound, 1 for 3D sound")] [DataReadProxy] [DataWriteProxy] [CopyableProxy] [SerializeField]
	...
	private void PlayAudioClip() {
		SoundUtility.PlayAudioClip3D(gameObject.transform, Sound, AudioMixerGroup, Volume.Value, MinDistance.Value,
			MaxDistance.Value, true, 0.0f);
	}
	...
	public override TaskStatus OnUpdate() {
		...
		if (detectable == null)
			Debug.LogWarning(
				gameObject.name + ": doesn't contain " + typeof(IDetectableComponent).Name +
				" engine component (this is not critical", gameObject);
		if (Sound == null) {
			Debug.LogWarning(
				gameObject.name + ": doesn't contain " + typeof(IDetectableComponent).Name + " engine component",
				gameObject);
			return TaskStatus.Failure;
		}
		...
		if (pivot == null) {
			Debug.LogErrorFormat(gameObject.name + " doesnt' contain " + typeof(Pivot).Name + " unity component",
				gameObject);
			return TaskStatus.Failure;
		}
