...
public class TeleportBehindPlayer : Action, IStub, ISerializeDataWrite, ISerializeDataRead {
	...
	public override TaskStatus OnUpdate() {
		...
		if (Time.time >= lastTryTime + (double)timeBetweenTries) {
			var vector3 = CountRandomPoint(((IEntityView)player).GameObject);
			if (TryPoint(vector3, ((IEntityView)player).GameObject.transform.position)) {
				var location = player.GetComponent<LocationItemComponent>()?.Location;
				entity.GetComponent<NavigationComponent>().TeleportTo(location, vector3,
					Quaternion.LookRotation(((IEntityView)player).GameObject.transform.position - vector3));
	...
	private Vector3 CountRandomPoint(GameObject player) {
		var num1 = UsePlayerVisionAngle.Value
			? InstanceByRequest<GraphicsGameSettings>.Instance.FieldOfView.Value
			: 0.0f;
		var num2 = Random.Range(num1 - 180f, 180f - num1);
		var y = player.transform.rotation.eulerAngles.y + num2;
		var num3 = Random.Range(MinSearchDistance.Value, MaxSearchDistance.Value);
	...
	private bool TryPoint(Vector3 point, Vector3 playerPosition) {
		...
		return path.status == NavMeshPathStatus.PathComplete &&
		       NavMeshUtility.GetPathLength(path) <= MaxSearchDistance.Value * 2.0;
	}
	...
	public void DataRead(IDataReader reader, Type type) {
		...
		UsePlayerVisionAngle =
			BehaviorTreeDataReadUtility.ReadShared(reader, "UsePlayerVisionAngle", UsePlayerVisionAngle);
