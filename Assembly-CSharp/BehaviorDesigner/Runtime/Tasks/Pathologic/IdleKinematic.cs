...
{
  [TaskDescription("Idle (at exact position, use it for Grif for ex)")]
  [TaskCategory("Pathologic")]
  [TaskIcon("Pathologic_IdleIcon.png")]
  [Factory]
  [GeneratePartial(TypeEnum.Cloneable | TypeEnum.Copyable | TypeEnum.DataRead | TypeEnum.DataWrite)]
  [FactoryProxy(typeof (IdleKinematic))]
  public class IdleKinematic : IdleBase, IStub, ISerializeDataWrite, ISerializeDataRead
  {
    protected override void DoIdle(NpcState state, float primaryIdleProbability)
    {
      state.IdleKinematic(primaryIdleProbability, MakeObstacle.Value);
    }
    ...
    public void DataRead(IDataReader reader, Type type)
    {
      ...
      primaryIdleProbability = BehaviorTreeDataReadUtility.ReadShared(reader, "PrimaryIdleProbability", primaryIdleProbability);
