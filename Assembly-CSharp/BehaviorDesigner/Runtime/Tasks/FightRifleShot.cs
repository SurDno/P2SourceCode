...
public class FightRifleShot : FightBase, IStub, ISerializeDataWrite, ISerializeDataRead {
	...
	public override TaskStatus DoUpdate(float deltaTime) {
		...
		if (!attackInited && !owner.IsAttacking && !owner.IsPushing && !owner.IsReacting && !owner.IsQuickBlock &&
		    CheckShootingLine()) {
			attackInited = true;
		...
		if (attackInited && !shotDone && (aimingTimePassed >= (double)aimingTime ||
		                                  (vector3.magnitude < (double)cancelRange &&
		                                   aimingTimePassed > (double)aimingTimeMinimum))) {
	...
	private bool CheckShootingLine() {
		...
		var direction = this.gameObject.transform.forward;
		var component = this.gameObject.GetComponent<Pivot>();
		if (component != null && component.ShootStart != null) {
			position = component.ShootStart.transform.position;
			var chest = owner.Enemy.GetComponent<Pivot>()?.Chest;
			if (chest != null)
		...
		PhysicsUtility.Raycast(result, position, direction, 50f, -1 ^ triggerInteractLayer ^ ragdollLayer,
			QueryTriggerInteraction.Ignore);
		for (var index = 0; index < result.Count; ++index) {
			var gameObject = result[index].collider.gameObject;
