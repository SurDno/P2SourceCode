...
public class MeleeFightSanitarFollow :
	...
	public override TaskStatus OnUpdate() {
		...
		if (description == null) {
			Debug.LogWarning(
				typeof(MeleeFightSanitarFollow).Name + " has no " + typeof(SanitarFollowDescription).Name + " attached",
				gameObject);
		...
		if (Target.Value == null)
			return TaskStatus.Success;
		UpdatePath();
		var vector3 = Target.Value.position - owner.transform.position;
		var magnitude = vector3.magnitude;
		vector3.Normalize();
		if (NavMesh.Raycast(owner.transform.position, Target.Value.position, out var _, -1)) {
			if (!agent.hasPath)
		...
		desiredWalkSpeed = agent.remainingDistance <= (double)description.KeepDistance
			?
			agent.remainingDistance <= (double)description.RetreatDistance ? -1f : 0.0f
			: agent.remainingDistance > (double)description.RunDistance
				? 2f
				: 1f;
