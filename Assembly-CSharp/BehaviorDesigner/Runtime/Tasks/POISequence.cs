...
public class POISequence : Composite, IStub, ISerializeDataWrite, ISerializeDataRead {
	...
	private List<POIBase> lastUsedPOIs = new();
	[HideInInspector] public POIBase OutPOI;

	[HideInInspector] [DataReadProxy] [DataWriteProxy] [CopyableProxy] [SerializeField]
	...
	protected List<POIBase> poiCache = new();
	...
	protected virtual void FillPOICache() {
		...
		if (SearchEntity.Entity != null) {
			var entity = (IEntityView)SearchEntity.Entity;
	...
	private void RecountIndoor() {
		var component = EntityUtility.GetEntity(gameObject)?.GetComponent<LocationItemComponent>();
		if (component == null)
			return;
	...
	public override void OnStart() {
		...
		if (POIBase.ActivePOIs.Count == 0)
			return;
		var character = gameObject.GetComponentNonAlloc<POISetup>();
		if (character == null)
			...
			var component1 = owner?.GetComponent<CrowdItemComponent>();
			if (component1 != null)
			...
			poiCache = poiCache.FindAll(x =>
				SearchEntity.Entity != null || InSearchRadius.Value == 0.0 ||
				(x.transform.position - gameObject.transform.position).magnitude < (double)InSearchRadius.Value);
			...
			if (FindClosest.Value) {
				var searchPosition = gameObject.transform.position + Quaternion.Euler(0.0f, Random.value * 360f, 0.0f) *
					(Vector3.forward * randomOffsetRadius);
				poiCache.Sort((c1, c2) =>
					Vector3.Distance(searchPosition, c1.transform.position)
						.CompareTo(Vector3.Distance(searchPosition, c2.transform.position)));
			}

			var component2 = owner?.GetComponent<NavigationComponent>();
			if (component2 != null)
				region = component2.Region;
			var num1 = FindClosest.Value ? 0 : Random.Range(0, poiCache.Count);
			for (var index1 = 0; index1 < poiCache.Count && OutPOI == null; ++index1) {
				var poiBase = poiCache[(index1 + num1) % poiCache.Count];
				if (!(poiBase.LockedBy != null)) {
					var poiAnimationEnum = poiBase.SupportedAnimations & character.SupportedAnimations;
					if (poiAnimationEnum != 0) {
						var magnitude = (poiBase.transform.position - gameObject.transform.position).magnitude;
						if ((poiCache.Count <= AvoidLastUsedPoiNumber.Value || !lastUsedPOIs.Contains(poiBase)) &&
						    (InSearchRadius.Value <= 0.0 || magnitude <= (double)InSearchRadius.Value ||
						     SearchEntity.Entity != null) && CheckRegion(poiBase.transform.position)) {
							var num2 = Random.Range(0, 32);
							for (var index2 = 0; index2 < 32; ++index2) {
								var num3 = (num2 + index2) % 32;
								if ((poiAnimationEnum & (POIAnimationEnum)(1 << num3)) != 0) {
									var animation = (POIAnimationEnum)(1 << num3);
									var animationSetup = character.GetAnimationSetup(animation);
									OutAnimationIndex = Random.Range(0, animationSetup.Elements.Count);
									if (animationSetup.Elements.Count > OutAnimationIndex &&
									    animationSetup.Elements[OutAnimationIndex] is POIAnimationSetupElementSlow)
										OutMiddleAnimationsCount =
											(animationSetup.Elements[OutAnimationIndex] as POIAnimationSetupElementSlow)
											.MiddleAnimationClips.Count;
									OutAngle = poiBase.GetAngle(Owner.gameObject);
									Vector3 targetPosition;
									Quaternion targetRotation;
									poiBase.GetRandomTargetPoint(animation, OutAnimationIndex, character,
	...
	public void DataRead(IDataReader reader, Type type) {
		...
		AvoidLastUsedPoiNumber =
			BehaviorTreeDataReadUtility.ReadShared(reader, "AvoidLastUsedPoiNumber", AvoidLastUsedPoiNumber);
		...
		PlayablePoisInRadius =
			BehaviorTreeDataReadUtility.ReadShared(reader, "PlayablePoisInRadius", PlayablePoisInRadius);
		...
		OutMiddleAnimationsCount =
			DefaultDataReadUtility.Read(reader, "OutMiddleAnimationsCount", OutMiddleAnimationsCount);
