...
public class MeleeFightRetreat : MeleeFightBase, IStub, ISerializeDataWrite, ISerializeDataRead {
	...
	public override TaskStatus DoUpdate(float deltaTime) {
		if (description == null) {
			Debug.LogWarning(
				typeof(MeleeFightRetreat).Name + " has no " + typeof(RetreatDescription).Name + " attached",
				gameObject);
		...
		var forward = owner.Enemy.transform.position - owner.transform.position;
		...
		if (fighter.IsReacting) {
			if (fighter.IsAttacking || fighter.IsContrReacting)
				owner.transform.rotation = Quaternion.RotateTowards(owner.transform.rotation,
					Quaternion.AngleAxis(0.0f, Vector3.up) * Quaternion.LookRotation(forward), 270f * Time.deltaTime);
			return TaskStatus.Running;
		}

		var retreatDirection =
			PathfindingHelper.FindBestRetreatDirection(owner.transform.position, owner.Enemy.transform.position);
		var quaternion1 = Quaternion.LookRotation(forward);
		if (!retreatDirection.HasValue)
			...
			if (magnitude < (double)description.RunDistance)
				...
				desiredWalkSpeed = 0.0f;
			if ((Escaping.Value && magnitude > (double)description.EscapeDistance) ||
			    (RetreatTime.Value > 0.0 && startTime + (double)waitDuration < Time.time))
				return TaskStatus.Success;
			var quaternion2 = quaternion1 * Quaternion.AngleAxis(retreatDirection.Value, Vector3.up);
		}
		...
		if (magnitude < (double)description.AttackDistance) {
			...
			if (attackCooldownTime <= 0.0 && !fightAnimatorState.IsAttacking && !fightAnimatorState.IsReaction &&
			    !fightAnimatorState.IsQuickBlock) {
