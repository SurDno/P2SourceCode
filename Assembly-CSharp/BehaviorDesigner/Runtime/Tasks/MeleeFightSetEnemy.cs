...
public class MeleeFightSetEnemy : Action, IStub, ISerializeDataWrite, ISerializeDataRead {
	...
	public override TaskStatus OnUpdate() {
		if (pivot == null) {
			...
			if (owner == null) {
				Debug.LogWarning(gameObject.name + ": doesn't contain " + typeof(EnemyBase).Name + " engine component",
					gameObject);
				return TaskStatus.Failure;
			}
			...
			if (animator == null) {
				Debug.LogWarning(gameObject.name + ": doesn't contain " + typeof(Animator).Name + " engine component",
					gameObject);
				return TaskStatus.Failure;
			}
		}

		if (EnemyTransform == null || EnemyTransform.Value == null ||
		    EnemyTransform.Value.GetComponentNonAlloc<EnemyBase>() == null) {
