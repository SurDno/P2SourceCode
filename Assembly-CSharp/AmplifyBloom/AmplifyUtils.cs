...
public class AmplifyUtils {
	...
	private static List<RenderTexture> _allocatedRT = new();
	...
	public static RenderTexture GetTempRenderTarget(int width, int height) {
		var temporary = RenderTexture.GetTemporary(width, height, 0, CurrentRTFormat, CurrentReadWriteMode);
	...
	public static void ReleaseAllRT() {
		for (var index = 0; index < _allocatedRT.Count; ++index) {
