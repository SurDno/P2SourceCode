...
public class StarDefData {
	...
	[SerializeField] private float m_sampleLength = 1f;
	[SerializeField] private float m_attenuation = 0.85f;
	[SerializeField] private float m_inclination;
	[SerializeField] private float m_rotation;
	[SerializeField] private StarLineData[] m_starLinesArr;
	[SerializeField] private float m_customIncrement = 90f;
	[SerializeField] private float m_longAttenuation;
	...
	public void CalculateStarData() {
		...
		var num = (m_customIncrement > 0.0 ? m_customIncrement : 180f / m_starlinesCount) * ((float)Math.PI / 180f);
		for (var index = 0; index < m_starlinesCount; ++index) {
			...
			m_starLinesArr[index].Attenuation = m_longAttenuation <= 0.0 ? m_attenuation :
				index % 2 == 0 ? m_longAttenuation : m_attenuation;
