...
namespace AmplifyBloom
{
  ...
  public class AmplifyBloomBase : MonoBehaviour
  {
    ...
    private void OnRenderImage(RenderTexture src, RenderTexture dest)
    {
      ...
      else
      {
            ...
            if (index == 0)
            {
              if (!this.m_temporalFilteringActive || this.m_gaussianSteps[index] != 0)
              {
                if (this.m_upscaleQuality == UpscaleQualityEnum.Realistic)
                  Graphics.Blit((Texture) renderTexture3, this.m_tempAuxDownsampleRTs[index], this.m_bloomMaterial, 10);
                else
                  Graphics.Blit((Texture) renderTexture3, this.m_tempAuxDownsampleRTs[index], this.m_bloomMaterial, 11);
              }
              else
              {
                if ((UnityEngine.Object) this.m_tempFilterBuffer != (UnityEngine.Object) null && this.m_temporalFilteringActive)
                {
                  ...
                  this.m_bloomMaterial.SetTexture(AmplifyUtils.AnamorphicRTS[0], (Texture) this.m_tempFilterBuffer);
                  if (this.m_upscaleQuality == UpscaleQualityEnum.Realistic)
                    Graphics.Blit((Texture) renderTexture3, this.m_tempAuxDownsampleRTs[index], this.m_bloomMaterial, 12);
                  else
                    Graphics.Blit((Texture) renderTexture3, this.m_tempAuxDownsampleRTs[index], this.m_bloomMaterial, 13);
                }
        ...
        if (this.m_separateFeaturesThreshold)
        {
          ...
          flag3 = true;
          Graphics.Blit((Texture) renderTexture3, renderTexture4, this.m_bloomMaterial, 0);
        ...
        {
          renderTexture1 = this.m_lensFlare.ApplyFlare(this.m_bloomMaterial, renderTexture4);
          this.ApplyGaussianBlur(renderTexture1, this.m_lensFlare.LensFlareGaussianBlurAmount);
        ...
        if (this.m_anamorphicGlare.ApplyLensGlare && this.m_debugToScreen != DebugToScreenEnum.Bloom)
        {
          ...
          this.m_anamorphicGlare.OnRenderImage(this.m_bloomMaterial, renderTexture4, renderTexture2, num1);
        ...
        if (flag1)
          this.ApplyGaussianBlur(renderTexture3, this.m_gaussianSteps[0], this.m_gaussianRadius[0]);
        if (this.m_bloomDownsampleCount > 0)
        {
          ...
          {
            for (int index3 = 0; index3 < this.m_bloomDownsampleCount; ++index3)
            {
              int index4 = this.m_bloomDownsampleCount - 1 - index3;
              this.m_finalCompositionMaterial.SetTexture(AmplifyUtils.MipResultsRTS[index4], (Texture) this.m_tempAuxDownsampleRTs[index4]);
              this.m_finalCompositionMaterial.SetFloat(AmplifyUtils.UpscaleWeightsStr[index4], this.m_upscaleWeights[index3]);
            }
          }
        }
        else
        {
          this.m_finalCompositionMaterial.SetTexture(AmplifyUtils.MipResultsRTS[0], (Texture) renderTexture3);
        ...
        else
        {
          ...
          if (this.m_anamorphicGlare.ApplyLensGlare)
            this.m_finalCompositionMaterial.SetTexture(AmplifyUtils.LensGlareRTId, (Texture) renderTexture2);
    ...
    public AmplifyGlare LensGlareInstance => this.m_anamorphicGlare;

    public AmplifyBokeh BokehFilterInstance => this.m_bokehFilter;

    public AmplifyLensFlare LensFlareInstance => this.m_lensFlare;

    public bool ApplyLensDirt
    {
      get => this.m_applyLensDirt;
      set => this.m_applyLensDirt = value;
    }

    public float LensDirtStrength
    {
      get => this.m_lensDirtStrength;
      set => this.m_lensDirtStrength = (double) value < 0.0 ? 0.0f : value;
    }

    public Texture LensDirtTexture
    {
      get => this.m_lensDirtTexture;
      set => this.m_lensDirtTexture = value;
    }

    public bool ApplyLensStardurst
    {
      get => this.m_applyLensStardurst;
      set => this.m_applyLensStardurst = value;
    }

    public Texture LensStardurstTex
    {
      get => this.m_lensStardurstTex;
      set => this.m_lensStardurstTex = value;
    }

    public float LensStarburstStrength
    {
      get => this.m_lensStarburstStrength;
      set => this.m_lensStarburstStrength = (double) value < 0.0 ? 0.0f : value;
    }

    public PrecisionModes CurrentPrecisionMode
    {
      get => this.m_highPrecision ? PrecisionModes.High : PrecisionModes.Low;
      set => this.HighPrecision = value == PrecisionModes.High;
    }

    public bool HighPrecision
    {
      get => this.m_highPrecision;
      set
      {
    ...
    {
      get => this.m_bloomRange.x;
      set => this.m_bloomRange.x = (double) value < 0.0 ? 0.0f : value;
    }

    public float OverallThreshold
    {
      get => this.m_overallThreshold;
      set => this.m_overallThreshold = (double) value < 0.0 ? 0.0f : value;
    }

    public Vector4 BloomParams
    {
      get => this.m_bloomParams;
      set => this.m_bloomParams = value;
    }

    public float OverallIntensity
    {
      get => this.m_bloomParams.x;
      set => this.m_bloomParams.x = (double) value < 0.0 ? 0.0f : value;
    }

    public float BloomScale
    {
      get => this.m_bloomParams.w;
      set => this.m_bloomParams.w = (double) value < 0.0 ? 0.0f : value;
    }

    public float UpscaleBlurRadius
    {
      get => this.m_bloomParams.z;
      set => this.m_bloomParams.z = value;
    }

    public bool TemporalFilteringActive
    {
      get => this.m_temporalFilteringActive;
      set
      {
    ...
    {
      get => this.m_temporalFilteringValue;
      set => this.m_temporalFilteringValue = value;
    }

    public int SoftMaxdownscales => this.m_softMaxdownscales;

    public int BloomDownsampleCount
    {
      get => this.m_bloomDownsampleCount;
      set => this.m_bloomDownsampleCount = Mathf.Clamp(value, 1, this.m_softMaxdownscales);
    }

    public int FeaturesSourceId
    {
      get => this.m_featuresSourceId;
      set => this.m_featuresSourceId = Mathf.Clamp(value, 0, this.m_bloomDownsampleCount - 1);
    }

    public bool[] DownscaleSettingsFoldout => this.m_downscaleSettingsFoldout;

    public float[] UpscaleWeights => this.m_upscaleWeights;

    public float[] LensDirtWeights => this.m_lensDirtWeights;

    public float[] LensStarburstWeights => this.m_lensStarburstWeights;

    public float[] GaussianRadius => this.m_gaussianRadius;

    public int[] GaussianSteps => this.m_gaussianSteps;

    public AnimationCurve TemporalFilteringCurve
    {
      get => this.m_temporalFilteringCurve;
      set => this.m_temporalFilteringCurve = value;
    }

    public bool SeparateFeaturesThreshold
    {
      get => this.m_separateFeaturesThreshold;
      set => this.m_separateFeaturesThreshold = value;
    }

    public float FeaturesThreshold
    {
      get => this.m_featuresThreshold;
      set => this.m_featuresThreshold = (double) value < 0.0 ? 0.0f : value;
    }

    public DebugToScreenEnum DebugToScreen
    {
      get => this.m_debugToScreen;
      set => this.m_debugToScreen = value;
    }

    public UpscaleQualityEnum UpscaleQuality
    {
      get => this.m_upscaleQuality;
      set => this.m_upscaleQuality = value;
    }

    public bool ShowDebugMessages
    {
      get => this.m_showDebugMessages;
      set => this.m_showDebugMessages = value;
    }

    public MainThresholdSizeEnum MainThresholdSize
    {
      get => this.m_mainThresholdSize;
      set => this.m_mainThresholdSize = value;
    }

    public RenderTexture TargetTexture
    {
      get => this.m_targetTexture;
      set => this.m_targetTexture = value;
    }

    public Texture MaskTexture
    {
      get => this.m_maskTexture;
      set => this.m_maskTexture = value;
    }

    public bool ApplyBokehFilter
    {
      get => this.m_bokehFilter.ApplyBokeh;
      set => this.m_bokehFilter.ApplyBokeh = value;
    }

    public bool ApplyLensFlare
    {
      get => this.m_lensFlare.ApplyLensFlare;
      set => this.m_lensFlare.ApplyLensFlare = value;
    }

    public bool ApplyLensGlare
    {
      get => this.m_anamorphicGlare.ApplyLensGlare;
      set => this.m_anamorphicGlare.ApplyLensGlare = value;
    }
  }
}
