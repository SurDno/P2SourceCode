...
public sealed class AmplifyGlare : IAmplifyItem {
	...
	[SerializeField] private GlareDefData[] m_customGlareDef;
	[SerializeField] private int m_customGlareDefIdx;
	[SerializeField] private int m_customGlareDefAmount;
	[SerializeField] private bool m_applyGlare = true;
	[SerializeField] private Color _overallTint = Color.white;
	[SerializeField] private Gradient m_cromaticAberrationGrad;
	[SerializeField] private int m_glareMaxPassCount = 4;
	...
	[SerializeField] private GlareLibType m_currentGlareType = GlareLibType.CheapLens;
	[SerializeField] private int m_currentGlareIdx;
	[SerializeField] private float m_perPassDisplacement = 4f;
	[SerializeField] private float m_intensity = 0.17f;
	[SerializeField] private float m_overallStreakScale = 1f;
	...
	public void Destroy() {
		for (var index = 0; index < m_starDefArr.Length; ++index)
			m_starDefArr[index].Destroy();
		m_glareDefArr = null;
		m_weigthsMat = null;
		m_offsetsMat = null;
		for (var index = 0; index < _rtBuffer.Length; ++index)
	...
	public void OnRenderFromCache(
		...
		for (var index = 0; index < m_amplifyGlareCache.TotalRT; ++index)
			_rtBuffer[index] = AmplifyUtils.GetTempRenderTarget(source.width, source.height);
		var index1 = 0;
		for (var index2 = 0; index2 < m_amplifyGlareCache.StarDef.StarlinesCount; ++index2) {
			for (var index3 = 0; index3 < m_amplifyGlareCache.CurrentPassCount; ++index3) {
				UpdateMatrixesForPass(material, m_amplifyGlareCache.Starlines[index2].Passes[index3].Offsets,
					m_amplifyGlareCache.Starlines[index2].Passes[index3].Weights, glareIntensity,
					cameraRotation * m_amplifyGlareCache.StarDef.CameraRotInfluence);
				if (index3 == 0)
		...
		for (var index4 = 0; index4 < m_amplifyGlareCache.StarDef.StarlinesCount; ++index4) {
			material.SetVector(AmplifyUtils.AnamorphicGlareWeightsStr[index4], m_amplifyGlareCache.AverageWeight);
			var index5 = (index4 + 1) * m_amplifyGlareCache.CurrentPassCount - 1;
			material.SetTexture(AmplifyUtils.AnamorphicRTS[index4], _rtBuffer[index5]);
		}

		var pass = 19 + m_amplifyGlareCache.StarDef.StarlinesCount - 1;
		dest.DiscardContents();
		Graphics.Blit(_rtBuffer[0], dest, material, pass);
		for (var index6 = 0; index6 < _rtBuffer.Length; ++index6) {
	...
	public void UpdateMatrixesForPass(
		...
		float glareIntensity,
		float rotation) {
		var num1 = Mathf.Cos(rotation);
		var num2 = Mathf.Sin(rotation);
	...
	public void OnRenderImage(
		...
		if (m_isDirty || m_currentWidth != source.width || m_currentHeight != source.height) {
			...
			var starDefData = flag ? glareDefData.CustomStarData : m_starDefArr[(int)glareDefData.StarType];
			m_amplifyGlareCache.StarDef = starDefData;
			var num1 = m_glareMaxPassCount < starDefData.PassCount ? m_glareMaxPassCount : starDefData.PassCount;
			m_amplifyGlareCache.CurrentPassCount = num1;
			var num2 = glareDefData.StarInclination + starDefData.Inclination;
			for (var index1 = 0; index1 < m_glareMaxPassCount; ++index1) {
				var t = (index1 + 1) / (float)m_glareMaxPassCount;
				for (var index2 = 0; index2 < 8; ++index2) {
					var b = _overallTint *
					        Color.Lerp(m_cromaticAberrationGrad.Evaluate(index2 / 7f), m_whiteReference, t);
					m_amplifyGlareCache.CromaticAberrationMat[index1, index2] =
						Color.Lerp(m_whiteReference, b, glareDefData.ChromaticAberration);
				}
			}
			...
			for (var index = 0; index < m_amplifyGlareCache.TotalRT; ++index)
				_rtBuffer[index] = AmplifyUtils.GetTempRenderTarget(source.width, source.height);
			var index3 = 0;
			for (var index4 = 0; index4 < starDefData.StarlinesCount; ++index4) {
				var starLineData = starDefData.StarLinesArr[index4];
				var f = num2 + starLineData.Inclination;
				var num3 = Mathf.Sin(f);
				var num4 = Mathf.Cos(f);
				...
				var num5 = (float)((m_aTanFoV + 0.10000000149011612) * 280.0 / (width + (double)height) *
				                   1.2000000476837158);
				for (var index5 = 0; index5 < num1; ++index5) {
					for (var index6 = 0; index6 < 8; ++index6) {
						var num6 = Mathf.Pow(starLineData.Attenuation, num5 * index6);
						m_amplifyGlareCache.Starlines[index4].Passes[index5].Weights[index6] =
							m_amplifyGlareCache.CromaticAberrationMat[num1 - 1 - index5, index6] * num6 *
							(index5 + 1f) * 0.5f;
						...
						if (Mathf.Abs(m_amplifyGlareCache.Starlines[index4].Passes[index5].Offsets[index6].x) >=
						    0.89999997615814209 ||
						    Mathf.Abs(m_amplifyGlareCache.Starlines[index4].Passes[index5].Offsets[index6].y) >=
						    0.89999997615814209) {
					...
					for (var index7 = 8; index7 < 16; ++index7) {
						m_amplifyGlareCache.Starlines[index4].Passes[index5].Offsets[index7] =
							-m_amplifyGlareCache.Starlines[index4].Passes[index5].Offsets[index7 - 8];
						m_amplifyGlareCache.Starlines[index4].Passes[index5].Weights[index7] =
							m_amplifyGlareCache.Starlines[index4].Passes[index5].Weights[index7 - 8];
					}

					UpdateMatrixesForPass(material, m_amplifyGlareCache.Starlines[index4].Passes[index5].Offsets,
						m_amplifyGlareCache.Starlines[index4].Passes[index5].Weights, m_intensity,
						starDefData.CameraRotInfluence * cameraRot);
					if (index5 == 0)
			...
			for (var index8 = 0; index8 < starDefData.StarlinesCount; ++index8) {
				material.SetVector(AmplifyUtils.AnamorphicGlareWeightsStr[index8], m_amplifyGlareCache.AverageWeight);
				var index9 = (index8 + 1) * num1 - 1;
				material.SetTexture(AmplifyUtils.AnamorphicRTS[index8], _rtBuffer[index9]);
			}

			var pass = 19 + starDefData.StarlinesCount - 1;
			dest.DiscardContents();
			Graphics.Blit(_rtBuffer[0], dest, material, pass);
			for (var index10 = 0; index10 < _rtBuffer.Length; ++index10) {
	...
	public int CustomGlareDefAmount {
		...
		set {
			...
			if (value == 0) {
				...
				var glareDefDataArray = new GlareDefData[value];
				for (var index = 0; index < value; ++index)
					glareDefDataArray[index] =
						index >= m_customGlareDefAmount ? new GlareDefData() : m_customGlareDef[index];
