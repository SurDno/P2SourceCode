...
public sealed class AmplifyBokeh : IAmplifyItem, ISerializationCallbackReceiver {
	...
	[SerializeField] private bool m_applyOnBloomSource;
	[SerializeField] private float m_bokehSampleRadius = 0.5f;
	[SerializeField] private Vector4 m_bokehCameraProperties = new(0.05f, 0.018f, 1.34f, 0.18f);
	[SerializeField] private float m_offsetRotation;
	...
	public void Destroy() {
		for (var index = 0; index < m_bokehOffsets.Count; ++index)
	...
	private Vector4[] CalculateBokehSamples(int sampleCount, float angle) {
		var bokehSamples = new Vector4[sampleCount];
		var f = (float)Math.PI / 180f * angle;
		var num = Screen.width / (float)Screen.height;
		var b = new Vector4(m_bokehSampleRadius * Mathf.Cos(f), m_bokehSampleRadius * Mathf.Sin(f));
		b.x /= num;
	...
	public void ApplyBokehFilter(RenderTexture source, Material material) {
		...
		for (var index1 = 0; index1 < m_bokehOffsets.Count; ++index1) {
			for (var index2 = 0; index2 < 8; ++index2)
				material.SetVector(AmplifyUtils.AnamorphicGlareWeightsStr[index2],
					m_bokehOffsets[index1].Offsets[index2]);
			Graphics.Blit(source, m_bokehOffsets[index1].BokehRenderTexture, material, 27);
		}

		for (var index = 0; index < m_bokehOffsets.Count - 1; ++index)
			material.SetTexture(AmplifyUtils.AnamorphicRTS[index], m_bokehOffsets[index].BokehRenderTexture);
		source.DiscardContents();
		Graphics.Blit(m_bokehOffsets[m_bokehOffsets.Count - 1].BokehRenderTexture, source, material,
			28 + (m_bokehOffsets.Count - 2));
		for (var index = 0; index < m_bokehOffsets.Count; ++index) {
