...
public class PlagueIntroRat : MonoBehaviour {
	...
	public void Init(PlagueIntroRatsManager manager) {
	...
	private void Update() {
		...
		if (Random.value < Time.deltaTime / 0.5 && NavMeshUtility.IsBrokenPath(agent)) {
			Debug.Log(ObjectInfoUtility.GetStream().Append("[Navigation]")
				.Append("  broken path detected, trying to reset: ").GetInfo(gameObject));
			...
			animator.SetFloat("Turn",
				Mathf.Clamp(Vector3.SignedAngle(transform.forward, agent.desiredVelocity, Vector3.up), -max, max) / max,
				0.5f, Time.deltaTime);
			...
			if (agent.pathStatus == NavMeshPathStatus.PathInvalid)
				...
				if (Random.value >= Time.deltaTime / 5.0 || agent.SamplePathPosition(-1, 3f, out var _))
					return;
