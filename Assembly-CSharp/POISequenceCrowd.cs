...
public class POISequenceCrowd : POISequence, IStub, ISerializeDataWrite, ISerializeDataRead {
	...
	protected override void FillPOICache() {
		...
		if (owner == null)
			...
			if (component != null && component.Crowd != null) {
				if (UseAllCrowdRegions.Value)
					FindPOIsInGameObject(((IEntityView)component.Point?.Region).GameObject, poiCache);
				else {
					var gameObject1 = component.Point?.GameObject;
					if (gameObject1 == null || !gameObject1.activeInHierarchy)
						return;
					var firstEngineObject = LocationItemUtility.GetFirstEngineObject(gameObject1.transform);
					if (firstEngineObject == null)
						return;
					var parentComponent =
						LocationItemUtility.FindParentComponent<LocationItemComponent>(firstEngineObject);
					if (parentComponent == null)
						return;
					var gameObject2 = ((IEntityView)parentComponent.Owner)?.GameObject;
					if (gameObject2 == null)
						return;
	...
	public new void DataWrite(IDataWriter writer) {
		...
		BehaviorTreeDataWriteUtility.WriteShared(writer, "UseAllCrowdRegions", UseAllCrowdRegions);
	}
	...
	public new void DataRead(IDataReader reader, Type type) {
		...
		UseAllCrowdRegions = BehaviorTreeDataReadUtility.ReadShared(reader, "UseAllCrowdRegions", UseAllCrowdRegions);
	}
}