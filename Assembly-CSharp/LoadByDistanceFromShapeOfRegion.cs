...
public class LoadByDistanceFromShapeOfRegion : BaseLoadByDistance, IEntityAttachable {
	...
	[Inspected] private bool on;
	[Inspected] private StaticModelComponent model;
	[Inspected] private RegionComponent region;
	[Inspected] private Bounds originalBounds;
	[Inspected] private Bounds loadBounds;
	[Inspected] private Bounds unloadBounds;
	[Inspected] private Vector3[] triangles;
	[Inspected] private Vector3 position;
	...
	private void Update() {
		...
		if (player == null || player.GetComponent<NavigationComponent>().WaitTeleport ||
		    Random.value > Time.deltaTime / 0.5)
			return;
	...
	private static float SquareDistanceToSegment(Vector2 a, Vector2 b, Vector2 p) {
		...
		var vector2_2 = p - a;
		var vector2_3 = p - b;
		var num = Vector2.Dot(vector2_2, vector2_1);
		if (num < 0.0)
			return Vector2.Dot(vector2_2, vector2_2);
		return num > (double)Vector2.Dot(vector2_1, vector2_1)
			? Vector2.Dot(vector2_3, vector2_3)
			: Vector2.Dot(vector2_2, vector2_2) - num * num / Vector2.Dot(vector2_1, vector2_1);
	}
	...
	private static bool IsPointInTriangle(Vector2 p, Vector2 t0, Vector2 t1, Vector2 t2) {
		...
		var lhs2 = t2 - t0;
		var lhs3 = p - t0;
		var rhs1 = new Vector2(-lhs1.y, lhs1.x);
		var rhs2 = new Vector2(-lhs2.y, lhs2.x);
		var num1 = Vector2.Dot(lhs3, rhs2) / Vector2.Dot(lhs1, rhs2);
		var num2 = Vector2.Dot(lhs3, rhs1) / Vector2.Dot(lhs2, rhs1);
	...
	private bool CheckInside(Vector3 position, Vector3[] triangles, float loadDistance) {
		...
		var num2 = loadDistance * loadDistance;
		var p = new Vector2(position.x, position.z);
		for (var index = 0; index < triangles.Length; index += 3) {
			var vector2_1 = new Vector2(triangles[index].x, triangles[index].z);
			var vector2_2 = new Vector2(triangles[index + 1].x, triangles[index + 1].z);
			var vector2_3 = new Vector2(triangles[index + 2].x, triangles[index + 2].z);
			var segment1 = SquareDistanceToSegment(vector2_1, vector2_2, p);
			if (segment1 < (double)num2)
				return true;
			if (segment1 < (double)num1)
				num1 = segment1;
			var segment2 = SquareDistanceToSegment(vector2_2, vector2_3, p);
			if (segment2 < (double)num2)
				return true;
			if (segment2 < (double)num1)
				num1 = segment2;
			var segment3 = SquareDistanceToSegment(vector2_3, vector2_1, p);
	...
	private void ComputeBounds() {
		...
		for (var index = 1; index < triangles.Length; ++index)
