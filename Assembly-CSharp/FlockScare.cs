...
public class FlockScare : MonoBehaviour {
	...
	private void IterateLandingSpots() {
		...
		var childCount = currentController.transform.childCount;
		if (ls <= childCount - 1)
			return;
		ls -= childCount;
	...
	private bool CheckDistanceToLandingSpot(LandingSpotController lc) {
		var child = lc.transform.GetChild(ls);
		return child.GetComponent<LandingSpot>().landingChild != null &&
		       (child.position - transform.position).sqrMagnitude < distanceToScare * (double)distanceToScare;
	}

	private void Invoker() {
		for (var index = 0; index < InvokeAmounts; ++index)
