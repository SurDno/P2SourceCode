...
public class FlockController : MonoBehaviour, IUpdatable {
	...
	[Space] [Header("Roaming Area")] public float _positionSphereWidth = 25f;
	...
	[Space] [Header("Size of the flock")] public int _childAmount = 250;
	...
	[Space] [Header("Behaviors and Appearance")]
	...
	[Space] [Header("Disable Pitch Rotation")]
	public bool _flatSoar;

	public bool _flatFly;
	[Space] [Header("Animations")] public string _soarAnimation = "Soar";
	...
	[NonSerialized] public List<FlockChild> childs = new();
	...
	private void AddChild(int amount) {
		for (var index = 0; index < amount; ++index) {
			var flockChild = Instantiate(_childPrefab);
	...
	private void UpdatePlayer() {
		...
		playerOffset = new Vector3(playerPosition.x - transform.position.x, playerYOffset,
			playerPosition.z - transform.position.z);
	}

	private void OnDrawGizmosSelected() {
		Gizmos.color = Color.blue;
		Gizmos.DrawWireCube(transform.position + targetPosition,
			new Vector3(_spawnSphereWidth * 2f, _spawnSphereHeight * 2f, _spawnSphereDepth * 2f));
		Gizmos.color = Color.cyan;
		Gizmos.DrawWireCube(transform.position + playerOffset,
			new Vector3(_positionSphereWidth * 2f, _positionSphereHeight * 2f, _positionSphereDepth * 2f));
	}
}