...
public class Clipper : ClipperBase {
	...
	protected override void Reset() {
		...
		m_ActiveEdges = null;
		m_SortedEdges = null;
		for (var localMinima = m_MinimaList; localMinima != null; localMinima = localMinima.Next)
	...
	private bool ExecuteInternal() {
		try {
			...
			for (var index = 0; index < m_PolyOuts.Count; ++index) {
				var polyOut = m_PolyOuts[index];
	...
	private void InsertLocalMinimaIntoAEL(long botY) {
		while (m_CurrentLM != null && m_CurrentLM.Y == botY) {
			var leftBound = m_CurrentLM.LeftBound;
			var rightBound = m_CurrentLM.RightBound;
			PopLocalMinima();
			OutPt outPt = null;
			if (leftBound == null) {
			...
			if (leftBound != null && rightBound != null) {
				if (outPt != null && IsHorizontal(rightBound) && m_GhostJoins.Count > 0 && rightBound.WindDelta != 0)
					for (var index = 0; index < m_GhostJoins.Count; ++index) {
						var ghostJoin = m_GhostJoins[index];
						if (HorzSegmentsOverlap(ghostJoin.OutPt1.Pt.X, ghostJoin.OffPt.X, rightBound.Bot.X,
							    rightBound.Top.X))
							AddJoin(ghostJoin.OutPt1, outPt, ghostJoin.OffPt);
					}

				if (leftBound.OutIdx >= 0 && leftBound.PrevInAEL != null &&
				    leftBound.PrevInAEL.Curr.X == leftBound.Bot.X && leftBound.PrevInAEL.OutIdx >= 0 &&
				    SlopesEqual(leftBound.PrevInAEL, leftBound, m_UseFullRange) && leftBound.WindDelta != 0 &&
				    leftBound.PrevInAEL.WindDelta != 0) {
					var Op2 = AddOutPt(leftBound.PrevInAEL, leftBound.Bot);
					AddJoin(outPt, Op2, leftBound.Top);
				}

				if (leftBound.NextInAEL != rightBound) {
					if (rightBound.OutIdx >= 0 && rightBound.PrevInAEL.OutIdx >= 0 &&
					    SlopesEqual(rightBound.PrevInAEL, rightBound, m_UseFullRange) && rightBound.WindDelta != 0 &&
					    rightBound.PrevInAEL.WindDelta != 0) {
						var Op2 = AddOutPt(rightBound.PrevInAEL, rightBound.Bot);
						AddJoin(outPt, Op2, rightBound.Top);
					}

					var nextInAel = leftBound.NextInAEL;
					if (nextInAel != null)
	...
	private bool IsEvenOddFillType(TEdge edge) {
		return edge.PolyTyp == PolyType.ptSubject
			? m_SubjFillType == PolyFillType.pftEvenOdd
			: m_ClipFillType == PolyFillType.pftEvenOdd;
	}
	...
	private void SetWindingCount(TEdge edge) {
		...
		if (prevInAel1 == null) {
			...
			if (edge.WindDelta == 0) {
				var flag = true;
				for (var prevInAel2 = prevInAel1.PrevInAEL; prevInAel2 != null; prevInAel2 = prevInAel2.PrevInAEL)
			...
			edge.WindCnt = prevInAel1.WindCnt * prevInAel1.WindDelta >= 0
				?
				edge.WindDelta != 0
					?
					prevInAel1.WindDelta * edge.WindDelta >= 0
						? prevInAel1.WindCnt + edge.WindDelta
						: prevInAel1.WindCnt
					: prevInAel1.WindCnt < 0
						? prevInAel1.WindCnt - 1
						: prevInAel1.WindCnt + 1
				: Math.Abs(prevInAel1.WindCnt) <= 1
					? edge.WindDelta == 0 ? 1 : edge.WindDelta
					: prevInAel1.WindDelta * edge.WindDelta >= 0
						? prevInAel1.WindCnt + edge.WindDelta
						: prevInAel1.WindCnt;
			edge.WindCnt2 = prevInAel1.WindCnt2;
	...
	private void SwapPositionsInAEL(TEdge edge1, TEdge edge2) {
		...
		if (edge1.NextInAEL == edge2) {
			var nextInAel = edge2.NextInAEL;
			if (nextInAel != null)
				nextInAel.PrevInAEL = edge1;
			var prevInAel = edge1.PrevInAEL;
			if (prevInAel != null)
			...
			var nextInAel = edge1.NextInAEL;
			if (nextInAel != null)
				nextInAel.PrevInAEL = edge2;
			var prevInAel = edge2.PrevInAEL;
			if (prevInAel != null)
	...
	private void SwapPositionsInSEL(TEdge edge1, TEdge edge2) {
		...
		if (edge1.NextInSEL == edge2) {
			var nextInSel = edge2.NextInSEL;
			if (nextInSel != null)
				nextInSel.PrevInSEL = edge1;
			var prevInSel = edge1.PrevInSEL;
			if (prevInSel != null)
			...
			var nextInSel = edge1.NextInSEL;
			if (nextInSel != null)
				nextInSel.PrevInSEL = edge2;
			var prevInSel = edge2.PrevInSEL;
			if (prevInSel != null)
	...
	private OutPt AddLocalMinPoly(TEdge e1, TEdge e2, IntPoint pt) {
		...
		if (tedge2 != null && tedge2.OutIdx >= 0 && TopX(tedge2, pt.Y) == TopX(tedge1, pt.Y) &&
		    SlopesEqual(tedge1, tedge2, m_UseFullRange) && tedge1.WindDelta != 0 && tedge2.WindDelta != 0) {
			var Op2 = AddOutPt(tedge2, pt);
	...
	private OutPt AddOutPt(TEdge e, IntPoint pt) {
		...
		var polyOut = m_PolyOuts[e.OutIdx];
		var pts = polyOut.Pts;
		if (flag && pt == pts.Pt)
			return pts;
		if (!flag && pt == pts.Prev.Pt)
			return pts.Prev;
		var outPt1 = new OutPt {
			Idx = polyOut.Idx,
			Pt = pt,
			Next = pts,
			Prev = pts.Prev
	...
	private void SetHoleState(TEdge e, OutRec outRec) {
		var flag = false;
		for (var prevInAel = e.PrevInAEL; prevInAel != null; prevInAel = prevInAel.PrevInAEL)
	...
	private bool FirstIsBottomPt(OutPt btmPt1, OutPt btmPt2) {
		var prev1 = btmPt1.Prev;
		while (prev1.Pt == btmPt1.Pt && prev1 != btmPt1)
			prev1 = prev1.Prev;
		var num1 = Math.Abs(GetDx(btmPt1.Pt, prev1.Pt));
		var next1 = btmPt1.Next;
		while (next1.Pt == btmPt1.Pt && next1 != btmPt1)
			next1 = next1.Next;
		var num2 = Math.Abs(GetDx(btmPt1.Pt, next1.Pt));
		var prev2 = btmPt2.Prev;
		while (prev2.Pt == btmPt2.Pt && prev2 != btmPt2)
			prev2 = prev2.Prev;
		var num3 = Math.Abs(GetDx(btmPt2.Pt, prev2.Pt));
		var next2 = btmPt2.Next;
		while (next2.Pt == btmPt2.Pt && next2 != btmPt2)
			next2 = next2.Next;
		var num4 = Math.Abs(GetDx(btmPt2.Pt, next2.Pt));
	...
	private OutRec GetLowermostRec(OutRec outRec1, OutRec outRec2) {
		...
		var bottomPt1 = outRec1.BottomPt;
		var bottomPt2 = outRec2.BottomPt;
		return bottomPt1.Pt.Y > bottomPt2.Pt.Y || (bottomPt1.Pt.Y >= bottomPt2.Pt.Y &&
		                                           (bottomPt1.Pt.X < bottomPt2.Pt.X ||
		                                            (bottomPt1.Pt.X <= bottomPt2.Pt.X && bottomPt1.Next != bottomPt1 &&
		                                             (bottomPt2.Next == bottomPt2 ||
		                                              FirstIsBottomPt(bottomPt1, bottomPt2)))))
			? outRec1
			: outRec2;
	}
	...
	private void AppendPolygon(TEdge e1, TEdge e2) {
		var polyOut1 = m_PolyOuts[e1.OutIdx];
		var polyOut2 = m_PolyOuts[e2.OutIdx];
		var outRec = !Param1RightOfParam2(polyOut1, polyOut2)
			? !Param1RightOfParam2(polyOut2, polyOut1) ? GetLowermostRec(polyOut1, polyOut2) : polyOut1
			: polyOut2;
		var pts1 = polyOut1.Pts;
		var prev1 = pts1.Prev;
		var pts2 = polyOut2.Pts;
		var prev2 = pts2.Prev;
		EdgeSide edgeSide;
		...
		var outIdx1 = e1.OutIdx;
		var outIdx2 = e2.OutIdx;
		e1.OutIdx = -1;
		e2.OutIdx = -1;
		for (var tedge = m_ActiveEdges; tedge != null; tedge = tedge.NextInAEL)
	...
	private void IntersectEdges(TEdge e1, TEdge e2, IntPoint pt) {
		var flag1 = e1.OutIdx >= 0;
		var flag2 = e2.OutIdx >= 0;
		if (e1.PolyTyp == e2.PolyTyp) {
			if (IsEvenOddFillType(e1)) {
				var windCnt = e1.WindCnt;
		...
		if (flag1 & flag2) {
			...
			if (e1.PolyTyp != e2.PolyTyp)
				...
				switch (m_ClipType) {
					case ClipType.ctIntersection:
						if (num3 > 0L && num4 > 0L) AddLocalMinPoly(e1, e2, pt);
						break;
					case ClipType.ctUnion:
						if (num3 <= 0L && num4 <= 0L) AddLocalMinPoly(e1, e2, pt);
						break;
					case ClipType.ctDifference:
						if ((e1.PolyTyp == PolyType.ptClip && num3 > 0L && num4 > 0L) ||
						    (e1.PolyTyp == PolyType.ptSubject && num3 <= 0L && num4 <= 0L)) AddLocalMinPoly(e1, e2, pt);
						break;
					case ClipType.ctXor:
						AddLocalMinPoly(e1, e2, pt);
						break;
	...
	private void DeleteFromAEL(TEdge e) {
		var prevInAel = e.PrevInAEL;
		var nextInAel = e.NextInAEL;
	...
	private void DeleteFromSEL(TEdge e) {
		var prevInSel = e.PrevInSEL;
		var nextInSel = e.NextInSEL;
	...
	private void ProcessHorizontals(bool isTopOfScanbeam) {
		for (var sortedEdges = m_SortedEdges; sortedEdges != null; sortedEdges = m_SortedEdges) {
	...
	private void ProcessHorizontal(TEdge horzEdge, bool isTopOfScanbeam) {
		...
		while (true) {
			var flag = horzEdge == e1;
			TEdge nextInAel;
			for (var tedge2 = GetNextInAEL(horzEdge, Dir);
			     tedge2 != null && (tedge2.Curr.X != horzEdge.Top.X || horzEdge.NextInLML == null ||
			                        tedge2.Dx >= horzEdge.NextInLML.Dx);
			     tedge2 = nextInAel) {
				nextInAel = GetNextInAEL(tedge2, Dir);
				if ((Dir == Direction.dLeftToRight && tedge2.Curr.X <= Right) ||
				    (Dir == Direction.dRightToLeft && tedge2.Curr.X >= Left)) {
					if ((tedge2 == tedge1) & flag) {
						if (horzEdge.OutIdx >= 0) {
							var outPt = AddOutPt(horzEdge, horzEdge.Top);
							for (var e2 = m_SortedEdges; e2 != null; e2 = e2.NextInSEL)
								if (e2.OutIdx >= 0 &&
								    HorzSegmentsOverlap(horzEdge.Bot.X, horzEdge.Top.X, e2.Bot.X, e2.Top.X))
					...
					if (Dir == Direction.dLeftToRight) {
						var pt = new IntPoint(tedge2.Curr.X, horzEdge.Curr.Y);
				...
				} else if ((Dir == Direction.dLeftToRight && tedge2.Curr.X >= Right) ||
				           (Dir == Direction.dRightToLeft && tedge2.Curr.X <= Left))
					break;
			}
		...
		if (horzEdge.NextInLML != null) {
			if (horzEdge.OutIdx >= 0) {
				...
				if (prevInAel != null && prevInAel.Curr.X == horzEdge.Bot.X && prevInAel.Curr.Y == horzEdge.Bot.Y &&
				    prevInAel.WindDelta != 0 && prevInAel.OutIdx >= 0 && prevInAel.Curr.Y > prevInAel.Top.Y &&
				    SlopesEqual(horzEdge, prevInAel, m_UseFullRange)) {
					var Op2 = AddOutPt(prevInAel, horzEdge.Bot);
					AddJoin(outPt, Op2, horzEdge.Top);
				} else {
					if (nextInAel == null || nextInAel.Curr.X != horzEdge.Bot.X || nextInAel.Curr.Y != horzEdge.Bot.Y ||
					    nextInAel.WindDelta == 0 || nextInAel.OutIdx < 0 || nextInAel.Curr.Y <= nextInAel.Top.Y ||
					    !SlopesEqual(horzEdge, nextInAel, m_UseFullRange))
						return;
					var Op2 = AddOutPt(nextInAel, horzEdge.Bot);
					AddJoin(outPt, Op2, horzEdge.Top);
				}
			} else
				UpdateEdgeIntoAEL(ref horzEdge);
	...
	private bool FixupIntersectionOrder() {
		...
		CopyAELToSEL();
		var count = m_IntersectList.Count;
		for (var index1 = 0; index1 < count; ++index1) {
			if (!EdgesAdjacent(m_IntersectList[index1])) {
				...
				if (index2 == count)
					return false;
				var intersect = m_IntersectList[index1];
	...
	private void ProcessIntersectList() {
		for (var index = 0; index < m_IntersectList.Count; ++index) {
			var intersect = m_IntersectList[index];
		...
		if (edge1.Dx == edge2.Dx) {
			...
			if (edge1.Delta.X == 0L) {
				...
				if (IsHorizontal(edge2))
					...
					var num = edge2.Bot.Y - edge2.Bot.X / edge2.Dx;
				...
				if (IsHorizontal(edge1))
					...
					var num = edge1.Bot.Y - edge1.Bot.X / edge1.Dx;
					ip.Y = Round(ip.X / edge1.Dx + num);
				}
			} else {
				var num1 = edge1.Bot.X - edge1.Bot.Y * edge1.Dx;
				var num2 = edge2.Bot.X - edge2.Bot.Y * edge2.Dx;
				var num3 = (num2 - num1) / (edge1.Dx - edge2.Dx);
				ip.Y = Round(num3);
				ip.X = Math.Abs(edge1.Dx) >= Math.Abs(edge2.Dx)
					? Round(edge2.Dx * num3 + num2)
					: Round(edge1.Dx * num3 + num1);
			}
	...
	private void ProcessEdgesAtTopOfScanbeam(long topY) {
		var e1 = m_ActiveEdges;
		while (e1 != null) {
			var flag = IsMaxima(e1, topY);
			if (flag) {
				var maximaPair = GetMaximaPair(e1);
				flag = maximaPair == null || !IsHorizontal(maximaPair);
			}

			if (flag) {
				var prevInAel = e1.PrevInAEL;
				DoMaxima(e1);
				...
				if (StrictlySimple) {
					var prevInAel = e1.PrevInAEL;
					if (e1.OutIdx >= 0 && e1.WindDelta != 0 && prevInAel != null && prevInAel.OutIdx >= 0 &&
					    prevInAel.Curr.X == e1.Curr.X && prevInAel.WindDelta != 0) {
						var intPoint = new IntPoint(e1.Curr);
		...
		for (var e2 = m_ActiveEdges; e2 != null; e2 = e2.NextInAEL)
			if (IsIntermediate(e2, topY)) {
				OutPt Op1 = null;
				if (e2.OutIdx >= 0)
					Op1 = AddOutPt(e2, e2.Top);
				UpdateEdgeIntoAEL(ref e2);
				var prevInAel = e2.PrevInAEL;
				var nextInAel = e2.NextInAEL;
				if (prevInAel != null && prevInAel.Curr.X == e2.Bot.X && prevInAel.Curr.Y == e2.Bot.Y && Op1 != null &&
				    prevInAel.OutIdx >= 0 && prevInAel.Curr.Y > prevInAel.Top.Y &&
				    SlopesEqual(e2, prevInAel, m_UseFullRange) && e2.WindDelta != 0 && prevInAel.WindDelta != 0) {
					var Op2 = AddOutPt(prevInAel, e2.Bot);
					AddJoin(Op1, Op2, e2.Top);
				} else if (nextInAel != null && nextInAel.Curr.X == e2.Bot.X && nextInAel.Curr.Y == e2.Bot.Y &&
				           Op1 != null && nextInAel.OutIdx >= 0 && nextInAel.Curr.Y > nextInAel.Top.Y &&
				           SlopesEqual(e2, nextInAel, m_UseFullRange) && e2.WindDelta != 0 &&
				           nextInAel.WindDelta != 0) {
					var Op2 = AddOutPt(nextInAel, e2.Bot);
					AddJoin(Op1, Op2, e2.Top);
				}
			}
	}
	...
	private void DoMaxima(TEdge e) {
		...
		if (maximaPair == null) {
			...
			for (var nextInAel = e.NextInAEL; nextInAel != null && nextInAel != maximaPair; nextInAel = e.NextInAEL) {
	...
	private void BuildResult(List<List<IntPoint>> polyg) {
		...
		for (var index1 = 0; index1 < m_PolyOuts.Count; ++index1) {
			var polyOut = m_PolyOuts[index1];
			if (polyOut.Pts != null) {
				var prev = polyOut.Pts.Prev;
				var capacity = PointCount(prev);
				if (capacity >= 2) {
					var intPointList = new List<IntPoint>(capacity);
	...
	private void BuildResult2(PolyTree polytree) {
		...
		for (var index1 = 0; index1 < m_PolyOuts.Count; ++index1) {
			var polyOut = m_PolyOuts[index1];
			var num = PointCount(polyOut.Pts);
	...
	private void FixupOutPolygon(OutRec outRec) {
		OutPt outPt1 = null;
		outRec.BottomPt = null;
		var outPt2 = outRec.Pts;
		while (outPt2.Prev != outPt2 && outPt2.Prev != outPt2.Next)
			if (outPt2.Pt == outPt2.Next.Pt || outPt2.Pt == outPt2.Prev.Pt ||
			    (SlopesEqual(outPt2.Prev.Pt, outPt2.Pt, outPt2.Next.Pt, m_UseFullRange) && (!PreserveCollinear ||
				    !Pt2IsBetweenPt1AndPt3(outPt2.Prev.Pt, outPt2.Pt, outPt2.Next.Pt)))) {
				outPt1 = null;
	...
	private bool JoinHorz(
		...
		IntPoint Pt,
		bool DiscardLeft) {
		var direction1 = op1.Pt.X > op1b.Pt.X ? Direction.dRightToLeft : Direction.dLeftToRight;
		var direction2 = op2.Pt.X > op2b.Pt.X ? Direction.dRightToLeft : Direction.dLeftToRight;
	...
	private bool JoinPoints(Join j, OutRec outRec1, OutRec outRec2) {
		var outPt1 = j.OutPt1;
		var outPt2 = j.OutPt2;
		var flag1 = j.OutPt1.Pt.Y == j.OffPt.Y;
		if (flag1 && j.OffPt == j.OutPt1.Pt && j.OffPt == j.OutPt2.Pt) {
			if (outRec1 != outRec2)
				return false;
			var next1 = j.OutPt1.Next;
			while (next1 != outPt1 && next1.Pt == j.OffPt)
				next1 = next1.Next;
			var flag2 = next1.Pt.Y > j.OffPt.Y;
			var next2 = j.OutPt2.Next;
			while (next2 != outPt2 && next2.Pt == j.OffPt)
				next2 = next2.Next;
			var flag3 = next2.Pt.Y > j.OffPt.Y;
			if (flag2 == flag3)
				return false;
			if (flag2) {
				var outPt3 = DupOutPt(outPt1, false);
				var outPt4 = DupOutPt(outPt2, true);
				outPt1.Prev = outPt2;
				outPt2.Next = outPt1;
				outPt3.Next = outPt4;
				outPt4.Prev = outPt3;
			...
			var outPt5 = DupOutPt(outPt1, true);
			var outPt6 = DupOutPt(outPt2, false);
			outPt1.Next = outPt2;
			outPt2.Prev = outPt1;
			outPt5.Prev = outPt6;
			outPt6.Next = outPt5;
		...
		if (flag1) {
			...
			long Left;
			long Right;
			if (op2b.Next == outPt2 || op2b.Next == outPt1 ||
			    !GetOverlap(outPt1.Pt.X, op1b.Pt.X, outPt2.Pt.X, op2b.Pt.X, out Left, out Right))
				return false;
			IntPoint pt;
			bool DiscardLeft;
		...
		var outPt7 = outPt1.Next;
		while (outPt7.Pt == outPt1.Pt && outPt7 != outPt1)
			outPt7 = outPt7.Next;
		var flag4 = outPt7.Pt.Y > outPt1.Pt.Y || !SlopesEqual(outPt1.Pt, outPt7.Pt, j.OffPt, m_UseFullRange);
		if (flag4) {
			outPt7 = outPt1.Prev;
			while (outPt7.Pt == outPt1.Pt && outPt7 != outPt1)
				outPt7 = outPt7.Prev;
		...
		var outPt8 = outPt2.Next;
		while (outPt8.Pt == outPt2.Pt && outPt8 != outPt2)
			outPt8 = outPt8.Next;
		var flag5 = outPt8.Pt.Y > outPt2.Pt.Y || !SlopesEqual(outPt2.Pt, outPt8.Pt, j.OffPt, m_UseFullRange);
		if (flag5) {
			outPt8 = outPt2.Prev;
			while (outPt8.Pt == outPt2.Pt && outPt8 != outPt2)
				outPt8 = outPt8.Prev;
		...
		if (outPt7 == outPt1 || outPt8 == outPt2 || outPt7 == outPt8 || (outRec1 == outRec2 && flag4 == flag5))
			return false;
		if (flag4) {
			var outPt9 = DupOutPt(outPt1, false);
			var outPt10 = DupOutPt(outPt2, true);
			outPt1.Prev = outPt2;
			outPt2.Next = outPt1;
			outPt9.Next = outPt10;
			outPt10.Prev = outPt9;
		...
		var outPt11 = DupOutPt(outPt1, true);
		var outPt12 = DupOutPt(outPt2, false);
		outPt1.Next = outPt2;
		outPt2.Prev = outPt1;
	...
	public static int PointInPolygon(IntPoint pt, List<IntPoint> path) {
		...
		if (count < 3)
			return 0;
		var intPoint1 = path[0];
		for (var index = 1; index <= count; ++index) {
			var intPoint2 = index == count ? path[0] : path[index];
			if (intPoint2.Y == pt.Y &&
			    (intPoint2.X == pt.X || (intPoint1.Y == pt.Y && intPoint2.X > pt.X == intPoint1.X < pt.X)))
				return -1;
			if (intPoint1.Y < pt.Y != intPoint2.Y < pt.Y) {
				if (intPoint1.X >= pt.X) {
					if (intPoint2.X > pt.X)
						...
						var num2 = (intPoint1.X - pt.X) * (double)(intPoint2.Y - pt.Y) -
						           (intPoint2.X - pt.X) * (double)(intPoint1.Y - pt.Y);
						if (num2 == 0.0)
							return -1;
					...
					var num3 = (intPoint1.X - pt.X) * (double)(intPoint2.Y - pt.Y) -
					           (intPoint2.X - pt.X) * (double)(intPoint1.Y - pt.Y);
					if (num3 == 0.0)
						return -1;
			...
			if (num3 < y1 != y2 < y1) {
				if (num2 >= x1) {
					if (x2 > x1)
						...
						var num4 = (num2 - x1) * (double)(y2 - y1) - (x2 - x1) * (double)(num3 - y1);
						if (num4 == 0.0)
							return -1;
						if (num4 > 0.0 == y2 > num3)
							num1 = 1 - num1;
					...
					var num5 = (num2 - x1) * (double)(y2 - y1) - (x2 - x1) * (double)(num3 - y1);
					if (num5 == 0.0)
						return -1;
					if (num5 > 0.0 == y2 > num3)
						num1 = 1 - num1;
	...
	private static bool Poly2ContainsPoly1(OutPt outPt1, OutPt outPt2) {
		var outPt = outPt1;
		do {
			var num = PointInPolygon(outPt.Pt, outPt2);
			if (num >= 0)
				return num > 0;
			outPt = outPt.Next;
		} while (outPt != outPt1);
	...
	private void FixupFirstLefts1(OutRec OldOutRec, OutRec NewOutRec) {
		for (var index = 0; index < m_PolyOuts.Count; ++index) {
			var polyOut = m_PolyOuts[index];
			if (polyOut.Pts != null && polyOut.FirstLeft != null && ParseFirstLeft(polyOut.FirstLeft) == OldOutRec &&
			    Poly2ContainsPoly1(polyOut.Pts, NewOutRec.Pts))
				polyOut.FirstLeft = NewOutRec;
		}
	}

	private void FixupFirstLefts2(OutRec OldOutRec, OutRec NewOutRec) {
		foreach (var polyOut in m_PolyOuts)
	...
	private void JoinCommonEdges() {
		for (var index1 = 0; index1 < m_Joins.Count; ++index1) {
			var join = m_Joins[index1];
			var outRec1 = GetOutRec(join.OutPt1.Idx);
			var outRec2 = GetOutRec(join.OutPt2.Idx);
			if (outRec1.Pts != null && outRec2.Pts != null) {
				var outRec3 = outRec1 != outRec2
					? !Param1RightOfParam2(outRec1, outRec2)
						? !Param1RightOfParam2(outRec2, outRec1) ? GetLowermostRec(outRec1, outRec2) : outRec1
						: outRec2
					: outRec1;
				if (JoinPoints(join, outRec1, outRec2)) {
					if (outRec1 == outRec2) {
						...
						var outRec4 = CreateOutRec();
						outRec4.Pts = join.OutPt2;
						UpdateOutPtIdxs(outRec4);
						if (m_UsingPolyTree)
							for (var index2 = 0; index2 < m_PolyOuts.Count - 1; ++index2) {
								var polyOut = m_PolyOuts[index2];
								if (polyOut.Pts != null && ParseFirstLeft(polyOut.FirstLeft) == outRec1 &&
								    polyOut.IsHole != outRec1.IsHole && Poly2ContainsPoly1(polyOut.Pts, join.OutPt2))
	...
	private void DoSimplePolygons() {
		...
		while (num < m_PolyOuts.Count) {
			var polyOut = m_PolyOuts[num++];
			var outPt1 = polyOut.Pts;
			if (outPt1 != null && !polyOut.IsOpen)
				do {
					for (var outPt2 = outPt1.Next; outPt2 != polyOut.Pts; outPt2 = outPt2.Next)
						if (outPt1.Pt == outPt2.Pt && outPt2.Next != outPt1 && outPt2.Prev != outPt1) {
							var prev1 = outPt1.Prev;
							var prev2 = outPt2.Prev;
							outPt1.Prev = prev2;
							prev2.Next = outPt1;
							outPt2.Prev = prev1;
							prev1.Next = outPt2;
	...
	private static double DistanceSqrd(IntPoint pt1, IntPoint pt2) {
		var num1 = pt1.X - (double)pt2.X;
		var num2 = pt1.Y - (double)pt2.Y;
	...
	private static double DistanceFromLineSqrd(IntPoint pt, IntPoint ln1, IntPoint ln2) {
		...
		var num3 = num1 * ln1.X + num2 * ln1.Y;
		var num4 = num1 * pt.X + num2 * pt.Y - num3;
	...
	private static bool SlopesNearCollinear(
		...
		if (Math.Abs(pt1.X - pt2.X) > Math.Abs(pt1.Y - pt2.Y)) {
			...
			return pt2.X > pt1.X == pt2.X < pt3.X
				? DistanceFromLineSqrd(pt2, pt1, pt3) < distSqrd
				: DistanceFromLineSqrd(pt3, pt1, pt2) < distSqrd;
		}
	...
	public static List<IntPoint> CleanPolygon(List<IntPoint> path, double distance = 1.415) {
		var capacity = path.Count;
		if (capacity == 0)
			return new List<IntPoint>();
		var outPtArray = new OutPt[capacity];
	...
	public static List<List<IntPoint>> CleanPolygons(List<List<IntPoint>> polys, double distance = 1.415) {
		var intPointListList = new List<List<IntPoint>>(polys.Count);
		for (var index = 0; index < polys.Count; ++index)
	...
	internal static List<List<IntPoint>> Minkowski(
		...
		bool IsSum,
		bool IsClosed) {
		var num = IsClosed ? 1 : 0;
		var count1 = pattern.Count;
		var count2 = path.Count;
		var intPointListList1 = new List<List<IntPoint>>(count2);
		if (IsSum)
			for (var index = 0; index < count2; ++index) {
				var intPointList = new List<IntPoint>(count1);
				foreach (var intPoint in pattern)
		...
		var intPointListList2 = new List<List<IntPoint>>((count2 + num) * (count1 + 1));
		for (var index1 = 0; index1 < count2 - 1 + num; ++index1) {
			for (var index2 = 0; index2 < count1; ++index2) {
				var poly = new List<IntPoint>(4);
	...
	public static List<List<IntPoint>> MinkowskiSum(
		...
		List<IntPoint> path,
		bool pathIsClosed) {
		var intPointListList = Minkowski(pattern, path, true, pathIsClosed);
		var clipper = new Clipper();
	...
	private static List<IntPoint> TranslatePath(List<IntPoint> path, IntPoint delta) {
		var intPointList = new List<IntPoint>(path.Count);
	...
	public static List<List<IntPoint>> MinkowskiSum(
		...
		for (var index = 0; index < paths.Count; ++index) {
			var ppg = Minkowski(pattern, paths[index], true, pathIsClosed);
			clipper.AddPaths(ppg, PolyType.ptSubject, true);
			if (pathIsClosed) {
				var pg = TranslatePath(paths[index], pattern[0]);
	...
	public static List<List<IntPoint>> MinkowskiDiff(List<IntPoint> poly1, List<IntPoint> poly2) {
		var intPointListList = Minkowski(poly1, poly2, false, true);
		var clipper = new Clipper();
		clipper.AddPaths(intPointListList, PolyType.ptSubject, true);
		clipper.Execute(ClipType.ctUnion, intPointListList, PolyFillType.pftNonZero, PolyFillType.pftNonZero);
		return intPointListList;
	...
	public static List<List<IntPoint>> OpenPathsFromPolyTree(PolyTree polytree) {
		var intPointListList = new List<List<IntPoint>>();
		intPointListList.Capacity = polytree.ChildCount;
		for (var index = 0; index < polytree.ChildCount; ++index)
