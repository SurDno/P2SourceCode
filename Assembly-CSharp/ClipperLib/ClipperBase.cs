...
public class ClipperBase {
	...
	internal bool PointOnLineSegment(
		...
		bool UseFullRange) {
		return UseFullRange
			? (pt.X == linePt1.X && pt.Y == linePt1.Y) || (pt.X == linePt2.X && pt.Y == linePt2.Y) ||
	...
	internal static bool SlopesEqual(TEdge e1, TEdge e2, bool UseFullRange) {
		return UseFullRange
			? Int128.Int128Mul(e1.Delta.Y, e2.Delta.X) == Int128.Int128Mul(e1.Delta.X, e2.Delta.Y)
	...
	protected static bool SlopesEqual(IntPoint pt1, IntPoint pt2, IntPoint pt3, bool UseFullRange) {
		return UseFullRange
			? Int128.Int128Mul(pt1.Y - pt2.Y, pt2.X - pt3.X) == Int128.Int128Mul(pt1.X - pt2.X, pt2.Y - pt3.Y)
			: (pt1.Y - pt2.Y) * (pt2.X - pt3.X) - (pt1.X - pt2.X) * (pt2.Y - pt3.Y) == 0L;
	}
	...
	protected static bool SlopesEqual(
		...
		bool UseFullRange) {
		return UseFullRange
			? Int128.Int128Mul(pt1.Y - pt2.Y, pt3.X - pt4.X) == Int128.Int128Mul(pt1.X - pt2.X, pt3.Y - pt4.Y)
			: (pt1.Y - pt2.Y) * (pt3.X - pt4.X) - (pt1.X - pt2.X) * (pt3.Y - pt4.Y) == 0L;
	}
	...
	public virtual void Clear() {
		...
		for (var index1 = 0; index1 < m_edges.Count; ++index1) {
			for (var index2 = 0; index2 < m_edges[index1].Count; ++index2)
	...
	private void RangeTest(IntPoint Pt, ref bool useFullRange) {
		if (useFullRange) {
			if (Pt.X > 4611686018427387903L || Pt.Y > 4611686018427387903L || -Pt.X > 4611686018427387903L ||
			    -Pt.Y > 4611686018427387903L)
	...
	private TEdge ProcessBound(TEdge E, bool LeftBoundIsForward) {
		...
		if (E.Dx == -3.4E+38) {
			var tedge3 = !LeftBoundIsForward ? E.Next : E.Prev;
			if (tedge3.OutIdx != -2) {
	...
	public bool AddPath(List<IntPoint> pg, PolyType polyType, bool Closed) {
		...
		if ((Closed && index1 < 2) || (!Closed && index1 < 1))
			return false;
		var tedgeList = new List<TEdge>(index1 + 1);
		...
		if (tedge2.Prev.Bot == tedge2.Prev.Top)
			tedge2 = tedge2.Next;
		while (true) {
			var nextLocMin = FindNextLocMin(tedge2);
			if (nextLocMin != tedge3) {
				...
				var E = ProcessBound(newLm.RightBound, !LeftBoundIsForward);
				if (E.OutIdx == -2)
	...
	public bool AddPaths(List<List<IntPoint>> ppg, PolyType polyType, bool closed) {
		var flag = false;
		for (var index = 0; index < ppg.Count; ++index)
	...
	public static IntRect GetBounds(List<List<IntPoint>> paths) {
		...
		for (; index1 < count; ++index1) {
			for (var index2 = 0; index2 < paths[index1].Count; ++index2) {
