...
namespace ClipperLib
{
  public class ClipperOffset
  {
    ...
    public void AddPath(List<IntPoint> path, JoinType joinType, EndType endType)
    {
      ...
      for (int index3 = 1; index3 <= index1; ++index3)
      {
        if (Child.m_polygon[index2] != path[index3])
        {
          ...
          if (path[index3].Y > Child.m_polygon[num].Y || path[index3].Y == Child.m_polygon[num].Y && path[index3].X < Child.m_polygon[num].X)
            num = index2;
        }
      }
      ...
      else
      {
        ...
        if (Child.m_polygon[num].Y > intPoint.Y || Child.m_polygon[num].Y == intPoint.Y && Child.m_polygon[num].X < intPoint.X)
          m_lowest = new IntPoint(m_polyNodes.ChildCount - 1, num);
      }
    }
    ...
    private void FixOrientations()
    {
      ...
      {
        for (int index = 0; index < m_polyNodes.ChildCount; ++index)
        {
          PolyNode child = m_polyNodes.Childs[index];
    ...
    private void DoOffset(double delta)
    {
      ...
      else
      {
        ...
        double num1 = Math.PI / Math.Acos(1.0 - (ArcTolerance > 0.0 ? (ArcTolerance <= Math.Abs(delta) * 0.25 ? ArcTolerance : Math.Abs(delta) * 0.25) : 0.25) / Math.Abs(delta));
        ...
        for (int index1 = 0; index1 < m_polyNodes.ChildCount; ++index1)
        {
          PolyNode child = m_polyNodes.Childs[index1];
          ...
          if (count != 0 && (delta > 0.0 || count >= 3 && child.m_endtype == 0))
          {
            ...
            if (count == 1)
            {
              if (child.m_jointype == JoinType.jtRound)
              {
                ...
                {
                  m_destPoly.Add(new IntPoint(Round(m_srcPoly[0].X + num2 * delta), Round(m_srcPoly[0].Y + num3 * delta)));
                  double num4 = num2;
              ...
              else
              {
                ...
                {
                  m_destPoly.Add(new IntPoint(Round(m_srcPoly[0].X + num5 * delta), Round(m_srcPoly[0].Y + num6 * delta)));
                  if (num5 < 0.0)
                    num5 = 1.0;
                  else if (num6 < 0.0)
                    num6 = 1.0;
            ...
            else
            {
              ...
              else
              {
                ...
                {
                  int index6 = count - 1;
                  intPoint = new IntPoint(Round(m_srcPoly[index6].X + m_normals[index6].X * delta), Round(m_srcPoly[index6].Y + m_normals[index6].Y * delta));
                ...
                int k5 = count - 1;
                for (int j = k5 - 1; j > 0; --j)
                  OffsetPoint(j, ref k5, child.m_jointype);
                if (child.m_endtype == EndType.etOpenButt)
                {
                  intPoint = new IntPoint(Round(m_srcPoly[0].X - m_normals[0].X * delta), Round(m_srcPoly[0].Y - m_normals[0].Y * delta));
                  m_destPoly.Add(intPoint);
                  intPoint = new IntPoint(Round(m_srcPoly[0].X + m_normals[0].X * delta), Round(m_srcPoly[0].Y + m_normals[0].Y * delta));
                  m_destPoly.Add(intPoint);
                }
      ...
      else
      {
        switch (jointype)
        {
          ...
          case JoinType.jtMiter:
            double r = 1.0 + (m_normals[j].X * m_normals[k].X + m_normals[j].Y * m_normals[k].Y);
            if (r >= m_miterLim)
            {
              DoMiter(j, k, r);
              break;
    ...
    internal void DoSquare(int j, int k)
    {
      ...
      m_destPoly.Add(new IntPoint(Round(m_srcPoly[j].X + m_delta * (m_normals[k].X - m_normals[k].Y * num)), Round(m_srcPoly[j].Y + m_delta * (m_normals[k].Y + m_normals[k].X * num))));
      m_destPoly.Add(new IntPoint(Round(m_srcPoly[j].X + m_delta * (m_normals[j].X + m_normals[j].Y * num)), Round(m_srcPoly[j].Y + m_delta * (m_normals[j].Y - m_normals[j].X * num))));
    }

    internal void DoMiter(int j, int k, double r)
    {
      double num = m_delta / r;
      m_destPoly.Add(new IntPoint(Round(m_srcPoly[j].X + (m_normals[k].X + m_normals[j].X) * num), Round(m_srcPoly[j].Y + (m_normals[k].Y + m_normals[j].Y) * num)));
    }

    internal void DoRound(int j, int k)
    {
      int num1 = Math.Max((int) Round(m_StepsPerRad * Math.Abs(Math.Atan2(m_sinA, m_normals[k].X * m_normals[j].X + m_normals[k].Y * m_normals[j].Y))), 1);
      double num2 = m_normals[k].X;
      double num3 = m_normals[k].Y;
      for (int index = 0; index < num1; ++index)
      {
        m_destPoly.Add(new IntPoint(Round(m_srcPoly[j].X + num2 * m_delta), Round(m_srcPoly[j].Y + num3 * m_delta)));
        double num4 = num2;
        num2 = num2 * m_cos - m_sin * num3;
        num3 = num4 * m_sin + num3 * m_cos;
      }
