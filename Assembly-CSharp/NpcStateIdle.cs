...
public class NpcStateIdle : INpcState {
	...
	public void Activate(float primaryIdleProbability, bool makeObstacle = false) {
		...
		var indoor = false;
		var component1 = npcState.Owner?.GetComponent<LocationItemComponent>();
		if (component1 == null)
		...
		if (npcState.Owner != null) {
			var component2 = npcState.Owner.GetComponent<ParametersComponent>();
			if (component2 != null) {
				var byName = component2.GetByName<bool>(ParameterNameEnum.SayReplicsInIdle);
				if (byName != null) {
	...
	public void OnAnimatorMove() {
		...
		if (agent.isActiveAndEnabled) {
			var monoHeapSizeLong1 = Profiler.GetMonoHeapSizeLong();
			if (agent.isOnNavMesh) {
				agent.nextPosition = vector3;
				GameObject.transform.position = agent.nextPosition;
			} else {
				var monoHeapSizeLong2 = Profiler.GetMonoHeapSizeLong();
				NavMeshHit hit;
				if (NavMesh.FindClosestEdge(GameObject.transform.position, out hit, agent.areaMask)) {
					...
					var monoHeapSizeLong3 = Profiler.GetMonoHeapSizeLong();
					if (NavMesh.Raycast(GameObject.transform.position + new Vector3(0.0f, 20f, 0.0f),
						    GameObject.transform.position + new Vector3(0.0f, -20f, 0.0f), out hit, agent.areaMask)) {
						agent.Warp(hit.position);
						GameObject.transform.position = agent.nextPosition;
						Debug.Log("Agent was teleported (Raycast)", agent.gameObject);
					} else {
						var monoHeapSizeLong4 = Profiler.GetMonoHeapSizeLong();
						Debug.LogWarning(
							ObjectInfoUtility.GetStream()
								.Append(
									"Agent was not able to teleport to closest edge and raycast also failed, memory : ")
								.Append(monoHeapSizeLong1).Append(" , ").Append(monoHeapSizeLong2).Append(" , ")
								.Append(monoHeapSizeLong3).Append(" , ").Append(monoHeapSizeLong4), agent.gameObject);
					}
				}
			}
		} else
