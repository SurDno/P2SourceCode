...
public class JerboaInstanceDescription {
	...
	public void UpdateAnimation() {
		...
		var totalFrame = Source.aniInfo[aniIndex].totalFrame;
		switch (wrapMode) {
			case WrapMode.Default:
			case WrapMode.Once:
				if (curFrame < 0.0 || curFrame > totalFrame - 1.0) Pause();
				break;
			case WrapMode.Loop:
				if (curFrame < 0.0) {
				...
				if (curFrame > (double)(totalFrame - 1)) curFrame -= totalFrame - 1;
				break;
	...
	public bool IsReady() {
		return Source.aniInfo != null;
	}

	public AnimationInfo GetCurrentAnimationInfoUnsafe() {
		return Source.aniInfo[aniIndex];
	}

	public AnimationInfo GetCurrentAnimationInfo() {
		return Source.aniInfo != null && 0 <= aniIndex && aniIndex < Source.aniInfo.Length
			? Source.aniInfo[aniIndex]
			: null;
	}

	public AnimationInfo GetPreAnimationInfo() {
		return Source.aniInfo != null && 0 <= preAniIndex && preAniIndex < Source.aniInfo.Length
			? Source.aniInfo[preAniIndex]
			: null;
	}
	...
	private void UpdateAnimationEvent() {
		var currentAnimationInfo = GetCurrentAnimationInfo();
		if (currentAnimationInfo == null || currentAnimationInfo.eventList.Count == 0)
			return;
		if (aniEvent == null) {
			var num = curFrame / currentAnimationInfo.fps;
			for (var eventIndex = this.eventIndex >= 0 ? this.eventIndex : 0;
			     eventIndex < currentAnimationInfo.eventList.Count;
			     ++eventIndex)
