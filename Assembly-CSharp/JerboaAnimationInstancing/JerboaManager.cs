...
public class JerboaManager : MonoBehaviour {
	...
	[Tooltip("Влияет линейно на максимальное количество тушканчиков")] [Range(0.0f, 1f)] [SerializeField]
	private float weight = 1f;

	[SerializeField] private JerboaColorEnum colorEnum = JerboaColorEnum.Default;
	private Color color = Color.gray;
	[SerializeField] private JerboaColor[] jerboaColors;
	[Header("Settings")] [SerializeField] private GameObject prefab;
	private GameObject prefabInstance;
	[SerializeField] private GameObject groupPrefab;
	[SerializeField] private int groupsCount = 1;
	[SerializeField] private int jerboaCount = 1000;
	[SerializeField] private float pathSampleRadius = 12f;
	[SerializeField] private float teleportRadius = 20f;
	[SerializeField] private float groupSpreadAngle = 90f;
	[SerializeField] private LayerMask raycastLayerMask;
	[SerializeField] private int groupWalkNavigationMask = -1;
	[SerializeField] private int groupRayCount = 10;
	[SerializeField] private float groupRayDistance = 10f;
	[SerializeField] private float jerboaRespawnLimitPerSecond = 50f;
	private float smoothedJerboaWeight;
	[Header("Audio")] [SerializeField] private AudioMixerGroup audioMixerGroup;
	[SerializeField] private AudioClip[] jerboaAudioClips = new AudioClip[3];

	[Header("Animations")] [SerializeField]
	private TextAsset jerboaAnimationsTexture;

	private List<JerboaGroupBarycentric> groups = new();
	...
	public JerboaColorEnum ColorEnum {
		...
		set {
			...
			if (!(prefabInstance != null) || !(prefabInstance.GetComponentInChildren<SkinnedMeshRenderer>() != null))
				return;
			foreach (var jerboaColor in jerboaColors)
	...
	public AudioClip GetJerboaAudioclip(int level) {
		return jerboaAudioClips[level];
	}
	...
	private void OnEnable() {
		...
		var component = prefabInstance.GetComponent<JerboaInstance>();
		component.InitializeAnimation(jerboaInstancingManager, jerboaAnimationManager, jerboaAnimationsTexture);
		component.transform.parent = transform;
		for (var index = 0; index < jerboaCount; ++index) {
			var instance = new JerboaInstanceDescription();
	...
	private void CreateGroups() {
		if (groupPrefab == null)
			...
			for (var index = 0; index < groupsCount; ++index) {
				var gameObject = Instantiate(groupPrefab, position, Quaternion.identity);
				gameObject.name = string.Format("Jerboa_Group_{0}", index);
				var component = gameObject.GetComponent<JerboaGroupBarycentric>();
	...
	private int SortDelegate(JerboaGroupBarycentric a, JerboaGroupBarycentric b) {
		return (transform.position - a.transform.position).sqrMagnitude.CompareTo(
			(transform.position - b.transform.position).sqrMagnitude);
	}

	private void Update() {
		jerboaInstancingManager.Update(transform.position);
		smoothedJerboaWeight = Mathf.MoveTowards(smoothedJerboaWeight, weight * quality,
			(float)(quality * (double)Time.deltaTime / 3.0));
		...
		for (var index = 0; index < groupsCount; ++index) {
			var group = groups[index];
			if ((transform.position - group.transform.position).magnitude >= teleportRadius * 2.0 &&
			    group.TryGroupTeleport(transform.position, teleportRadius))
				break;
		}
	}
