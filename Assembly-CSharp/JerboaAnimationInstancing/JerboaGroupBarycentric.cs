...
public class JerboaGroupBarycentric : MonoBehaviour {
	...
	public void SetRandomPosition(JerboaInstanceDescription instance) {
		...
		if (instance.GroupBarycentricA + (double)instance.GroupBarycentricB <= 1.0)
			return;
		var groupBarycentricA = instance.GroupBarycentricA;
	...
	public Vector3 GetWorldPosition(JerboaInstanceDescription position) {
		var groupTriangleIndex = position.GroupTriangleIndex;
		var position1 = transform.position;
		var vector3_1 = transform.position +
		                raysInWorldSpace[position.GroupTriangleIndex] * rayDistances[position.GroupTriangleIndex];
		var vector3_2 = transform.position + raysInWorldSpace[position.GroupTriangleIndexNext] *
			rayDistances[position.GroupTriangleIndexNext];
		var num1 = Random.value;
		var num2 = Random.value;
		return position1 + (vector3_1 - position1) * position.GroupBarycentricA +
		       (vector3_2 - position1) * position.GroupBarycentricB;
	}

	private void PrepareRays() {
		rayDistances = new float[groupRayCount];
		raysInWorldSpace = new Vector3[groupRayCount];
		for (var index = 0; index < groupRayCount; ++index) {
			var num = (float)(index * (double)groupSpreadAngle / (groupRayCount - 1) - groupSpreadAngle / 2.0);
			raysInWorldSpace[index] = new Vector3(Mathf.Sin(num * ((float)Math.PI / 180f)), 0.0f,
				Mathf.Cos(num * ((float)Math.PI / 180f)));
		...
		if (!Aloud)
			...
			foreach (var instance in instances)
				if ((instance.Position - transform.position).sqrMagnitude < 4.0)
					++num1;
			var num2 = jerboaManager.Weight * jerboaManager.Quality;
			var num3 = num1 * num2;
			if (num3 < 7.5)
				AudioClip = null;
			else if (num3 < 15.0)
	...
	private void UpdateSounds() {
		if (audioSource == null || audioSource.clip == null)
			return;
		volume = !Aloud
			? Mathf.MoveTowards(volume, 0.0f, Time.deltaTime / 2f)
			: Mathf.MoveTowards(volume, 1f, Time.deltaTime / 2f);
	...
	private void Update() {
		...
		var num1 = (int)(instances.Count * 0.10000000149011612 + 1.0);
		var num2 = 10f * Time.deltaTime / num1;
		for (var updateIndex = this.updateIndex; updateIndex < this.updateIndex + num1; ++updateIndex) {
			var instance = instances[updateIndex % instances.Count];
			var forward = GetWorldPosition(instance) - instance.Position;
			var sqrMagnitude = forward.sqrMagnitude;
			if (sqrMagnitude > (double)groupRayDistanceSquared) {
				if (Random.value < (double)num2)
					instance.Respawn();
				else {
					var to = Quaternion.LookRotation(forward);
					instance.Rotation = Quaternion.RotateTowards(instance.Rotation, to,
						(float)(40.0 * Time.deltaTime / 0.10000000149011612));
				}
	...
	private void TryCreateRandomPath(NavMeshAgent agent) {
		...
		if (!NavMesh.SamplePosition(
			    ...
			    new Vector3(insideUnitCircle.x, 0.0f, insideUnitCircle.y) * pathSampleRadius, out hit, 2f,
			    groupWalkNavigationMask))
			return;
		agent.destination = hit.position;
	}

	public bool TryGroupTeleport(Vector3 center, float radius) {
		var f = Random.Range(0.0f, 6.28318548f);
		NavMeshHit hit;
		if (!NavMesh.SamplePosition(center + new Vector3(Mathf.Sin(f), 0.0f, Mathf.Cos(f)) * radius, out hit, 3f,
			    groupWalkNavigationMask))
			return false;
	...
	private void UpdateRays() {
		for (var index = 0; index < groupRayCount; ++index) {
			NavMeshHit hit;
			rayDistances[index] =
				!NavMesh.Raycast(transform.position, transform.position + raysInWorldSpace[index] * groupRayDistance,
					out hit, groupWalkNavigationMask)
					? groupRayDistance
					: hit.distance;
		}
	}
	...
	private void UpdateRaysLazy() {
		...
		NavMeshHit hit;
		if (NavMesh.Raycast(transform.position, transform.position + raysInWorldSpace[lastRayIndex] * groupRayDistance,
			    out hit, groupWalkNavigationMask))
	...
	private void OnDrawGizmos() {
		...
		for (var index = 0; index < raysInWorldSpace.Length; ++index) {
			var from = transform.position + 0.5f * Vector3.up;
