...
public class NGSS_Directional : MonoBehaviourInstance<NGSS_Directional> {
	...
	[Tooltip(
		"If disabled, NGSS Directional shadows replacement will be removed from Graphics settings when OnDisable is called in this component.")]
	...
	private void OnDisable() {
		...
		if (NGSS_KEEP_ONDISABLE || !isGraphicSet)
			return;
		isGraphicSet = false;
		GraphicsSettings.SetCustomShader(BuiltinShaderType.ScreenSpaceShadows,
	...
	private void OnEnable() {
		if (IsNotSupported()) {
			Debug.LogWarning(
				"Unsupported graphics API, NGSS requires at least SM3.0 or higher and DX9 is not supported.", this);
			enabled = false;
		} else
			Init();
	}
	...
	private void Init() {
		...
		if (!isGraphicSet) {
			...
			GraphicsSettings.SetCustomShader(BuiltinShaderType.ScreenSpaceShadows,
				Shader.Find("Hidden/NGSS_Directional"));
			isGraphicSet = true;
		}
	...
	private bool IsNotSupported() {
		return SystemInfo.graphicsDeviceType == GraphicsDeviceType.OpenGLES2 ||
		       SystemInfo.graphicsDeviceType == GraphicsDeviceType.PlayStationVita ||
		       SystemInfo.graphicsDeviceType == GraphicsDeviceType.N3DS;
	}
	...
	private void Update() {
		if (NGSS_HARD_SHADOWS) {
			...
			var num1 = NGSS_SOFTNESS / (QualitySettings.shadowDistance * 0.6600000262260437);
			double num2;
			...
			if (NGSS_PCSS_ENABLED) {
				var num3 = NGSS_PCSS_SOFTNESS_MIN * 0.1f;
				var num4 = NGSS_PCSS_SOFTNESS_MAX * 0.25f;
			...
			if (GLOBAL_SETTINGS_OVERRIDE) {
				...
				if (GLOBAL_CASCADES_COUNT > 1) {
					...
					QualitySettings.shadowCascade4Split = new Vector3(GLOBAL_CASCADES_SPLIT_VALUE,
						GLOBAL_CASCADES_SPLIT_VALUE * 2f, (float)(GLOBAL_CASCADES_SPLIT_VALUE * 2.0 * 2.0));
			...
			if (GLOBAL_CASCADES_COUNT > 1) {
				...
				Shader.SetGlobalVector("NGSS_CASCADES_SPLITS",
					QualitySettings.shadowCascades == 2
						? new Vector4(QualitySettings.shadowCascade2Split, 1f, 1f, 1f)
						: new Vector4(QualitySettings.shadowCascade4Split.x, QualitySettings.shadowCascade4Split.y,
							QualitySettings.shadowCascade4Split.z, 1f));
			}
