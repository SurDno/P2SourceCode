...
public class FastTravelGraph : ScriptableObject {
	...
	public int GetPath(
		...
		var index = IndexOf(flow, destination);
		if (index == -1)
			return -1;
		result.Add(destination);
		var link = flow[index];
		var time = link.Time;
		for (var pointB = link.PointB; pointB != origin; pointB = flow[IndexOf(flow, pointB)].PointB)
	...
	private void CalculateFlow(FastTravelPointEnum origin) {
		...
		while (searchQueue.Count > 0) {
			var point1 = searchQueue.Dequeue();
			var time = flow[IndexOf(flow, point1)].Time;
			for (var index1 = 0; index1 < links.Length; ++index1)
				if (links[index1].PointA == point1 || links[index1].PointB == point1) {
					var point2 = links[index1].PointA != point1 ? links[index1].PointA : links[index1].PointB;
					var num = links[index1].Time + time;
					var index2 = IndexOf(flow, point2);
					if (index2 == -1) {
						flow.Add(new Link {
							PointA = point2,
							PointB = point1,
	...
	private struct Link {
		...
		[Tooltip("Travel time in minutes")] public int Time;
	}
}