...
public class AmplifyOcclusionBase : MonoBehaviour {
	...
	[Range(0.0f, 1f)] public float Intensity = 1f;
	public Color Tint = Color.black;
	[Range(0.0f, 16f)] public float Radius = 1f;
	[Range(0.0f, 16f)] public float PowerExponent = 1.8f;
	[Range(0.0f, 0.99f)] public float Bias = 0.05f;
	public bool CacheAware;
	public bool Downsample;
	[Header("Distance Fade")] public bool FadeEnabled;
	public float FadeStart = 100f;
	public float FadeLength = 50f;
	[Range(0.0f, 1f)] public float FadeToIntensity = 1f;
	[Range(0.0f, 16f)] public float FadeToRadius = 1f;
	[Range(0.0f, 16f)] public float FadeToPowerExponent = 1.8f;
	[Header("Bilateral Blur")] public bool BlurEnabled = true;
	[Range(1f, 4f)] public int BlurRadius = 2;
	[Range(1f, 4f)] public int BlurPasses = 1;
	[Range(0.0f, 20f)] public float BlurSharpness = 10f;
	...
	private bool CheckParamsChanged() {
		return prevScreenWidth != m_camera.pixelWidth || prevScreenHeight != m_camera.pixelHeight ||
		       prevHDR != m_camera.allowHDR || prevApplyMethod != ApplyMethod || prevSampleCount != SampleCount ||
		       prevPerPixelNormals != PerPixelNormals || prevCacheAware != CacheAware || prevDownscale != Downsample ||
		       prevFadeEnabled != FadeEnabled || prevFadeToIntensity != (double)FadeToIntensity ||
		       prevFadeToRadius != (double)FadeToRadius || prevFadeToPowerExponent != (double)FadeToPowerExponent ||
		       prevFadeStart != (double)FadeStart || prevFadeLength != (double)FadeLength ||
	...
	private void Warmup() {
		...
		for (var index = 0; index < m_mrtCount; ++index) {
	...
	private void OnEnable() {
		if (!CheckRenderTextureFormats()) {
			Debug.LogError(
				"[AmplifyOcclusion] Target platform does not meet the minimum requirements for this effect to work properly.");
			enabled = false;
		} else {
	...
	private void Update() {
		...
		if (ApplyMethod == ApplicationMethod.Deferred && PerPixelNormals == PerPixelNormalSource.Camera) {
			...
			Debug.LogWarning(
				"[AmplifyOcclusion] Camera Normals not supported for Deferred Method. Switching to GBuffer Normals.");
		}
		...
		if (PerPixelNormals == PerPixelNormalSource.Camera &&
		    (m_camera.depthTextureMode & DepthTextureMode.DepthNormals) == DepthTextureMode.None)
	...
	private void CheckMaterial() {
		if (m_occlusionMat == null) {
			var material = new Material(Shader.Find("Hidden/Amplify Occlusion/Occlusion"));
			material.hideFlags = HideFlags.DontSave;
			m_occlusionMat = material;
		}

		if (m_blurMat == null) {
			var material = new Material(Shader.Find("Hidden/Amplify Occlusion/Blur"));
			material.hideFlags = HideFlags.DontSave;
			m_blurMat = material;
		}
		...
		if (!(m_copyMat == null))
			return;
		var material1 = new Material(Shader.Find("Hidden/Amplify Occlusion/Copy"));
	...
	public static Color[] GenerateRandomizationData() {
		var randomizationData = new Color[16];
		var index1 = 0;
		var num1 = 0;
		for (; index1 < 16; ++index1) {
			var values1 = RandomTable.Values;
			var index2 = num1;
			var num2 = index2 + 1;
			var num3 = values1[index2];
			var values2 = RandomTable.Values;
			var index3 = num2;
			num1 = index3 + 1;
			var num4 = values2[index3];
			var f = (float)(6.2831854820251465 * num3 / 8.0);
	...
	public static Texture2D GenerateRandomizationTexture(Color[] randomPixels) {
		var texture2D = new Texture2D(4, 4, TextureFormat.ARGB32, false, true);
		texture2D.hideFlags = HideFlags.DontSave;
		var randomizationTexture = texture2D;
	...
	private RenderTexture SafeAllocateRT(
		...
		var renderTexture1 = new RenderTexture(width, height, 0, format, readWrite);
		renderTexture1.hideFlags = HideFlags.DontSave;
		var renderTexture2 = renderTexture1;
	...
	private void SetBlitTarget(
		...
		int targetWidth,
		int targetHeight) {
		cb.SetGlobalVector("_AO_Target_TexelSize",
			new Vector4(1f / targetWidth, 1f / targetHeight, targetWidth, targetHeight));
	...
	private void PerformBlit(
		...
		cb.SetGlobalVector("_AO_Source_TexelSize",
	...
	private void CommandBuffer_Unregister(CameraEvent cameraEvent, CommandBuffer commandBuffer) {
		if (!(m_camera != null))
			return;
		foreach (var commandBuffer1 in m_camera.GetCommandBuffers(cameraEvent))
	...
	private CommandBuffer CommandBuffer_AllocateRegister(CameraEvent cameraEvent) {
		...
		var commandBuffer = CommandBuffer_Allocate(name);
		CommandBuffer_Register(cameraEvent, commandBuffer);
		return commandBuffer;
	}

	private void CommandBuffer_UnregisterAll() {
		foreach (var registeredCommandBuffer in m_registeredCommandBuffers)
	...
	private void UpdateGlobalShaderConstants(TargetDesc target) {
		var num1 = m_camera.fieldOfView * ((float)Math.PI / 180f);
		var vector2_1 = new Vector2((float)(1.0 / Mathf.Tan(num1 * 0.5f) * (target.height / (double)target.width)),
			1f / Mathf.Tan(num1 * 0.5f));
		var vector2_2 = new Vector2(1f / vector2_1.x, 1f / vector2_1.y);
		var num2 = !m_camera.orthographic
			? target.height / (Mathf.Tan(num1 * 0.5f) * 2f)
			: target.height / m_camera.orthographicSize;
		var num3 = Mathf.Clamp(Bias, 0.0f, 1f);
		FadeStart = Mathf.Max(0.0f, FadeStart);
		FadeLength = Mathf.Max(0.01f, FadeLength);
		var y = FadeEnabled ? 1f / FadeLength : 0.0f;
		Shader.SetGlobalMatrix("_AO_CameraProj",
			GL.GetGPUProjectionMatrix(Matrix4x4.Ortho(0.0f, 1f, 0.0f, 1f, -1f, 100f), false));
		Shader.SetGlobalMatrix("_AO_CameraView", m_camera.worldToCameraMatrix);
		Shader.SetGlobalVector("_AO_UVToView",
			new Vector4(2f * vector2_2.x, -2f * vector2_2.y, -1f * vector2_2.x, 1f * vector2_2.y));
	...
	private void CommandBuffer_FillComputeOcclusion(
		...
		CheckMaterial();
		CheckRandomData();
		cb.SetGlobalVector("_AO_Buffer_PadScale",
			new Vector4(target.padRatioWidth, target.padRatioHeight, 1f / target.padRatioWidth,
				1f / target.padRatioHeight));
		cb.SetGlobalVector("_AO_Buffer_TexelSize",
			new Vector4(1f / target.width, 1f / target.height, target.width, target.height));
		cb.SetGlobalVector("_AO_QuarterBuffer_TexelSize",
			new Vector4(1f / target.quarterWidth, 1f / target.quarterHeight, target.quarterWidth,
				target.quarterHeight));
		cb.SetGlobalFloat("_AO_MaxRadiusPixels", Mathf.Min(target.width, target.height));
		if (m_occlusionRT == null || m_occlusionRT.width != target.width || m_occlusionRT.height != target.height ||
		    !m_occlusionRT.IsCreated()) {
			SafeReleaseRT(ref m_occlusionRT);
			m_occlusionRT = SafeAllocateRT("_AO_OcclusionTexture", target.width, target.height, m_occlusionRTFormat,
				RenderTextureReadWrite.Linear);
		}

		var num1 = -1;
		if (Downsample)
			num1 = SafeAllocateTemporaryRT(cb, "_AO_SmallOcclusionTexture", target.width / 2, target.height / 2,
				m_occlusionRTFormat, RenderTextureReadWrite.Linear, FilterMode.Bilinear);
		if (CacheAware && !Downsample) {
			var num2 = SafeAllocateTemporaryRT(cb, "_AO_OcclusionAtlas", target.width, target.height,
				m_occlusionRTFormat, RenderTextureReadWrite.Linear);
			for (var index = 0; index < 16; ++index) {
				m_depthLayerRT[index] = SafeAllocateTemporaryRT(cb, m_layerDepthNames[index], target.quarterWidth,
					target.quarterHeight, m_depthRTFormat, RenderTextureReadWrite.Linear);
				m_normalLayerRT[index] = SafeAllocateTemporaryRT(cb, m_layerNormalNames[index], target.quarterWidth,
					target.quarterHeight, m_normalRTFormat, RenderTextureReadWrite.Linear);
				m_occlusionLayerRT[index] = SafeAllocateTemporaryRT(cb, m_layerOcclusionNames[index],
					target.quarterWidth, target.quarterHeight, m_occlusionRTFormat, RenderTextureReadWrite.Linear);
			}

			for (var index1 = 0; index1 < 16; index1 += m_mrtCount) {
				for (var index2 = 0; index2 < m_mrtCount; ++index2) {
					var index3 = index2 + index1;
					var num3 = index3 & 3;
					var num4 = index3 >> 2;
					cb.SetGlobalVector(m_layerOffsetNames[index2], new Vector2(num3 + 0.5f, num4 + 0.5f));
					m_depthTargets[index2] = m_depthLayerRT[index3];
					m_normalTargets[index2] = m_normalLayerRT[index3];
				}
			...
			for (var index = 0; index < 16; ++index) {
				cb.SetGlobalVector("_AO_LayerOffset", new Vector2((index & 3) + 0.5f, (index >> 2) + 0.5f));
				cb.SetGlobalVector("_AO_LayerRandom", m_randomData[index]);
				cb.SetGlobalTexture("_AO_NormalTexture", m_normalLayerRT[index]);
				cb.SetGlobalTexture("_AO_DepthTexture", m_depthLayerRT[index]);
				SetBlitTarget(cb, m_occlusionLayerRT[index], target.quarterWidth, target.quarterHeight);
				PerformBlit(cb, m_occlusionMat, (int)(15 + SampleCount));
			}

			SetBlitTarget(cb, num2, target.width, target.height);
			for (var index = 0; index < 16; ++index) {
				var x = (index & 3) * target.quarterWidth;
				var y = (index >> 2) * target.quarterHeight;
				PerformBlit(cb, m_occlusionLayerRT[index], target.quarterWidth, target.quarterHeight, m_copyMat, 0, x,
					y);
			}
			...
			var pass = (int)(20 + (int)SampleCount * 4 + PerPixelNormals);
			if (Downsample) {
				cb.Blit(null, new RenderTargetIdentifier(num1), m_occlusionMat, pass);
				SetBlitTarget(cb, (RenderTargetIdentifier)(Texture)m_occlusionRT, target.width, target.height);
				PerformBlit(cb, num1, target.width / 2, target.height / 2, m_occlusionMat, 41);
			} else
				cb.Blit(null, (RenderTargetIdentifier)(Texture)m_occlusionRT, m_occlusionMat, pass);
		}

		if (BlurEnabled) {
			var num5 = SafeAllocateTemporaryRT(cb, "_AO_TEMP", target.width, target.height, m_occlusionRTFormat,
				RenderTextureReadWrite.Linear);
			for (var index = 0; index < BlurPasses; ++index) {
				SetBlitTarget(cb, num5, target.width, target.height);
				PerformBlit(cb, (RenderTargetIdentifier)(Texture)m_occlusionRT, target.width, target.height, m_blurMat,
					(BlurRadius - 1) * 2);
				SetBlitTarget(cb, (RenderTargetIdentifier)(Texture)m_occlusionRT, target.width, target.height);
				PerformBlit(cb, num5, target.width, target.height, m_blurMat, 1 + (BlurRadius - 1) * 2);
			}
	...
	private void CommandBuffer_FillApplyDeferred(
		...
		m_applyDeferredTargets[1] =
			logTarget ? BuiltinRenderTextureType.GBuffer3 : BuiltinRenderTextureType.CameraTarget;
		if (!logTarget) {
			SetBlitTarget(cb, m_applyDeferredTargets, target.fullWidth, target.fullHeight);
			PerformBlit(cb, m_occlusionMat, 37);
		} else {
			var num1 = SafeAllocateTemporaryRT(cb, "_AO_GBufferAlbedo", target.fullWidth, target.fullHeight,
				RenderTextureFormat.ARGB32);
			var num2 = SafeAllocateTemporaryRT(cb, "_AO_GBufferEmission", target.fullWidth, target.fullHeight,
				RenderTextureFormat.ARGB32);
			cb.Blit(m_applyDeferredTargets[0], num1);
			cb.Blit(m_applyDeferredTargets[1], num2);
			cb.SetGlobalTexture("_AO_GBufferAlbedo", num1);
	...
	private void CommandBuffer_FillApplyPostEffect(
		...
		if (!logTarget) {
			...
			var num = SafeAllocateTemporaryRT(cb, "_AO_GBufferEmission", target.fullWidth, target.fullHeight,
				RenderTextureFormat.ARGB32);
	...
	private void CommandBuffer_Rebuild(TargetDesc target) {
		var flag = PerPixelNormals == PerPixelNormalSource.GBuffer ||
		           PerPixelNormals == PerPixelNormalSource.GBufferOctaEncoded;
		var cameraEvent = flag ? CameraEvent.AfterLighting : CameraEvent.BeforeImageEffectsOpaque;
		if (ApplyMethod == ApplicationMethod.Debug) {
			var cb = CommandBuffer_AllocateRegister(cameraEvent);
			CommandBuffer_FillComputeOcclusion(cb, target);
			CommandBuffer_FillApplyDebug(cb, target);
		} else {
			var logTarget = !m_camera.allowHDR & flag;
			var cb = CommandBuffer_AllocateRegister(ApplyMethod == ApplicationMethod.Deferred
	...
	private void OnPreRender() {
		var allowHdr = m_camera.allowHDR;
