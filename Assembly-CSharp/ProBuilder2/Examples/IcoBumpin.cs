...
public class IcoBumpin : MonoBehaviour {
	...
	[Range(0.0f, 3f)] public int icoSubdivisions = 2;
	[Range(0.0f, 1f)] public float startingExtrusion = 0.1f;
	public Material material;
	[Range(1f, 50f)] public float extrusion = 30f;
	[Range(8f, 128f)] public int fftBounds = 32;
	[Range(0.0f, 10f)] public float verticalBounce = 4f;
	...
	private void Start() {
		...
		var dictionary = ico.sharedIndices.ToDictionary();
		for (var index = 0; index < faces.Length; ++index)
			outsides[index] = new FaceRef(faces[index], pb_Math.Normal(ico, faces[index]),
				ico.sharedIndices.AllIndicesWithValues(dictionary, faces[index].distinctIndices).ToArray());
	...
	private void Update() {
		...
		rms = RMS(data);
		for (var index1 = 0; index1 < outsides.Length; ++index1) {
			var time = index1 / faces_length;
			var index2 = (int)(time * (double)fftBounds);
			var vector3 = outsides[index1].nrm *
			              (float)((fft[index2] + (double)fft_history[index2]) * 0.5 *
			                      (frequencyCurve.Evaluate(time) * 0.5 + 0.5)) * extrusion;
			foreach (var index3 in outsides[index1].indices)
				displaced_vertices[index3] = original_vertices[index3] + vector3;
		}

		var zero = Vector3.zero;
		for (var index4 = 0; index4 < 1024; ++index4) {
			var index5 = index4 < 1023 ? index4 : 0;
			zero.x = Mathf.Cos((float)(index5 / 1024.0 * 6.2831850051879883)) * (waveformRadius +
				(float)((data[index5] + (double)data_history[index5]) * 0.5) * waveformHeight);
			zero.z = Mathf.Sin((float)(index5 / 1024.0 * 6.2831850051879883)) * (waveformRadius +
				(float)((data[index5] + (double)data_history[index5]) * 0.5) * waveformHeight);
			zero.y = 0.0f;
	...
	private float RMS(float[] arr) {
		...
		for (var index = 0; index < (double)length; ++index)
