...
public class BurnedSurfaceEffect : MonoBehaviour {
	...
	[SerializeField] [FormerlySerializedAs("EffectMaterial")]
	private Material effectMaterial;

	[SerializeField] [FormerlySerializedAs("IsAlive")]
	private bool isAlive = true;

	[Range(0.0f, 1f)] public float SmokeLevel;
	[Range(0.0f, 1f)] public float FireLevel;
		...
		for (var index = 0; index < sharedMaterials.Length; ++index) {
			...
			if (!(material == null)) {
				...
				if (!(shader == null) && shader.name.Contains("Cutout")) {
					...
					alphaOffsetScales[index] = new Vector4(mainTextureScale.x, mainTextureScale.y, mainTextureOffset.x,
						mainTextureOffset.y);
				}
			}
		}
	...
	private void DrawMesh(Mesh mesh, int subMeshIndex) {
		var vector4 = new Vector4(1f, 1f, 0.0f, 0.0f);
		Texture texture;
		...
		Graphics.DrawMesh(mesh, transform.localToWorldMatrix, effectMaterial, gameObject.layer, null, subMeshIndex,
			propertyBlock);
	}
	...
	private void LateUpdate() {
		...
		if (skinnedMeshRenderer != null) {
			...
			for (var subMeshIndex = 0; subMeshIndex < bakedMesh.subMeshCount; ++subMeshIndex)
