...
public class RendererBurn : MonoBehaviour {
	[SerializeField] [FormerlySerializedAs("FlameSystem")]
	private ParticleSystem flameSystem;

	[SerializeField] [FormerlySerializedAs("SmokeSystem")]
	private ParticleSystem smokeSystem;

	[SerializeField] [FormerlySerializedAs("SparkSystem")]
	private ParticleSystem sparkSystem;

	[Space] [Range(0.0f, 1f)] public float Strength;
	...
	private void Awake() {
		if (flameSystem != null) {
			var minMaxCurve = flameSystem.emission.rateOverTime;
	...
	private void UpdateSystems() {
		if (lastBurningRenderer != BurningRenderer || (meshAssigned && BurningRenderer == null)) {
			...
			if (BurningRenderer is MeshRenderer) {
				...
				if (BurningRenderer != null)
					Debug.LogError("BurningRenderer can only be null, MeshRenderer or SkinnedMeshRenderer, : " +
					               BurningRenderer.GetInfo());
				BurningRenderer = null;
				ParticleSystem.ShapeModule shape;
				if (flameSystem != null)
					shape = flameSystem.shape with {
		...
		var num1 = lastBurningRenderer != null ? Strength : 0.0f;
		...
		if (lastStrength == 0.0) {
			...
			if (flameSystem != null) {
				...
				var num2 = (float)(0.25 + lastStrength * 0.75);
			...
			if (smokeSystem != null) {
				...
				var color = main.startColor.color with {
