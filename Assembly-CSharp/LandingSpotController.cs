...
public class LandingSpotController : MonoBehaviour {
	...
	public void ScareAll(float minDelay, float maxDelay) {
		for (var index = 0; index < _thisT.childCount; ++index)
			if (_thisT.GetChild(index).GetComponent<LandingSpot>() != null)
				_thisT.GetChild(index).GetComponent<LandingSpot>()
					.Invoke("ReleaseFlockChild", Random.Range(minDelay, maxDelay));
	}
	...
	public IEnumerator InstantLandOnStart(float delay) {
		...
		for (var i = 0; i < _thisT.childCount; ++i)
			if (_thisT.GetChild(i).GetComponent<LandingSpot>() != null) {
				var spot = _thisT.GetChild(i).GetComponent<LandingSpot>();
				spot.InstantLand();
				spot = null;
			}
	}
