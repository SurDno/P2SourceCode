...
public class InspectedAttribute : MemberAttribute {
	...
	private void ComputeMethod(Container container, MethodInfo method) {
		ComputeHandle handle = (target, data) => {
			var inspectedContext = (InspectedContext)data;
			string name1;
			if (!Name.IsNullOrEmpty())
				name1 = Name;
			else {
				var name2 = method.Name;
			...
			var type2 = type1;
			inspectedContext.Provider.DrawInspected(name1, type2, null, true, target, method,
				parameters => method.Invoke(target, (object[])parameters));
		};
	...
	private void ComputeProperty(Container container, PropertyInfo property) {
		...
		if (Header)
			container.GetHandler(HeaderInspectedId).AddHandle((target, data) => {
				var inspectedProvider = (IInspectedProvider)data;
		...
		ComputeHandle handle = (target, data) => {
			var provider = (InspectedContext)data;
			var obj = property.GetValue(target, null);
			string name1;
			if (!Name.IsNullOrEmpty())
				name1 = Name;
			...
			provider.Provider.DrawInspected(name1, property.PropertyType, obj, Mutable & hasMutator, target, property,
				value2 => {
