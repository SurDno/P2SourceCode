...
public class ExecuteWithObjectMethodDrawer : BaseMethodDrawer<ExecuteWithObjectMethodDrawer> {
	public override void DrawInspected(
		...
		for (var index1 = 0; index1 < 10; ++index1) {
			var index2 = index1;
			var item = ServiceLocator.GetService<SelectionService>().GetSelection(index2) as IObject;
			menu.AddItem(
				"Execute With Object/From Slot " + index2 + " : " + (item != null ? item.Name : (object)"null"), false,
				(Action)(() => {
					var action = setter;
					if (action == null)
						return;
