...
public class LightCameraDistance : MonoBehaviour {
	[SerializeField] private AnimationCurve rangeCurve = AnimationCurve.Constant(0.0f, 10f, 1f);
	...
	private void OnPreCullEvent2(Camera camera) {
		var num = rangeCurve.Evaluate(Vector3.Distance(camera.transform.position, cachedTransform.position));
