...
public class TerrainDetailChunk : MonoBehaviour
{
      ...
      if (num < (double) drawDistance)
      {
        if (renderers[index2] == null)
        {
          ...
          if (num < 3.0 || renderersCreatedThisFrame < 1)
          {
            ...
            renderers[index2] = TerrainDetailRenderer.Create(rendererPrefab, this, indices[index2 * 2], indices[index2 * 2 + 1], out bounds[index2 * 2], out bounds[index2 * 2 + 1]);
            needsUpdate = true;
          }
        }
        else
    ...
    int chunkY)
  {
    ...
    for (int index1 = 0; index1 < num4; ++index1)
    {
      ...
      int index2 = num5 * pointList.Count / f;
      int count = num6 * pointList.Count / f - index2;
      pointList.Sort(index2, count, new Byte2.YComparer());
      for (int index3 = num5; index3 < num6; ++index3)
      {
        int num7 = index3 * pointList.Count / f;
        ...
        {
          Byte2 point = pointList[index4 + num7];
          if (num9 > point.X)
            num9 = point.X;
          if (num11 > point.Y)
            num11 = point.Y;
