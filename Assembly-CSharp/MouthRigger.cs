...
public class MouthRigger : MonoBehaviour {
	...
	private void MakeRuntimeDeformer() {
		...
		foreach (var visemeBone in VisemeBones) {
			var phonemes = phnMap.GetPhonemes(visemeBone.m_visemeLabel);
			if (phonemes == null)
				Debug.Log("Invalid Phoneme Map: can't find " + visemeBone.m_visemeLabel);
			else
				foreach (var phn in phonemes.phns)
	...
	public void ImportBoneConfig() {
		var bKeepLocalRotations = true;
		if (!(BoneConfigFile != null))
			return;
		var visemeBoneDefineList = new List<VisemeBoneDefine>();
		var cache = new Hashtable();
		var reader = new XmlTextReader(new StringReader(BoneConfigFile.text));
		while (reader.Read())
			if (reader.Name == "mapping") {
				...
				var visemeBoneDefine = new VisemeBoneDefine();
				...
				var viseme = phnMap.PhonemeToViseme(visemeBoneDefine.m_visemeLabel);
				if (viseme.Length != 0)
					visemeBoneDefine.m_visemeLabel = viseme;
				else
					Debug.LogError("Can't find phoneme to viseme mapping for <viseme><label>" +
					               visemeBoneDefine.m_visemeLabel + "</label> in BoneConfigFile");
				visemeBoneDefineList.Add(visemeBoneDefine);
			}

		VisemeBones = new VisemeBoneDefine[visemeBoneDefineList.Count];
		for (var index = 0; index < visemeBoneDefineList.Count; ++index)
	...
	public VisemeBoneDefine GetViseme(string which) {
		...
		foreach (var visemeBone in VisemeBones)
			if (visemeBone != null && visemeBone.m_visemeLabel == which)
				return visemeBone;
		var length = VisemeBones.Length;
		var visemeBoneDefineArray = new VisemeBoneDefine[length + 1];
	...
	public void ShowViseme(string which) {
		GetViseme(which).ResetToThisPose();
	}
}