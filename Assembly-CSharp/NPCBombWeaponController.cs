...
public class NPCBombWeaponController : NPCWeaponControllerBase {
	...
	private void CreateBomb() {
		var enemy = npcEnemy.Enemy;
		if (enemy == null)
			return;
		var vector3 = npcEnemy.LastThrowV *
		              ((enemy.transform.position - npcEnemy.transform.position).normalized *
			              Mathf.Cos(npcEnemy.LastThrowAngle) + Vector3.up * Mathf.Sin(npcEnemy.LastThrowAngle));
		var transform = service.BombParent.transform;
		var gameObject = Object.Instantiate(ScriptableObjectInstance<ResourceFromCodeData>.Instance.BottleBomb,
