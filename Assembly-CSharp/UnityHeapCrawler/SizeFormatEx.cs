  ...
  public static class SizeFormatEx
  {
    ...
    public static string Format(this SizeFormat format, long size)
    {
      ...
      for (num2 = 0; num1 * 1024L < size && num2 < 4; ++num2)
        num1 *= 1024L;
      double num3 = 1.0 * size / num1;
      string str1 = (num2 != 0 ? (num3 < 9.995 ? num3.ToString("F2", CultureInfo.InvariantCulture) : num3.ToString("F1", CultureInfo.InvariantCulture)) : size.ToString(CultureInfo.InvariantCulture)) + " ";
      string str2;
      switch (num2)
      {
        case 0:
