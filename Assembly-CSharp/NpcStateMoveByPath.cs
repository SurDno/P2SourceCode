...
public class NpcStateMoveByPath : INpcState {
	...
	[Inspected] private List<Vector3> path;
	[Inspected] private int currentIndex;
	[Inspected] private Vector3 currentPoint;
	[Inspected] private float remainingPointsDistance;
	...
	private void RecountRemainingDistance() {
		...
		for (var currentIndex = this.currentIndex; currentIndex < path.Count; ++currentIndex)
	...
	public void OnUpdateMove() {
		var direction = currentPoint - GameObject.transform.position;
		...
		if (!movementStarted) {
			...
			var flag = behavior.Move(direction, magnitude + remainingPointsDistance);
