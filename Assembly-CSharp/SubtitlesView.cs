...
public class SubtitlesView : EngineDependent {
	...
	public void AddSubtitles(IEntity actor, string tag, AudioState audioState, Object context) {
		if (!service.SubtitlesEnabled || (!service.DialogSubtitlesEnabled &&
		                                  ServiceLocator.GetService<UIService>().Active is IDialogWindow))
			return;
		var text = ServiceLocator.GetService<LocalizationService>().GetSubTitlesText(tag).Replace("<empty>", "").Trim();
		if (text == "")
			return;
		var subtitlesItem = FindItem(actor);
		if (subtitlesItem == null) {
			if (itemPool.Count == 0)
				subtitlesItem = new SubtitlesItem(this);
			else {
				var index = itemPool.Count - 1;
	...
	public GameObject CreateLineView() {
		...
		var index = lineViewPool.Count - 1;
		var lineView = lineViewPool[index];
