...
public class BoneDeformMemento {
	private List<DeformData> BoneDeformation = new();
	...
	public void ResetBones() {
		foreach (var deformData in BoneDeformation) {
			var localPosition = deformData.bone.localPosition;
			...
			var localEulerAngles = deformData.bone.localEulerAngles;
	...
	private DeformData FindBone(Transform theBone) {
		foreach (var bone in BoneDeformation)
			if (bone.bone == theBone)
				return bone;
		var bone1 = new DeformData();
		bone1.bone = theBone;
