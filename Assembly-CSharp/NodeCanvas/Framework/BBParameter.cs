...
public abstract class BBParameter {
	...
	[NonSerialized] private Blackboard _bb;
	[NonSerialized] private Variable _varRef;

	public static BBParameter CreateInstance(Type t, Blackboard bb) {
		if (t == null)
			return null;
		var instance = (BBParameter)Activator.CreateInstance(typeof(BBParameter<>).RTMakeGenericType(new Type[1] {
			t
		}));
		instance.bb = bb;
		return instance;
	...
	public static void SetBBFields(object o, Blackboard bb) {
		var objectBbParameters = GetObjectBBParameters(o);
		for (var index = 0; index < objectBbParameters.Count; ++index)
			objectBbParameters[index].bb = bb;
	}

	public static List<BBParameter> GetObjectBBParameters(object o) {
		var objectBbParameters = new List<BBParameter>();
		if (o == null)
			...
			if (typeof(BBParameter).RTIsAssignableFrom(field.FieldType)) {
				...
				if (instance != null)
					objectBbParameters.Add((BBParameter)instance);
			           ...
			           typeof(BBParameter).RTIsAssignableFrom(field.FieldType.RTGetGenericArguments()[0]) &&
			           field.GetValue(o) is IList list)
	...
	public Variable PromoteToVariable(Blackboard targetBB) {
		...
		var name = this.name;
		var str = targetBB != null ? targetBB.name : string.Empty;
		if (targetBB == null) {
			varRef = null;
		...
		if (varRef == null)
			Debug.LogError(string.Format(
				"Parameter {0} (of type '{1}') failed to promote to a Variable in Blackboard '{2}'.", name,
	...
	public override string ToString() {
		...
		return objectValue is IDictionary
			? string.Format("<b>{0}</b>", varType.FriendlyName())
			: string.Format("<b>{0}</b>", objectValue.ToStringAdvanced());
	}
}