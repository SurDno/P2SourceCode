...
public abstract class Node {
	...
	private List<Connection> _inConnections = new();
	private List<Connection> _outConnections = new();
	private int _id;
	...
	public static Node Create(Graph targetGraph, Type nodeType, Vector2 pos) {
		...
		var instance = (Node)Activator.CreateInstance(nodeType);
	...
	public Node Duplicate(Graph targetGraph) {
		...
		var o = JSONSerializer.Deserialize<Node>(JSONSerializer.Serialize(typeof(Node), this));
	...
	public Status Execute(FlowScriptController agent, Blackboard blackboard) {
		if (isChecked)
			return Error(
				"Infinite Loop. Please check for other errors that may have caused this in the log before this.");
		isChecked = true;
	...
	protected Status Error(string log) {
		Debug.LogError("<b>Graph Error:</b> '" + log + "' On node '" + name + "' ID " + Id + " | On graph '" +
		               graph.agent + "'");
		return Status.Error;
	}
	...
	public void RegisterEvents(Component targetAgent, params string[] eventNames) {
		if (targetAgent == null)
			...
			var messageRouter = targetAgent.GetComponent<MessageRouter>();
	...
	public void UnRegisterEvents(Component targetAgent, params string[] eventNames) {
		if (targetAgent == null)
			return;
		var component = targetAgent.GetComponent<MessageRouter>();
		if (!(component != null))
			return;
