...
public abstract class Connection {
	[SerializeField] private Node _sourceNode;
	...
	public static Connection Create(Node source, Node target, int sourceIndex) {
		...
		var instance = (Connection)Activator.CreateInstance(source.outConnectionType);
	...
	public Connection Duplicate(Node newSource, Node newTarget) {
		...
		var connection = JSONSerializer.Deserialize<Connection>(JSONSerializer.Serialize(typeof(Connection), this));
		connection.SetSource(newSource, false);
		connection.SetTarget(newTarget, false);
		connection.OnValidate(newSource.outConnections.IndexOf(connection),
			newTarget.inConnections.IndexOf(connection));
		return connection;
	}
	...
	public void SetSource(Node newSource, bool isRelink = true) {
		if (isRelink) {
			var connectionIndex = sourceNode.outConnections.IndexOf(this);
	...
	public void SetTarget(Node newTarget, bool isRelink = true) {
		if (isRelink) {
			var connectionIndex = targetNode.inConnections.IndexOf(this);
