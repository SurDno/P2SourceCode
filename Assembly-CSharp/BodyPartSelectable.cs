...
public class BodyPartSelectable : MonoBehaviour {
	...
	[SerializeField] private GameObject _buttonExtractHint;
	[SerializeField] private GameObject _buttonTakeHint;
	...
	public void SetSelected(bool selected) {
		...
		if (selected) {
			...
			_buttonExtractHint.transform.parent.position =
	...
	private bool AutopsyOrgan(GameActionType type, bool down) {
		...
		if (OrganRemoved) {
			ServiceLocator.GetService<GameActionService>().RemoveListener(GameActionType.Submit, AutopsyOrgan);
			ServiceLocator.GetService<GameActionService>().AddListener(GameActionType.Submit, TakeOrgan);
		} else if (toolSelector.Item == null && transform.parent.GetComponentsInChildren<BodyPartSelectable>()
			           .Count(organ => organ.OrganRemoved) == 0)
			SetSelected(false);
		...
		if (source.Count != 0) {
			...
			if (gameObject != null) {
				...
				CoroutineService.Instance.WaitFrame(1, (Action)(() => {
					if (toolSelector.Item == null && !OrganRemoved && transform.parent
						    .GetComponentsInChildren<BodyPartSelectable>().Count(organ => organ.OrganRemoved) == 0)
						SetSelected(false);
					OnJoystick(InputService.Instance.JoystickUsed);
				}));
				return true;
	...
	private void Start() {
		uiButton = GetComponentInChildren<HoldableButton2>();
	}
