...
public class FlockChildSound : MonoBehaviour {
	...
	public void PlayRandomSound() {
		if (!gameObject.activeInHierarchy)
			return;
		if (!_audio.isPlaying && _flightSounds.Length != 0 && _flightSoundRandomChance > (double)Random.value &&
		    !_flockChild.landing) {
			...
			_audio.PlayAndCheck();
		} else if (!_audio.isPlaying && _idleSounds.Length != 0 && _idleSoundRandomChance > (double)Random.value &&
		           _flockChild.landing) {
	...
	public void ScareSound() {
		if (!gameObject.activeInHierarchy || !_hasLanded || _flockChild.landing ||
		    _idleSoundRandomChance * 2.0 <= Random.value)
			return;
