...
public sealed class EyeAdaptationComponent :
	...
	public override void OnDisable() {
		foreach (Object @object in m_AutoExposurePool)
	...
	private Vector4 GetHistogramScaleOffsetRes() {
		...
		var x = 1f / (settings.logMax - settings.logMin);
		var y = -settings.logMin * x;
		return new Vector4(x, y, Mathf.Max(1f, Mathf.Floor(context.width / 2f)),
			Mathf.Max(1f, Mathf.Floor(context.height / 2f)));
	}
	...
	public Texture Prepare(RenderTexture source, Material uberMaterial) {
		...
		var mat = context.materialFactory.Get("Hidden/Post FX/Eye Adaptation");
		mat.shaderKeywords = null;
		if (m_HistogramBuffer == null)
			m_HistogramBuffer = new ComputeBuffer(64, 4);
		if (s_EmptyHistogramBuffer == null)
			s_EmptyHistogramBuffer = new uint[64];
		var histogramScaleOffsetRes = GetHistogramScaleOffsetRes();
		var renderTexture1 = context.renderTextureFactory.Get((int)histogramScaleOffsetRes.z,
			(int)histogramScaleOffsetRes.w, format: source.format);
		...
		m_EyeCompute.SetTexture(kernel, "_Source", renderTexture1);
		m_EyeCompute.SetVector("_ScaleOffsetRes", histogramScaleOffsetRes);
		m_EyeCompute.Dispatch(kernel, Mathf.CeilToInt(renderTexture1.width / 16f),
			Mathf.CeilToInt(renderTexture1.height / 16f), 1);
		...
		mat.SetVector(Uniforms._Params,
			new Vector4(settings.lowPercent * 0.01f, settings.highPercent * 0.01f,
				Mathf.Exp(settings.minLuminance * 0.6931472f), Mathf.Exp(settings.maxLuminance * 0.6931472f)));
		...
		if (m_FirstFrame || !Application.isPlaying) {
			...
			Graphics.Blit(null, m_CurrentAutoExposure, mat, 1);
			Graphics.Blit(m_AutoExposurePool[0], m_AutoExposurePool[1]);
		} else {
			var exposurePingPing = m_AutoExposurePingPing;
			int num1;
			var source1 = m_AutoExposurePool[(num1 = exposurePingPing + 1) % 2];
			int num2;
			var dest = m_AutoExposurePool[(num2 = num1 + 1) % 2];
			Graphics.Blit(source1, dest, mat, (int)settings.adaptationType);
			int num3;
		...
		if (context.profile.debugViews.IsModeActive(BuiltinDebugViewsModel.Mode.EyeAdaptation)) {
			if (m_DebugHistogram == null || !m_DebugHistogram.IsCreated()) {
				var renderTexture2 = new RenderTexture(256, 128, 0, RenderTextureFormat.ARGB32);
				renderTexture2.filterMode = FilterMode.Point;
				renderTexture2.wrapMode = TextureWrapMode.Clamp;
				m_DebugHistogram = renderTexture2;
			}

			mat.SetFloat(Uniforms._DebugWidth, m_DebugHistogram.width);
			Graphics.Blit(null, m_DebugHistogram, mat, 2);
		}
	...
	public void OnGUI() {
		if (m_DebugHistogram == null || !m_DebugHistogram.IsCreated())
			return;
		GUI.DrawTexture(
			new Rect((float)(context.viewport.x * (double)Screen.width + 8.0), 8f, m_DebugHistogram.width,
				m_DebugHistogram.height), m_DebugHistogram);
	}
