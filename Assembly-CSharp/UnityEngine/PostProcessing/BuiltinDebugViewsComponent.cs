...
public sealed class BuiltinDebugViewsComponent :
	...
	public override bool active => model.IsModeActive(BuiltinDebugViewsModel.Mode.Depth) ||
	...
	public override void PopulateCommandBuffer(CommandBuffer cb) {
		...
		var material = context.materialFactory.Get("Hidden/Post FX/Builtin Debug Views");
	...
	private void DepthPass(CommandBuffer cb) {
		var mat = context.materialFactory.Get("Hidden/Post FX/Builtin Debug Views");
		var depth = model.settings.depth;
	...
	private void MotionVectorsPass(CommandBuffer cb) {
		var material = context.materialFactory.Get("Hidden/Post FX/Builtin Debug Views");
		var motionVectors = model.settings.motionVectors;
		var num = Uniforms._TempRT;
		...
		if (motionVectors.motionVectorsOpacity > 0.0 && motionVectors.motionVectorsAmplitude > 0.0) {
			PrepareArrows();
			var y = 1f / motionVectors.motionVectorsResolution;
			var x = y * context.height / context.width;
	...
	private void PrepareArrows() {
		var vectorsResolution = model.settings.motionVectors.motionVectorsResolution;
		var columns = vectorsResolution * Screen.width / Screen.height;
		if (m_Arrows == null)
	...
	private class ArrowArray {
		...
		public void BuildMesh(int columns, int rows) {
			...
			var capacity = 6 * columns * rows;
			var vector3List = new List<Vector3>(capacity);
			var vector2List = new List<Vector2>(capacity);
			for (var index1 = 0; index1 < rows; ++index1) {
				for (var index2 = 0; index2 < columns; ++index2) {
					var vector2 = new Vector2((0.5f + index2) / columns, (0.5f + index1) / rows);
