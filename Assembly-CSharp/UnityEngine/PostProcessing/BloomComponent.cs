...
public sealed class BloomComponent : PostProcessingComponentRenderTexture<BloomModel> {
	...
	public void Prepare(RenderTexture source, Material uberMaterial, Texture autoExposure) {
		...
		var mat = context.materialFactory.Get("Hidden/Post FX/Bloom");
		mat.shaderKeywords = null;
		mat.SetTexture(Uniforms._AutoExposure, autoExposure);
		var width = context.width / 2;
		var num1 = context.height / 2;
		if (width < 1)
			width = 1;
		if (num1 < 1)
			num1 = 1;
		var format = Application.isMobilePlatform ? RenderTextureFormat.Default : RenderTextureFormat.DefaultHDR;
		var num2 = (float)(Mathf.Log(num1, 2f) + (double)bloom.radius - 8.0);
		var num3 = (int)num2;
		var num4 = Mathf.Clamp(num3, 1, 16);
		var thresholdLinear = bloom.thresholdLinear;
		mat.SetFloat(Uniforms._Threshold, thresholdLinear);
		var num5 = (float)(thresholdLinear * (double)bloom.softKnee + 9.9999997473787516E-06);
		var vector3 = new Vector3(thresholdLinear - num5, num5 * 2f, 0.25f / num5);
		mat.SetVector(Uniforms._Curve, vector3);
		mat.SetFloat(Uniforms._PrefilterOffs, bloom.antiFlicker ? -0.5f : 0.0f);
		var x = 0.5f + num2 - num3;
		mat.SetFloat(Uniforms._SampleScale, x);
		if (bloom.antiFlicker)
			mat.EnableKeyword("ANTI_FLICKER");
		var renderTexture1 = context.renderTextureFactory.Get(width, num1, format: format);
		Graphics.Blit(source, renderTexture1, mat, 0);
		var source1 = renderTexture1;
		for (var index = 0; index < num4; ++index) {
			m_BlurBuffer1[index] = context.renderTextureFactory.Get(Mathf.Max(1, source1.width / 2),
				Mathf.Max(1, source1.height / 2), format: format);
			var pass = index == 0 ? 1 : 2;
			Graphics.Blit(source1, m_BlurBuffer1[index], mat, pass);
			source1 = m_BlurBuffer1[index];
		}

		for (var index = num4 - 2; index >= 0; --index) {
			var renderTexture2 = m_BlurBuffer1[index];
			mat.SetTexture(Uniforms._BaseTex, renderTexture2);
			m_BlurBuffer2[index] =
				context.renderTextureFactory.Get(renderTexture2.width, renderTexture2.height, format: format);
			Graphics.Blit(source1, m_BlurBuffer2[index], mat, 3);
			source1 = m_BlurBuffer2[index];
		}

		var renderTexture3 = source1;
