  ...
  public sealed class UserLutComponent : PostProcessingComponentRenderTexture<UserLutModel>
  {
    public override bool active
    {
      get
      {
        ...
        return model.enabled && settings.lut != null && settings.contribution > 0.0 && settings.lut.height == (int) Mathf.Sqrt(settings.lut.width) && !context.interrupted;
      }
    }
    ...
    {
      UserLutModel.Settings settings = model.settings;
      GUI.DrawTexture(new Rect((float) (context.viewport.x * (double) Screen.width + 8.0), 8f, settings.lut.width, settings.lut.height), settings.lut);
    }
