...
public sealed class MotionBlurComponent : PostProcessingComponentCommandBuffer<MotionBlurModel> {
	...
	public override bool active {
		get {
			...
			return model.enabled &&
			       ((settings.shutterAngle > 0.0 && reconstructionFilter.IsSupported()) ||
			        settings.frameBlending > 0.0) && SystemInfo.graphicsDeviceType != GraphicsDeviceType.OpenGLES2 &&
			       !context.interrupted;
		}
	}
	...
	public override void PopulateCommandBuffer(CommandBuffer cb) {
		if (m_FirstFrame)
			...
			var mat = context.materialFactory.Get("Hidden/Post FX/Blit");
			var settings = model.settings;
	...
	public class ReconstructionFilter {
		...
		public void ProcessImage(
			...
			Material material) {
			var num1 = (int)(5.0 * context.height / 100.0);
			var num2 = ((num1 - 1) / 8 + 1) * 8;
			var num3 = (float)(settings.shutterAngle / 360.0 / (Time.deltaTime * 30.0));
			...
			var velocityTex = Uniforms._VelocityTex;
			cb.GetTemporaryRT(velocityTex, context.width, context.height, 0, FilterMode.Point, m_PackedRTFormat,
				RenderTextureReadWrite.Linear);
			cb.Blit(null, velocityTex, material, 0);
			var tile2Rt = Uniforms._Tile2RT;
			cb.GetTemporaryRT(tile2Rt, context.width / 2, context.height / 2, 0, FilterMode.Point, m_VectorRTFormat,
				RenderTextureReadWrite.Linear);
			cb.SetGlobalTexture(Uniforms._MainTex, velocityTex);
			cb.Blit(velocityTex, tile2Rt, material, 1);
			var tile4Rt = Uniforms._Tile4RT;
			cb.GetTemporaryRT(tile4Rt, context.width / 4, context.height / 4, 0, FilterMode.Point, m_VectorRTFormat,
				RenderTextureReadWrite.Linear);
			cb.SetGlobalTexture(Uniforms._MainTex, tile2Rt);
			cb.Blit(tile2Rt, tile4Rt, material, 2);
			cb.ReleaseTemporaryRT(tile2Rt);
			var tile8Rt = Uniforms._Tile8RT;
			cb.GetTemporaryRT(tile8Rt, context.width / 8, context.height / 8, 0, FilterMode.Point, m_VectorRTFormat,
				RenderTextureReadWrite.Linear);
			cb.SetGlobalTexture(Uniforms._MainTex, tile4Rt);
			cb.Blit(tile4Rt, tile8Rt, material, 2);
			cb.ReleaseTemporaryRT(tile4Rt);
			var vector2 = Vector2.one * (float)(num2 / 8.0 - 1.0) * -0.5f;
			cb.SetGlobalVector(Uniforms._TileMaxOffs, vector2);
			cb.SetGlobalFloat(Uniforms._TileMaxLoop, (int)(num2 / 8.0));
			var tileVrt = Uniforms._TileVRT;
			cb.GetTemporaryRT(tileVrt, context.width / num2, context.height / num2, 0, FilterMode.Point,
			...
			var neighborMaxTex = Uniforms._NeighborMaxTex;
			var width = context.width / num2;
			var height = context.height / num2;
			cb.GetTemporaryRT(neighborMaxTex, width, height, 0, FilterMode.Point, m_VectorRTFormat,
				RenderTextureReadWrite.Linear);
	...
	public class FrameBlendingFilter {
		...
		public void BlendFrames(
			...
			Material material) {
			var time = Time.time;
			var frameRelative1 = GetFrameRelative(-1);
			var frameRelative2 = GetFrameRelative(-2);
			var frameRelative3 = GetFrameRelative(-3);
			var frameRelative4 = GetFrameRelative(-4);
			...
			cb.SetGlobalTexture(Uniforms._History1ChromaTex,
				(RenderTargetIdentifier)(Texture)frameRelative1.chromaTexture);
			cb.SetGlobalTexture(Uniforms._History2ChromaTex,
				(RenderTargetIdentifier)(Texture)frameRelative2.chromaTexture);
			cb.SetGlobalTexture(Uniforms._History3ChromaTex,
				(RenderTargetIdentifier)(Texture)frameRelative3.chromaTexture);
			cb.SetGlobalTexture(Uniforms._History4ChromaTex,
				(RenderTargetIdentifier)(Texture)frameRelative4.chromaTexture);
		...
		private static RenderTextureFormat GetPreferredRenderTextureFormat() {
			var renderTextureFormatArray = new RenderTextureFormat[3] {
				RenderTextureFormat.RGB565,
				RenderTextureFormat.ARGB1555,
				RenderTextureFormat.ARGB4444
			};
			foreach (var format in renderTextureFormatArray)
		...
		private struct Frame {
			...
			public float CalculateWeight(float strength, float currentTime) {
				if (Mathf.Approximately(m_Time, 0.0f))
					return 0.0f;
				var num = Mathf.Lerp(80f, 16f, strength);
