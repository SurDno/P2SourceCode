...
public sealed class TaaComponent : PostProcessingComponentRenderTexture<AntialiasingModel> {
	...
	public override bool active => model.enabled && model.settings.method == AntialiasingModel.Method.Taa &&
	...
	public void SetProjectionMatrix(Func<Vector2, Matrix4x4> jitteredFunc) {
		var taaSettings = model.settings.taaSettings;
		var offset = GenerateRandomOffset() * taaSettings.jitterSpread;
		...
		if (jitteredFunc != null)
			...
			context.camera.projectionMatrix = context.camera.orthographic
				? GetOrthographicProjectionMatrix(offset)
				: GetPerspectiveProjectionMatrix(offset);
	...
	public void Render(RenderTexture source, RenderTexture destination) {
		var material = context.materialFactory.Get("Hidden/Post FX/Temporal Anti-aliasing");
		...
		material.SetVector(Uniforms._FinalBlendParameters,
			new Vector4(taaSettings.stationaryBlending, taaSettings.motionBlending, 6000f, 0.0f));
		material.SetTexture(Uniforms._MainTex, source);
		material.SetTexture(Uniforms._HistoryTex, m_HistoryTexture);
		var temporary = RenderTexture.GetTemporary(source.width, source.height, 0, source.format);
	...
	private Vector2 GenerateRandomOffset() {
		var randomOffset =
			new Vector2(GetHaltonValue(m_SampleIndex & 1023, 2), GetHaltonValue(m_SampleIndex & 1023, 3));
		if (++m_SampleIndex >= 8)
			m_SampleIndex = 0;
		return randomOffset;
	}
	...
	private Matrix4x4 GetPerspectiveProjectionMatrix(Vector2 offset) {
		var num1 = Mathf.Tan((float)Math.PI / 360f * context.camera.fieldOfView);
		var num2 = num1 * context.camera.aspect;
		offset.x *= num2 / (0.5f * context.width);
		offset.y *= num1 / (0.5f * context.height);
		var num3 = (offset.x - num2) * context.camera.nearClipPlane;
		var num4 = (offset.x + num2) * context.camera.nearClipPlane;
		var num5 = (offset.y + num1) * context.camera.nearClipPlane;
		var num6 = (offset.y - num1) * context.camera.nearClipPlane;
		return new Matrix4x4 {
			...
			[2, 0] = 0.0f,
			[2, 1] = 0.0f,
			[2, 2] = (float)(-(context.camera.farClipPlane + (double)context.camera.nearClipPlane) /
			                 (context.camera.farClipPlane - (double)context.camera.nearClipPlane)),
			[2, 3] = (float)(-(2.0 * context.camera.farClipPlane * context.camera.nearClipPlane) /
			                 (context.camera.farClipPlane - (double)context.camera.nearClipPlane)),
			[3, 0] = 0.0f,
			[3, 1] = 0.0f,
			[3, 2] = -1f,
	...
	private Matrix4x4 GetOrthographicProjectionMatrix(Vector2 offset) {
		var orthographicSize = context.camera.orthographicSize;
		var num = orthographicSize * context.camera.aspect;
		offset.x *= num / (0.5f * context.width);
		offset.y *= orthographicSize / (0.5f * context.height);
		var left = offset.x - num;
		var right = offset.x + num;
		var top = offset.y + orthographicSize;
		var bottom = offset.y - orthographicSize;
