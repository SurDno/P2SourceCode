...
public sealed class DepthOfFieldComponent : PostProcessingComponentRenderTexture<DepthOfFieldModel> {
	...
	private float CalculateFocalLength() {
		...
		return !settings.useCameraFov
			? settings.focalLength / 1000f
			: 0.012f / Mathf.Tan(0.5f * (context.camera.fieldOfView * ((float)Math.PI / 180f)));
	}
	...
	private bool CheckHistory(int width, int height) {
		return m_CoCHistory != null && m_CoCHistory.IsCreated() && m_CoCHistory.width == width &&
	...
	public void Prepare(
		...
		float taaBlending) {
		var settings = model.settings;
		var format1 = RenderTextureFormat.ARGBHalf;
		var format2 = SelectFormat(RenderTextureFormat.R8, RenderTextureFormat.RHalf);
		var focalLength = CalculateFocalLength();
		var x = Mathf.Max(settings.focusDistance, focalLength);
		var num = source.width / (float)source.height;
		var y = (float)(focalLength * (double)focalLength /
		                (settings.aperture * (x - (double)focalLength) * 0.024000000208616257 * 2.0));
		var maxCoCradius = CalculateMaxCoCRadius(source.height);
		var mat = context.materialFactory.Get("Hidden/Post FX/Depth Of Field");
		...
		var renderTexture1 = context.renderTextureFactory.Get(Mathf.Max(1, context.width), Mathf.Max(1, context.height),
			format: format2);
		Graphics.Blit(null, renderTexture1, mat, 0);
		if (antialiasCoC) {
			mat.SetTexture(Uniforms._CoCTex, renderTexture1);
			var z = CheckHistory(context.width, context.height) ? taaBlending : 0.0f;
			mat.SetVector(Uniforms._TaaParams, new Vector3(taaJitter.x, taaJitter.y, z));
			var temporary = RenderTexture.GetTemporary(context.width, context.height, 0, format2);
		...
		var width = Mathf.Max(1, context.width / 2);
		var height = Mathf.Max(1, context.height / 2);
		var renderTexture2 = context.renderTextureFactory.Get(width, height, format: format1);
		mat.SetTexture(Uniforms._CoCTex, renderTexture1);
		Graphics.Blit(source, renderTexture2, mat, 2);
		var renderTexture3 = context.renderTextureFactory.Get(width, height, format: format1);
