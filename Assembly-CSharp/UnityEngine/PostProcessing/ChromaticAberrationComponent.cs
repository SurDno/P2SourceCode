...
public sealed class ChromaticAberrationComponent :
	...
	public override void Prepare(Material uberMaterial) {
		...
		var texture2D1 = settings.spectralTexture;
		if (texture2D1 == null) {
			if (m_SpectrumLut == null) {
				var texture2D2 = new Texture2D(3, 1, TextureFormat.RGB24, false);
	...
	private static class Uniforms {
		internal static readonly int _ChromaticAberration_Amount =
			Shader.PropertyToID(nameof(_ChromaticAberration_Amount));

		internal static readonly int _ChromaticAberration_Spectrum =
			Shader.PropertyToID(nameof(_ChromaticAberration_Spectrum));
	}
}