...
public class PostProcessingBehaviour : MonoBehaviour {
	...
	private List<PostProcessingComponentBase> m_ComponentsToEnable = new();
	private List<PostProcessingComponentBase> m_ComponentsToDisable = new();
	...
	private void OnPostRender() {
		if (profile == null || m_Camera == null || m_RenderingInSceneView || !m_Taa.active ||
		    profile.debugViews.willInterrupt)
			return;
	...
	private void OnRenderImage(RenderTexture source, RenderTexture destination) {
		if (profile == null || m_Camera == null)
			...
			var flag1 = false;
			var active = m_Fxaa.active;
			var antialiasCoC = m_Taa.active && !m_RenderingInSceneView;
			var flag2 = m_DepthOfField.active && !m_RenderingInSceneView;
			var material1 = m_MaterialFactory.Get("Hidden/Post FX/Uber Shader");
			material1.shaderKeywords = null;
			var renderTexture1 = source;
			var renderTexture2 = destination;
			if (antialiasCoC) {
				var destination1 = m_RenderTextureFactory.Get(renderTexture1);
			...
			if (flag2) {
				flag1 = true;
				m_DepthOfField.Prepare(renderTexture1, material1, antialiasCoC, m_Taa.jitterVector,
			...
			var flag3 = flag1 | TryPrepareUberImageEffect(m_ChromaticAberration, material1) |
			            TryPrepareUberImageEffect(m_ColorGrading, material1) |
			            TryPrepareUberImageEffect(m_Vignette, material1) |
			            TryPrepareUberImageEffect(m_UserLut, material1);
			var material2 = active ? m_MaterialFactory.Get("Hidden/Post FX/FXAA") : null;
			if (active) {
				material2.shaderKeywords = null;
				TryPrepareUberImageEffect(m_Grain, material2);
				TryPrepareUberImageEffect(m_Dithering, material2);
				if (flag3) {
					var dest = m_RenderTextureFactory.Get(renderTexture1);
				...
				flag3 = flag3 | TryPrepareUberImageEffect(m_Grain, material1) |
				        TryPrepareUberImageEffect(m_Dithering, material1);
				if (flag3) {
	...
	private void OnDisable() {
		foreach (var keyValuePair in m_CommandBuffers.Values) {
	...
	private void CheckObservers() {
		foreach (var componentState in m_ComponentStates) {
			var key = componentState.Key;
			var enabled = key.GetModel().enabled;
		...
		for (var index = 0; index < m_ComponentsToDisable.Count; ++index) {
			var key = m_ComponentsToDisable[index];
			m_ComponentStates[key] = false;
			key.OnDisable();
		}

		for (var index = 0; index < m_ComponentsToEnable.Count; ++index) {
			var key = m_ComponentsToEnable[index];
	...
	private CommandBuffer AddCommandBuffer<T>(CameraEvent evt, string name) where T : PostProcessingModel {
		...
		var keyValuePair = new KeyValuePair<CameraEvent, CommandBuffer>(evt, commandBuffer);
	...
	private void TryExecuteCommandBuffer<T>(PostProcessingComponentCommandBuffer<T> component)
		...
		if (component.active) {
			var commandBuffer = GetCommandBuffer<T>(component.GetCameraEvent(), component.GetName());
			commandBuffer.Clear();
