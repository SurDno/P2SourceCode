...
public sealed class GrainComponent : PostProcessingComponentRenderTexture<GrainModel> {
	...
	public override bool active => model.enabled && model.settings.intensity > 0.0 &&
	...
	public override void Prepare(Material uberMaterial) {
		...
		if (m_GrainLookupRT == null || !m_GrainLookupRT.IsCreated()) {
			GraphicsUtils.Destroy(m_GrainLookupRT);
			var renderTexture = new RenderTexture(192, 192, 0, RenderTextureFormat.ARGBHalf);
		...
		var mat = context.materialFactory.Get("Hidden/Post FX/Grain Generator");
		...
		uberMaterial.SetVector(Uniforms._Grain_Params2,
			new Vector4(context.width / (float)m_GrainLookupRT.width / settings.size,
				context.height / (float)m_GrainLookupRT.height / settings.size, z, w));
	}
