...
public abstract class EnginePointerInputModule : BaseInputModule {
	...
	protected static PointerEventData.FramePressState StateForMouseButton(int buttonId) {
		var mouseButtonDown = Input.GetMouseButtonDown(buttonId);
		var mouseButtonUp = Input.GetMouseButtonUp(buttonId);
	...
	protected virtual MouseState GetMousePointerEventData() {
		...
		var pointerData = GetPointerData(-1, out data1, true);
		data1.Reset();
		if (pointerData)
			data1.position = CursorService.Instance.Position;
		var position = CursorService.Instance.Position;
	...
	protected virtual void ProcessMove(PointerEventData pointerEvent) {
		var gameObject = pointerEvent.pointerCurrentRaycast.gameObject;
		HandlePointerExitAndEnter(pointerEvent, gameObject);
	}

	protected virtual void ProcessDrag(PointerEventData pointerEvent) {
		var flag = pointerEvent.IsPointerMoving();
		if (flag && pointerEvent.pointerDrag != null && !pointerEvent.dragging && ShouldStartDrag(
			    pointerEvent.pressPosition, pointerEvent.position, eventSystem.pixelDragThreshold,
	...
	protected void ClearSelection() {
		...
		foreach (var currentPointerData in m_PointerData.Values)
