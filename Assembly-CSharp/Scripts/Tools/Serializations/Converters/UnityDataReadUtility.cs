...
public static class UnityDataReadUtility {
	...
	public static GradientAlphaKey ReadGradientAlphaKey(IDataReader reader) {
		return new GradientAlphaKey(DefaultConverter.ParseFloat(reader.ReadSimple("Alpha")),
			DefaultConverter.ParseFloat(reader.ReadSimple("Time")));
	}

	public static GradientColorKey ReadGradientColorKey(IDataReader reader) {
		return new GradientColorKey(
			new Color(DefaultConverter.ParseFloat(reader.ReadSimple("R")),
				DefaultConverter.ParseFloat(reader.ReadSimple("G")),
				DefaultConverter.ParseFloat(reader.ReadSimple("B")),
				DefaultConverter.ParseFloat(reader.ReadSimple("A"))),
			DefaultConverter.ParseFloat(reader.ReadSimple("Time")));
	}
	...
	public static UnitySubAsset<T> Read<T>(IDataReader reader, string name, UnitySubAsset<T> value) where T : Object {
		...
		return child == null
			? new UnitySubAsset<T>()
			: new UnitySubAsset<T>(DefaultDataReadUtility.Read(child, "Id", value.Id),
				DefaultDataReadUtility.Read(child, "Name", value.Name));
	}
	...
	public static UnityAsset<T> Read<T>(IDataReader reader, string name, UnityAsset<T> value) where T : Object {
		...
		return child == null
			? new UnityAsset<T>()
			: new UnityAsset<T>(DefaultDataReadUtility.Read(child, "Id", value.Id));
	}
	...
	public static List<GradientAlphaKey> ReadList(
		...
		foreach (var child in reader.GetChild(name).GetChilds()) {
			var gradientAlphaKey = ReadGradientAlphaKey(child);
	...
	public static List<GradientColorKey> ReadList(
		...
		value.Clear();
		foreach (var child in reader.GetChild(name).GetChilds()) {
			var gradientColorKey = ReadGradientColorKey(child);
	...
	public static List<Typed<T>> ReadList<T>(IDataReader reader, string name, List<Typed<T>> value)
		...
		if (value == null)
			...
			value.Clear();
		foreach (var child in reader.GetChild(name).GetChilds()) {
			var guid = DefaultConverter.ParseGuid(child.ReadSimple("Id"));
	...
	public static List<UnitySubAsset<T>> ReadList<T>(
		...
		foreach (var child in reader.GetChild(name).GetChilds()) {
			var guid = DefaultConverter.ParseGuid(child.ReadSimple("Id"));
			var name1 = child.ReadSimple("Name");
