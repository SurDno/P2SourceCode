...
public static class BehaviorTreeDataWriteUtility {
	public static void WriteShared<T>(IDataWriter writer, string name, T value, bool common = false)
		where T : SharedVariable {
		if (value == null)
			return;
		if (value is ISerializeDataWrite serializeDataWrite) {
			var type = ProxyFactory.GetType(value.GetType());
	...
	public static void WriteCommonSharedList<T>(IDataWriter writer, string name, List<T> value)
		where T : SharedVariable {
	...
	public static void WriteUnity<T>(IDataWriter writer, string name, T value) where T : Object {
		if (value != null) {
			var objectIndex =
				BehaviorTreeDataContext.GetObjectIndex(value, BehaviorTreeDataContext.ContextUnityObjects);
