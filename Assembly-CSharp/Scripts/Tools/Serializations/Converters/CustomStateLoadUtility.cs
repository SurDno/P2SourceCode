...
public static class CustomStateLoadUtility {
	...
	public static List<T> LoadListReferences<T>(IDataReader reader, string name, List<T> value) where T : class {
		...
		foreach (var child in reader.GetChild(name).GetChilds()) {
			var obj = (T)CustomReferenceUtility.LoadReference(child, typeof(T));
			value.Add(obj);
		}
	...
	public static List<IParameter> LoadListParameters(
		...
		foreach (var child1 in reader.GetChild(name).GetChilds()) {
			var child2 = child1.GetChild("Name");
			if (child2 == null)
				...
				var parameter = value.FirstOrDefault(o => o.Name == parameterName);
				if (parameter != null) {
					if (!(parameter is ISerializeStateLoad serializeStateLoad))
						Logger.AddError("Type : " + TypeUtility.GetTypeName(parameter.GetType()) + " is not " +
						                typeof(ISerializeStateLoad));
					else {
						var type = ProxyFactory.GetType(parameter.GetType());
	...
	public static List<IComponent> LoadListComponents(
		...
		if (child1 != null)
			foreach (var child2 in child1.GetChilds()) {
				var dataType = child2.GetDataType();
				for (var index = 0; index < value.Count; ++index) {
					var component = value[index];
					if (component != null) {
						var type = ProxyFactory.GetType(component.GetType());
						if (type.Name == dataType) {
							if (!(component is ISerializeStateLoad serializeStateLoad))
								Logger.AddError("Type : " + TypeUtility.GetTypeName(component.GetType()) + " is not " +
								                typeof(ISerializeStateLoad));
							else
