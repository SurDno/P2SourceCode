...
public class EnemyBase : MonoBehaviour, IEntityAttachable {
	...
	[Inspected] private HashSet<EnemyBase> attackers = new();

	public event Action<EnemyBase> WasPrepunchEvent;

	public void FireWasWasPrepunchEvent() {
		var wasPrepunchEvent = WasPrepunchEvent;
	...
	public void FireWasPunchedEvent(EnemyBase enemy) {
		var wasPunchedEvent = WasPunchedEvent;
	...
	public void FireWasPunchedToSurrenderEvent(EnemyBase enemy) {
		var toSurrenderEvent = WasPunchedToSurrenderEvent;
	...
	public void FireWasLowStaminaPunchedEvent(EnemyBase enemy) {
		var staminaPunchedEvent = WasLowStaminaPunchedEvent;
	...
	public void FireWasStaggeredEvent(EnemyBase enemy) {
		var wasStaggeredEvent = WasStaggeredEvent;
	...
	public void FireWasPunchedToBlockEvent(EnemyBase enemy) {
		var punchedToBlockEvent = WasPunchedToBlockEvent;
	...
	public void FireWasPunchedToQuickBlockEvent(EnemyBase enemy) {
		var punchedToQuickBlock = WasPunchedToQuickBlock;
	...
	public void FireWasPunchedToDodgeEvent(EnemyBase enemy) {
		var punchedToDodgeEvent = WasPunchedToDodgeEvent;
	...
	public void FireWasPunchedToStaggerEvent(EnemyBase enemy) {
		var punchedToStaggerEvent = WasPunchedToStaggerEvent;
	...
	public void FireWasPushedEvent(EnemyBase enemy) {
		var wasPushedEvent = WasPushedEvent;
	...
	public void FireWasKnockDownedEvent(EnemyBase enemy) {
		var knockDownedEvent = WasKnockDownedEvent;
	...
	public void FirePunchHitEvent(WeaponEnum weapon) {
		var punchHitEvent = PunchHitEvent;
		if (punchHitEvent == null)
			return;
		punchHitEvent(weapon);
	}

	[Inspected] public virtual bool IsStagger => false;

	[Inspected] public float StaggerTime { get; set; }
	...
	public Vector3 CalculateRepulseVelocity(EnemyBase attacker) {
		...
		foreach (var attacker1 in attackers)
			if (!(attacker1 == attacker)) {
				var vector3 = attacker.transform.position - attacker1.transform.position;
				var num3 = num1 * num1 / vector3.sqrMagnitude;
	...
	protected void CaclulateRection(
		...
		var normalized = (position - transform.position).normalized;
	...
	public void PlayLipSync(CombatCryEnum cryEnum) {
		...
		if (entity != null) {
			var component = entity.GetComponent<LipSyncComponent>();
			var soundBankCrySettings = pivot.SoundBank.CombatCries.Find(x => x.Name == cryEnum);
			if (component != null && soundBankCrySettings != null &&
			    soundBankCrySettings.Chance > (double)Random.value) {
				var description = soundBankCrySettings.Description;
				component.Play3D(description.Value,
					ScriptableObjectInstance<ResourceFromCodeData>.Instance.AudioSourceForNpcCombatReplics, true);
			}
		}
	}

	private void OnEnable() {
		ServiceLocator.GetService<CombatService>()?.RegisterCharacter(this);
	}
	...
	[Inspected] public virtual Vector3 Velocity { get; }
