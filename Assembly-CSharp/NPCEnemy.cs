...
public class NPCEnemy : EnemyBase {
	...
	[SerializeField] private NPCWeaponService weaponService;

	[Header("Other")]
	[Tooltip(
		"Обычно реакции на удары накладываются аддитивно, чтобы проигрываться поверх боевых. Но для персонажей, которые не дерутся могут применяться неаддитивные анимации реакций. В этих анимациях возможны сильные смещения.")]
	[SerializeField]
	...
	private List<float> punches = new();
	...
	public void TriggerAction(WeaponActionEnum weaponAction) {
		...
		if (weaponAction != WeaponActionEnum.JabAttack && weaponAction != WeaponActionEnum.KnockDown &&
		    weaponAction != WeaponActionEnum.StepAttack && weaponAction != WeaponActionEnum.Uppercut)
			return;
	...
	[Inspected] public float AttackCooldownTimeLeft { get; set; }
	...
	[Inspected] public float QuickBlockProbability { get; set; }

	[Inspected] public float DodgeProbability { get; set; }
	...
	public int GetPunchesCount(float perTime) {
		if (punches.Count == 0)
			return 0;
		var punchesCount = 0;
		for (var index = punches.Count - 1; index >= 0 && punches[index] <= (double)perTime; --index)
			++punchesCount;
		return punchesCount;
	}
	...
	private void UpdateBlockStance() {
		if (!(animator != null))
			return;
		animator.SetFloat("Fight.BlockStance",
			Mathf.MoveTowards(animator.GetFloat("Fight.BlockStance"), BlockStance ? 1f : 0.0f, Time.deltaTime / 0.5f));
	}
	...
	public float GetAimingRotationDelta() {
		return aimingDeltaRotation;
	}
	...
	private void UpdateRotation() {
		if (IsDead || InstanceByRequest<EngineApplication>.Instance.IsPaused || RotationTarget == null ||
		    fightAnimatorState.Condition == FightAnimatorBehavior.AnimatorState.NPCCondition.EscapeBegin ||
		    fightAnimatorState.IsStagger)
			return;
		var retreatAngle = RetreatAngle;
		if (!retreatAngle.HasValue) {
			if (RotateByPath)
				SetRotation(Quaternion.RotateTowards(transform.rotation,
					Quaternion.LookRotation(agent.steeringTarget - transform.position),
					rotationSpeed * Time.deltaTime));
			else {
				var forward = RotationTarget.position - transform.position;
			...
			var vector3 = RotationTarget.position - transform.position;
			if (vector3 == Vector3.zero)
				return;
			vector3.Normalize();
			var quaternion1 = Quaternion.LookRotation(-vector3);
			retreatAngle = RetreatAngle;
			if (retreatAngle.HasValue) {
				var quaternion2 = quaternion1;
				retreatAngle = RetreatAngle;
				var quaternion3 = Quaternion.AngleAxis(retreatAngle.Value, Vector3.up);
				quaternion1 = quaternion2 * quaternion3;
			}

			var rotationSpeed = this.rotationSpeed;
			var num1 = Mathf.InverseLerp(0.0f, 40f, Quaternion.Angle(transform.rotation, quaternion1));
			var num2 = Mathf.Lerp(0.0f, 1f, num1 * num1);
			var num3 = rotationSpeed * num2;
	...
	public override void OnExternalAnimatorMove() {
		...
		if (agent.isActiveAndEnabled && agent.isOnNavMesh) {
			...
			var vector3_2 = gameObject.transform.position + animator.deltaPosition + vector3_1;
	...
	private bool Dodge(EnemyBase enemy) {
		...
		var num = 1.5f;
		var y = Vector3.Cross(-enemy.transform.forward, transform.forward).y;
		var flag1 = y < 0.0;
		var flag2 = PathfindingHelper.IsFreeSpace(transform.position, transform.position - transform.forward * num);
	...
	private void ReactionBase(float reactionLayerWeight, EnemyBase enemy, ReactionType reactionType) {
		...
		if ((bool)(Object)pivot)
			pivot.RagdollApplyImpulseToHead(25f *
			                                transform.TransformVector(new Vector3(-fightReactionX, 0.0f,
				                                -fightReactionY)));
	...
	private void ReactionBaseEnemy(
		...
		var normalized = (enemy.transform.position - transform.position).normalized;
		var y = Vector3.Cross(normalized, transform.forward).y;
	...
	public override void PrepunchUppercut(
		...
		WeaponEnum weapon,
		EnemyBase enemy) {
		if (IsCombatIgnored || weaponService == null || weaponService.Weapon == WeaponEnum.Unknown || IsSurrender ||
		    BlockStance || fightAnimatorState.IsCheating || fightAnimatorState.IsAttacking || IsQuickBlock)
			return;
	...
	public void Fire() {
		animatorState.SetTrigger("Fight.Triggers/Fire");
	}
}