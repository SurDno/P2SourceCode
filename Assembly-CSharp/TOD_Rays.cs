...
public class TOD_Rays : TOD_ImageEffect {
	...
	[Tooltip("The number of blur iterations to be performed.")] [TOD_Range(0.0f, 4f)]
	public int BlurIterations = 2;

	[Tooltip("The radius to blur filter applied to the god rays.")] [TOD_Min(0.0f)]
	public float BlurRadius = 2f;

	[Tooltip("The intensity of the god rays.")] [TOD_Min(0.0f)]
	public float Intensity = 1f;

	[Tooltip("The maximum radius of the god rays.")] [TOD_Min(0.0f)]
	...
	protected void OnRenderImage(RenderTexture source, RenderTexture destination) {
		if (!CheckSupport(UseDepthTexture))
			...
			var viewportPoint = cam.WorldToViewportPoint(sky.Components.LightTransform.position);
			godRayMaterial.SetVector("_BlurRadius4", new Vector4(1f, 1f, 0.0f, 0.0f) * BlurRadius);
			godRayMaterial.SetVector("_LightPosition",
				new Vector4(viewportPoint.x, viewportPoint.y, viewportPoint.z, MaxRadius));
			var temporary1 = RenderTexture.GetTemporary(width, height, depthBuffer);
			if (UseDepthTexture)
			...
			for (var index = 0; index < BlurIterations; ++index) {
				...
				var num3 = (float)(BlurRadius * ((index * 2.0 + 2.0) * 6.0) / 768.0);
			...
			if (viewportPoint.z >= 0.0)
				color = !sky.IsDay
					? Intensity * sky.MoonVisibility * sky.MoonRayColor
					: Intensity * sky.SunVisibility * sky.SunRayColor;
