...
public class FirstPersonController : MonoBehaviour {
	...
	[SerializeField] [Range(0.0f, 1f)] private float m_RunstepLenghten;
	...
	private void FixedUpdate() {
		float speed;
		GetInput(out speed);
		var vector = transform.forward * m_Input.y + transform.right * m_Input.x;
		RaycastHit hitInfo;
		Physics.SphereCast(transform.position, m_CharacterController.radius, Vector3.down, out hitInfo,
			m_CharacterController.height / 2f, -1, QueryTriggerInteraction.Ignore);
		var normalized = Vector3.ProjectOnPlane(vector, hitInfo.normal).normalized;
