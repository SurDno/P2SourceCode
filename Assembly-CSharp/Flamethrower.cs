...
public class Flamethrower : MonoBehaviour {
	[Header("Physics")] public float EventBufferingTime = 0.25f;
	[Header("Audio")] public AudioSource AudioSource;
	public AudioClip StartClip;
	public AudioClip EndClip;
	[Header("State")] public bool Fire;
	...
	private void Awake() {
		...
		for (var index = 0; index < particleSystems.Length; ++index) {
			var collision = particleSystems[index].collision;
			if (collision.enabled && collision.sendCollisionMessages) {
				ref var local = ref collision;
				local.collidesWith = local.collidesWith |
				                     ScriptableObjectInstance<GameSettingsData>.Instance.FlamethrowerLayer;
			}
		}
	...
	private void OnParticleCollision(GameObject go) {
		if (!ScriptableObjectInstance<GameSettingsData>.Instance.FlamethrowerLayer.Contains(go.layer))
			return;
		var componentInParent = go.GetComponentInParent<IFlamable>();
	...
	public void SetIndoor(bool indoor) {
		AudioSource.outputAudioMixerGroup = indoor
