...
public class AmplifyColorBase : MonoBehaviour {
	...
	[FormerlySerializedAs("UseDithering")] public bool ApplyDithering;
	...
	private List<AmplifyColorVolumeBase> enteredVolumes = new();
	...
	private float effectVolumesBlendAdjusted => Mathf.Clamp01(effectVolumesBlendAdjust < 0.99000000953674316
		? (float)((volumesBlendAmount - (double)effectVolumesBlendAdjust) / (1.0 - effectVolumesBlendAdjust))
		: 1f);
	...
	private void ReportNotSupported() {
		Debug.LogError(
			"[AmplifyColor] This image effect is not supported on this platform. Please make sure your Unity 4 license supports Full-Screen Post-Processing Effects which is usually reserved for Pro licenses. If this is the case, please contact support@amplify.pt");
		enabled = false;
	}
	...
	private void OnEnable() {
		if (SystemInfo.graphicsDeviceName == "Null Device") {
			...
			if (!CheckSupport() || !CreateMaterials())
				return;
			var lutTexture = LutTexture as Texture2D;
			var lutBlendTexture = LutBlendTexture as Texture2D;
			if ((!(lutTexture != null) || lutTexture.mipmapCount <= 1) &&
			    (!(lutBlendTexture != null) || lutBlendTexture.mipmapCount <= 1))
				return;
			Debug.LogError(
	...
	private void Update() {
		...
		if (blending) {
			...
			if (BlendAmount >= 1.0) {
				...
				var onFinishBlend = this.onFinishBlend;
		...
		if (UseVolumes) {
			if (actualTriggerProxy == null) {
				var gameObject = new GameObject(name + "+ACVolumeProxy");
	...
	private void UpdateVolumes() {
		...
		var transform = TriggerVolumeProxy == null ? this.transform : TriggerVolumeProxy;
		...
		for (var index = 0; index < enteredVolumes.Count; ++index) {
			var enteredVolume = enteredVolumes[index];
		...
		var blendTargetLUT = amplifyColorVolumeBase == null ? worldLUT : amplifyColorVolumeBase.LutTexture;
		var blendTimeInSec =
			amplifyColorVolumeBase == null ? ExitVolumeBlendTime : amplifyColorVolumeBase.EnterBlendTime;
		if (volumesBlending && !blendingFromMidBlend && blendTargetLUT == LutTexture) {
			LutTexture = volumesLutBlendTexture;
			volumesLutBlendTexture = blendTargetLUT;
			volumesBlendingTimeCountdown = blendTimeInSec *
			                               ((volumesBlendingTime - volumesBlendingTimeCountdown) / volumesBlendingTime);
			volumesBlendingTime = blendTimeInSec;
			currentVolumeEffects = VolumeEffect.BlendValuesToVolumeEffect(EffectFlags, currentVolumeEffects,
				blendVolumeEffects, effectVolumesBlendAdjusted);
			...
			if (volumesBlending) {
				...
				materialBlendCache.SetTexture("_LerpRgbTex",
					volumesLutBlendTexture != null ? volumesLutBlendTexture : defaultLut);
		...
		blendVolumeEffects = amplifyColorVolumeBase == null
			? worldVolumeEffects
			: amplifyColorVolumeBase.EffectContainer.FindVolumeEffect(this);
	...
	private Texture2D CreateDefaultLut() {
		var texture2D = new Texture2D(1024, 32, TextureFormat.RGB24, false, true);
		...
		var colors = new Color32[32768];
		for (var index1 = 0; index1 < 32; ++index1) {
			var num1 = index1 * 32;
			for (var index2 = 0; index2 < 32; ++index2) {
				var num2 = num1 + index2 * 1024;
				for (var index3 = 0; index3 < 32; ++index3) {
					var num3 = index3 / 31f;
					var num4 = index2 / 31f;
					var num5 = index1 / 31f;
					var r = (byte)(num3 * (double)byte.MaxValue);
					var g = (byte)(num4 * (double)byte.MaxValue);
					var b = (byte)(num5 * (double)byte.MaxValue);
	...
	private Texture2D CreateDepthCurveLut() {
		...
		var texture2D = new Texture2D(1024, 1, TextureFormat.Alpha8, false, true);
		texture2D.hideFlags = HideFlags.HideAndDontSave;
	...
	private void UpdateDepthCurveLut() {
		...
		var index = 0;
		while (index < 1024) {
			depthCurveColors[index].a =
				(byte)Mathf.FloorToInt(Mathf.Clamp01(DepthMaskCurve.Evaluate(time)) * byte.MaxValue);
			++index;
			time += 0.0009775171f;
		}
	...
	private void CheckUpdateDepthCurveLut() {
		...
		if (DepthMaskCurve.length != prevDepthMaskCurve.length)
			...
			while (num < DepthMaskCurve.length) {
				if (Mathf.Abs(DepthMaskCurve.Evaluate(time) - prevDepthMaskCurve.Evaluate(time)) >
	...
	private void CreateHelperTextures() {
		ReleaseTextures();
		var renderTexture1 = new RenderTexture(1024, 32, 0, RenderTextureFormat.ARGB32, RenderTextureReadWrite.Linear);
		...
		var renderTexture2 = new RenderTexture(1024, 32, 0, RenderTextureFormat.ARGB32, RenderTextureReadWrite.Linear);
	...
	private bool CreateMaterials() {
		...
		if (!CheckMaterialAndShader(materialBase, "BaseMaterial") ||
		    ...
		    !CheckMaterialAndShader(materialMaskBlend, "MaskBlendMaterial") ||
		    !CheckMaterialAndShader(materialDepthMask, "DepthMaskMaterial") ||
		    !CheckMaterialAndShader(materialDepthMaskBlend, "DepthMaskBlendMaterial") ||
		    !CheckMaterialAndShader(materialProcessOnly, "ProcessOnlyMaterial"))
			return false;
	...
	private int ComputeShaderPass() {
		var flag1 = QualityLevel == Quality.Mobile;
		var flag2 = colorSpace == ColorSpace.Linear;
		var allowHdr = ownerCamera.allowHDR;
		var num = flag1 ? 18 : 0;
		return !allowHdr
			? num + (flag2 ? 1 : 0)
			: (int)(num + 2 + (flag2 ? 8 : 0) + (ApplyDithering ? 4 : 0) + Tonemapper);
	}
	...
	private void OnRenderImage(RenderTexture source, RenderTexture destination) {
		if (silentError)
			...
			var flag1 = ValidateLutDimensions(LutTexture);
			var flag2 = ValidateLutDimensions(LutBlendTexture);
			var flag3 = LutTexture == null && LutBlendTexture == null && volumesLutBlendTexture == null;
			var source1 = LutTexture == null ? defaultLut : LutTexture;
			var lutBlendTexture = LutBlendTexture;
			var shaderPass = ComputeShaderPass();
			var flag4 = BlendAmount != 0.0 || blending;
			var flag5 = flag4 || (flag4 && lutBlendTexture != null);
			var flag6 = flag5;
			var flag7 = ((!flag1 ? 1 : !flag2 ? 1 : 0) | (flag3 ? 1 : 0)) != 0;
			var mat = !flag7
				? !flag5 && !volumesBlending
					?
					!UseDepthMask ? MaskTexture != null ? materialMask : materialBase : materialDepthMask
					: !UseDepthMask
						? MaskTexture != null ? materialMaskBlend : materialBlend
						: materialDepthMaskBlend
				: materialProcessOnly;
			...
			if (!flag7) {
				...
				if (flag6) {
					...
					if (volumesBlending) {
						renderTexture = RenderTexture.GetTemporary(blendCacheLut.width, blendCacheLut.height,
							blendCacheLut.depth, blendCacheLut.format, RenderTextureReadWrite.Linear);
					...
					materialBlendCache.SetTexture("_LerpRgbTex",
						lutBlendTexture != null ? lutBlendTexture : defaultLut);
					Graphics.Blit(source1, blendCacheLut, materialBlendCache);
