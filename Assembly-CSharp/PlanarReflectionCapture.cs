...
public class PlanarReflectionCapture : MonoBehaviour {
	...
	private Vector4 CameraSpacePlane(Camera cam, Vector3 pos, Vector3 normal, float sideSign) {
		...
		var lhs = worldToCameraMatrix.MultiplyPoint(pos);
		var rhs = worldToCameraMatrix.MultiplyVector(normal).normalized * sideSign;
	...
	private bool CheckObjects() {
		...
		var width = Mathf.RoundToInt(currentCamera.pixelWidth * RelativeResolution);
		var height = Mathf.RoundToInt(currentCamera.pixelHeight * RelativeResolution);
		if (!(bool)(Object)reflectionTexture || reflectionTexture.width != width ||
			...
			reflectionTexture = new RenderTexture(width, height, 24, RenderTextureFormat.ARGBHalf,
		...
		if (!(bool)(Object)reflectionCamera) {
			var gameObject = new GameObject("Planar Reflection Camera", typeof(Camera), typeof(GlobalFogNonopaque),
				typeof(GlobalFog));
	...
	private void OnPreCull() {
		if (!CheckObjects())
			...
			var flag1 = UpdateCondition == Condition.Always;
			var num = DefaultPlaneHeight;
			RaycastHit hitInfo;
			if (Physics.Raycast(transform.position, Vector3.down, out hitInfo, 100f, HeightCollidersLayer,
			...
			var eulerAngles = currentCamera.transform.eulerAngles;
	...
	private void SetCullingDistances(float[] source, float[] destination, float multiplier) {
		var num = source.Length <= destination.Length ? source.Length : destination.Length;
