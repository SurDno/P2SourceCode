...
public class SplineBendMarker : MonoBehaviour {
	...
	[HideInInspector] public Vector3 prewHandle;
	[HideInInspector] public Vector3 nextHandle;
	[HideInInspector] public float dist;
	[HideInInspector] public float percent;
	[HideInInspector] public Vector3[] subPoints = new Vector3[10];
	[HideInInspector] public float[] subPointPercents = new float[10];
	[HideInInspector] public float[] subPointFactors = new float[10];
	[HideInInspector] public float[] subPointMustPercents = new float[10];
	...
	public void Init(SplineBend script, int mnum) {
		...
		if (splineScript.closed && num == splineScript.markers.Length - 1) {
			var marker = splineScript.markers[splineScript.markers.Length - 2];
		...
		if ((bool)(Object)marker2) {
			...
			var num1 = 1f / (subPoints.Length - 1);
			for (var index = 0; index < subPoints.Length; ++index)
				subPoints[index] = SplineBend.AlignPoint(this, marker2, num1 * index, new Vector3(0.0f, 0.0f, 0.0f));
			var num2 = 0.0f;
			subPointPercents[0] = 0.0f;
			var num3 = 1f / (subPoints.Length - 1);
			for (var index = 1; index < subPoints.Length; ++index) {
			...
			for (var index = 0; index < subPoints.Length - 1; ++index)
				subPointFactors[index] = num3 / (subPointPercents[index + 1] - subPointPercents[index]);
		}

		var vector3_1 = new Vector3(0.0f, 0.0f, 0.0f);
		if ((bool)(Object)marker2)
		...
		switch (type) {
			case MarkerType.Smooth:
				...
				nextHandle = Vector3.Slerp(-(marker1.position - position) * 0.333f, (vector3_1 - position) * 0.333f,
					0.5f);
				prewHandle = Vector3.Slerp((marker1.position - position) * 0.333f, -(vector3_1 - position) * 0.333f,
					0.5f);
				break;
			case MarkerType.Transform:
				if ((bool)(Object)marker1)
					prewHandle = -transform.forward * transform.localScale.z * (position - marker1.position).magnitude *
					             0.4f;
				if ((bool)(Object)marker2) {
			...
			case MarkerType.Corner:
				prewHandle = !(bool)(Object)marker1
					? new Vector3(0.0f, 0.0f, 0.0f)
					: (marker1.position - position) * 0.333f;
