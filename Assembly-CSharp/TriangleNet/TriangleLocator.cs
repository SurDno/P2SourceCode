...
internal class TriangleLocator {
	...
	public LocateResult Locate(Point searchpoint, ref Otri searchtri) {
		...
		sampler.Update(mesh);
		foreach (var sample in sampler.GetSamples(mesh)) {
			otri.triangle = mesh.triangles[sample];
			if (!Otri.IsDead(otri.triangle)) {
				var vertex3 = otri.Org();
				var num3 = (searchpoint.X - vertex3.x) * (searchpoint.X - vertex3.x) +
		...
		var num4 = Primitives.CounterClockwise(pa, pb, searchpoint);
		if (num4 < 0.0)
			searchtri.SymSelf();
