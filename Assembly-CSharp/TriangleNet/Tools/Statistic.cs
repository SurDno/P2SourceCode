...
public class Statistic {
	...
	private void GetAspectHistogram(Mesh mesh) {
		...
		var otri = new Otri();
		var vertexArray = new Vertex[3];
		var numArray3 = new double[3];
		var numArray4 = new double[3];
		var numArray5 = new double[3];
		otri.orient = 0;
		foreach (var triangle in mesh.triangles.Values) {
			...
			for (var index1 = 0; index1 < 3; ++index1) {
				var index2 = plus1Mod3[index1];
				var index3 = minus1Mod3[index1];
			...
			var num2 = Math.Abs((vertexArray[2].x - vertexArray[0].x) * (vertexArray[1].y - vertexArray[0].y) -
			                    (vertexArray[1].x - vertexArray[0].x) * (vertexArray[2].y - vertexArray[0].y)) / 2.0;
			var num3 = num2 * num2 / num1;
			var num4 = num1 / num3;
			var index = 0;
	...
	public void Update(Mesh mesh, int sampleDegrees) {
		...
		foreach (var triangle in mesh.triangles.Values) {
			...
			var num6 = Math.Abs((pointArray[2].X - pointArray[0].X) * (pointArray[1].Y - pointArray[0].Y) -
			                    (pointArray[1].X - pointArray[0].X) * (pointArray[2].Y - pointArray[0].Y));
			if (num6 < minArea)
				minArea = num6;
			if (num6 > maxArea)
				maxArea = num6;
			var num7 = num6 * num6 / num5;
			if (num7 < minAspect)
				minAspect = num7;
			var num8 = num5 / num7;
			if (num8 > maxAspect)
			...
			for (var index4 = 0; index4 < 3; ++index4) {
				var index5 = plus1Mod3[index4];
				var index6 = minus1Mod3[index4];
				var num9 = numArray2[index5] * numArray2[index6] + numArray3[index5] * numArray3[index6];
				var num10 = num9 * num9 / (numArray4[index5] * numArray4[index6]);
				var index7 = sampleDegrees / 2 - 1;
				for (var index8 = index7 - 1; index8 >= 0; --index8)
			...
			var index9 = sampleDegrees / 2 - 1;
			for (var index10 = index9 - 1; index10 >= 0; --index10)
				if (num3 > numArray1[index10])
					index9 = index10;
			++minAngles[index9];
			var index11 = sampleDegrees / 2 - 1;
			for (var index12 = index11 - 1; index12 >= 0; --index12)
