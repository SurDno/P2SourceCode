...
public class Voronoi : IVoronoi {
	...
	private void ComputeCircumCenters() {
		...
		var num = Math.Max(bounds.Width, bounds.Height);
	...
	private void ConstructVoronoiRegion(Vertex vertex) {
		...
		if (rayPoints.ContainsKey(hash1))
			...
			var vertex2 = o2_1.Apex();
			BoxRayIntersection(this.points[o2_1.triangle.id], vertex1.y - vertex2.y, vertex2.x - vertex1.x,
				out intersect);
		...
		var hash2 = os.seg.hash;
		if (rayPoints.ContainsKey(hash2))
			points.Add(rayPoints[hash2]);
		else {
			var vertex3 = o2_1.Org();
			var vertex4 = o2_1.Dest();
			BoxRayIntersection(this.points[o2_1.triangle.id], vertex4.y - vertex3.y, vertex3.x - vertex4.x,
				out intersect);
			intersect.id = count + rayIndex;
			this.points[count + rayIndex] = intersect;
			++rayIndex;
