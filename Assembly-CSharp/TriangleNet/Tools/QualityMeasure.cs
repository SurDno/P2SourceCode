...
public class QualityMeasure {
	...
	private void Compute() {
		...
		foreach (var triangle in mesh.triangles.Values) {
			...
			var num1 = vertex1.x - vertex2.x;
			var num2 = vertex1.y - vertex2.y;
			var ab = Math.Sqrt(num1 * num1 + num2 * num2);
			var num3 = vertex2.x - vertex3.x;
			var num4 = vertex2.y - vertex3.y;
			var bc = Math.Sqrt(num3 * num3 + num4 * num4);
			var num5 = vertex3.x - vertex1.x;
			var num6 = vertex3.y - vertex1.y;
			var ca = Math.Sqrt(num5 * num5 + num6 * num6);
			var area = areaMeasure.Measure(vertex1, vertex2, vertex3);
	...
	public int Bandwidth() {
		...
		foreach (var triangle in mesh.triangles.Values)
			for (var index1 = 0; index1 < 3; ++index1) {
				var id1 = triangle.GetVertex(index1).id;
				for (var index2 = 0; index2 < 3; ++index2) {
					var id2 = triangle.GetVertex(index2).id;
	...
	private class AreaMeasure {
		...
		public double Measure(Point a, Point b, Point c) {
			var val2 = 0.5 * Math.Abs(a.x * (b.y - c.y) + b.x * (c.y - a.y) + c.x * (a.y - b.y));
	...
	private class AlphaMeasure {
		...
		public double Measure(double ab, double bc, double ca, double area) {
			...
			var val1 = Math.Min(Math.Min(Math.Min(maxValue, val2_1), val2_2), val2_3) * 3.0 / Math.PI;
			alpha_ave += val1;
	...
	private class Q_Measure {
		...
		public double Measure(double ab, double bc, double ca, double area) {
			var val2 = (bc + ca - ab) * (ca + ab - bc) * (ab + bc - ca) / (ab * bc * ca);
