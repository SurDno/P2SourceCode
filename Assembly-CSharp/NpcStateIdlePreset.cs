...
public class NpcStateIdlePreset : INpcState, INpcStateNeedSyncBack {
	...
	protected void SetIdle() {
		if (preset == null) {
			var gameObject = ((IEntityView)setupPoint).GameObject;
			if (!(gameObject != null))
				return;
			...
			if (preset.PresetAnimation == IdlePresetEnum.StandAggressive ||
			    preset.PresetAnimation == IdlePresetEnum.StandSurrender) {
				combatIdle = true;
				...
				var poiAnimationEnum = preset.GetPOIAnimationEnum();
				if (poiAnimationEnum == POIAnimationEnum.Unknown)
					animatorState.ControlMovableState = AnimatorState45.MovableState45.Idle;
				else {
					var animationSetup = poiSetup.GetAnimationSetup(poiAnimationEnum);
					if (animationSetup != null) {
						animationIndex = !preset.RandomAnimationIndex
							? preset.AnimationIndex
							: Random.Range(0, animationSetup.Elements.Count);
						animationsCount = 1;
						if (animationSetup.Elements.Count > animationIndex &&
						    animationSetup.Elements[animationIndex] is POIAnimationSetupElementSlow)
							animationsCount = (animationSetup.Elements[animationIndex] as POIAnimationSetupElementSlow)
