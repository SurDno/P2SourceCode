...
public class RelayValueOutput<T> : FlowControlNode {
	[SerializeField] private string _sourceInputUID;
	...
	private RelayValueInput<T> sourceInput {
		get {
			if (_sourceInput == null) {
				_sourceInput = graph.GetAllNodesOfType<RelayValueInput<T>>()
	...
	public override string name =>
		string.Format("{0}", sourceInput != null ? sourceInput.ToString() : (object)"@ NONE");

	protected override void RegisterPorts() {
		AddValueOutput("Value", () => sourceInput != null ? sourceInput.port.value : default);
	}
}