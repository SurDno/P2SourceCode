...
public class PortAttribute : MemberAttribute {
	...
	private void ComputeMethod(Container container, MethodInfo method) {
		...
		if (parameters.Length == 0 && method.ReturnType != typeof(void))
			container.GetHandler(Id).AddHandle((target, data) => {
				var instance = (FlowNode)target;
				var type = typeof(ValueHandler<>);
				var typeArray = new Type[1] {
					method.ReturnType
				};
				var getter = method.RTCreateDelegate(type.MakeGenericType(typeArray), instance);
				instance.AddValueOutputCommon(name, method.ReturnType, getter);
			});
		else if (method.ReturnType == typeof(void) && parameters.Length == 0)
			container.GetHandler(Id).AddHandle((target, data) => {
				var instance = (FlowNode)target;
				var pointer = (FlowHandler)method.RTCreateDelegate(typeof(FlowHandler), instance);
	...
	private void ComputeField(Container container, FieldInfo field) {
		...
		if (type.IsGenericType && type.GetGenericTypeDefinition() == typeof(ValueInput<>))
			container.GetHandler(Id).AddHandle((target, data) => {
				var flowNode = (FlowNode)target;
				var input = flowNode.AddValueInput(name, type.GetGenericArguments()[0]);
			...
			container.GetHandler(Id).AddHandle((target, data) => {
				var flowNode = (FlowNode)target;
				var flowOutput = flowNode.AddFlowOutput(name);
	...
	private void SetDefaultValue(ValueInput input, object[] value) {
		if (value == null)
			return;
		var genericArgument = input.GetType().GetGenericArguments()[0];
		if (value.Length == 1 && genericArgument == value[0].GetType())
			input.serializedValue = value[0];
		else {
			var instance = Activator.CreateInstance(genericArgument, value);
