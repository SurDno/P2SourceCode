...
public class SetOtherVariable<T> : FlowNode {
	...
	public override string name =>
		string.Format("${0}{1}Value", varName.value, OperationTools.GetOperationString(operation));
	...
	private void DoSet(Blackboard bb, string name, T value) {
		var variable = bb.GetVariable<T>(name);
		if (operation != 0) {
			if (typeof(T) == typeof(float))
				variable.value =
					(T)(object)OperationTools.Operate((float)(object)variable.value, (float)(object)value, operation);
			else if (typeof(T) == typeof(int))
				variable.value =
					(T)(object)OperationTools.Operate((int)(object)variable.value, (int)(object)value, operation);
			else if (typeof(T) == typeof(Vector3))
				variable.value = (T)(object)OperationTools.Operate((Vector3)(object)variable.value,
