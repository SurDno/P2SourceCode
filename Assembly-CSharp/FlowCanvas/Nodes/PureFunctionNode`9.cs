...
// Type: FlowCanvas.Nodes.PureFunctionNode`9
...
{
  public abstract class PureFunctionNode<TResult, T1, T2, T3, T4, T5, T6, T7, T8> : 
    PureFunctionNodeBase
  {
    ...
    protected override sealed void OnRegisterPorts(FlowNode node)
    {
      ...
      node.AddValueOutput<TResult>("Value", (ValueHandler<TResult>) (() => this.Invoke(p1.value, p2.value, p3.value, p4.value, p5.value, p6.value, p7.value, p8.value)));
    }
  }
}
