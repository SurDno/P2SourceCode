...
// Type: FlowCanvas.Nodes.CallableFunctionNode`8
...
{
  public abstract class CallableFunctionNode<TResult, T1, T2, T3, T4, T5, T6, T7> : 
    CallableFunctionNodeBase
  {
    ...
    protected override sealed void OnRegisterPorts(FlowNode node)
    {
      ...
      {
        this.result = this.Invoke(p1.value, p2.value, p3.value, p4.value, p5.value, p6.value, p7.value);
        o.Call();
      }));
    }
  }
