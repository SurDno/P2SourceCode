...
public class BinderConnection : Connection {
	[SerializeField] private string _sourcePortName;
	[SerializeField] private string _targetPortName;
	[NonSerialized] private Port _sourcePort;
	[NonSerialized] private Port _targetPort;
	...
	public static BinderConnection Create(Port source, Port target) {
		...
		if (!TypeConverter.HasConvertion(source.type, target.type)) {
			Debug.LogWarning(string.Format(
				"Can't connect ports. Type '{0}' is not assignable from Type '{1}' and there exists no internal convertion for those types.",
				target.type.FriendlyName(), source.type.FriendlyName()));
			return null;
		}

		if (source is FlowOutput && target is FlowInput) {
			var binderConnection = new BinderConnection();
		...
		if (!(source is ValueOutput) || !(target is ValueInput))
			return null;
		var instance = (BinderConnection)Activator.CreateInstance(typeof(BinderConnection<>).RTMakeGenericType(
			new Type[1] {
				target.type
			}));
