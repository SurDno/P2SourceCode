...
public class TextTooltipViewInstance : TextTooltipView {
	...
	[SerializeField] private ProgressView opacityView;
	...
	private void SetData(Vector2 screenPosition, string text) {
		...
		var vector2 = new Vector2(Screen.width * 0.5f, Screen.height * 0.5f);
		var transform = (RectTransform)this.transform;
		transform.pivot = new Vector2(screenPosition.x < (double)vector2.x ? 0.0f : 1f,
			screenPosition.y < (double)vector2.y ? 0.0f : 1f);
