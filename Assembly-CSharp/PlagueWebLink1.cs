...
public class PlagueWebLink1 : PlagueWebLink {
	...
	[Space] public float TailLifeTime = 1f;
	...
	private void Rebuild() {
		...
		if (pointA != null) {
			p0 = pointA.Position;
			p1 = new Vector3(p0.x + pointA.Directionality.x, p0.y + pointA.Directionality.y,
				p0.z + pointA.Directionality.z);
		}

		if (pointB != null) {
			p3 = pointB.Position;
			p2 = new Vector3(p3.x + pointB.Directionality.x, p3.y + pointB.Directionality.y,
				p3.z + pointB.Directionality.z);
		}

		for (var index = 0; index < linePositions.Length; ++index)
			linePositions[index] =
				CalculatePosition(
					Mathf.Clamp01((time - TailLifeTime / (linePositions.Length - 1) * index) / FlightTime));
		var component = GetComponent<LineRenderer>();
		component.positionCount = linePositions.Length;
		component.SetPositions(linePositions);
		if (isCanceled) {
			var num = (float)(1.0 - cancelTime / (double)CancelFadeTime);
			if (baseAlphaKeys == null) {
				baseAlphaKeys = component.colorGradient.alphaKeys;
				currentAlphaKeys = new GradientAlphaKey[baseAlphaKeys.Length];
				for (var index = 0; index < currentAlphaKeys.Length; ++index)
	...
	public Vector3 CalculatePosition(float time) {
		...
		var num2 = 1f - time;
		var num3 = num2 * num2;
		var vector3_1 = num3 * num2 * p0 + 3f * num3 * time * p1 + 3f * num2 * num1 * p2 + num1 * time * p3;
		time = Mathf.Abs(time - 0.5f) * 2f;
		time = (float)(1.0 - time * (double)time);
		var vector3_2 = new Vector3((vector3_1.x + noiseSalt.x) * NoiseFrequency,
			(vector3_1.y + noiseSalt.y) * NoiseFrequency, (vector3_1.z + noiseSalt.z) * NoiseFrequency);
		vector3_2 = new Vector3((float)(Mathf.PerlinNoise(vector3_2.z, vector3_2.y) * 2.0 - 1.0),
			(float)(Mathf.PerlinNoise(vector3_2.x, vector3_2.z) * 2.0 - 1.0),
			(float)(Mathf.PerlinNoise(vector3_2.y, vector3_2.x) * 2.0 - 1.0));
		var vector3_3 = vector3_2 * (time * NoiseAmplitude);
