...
public class HUDWeaponsView : EntityView {
	[SerializeField] private EntityView currentItemView;
	[SerializeField] private EntityView[] availableItemViews;
	[SerializeField] private HideableView[] availableItemSelectionViews;
	[SerializeField] protected HideableEventView changeEventView;
	...
	private void AssignAvailableItems() {
		var index = 0;
		foreach (var availableWeaponItem in attacker.AvailableWeaponItems) {
	...
	public virtual void AssignCurrentItem() {
		if (attacker.CurrentWeaponItem == null)
			...
			for (var index = 0; index < availableItemViews.Length; ++index)
