...
public static class TOD_Util {
	...
	public static Color PowRGB(Color color, float power) {
		return power == 1.0
			? color
			: new Color(Mathf.Pow(color.r, power), Mathf.Pow(color.g, power), Mathf.Pow(color.b, power), color.a);
	}

	public static Color PowRGBA(Color color, float power) {
		return power == 1.0
			? color
			: new Color(Mathf.Pow(color.r, power), Mathf.Pow(color.g, power), Mathf.Pow(color.b, power),
				Mathf.Pow(color.a, power));
	}
