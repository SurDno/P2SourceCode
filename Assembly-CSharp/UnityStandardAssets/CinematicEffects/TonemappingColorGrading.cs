...
public class TonemappingColorGrading : MonoBehaviour {
	...
	[SerializeField] [SettingsGroup] private TonemappingSettings m_Tonemapping = TonemappingSettings.defaultSettings;
	[SerializeField] [SettingsGroup] private ColorGradingSettings m_ColorGrading = ColorGradingSettings.defaultSettings;
	[SerializeField] [SettingsGroup] private LUTSettings m_Lut = LUTSettings.defaultSettings;
	...
	private RenderTexture internalLutRt {
		get {
			if (m_InternalLut == null || !m_InternalLut.IsCreated() || m_InternalLut.height != lutSize) {
				...
				var renderTexture = new RenderTexture(lutSize * lutSize, lutSize, 0, RenderTextureFormat.ARGB32);
	...
	private Texture2D curveTexture {
		get {
			if (m_CurveTexture == null) {
				var texture2D = new Texture2D(256, 1, TextureFormat.ARGB32, false, true);
	...
	private Texture2D tonemapperCurve {
		get {
			if (m_TonemapperCurve == null) {
				...
				var texture2D = new Texture2D(256, 1, textureFormat, false, true);
	...
	private static Texture2D GenerateIdentityLut(int dim) {
		var colors = new Color[dim * dim * dim];
		var num = (float)(1.0 / (dim - 1.0));
		for (var index1 = 0; index1 < dim; ++index1) {
			for (var index2 = 0; index2 < dim; ++index2) {
				for (var index3 = 0; index3 < dim; ++index3)
					colors[index1 + index2 * dim + index3 * dim * dim] =
						new Color(index1 * num, Mathf.Abs(index3 * num), index2 * num, 1f);
			}
		}

		var texture2D = new Texture2D(dim * dim, dim, TextureFormat.RGB24, false, true);
		texture2D.name = "Identity LUT";
		texture2D.filterMode = FilterMode.Bilinear;
		texture2D.anisoLevel = 0;
		texture2D.hideFlags = HideFlags.DontSave;
		var identityLut = texture2D;
	...
	private Vector3 GetWhiteBalance() {
		var temperatureShift = colorGrading.basics.temperatureShift;
		var tint = colorGrading.basics.tint;
		var x = (float)(0.3127099871635437 -
		                temperatureShift * (temperatureShift < 0.0 ? 0.10000000149011612 : 0.05000000074505806));
	...
	private void GenerateLiftGammaGain(out Color lift, out Color gamma, out Color gain) {
		var color1 = NormalizeColor(colorGrading.colorWheels.shadows);
		var color2 = NormalizeColor(colorGrading.colorWheels.midtones);
		var color3 = NormalizeColor(colorGrading.colorWheels.highlights);
		var num1 = (float)((color1.r + (double)color1.g + color1.b) / 3.0);
		var num2 = (float)((color2.r + (double)color2.g + color2.b) / 3.0);
		var num3 = (float)((color3.r + (double)color3.g + color3.b) / 3.0);
		var r1 = (float)((color1.r - (double)num1) * 0.10000000149011612);
		var g1 = (float)((color1.g - (double)num1) * 0.10000000149011612);
		var b1 = (float)((color1.b - (double)num1) * 0.10000000149011612);
		var b2 = Mathf.Pow(2f, (float)((color2.r - (double)num2) * 0.5));
		var b3 = Mathf.Pow(2f, (float)((color2.g - (double)num2) * 0.5));
		var b4 = Mathf.Pow(2f, (float)((color2.b - (double)num2) * 0.5));
		var r2 = Mathf.Pow(2f, (float)((color3.r - (double)num3) * 0.5));
		var g2 = Mathf.Pow(2f, (float)((color3.g - (double)num3) * 0.5));
		var b5 = Mathf.Pow(2f, (float)((color3.b - (double)num3) * 0.5));
		var r3 = 1f / Mathf.Max(0.01f, b2);
		var g3 = 1f / Mathf.Max(0.01f, b3);
		var b6 = 1f / Mathf.Max(0.01f, b4);
	...
	private void GenCurveTexture() {
		var master = colorGrading.curves.master;
		var red = colorGrading.curves.red;
		var green = colorGrading.curves.green;
		var blue = colorGrading.curves.blue;
		var colors = new Color[256];
		for (var time = 0.0f; time <= 1.0; time += 0.003921569f) {
			var a = Mathf.Clamp(master.Evaluate(time), 0.0f, 1f);
			var r = Mathf.Clamp(red.Evaluate(time), 0.0f, 1f);
			var g = Mathf.Clamp(green.Evaluate(time), 0.0f, 1f);
			var b = Mathf.Clamp(blue.Evaluate(time), 0.0f, 1f);
	...
	private void OnRenderImage(RenderTexture source, RenderTexture destination) {
		...
		if (eyeAdaptation.enabled) {
			var flag = CheckSmallAdaptiveRt();
			var num1 = source.width < source.height ? source.width : source.height;
			var num2 = num1 | (num1 >> 1);
			var num3 = num2 | (num2 >> 2);
			var num4 = num3 | (num3 >> 4);
			var num5 = num4 | (num4 >> 8);
			var num6 = num5 | (num5 >> 16);
			var num7 = num6 - (num6 >> 1);
			renderTexture = RenderTexture.GetTemporary(num7, num7, 0, m_AdaptiveRtFormat);
			Graphics.Blit(source, renderTexture);
			var length = (int)Mathf.Log(renderTexture.width, 2f);
			var num8 = 2;
			renderTextureArray = new RenderTexture[length];
			for (var index = 0; index < length; ++index) {
				renderTextureArray[index] = RenderTexture.GetTemporary(renderTexture.width / num8,
					renderTexture.width / num8, 0, m_AdaptiveRtFormat);
				num8 <<= 1;
			}

			var source1 = renderTextureArray[length - 1];
			...
			if (tonemapping.tonemapper == Tonemapper.Curve) {
				if (m_TonemapperDirty) {
					...
					if (tonemapping.curve.length > 0) {
						...
						for (var num10 = 0.0f; num10 <= 1.0; num10 += 0.003921569f) {
							var num11 = tonemapping.curve.Evaluate(num10 * num9);
							tonemapperCurve.SetPixel(Mathf.FloorToInt(num10 * byte.MaxValue), 0,
								new Color(num11, num11, num11));
						}
				...
				var num12 = (float)(tonemapping.neutralBlackIn * 20.0 + 1.0);
				var num13 = (float)(tonemapping.neutralBlackOut * 10.0 + 1.0);
				var num14 = tonemapping.neutralWhiteIn / 20f;
				var num15 = (float)(1.0 - tonemapping.neutralWhiteOut / 20.0);
				var t1 = num12 / num13;
				var t2 = num14 / num15;
				material.SetVector("_NeutralTonemapperParams1",
					new Vector4(0.2f, Mathf.Max(0.0f, Mathf.LerpUnclamped(0.57f, 0.37f, t1)),
						Mathf.LerpUnclamped(0.01f, 0.24f, t2), Mathf.Max(0.0f, Mathf.LerpUnclamped(0.02f, 0.2f, t1))));
				material.SetVector("_NeutralTonemapperParams2",
		...
		if (colorGrading.enabled) {
			if (m_Dirty || !m_InternalLut.IsCreated()) {
				...
				material.SetVector("_ContrastGainGamma",
				...
				material.SetVector("_HSV",
					new Vector4(colorGrading.basics.hue, colorGrading.basics.saturation, colorGrading.basics.value));
			...
			material.SetVector("_InternalLutParams",
		...
		if (lut.enabled && lut.texture != null && CheckUserLut()) {
			...
			material.SetVector("_UserLutParams",
				new Vector4(1f / lut.texture.width, 1f / lut.texture.height, lut.texture.height - 1f,
					lut.contribution));
		...
		if (!eyeAdaptation.enabled)
			return;
		for (var index = 0; index < renderTextureArray.Length; ++index)
			RenderTexture.ReleaseTemporary(renderTextureArray[index]);
		RenderTexture.ReleaseTemporary(renderTexture);
	}

	public Texture2D BakeLUT() {
		var texture2D = new Texture2D(internalLutRt.width, internalLutRt.height, TextureFormat.RGB24, false, true);
	...
	public struct EyeAdaptationSettings {
		...
		[Min(0.0f)] [Tooltip("Midpoint Adjustment.")]
		...
		[Min(0.0f)] [Tooltip("Speed of linear adaptation. Higher is faster.")]
		public float speed;
	...
	public struct TonemappingSettings {
		...
		[Min(0.0f)] [Tooltip("Adjusts the overall exposure of the scene.")]
		public float exposure;

		[Tooltip("Custom tonemapping curve.")] public AnimationCurve curve;
	...
	public struct LUTSettings {
		...
		[Range(0.0f, 1f)] [Tooltip("Blending factor.")]
	...
	public struct BasicsSettings {
		...
		[Range(0.0f, 10f)] [Tooltip("Brightens or darkens all colors.")]
		public float value;

		[Space] [Range(0.0f, 2f)] [Tooltip("Expands or shrinks the overall range of tonal values.")]
	...
	public struct CurvesSettings {
		...
		[Curve(1f, 0.0f, 0.0f, 1f)] public AnimationCurve red;
		[Curve(0.0f, 1f, 0.0f, 1f)] public AnimationCurve green;
		[Curve(0.0f, 1f, 1f, 1f)] public AnimationCurve blue;
		...
		public static AnimationCurve defaultCurve =>
			new(new Keyframe(0.0f, 0.0f, 1f, 1f), new Keyframe(1f, 1f, 1f, 1f));
	}
	...
	public struct ColorGradingSettings {
		...
		[Tooltip(
			"Internal LUT precision. \"Normal\" is 256x16, \"High\" is 1024x32. Prefer \"Normal\" on mobile devices.")]
