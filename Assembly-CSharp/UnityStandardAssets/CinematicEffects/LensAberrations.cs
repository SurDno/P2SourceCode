...
public class LensAberrations : MonoBehaviour {
	...
	private void OnRenderImage(RenderTexture source, RenderTexture destination) {
		if (!vignette.enabled && !chromaticAberration.enabled && !distortion.enabled)
			...
			if (distortion.enabled) {
				var num = (float)Math.PI / 180f * Math.Min(160f, 1.6f * Math.Max(Mathf.Abs(distortion.amount), 1f));
				var y = 2f * Mathf.Tan(num * 0.5f);
				var vector4 = new Vector4(distortion.centerX, distortion.centerY,
					Mathf.Max(distortion.amountX, 0.0001f), Mathf.Max(distortion.amountY, 0.0001f));
				var vector3 = new Vector3(distortion.amount >= 0.0 ? num : 1f / num, y, 1f / distortion.scale);
			...
			if (chromaticAberration.enabled) {
				...
				material.SetVector("_ChromaticAberration",
					new Vector4(chromaticAberration.color.r, chromaticAberration.color.g, chromaticAberration.color.b,
						chromaticAberration.amount * (1f / 1000f)));
			}
			...
			if (vignette.enabled) {
				...
				if (vignette.blur > 0.0) {
					...
					var temporaryRenderTexture1 = m_RTU.GetTemporaryRenderTexture(width, height, format: source.format);
					var temporaryRenderTexture2 = m_RTU.GetTemporaryRenderTexture(width, height, format: source.format);
				...
				if (Mathf.Approximately(vignette.roundness, 1f)) {
					...
					material.SetVector("_VignetteSettings",
						new Vector3(vignette.intensity, vignette.smoothness,
							(float)((1.0 - vignette.roundness) * 6.0) + vignette.roundness));
				}
			}
	...
	public struct DistortionSettings {
		...
		[Range(0.0f, 1f)] [Tooltip("Amount multiplier on X axis. Set it to 0 to disable distortion on this axis.")]
		public float amountX;

		[Range(0.0f, 1f)] [Tooltip("Amount multiplier on Y axis. Set it to 0 to disable distortion on this axis.")]
	...
	public struct VignetteSettings {
		...
		[Range(0.0f, 3f)] [Tooltip("Amount of vignetting on screen.")]
		public float intensity;

		[Range(0.01f, 3f)] [Tooltip("Smoothness of the vignette borders.")]
		public float smoothness;

		[AdvancedSetting] [Range(0.0f, 1f)] [Tooltip("Lower values will make a square-ish vignette.")]
		public float roundness;

		[Range(0.0f, 1f)] [Tooltip("Blurs the corners of the screen. Leave this at 0 to disable it.")]
		public float blur;

		[Range(0.0f, 1f)] [Tooltip("Desaturate the corners of the screen. Leave this to 0 to disable it.")]
