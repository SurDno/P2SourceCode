...
public class ScreenSpaceReflection : MonoBehaviour {
	...
	public void OnPreRender() {
		if (material == null || Camera.current.actualRenderingPath != RenderingPath.DeferredShading)
			return;
		var num1 = settings.reflectionSettings.reflectionQuality == SSRResolution.High ? 1 : 2;
		var num2 = camera_.pixelWidth / num1;
		var num3 = camera_.pixelHeight / num1;
		float pixelWidth = camera_.pixelWidth;
		float pixelHeight = camera_.pixelHeight;
		var num4 = pixelWidth / 2f;
		var num5 = pixelHeight / 2f;
		var format = camera_.allowHDR ? RenderTextureFormat.ARGBHalf : RenderTextureFormat.ARGB32;
		...
		material.SetFloat("_PixelsPerMeterAtOneMeter",
			pixelWidth / (-2f * (float)Math.Tan(camera_.fieldOfView / 180.0 * Math.PI * 0.5)));
		...
		var projectionMatrix = camera_.projectionMatrix;
		var vector4 = new Vector4((float)(-2.0 / (pixelWidth * (double)projectionMatrix[0])),
			(float)(-2.0 / (pixelHeight * (double)projectionMatrix[5])),
			(1f - projectionMatrix[2]) / projectionMatrix[0], (1f + projectionMatrix[6]) / projectionMatrix[5]);
		var vector3 = float.IsPositiveInfinity(camera_.farClipPlane)
			? new Vector3(camera_.nearClipPlane, -1f, 1f)
			: new Vector3(camera_.nearClipPlane * camera_.farClipPlane, camera_.nearClipPlane - camera_.farClipPlane,
				camera_.farClipPlane);
		...
		m_CommandBuffer.GetTemporaryRT(kHitPointTexture, num2, num3, 0, FilterMode.Bilinear,
			RenderTextureFormat.ARGBHalf, RenderTextureReadWrite.Linear);
		for (var index = 0; index < 5; ++index)
			m_CommandBuffer.GetTemporaryRT(kReflectionTextures[index], num2 >> index, num3 >> index, 0,
				FilterMode.Bilinear, format);
		m_CommandBuffer.GetTemporaryRT(kFilteredReflections, num2, num3, 0,
			bilateralUpsample ? FilterMode.Point : FilterMode.Bilinear, format);
		...
		for (var index = 1; index < 5; ++index) {
			var reflectionTexture1 = kReflectionTextures[index - 1];
			var num6 = index;
			...
			var reflectionTexture2 = kReflectionTextures[index];
		...
		m_CommandBuffer.GetTemporaryRT(kTempTexture, camera_.pixelWidth, camera_.pixelHeight, 0, FilterMode.Bilinear,
			format);
	...
	public struct IntensitySettings {
		[Tooltip("Nonphysical multiplier for the SSR reflections. 1.0 is physically based.")] [Range(0.0f, 2f)]
		public float reflectionMultiplier;

		[Tooltip("How far away from the maxDistance to begin fading SSR.")] [Range(0.0f, 1000f)]
