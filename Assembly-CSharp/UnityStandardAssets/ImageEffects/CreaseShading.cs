...
public class CreaseShading : PostEffectsBase {
	...
	private void OnRenderImage(RenderTexture source, RenderTexture destination) {
		if (!CheckResources())
			...
			var num1 = (float)(1.0 * width / (1.0 * height));
			var num2 = 1f / 512f;
			var temporary1 = RenderTexture.GetTemporary(width, height, 0);
			var renderTexture1 = RenderTexture.GetTemporary(width / 2, height / 2, 0);
			Graphics.Blit(source, temporary1, depthFetchMaterial);
			Graphics.Blit(temporary1, renderTexture1);
			for (var index = 0; index < softness; ++index) {
				var temporary2 = RenderTexture.GetTemporary(width / 2, height / 2, 0);
				...
				var temporary3 = RenderTexture.GetTemporary(width / 2, height / 2, 0);
