...
public class Bloom : PostEffectsBase {
	...
	public void OnRenderImage(RenderTexture source, RenderTexture destination) {
		if (!CheckResources())
			...
			doHdr = false;
			doHdr = hdr != HDRBloomMode.Auto
				? hdr == HDRBloomMode.On
				: source.format == RenderTextureFormat.ARGBHalf && GetComponent<Camera>().allowHDR;
			doHdr = doHdr && supportHDRTextures;
			var bloomScreenBlendMode = screenBlendMode;
			if (doHdr)
				bloomScreenBlendMode = BloomScreenBlendMode.Add;
			var format = doHdr ? RenderTextureFormat.ARGBHalf : RenderTextureFormat.Default;
			var width1 = source.width / 2;
			var height1 = source.height / 2;
			var width2 = source.width / 4;
			var height2 = source.height / 4;
			var num1 = (float)(1.0 * source.width / (1.0 * source.height));
			var num2 = 1f / 512f;
			var temporary1 = RenderTexture.GetTemporary(width2, height2, 0, format);
			var temporary2 = RenderTexture.GetTemporary(width1, height1, 0, format);
			if (quality > BloomQuality.Cheap) {
				Graphics.Blit(source, temporary2, screenBlend, 2);
				var temporary3 = RenderTexture.GetTemporary(width2, height2, 0, format);
			...
			var renderTexture1 = RenderTexture.GetTemporary(width2, height2, 0, format);
			...
			for (var index = 0; index < bloomBlurIterations; ++index) {
				var num3 = (float)(1.0 + index * 0.25) * sepBlurSpread;
				var temporary4 = RenderTexture.GetTemporary(width2, height2, 0, format);
				...
				var renderTexture2 = temporary4;
				var temporary5 = RenderTexture.GetTemporary(width2, height2, 0, format);
			...
			if (lensflareIntensity > (double)Mathf.Epsilon) {
				var temporary6 = RenderTexture.GetTemporary(width2, height2, 0, format);
				if (lensflareMode == LensFlareStyle.Ghosting) {
					...
					if (quality > BloomQuality.Cheap) {
						blurAndFlaresMaterial.SetVector("_Offsets",
							new Vector4(0.0f, (float)(1.5 / (1.0 * temporary1.height)), 0.0f, 0.0f));
						Graphics.SetRenderTarget(temporary1);
						GL.Clear(false, true, Color.black);
						Graphics.Blit(temporary6, temporary1, blurAndFlaresMaterial, 4);
						blurAndFlaresMaterial.SetVector("_Offsets",
							new Vector4((float)(1.5 / (1.0 * temporary1.width)), 0.0f, 0.0f, 0.0f));
					...
					var x = 1f * Mathf.Cos(flareRotation);
					var y = 1f * Mathf.Sin(flareRotation);
					var num4 = hollyStretchWidth * 1f / num1 * num2;
					...
					temporary1.DiscardContents();
					Graphics.Blit(temporary6, temporary1, blurAndFlaresMaterial, 1);
					for (var index = 0; index < hollywoodFlareBlurIterations; ++index) {
						var num5 = hollyStretchWidth * 2f / num1 * num2;
			...
			var pass = (int)bloomScreenBlendMode;
			screenBlend.SetFloat("_Intensity", bloomIntensity);
			screenBlend.SetTexture("_ColorBuffer", source);
			if (quality > BloomQuality.Cheap) {
				var temporary7 = RenderTexture.GetTemporary(width1, height1, 0, format);
