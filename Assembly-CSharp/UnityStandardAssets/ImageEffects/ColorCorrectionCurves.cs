...
public class ColorCorrectionCurves : PostEffectsBase {
	...
	public void UpdateParameters() {
		...
		for (var time = 0.0f; time <= 1.0; time += 0.003921569f) {
			var num1 = Mathf.Clamp(redChannel.Evaluate(time), 0.0f, 1f);
			var num2 = Mathf.Clamp(greenChannel.Evaluate(time), 0.0f, 1f);
			var num3 = Mathf.Clamp(blueChannel.Evaluate(time), 0.0f, 1f);
			...
			var num4 = Mathf.Clamp(zCurve.Evaluate(time), 0.0f, 1f);
			zCurveTex.SetPixel((int)Mathf.Floor(time * byte.MaxValue), 0, new Color(num4, num4, num4));
			var num5 = Mathf.Clamp(depthRedChannel.Evaluate(time), 0.0f, 1f);
			var num6 = Mathf.Clamp(depthGreenChannel.Evaluate(time), 0.0f, 1f);
			var num7 = Mathf.Clamp(depthBlueChannel.Evaluate(time), 0.0f, 1f);
