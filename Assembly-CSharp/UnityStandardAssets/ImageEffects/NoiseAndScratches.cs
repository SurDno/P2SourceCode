...
public class NoiseAndScratches : MonoBehaviour {
	...
	[Range(0.0f, 5f)] public float grainIntensityMin = 0.1f;
	[Range(0.0f, 5f)] public float grainIntensityMax = 0.2f;
	[Range(0.1f, 50f)] public float grainSize = 2f;
	[Range(0.0f, 5f)] public float scratchIntensityMin = 0.05f;
	[Range(0.0f, 5f)] public float scratchIntensityMax = 0.25f;
	[Range(1f, 30f)] public float scratchFPS = 10f;
	[Range(0.0f, 1f)] public float scratchJitter = 0.01f;
	...
	private void OnRenderImage(RenderTexture source, RenderTexture destination) {
		...
		var num = 1f / grainSize;
		material.SetVector("_GrainOffsetScale",
			new Vector4(Random.value, Random.value, Screen.width / (float)grainTexture.width * num,
				Screen.height / (float)grainTexture.height * num));
		material.SetVector("_ScratchOffsetScale",
			new Vector4(scratchX + Random.value * scratchJitter, scratchY + Random.value * scratchJitter,
				Screen.width / (float)scratchTexture.width, Screen.height / (float)scratchTexture.height));
		material.SetVector("_Intensity",
			new Vector4(Random.Range(grainIntensityMin, grainIntensityMax),
				Random.Range(scratchIntensityMin, scratchIntensityMax), 0.0f, 0.0f));
