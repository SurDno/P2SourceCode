...
public class GlobalFogBase : PostEffectsBase {
	...
	[Range(0.001f, 10f)] public float distanceDensity = 1f;
	[Tooltip("Apply height-based fog?")] public bool heightFog = true;
	[Tooltip("Fog top Y coordinate")] public float height = 1f;
	[Range(0.001f, 10f)] public float heightDensity = 2f;
	...
	protected void OnRenderImage_Internal(RenderTexture source, RenderTexture destination) {
		if (!CheckResources() || (!distanceFog && !heightFog)) {
			...
			var matrix4x4 = GL.GetGPUProjectionMatrix(cam.projectionMatrix, true) * worldToCameraMatrix;
			...
			var y = -height;
			var z = y <= 0.0 ? 1f : 0.0f;
			Shader.SetGlobalVector("_FogHeightParams",
				new Vector4(position.y + height, y, z, heightFog ? heightDensity * 0.5f : 0.0f));
			Shader.SetGlobalVector("_FogDistanceParams",
				new Vector4(distanceFog ? distanceDensity : 0.0f, -Mathf.Max(startDistance, 0.0f), 0.0f, 0.0f));
