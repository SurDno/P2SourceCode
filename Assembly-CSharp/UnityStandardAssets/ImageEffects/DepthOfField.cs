...
public class DepthOfField : PostEffectsBase {
	...
	private void WriteCoc(RenderTexture fromTo, bool fgDilate) {
		...
		if (nearBlur & fgDilate) {
			var width = fromTo.width / 2;
			var height = fromTo.height / 2;
			var temporary1 = RenderTexture.GetTemporary(width, height, 0, fromTo.format);
			Graphics.Blit(fromTo, temporary1, dofHdrMaterial, 4);
			var num = internalBlurWidth * foregroundOverlap;
			dofHdrMaterial.SetVector("_Offsets", new Vector4(0.0f, num, 0.0f, num));
			var temporary2 = RenderTexture.GetTemporary(width, height, 0, fromTo.format);
			...
			var temporary3 = RenderTexture.GetTemporary(width, height, 0, fromTo.format);
	...
	private void OnRenderImage(RenderTexture source, RenderTexture destination) {
		if (!CheckResources())
			...
			focalDistance01 = (bool)(Object)focalTransform
			...
			dofHdrMaterial.SetVector("_CurveParams",
				new Vector4(1f, focalSize, (float)(1.0 / (1.0 - aperture) - 1.0), focalDistance01));
			RenderTexture renderTexture1 = null;
			RenderTexture renderTexture2 = null;
			var num1 = internalBlurWidth * foregroundOverlap;
			if (visualizeFocus) {
				...
				if (highResolution) {
					...
					var num2 = internalBlurWidth * foregroundOverlap;
					renderTexture1 = RenderTexture.GetTemporary(source.width, source.height, 0, source.format);
					var temporary1 = RenderTexture.GetTemporary(source.width, source.height, 0, source.format);
					...
					dx11bokehMaterial.SetVector("_BokehParams",
						new Vector4(dx11BokehScale, dx11BokehIntensity, Mathf.Clamp(dx11BokehThreshold, 0.005f, 4f),
							internalBlurWidth));
					...
					dx11bokehMaterial.SetVector("_Screen",
						new Vector3((float)(1.0 / (1.0 * source.width)), (float)(1.0 / (1.0 * source.height)),
							internalBlurWidth));
					...
					renderTexture2 =
						RenderTexture.GetTemporary(source.width >> 1, source.height >> 1, 0, source.format);
					var num3 = internalBlurWidth * foregroundOverlap;
					WriteCoc(source, false);
					source.filterMode = FilterMode.Bilinear;
					Graphics.Blit(source, renderTexture1, dofHdrMaterial, 6);
					var temporary4 = RenderTexture.GetTemporary(renderTexture1.width >> 1, renderTexture1.height >> 1,
						0, renderTexture1.format);
					var temporary5 = RenderTexture.GetTemporary(renderTexture1.width >> 1, renderTexture1.height >> 1,
						0, renderTexture1.format);
					...
					dx11bokehMaterial.SetTexture("_MainTex", dx11BokehTexture);
					dx11bokehMaterial.SetVector("_Screen",
						new Vector3((float)(1.0 / (1.0 * renderTexture2.width)),
							(float)(1.0 / (1.0 * renderTexture2.height)), internalBlurWidth));
					...
					dofHdrMaterial.SetVector("_Offsets",
						(float)(1.0 * source.width / (1.0 * renderTexture2.width)) * internalBlurWidth * Vector4.one);
				...
				var pass = blurSampleCount == BlurSampleCount.High || blurSampleCount == BlurSampleCount.Medium
					? 17
					: 11;
				if (highResolution) {
					dofHdrMaterial.SetVector("_Offsets",
					...
					dofHdrMaterial.SetTexture("_FgOverlap", null);
					dofHdrMaterial.SetVector("_Offsets",
						Vector4.one * (float)(1.0 * source.width / (1.0 * renderTexture2.width)) * internalBlurWidth);
