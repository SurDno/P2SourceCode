...
public class BloomAndFlares : PostEffectsBase {
	...
	private void OnRenderImage(RenderTexture source, RenderTexture destination) {
		if (!CheckResources())
			...
			var pass = screenBlendMode;
			if (doHdr)
				pass = BloomScreenBlendMode.Add;
			var format = doHdr ? RenderTextureFormat.ARGBHalf : RenderTextureFormat.Default;
			var temporary1 = RenderTexture.GetTemporary(source.width / 2, source.height / 2, 0, format);
			var temporary2 = RenderTexture.GetTemporary(source.width / 4, source.height / 4, 0, format);
			var temporary3 = RenderTexture.GetTemporary(source.width / 4, source.height / 4, 0, format);
			var temporary4 = RenderTexture.GetTemporary(source.width / 4, source.height / 4, 0, format);
			var num1 = (float)(1.0 * source.width / (1.0 * source.height));
			var num2 = 1f / 512f;
			Graphics.Blit(source, temporary1, screenBlend, 2);
			Graphics.Blit(temporary1, temporary2, screenBlend, 2);
			...
			for (var index = 0; index < bloomBlurIterations; ++index) {
				var num3 = (float)(1.0 + index * 0.5) * sepBlurSpread;
				separableBlurMaterial.SetVector("offsets", new Vector4(0.0f, num3 * num2, 0.0f, 0.0f));
				var source1 = index == 0 ? temporary3 : temporary2;
				Graphics.Blit(source1, temporary4, separableBlurMaterial);
			...
			if (lensflares) {
				if (lensflareMode == LensflareStyle34.Ghosting) {
					...
					hollywoodFlaresMaterial.SetVector("_threshold",
						new Vector4(lensflareThreshold, (float)(1.0 / (1.0 - lensflareThreshold)), 0.0f, 0.0f));
					...
					hollywoodFlaresMaterial.SetVector("offsets",
						new Vector4(sepBlurSpread * 1f / num1 * num2, 0.0f, 0.0f, 0.0f));
					...
					if (lensflareMode == LensflareStyle34.Anamorphic) {
						for (var index = 0; index < hollywoodFlareBlurIterations; ++index) {
							separableBlurMaterial.SetVector("offsets",
								new Vector4(hollyStretchWidth * 2f / num1 * num2, 0.0f, 0.0f, 0.0f));
	...
	private void BrightFilter(
		...
		if (doHdr)
			...
			brightPassFilterMaterial.SetVector("threshold",
				new Vector4(thresh, (float)(1.0 / (1.0 - thresh)), 0.0f, 0.0f));
