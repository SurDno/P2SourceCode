...
public class DepthOfFieldDeprecated : PostEffectsBase {
	...
	private int GetLowResolutionDividerBasedOnQuality(int baseDivider) {
		var dividerBasedOnQuality = baseDivider;
	...
	private void OnRenderImage(RenderTexture source, RenderTexture destination) {
		if (!CheckResources())
			...
			var num1 = bokeh ? BOKEH_EXTRA_BLUR : 1f;
			var flag = quality > Dof34QualitySetting.OnlyBackground;
			var num2 = focalSize / (_camera.farClipPlane - _camera.nearClipPlane);
			bool blurForeground;
			if (simpleTweakMode) {
				focalDistance01 = (bool)(Object)objectFocus
				...
				if ((bool)(Object)objectFocus) {
					var viewportPoint = _camera.WorldToViewportPoint(objectFocus.position);
			...
			dofMaterial.SetVector("_CurveParams",
				new Vector4(simpleTweakMode ? 1f / focalStartCurve : focalStartCurve,
					simpleTweakMode ? 1f / focalEndCurve : focalEndCurve, num2 * 0.5f, focalDistance01));
			dofMaterial.SetVector("_InvRenderTargetSize",
				new Vector4((float)(1.0 / (1.0 * source.width)), (float)(1.0 / (1.0 * source.height)), 0.0f, 0.0f));
			var dividerBasedOnQuality1 = GetDividerBasedOnQuality();
			var dividerBasedOnQuality2 = GetLowResolutionDividerBasedOnQuality(dividerBasedOnQuality1);
	...
	private void Blur(
		...
		int blurPass,
		float spread) {
		var temporary = RenderTexture.GetTemporary(to.width, to.height);
	...
	private void Downsample(RenderTexture from, RenderTexture to) {
		dofMaterial.SetVector("_InvRenderTargetSize",
			new Vector4((float)(1.0 / (1.0 * to.width)), (float)(1.0 / (1.0 * to.height)), 0.0f, 0.0f));
	...
	private void AddBokeh(
		...
		var num = (float)(bokehInfo.width * 1.0 / (bokehInfo.height * 1.0));
		var x = (float)(2.0 / (1.0 * bokehInfo.width)) +
		        bokehScale * maxBlurSpread * BOKEH_EXTRA_BLUR * oneOverBaseSize;
