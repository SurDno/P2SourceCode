...
public class Tonemapping : PostEffectsBase {
	...
	public float UpdateCurve() {
		...
		if (remapCurve != null) {
			...
			for (var num2 = 0.0f; num2 <= 1.0; num2 += 0.003921569f) {
				var num3 = remapCurve.Evaluate(num2 * 1f * num1);
	...
	private void OnRenderImage(RenderTexture source, RenderTexture destination) {
		if (!CheckResources())
			...
			if (type == TonemapperType.UserCurve) {
				...
				var internalRenderTexture = CreateInternalRenderTexture();
				var temporary =
					RenderTexture.GetTemporary((int)adaptiveTextureSize, (int)adaptiveTextureSize, 0, rtFormat);
				Graphics.Blit(source, temporary);
				var length = (int)Mathf.Log(temporary.width * 1f, 2f);
				var num = 2;
				var renderTextureArray = new RenderTexture[length];
				for (var index = 0; index < length; ++index) {
					renderTextureArray[index] =
						RenderTexture.GetTemporary(temporary.width / num, temporary.width / num, 0, rtFormat);
					num *= 2;
				}
