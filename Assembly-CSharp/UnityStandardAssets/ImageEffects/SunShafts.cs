	...
	private void OnRenderImage(RenderTexture source, RenderTexture destination) {
		if (!CheckResources())
			...
			if (resolution == SunShaftsResolution.Normal)
				...
				num1 = 1;
			var vector3 = Vector3.one * 0.5f;
			vector3 = !(bool)(Object)sunTransform
				? new Vector3(0.5f, 0.5f, 0.0f)
				: GetComponent<Camera>().WorldToViewportPoint(sunTransform.position);
			var width = source.width / num1;
			var height = source.height / num1;
			var temporary1 = RenderTexture.GetTemporary(width, height, 0);
			...
			if (!useDepthTexture) {
				...
				var temporary2 = RenderTexture.GetTemporary(source.width, source.height, 0, format);
				RenderTexture.active = temporary2;
				GL.ClearWithSkybox(false, GetComponent<Camera>());
				sunShaftsMaterial.SetTexture("_Skybox", temporary2);
				Graphics.Blit(source, temporary1, sunShaftsMaterial, 3);
				RenderTexture.ReleaseTemporary(temporary2);
			} else
				Graphics.Blit(source, temporary1, sunShaftsMaterial, 2);

			DrawBorder(temporary1, simpleClearMaterial);
			radialBlurIterations = Mathf.Clamp(radialBlurIterations, 1, 4);
			var num2 = sunShaftBlurRadius * (1f / 768f);
			sunShaftsMaterial.SetVector("_BlurRadius4", new Vector4(num2, num2, 0.0f, 0.0f));
			sunShaftsMaterial.SetVector("_SunPosition", new Vector4(vector3.x, vector3.y, vector3.z, maxRadius));
			for (var index = 0; index < radialBlurIterations; ++index) {
				var temporary3 = RenderTexture.GetTemporary(width, height, 0);
				Graphics.Blit(temporary1, temporary3, sunShaftsMaterial, 1);
				RenderTexture.ReleaseTemporary(temporary1);
				var num3 = (float)(sunShaftBlurRadius * ((index * 2.0 + 1.0) * 6.0) / 768.0);
				...
				Graphics.Blit(temporary3, temporary1, sunShaftsMaterial, 1);
				RenderTexture.ReleaseTemporary(temporary3);
				var num4 = (float)(sunShaftBlurRadius * ((index * 2.0 + 2.0) * 6.0) / 768.0);
			...
			sunShaftsMaterial.SetTexture("_ColorBuffer", temporary1);
