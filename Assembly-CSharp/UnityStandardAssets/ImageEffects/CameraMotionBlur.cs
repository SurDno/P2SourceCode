...
public class CameraMotionBlur : PostEffectsBase {
	...
	private void CalculateViewProjection() {
		var worldToCameraMatrix = _camera.worldToCameraMatrix;
	...
	private void OnRenderImage(RenderTexture source, RenderTexture destination) {
		if (!CheckResources())
			...
			if (filterType == MotionBlurFilter.CameraMotion)
				StartFrame();
			var format = SystemInfo.SupportsRenderTextureFormat(RenderTextureFormat.RGHalf)
				? RenderTextureFormat.RGHalf
				: RenderTextureFormat.ARGBHalf;
			var temporary1 = RenderTexture.GetTemporary(divRoundUp(source.width, velocityDownsample),
				divRoundUp(source.height, velocityDownsample), 0, format);
			this.maxVelocity = Mathf.Max(2f, this.maxVelocity);
			var maxVelocity = this.maxVelocity;
			var flag = filterType == MotionBlurFilter.ReconstructionDX11 && dx11MotionBlurMaterial == null;
			int width;
			int height;
			float num1;
			...
			var temporary2 = RenderTexture.GetTemporary(width, height, 0, format);
			var temporary3 = RenderTexture.GetTemporary(width, height, 0, format);
			...
			if (gameObject.activeInHierarchy && !wasActive)
				Remember();
			wasActive = gameObject.activeInHierarchy;
			var matrix4x4 = Matrix4x4.Inverse(currentViewProjMat);
			...
			if (preview) {
				...
				prevViewProjMat = GL.GetGPUProjectionMatrix(_camera.projectionMatrix, true) * identity *
				                  worldToCameraMatrix;
				motionBlurMaterial.SetMatrix("_PrevViewProj", prevViewProjMat);
				motionBlurMaterial.SetMatrix("_ToPrevViewProjCombined", prevViewProjMat * matrix4x4);
			}
			...
			if (filterType == MotionBlurFilter.CameraMotion) {
				...
				var num2 = Vector3.Dot(transform.up, Vector3.up);
				var rhs = prevFramePos - transform.position;
				var magnitude = rhs.magnitude;
				var num3 = (float)(Vector3.Angle(transform.up, prevFrameUp) / (double)_camera.fieldOfView *
				                   (source.width * 0.75));
				zero.x = rotationScale * num3;
				var num4 = (float)(Vector3.Angle(transform.forward, prevFrameForward) / (double)_camera.fieldOfView *
				                   (source.width * 0.75));
				zero.y = rotationScale * num2 * num4;
				var num5 = (float)(Vector3.Angle(transform.forward, prevFrameForward) / (double)_camera.fieldOfView *
				                   (source.width * 0.75));
				zero.z = rotationScale * (1f - num2) * num5;
				if (magnitude > (double)Mathf.Epsilon && movementScale > (double)Mathf.Epsilon) {
					zero.w = (float)(movementScale * (double)Vector3.Dot(transform.forward, rhs) *
					                 (source.width * 0.5));
				...
				if (excludeLayers.value != 0)
					camera = GetTmpCam();
				if ((bool)(Object)camera && excludeLayers.value != 0 && (bool)(Object)replacementClear &&
