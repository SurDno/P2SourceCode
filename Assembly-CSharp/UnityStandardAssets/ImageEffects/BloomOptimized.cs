...
public class BloomOptimized : PostEffectsBase {
	...
	private void OnRenderImage(RenderTexture source, RenderTexture destination) {
		if (!CheckResources())
			...
			var num1 = resolution == Resolution.Low ? 4 : 2;
			var num2 = resolution == Resolution.Low ? 0.5f : 1f;
			...
			Graphics.Blit(source, renderTexture1, fastBloomMaterial, 1);
			var num3 = blurType == BlurType.Standard ? 0 : 2;
			for (var index = 0; index < blurIterations; ++index) {
				fastBloomMaterial.SetVector("_Parameter",
					new Vector4((float)(blurSize * (double)num2 + index * 1.0), 0.0f, threshold, intensity));
				var temporary1 = RenderTexture.GetTemporary(width, height, 0, source.format);
				temporary1.filterMode = FilterMode.Bilinear;
				Graphics.Blit(renderTexture1, temporary1, fastBloomMaterial, 2 + num3);
				...
				Graphics.Blit(renderTexture2, temporary2, fastBloomMaterial, 3 + num3);
				RenderTexture.ReleaseTemporary(renderTexture2);
				renderTexture1 = temporary2;
			}

			fastBloomMaterial.SetTexture("_Bloom", renderTexture1);
