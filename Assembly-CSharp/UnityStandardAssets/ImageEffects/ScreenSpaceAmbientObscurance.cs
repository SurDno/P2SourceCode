...
internal class ScreenSpaceAmbientObscurance : PostEffectsBase {
	[Range(0.0f, 3f)] public float intensity = 0.5f;
	[Range(0.1f, 3f)] public float radius = 0.2f;
	[Range(0.0f, 3f)] public int blurIterations = 1;
	[Range(0.0f, 5f)] public float blurFilterDistance = 1.25f;
	...
	private void OnRenderImage(RenderTexture source, RenderTexture destination) {
		if (!CheckResources())
			...
			var projectionMatrix = GetComponent<Camera>().projectionMatrix;
			var inverse = projectionMatrix.inverse;
			aoMaterial.SetVector("_ProjInfo",
				new Vector4((float)(-2.0 / (Screen.width * (double)projectionMatrix[0])),
					(float)(-2.0 / (Screen.height * (double)projectionMatrix[5])),
					(1f - projectionMatrix[2]) / projectionMatrix[0],
			...
			var renderTexture = RenderTexture.GetTemporary(width >> downsample, height >> downsample);
