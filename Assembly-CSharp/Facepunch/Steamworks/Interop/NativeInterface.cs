...
internal class NativeInterface : IDisposable {
	...
	internal bool InitClient(BaseSteamworks steamworks) {
		...
		var hsteamUser = api.SteamAPI_GetHSteamUser();
		var hsteamPipe = api.SteamAPI_GetHSteamPipe();
		if (hsteamPipe == 0) {
	...
	internal bool InitServer(
		...
		var hsteamUser = api.SteamGameServer_GetHSteamUser();
		var hsteamPipe = api.SteamGameServer_GetHSteamPipe();
		if (hsteamPipe == 0) {
	...
	public void FillInterfaces(BaseSteamworks steamworks, int hpipe, int huser) {
		var pointer = api.SteamInternal_CreateInterface("SteamClient017");
		client = !(pointer == IntPtr.Zero)
			? new SteamClient(steamworks, pointer)
