...
public class Client : BaseSteamworks, IDisposable {
	...
	public Client(uint appId) {
		Instance = Instance == null
			? this
			: throw new Exception(
				"Only one Facepunch.Steamworks.Client can exist - dispose the old one before trying to create a new one.");
	...
	public Leaderboard GetLeaderboard(
		...
		var leaderboard = new Leaderboard(this);
		native.userstats.FindOrCreateLeaderboard(name, (SteamNative.LeaderboardSortMethod)sortMethod,
			(SteamNative.LeaderboardDisplayType)displayType, leaderboard.OnBoardCreated);
		return leaderboard;
	}
