...
public class Leaderboard : IDisposable {
	...
	internal void OnBoardCreated(LeaderboardFindResult_t result, bool error) {
		if (error || result.LeaderboardFound == 0)
			...
			if (IsValid) {
				...
				var boardInformation = OnBoardInformation;
	...
	public bool AddScore(bool onlyIfBeatsOldScore, int score, params int[] subscores) {
		...
		var eLeaderboardUploadScoreMethod = LeaderboardUploadScoreMethod.ForceUpdate;
		if (onlyIfBeatsOldScore)
			eLeaderboardUploadScoreMethod = LeaderboardUploadScoreMethod.KeepBest;
		client.native.userstats.UploadLeaderboardScore(BoardId, eLeaderboardUploadScoreMethod, score, subscores,
			subscores.Length);
		return true;
	}
	...
	public bool AddScore(
		...
		if (IsError)
			return false;
		if (!IsValid)
			return DeferOnCreated(() => AddScore(onlyIfBeatsOldScore, score, subscores, onSuccess, onFailure),
				onFailure);
		if (subscores == null)
			subscores = new int[0];
		var eLeaderboardUploadScoreMethod = LeaderboardUploadScoreMethod.ForceUpdate;
		if (onlyIfBeatsOldScore)
			eLeaderboardUploadScoreMethod = LeaderboardUploadScoreMethod.KeepBest;
		client.native.userstats.UploadLeaderboardScore(BoardId, eLeaderboardUploadScoreMethod, score, subscores,
			subscores.Length, (result, error) => {
				if (!error && result.Success > 0) {
					var addScoreCallback = onSuccess;
		...
		if (file.IsShared)
			return client.native.userstats.AttachLeaderboardUGC(BoardId, file.UGCHandle, (result, error) => {
				if (!error && result.Result == SteamNative.Result.OK) {
					var remoteFileCallback = onSuccess;
	...
	public bool FetchScores(RequestType RequestType, int start, int end) {
		if (!IsValid || IsQuerying)
			return false;
		client.native.userstats.DownloadLeaderboardEntries(BoardId, (LeaderboardDataRequest)RequestType, start, end,
			OnScores);
		Results = null;
		IsQuerying = true;
		return true;
	...
	private unsafe void ReadScores(
		...
		List<Entry> dest) {
		for (var index = 0; index < result.CEntryCount; ++index)
			fixed (int* pDetails = subEntriesBuffer) {
				var pLeaderboardEntry = new LeaderboardEntry_t();
				if (client.native.userstats.GetDownloadedLeaderboardEntry(result.SteamLeaderboardEntries, index,
					    ref pLeaderboardEntry, (IntPtr)pDetails, subEntriesBuffer.Length)) {
					var entryList = dest;
					var entry = new Entry {
						GlobalRank = pLeaderboardEntry.GlobalRank,
						Score = pLeaderboardEntry.Score,
						SteamId = pLeaderboardEntry.SteamIDUser,
						SubScores = pLeaderboardEntry.CDetails == 0
							? null
							: subEntriesBuffer.Take(pLeaderboardEntry.CDetails).ToArray(),
						Name = client.Friends.GetName(pLeaderboardEntry.SteamIDUser),
						AttachedFile = pLeaderboardEntry.UGC >> 32 == uint.MaxValue
							? null
							: new RemoteFile(client.RemoteStorage, pLeaderboardEntry.UGC)
					};
					entryList.Add(entry);
				}
			}
