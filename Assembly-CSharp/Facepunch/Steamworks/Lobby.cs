...
public class Lobby : IDisposable {
	...
	public string GetMemberData(ulong steamID, string key) {
		return CurrentLobby == 0UL
			? "ERROR: NOT IN ANY LOBBY"
			: client.native.matchmaking.GetLobbyMemberData(CurrentLobby, steamID, key);
	}
	...
	internal void UpdateLobbyData() {
		var lobbyDataCount = client.native.matchmaking.GetLobbyDataCount(CurrentLobby);
		CurrentLobbyData = new LobbyData(client, CurrentLobby);
	...
	private unsafe void OnLobbyChatMessageRecievedAPI(LobbyChatMsg_t callback, bool error) {
		...
		fixed (byte* pvData = chatMessageData) {
			lobbyChatEntry = client.native.matchmaking.GetLobbyChatEntry(CurrentLobby, (int)callback.ChatID,
				out pSteamIDUser, (IntPtr)pvData, chatMessageData.Length, out var _);
		}

		var chatMessageRecieved = OnChatMessageRecieved;
		if (chatMessageRecieved != null)
			chatMessageRecieved(pSteamIDUser, chatMessageData, lobbyChatEntry);
		var chatStringRecieved = OnChatStringRecieved;
	...
	internal void OnLobbyStateUpdatedAPI(LobbyChatUpdate_t callback, bool error) {
		if (error || (long)callback.SteamIDLobby != (long)CurrentLobby)
			return;
		var memberStateChange = (MemberStateChange)callback.GfChatMemberStateChange;
		var steamIdMakingChange = callback.SteamIDMakingChange;
		var steamIdUserChanged = callback.SteamIDUserChanged;
		var lobbyStateChanged = OnLobbyStateChanged;
	...
	public ulong[] GetMemberIDs() {
		var memberIds = new ulong[NumMembers];
		for (var iMember = 0; iMember < NumMembers; ++iMember)
