...
public class Server : BaseSteamworks {
	...
	public Server(uint appId, ServerInit init) {
		Instance = Instance == null
			? this
			: throw new Exception(
				"Only one Facepunch.Steamworks.Server can exist - dispose the old one before trying to create a new one.");
		native = new NativeInterface();
		if (init.SteamPort == 0)
			init.RandomSteamPort();
		if (!native.InitServer(this, init.IpAddress, init.SteamPort, init.GamePort, init.QueryPort, init.Secure ? 3 : 2,
			    init.VersionString)) {
			native.Dispose();
			native = null;
			Instance = null;
	...
	public IPAddress PublicIp {
		get {
			var publicIp = native.gameServer.GetPublicIP();
