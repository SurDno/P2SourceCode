  ...
  public class Voice
  {
      ...
      else
      {
        ...
        fixed (byte* pDestBuffer = ReadCompressedBuffer)
          fixed (byte* pUncompressedDestBuffer_Deprecated = ReadUncompressedBuffer)
            voice = client.native.user.GetVoice(OnCompressedData != null, (IntPtr) pDestBuffer, 131072U, out num2, OnUncompressedData != null, (IntPtr) pUncompressedDestBuffer_Deprecated, 131072U, out num1, DesiredSampleRate == 0U ? OptimalSampleRate : DesiredSampleRate);
        IsRecording = true;
      ...
      uint samepleRate = 0)
    {
      ...
      fixed (byte* pDestBuffer = UncompressBuffer)
        voiceResult = client.native.user.DecompressVoice((IntPtr) (void*) ((IntPtr) (void*) input + inputoffset), (uint) inputsize, (IntPtr) pDestBuffer, (uint) UncompressBuffer.Length, out nBytesWritten, samepleRate);
