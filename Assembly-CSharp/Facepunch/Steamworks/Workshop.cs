...
public class Workshop : IDisposable {
	...
	public class Editor {
		...
		public int BytesUploaded {
			get {
				...
				var itemUpdateProgress =
					(int)workshop.steamworks.native.ugc.GetItemUpdateProgress(UpdateHandle, out punBytesProcessed,
						out punBytesTotal);
	...
	public class Item {
		...
		public DirectoryInfo Directory {
			get {
				...
				ulong punSizeOnDisk;
				string pchFolder;
				if (workshop.ugc.GetItemInstallInfo(Id, out punSizeOnDisk, out pchFolder, out var _)) {
	...
	public class Query : IDisposable {
		...
		private void RunInternal() {
			if (FileId.Count != 0) {
				var array = FileId.Select<ulong, PublishedFileId_t>(x => x).ToArray();
				_resultsRemain = array.Length;
				Handle = workshop.ugc.CreateQueryUGCDetailsRequest(array);
			} else
				Handle = !UserId.HasValue
					? workshop.ugc.CreateQueryAllUGCRequest((UGCQuery)Order, (UGCMatchingUGCType)QueryType,
						UploaderAppId, AppId, (uint)(_resultPage + 1))
					: workshop.ugc.CreateQueryUserUGCRequest((uint)(UserId.Value & uint.MaxValue),
						(UserUGCList)UserQueryType, (UGCMatchingUGCType)QueryType, UserUGCListSortOrder.LastUpdatedDesc,
						UploaderAppId, AppId, (uint)(_resultPage + 1));
		...
		private void ResultCallback(SteamUGCQueryCompleted_t data, bool bFailed) {
			...
			var num = 0;
			for (var index = 0; index < data.NumResultsReturned; ++index)
				if (_resultSkip > 0)
					--_resultSkip;
				else {
					var details = new SteamUGCDetails_t();
					if (workshop.ugc.GetQueryUGCResult(data.Handle, (uint)index, ref details) &&
					    !_results.Any(x => (long)x.Id == (long)details.PublishedFileId)) {
						var obj = Item.From(details, workshop);
		...
		private int GetStat(ulong handle, int index, ItemStatistic stat) {
			ulong pStatValue = 0;
			return !workshop.ugc.GetQueryUGCStatistic(handle, (uint)index, (SteamNative.ItemStatistic)stat,
				out pStatValue)
				? 0
				: (int)pStatValue;
		}
		...
		public List<ulong> FileId { get; set; } = new();
