...
namespace Facepunch.Steamworks
{
  public class Inventory : IDisposable
  {
    ...
    private void onFullUpdate(SteamInventoryFullUpdate_t data, bool error)
    {
      if (error)
        return;
      Result r = new Result(this, data.Handle, false);
      r.Fill();
      onResult(r, true);
    }
    ...
    private void onResultReady(SteamInventoryResultReady_t data, bool error)
    {
      if (!Result.Pending.ContainsKey(data.Handle))
        return;
      Result r = Result.Pending[data.Handle];
    ...
    public unsafe Result Deserialize(byte[] data, int dataLength = -1)
    {
      ...
      fixed (byte* pBuffer = data)
      {
        if (!inventory.DeserializeResult(ref pOutResultHandle, (IntPtr) pBuffer, (uint) dataLength, false) || pOutResultHandle == -1)
          return null;
        Result result = new Result(this, pOutResultHandle, false);
        result.Fill();
        return result;
      }
    }
    ...
    public class Definition
    {
      ...
      internal void Link(Definition[] definitions) => LinkExchange(definitions);
    ...
    public struct Item : IEquatable<Item>
    {
      ...
      public bool Equals(Item other) => Equals(other, this);
      ...
      public static bool operator ==(Item c1, Item c2) => c1.Equals(c2);
    ...
    public class Result : IDisposable
    {
      ...
      {
        return Handle == -1 ? Callbacks.Result.InvalidParam : (Callbacks.Result) inventory.inventory.GetResultStatus(Handle);
      }
