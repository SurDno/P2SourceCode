...
public class ServerList : IDisposable {
	...
	private HashSet<ulong> FavouriteHash = new();
	private HashSet<ulong> HistoryHash = new();
	...
	internal void UpdateFavouriteList() {
		FavouriteHash.Clear();
		HistoryHash.Clear();
		for (var iGame = 0; iGame < client.native.matchmaking.GetFavoriteGameCount(); ++iGame) {
			AppId_t pnAppID = 0U;
			uint pnIP;
			ushort pnConnPort;
			uint punFlags;
			client.native.matchmaking.GetFavoriteGame(iGame, ref pnAppID, out pnIP, out pnConnPort, out var _,
				out punFlags, out var _);
			var num = ((ulong)pnIP << 32) | pnConnPort;
			if (((int)punFlags & 1) == 1)
	...
	public Request Internet(Filter filter = null) {
		...
		request.AddRequest(client.native.servers.RequestInternetServerList(client.AppId, filter.NativeArray,
			(uint)filter.Count, IntPtr.Zero));
		filter.Free();
		return request;
	}
	...
	public Request History(Filter filter = null) {
		...
		var request = new Request(client);
		request.Filter = filter;
		request.AddRequest(client.native.servers.RequestHistoryServerList(client.AppId, filter.NativeArray,
			(uint)filter.Count, IntPtr.Zero));
		filter.Free();
		return request;
	}
	...
	public Request Favourites(Filter filter = null) {
		...
		var request = new Request(client);
		request.Filter = filter;
		request.AddRequest(client.native.servers.RequestFavoritesServerList(client.AppId, filter.NativeArray,
			(uint)filter.Count, IntPtr.Zero));
		filter.Free();
		return request;
	}
	...
	public Request Friends(Filter filter = null) {
		...
		var request = new Request(client);
		request.Filter = filter;
		request.AddRequest(client.native.servers.RequestFriendsServerList(client.AppId, filter.NativeArray,
			(uint)filter.Count, IntPtr.Zero));
		filter.Free();
		return request;
	}
	...
	public class Filter : List<KeyValuePair<string, string>> {
		...
		internal void Start() {
			...
			var num = Marshal.SizeOf(typeof(MatchMakingKeyValuePair_t));
	...
	public class Request : IDisposable {
		internal Client client;
		internal List<SubRequest> Requests = new();
		public Action OnUpdate;
		public Action<Server> OnServerResponded;
		public Action OnFinished;
		public List<Server> Responded = new();
		public List<Server> Unresponsive = new();
		...
		internal void StartCustomQuery() {
			...
			var count2 = 0;
			while (true) {
				var source = ServerList.Skip(count2).Take(count1);
				if (source.Count() != 0) {
					count2 += source.Count();
					...
					filter.Start();
					var id = client.native.servers.RequestInternetServerList(client.AppId, filter.NativeArray,
						(uint)filter.Count, IntPtr.Zero);
					filter.Free();
					AddRequest(id);
				} else
		...
		private void OnServer(gameserveritem_t info) {
			if (info.HadSuccessfulResponse) {
				if (Filter != null && !Filter.Test(info))
					return;
				var server = Server.FromSteam(client, info);
				Responded.Add(server);
				var onServerResponded = OnServerResponded;
		...
		internal class SubRequest {
			...
			internal bool Update(
				...
				Timer.Start();
				var changes = false;
				var serverCount = servers.GetServerCount(Request);
				if (serverCount != Pointer)
					for (var pointer = Pointer; pointer < serverCount; ++pointer)
						WatchList.Add(pointer);
				Pointer = serverCount;
				WatchList.RemoveAll(x => {
					var serverDetails = servers.GetServerDetails(Request, x);
	...
	public class Server {
		...
		public void AddToHistory() {
			Client.native.matchmaking.AddFavoriteGame(AppId, Address.IpToInt32(), (ushort)ConnectionPort,
				(ushort)QueryPort, 2U, (uint)Utility.Epoch.Current);
			Client.ServerList.UpdateFavouriteList();
		}

		public void RemoveFromHistory() {
			Client.native.matchmaking.RemoveFavoriteGame(AppId, Address.IpToInt32(), (ushort)ConnectionPort,
				(ushort)QueryPort, 2U);
			Client.ServerList.UpdateFavouriteList();
		}
		...
		public void AddToFavourites() {
			Client.native.matchmaking.AddFavoriteGame(AppId, Address.IpToInt32(), (ushort)ConnectionPort,
				(ushort)QueryPort, 1U, (uint)Utility.Epoch.Current);
			Client.ServerList.UpdateFavouriteList();
		}
