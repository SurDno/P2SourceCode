...
public class LightShaftsBuilder {
	...
	private List<float> rayLengths = new();
	private List<Vector3> rayOrigins = new();
	...
	public void BuildTo(Mesh mesh) {
		var num1 = 0;
		var num2 = 0;
		for (var index = 0; index < rayLengths.Count; ++index)
			if (rayLengths[index] > 0.0) {
				if (rayLengths[index] == (double)lightShafts.length ||
				    lightShafts.length <= lightShafts.fadeIn + (double)lightShafts.radius) {
					num1 += 6;
					num2 += 12;
				} else {
					num1 += 8;
		...
		up = Vector3.Cross(LightShafts.LightDirection,
			Vector3.Cross(LightShafts.LightDirection, Vector3.down).normalized).normalized * (1f / 1000f);
		for (var index = 0; index < rayLengths.Count; ++index)
			if (rayLengths[index] != 0.0) {
				...
				if (rayLengths[index] == (double)lightShafts.length) {
					...
					var num3 = (rayLengths[index] - lightShafts.radius) / lightShafts.fadeIn;
					AddTriangles();
					AddVertices(
						rayOrigins[index] + LightShafts.LightDirection * (rayLengths[index] - lightShafts.radius),
						opacity * num3);
					AddTriangles();
					AddVertices(rayOrigins[index] + LightShafts.LightDirection * rayLengths[index], 0.0f);
				} else {
					var num4 = (float)((rayLengths[index] - (double)lightShafts.radius) /
					                   (lightShafts.length - (double)lightShafts.fadeIn));
					AddTriangles();
					AddVertices(rayOrigins[index] + LightShafts.LightDirection * lightShafts.fadeIn, opacity);
					AddTriangles();
					AddVertices(
