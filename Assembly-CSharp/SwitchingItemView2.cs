...
public class SwitchingItemView2 : ItemView {
	...
	[SerializeField] private ProgressViewBase backOpacity;
	[SerializeField] private ProgressViewBase frontOpacity;
	[SerializeField] private ProgressViewBase nullOpacity;
	[SerializeField] private Transform backTransform;
	[SerializeField] private Transform frontTransform;
	[SerializeField] private ItemView backView;
	[SerializeField] private ItemView frontView;
	...
	public bool IsAnimated {
		...
		private set {
			...
			isAnimated = value;
			if (isAnimated) {
				var animationBeginEvent = AnimationBeginEvent;
				if (animationBeginEvent == null)
					return;
				animationBeginEvent();
			} else {
				var animationEndEvent = AnimationEndEvent;
	...
	private void ApplyPhase() {
		...
		if (!(nullOpacity != null))
			return;
		nullOpacity.Progress =
			(float)((frontView.Storable == null ? phase : 0.0) + (backView.Storable == null ? num : 0.0));
	}
	...
	private void SwitchViews() {
		ApplyDirection();
		var backView = this.backView;
		this.backView = frontView;
		frontView = backView;
		var backTransform = this.backTransform;
	...
	private void ApplyDirection() {
		...
		var backAnchor = this.backAnchor;
		this.backAnchor = frontAnchor;
		frontAnchor = backAnchor;
		var backOpacity = this.backOpacity;
