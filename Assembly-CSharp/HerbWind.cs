...
public class HerbWind : MonoBehaviour {
	[SerializeField] private float maxPlayerDistance = 25f;
	[Space] [SerializeField] private float changeTime = 3f;
	[SerializeField] private float smoothness = 1f;
	[SerializeField] private float bendAngle = 5f;
	[Space] [SerializeField] private ParticleSystem particles;
	[SerializeField] private Vector2 speed = Vector2.one;
	[SerializeField] private Vector2 emissionRateRange = new(1f, 2f);
	...
	private void Update() {
		...
		currentWind = Vector2.SmoothDamp(currentWind, targetWind, ref windChangeVelocity, smoothness, float.MaxValue,
			Time.deltaTime);
	...
	private void UpdateParticles() {
		if (particles == null)
			return;
		var magnitude = currentWind.magnitude;

		var emission = particles.emission;
		emission.rateOverTime =
			new ParticleSystem.MinMaxCurve(Mathf.Lerp(emissionRateRange.x, emissionRateRange.y, magnitude));
		var velocityOverLifetime = particles.velocityOverLifetime with {
