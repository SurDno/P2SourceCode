...
// Type: SteamNative.GSStatsUnloaded_t
...
namespace SteamNative
{
  ...
  internal struct GSStatsUnloaded_t
  {
    ...
    {
      return Platform.PackSmall ? (GSStatsUnloaded_t) (GSStatsUnloaded_t.PackSmall) Marshal.PtrToStructure(p, typeof (GSStatsUnloaded_t.PackSmall)) : (GSStatsUnloaded_t) Marshal.PtrToStructure(p, typeof (GSStatsUnloaded_t));
    }

    public static void RegisterCallback(
      BaseSteamworks steamworks,
      Action<GSStatsUnloaded_t, bool> CallbackFunction)
    {
      CallbackHandle handle = new CallbackHandle();
      handle.steamworks = steamworks;
      if (Config.UseThisCall)
      {
        Callback.ThisCall.Result d1 = (Callback.ThisCall.Result) ((_, p) => CallbackFunction(GSStatsUnloaded_t.FromPointer(p), false));
        Callback.ThisCall.ResultWithInfo d2 = (Callback.ThisCall.ResultWithInfo) ((_, p, bIOFailure, hSteamAPICall) => CallbackFunction(GSStatsUnloaded_t.FromPointer(p), bIOFailure));
        Callback.ThisCall.GetSize d3 = (Callback.ThisCall.GetSize) (_ => Marshal.SizeOf(typeof (GSStatsUnloaded_t)));
        if (Platform.PackSmall)
          d3 = (Callback.ThisCall.GetSize) (_ => Marshal.SizeOf(typeof (GSStatsUnloaded_t.PackSmall)));
      ...
      {
        Callback.StdCall.Result d4 = (Callback.StdCall.Result) (p => CallbackFunction(GSStatsUnloaded_t.FromPointer(p), false));
        Callback.StdCall.ResultWithInfo d5 = (Callback.StdCall.ResultWithInfo) ((p, bIOFailure, hSteamAPICall) => CallbackFunction(GSStatsUnloaded_t.FromPointer(p), bIOFailure));
        Callback.StdCall.GetSize d6 = (Callback.StdCall.GetSize) (() => Marshal.SizeOf(typeof (GSStatsUnloaded_t)));
        if (Platform.PackSmall)
          d6 = (Callback.StdCall.GetSize) (() => Marshal.SizeOf(typeof (GSStatsUnloaded_t.PackSmall)));
