...
// Type: SteamNative.PersonaStateChange_t
...
namespace SteamNative
{
  ...
  {
    public const int CallbackId = 304;
    public ulong SteamID;
    public int ChangeFlags;

    public static PersonaStateChange_t FromPointer(IntPtr p)
    {
      return Platform.PackSmall ? (PersonaStateChange_t) (PersonaStateChange_t.PackSmall) Marshal.PtrToStructure(p, typeof (PersonaStateChange_t.PackSmall)) : (PersonaStateChange_t) Marshal.PtrToStructure(p, typeof (PersonaStateChange_t));
    }

    public static void RegisterCallback(
      BaseSteamworks steamworks,
      Action<PersonaStateChange_t, bool> CallbackFunction)
    {
      CallbackHandle handle = new CallbackHandle();
      handle.steamworks = steamworks;
      if (Config.UseThisCall)
      {
        Callback.ThisCall.Result d1 = (Callback.ThisCall.Result) ((_, p) => CallbackFunction(PersonaStateChange_t.FromPointer(p), false));
        Callback.ThisCall.ResultWithInfo d2 = (Callback.ThisCall.ResultWithInfo) ((_, p, bIOFailure, hSteamAPICall) => CallbackFunction(PersonaStateChange_t.FromPointer(p), bIOFailure));
        Callback.ThisCall.GetSize d3 = (Callback.ThisCall.GetSize) (_ => Marshal.SizeOf(typeof (PersonaStateChange_t)));
        if (Platform.PackSmall)
          d3 = (Callback.ThisCall.GetSize) (_ => Marshal.SizeOf(typeof (PersonaStateChange_t.PackSmall)));
      ...
      {
        Callback.StdCall.Result d4 = (Callback.StdCall.Result) (p => CallbackFunction(PersonaStateChange_t.FromPointer(p), false));
        Callback.StdCall.ResultWithInfo d5 = (Callback.StdCall.ResultWithInfo) ((p, bIOFailure, hSteamAPICall) => CallbackFunction(PersonaStateChange_t.FromPointer(p), bIOFailure));
        Callback.StdCall.GetSize d6 = (Callback.StdCall.GetSize) (() => Marshal.SizeOf(typeof (PersonaStateChange_t)));
        if (Platform.PackSmall)
          d6 = (Callback.StdCall.GetSize) (() => Marshal.SizeOf(typeof (PersonaStateChange_t.PackSmall)));
      ...
      steamworks.native.api.SteamAPI_RegisterCallback(handle.PinnedCallback.AddrOfPinnedObject(), 304);
