...
// Type: SteamNative.PS3TrophiesInstalled_t
...
namespace SteamNative
{
  ...
  internal struct PS3TrophiesInstalled_t
  {
    ...
    public ulong RequiredDiskSpace;

    public static PS3TrophiesInstalled_t FromPointer(IntPtr p)
    {
      return Platform.PackSmall ? (PS3TrophiesInstalled_t) (PS3TrophiesInstalled_t.PackSmall) Marshal.PtrToStructure(p, typeof (PS3TrophiesInstalled_t.PackSmall)) : (PS3TrophiesInstalled_t) Marshal.PtrToStructure(p, typeof (PS3TrophiesInstalled_t));
    }

    public static void RegisterCallback(
      BaseSteamworks steamworks,
      Action<PS3TrophiesInstalled_t, bool> CallbackFunction)
    {
      CallbackHandle handle = new CallbackHandle();
      handle.steamworks = steamworks;
      if (Config.UseThisCall)
      {
        Callback.ThisCall.Result d1 = (Callback.ThisCall.Result) ((_, p) => CallbackFunction(PS3TrophiesInstalled_t.FromPointer(p), false));
        Callback.ThisCall.ResultWithInfo d2 = (Callback.ThisCall.ResultWithInfo) ((_, p, bIOFailure, hSteamAPICall) => CallbackFunction(PS3TrophiesInstalled_t.FromPointer(p), bIOFailure));
        Callback.ThisCall.GetSize d3 = (Callback.ThisCall.GetSize) (_ => Marshal.SizeOf(typeof (PS3TrophiesInstalled_t)));
        if (Platform.PackSmall)
          d3 = (Callback.ThisCall.GetSize) (_ => Marshal.SizeOf(typeof (PS3TrophiesInstalled_t.PackSmall)));
      ...
      {
        Callback.StdCall.Result d4 = (Callback.StdCall.Result) (p => CallbackFunction(PS3TrophiesInstalled_t.FromPointer(p), false));
        Callback.StdCall.ResultWithInfo d5 = (Callback.StdCall.ResultWithInfo) ((p, bIOFailure, hSteamAPICall) => CallbackFunction(PS3TrophiesInstalled_t.FromPointer(p), bIOFailure));
        Callback.StdCall.GetSize d6 = (Callback.StdCall.GetSize) (() => Marshal.SizeOf(typeof (PS3TrophiesInstalled_t)));
        if (Platform.PackSmall)
          d6 = (Callback.StdCall.GetSize) (() => Marshal.SizeOf(typeof (PS3TrophiesInstalled_t.PackSmall)));
