...
internal class SteamApps : IDisposable {
	...
	public bool BGetDLCDataByIndex(
		...
		var cchNameBufferSize = 4096;
		var flag = platform.ISteamApps_BGetDLCDataByIndex(iDLC, ref pAppID.Value, ref pbAvailable, stringBuilder,
			cchNameBufferSize);
		if (!flag)
			return flag;
	...
	public string GetAppInstallDir(AppId_t appID) {
		...
		return platform.ISteamApps_GetAppInstallDir(appID.Value, stringBuilder, cchFolderBufferSize) <= 0U
			? null
			: stringBuilder.ToString();
	}

	public ulong GetAppOwner() {
		return (ulong)platform.ISteamApps_GetAppOwner();
	}
	...
	public string GetCurrentBetaName() {
		var stringBuilder = Helpers.TakeStringBuilder();
		var cchNameBufferSize = 4096;
		return !platform.ISteamApps_GetCurrentBetaName(stringBuilder, cchNameBufferSize)
			? null
			: stringBuilder.ToString();
	}
	...
	public CallbackHandle GetFileDetails(
		...
		var fileDetails = platform.ISteamApps_GetFileDetails(pszFileName);
		return CallbackFunction == null
			? null
			: FileDetailsResult_t.CallResult(steamworks, fileDetails, CallbackFunction);
	}
