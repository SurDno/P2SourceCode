...
// Type: SteamNative.GameWebCallback_t
...
namespace SteamNative
{
  ...
  {
    public const int CallbackId = 164;
    [MarshalAs(UnmanagedType.ByValTStr, SizeConst = 256)]
    public string URL;

    public static GameWebCallback_t FromPointer(IntPtr p)
    {
      return Platform.PackSmall ? (GameWebCallback_t) (GameWebCallback_t.PackSmall) Marshal.PtrToStructure(p, typeof (GameWebCallback_t.PackSmall)) : (GameWebCallback_t) Marshal.PtrToStructure(p, typeof (GameWebCallback_t));
    }

    public static void RegisterCallback(
      BaseSteamworks steamworks,
      Action<GameWebCallback_t, bool> CallbackFunction)
    {
      CallbackHandle handle = new CallbackHandle();
      handle.steamworks = steamworks;
      if (Config.UseThisCall)
      {
        Callback.ThisCall.Result d1 = (Callback.ThisCall.Result) ((_, p) => CallbackFunction(GameWebCallback_t.FromPointer(p), false));
        Callback.ThisCall.ResultWithInfo d2 = (Callback.ThisCall.ResultWithInfo) ((_, p, bIOFailure, hSteamAPICall) => CallbackFunction(GameWebCallback_t.FromPointer(p), bIOFailure));
        Callback.ThisCall.GetSize d3 = (Callback.ThisCall.GetSize) (_ => Marshal.SizeOf(typeof (GameWebCallback_t)));
        if (Platform.PackSmall)
          d3 = (Callback.ThisCall.GetSize) (_ => Marshal.SizeOf(typeof (GameWebCallback_t.PackSmall)));
      ...
      {
        Callback.StdCall.Result d4 = (Callback.StdCall.Result) (p => CallbackFunction(GameWebCallback_t.FromPointer(p), false));
        Callback.StdCall.ResultWithInfo d5 = (Callback.StdCall.ResultWithInfo) ((p, bIOFailure, hSteamAPICall) => CallbackFunction(GameWebCallback_t.FromPointer(p), bIOFailure));
        Callback.StdCall.GetSize d6 = (Callback.StdCall.GetSize) (() => Marshal.SizeOf(typeof (GameWebCallback_t)));
        if (Platform.PackSmall)
          d6 = (Callback.StdCall.GetSize) (() => Marshal.SizeOf(typeof (GameWebCallback_t.PackSmall)));
      ...
      steamworks.native.api.SteamAPI_RegisterCallback(handle.PinnedCallback.AddrOfPinnedObject(), 164);
    ...
    internal struct PackSmall
    {
      ...
      {
        return new GameWebCallback_t() { URL = d.URL };
      }
    }
  }
}
