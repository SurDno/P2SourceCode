...
// Type: SteamNative.MusicPlayerWantsVolume_t
...
namespace SteamNative
{
  ...
  {
    public const int CallbackId = 4011;
    public float NewVolume;

    public static MusicPlayerWantsVolume_t FromPointer(IntPtr p)
    {
      return Platform.PackSmall ? (MusicPlayerWantsVolume_t) (MusicPlayerWantsVolume_t.PackSmall) Marshal.PtrToStructure(p, typeof (MusicPlayerWantsVolume_t.PackSmall)) : (MusicPlayerWantsVolume_t) Marshal.PtrToStructure(p, typeof (MusicPlayerWantsVolume_t));
    }

    public static void RegisterCallback(
      BaseSteamworks steamworks,
      Action<MusicPlayerWantsVolume_t, bool> CallbackFunction)
    {
      CallbackHandle handle = new CallbackHandle();
      handle.steamworks = steamworks;
      if (Config.UseThisCall)
      {
        Callback.ThisCall.Result d1 = (Callback.ThisCall.Result) ((_, p) => CallbackFunction(MusicPlayerWantsVolume_t.FromPointer(p), false));
        Callback.ThisCall.ResultWithInfo d2 = (Callback.ThisCall.ResultWithInfo) ((_, p, bIOFailure, hSteamAPICall) => CallbackFunction(MusicPlayerWantsVolume_t.FromPointer(p), bIOFailure));
        Callback.ThisCall.GetSize d3 = (Callback.ThisCall.GetSize) (_ => Marshal.SizeOf(typeof (MusicPlayerWantsVolume_t)));
        if (Platform.PackSmall)
          d3 = (Callback.ThisCall.GetSize) (_ => Marshal.SizeOf(typeof (MusicPlayerWantsVolume_t.PackSmall)));
      ...
      {
        Callback.StdCall.Result d4 = (Callback.StdCall.Result) (p => CallbackFunction(MusicPlayerWantsVolume_t.FromPointer(p), false));
        Callback.StdCall.ResultWithInfo d5 = (Callback.StdCall.ResultWithInfo) ((p, bIOFailure, hSteamAPICall) => CallbackFunction(MusicPlayerWantsVolume_t.FromPointer(p), bIOFailure));
        Callback.StdCall.GetSize d6 = (Callback.StdCall.GetSize) (() => Marshal.SizeOf(typeof (MusicPlayerWantsVolume_t)));
        if (Platform.PackSmall)
          d6 = (Callback.StdCall.GetSize) (() => Marshal.SizeOf(typeof (MusicPlayerWantsVolume_t.PackSmall)));
      ...
      steamworks.native.api.SteamAPI_RegisterCallback(handle.PinnedCallback.AddrOfPinnedObject(), 4011);
