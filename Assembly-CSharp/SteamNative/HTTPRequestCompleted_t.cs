...
// Type: SteamNative.HTTPRequestCompleted_t
...
namespace SteamNative
{
  ...
  {
    public const int CallbackId = 2101;
    public uint Request;
    ...
    {
      return Platform.PackSmall ? (HTTPRequestCompleted_t) (HTTPRequestCompleted_t.PackSmall) Marshal.PtrToStructure(p, typeof (HTTPRequestCompleted_t.PackSmall)) : (HTTPRequestCompleted_t) Marshal.PtrToStructure(p, typeof (HTTPRequestCompleted_t));
    }

    public static void RegisterCallback(
      BaseSteamworks steamworks,
      Action<HTTPRequestCompleted_t, bool> CallbackFunction)
    {
      CallbackHandle handle = new CallbackHandle();
      handle.steamworks = steamworks;
      if (Config.UseThisCall)
      {
        Callback.ThisCall.Result d1 = (Callback.ThisCall.Result) ((_, p) => CallbackFunction(HTTPRequestCompleted_t.FromPointer(p), false));
        Callback.ThisCall.ResultWithInfo d2 = (Callback.ThisCall.ResultWithInfo) ((_, p, bIOFailure, hSteamAPICall) => CallbackFunction(HTTPRequestCompleted_t.FromPointer(p), bIOFailure));
        Callback.ThisCall.GetSize d3 = (Callback.ThisCall.GetSize) (_ => Marshal.SizeOf(typeof (HTTPRequestCompleted_t)));
        if (Platform.PackSmall)
          d3 = (Callback.ThisCall.GetSize) (_ => Marshal.SizeOf(typeof (HTTPRequestCompleted_t.PackSmall)));
      ...
      {
        Callback.StdCall.Result d4 = (Callback.StdCall.Result) (p => CallbackFunction(HTTPRequestCompleted_t.FromPointer(p), false));
        Callback.StdCall.ResultWithInfo d5 = (Callback.StdCall.ResultWithInfo) ((p, bIOFailure, hSteamAPICall) => CallbackFunction(HTTPRequestCompleted_t.FromPointer(p), bIOFailure));
        Callback.StdCall.GetSize d6 = (Callback.StdCall.GetSize) (() => Marshal.SizeOf(typeof (HTTPRequestCompleted_t)));
        if (Platform.PackSmall)
          d6 = (Callback.StdCall.GetSize) (() => Marshal.SizeOf(typeof (HTTPRequestCompleted_t.PackSmall)));
      ...
      steamworks.native.api.SteamAPI_RegisterCallback(handle.PinnedCallback.AddrOfPinnedObject(), 2101);
