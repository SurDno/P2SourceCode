...
// Type: SteamNative.HTML_JSAlert_t
...
namespace SteamNative
{
  ...
  {
    public const int CallbackId = 4514;
    public uint UnBrowserHandle;
    public string PchMessage;

    public static HTML_JSAlert_t FromPointer(IntPtr p)
    {
      return Platform.PackSmall ? (HTML_JSAlert_t) (HTML_JSAlert_t.PackSmall) Marshal.PtrToStructure(p, typeof (HTML_JSAlert_t.PackSmall)) : (HTML_JSAlert_t) Marshal.PtrToStructure(p, typeof (HTML_JSAlert_t));
    }

    public static void RegisterCallback(
      BaseSteamworks steamworks,
      Action<HTML_JSAlert_t, bool> CallbackFunction)
    {
      CallbackHandle handle = new CallbackHandle();
      handle.steamworks = steamworks;
      if (Config.UseThisCall)
      {
        Callback.ThisCall.Result d1 = (Callback.ThisCall.Result) ((_, p) => CallbackFunction(HTML_JSAlert_t.FromPointer(p), false));
        Callback.ThisCall.ResultWithInfo d2 = (Callback.ThisCall.ResultWithInfo) ((_, p, bIOFailure, hSteamAPICall) => CallbackFunction(HTML_JSAlert_t.FromPointer(p), bIOFailure));
        Callback.ThisCall.GetSize d3 = (Callback.ThisCall.GetSize) (_ => Marshal.SizeOf(typeof (HTML_JSAlert_t)));
        if (Platform.PackSmall)
          d3 = (Callback.ThisCall.GetSize) (_ => Marshal.SizeOf(typeof (HTML_JSAlert_t.PackSmall)));
      ...
      {
        Callback.StdCall.Result d4 = (Callback.StdCall.Result) (p => CallbackFunction(HTML_JSAlert_t.FromPointer(p), false));
        Callback.StdCall.ResultWithInfo d5 = (Callback.StdCall.ResultWithInfo) ((p, bIOFailure, hSteamAPICall) => CallbackFunction(HTML_JSAlert_t.FromPointer(p), bIOFailure));
        Callback.StdCall.GetSize d6 = (Callback.StdCall.GetSize) (() => Marshal.SizeOf(typeof (HTML_JSAlert_t)));
        if (Platform.PackSmall)
          d6 = (Callback.StdCall.GetSize) (() => Marshal.SizeOf(typeof (HTML_JSAlert_t.PackSmall)));
      ...
      steamworks.native.api.SteamAPI_RegisterCallback(handle.PinnedCallback.AddrOfPinnedObject(), 4514);
