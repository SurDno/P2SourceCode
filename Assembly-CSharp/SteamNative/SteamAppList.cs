...
internal class SteamAppList : IDisposable {
	...
	public string GetAppInstallDir(AppId_t nAppID) {
		...
		var cchNameMax = 4096;
		return platform.ISteamAppList_GetAppInstallDir(nAppID.Value, stringBuilder, cchNameMax) <= 0
			? null
	...
	public string GetAppName(AppId_t nAppID) {
		var stringBuilder = Helpers.TakeStringBuilder();
		var cchNameMax = 4096;
		return platform.ISteamAppList_GetAppName(nAppID.Value, stringBuilder, cchNameMax) <= 0
			? null
			: stringBuilder.ToString();
	}

	public unsafe uint GetInstalledApps(AppId_t[] pvecAppID) {
		var length = (uint)pvecAppID.Length;
