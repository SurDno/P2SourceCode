...
// Type: SteamNative.GameConnectedChatJoin_t
...
namespace SteamNative
{
  ...
  {
    public const int CallbackId = 339;
    public ulong SteamIDClanChat;
    public ulong SteamIDUser;

    public static GameConnectedChatJoin_t FromPointer(IntPtr p)
    {
      return Platform.PackSmall ? (GameConnectedChatJoin_t) (GameConnectedChatJoin_t.PackSmall) Marshal.PtrToStructure(p, typeof (GameConnectedChatJoin_t.PackSmall)) : (GameConnectedChatJoin_t) Marshal.PtrToStructure(p, typeof (GameConnectedChatJoin_t));
    }

    public static void RegisterCallback(
      BaseSteamworks steamworks,
      Action<GameConnectedChatJoin_t, bool> CallbackFunction)
    {
      CallbackHandle handle = new CallbackHandle();
      handle.steamworks = steamworks;
      if (Config.UseThisCall)
      {
        Callback.ThisCall.Result d1 = (Callback.ThisCall.Result) ((_, p) => CallbackFunction(GameConnectedChatJoin_t.FromPointer(p), false));
        Callback.ThisCall.ResultWithInfo d2 = (Callback.ThisCall.ResultWithInfo) ((_, p, bIOFailure, hSteamAPICall) => CallbackFunction(GameConnectedChatJoin_t.FromPointer(p), bIOFailure));
        Callback.ThisCall.GetSize d3 = (Callback.ThisCall.GetSize) (_ => Marshal.SizeOf(typeof (GameConnectedChatJoin_t)));
        if (Platform.PackSmall)
          d3 = (Callback.ThisCall.GetSize) (_ => Marshal.SizeOf(typeof (GameConnectedChatJoin_t.PackSmall)));
      ...
      {
        Callback.StdCall.Result d4 = (Callback.StdCall.Result) (p => CallbackFunction(GameConnectedChatJoin_t.FromPointer(p), false));
        Callback.StdCall.ResultWithInfo d5 = (Callback.StdCall.ResultWithInfo) ((p, bIOFailure, hSteamAPICall) => CallbackFunction(GameConnectedChatJoin_t.FromPointer(p), bIOFailure));
        Callback.StdCall.GetSize d6 = (Callback.StdCall.GetSize) (() => Marshal.SizeOf(typeof (GameConnectedChatJoin_t)));
        if (Platform.PackSmall)
          d6 = (Callback.StdCall.GetSize) (() => Marshal.SizeOf(typeof (GameConnectedChatJoin_t.PackSmall)));
      ...
      steamworks.native.api.SteamAPI_RegisterCallback(handle.PinnedCallback.AddrOfPinnedObject(), 339);
