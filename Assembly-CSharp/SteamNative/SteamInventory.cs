...
internal class SteamInventory : IDisposable {
	...
	public bool AddPromoItems(
		...
		uint unArrayLength) {
		return platform.ISteamInventory_AddPromoItems(ref pResultHandle.Value,
			pArrayItemDefs.Select(x => x.Value).ToArray(), unArrayLength);
	}
	...
	public bool DeserializeResult(
		...
		return platform.ISteamInventory_DeserializeResult(ref pOutResultHandle.Value, pBuffer, unBufferSize,
	...
	public bool ExchangeItems(
		...
		return platform.ISteamInventory_ExchangeItems(ref pResultHandle.Value,
			pArrayGenerate.Select(x => x.Value).ToArray(), punArrayGenerateQuantity, unArrayGenerateLength,
			pArrayDestroy.Select(x => x.Value).ToArray(), punArrayDestroyQuantity, unArrayDestroyLength);
	}
	...
	public bool GenerateItems(
		...
		uint unArrayLength) {
		return platform.ISteamInventory_GenerateItems(ref pResultHandle.Value,
			pArrayItemDefs.Select(x => x.Value).ToArray(), punArrayQuantity, unArrayLength);
	}
	...
	public unsafe SteamItemDef_t[] GetEligiblePromoItemDefinitionIDs(CSteamID steamID) {
		...
		if (!platform.ISteamInventory_GetEligiblePromoItemDefinitionIDs(steamID.Value, IntPtr.Zero,
			    out punItemDefIDsArraySize) || punItemDefIDsArraySize == 0U)
			return null;
		var steamItemDefTArray = new SteamItemDef_t[(int)punItemDefIDsArraySize];
		fixed (SteamItemDef_t* pItemDefIDs = steamItemDefTArray) {
			return !platform.ISteamInventory_GetEligiblePromoItemDefinitionIDs(steamID.Value, (IntPtr)pItemDefIDs,
	...
	public unsafe SteamItemDef_t[] GetItemDefinitionIDs() {
		uint punItemDefIDsArraySize = 0;
		if (!platform.ISteamInventory_GetItemDefinitionIDs(IntPtr.Zero, out punItemDefIDsArraySize) ||
		    punItemDefIDsArraySize == 0U)
			return null;
		var steamItemDefTArray = new SteamItemDef_t[(int)punItemDefIDsArraySize];
		fixed (SteamItemDef_t* pItemDefIDs = steamItemDefTArray) {
			return !platform.ISteamInventory_GetItemDefinitionIDs((IntPtr)pItemDefIDs, out punItemDefIDsArraySize)
				? null
				: steamItemDefTArray;
		}
	}
	...
	public bool GetItemDefinitionProperty(
		...
		var definitionProperty = platform.ISteamInventory_GetItemDefinitionProperty(iDefinition.Value, pchPropertyName,
			stringBuilder, out punValueBufferSizeOut);
	...
	public bool GetItemsByID(
		...
		return platform.ISteamInventory_GetItemsByID(ref pResultHandle.Value,
			pInstanceIDs.Select(x => x.Value).ToArray(), unCountInstanceIDs);
	}
	...
	public bool GetResultItemProperty(
		...
		var stringBuilder = Helpers.TakeStringBuilder();
		uint punValueBufferSizeOut = 4096;
		var resultItemProperty = platform.ISteamInventory_GetResultItemProperty(resultHandle.Value, unItemIndex,
			pchPropertyName, stringBuilder, out punValueBufferSizeOut);
	...
	public unsafe SteamItemDetails_t[] GetResultItems(SteamInventoryResult_t resultHandle) {
		...
		if (!platform.ISteamInventory_GetResultItems(resultHandle.Value, IntPtr.Zero, out punOutItemsArraySize) ||
		    punOutItemsArraySize == 0U)
			return null;
		var steamItemDetailsTArray = new SteamItemDetails_t[(int)punOutItemsArraySize];
		fixed (SteamItemDetails_t* pOutItemsArray = steamItemDetailsTArray) {
			return !platform.ISteamInventory_GetResultItems(resultHandle.Value, (IntPtr)pOutItemsArray,
				out punOutItemsArraySize)
				? null
				: steamItemDetailsTArray;
		}
	}
	...
	public CallbackHandle RequestEligiblePromoItemDefinitionsIDs(
		...
		var call = platform.ISteamInventory_RequestEligiblePromoItemDefinitionsIDs(steamID.Value);
		return CallbackFunction == null
			? null
			: SteamInventoryEligiblePromoItemDefIDs_t.CallResult(steamworks, call, CallbackFunction);
	}
	...
	public bool TradeItems(
		...
		return platform.ISteamInventory_TradeItems(ref pResultHandle.Value, steamIDTradePartner.Value,
			pArrayGive.Select(x => x.Value).ToArray(), pArrayGiveQuantity, nArrayGiveLength,
			pArrayGet.Select(x => x.Value).ToArray(), pArrayGetQuantity, nArrayGetLength);
	}
	...
	public bool TransferItemQuantity(
		...
		return platform.ISteamInventory_TransferItemQuantity(ref pResultHandle.Value, itemIdSource.Value, unQuantity,
			itemIdDest.Value);
	}
