...
// Type: SteamNative.GameLobbyJoinRequested_t
...
namespace SteamNative
{
  ...
  {
    public const int CallbackId = 333;
    public ulong SteamIDLobby;
    public ulong SteamIDFriend;

    public static GameLobbyJoinRequested_t FromPointer(IntPtr p)
    {
      return Platform.PackSmall ? (GameLobbyJoinRequested_t) (GameLobbyJoinRequested_t.PackSmall) Marshal.PtrToStructure(p, typeof (GameLobbyJoinRequested_t.PackSmall)) : (GameLobbyJoinRequested_t) Marshal.PtrToStructure(p, typeof (GameLobbyJoinRequested_t));
    }

    public static void RegisterCallback(
      BaseSteamworks steamworks,
      Action<GameLobbyJoinRequested_t, bool> CallbackFunction)
    {
      CallbackHandle handle = new CallbackHandle();
      handle.steamworks = steamworks;
      if (Config.UseThisCall)
      {
        Callback.ThisCall.Result d1 = (Callback.ThisCall.Result) ((_, p) => CallbackFunction(GameLobbyJoinRequested_t.FromPointer(p), false));
        Callback.ThisCall.ResultWithInfo d2 = (Callback.ThisCall.ResultWithInfo) ((_, p, bIOFailure, hSteamAPICall) => CallbackFunction(GameLobbyJoinRequested_t.FromPointer(p), bIOFailure));
        Callback.ThisCall.GetSize d3 = (Callback.ThisCall.GetSize) (_ => Marshal.SizeOf(typeof (GameLobbyJoinRequested_t)));
        if (Platform.PackSmall)
          d3 = (Callback.ThisCall.GetSize) (_ => Marshal.SizeOf(typeof (GameLobbyJoinRequested_t.PackSmall)));
      ...
      {
        Callback.StdCall.Result d4 = (Callback.StdCall.Result) (p => CallbackFunction(GameLobbyJoinRequested_t.FromPointer(p), false));
        Callback.StdCall.ResultWithInfo d5 = (Callback.StdCall.ResultWithInfo) ((p, bIOFailure, hSteamAPICall) => CallbackFunction(GameLobbyJoinRequested_t.FromPointer(p), bIOFailure));
        Callback.StdCall.GetSize d6 = (Callback.StdCall.GetSize) (() => Marshal.SizeOf(typeof (GameLobbyJoinRequested_t)));
        if (Platform.PackSmall)
          d6 = (Callback.StdCall.GetSize) (() => Marshal.SizeOf(typeof (GameLobbyJoinRequested_t.PackSmall)));
      ...
      steamworks.native.api.SteamAPI_RegisterCallback(handle.PinnedCallback.AddrOfPinnedObject(), 333);
