...
// Type: SteamNative.P2PSessionConnectFail_t
...
namespace SteamNative
{
  ...
  {
    public const int CallbackId = 1203;
    public ulong SteamIDRemote;
    public byte P2PSessionError;

    public static P2PSessionConnectFail_t FromPointer(IntPtr p)
    {
      return Platform.PackSmall ? (P2PSessionConnectFail_t) (P2PSessionConnectFail_t.PackSmall) Marshal.PtrToStructure(p, typeof (P2PSessionConnectFail_t.PackSmall)) : (P2PSessionConnectFail_t) Marshal.PtrToStructure(p, typeof (P2PSessionConnectFail_t));
    }

    public static void RegisterCallback(
      BaseSteamworks steamworks,
      Action<P2PSessionConnectFail_t, bool> CallbackFunction)
    {
      CallbackHandle handle = new CallbackHandle();
      handle.steamworks = steamworks;
      if (Config.UseThisCall)
      {
        Callback.ThisCall.Result d1 = (Callback.ThisCall.Result) ((_, p) => CallbackFunction(P2PSessionConnectFail_t.FromPointer(p), false));
        Callback.ThisCall.ResultWithInfo d2 = (Callback.ThisCall.ResultWithInfo) ((_, p, bIOFailure, hSteamAPICall) => CallbackFunction(P2PSessionConnectFail_t.FromPointer(p), bIOFailure));
        Callback.ThisCall.GetSize d3 = (Callback.ThisCall.GetSize) (_ => Marshal.SizeOf(typeof (P2PSessionConnectFail_t)));
        if (Platform.PackSmall)
          d3 = (Callback.ThisCall.GetSize) (_ => Marshal.SizeOf(typeof (P2PSessionConnectFail_t.PackSmall)));
      ...
      {
        Callback.StdCall.Result d4 = (Callback.StdCall.Result) (p => CallbackFunction(P2PSessionConnectFail_t.FromPointer(p), false));
        Callback.StdCall.ResultWithInfo d5 = (Callback.StdCall.ResultWithInfo) ((p, bIOFailure, hSteamAPICall) => CallbackFunction(P2PSessionConnectFail_t.FromPointer(p), bIOFailure));
        Callback.StdCall.GetSize d6 = (Callback.StdCall.GetSize) (() => Marshal.SizeOf(typeof (P2PSessionConnectFail_t)));
        if (Platform.PackSmall)
          d6 = (Callback.StdCall.GetSize) (() => Marshal.SizeOf(typeof (P2PSessionConnectFail_t.PackSmall)));
      ...
      steamworks.native.api.SteamAPI_RegisterCallback(handle.PinnedCallback.AddrOfPinnedObject(), 1203);
