...
// Type: SteamNative.RemoteStorageAppSyncStatusCheck_t
...
namespace SteamNative
{
  ...
  {
    public const int CallbackId = 1305;
    public uint AppID;
    public Result Result;

    public static RemoteStorageAppSyncStatusCheck_t FromPointer(IntPtr p)
    {
      return Platform.PackSmall ? (RemoteStorageAppSyncStatusCheck_t) (RemoteStorageAppSyncStatusCheck_t.PackSmall) Marshal.PtrToStructure(p, typeof (RemoteStorageAppSyncStatusCheck_t.PackSmall)) : (RemoteStorageAppSyncStatusCheck_t) Marshal.PtrToStructure(p, typeof (RemoteStorageAppSyncStatusCheck_t));
    }

    public static void RegisterCallback(
      BaseSteamworks steamworks,
      Action<RemoteStorageAppSyncStatusCheck_t, bool> CallbackFunction)
    {
      CallbackHandle handle = new CallbackHandle();
      handle.steamworks = steamworks;
      if (Config.UseThisCall)
      {
        Callback.ThisCall.Result d1 = (Callback.ThisCall.Result) ((_, p) => CallbackFunction(RemoteStorageAppSyncStatusCheck_t.FromPointer(p), false));
        Callback.ThisCall.ResultWithInfo d2 = (Callback.ThisCall.ResultWithInfo) ((_, p, bIOFailure, hSteamAPICall) => CallbackFunction(RemoteStorageAppSyncStatusCheck_t.FromPointer(p), bIOFailure));
        Callback.ThisCall.GetSize d3 = (Callback.ThisCall.GetSize) (_ => Marshal.SizeOf(typeof (RemoteStorageAppSyncStatusCheck_t)));
        if (Platform.PackSmall)
          d3 = (Callback.ThisCall.GetSize) (_ => Marshal.SizeOf(typeof (RemoteStorageAppSyncStatusCheck_t.PackSmall)));
      ...
      {
        Callback.StdCall.Result d4 = (Callback.StdCall.Result) (p => CallbackFunction(RemoteStorageAppSyncStatusCheck_t.FromPointer(p), false));
        Callback.StdCall.ResultWithInfo d5 = (Callback.StdCall.ResultWithInfo) ((p, bIOFailure, hSteamAPICall) => CallbackFunction(RemoteStorageAppSyncStatusCheck_t.FromPointer(p), bIOFailure));
        Callback.StdCall.GetSize d6 = (Callback.StdCall.GetSize) (() => Marshal.SizeOf(typeof (RemoteStorageAppSyncStatusCheck_t)));
        if (Platform.PackSmall)
          d6 = (Callback.StdCall.GetSize) (() => Marshal.SizeOf(typeof (RemoteStorageAppSyncStatusCheck_t.PackSmall)));
      ...
      steamworks.native.api.SteamAPI_RegisterCallback(handle.PinnedCallback.AddrOfPinnedObject(), 1305);
    ...
    internal struct PackSmall
    {
      ...
      public static implicit operator RemoteStorageAppSyncStatusCheck_t(
        RemoteStorageAppSyncStatusCheck_t.PackSmall d)
      {
