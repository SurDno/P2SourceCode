...
// Type: SteamNative.SteamInventoryFullUpdate_t
...
namespace SteamNative
{
  ...
  {
    public const int CallbackId = 4701;
    public int Handle;

    public static SteamInventoryFullUpdate_t FromPointer(IntPtr p)
    {
      return Platform.PackSmall ? (SteamInventoryFullUpdate_t) (SteamInventoryFullUpdate_t.PackSmall) Marshal.PtrToStructure(p, typeof (SteamInventoryFullUpdate_t.PackSmall)) : (SteamInventoryFullUpdate_t) Marshal.PtrToStructure(p, typeof (SteamInventoryFullUpdate_t));
    }

    public static void RegisterCallback(
      BaseSteamworks steamworks,
      Action<SteamInventoryFullUpdate_t, bool> CallbackFunction)
    {
      CallbackHandle handle = new CallbackHandle();
      handle.steamworks = steamworks;
      if (Config.UseThisCall)
      {
        Callback.ThisCall.Result d1 = (Callback.ThisCall.Result) ((_, p) => CallbackFunction(SteamInventoryFullUpdate_t.FromPointer(p), false));
        Callback.ThisCall.ResultWithInfo d2 = (Callback.ThisCall.ResultWithInfo) ((_, p, bIOFailure, hSteamAPICall) => CallbackFunction(SteamInventoryFullUpdate_t.FromPointer(p), bIOFailure));
        Callback.ThisCall.GetSize d3 = (Callback.ThisCall.GetSize) (_ => Marshal.SizeOf(typeof (SteamInventoryFullUpdate_t)));
        if (Platform.PackSmall)
          d3 = (Callback.ThisCall.GetSize) (_ => Marshal.SizeOf(typeof (SteamInventoryFullUpdate_t.PackSmall)));
      ...
      {
        Callback.StdCall.Result d4 = (Callback.StdCall.Result) (p => CallbackFunction(SteamInventoryFullUpdate_t.FromPointer(p), false));
        Callback.StdCall.ResultWithInfo d5 = (Callback.StdCall.ResultWithInfo) ((p, bIOFailure, hSteamAPICall) => CallbackFunction(SteamInventoryFullUpdate_t.FromPointer(p), bIOFailure));
        Callback.StdCall.GetSize d6 = (Callback.StdCall.GetSize) (() => Marshal.SizeOf(typeof (SteamInventoryFullUpdate_t)));
        if (Platform.PackSmall)
          d6 = (Callback.StdCall.GetSize) (() => Marshal.SizeOf(typeof (SteamInventoryFullUpdate_t.PackSmall)));
      ...
      steamworks.native.api.SteamAPI_RegisterCallback(handle.PinnedCallback.AddrOfPinnedObject(), 4701);
    ...
    internal struct PackSmall
    {
      ...
      public static implicit operator SteamInventoryFullUpdate_t(
        SteamInventoryFullUpdate_t.PackSmall d)
      {
