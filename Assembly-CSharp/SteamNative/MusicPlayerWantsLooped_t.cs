...
// Type: SteamNative.MusicPlayerWantsLooped_t
...
namespace SteamNative
{
  ...
  internal struct MusicPlayerWantsLooped_t
  {
    ...
    {
      return Platform.PackSmall ? (MusicPlayerWantsLooped_t) (MusicPlayerWantsLooped_t.PackSmall) Marshal.PtrToStructure(p, typeof (MusicPlayerWantsLooped_t.PackSmall)) : (MusicPlayerWantsLooped_t) Marshal.PtrToStructure(p, typeof (MusicPlayerWantsLooped_t));
    }

    public static void RegisterCallback(
      BaseSteamworks steamworks,
      Action<MusicPlayerWantsLooped_t, bool> CallbackFunction)
    {
      CallbackHandle handle = new CallbackHandle();
      handle.steamworks = steamworks;
      if (Config.UseThisCall)
      {
        Callback.ThisCall.Result d1 = (Callback.ThisCall.Result) ((_, p) => CallbackFunction(MusicPlayerWantsLooped_t.FromPointer(p), false));
        Callback.ThisCall.ResultWithInfo d2 = (Callback.ThisCall.ResultWithInfo) ((_, p, bIOFailure, hSteamAPICall) => CallbackFunction(MusicPlayerWantsLooped_t.FromPointer(p), bIOFailure));
        Callback.ThisCall.GetSize d3 = (Callback.ThisCall.GetSize) (_ => Marshal.SizeOf(typeof (MusicPlayerWantsLooped_t)));
        if (Platform.PackSmall)
          d3 = (Callback.ThisCall.GetSize) (_ => Marshal.SizeOf(typeof (MusicPlayerWantsLooped_t.PackSmall)));
      ...
      {
        Callback.StdCall.Result d4 = (Callback.StdCall.Result) (p => CallbackFunction(MusicPlayerWantsLooped_t.FromPointer(p), false));
        Callback.StdCall.ResultWithInfo d5 = (Callback.StdCall.ResultWithInfo) ((p, bIOFailure, hSteamAPICall) => CallbackFunction(MusicPlayerWantsLooped_t.FromPointer(p), bIOFailure));
        Callback.StdCall.GetSize d6 = (Callback.StdCall.GetSize) (() => Marshal.SizeOf(typeof (MusicPlayerWantsLooped_t)));
        if (Platform.PackSmall)
          d6 = (Callback.StdCall.GetSize) (() => Marshal.SizeOf(typeof (MusicPlayerWantsLooped_t.PackSmall)));
      ...
      steamworks.native.api.SteamAPI_RegisterCallback(handle.PinnedCallback.AddrOfPinnedObject(), 4110);
