...
// Type: SteamNative.GameConnectedFriendChatMsg_t
...
namespace SteamNative
{
  ...
  {
    public const int CallbackId = 343;
    public ulong SteamIDUser;
    public int MessageID;

    public static GameConnectedFriendChatMsg_t FromPointer(IntPtr p)
    {
      return Platform.PackSmall ? (GameConnectedFriendChatMsg_t) (GameConnectedFriendChatMsg_t.PackSmall) Marshal.PtrToStructure(p, typeof (GameConnectedFriendChatMsg_t.PackSmall)) : (GameConnectedFriendChatMsg_t) Marshal.PtrToStructure(p, typeof (GameConnectedFriendChatMsg_t));
    }

    public static void RegisterCallback(
      BaseSteamworks steamworks,
      Action<GameConnectedFriendChatMsg_t, bool> CallbackFunction)
    {
      CallbackHandle handle = new CallbackHandle();
      handle.steamworks = steamworks;
      if (Config.UseThisCall)
      {
        Callback.ThisCall.Result d1 = (Callback.ThisCall.Result) ((_, p) => CallbackFunction(GameConnectedFriendChatMsg_t.FromPointer(p), false));
        Callback.ThisCall.ResultWithInfo d2 = (Callback.ThisCall.ResultWithInfo) ((_, p, bIOFailure, hSteamAPICall) => CallbackFunction(GameConnectedFriendChatMsg_t.FromPointer(p), bIOFailure));
        Callback.ThisCall.GetSize d3 = (Callback.ThisCall.GetSize) (_ => Marshal.SizeOf(typeof (GameConnectedFriendChatMsg_t)));
        if (Platform.PackSmall)
          d3 = (Callback.ThisCall.GetSize) (_ => Marshal.SizeOf(typeof (GameConnectedFriendChatMsg_t.PackSmall)));
      ...
      {
        Callback.StdCall.Result d4 = (Callback.StdCall.Result) (p => CallbackFunction(GameConnectedFriendChatMsg_t.FromPointer(p), false));
        Callback.StdCall.ResultWithInfo d5 = (Callback.StdCall.ResultWithInfo) ((p, bIOFailure, hSteamAPICall) => CallbackFunction(GameConnectedFriendChatMsg_t.FromPointer(p), bIOFailure));
        Callback.StdCall.GetSize d6 = (Callback.StdCall.GetSize) (() => Marshal.SizeOf(typeof (GameConnectedFriendChatMsg_t)));
        if (Platform.PackSmall)
          d6 = (Callback.StdCall.GetSize) (() => Marshal.SizeOf(typeof (GameConnectedFriendChatMsg_t.PackSmall)));
      ...
      steamworks.native.api.SteamAPI_RegisterCallback(handle.PinnedCallback.AddrOfPinnedObject(), 343);
    ...
    internal struct PackSmall
    {
      ...
      public static implicit operator GameConnectedFriendChatMsg_t(
        GameConnectedFriendChatMsg_t.PackSmall d)
      {
