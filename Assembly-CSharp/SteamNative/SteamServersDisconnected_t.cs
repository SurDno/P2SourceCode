...
// Type: SteamNative.SteamServersDisconnected_t
...
namespace SteamNative
{
  ...
  {
    public const int CallbackId = 103;
    public Result Result;

    public static SteamServersDisconnected_t FromPointer(IntPtr p)
    {
      return Platform.PackSmall ? (SteamServersDisconnected_t) (SteamServersDisconnected_t.PackSmall) Marshal.PtrToStructure(p, typeof (SteamServersDisconnected_t.PackSmall)) : (SteamServersDisconnected_t) Marshal.PtrToStructure(p, typeof (SteamServersDisconnected_t));
    }

    public static void RegisterCallback(
      BaseSteamworks steamworks,
      Action<SteamServersDisconnected_t, bool> CallbackFunction)
    {
      CallbackHandle handle = new CallbackHandle();
      handle.steamworks = steamworks;
      if (Config.UseThisCall)
      {
        Callback.ThisCall.Result d1 = (Callback.ThisCall.Result) ((_, p) => CallbackFunction(SteamServersDisconnected_t.FromPointer(p), false));
        Callback.ThisCall.ResultWithInfo d2 = (Callback.ThisCall.ResultWithInfo) ((_, p, bIOFailure, hSteamAPICall) => CallbackFunction(SteamServersDisconnected_t.FromPointer(p), bIOFailure));
        Callback.ThisCall.GetSize d3 = (Callback.ThisCall.GetSize) (_ => Marshal.SizeOf(typeof (SteamServersDisconnected_t)));
        if (Platform.PackSmall)
          d3 = (Callback.ThisCall.GetSize) (_ => Marshal.SizeOf(typeof (SteamServersDisconnected_t.PackSmall)));
      ...
      {
        Callback.StdCall.Result d4 = (Callback.StdCall.Result) (p => CallbackFunction(SteamServersDisconnected_t.FromPointer(p), false));
        Callback.StdCall.ResultWithInfo d5 = (Callback.StdCall.ResultWithInfo) ((p, bIOFailure, hSteamAPICall) => CallbackFunction(SteamServersDisconnected_t.FromPointer(p), bIOFailure));
        Callback.StdCall.GetSize d6 = (Callback.StdCall.GetSize) (() => Marshal.SizeOf(typeof (SteamServersDisconnected_t)));
        if (Platform.PackSmall)
          d6 = (Callback.StdCall.GetSize) (() => Marshal.SizeOf(typeof (SteamServersDisconnected_t.PackSmall)));
      ...
      steamworks.native.api.SteamAPI_RegisterCallback(handle.PinnedCallback.AddrOfPinnedObject(), 103);
    ...
    internal struct PackSmall
    {
      ...
      public static implicit operator SteamServersDisconnected_t(
        SteamServersDisconnected_t.PackSmall d)
      {
