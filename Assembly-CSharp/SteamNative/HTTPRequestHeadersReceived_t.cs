...
// Type: SteamNative.HTTPRequestHeadersReceived_t
...
namespace SteamNative
{
  ...
  internal struct HTTPRequestHeadersReceived_t
  {
    ...
    {
      return Platform.PackSmall ? (HTTPRequestHeadersReceived_t) (HTTPRequestHeadersReceived_t.PackSmall) Marshal.PtrToStructure(p, typeof (HTTPRequestHeadersReceived_t.PackSmall)) : (HTTPRequestHeadersReceived_t) Marshal.PtrToStructure(p, typeof (HTTPRequestHeadersReceived_t));
    }

    public static void RegisterCallback(
      BaseSteamworks steamworks,
      Action<HTTPRequestHeadersReceived_t, bool> CallbackFunction)
    {
      CallbackHandle handle = new CallbackHandle();
      handle.steamworks = steamworks;
      if (Config.UseThisCall)
      {
        Callback.ThisCall.Result d1 = (Callback.ThisCall.Result) ((_, p) => CallbackFunction(HTTPRequestHeadersReceived_t.FromPointer(p), false));
        Callback.ThisCall.ResultWithInfo d2 = (Callback.ThisCall.ResultWithInfo) ((_, p, bIOFailure, hSteamAPICall) => CallbackFunction(HTTPRequestHeadersReceived_t.FromPointer(p), bIOFailure));
        Callback.ThisCall.GetSize d3 = (Callback.ThisCall.GetSize) (_ => Marshal.SizeOf(typeof (HTTPRequestHeadersReceived_t)));
        if (Platform.PackSmall)
          d3 = (Callback.ThisCall.GetSize) (_ => Marshal.SizeOf(typeof (HTTPRequestHeadersReceived_t.PackSmall)));
      ...
      {
        Callback.StdCall.Result d4 = (Callback.StdCall.Result) (p => CallbackFunction(HTTPRequestHeadersReceived_t.FromPointer(p), false));
        Callback.StdCall.ResultWithInfo d5 = (Callback.StdCall.ResultWithInfo) ((p, bIOFailure, hSteamAPICall) => CallbackFunction(HTTPRequestHeadersReceived_t.FromPointer(p), bIOFailure));
        Callback.StdCall.GetSize d6 = (Callback.StdCall.GetSize) (() => Marshal.SizeOf(typeof (HTTPRequestHeadersReceived_t)));
        if (Platform.PackSmall)
          d6 = (Callback.StdCall.GetSize) (() => Marshal.SizeOf(typeof (HTTPRequestHeadersReceived_t.PackSmall)));
      ...
      steamworks.native.api.SteamAPI_RegisterCallback(handle.PinnedCallback.AddrOfPinnedObject(), 2102);
    ...
    internal struct PackSmall
    {
      ...
      public static implicit operator HTTPRequestHeadersReceived_t(
        HTTPRequestHeadersReceived_t.PackSmall d)
      {
