...
internal class SteamGameServerStats : IDisposable {
	...
	public CallbackHandle RequestUserStats(
		...
		var call = platform.ISteamGameServerStats_RequestUserStats(steamIDUser.Value);
	...
	public CallbackHandle StoreUserStats(
		...
		SteamAPICall_t steamApiCallT = 0UL;
		var call = platform.ISteamGameServerStats_StoreUserStats(steamIDUser.Value);
	...
	public bool UpdateUserAvgRateStat(
		...
		return platform.ISteamGameServerStats_UpdateUserAvgRateStat(steamIDUser.Value, pchName, flCountThisSession,
			dSessionLength);
	}
}