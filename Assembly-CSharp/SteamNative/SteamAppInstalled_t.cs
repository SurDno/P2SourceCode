...
// Type: SteamNative.SteamAppInstalled_t
...
namespace SteamNative
{
  ...
  {
    public const int CallbackId = 3901;
    public uint AppID;

    public static SteamAppInstalled_t FromPointer(IntPtr p)
    {
      return Platform.PackSmall ? (SteamAppInstalled_t) (SteamAppInstalled_t.PackSmall) Marshal.PtrToStructure(p, typeof (SteamAppInstalled_t.PackSmall)) : (SteamAppInstalled_t) Marshal.PtrToStructure(p, typeof (SteamAppInstalled_t));
    }

    public static void RegisterCallback(
      BaseSteamworks steamworks,
      Action<SteamAppInstalled_t, bool> CallbackFunction)
    {
      CallbackHandle handle = new CallbackHandle();
      handle.steamworks = steamworks;
      if (Config.UseThisCall)
      {
        Callback.ThisCall.Result d1 = (Callback.ThisCall.Result) ((_, p) => CallbackFunction(SteamAppInstalled_t.FromPointer(p), false));
        Callback.ThisCall.ResultWithInfo d2 = (Callback.ThisCall.ResultWithInfo) ((_, p, bIOFailure, hSteamAPICall) => CallbackFunction(SteamAppInstalled_t.FromPointer(p), bIOFailure));
        Callback.ThisCall.GetSize d3 = (Callback.ThisCall.GetSize) (_ => Marshal.SizeOf(typeof (SteamAppInstalled_t)));
        if (Platform.PackSmall)
          d3 = (Callback.ThisCall.GetSize) (_ => Marshal.SizeOf(typeof (SteamAppInstalled_t.PackSmall)));
      ...
      {
        Callback.StdCall.Result d4 = (Callback.StdCall.Result) (p => CallbackFunction(SteamAppInstalled_t.FromPointer(p), false));
        Callback.StdCall.ResultWithInfo d5 = (Callback.StdCall.ResultWithInfo) ((p, bIOFailure, hSteamAPICall) => CallbackFunction(SteamAppInstalled_t.FromPointer(p), bIOFailure));
        Callback.StdCall.GetSize d6 = (Callback.StdCall.GetSize) (() => Marshal.SizeOf(typeof (SteamAppInstalled_t)));
        if (Platform.PackSmall)
          d6 = (Callback.StdCall.GetSize) (() => Marshal.SizeOf(typeof (SteamAppInstalled_t.PackSmall)));
      ...
      steamworks.native.api.SteamAPI_RegisterCallback(handle.PinnedCallback.AddrOfPinnedObject(), 3901);
    ...
    internal struct PackSmall
    {
      ...
      {
        return new SteamAppInstalled_t() { AppID = d.AppID };
      }
    }
  }
}
