...
// Type: SteamNative.SocketStatusCallback_t
...
namespace SteamNative
{
  ...
  {
    public const int CallbackId = 1201;
    public uint Socket;
    public uint ListenSocket;
    public ulong SteamIDRemote;
    public int SNetSocketState;

    public static SocketStatusCallback_t FromPointer(IntPtr p)
    {
      return Platform.PackSmall ? (SocketStatusCallback_t) (SocketStatusCallback_t.PackSmall) Marshal.PtrToStructure(p, typeof (SocketStatusCallback_t.PackSmall)) : (SocketStatusCallback_t) Marshal.PtrToStructure(p, typeof (SocketStatusCallback_t));
    }

    public static void RegisterCallback(
      BaseSteamworks steamworks,
      Action<SocketStatusCallback_t, bool> CallbackFunction)
    {
      CallbackHandle handle = new CallbackHandle();
      handle.steamworks = steamworks;
      if (Config.UseThisCall)
      {
        Callback.ThisCall.Result d1 = (Callback.ThisCall.Result) ((_, p) => CallbackFunction(SocketStatusCallback_t.FromPointer(p), false));
        Callback.ThisCall.ResultWithInfo d2 = (Callback.ThisCall.ResultWithInfo) ((_, p, bIOFailure, hSteamAPICall) => CallbackFunction(SocketStatusCallback_t.FromPointer(p), bIOFailure));
        Callback.ThisCall.GetSize d3 = (Callback.ThisCall.GetSize) (_ => Marshal.SizeOf(typeof (SocketStatusCallback_t)));
        if (Platform.PackSmall)
          d3 = (Callback.ThisCall.GetSize) (_ => Marshal.SizeOf(typeof (SocketStatusCallback_t.PackSmall)));
      ...
      {
        Callback.StdCall.Result d4 = (Callback.StdCall.Result) (p => CallbackFunction(SocketStatusCallback_t.FromPointer(p), false));
        Callback.StdCall.ResultWithInfo d5 = (Callback.StdCall.ResultWithInfo) ((p, bIOFailure, hSteamAPICall) => CallbackFunction(SocketStatusCallback_t.FromPointer(p), bIOFailure));
        Callback.StdCall.GetSize d6 = (Callback.StdCall.GetSize) (() => Marshal.SizeOf(typeof (SocketStatusCallback_t)));
        if (Platform.PackSmall)
          d6 = (Callback.StdCall.GetSize) (() => Marshal.SizeOf(typeof (SocketStatusCallback_t.PackSmall)));
      ...
      steamworks.native.api.SteamAPI_RegisterCallback(handle.PinnedCallback.AddrOfPinnedObject(), 1201);
