...
// Type: SteamNative.GameOverlayActivated_t
...
namespace SteamNative
{
  ...
  {
    public const int CallbackId = 331;
    public byte Active;

    public static GameOverlayActivated_t FromPointer(IntPtr p)
    {
      return Platform.PackSmall ? (GameOverlayActivated_t) (GameOverlayActivated_t.PackSmall) Marshal.PtrToStructure(p, typeof (GameOverlayActivated_t.PackSmall)) : (GameOverlayActivated_t) Marshal.PtrToStructure(p, typeof (GameOverlayActivated_t));
    }

    public static void RegisterCallback(
      BaseSteamworks steamworks,
      Action<GameOverlayActivated_t, bool> CallbackFunction)
    {
      CallbackHandle handle = new CallbackHandle();
      handle.steamworks = steamworks;
      if (Config.UseThisCall)
      {
        Callback.ThisCall.Result d1 = (Callback.ThisCall.Result) ((_, p) => CallbackFunction(GameOverlayActivated_t.FromPointer(p), false));
        Callback.ThisCall.ResultWithInfo d2 = (Callback.ThisCall.ResultWithInfo) ((_, p, bIOFailure, hSteamAPICall) => CallbackFunction(GameOverlayActivated_t.FromPointer(p), bIOFailure));
        Callback.ThisCall.GetSize d3 = (Callback.ThisCall.GetSize) (_ => Marshal.SizeOf(typeof (GameOverlayActivated_t)));
        if (Platform.PackSmall)
          d3 = (Callback.ThisCall.GetSize) (_ => Marshal.SizeOf(typeof (GameOverlayActivated_t.PackSmall)));
      ...
      {
        Callback.StdCall.Result d4 = (Callback.StdCall.Result) (p => CallbackFunction(GameOverlayActivated_t.FromPointer(p), false));
        Callback.StdCall.ResultWithInfo d5 = (Callback.StdCall.ResultWithInfo) ((p, bIOFailure, hSteamAPICall) => CallbackFunction(GameOverlayActivated_t.FromPointer(p), bIOFailure));
        Callback.StdCall.GetSize d6 = (Callback.StdCall.GetSize) (() => Marshal.SizeOf(typeof (GameOverlayActivated_t)));
        if (Platform.PackSmall)
          d6 = (Callback.StdCall.GetSize) (() => Marshal.SizeOf(typeof (GameOverlayActivated_t.PackSmall)));
      ...
      steamworks.native.api.SteamAPI_RegisterCallback(handle.PinnedCallback.AddrOfPinnedObject(), 331);
