...
// Type: SteamNative.DownloadClanActivityCountsResult_t
...
namespace SteamNative
{
  ...
  {
    public const int CallbackId = 341;
    [MarshalAs(UnmanagedType.I1)]
    public bool Success;

    public static DownloadClanActivityCountsResult_t FromPointer(IntPtr p)
    {
      return Platform.PackSmall ? (DownloadClanActivityCountsResult_t) (DownloadClanActivityCountsResult_t.PackSmall) Marshal.PtrToStructure(p, typeof (DownloadClanActivityCountsResult_t.PackSmall)) : (DownloadClanActivityCountsResult_t) Marshal.PtrToStructure(p, typeof (DownloadClanActivityCountsResult_t));
    }

    public static void RegisterCallback(
      BaseSteamworks steamworks,
      Action<DownloadClanActivityCountsResult_t, bool> CallbackFunction)
    {
      CallbackHandle handle = new CallbackHandle();
      handle.steamworks = steamworks;
      if (Config.UseThisCall)
      {
        Callback.ThisCall.Result d1 = (Callback.ThisCall.Result) ((_, p) => CallbackFunction(DownloadClanActivityCountsResult_t.FromPointer(p), false));
        Callback.ThisCall.ResultWithInfo d2 = (Callback.ThisCall.ResultWithInfo) ((_, p, bIOFailure, hSteamAPICall) => CallbackFunction(DownloadClanActivityCountsResult_t.FromPointer(p), bIOFailure));
        Callback.ThisCall.GetSize d3 = (Callback.ThisCall.GetSize) (_ => Marshal.SizeOf(typeof (DownloadClanActivityCountsResult_t)));
        if (Platform.PackSmall)
          d3 = (Callback.ThisCall.GetSize) (_ => Marshal.SizeOf(typeof (DownloadClanActivityCountsResult_t.PackSmall)));
      ...
      {
        Callback.StdCall.Result d4 = (Callback.StdCall.Result) (p => CallbackFunction(DownloadClanActivityCountsResult_t.FromPointer(p), false));
        Callback.StdCall.ResultWithInfo d5 = (Callback.StdCall.ResultWithInfo) ((p, bIOFailure, hSteamAPICall) => CallbackFunction(DownloadClanActivityCountsResult_t.FromPointer(p), bIOFailure));
        Callback.StdCall.GetSize d6 = (Callback.StdCall.GetSize) (() => Marshal.SizeOf(typeof (DownloadClanActivityCountsResult_t)));
        if (Platform.PackSmall)
          d6 = (Callback.StdCall.GetSize) (() => Marshal.SizeOf(typeof (DownloadClanActivityCountsResult_t.PackSmall)));
      ...
      steamworks.native.api.SteamAPI_RegisterCallback(handle.PinnedCallback.AddrOfPinnedObject(), 341);
    ...
    internal struct PackSmall
    {
      ...
      public static implicit operator DownloadClanActivityCountsResult_t(
        DownloadClanActivityCountsResult_t.PackSmall d)
      {
