...
internal class SteamGameServer : IDisposable {
	...
	public CallbackHandle AssociateWithClan(
		...
		var call = platform.ISteamGameServer_AssociateWithClan(steamIDClan.Value);
		return CallbackFunction == null
			? null
			: AssociateWithClanResult_t.CallResult(steamworks, call, CallbackFunction);
	}
	...
	public CallbackHandle ComputeNewPlayerCompatibility(
		...
		SteamAPICall_t steamApiCallT = 0UL;
		var playerCompatibility = platform.ISteamGameServer_ComputeNewPlayerCompatibility(steamIDNewPlayer.Value);
		return CallbackFunction == null
			? null
			: ComputeNewPlayerCompatibilityResult_t.CallResult(steamworks, playerCompatibility, CallbackFunction);
	}
	...
	public CallbackHandle GetServerReputation(Action<GSReputation_t, bool> CallbackFunction = null) {
		SteamAPICall_t steamApiCallT = 0UL;
		var serverReputation = platform.ISteamGameServer_GetServerReputation();
		return CallbackFunction == null
			? null
			: GSReputation_t.CallResult(steamworks, serverReputation, CallbackFunction);
	}

	public ulong GetSteamID() {
		return (ulong)platform.ISteamGameServer_GetSteamID();
	}
	...
	public bool InitGameServer(
		...
		return platform.ISteamGameServer_InitGameServer(unIP, usGamePort, usQueryPort, unFlags, nGameAppId.Value,
			pchVersionString);
	}
	...
	public bool SendUserConnectAndAuthenticate(
		...
		return platform.ISteamGameServer_SendUserConnectAndAuthenticate(unIPClient, pvAuthBlob, cubAuthBlobSize,
			out pSteamIDUser.Value);
	}
