...
// Type: SteamNative.FavoritesListAccountsUpdated_t
...
namespace SteamNative
{
  ...
  {
    public const int CallbackId = 516;
    public Result Result;

    public static FavoritesListAccountsUpdated_t FromPointer(IntPtr p)
    {
      return Platform.PackSmall ? (FavoritesListAccountsUpdated_t) (FavoritesListAccountsUpdated_t.PackSmall) Marshal.PtrToStructure(p, typeof (FavoritesListAccountsUpdated_t.PackSmall)) : (FavoritesListAccountsUpdated_t) Marshal.PtrToStructure(p, typeof (FavoritesListAccountsUpdated_t));
    }

    public static void RegisterCallback(
      BaseSteamworks steamworks,
      Action<FavoritesListAccountsUpdated_t, bool> CallbackFunction)
    {
      CallbackHandle handle = new CallbackHandle();
      handle.steamworks = steamworks;
      if (Config.UseThisCall)
      {
        Callback.ThisCall.Result d1 = (Callback.ThisCall.Result) ((_, p) => CallbackFunction(FavoritesListAccountsUpdated_t.FromPointer(p), false));
        Callback.ThisCall.ResultWithInfo d2 = (Callback.ThisCall.ResultWithInfo) ((_, p, bIOFailure, hSteamAPICall) => CallbackFunction(FavoritesListAccountsUpdated_t.FromPointer(p), bIOFailure));
        Callback.ThisCall.GetSize d3 = (Callback.ThisCall.GetSize) (_ => Marshal.SizeOf(typeof (FavoritesListAccountsUpdated_t)));
        if (Platform.PackSmall)
          d3 = (Callback.ThisCall.GetSize) (_ => Marshal.SizeOf(typeof (FavoritesListAccountsUpdated_t.PackSmall)));
      ...
      {
        Callback.StdCall.Result d4 = (Callback.StdCall.Result) (p => CallbackFunction(FavoritesListAccountsUpdated_t.FromPointer(p), false));
        Callback.StdCall.ResultWithInfo d5 = (Callback.StdCall.ResultWithInfo) ((p, bIOFailure, hSteamAPICall) => CallbackFunction(FavoritesListAccountsUpdated_t.FromPointer(p), bIOFailure));
        Callback.StdCall.GetSize d6 = (Callback.StdCall.GetSize) (() => Marshal.SizeOf(typeof (FavoritesListAccountsUpdated_t)));
        if (Platform.PackSmall)
          d6 = (Callback.StdCall.GetSize) (() => Marshal.SizeOf(typeof (FavoritesListAccountsUpdated_t.PackSmall)));
      ...
      steamworks.native.api.SteamAPI_RegisterCallback(handle.PinnedCallback.AddrOfPinnedObject(), 516);
    ...
    internal struct PackSmall
    {
      ...
      public static implicit operator FavoritesListAccountsUpdated_t(
        FavoritesListAccountsUpdated_t.PackSmall d)
      {
