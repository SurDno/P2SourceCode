...
// Type: SteamNative.HTML_VerticalScroll_t
...
namespace SteamNative
{
  ...
  internal struct HTML_VerticalScroll_t
  {
    ...
    {
      return Platform.PackSmall ? (HTML_VerticalScroll_t) (HTML_VerticalScroll_t.PackSmall) Marshal.PtrToStructure(p, typeof (HTML_VerticalScroll_t.PackSmall)) : (HTML_VerticalScroll_t) Marshal.PtrToStructure(p, typeof (HTML_VerticalScroll_t));
    }

    public static void RegisterCallback(
      BaseSteamworks steamworks,
      Action<HTML_VerticalScroll_t, bool> CallbackFunction)
    {
      CallbackHandle handle = new CallbackHandle();
      handle.steamworks = steamworks;
      if (Config.UseThisCall)
      {
        Callback.ThisCall.Result d1 = (Callback.ThisCall.Result) ((_, p) => CallbackFunction(HTML_VerticalScroll_t.FromPointer(p), false));
        Callback.ThisCall.ResultWithInfo d2 = (Callback.ThisCall.ResultWithInfo) ((_, p, bIOFailure, hSteamAPICall) => CallbackFunction(HTML_VerticalScroll_t.FromPointer(p), bIOFailure));
        Callback.ThisCall.GetSize d3 = (Callback.ThisCall.GetSize) (_ => Marshal.SizeOf(typeof (HTML_VerticalScroll_t)));
        if (Platform.PackSmall)
          d3 = (Callback.ThisCall.GetSize) (_ => Marshal.SizeOf(typeof (HTML_VerticalScroll_t.PackSmall)));
      ...
      {
        Callback.StdCall.Result d4 = (Callback.StdCall.Result) (p => CallbackFunction(HTML_VerticalScroll_t.FromPointer(p), false));
        Callback.StdCall.ResultWithInfo d5 = (Callback.StdCall.ResultWithInfo) ((p, bIOFailure, hSteamAPICall) => CallbackFunction(HTML_VerticalScroll_t.FromPointer(p), bIOFailure));
        Callback.StdCall.GetSize d6 = (Callback.StdCall.GetSize) (() => Marshal.SizeOf(typeof (HTML_VerticalScroll_t)));
        if (Platform.PackSmall)
          d6 = (Callback.StdCall.GetSize) (() => Marshal.SizeOf(typeof (HTML_VerticalScroll_t.PackSmall)));
      ...
      steamworks.native.api.SteamAPI_RegisterCallback(handle.PinnedCallback.AddrOfPinnedObject(), 4512);
