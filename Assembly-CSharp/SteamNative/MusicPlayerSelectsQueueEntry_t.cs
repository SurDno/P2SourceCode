...
// Type: SteamNative.MusicPlayerSelectsQueueEntry_t
...
namespace SteamNative
{
  ...
  {
    public const int CallbackId = 4012;
    public int NID;

    public static MusicPlayerSelectsQueueEntry_t FromPointer(IntPtr p)
    {
      return Platform.PackSmall ? (MusicPlayerSelectsQueueEntry_t) (MusicPlayerSelectsQueueEntry_t.PackSmall) Marshal.PtrToStructure(p, typeof (MusicPlayerSelectsQueueEntry_t.PackSmall)) : (MusicPlayerSelectsQueueEntry_t) Marshal.PtrToStructure(p, typeof (MusicPlayerSelectsQueueEntry_t));
    }

    public static void RegisterCallback(
      BaseSteamworks steamworks,
      Action<MusicPlayerSelectsQueueEntry_t, bool> CallbackFunction)
    {
      CallbackHandle handle = new CallbackHandle();
      handle.steamworks = steamworks;
      if (Config.UseThisCall)
      {
        Callback.ThisCall.Result d1 = (Callback.ThisCall.Result) ((_, p) => CallbackFunction(MusicPlayerSelectsQueueEntry_t.FromPointer(p), false));
        Callback.ThisCall.ResultWithInfo d2 = (Callback.ThisCall.ResultWithInfo) ((_, p, bIOFailure, hSteamAPICall) => CallbackFunction(MusicPlayerSelectsQueueEntry_t.FromPointer(p), bIOFailure));
        Callback.ThisCall.GetSize d3 = (Callback.ThisCall.GetSize) (_ => Marshal.SizeOf(typeof (MusicPlayerSelectsQueueEntry_t)));
        if (Platform.PackSmall)
          d3 = (Callback.ThisCall.GetSize) (_ => Marshal.SizeOf(typeof (MusicPlayerSelectsQueueEntry_t.PackSmall)));
      ...
      {
        Callback.StdCall.Result d4 = (Callback.StdCall.Result) (p => CallbackFunction(MusicPlayerSelectsQueueEntry_t.FromPointer(p), false));
        Callback.StdCall.ResultWithInfo d5 = (Callback.StdCall.ResultWithInfo) ((p, bIOFailure, hSteamAPICall) => CallbackFunction(MusicPlayerSelectsQueueEntry_t.FromPointer(p), bIOFailure));
        Callback.StdCall.GetSize d6 = (Callback.StdCall.GetSize) (() => Marshal.SizeOf(typeof (MusicPlayerSelectsQueueEntry_t)));
        if (Platform.PackSmall)
          d6 = (Callback.StdCall.GetSize) (() => Marshal.SizeOf(typeof (MusicPlayerSelectsQueueEntry_t.PackSmall)));
      ...
      steamworks.native.api.SteamAPI_RegisterCallback(handle.PinnedCallback.AddrOfPinnedObject(), 4012);
    ...
    internal struct PackSmall
    {
      ...
      public static implicit operator MusicPlayerSelectsQueueEntry_t(
        MusicPlayerSelectsQueueEntry_t.PackSmall d)
      {
