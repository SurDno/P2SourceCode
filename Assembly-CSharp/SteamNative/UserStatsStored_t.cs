...
// Type: SteamNative.UserStatsStored_t
...
namespace SteamNative
{
  ...
  {
    public const int CallbackId = 1102;
    public ulong GameID;
    public Result Result;

    public static UserStatsStored_t FromPointer(IntPtr p)
    {
      return Platform.PackSmall ? (UserStatsStored_t) (UserStatsStored_t.PackSmall) Marshal.PtrToStructure(p, typeof (UserStatsStored_t.PackSmall)) : (UserStatsStored_t) Marshal.PtrToStructure(p, typeof (UserStatsStored_t));
    }

    public static void RegisterCallback(
      BaseSteamworks steamworks,
      Action<UserStatsStored_t, bool> CallbackFunction)
    {
      CallbackHandle handle = new CallbackHandle();
      handle.steamworks = steamworks;
      if (Config.UseThisCall)
      {
        Callback.ThisCall.Result d1 = (Callback.ThisCall.Result) ((_, p) => CallbackFunction(UserStatsStored_t.FromPointer(p), false));
        Callback.ThisCall.ResultWithInfo d2 = (Callback.ThisCall.ResultWithInfo) ((_, p, bIOFailure, hSteamAPICall) => CallbackFunction(UserStatsStored_t.FromPointer(p), bIOFailure));
        Callback.ThisCall.GetSize d3 = (Callback.ThisCall.GetSize) (_ => Marshal.SizeOf(typeof (UserStatsStored_t)));
        if (Platform.PackSmall)
          d3 = (Callback.ThisCall.GetSize) (_ => Marshal.SizeOf(typeof (UserStatsStored_t.PackSmall)));
      ...
      {
        Callback.StdCall.Result d4 = (Callback.StdCall.Result) (p => CallbackFunction(UserStatsStored_t.FromPointer(p), false));
        Callback.StdCall.ResultWithInfo d5 = (Callback.StdCall.ResultWithInfo) ((p, bIOFailure, hSteamAPICall) => CallbackFunction(UserStatsStored_t.FromPointer(p), bIOFailure));
        Callback.StdCall.GetSize d6 = (Callback.StdCall.GetSize) (() => Marshal.SizeOf(typeof (UserStatsStored_t)));
        if (Platform.PackSmall)
          d6 = (Callback.StdCall.GetSize) (() => Marshal.SizeOf(typeof (UserStatsStored_t.PackSmall)));
      ...
      steamworks.native.api.SteamAPI_RegisterCallback(handle.PinnedCallback.AddrOfPinnedObject(), 1102);
