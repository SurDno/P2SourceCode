...
// Type: SteamNative.DlcInstalled_t
...
namespace SteamNative
{
  ...
  {
    public const int CallbackId = 1005;
    public uint AppID;

    public static DlcInstalled_t FromPointer(IntPtr p)
    {
      return Platform.PackSmall ? (DlcInstalled_t) (DlcInstalled_t.PackSmall) Marshal.PtrToStructure(p, typeof (DlcInstalled_t.PackSmall)) : (DlcInstalled_t) Marshal.PtrToStructure(p, typeof (DlcInstalled_t));
    }

    public static void RegisterCallback(
      BaseSteamworks steamworks,
      Action<DlcInstalled_t, bool> CallbackFunction)
    {
      CallbackHandle handle = new CallbackHandle();
      handle.steamworks = steamworks;
      if (Config.UseThisCall)
      {
        Callback.ThisCall.Result d1 = (Callback.ThisCall.Result) ((_, p) => CallbackFunction(DlcInstalled_t.FromPointer(p), false));
        Callback.ThisCall.ResultWithInfo d2 = (Callback.ThisCall.ResultWithInfo) ((_, p, bIOFailure, hSteamAPICall) => CallbackFunction(DlcInstalled_t.FromPointer(p), bIOFailure));
        Callback.ThisCall.GetSize d3 = (Callback.ThisCall.GetSize) (_ => Marshal.SizeOf(typeof (DlcInstalled_t)));
        if (Platform.PackSmall)
          d3 = (Callback.ThisCall.GetSize) (_ => Marshal.SizeOf(typeof (DlcInstalled_t.PackSmall)));
      ...
      {
        Callback.StdCall.Result d4 = (Callback.StdCall.Result) (p => CallbackFunction(DlcInstalled_t.FromPointer(p), false));
        Callback.StdCall.ResultWithInfo d5 = (Callback.StdCall.ResultWithInfo) ((p, bIOFailure, hSteamAPICall) => CallbackFunction(DlcInstalled_t.FromPointer(p), bIOFailure));
        Callback.StdCall.GetSize d6 = (Callback.StdCall.GetSize) (() => Marshal.SizeOf(typeof (DlcInstalled_t)));
        if (Platform.PackSmall)
          d6 = (Callback.StdCall.GetSize) (() => Marshal.SizeOf(typeof (DlcInstalled_t.PackSmall)));
      ...
      steamworks.native.api.SteamAPI_RegisterCallback(handle.PinnedCallback.AddrOfPinnedObject(), 1005);
    ...
    internal struct PackSmall
    {
      ...
      {
        return new DlcInstalled_t() { AppID = d.AppID };
      }
    }
  }
}
