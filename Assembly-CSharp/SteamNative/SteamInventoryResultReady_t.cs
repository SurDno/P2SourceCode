...
// Type: SteamNative.SteamInventoryResultReady_t
...
namespace SteamNative
{
  ...
  {
    public const int CallbackId = 4700;
    public int Handle;
    public Result Esult;

    public static SteamInventoryResultReady_t FromPointer(IntPtr p)
    {
      return Platform.PackSmall ? (SteamInventoryResultReady_t) (SteamInventoryResultReady_t.PackSmall) Marshal.PtrToStructure(p, typeof (SteamInventoryResultReady_t.PackSmall)) : (SteamInventoryResultReady_t) Marshal.PtrToStructure(p, typeof (SteamInventoryResultReady_t));
    }

    public static void RegisterCallback(
      BaseSteamworks steamworks,
      Action<SteamInventoryResultReady_t, bool> CallbackFunction)
    {
      CallbackHandle handle = new CallbackHandle();
      handle.steamworks = steamworks;
      if (Config.UseThisCall)
      {
        Callback.ThisCall.Result d1 = (Callback.ThisCall.Result) ((_, p) => CallbackFunction(SteamInventoryResultReady_t.FromPointer(p), false));
        Callback.ThisCall.ResultWithInfo d2 = (Callback.ThisCall.ResultWithInfo) ((_, p, bIOFailure, hSteamAPICall) => CallbackFunction(SteamInventoryResultReady_t.FromPointer(p), bIOFailure));
        Callback.ThisCall.GetSize d3 = (Callback.ThisCall.GetSize) (_ => Marshal.SizeOf(typeof (SteamInventoryResultReady_t)));
        if (Platform.PackSmall)
          d3 = (Callback.ThisCall.GetSize) (_ => Marshal.SizeOf(typeof (SteamInventoryResultReady_t.PackSmall)));
      ...
      {
        Callback.StdCall.Result d4 = (Callback.StdCall.Result) (p => CallbackFunction(SteamInventoryResultReady_t.FromPointer(p), false));
        Callback.StdCall.ResultWithInfo d5 = (Callback.StdCall.ResultWithInfo) ((p, bIOFailure, hSteamAPICall) => CallbackFunction(SteamInventoryResultReady_t.FromPointer(p), bIOFailure));
        Callback.StdCall.GetSize d6 = (Callback.StdCall.GetSize) (() => Marshal.SizeOf(typeof (SteamInventoryResultReady_t)));
        if (Platform.PackSmall)
          d6 = (Callback.StdCall.GetSize) (() => Marshal.SizeOf(typeof (SteamInventoryResultReady_t.PackSmall)));
      ...
      steamworks.native.api.SteamAPI_RegisterCallback(handle.PinnedCallback.AddrOfPinnedObject(), 4700);
    ...
    internal struct PackSmall
    {
      ...
      public static implicit operator SteamInventoryResultReady_t(
        SteamInventoryResultReady_t.PackSmall d)
      {
