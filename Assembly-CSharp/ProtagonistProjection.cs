...
public class ProtagonistProjection : MonoBehaviour {
	...
	private void LateUpdate() {
		var transform = this.transform;
		var num =
			Mathf.Tan(InstanceByRequest<GraphicsGameSettings>.Instance.FieldOfView.Value * ((float)Math.PI / 360f)) /
			Mathf.Tan(fieldOfView * ((float)Math.PI / 360f)) * scale;
		...
		child.localPosition = Matrix4x4.TRS(cameraAnchor.localPosition, cameraAnchor.localRotation, Vector3.one).inverse
