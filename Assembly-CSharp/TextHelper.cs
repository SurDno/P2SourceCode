...
public class TextHelper {
	...
	private static string Localize(string message) {
		if (string.IsNullOrEmpty(message))
			return "";
		string str;
		for (var index = message.IndexOf('{'); index != -1; index = message.IndexOf('{', index + str.Length)) {
			var num = message.IndexOf('}', index + 1);
			if (num != -1) {
				var startIndex = num + 1;
				var tag = message.Substring(index, startIndex - index);
	...
	public static string FormatStringUpperCharsToCapital(string message, int fontSize) {
		if (string.IsNullOrEmpty(message))
			return null;
		var str = "<size=" + Mathf.RoundToInt(fontSize * 1.25f) + ">";
		var stringBuilder = new StringBuilder();
		var flag = false;
		for (var index = 0; index < message.Length; ++index) {
			var c = message[index];
	...
	public static string ReplaceTags(string text, string hotkeyPrefix, string hotkeySuffix) {
		...
		var str2 = "</hotkey>";
		while (true) {
			var length = text.IndexOf(str1);
			if (length != -1) {
				var num = text.IndexOf(str2);
				if (num != -1 && num > length) {
					var str3 = text.Substring(0, length);
					var str4 = text.Substring(num + str2.Length);
					var str5 = text.Substring(length + str1.Length, num - length - str1.Length);
