...
public class NpcState : MonoBehaviour, IEntityAttachable {
	...
	[Inspected] private bool inLodState;
	...
	[Inspected] public NpcStateEnum CurrentNpcState { get; private set; }
	...
	public WeaponEnum Weapon {
		...
		set {
			weapon = value;
			var weaponChangeEvent = WeaponChangeEvent;
	...
	public void MoveFollowTeleport(float followDistance, float trialTime, bool waitForTargetSeesMe) {
		...
		CheckState(CurrentNpcState);
		(states[CurrentNpcState] as NpcStateMoveFollowTeleport).Activate(followDistance, trialTime,
			waitForTargetSeesMe);
		if (!inLodState)
			return;
		states[CurrentNpcState].OnLodStateChanged(true);
	...
	public void PointOfInterest(
		...
		CheckState(CurrentNpcState);
		(states[CurrentNpcState] as NpcStatePointOfInterest).Activate(poiTime, poi, animation, animationIndex,
			animationsCount);
		if (!inLodState)
			return;
		states[CurrentNpcState].OnLodStateChanged(true);
	...
	public void POIExtraExit(float time) {
		if (inLodState)
			states[CurrentNpcState].OnLodStateChanged(false);
		var state = states[CurrentNpcState];
		states[CurrentNpcState].Shutdown();
	...
	public void FightFollowTarget(
		...
		CheckState(CurrentNpcState);
		(states[CurrentNpcState] as NpcStateFightFollowTarget).Activate(stopDistance, runDistance, retreatDistance,
			target, aim);
		if (!inLodState)
			return;
		states[CurrentNpcState].OnLodStateChanged(true);
	...
	private void Awake() {
		...
		if ((bool)(Object)pivot) {
			var animatorEventProxy = pivot.GetAnimatorEventProxy();
	...
	private void Proxy_AnimatorMoveEvent() {
		states[CurrentNpcState].OnAnimatorMove();
	}

	private void OnEnable() {
		ServiceLocator.GetService<LodService>().RegisterLod(this);
		foreach (var component in gameObject.GetComponents<BehaviorTree>())
