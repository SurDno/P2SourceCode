...
public class NGSS_ContactShadows : MonoBehaviour {
	...
	[Tooltip("Overall distance of the shadows.")] [Range(1f, 4f)]
	public float m_shadowsDistance = 1f;

	[Tooltip("The distance where shadows start to fade.")] [Range(0.1f, 4f)]
	public float m_shadowsFade = 1f;

	[Tooltip("Tweak this value if your objects display backface shadows.")] [Range(0.0f, 2f)]
	public float m_shadowsOffset = 0.325f;

	[Header("RAY SETTINGS")] [Tooltip("The higher the value, the ticker the shadows will look.")] [Range(0.0f, 1f)]
	public float m_rayWidth = 0.1f;

	[Tooltip(
		"Number of samplers between each step. The higher values produces less gaps between shadows. Keep this value as low as you can!")]
	[Range(16f, 128f)]
	public int m_raySamples = 64;

	[Tooltip(
		"Samplers scale over distance. Lower this value if you want to speed things up by doing less sampling on far away objects.")]
	[Range(0.0f, 1f)]
	...
	private Material mMaterial {
		get {
			if (_mMaterial == null) {
				...
				if (_mMaterial == null) {
					Debug.LogWarning(
						"NGSS Warning: can't find NGSS_ContactShadows shader, make sure it's on your project.", this);
					enabled = false;
					return null;
				}
			}
	...
	private void AddCommandBuffers() {
		...
		var flag = mCamera.actualRenderingPath == RenderingPath.Forward;
		if ((bool)(Object)mCamera) {
			foreach (var commandBuffer in mCamera.GetCommandBuffers(flag
				         ? CameraEvent.AfterDepthTexture
				         : CameraEvent.BeforeLighting))
				if (commandBuffer.name == computeShadowsCB.name)
					return;
			mCamera.AddCommandBuffer(flag ? CameraEvent.AfterDepthTexture : CameraEvent.BeforeLighting,
				computeShadowsCB);
		}

		var light = MonoBehaviourInstance<NGSS_ContactShadowsSource>.Instance?.Light;
		if (!(bool)(Object)light)
			return;
		foreach (var commandBuffer in light.GetCommandBuffers(LightEvent.AfterScreenspaceMask))
	...
	private void Deinitialize() {
		...
		var flag = mCamera.actualRenderingPath == RenderingPath.Forward;
		if ((bool)(Object)mCamera)
			mCamera.RemoveCommandBuffer(flag ? CameraEvent.AfterDepthTexture : CameraEvent.BeforeLighting,
				computeShadowsCB);
		var light = MonoBehaviourInstance<NGSS_ContactShadowsSource>.Instance?.Light;
		if ((bool)(Object)light)
	...
	private void Initialize() {
		...
		if (mCamera.actualRenderingPath == RenderingPath.VertexLit) {
			Debug.LogWarning(
				"Vertex Lit Rendering Path is not supported by NGSS Contact Shadows. Please set the Rendering Path in your game camera or Graphics Settings to something else than Vertex Lit.",
				this);
			enabled = false;
		} else {
			AddCommandBuffers();
			var id1 = Shader.PropertyToID("NGSS_ContactShadowRT");
			var id2 = Shader.PropertyToID("NGSS_ContactShadowRT2");
			var id3 = Shader.PropertyToID("NGSS_DepthSourceRT");
			...
			blendShadowsCB.Blit(BuiltinRenderTextureType.CurrentActive, BuiltinRenderTextureType.CurrentActive,
				mMaterial, 3);
			isInitialized = true;
		}
	}
