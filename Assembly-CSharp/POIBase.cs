...
public class POIBase : MonoBehaviour {
	public static HashSet<POIBase> ActivePOIs = new();

	[SerializeField] [FormerlySerializedAs("SupportedAnimations")]
	private POIAnimationEnum _supportedAnimations;

	[SerializeField] private bool supportsDialogs;
	...
	public float GetAngle(GameObject to) {
		var lhs = to.transform.position - transform.position;
		lhs = lhs.normalized;
		var vector3 = transform.forward * -1f;
		var num = Mathf.Acos(Vector3.Dot(lhs, vector3)) * 57.29578f;
		var rhs = to.transform.position - transform.position;
		var y = Vector3.Cross(vector3, rhs).y;
