...
public class PhonemeVisemeMapping {
	...
	public PhonemeVisemeMapping(string[] names, string[][] phns) {
		...
		for (var index = 0; index < phns.Length; ++index)
			mapping[index] = new phn_string_array_t(phns[index]);
	}

	public virtual int GetNumVisemes() {
		return visNames.Length;
	}

	public virtual string[] GetVisemeNames() {
		return visNames;
	}

	public string[] GetVisemePhonemes(string vis) {
		for (var index = 0; index < visNames.Length; ++index)
	...
	public string PhonemeToViseme(string phn) {
		foreach (var phnStringArrayT in mapping) {
			foreach (var phn1 in phnStringArrayT.phns)
	...
	public void ReadMapping(XmlTextReader reader, string endTag) {
		...
		for (var index = 0; index < stringListList.Count; ++index) {
	...
	private List<string> Read_Phns(XmlTextReader reader, string endTag) {
		...
		while (!flag && reader.Read())
			if (reader.Name == endTag && reader.NodeType == XmlNodeType.EndElement)
				...
				var str = XMLUtils.ReadXMLInnerText(reader, reader.Name);
				if (str.Length != 0)
					stringList.Add(str);
			}
