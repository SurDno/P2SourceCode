...
public class BoundCharacterStateChangeView : MonoBehaviour {
	...
	[SerializeField] private HideableView female;
	[SerializeField] private HideableView male;
	[SerializeField] private FloatView stat;
	[SerializeField] private RollAnimation roll;
	[SerializeField] private HideableView hadImmunity;
	[SerializeField] private HideableView hadInfection;
	[SerializeField] private HideableView medicated;
	[SerializeField] private EventView notInfected;
	[SerializeField] private EventView infected;
	[SerializeField] private EventView survived;
	[SerializeField] private EventView died;
	[SerializeField] private FloatView opacity;
	[SerializeField] private float fadeTime = 1f;
	[SerializeField] private float delayWithoutRoll = 2f;
	...
	public void ContinueWithoutRoll() {
		Invoke("Reveal", delayWithoutRoll);
	}

	private float FakeRoll(float min, float max) {
		var num = Random.value;
		var t = (float)(0.10000000149011612 + num * num * 0.89999997615814209);
	...
	public void Reveal() {
		...
		if (character.PreRollHealthState == BoundHealthStateEnum.Diseased &&
		    character.PreRollHealthState == BoundHealthStateEnum.Diseased)
		...
		if (character.BoundHealthState.Value == BoundHealthStateEnum.Dead)
			...
			if (character.BoundHealthState.Value != BoundHealthStateEnum.Normal ||
			    character.PreRollHealthState != BoundHealthStateEnum.Danger)
				return;
			notInfected.Invoke();
		}
	}
	...
	public void Show(BoundCharacterComponent character) {
		...
		var gender = BoundCharacterUtility.GetGender(character);
