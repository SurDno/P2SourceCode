...
public class PhysicsSimulation : MonoBehaviour {
	...
	public void RunSimulation() {
		...
		simulatedBodies = FindObjectsOfType<Rigidbody>()
			.Select(rb => new SimulatedBody(rb, rb.transform.IsChildOf(transform))).ToArray();
		foreach (var simulatedBody in simulatedBodies)
			if (simulatedBody.isChild) {
				var num = Random.Range(forceMinMax.x, forceMinMax.y);
				var f = (float)((randomizeForceAngle ? Random.Range(0.0f, 360f) : (double)forceAngleInDegrees) *
				                (Math.PI / 180.0));
				var vector3 = new Vector3(Mathf.Sin(f), 0.0f, Mathf.Cos(f));
	...
	private void AutoGenerateComponents() {
		...
		foreach (Transform transform in this.transform) {
			...
			if (!(bool)(Object)transform.GetComponent<Collider>())
				generatedColliders.Add(transform.gameObject.AddComponent<BoxCollider>());
		}
	}

	private void RemoveAutoGeneratedComponents() {
		foreach (Object generatedRigidbody in generatedRigidbodies)
			DestroyImmediate(generatedRigidbody);
		foreach (Object generatedCollider in generatedColliders)
