	...
	public void Update() {
		...
		if (InstanceByRequest<EngineApplication>.Instance.IsPaused)
			...
			if (enemy.IsReacting || enemy.IsQuickBlock) {
				...
				if (enemy.Enemy == null)
					return;
				var vector3 = enemy.Enemy.transform.position - enemy.transform.position;
				...
				if (enemy.IsAttacking || enemy.IsContrReacting) {
					...
					var num = 0.0f;
					var flag = enemy.TimeFromLastHit > (double)timeAfterHitToRun;
					if (NavMesh.Raycast(enemy.transform.position, enemy.Enemy.transform.position, out var _, -1)) {
						...
						if (agent.remainingDistance > (double)stopDistance) {
							num = !IsEnemyRunningAway()
								? (agent.remainingDistance > (double)runDistance) | flag ? 2f : 1f
								: 2f;
							enemy.RotationTarget = enemy.Enemy.transform;
						...
						num = !IsEnemyRunningAway()
							? ((!agent.hasPath || !agent.isActiveAndEnabled || !agent.isOnNavMesh ? 0 :
								agent.remainingDistance > (double)runDistance ? 1 : 0) | (flag ? 1 : 0)) != 0
								? 2f
								: 1f
							: 2f;
						enemy.RotationTarget = enemy.Enemy.transform;
