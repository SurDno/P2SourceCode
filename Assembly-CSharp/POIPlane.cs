...
public class POIPlane : POIBase {
	...
	public override void GetClosestTargetPoint(
		...
		GetClosestSurfacePoint(currentPosition, out closestSurfacePosition, out var _);
	...
	private void GetClosestSurfacePoint(
		...
		var vector3 = transform.InverseTransformPoint(surfacePosition);
		var position = new Vector3(Mathf.Clamp(vector3.x, (float)(-(double)Width / 2.0), Width / 2f), 0.0f,
			Mathf.Clamp(vector3.z, (float)(-(double)Length / 2.0), Length / 2f));
	...
	public void GetRandomPointOnSurface(out Vector3 position, out Quaternion rotation) {
		...
		position = transform.position + transform.right * Width / 2f * (float)(2.0 * (Random.value - 0.5)) +
		           transform.forward * Length / 2f * (float)(2.0 * (Random.value - 0.5));
	}
