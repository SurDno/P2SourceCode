...
public class NpcStateMoveFollowTeleport : NpcStateMoveBase {
	...
	public override void DoUpdate() {
		...
		if (player == null || Status != 0)
			return;
		var service = ServiceLocator.GetService<PostmanTeleportService>();
		var vector3 = player.transform.position - GameObject.transform.position;
		var magnitude1 = vector3.magnitude;
		...
		if (vector3.magnitude < (double)followDistance && TargetIsVisible(player)) {
			...
			if (agent.hasPath && agent.remainingDistance < (double)followDistance &&
			    magnitude3 < (double)followDistance) {
				if (!waitForTargetSeesMe || TargetSeesMe(player))
					Status = NpcStateStatusEnum.Success;
				CompleteTask(false);
				service.ReportPostmanIsOK(npcState.Owner, true);
	...
	private bool TargetSeesMe(GameObject target) {
		return Vector3.Angle((GameObject.transform.position - target.transform.position).normalized,
			target.transform.forward) < 30.0;
	}

	private bool TargetIsVisible(GameObject target) {
		return !NavMesh.Raycast(GameObject.transform.position, target.transform.position, out var _, -1);
	}
}