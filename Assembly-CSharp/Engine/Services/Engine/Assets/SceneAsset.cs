...
public class SceneAsset : IAsset {
	...
	[Inspected] private IScene reference;
	...
	[Inspected] public Scene Scene { get; private set; }
	...
	public void Update() {
		if (!initialized) {
			...
			var path = AssetDatabaseService.Instance.GetPath(reference.Id);
			if (path.IsNullOrEmpty()) {
				Debug.Log(ObjectInfoUtility.GetStream().Append("[Loader]").Append(" Scene not found : ")
					.Append(reference.Id).Append(" , context : ").Append(context));
				IsError = true;
				return;
			}

			Debug.Log(ObjectInfoUtility.GetStream().Append("[Loader]").Append(" Begin async load scene : ").Append(path)
		...
		if (async == null)
			...
			if (!IsDisposed || !Scene.IsValid())
				return;
			Debug.Log(ObjectInfoUtility.GetStream().Append("[Loader]").Append(" Unload scene : ").Append(Scene.path)
				.Append(" , context : ").Append(context).Append(" , reason : ").Append(reason));
