...
public class StatBar : ProgressViewBase {
	[SerializeField] [FormerlySerializedAs("leftFillColor")]
	private Color positiveFillColor;

	[SerializeField] [FormerlySerializedAs("reversedLeftFillColor")]
	private Color negativeFillColor;

	[SerializeField] [FormerlySerializedAs("reversedRightFillColor")]
	private Color negativeBaseColor;

	[SerializeField] [FormerlySerializedAs("rightFillColor")]
	private Color positiveBaseColor;

	[SerializeField] [FormerlySerializedAs("leftBlinkColor")]
	private Color negativeBlinkColor;

	[SerializeField] [FormerlySerializedAs("rightBlinkColor")]
	private Color positiveBlinkColor;

	[SerializeField] [FormerlySerializedAs("markerColor")]
	private Color markerColor;

	[SerializeField] [FormerlySerializedAs("leftLimit")]
	private float leftLimit = 0.05f;

	[SerializeField] [FormerlySerializedAs("rightLimit")]
	private float rightLimit = 0.95f;

	[SerializeField] [FormerlySerializedAs("markerWidth")]
	private float markerWidth = 0.05f;

	[SerializeField] [FormerlySerializedAs("tailAcceleration")]
	private float tailAcceleration = 0.5f;

	[Space] [SerializeField] [FormerlySerializedAs("Material")]
	private Material material;

	[SerializeField] [FormerlySerializedAs("Image")]
	private Sprite image;

	[SerializeField] [FormerlySerializedAs("IsReversed")]
	...
	[SerializeField] [Range(0.0f, 1f)] private float currentValue;
	...
	private void Start() {
		...
		var transform = (RectTransform)rightFill.transform;
		...
		var markerColor = this.markerColor with { a = 0.0f };
	...
	private Gradient CreateGradient() {
		var gameObject = new GameObject("Gradient", typeof(RectTransform), typeof(CanvasRenderer), typeof(Gradient));
		var component1 = gameObject.GetComponent<RectTransform>();
		...
		var component2 = gameObject.GetComponent<Gradient>();
	...
	private float BlinkPhase() {
		var f = Time.unscaledTime * 2f;
		return (float)(0.25 + Mathf.Abs((f - Mathf.Round(f)) * 2f) * 0.75);
	}

	private void Redraw() {
		var num1 = rightLimit - leftLimit;
		var num2 = leftLimit + num1 * currentValue;
		float num3;
		if (tailValue == (double)currentValue) {
			...
			num3 = num2;
			var num5 = leftLimit + num1 * tailValue;
			if (IsReversed)
