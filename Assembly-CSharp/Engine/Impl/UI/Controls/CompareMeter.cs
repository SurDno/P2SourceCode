...
public class CompareMeter : MonoBehaviour {
	...
	[SerializeField] [FormerlySerializedAs("_Unity_Allowed")]
	private Button unityAllowed;

	[SerializeField] [FormerlySerializedAs("_Unity_Forbidden")]
	private Button unityForbidden;

	[SerializeField] [FormerlySerializedAs("_Unity_Marker")]
	private Image unityMarker;

	[SerializeField] [FormerlySerializedAs("_Unity_Separator")]
	...
	protected void Calculate() {
		...
		if (float.IsNaN(factor) || float.IsPositiveInfinity(factor) || float.IsNegativeInfinity(factor)) {
			...
			var position = unityMarker.gameObject.transform.position;
			var rect = unitySeparator.GetComponent<RectTransform>().rect;
			...
			unityMarker.gameObject.SetActive(true);
			var position = unityMarker.gameObject.transform.position;
			var rect = unityForbidden.GetComponent<RectTransform>().rect;
			position.x = unityForbidden.gameObject.transform.position.x +
			             (unitySeparator.gameObject.transform.position.x -
			              unityForbidden.gameObject.transform.position.x) * Mathf.Clamp(factor, 0.0f, 1f);
			unityMarker.gameObject.transform.position = position;
		} else {
			unityAllowed.interactable = true;
			unityForbidden.interactable = false;
			unityMarker.gameObject.SetActive(true);
			var position = unityMarker.gameObject.transform.position;
			var rect = unityAllowed.GetComponent<RectTransform>().rect;
			position.x = unitySeparator.gameObject.transform.position.x +
			             (unityAllowed.gameObject.transform.position.x -
			              unitySeparator.gameObject.transform.position.x) * Mathf.Clamp(factor - 1f, 0.0f, 1f);
			unityMarker.gameObject.transform.position = position;
		}
	}
}