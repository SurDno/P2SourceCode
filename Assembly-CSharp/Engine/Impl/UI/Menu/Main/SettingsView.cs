...
public class SettingsView : MonoBehaviour {
	...
	[SerializeField] protected LayoutContainer listLayoutPrefab;
	[SerializeField] protected FloatSettingsValueView floatValueViewPrefab;
	[SerializeField] protected NamedIntSettingsValueView namedIntValueViewPrefab;
	[SerializeField] protected ConfirmationWindow confirmationPrefab;
	[SerializeField] protected BoolSettingsValueView boolValueViewPrefab;
	...
	protected bool SelectItem(int index) {
		var all = new List<ISettingEntity>(layout.transform.GetComponentsInChildren<ISettingEntity>()).FindAll(e =>
			e.IsActive() && e.Interactable);
		if (all.Count == 0)
			return false;
	...
	public void FillForSelected() {
		...
		var num1 = 84f;
		var num2 = -((Component)currentEntity).GetComponent<RectTransform>().anchoredPosition.y + num1;
	...
	private IEnumerator ScrollCoroutine(bool isUp) {
		...
		while (true) {
			var sellected = !isUp ? currentIndex + 1 : currentIndex - 1;
