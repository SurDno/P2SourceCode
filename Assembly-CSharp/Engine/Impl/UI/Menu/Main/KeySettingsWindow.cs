...
public abstract class KeySettingsWindow : UIWindow {
	...
	public void Fill() {
		...
		foreach (var bind in gameActionService.GetBinds())
			if (!bind.Hide) {
				var gameObject = Instantiate(keyItem);
				gameObject.transform.SetParent(keyView, false);
				var component = gameObject.GetComponent<KeyItem>();
	...
	private string GetText(ActionGroup bind) {
		var text = localizationService.GetText(InputUtility.GetTagName(bind));
		return text.IsNullOrEmpty()
			? ""
			: text + "  [" + InputUtility.GetHotKeyNameByGroup(bind, InputService.Instance.JoystickUsed) + "]";
	}
