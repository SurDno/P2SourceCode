...
public class DialogWindow : UIWindow, IDialogWindow, IWindow {
	...
	[SerializeField] [FormerlySerializedAs("_Unity_Answer_Prefab")]
	private GameObject unityAnswerPrefab;

	[SerializeField] private GameObject unityFinalAnswerPrefab;

	[SerializeField] [FormerlySerializedAs("_Unity_Answers_Content")]
	private RectTransform unityAnswersContent;

	[SerializeField] [FormerlySerializedAs("_Unity_Day")]
	private Text characterName;

	[SerializeField] [FormerlySerializedAs("_Unity_Speech_Content")]
	private RectTransform unitySpeechContent;

	[SerializeField] [FormerlySerializedAs("_Unity_Speech_Prefab")]
	private GameObject unitySpeechPrefab;

	[Inspected] private IEntity entityEffect;
	...
	public void Answer(ISpeakingComponent actor, IList<DialogString> answers) {
		...
		for (var index = 0; index < answers.Count; ++index) {
			...
			var gameObject = Instantiate(unityAnswerPrefab);
	...
	private void Update() {
		if (mode != DialogKind.Answering)
			return;
		var index = 0;
		for (var key = 49; key < 57 && index < unityAnswersContent.childCount; ++index) {
	...
	public void Speech(ISpeakingComponent actor, LocalizedText localizedText) {
		SpeechClear();
		var service = ServiceLocator.GetService<LocalizationService>();
		var gameObject = Instantiate(unitySpeechPrefab);
		var component1 = gameObject.GetComponent<Text>();
		var str = service.GetText(localizedText);
		if (str.Length > 750) {
			str = str.Substring(0, 750) + " (max length: 750)";
			Debug.LogError("Speech text is too long to render (over 750) , tag : " + localizedText.Id +
			               " , localization : " + ServiceLocator.GetService<LocalizationService>().CurrentLanguage +
			               " , actor : " + actor.Owner.GetInfo());
		}

		component1.text = str.Replace("\n", "\n<size=" + component1.fontSize / 2 + ">\n</size>");
		var component2 = actor.GetComponent<InteractableComponent>();
		if (component2 != null)
	...
	private void Clear() {
		...
		if (Target != null) {
			var target = (SpeakingComponent)Target;
	...
	private bool ChangeSelectedItem(GameActionType type, bool down) {
		...
		if (!((type == GameActionType.Submit) & down))
			return false;
		ExecuteEvents.Execute(buttons[currentIndex].gameObject, new PointerEventData(EventSystem.current),
	...
	private void AnswersClear() {
		for (var index = unityAnswersContent.childCount - 1; index >= 0; --index) {
			var gameObject = unityAnswersContent.GetChild(index).gameObject;
	...
	private void SpeechClear() {
		for (var index = unitySpeechContent.childCount - 1; index >= 0; --index) {
			var gameObject = unitySpeechContent.GetChild(index).gameObject;
			gameObject.SetActive(false);
			Destroy(gameObject);
		}
	}
	...
	private void OnExitTalking() {
		CoroutineService.Instance.Route(WaitingTranslate());
	}

	private IEnumerator WaitingTranslate() {
		var ui = ServiceLocator.GetService<UIService>();
	...
	private void PlayInitialPhrase() {
		if (Target == null)
			...
			Target.GetComponent<ILipSyncComponent>()?.Play3D(Target.InitialPhrases.RandomUniform(),
				ExternalSettingsInstance<ExternalCommonSettings>.Instance.IdleReplicsDistanceMin,
				ExternalSettingsInstance<ExternalCommonSettings>.Instance.IdleReplicsDistanceMax, false);
	}
}