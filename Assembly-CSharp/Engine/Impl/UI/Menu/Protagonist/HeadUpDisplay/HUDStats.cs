...
public class HUDStats : MonoBehaviour {
	[SerializeField] [FormerlySerializedAs("FadedLayer")]
	private CanvasGroup fadedLayer;

	[SerializeField] [FormerlySerializedAs("SplashLayer")]
	private CanvasGroup splashLayer;

	[SerializeField] [FormerlySerializedAs("NamePrototype")]
	private Localizer namePrototype;

	[SerializeField] [FormerlySerializedAs("BarPrototype")]
	private StatBar barPrototype;

	[SerializeField] [FormerlySerializedAs("LocalizerPrefix")]
	private string localizerPrefix;

	[SerializeField] [FormerlySerializedAs("LocalizerSuffix")]
	private string localizerSuffix;

	[Space] [SerializeField] [FormerlySerializedAs("Stats")]
	private Stat[] stats;

	[Header("Layout")] [SerializeField] [FormerlySerializedAs("FirstNamePosition")]
	private Vector2 firstNamePosition;

	[SerializeField] [FormerlySerializedAs("FirstBarPosition")]
	private Vector2 firstBarPosition;

	[SerializeField] [FormerlySerializedAs("HeightStep")]
	private float heightStep;

	[Header("Animation")] [SerializeField] [FormerlySerializedAs("AlertedValueChange")]
	private float alertedValueChange = 0.2f;

	[SerializeField] [FormerlySerializedAs("ShowTime")]
	private float showTime = 0.25f;

	[SerializeField] [FormerlySerializedAs("WaitTime")]
	private float waitTime = 1f;

	[SerializeField] [FormerlySerializedAs("ReturnTime")]
	private float returnTime = 1.5f;

	[SerializeField] [FormerlySerializedAs("BarScale")]
	private float barScale = 2f;

	[SerializeField] [FormerlySerializedAs("BarShift")]
	private float barShift = -20f;

	[SerializeField] [FormerlySerializedAs("FadedLayerOpacity")]
	...
	private void Start() {
		...
		for (var index = 0; index < stats.Length; ++index) {
			...
			var gameObject1 = Instantiate(namePrototype.gameObject);
	...
	private void Update() {
		switch (currentAnimation) {
			case Animation.Idle:
				for (var index = 0; index < stats.Length; ++index) {
					if (visibility[index] && Mathf.Abs(bars[index].CurrentValue - stats[index].Value) >=
					    (double)alertedValueChange) {
					...
					var flag = stats[index].IsReversed
						? stats[index].Value > (double)stats[index].VisibilityThreshold
						: stats[index].Value < (double)stats[index].VisibilityThreshold;
			...
			case Animation.Show:
				...
				var visibilityPhase1 = 1f;
				var scalePhase1 = 0.0f;
			...
			case Animation.Alert:
				animationPhase += Time.unscaledDeltaTime;
				var visibilityPhase2 = 1f;
				var scalePhase2 = 0.0f;
				if (animationPhase < (double)showTime)
			...
			case Animation.Hide:
				animationPhase += Time.unscaledDeltaTime;
				var visibilityPhase3 = 0.0f;
				var scalePhase3 = 0.0f;
	...
	public void UpdateLayout(float visibilityPhase, float scalePhase) {
		var firstNamePosition = this.firstNamePosition;
		var firstBarPosition = this.firstBarPosition;
		for (var index = 0; index < visibility.Length; ++index)
			if (visibility[index]) {
				...
				var component = barCanvasGroup[index].GetComponent<RectTransform>();
				if (index == animatedStatIndex) {
					var t = scalePhase * scalePhase;
					var num1 = 1f - scalePhase;
					var num2 = (float)(1.0 - num1 * (double)num1);
					var num3 = Mathf.Lerp(1f, barScale, t);
	...
	public class Stat {
		...
		[Range(0.0f, 1f)] public float VisibilityThreshold = 0.5f;
		[Range(0.0f, 1f)] public float Value = 0.5f;
	}
}