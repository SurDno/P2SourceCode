...
public class RepairingWindow :
	...
	[Space] [SerializeField] private RepairingCostView costView;
	[SerializeField] private ItemSelector itemSelector;
	[SerializeField] private Image nonItemImage;

	[SerializeField] [FormerlySerializedAs("selecteEntityView")]
	private EntityView selectedEntityView;

	[SerializeField] private Button repairButton;
	[SerializeField] private HideableView noTargetMessage;
	[SerializeField] private HideableView noNeedForRepairMessage;
	[SerializeField] private HideableView notEnoughResourcesMessage;
	...
	[SerializeField] private GameObject _repairingHint;
	...
	public IEntity Target {
		...
		set {
			...
			if (target == null) {
				...
				var component = target.GetComponent<TagsComponent>();
				if (component == null)
					return;
	...
	private RepairableComponent TargetRepairable {
		...
		set {
			...
			if (targetRepairable == null) {
				...
				DurabilityParameter = targetRepairable.GetComponent<ParametersComponent>()
					?.GetByName<float>(ParameterNameEnum.Durability);
			}
		}
	}
	...
	private RepairerComponent TargetRepairer {
		...
		set {
			...
			if (targetRepairer != null) {
				foreach (var repairableGroup in targetRepairer.RepairableGroups)
	...
	private bool HaveEnoughResources() {
		foreach (var repairableCostItem in targetRepairable.TargetLevel().Сost)
			if (repairableCostItem.Template.Value != null &&
			    StorageUtility.GetItemAmount(Actor.Items, repairableCostItem.Template.Value) < repairableCostItem.Count)
				return false;
		return true;
	}
	...
	public override Type GetWindowType() {
		return typeof(IRepairingWindow);
	}

	protected override bool ItemIsInteresting(IStorableComponent item) {
		return targetRepairer != null && item.GetComponent<RepairableComponent>() != null &&
		       targetRepairer.CanRepairItem(item);
	}
	...
	protected override bool ConsoleController(GameActionType type, bool down) {
		...
		if ((type == GameActionType.LStickLeft) & down) {
			ExecuteEvents.Execute(SelectorButtons[0].gameObject, new PointerEventData(EventSystem.current),
		...
		if (!((type == GameActionType.LStickRight) & down))
			return false;
		ExecuteEvents.Execute(SelectorButtons[1].gameObject, new PointerEventData(EventSystem.current),
			ExecuteEvents.submitHandler);
		OnInvalidate();
		return true;
	}
			...
			if (flag1) {
				...
				if (RepairingVisible)
					...
					ServiceLocator.GetService<GameActionService>()
						.RemoveListener(GameActionType.Submit, RepairController);
			}
		}
	...
	private void RedrawCost() {
		if (costView == null)
			return;
		var repairableLevel = targetRepairable?.TargetLevel();
	...
	private void Repair() {
		if (!HaveEnoughResources())
			return;
		var repairableLevel = targetRepairable.TargetLevel();
		foreach (var repairableCostItem in repairableLevel.Сost) {
			var removingItem = repairableCostItem.Template.Value;
