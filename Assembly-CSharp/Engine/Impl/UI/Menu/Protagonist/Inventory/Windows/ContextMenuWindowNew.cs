...
public class ContextMenuWindowNew : UIControl {
	...
	[SerializeField] private Button buttonPourOut;
	...
	private void Build() {
		...
		if (this.target == null) {
			...
			if (flag) {
				var placeholder = this.target is StorableComponent target ? target.Placeholder : null;
				if (placeholder != null)
					sprite = useBigPicture
	...
	public static ContextMenuWindowNew Instantiate(GameObject prefab) {
		...
		var component = gameObject.GetComponent<ContextMenuWindowNew>();
