...
public class InfoWindowNew : UIControl {
	...
	private float price;
	[SerializeField] private Image unityImage;
	[SerializeField] private bool useBigPicture;
	[SerializeField] private Text unityInformation;
	[SerializeField] private Text unityName;
	[SerializeField] private GameObject priceItem;
	[SerializeField] private Text priceText;
	[SerializeField] private Image priceCoin;
	[SerializeField] private Sprite coinSprite;
	[SerializeField] private Sprite handSprite;
	[SerializeField] private DurabilityView durabilityView;
	[SerializeField] private GameObject keyCodeSeparator;
	[SerializeField] private GameObject imageSeparator;
	[SerializeField] private GameObject keyCodeLinePrefab;
	[SerializeField] private GameObject keyCodePrefab;
		...
		set {
			...
			if (this.target == null) {
				...
				if (!(this.target is StorableComponent target)) {
					...
					unityImage.sprite = useBigPicture
						? placeholder?.ImageInformation.Value
						: placeholder?.ImageInventorySlotBig.Value;
	...
	public float Price {
		...
		set {
			...
			if (price <= 0.0) {
				...
				priceText.text = ServiceLocator.GetService<LocalizationService>()
					.GetText("{StorableTooltip.NotInteresting}");
			} else {
	...
	public void AddActionTooltip(GameActionType gameAction, string text) {
		...
		if (keyCodeLines != null && keyCodeLines.Count > 0) {
			var keyCodeLine = keyCodeLines[keyCodeLines.Count - 1];
		...
		var gameObject = Instantiate(keyCodePrefab, parent, false);
	...
	public static InfoWindowNew Instantiate(bool withPrice, GameObject prefab) {
		...
		var component = gameObject.GetComponent<InfoWindowNew>();
	...
	public void ShowSimpliedWindow(bool simplified, bool showActionTooltips = true) {
		var flag = unityImage.sprite != null && !simplified;
