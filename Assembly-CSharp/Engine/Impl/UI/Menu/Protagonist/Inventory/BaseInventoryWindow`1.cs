...
public abstract class BaseInventoryWindow<T> :
	...
	[SerializeField] [FormerlySerializedAs("_ActorAnchors")]
	private List<UIControl> actorAnchors = new();

	[SerializeField] [FormerlySerializedAs("_Button_Cancel")]
	private Button buttonCancel;

	[SerializeField] [FormerlySerializedAs("_DragAnchor")]
	private UIControl dragAnchor;

	[SerializeField] [FormerlySerializedAs("_ShowInfo")]
	private bool showInfo;

	[SerializeField] private InventoryCellStyle styleOneCellOneStorable;
	[SerializeField] private InventoryCellStyle styleMultipleCellOneStorable;

	[SerializeField] [FormerlySerializedAs("_SlotAnchor")]
	private List<SlotAnchor> slotAnchor = new();

	[SerializeField] private Sprite nonAvailableIcon;

	[SerializeField] [FormerlySerializedAs("openProgressAudio")]
	private AudioClip overrideUnlockProgressAudio;

	[SerializeField] [FormerlySerializedAs("openCompleteAudio")]
	private AudioClip overrideUnlockCompleteAudio;

	[SerializeField] [Tooltip("Set -1 to use container time")]
	private float overrideUnlockTime = -1f;

	[SerializeField] protected AudioClip dropItemAudio;
	[SerializeField] private AudioMixerGroup mixer;
	[SerializeField] private GameObject inventoryContainerInfinitedPrefab;
	[SerializeField] private GameObject inventoryContainerLimitedPrefab;
	[SerializeField] private GameObject inventoryInfoPrefabNew;
	[SerializeField] private GameObject contextMenuPrefab;
	[SerializeField] private GameObject joystickContextMenuPrefab;
	[SerializeField] private GameObject inventorySplitPrefab;
	[SerializeField] private GameObject inventoryStorablePrefab;
	[SerializeField] private Text moneyText;
	[SerializeField] private ContainerResizableWindow actorContainerWindow;
	[SerializeField] private IEntitySerializable splitContainerTemplate;
	protected IStorageComponent splitContainer;
	[Inspected] protected List<IStorageComponent> actors = new();
	...
	[Inspected] protected Dictionary<IStorableComponent, StorableUI> storables = new();
	...
	[Inspected] protected DragInventoryCell drag = new();
	[Inspected] protected Intersect intersect;
	protected bool splitEnabled;
	[SerializeField] protected GameObject controlPanel;
	[SerializeField] protected GameObject closeButtonObject;
	...
	[SerializeField] protected Image selectionFrame;
	...
	protected List<GameObject> selectableList = new();
	...
	[Inspected] public IStorageComponent Actor { get; set; }
	...
	private void ShowContextMenu(StorableComponent storable) {
		...
		HideMainNavigationPanel();
		windowContextMenu =
			ContextMenuWindowNew.Instantiate(InputService.Instance.JoystickUsed
				? joystickContextMenuPrefab
				: contextMenuPrefab);
			...
			if (InputService.Instance.JoystickUsed)
				...
				switch (storable.Container.GetKind()) {
					case ContainerCellKind.MultipleCellToOneStorable:
						y = (storableUi.Image.rectTransform.localPosition.y +
						     styleMultipleCellOneStorable.Size.y / 4f) * storableUi.Image.rectTransform.lossyScale.y;
						break;
	...
	private void ShowInvestigationWindow(IStorableComponent storable) {
		HideInfoWindow();
		HideContextMenu();
		var investigationWindow = ServiceLocator.GetService<UIService>().Get<IInvestigationWindow>();
	...
	private void RemoveOpenResources(IInventoryComponent container) {
		if (container.GetOpenResources() == null)
			return;
		foreach (var openResource in container.GetOpenResources())
	...
	protected void RemoveItemsAmount(IStorageComponent storage, IEntity removingItem, int amount) {
		var itemId = StorageUtility.GetItemId(removingItem);
		var num = amount;
	...
	protected override void OnJoystick(bool joystick) {
		...
		if (InputService.Instance.JoystickUsed && !selectedStorable.IsSliderItem && (windowContextMenu == null ||
			    (windowContextMenu != null && !windowContextMenu.IsEnabled))) {
			HideInfoWindow();
			ShowInfoWindow(selectedStorable.Internal);
		}
	...
	protected virtual bool WithPrice() {
		return false;
	}
	...
	protected void OpenBegin(IInventoryComponent container) {
		OnContainerOpenBegin(container);
	}
	...
	protected void OpenEnd(IInventoryComponent container, bool complete, bool keepClosedOnUnlock) {
		if (complete) {
			...
			var num = !keepClosedOnUnlock ? 0 : container.OpenState.Value == ContainerOpenStateEnum.Locked ? 1 : 0;
	...
	private void UpdateContainerStates(InventoryContainerUI uiContainer) {
		var inventoryContainer = uiContainer.InventoryContainer;
		if (inventoryContainer == null)
			return;
		var b = CanOpenContainer(inventoryContainer);
		if (uiContainer.ImageForeground != null) {
			uiContainer.ImageForeground.sprite = inventoryContainer.OpenState.Value == ContainerOpenStateEnum.Open
				? null
				: inventoryContainer.GetImageForeground();
		...
		if (uiContainer.ImageIcon != null) {
			if (!inventoryContainer.Available.Value) {
				var imageNotAvailable = inventoryContainer.GetImageNotAvailable();
				uiContainer.ImageIcon.sprite = imageNotAvailable ?? nonAvailableIcon;
			} else if (inventoryContainer.OpenState.Value == ContainerOpenStateEnum.Locked &&
			           inventoryContainer.GetInstrument() != 0) {
		...
		if (uiContainer.ImageDisease != null)
			uiContainer.ImageDisease.gameObject.SetActive(
				inventoryContainer.OpenState.Value != ContainerOpenStateEnum.Open &&
				inventoryContainer.Disease.Value > 0.0);
		...
		if (uiContainer.Button != null) {
			var flag = ((!inventoryContainer.Available.Value ? 0 :
				inventoryContainer.OpenState.Value != ContainerOpenStateEnum.Open ? 1 : 0) & (b ? 1 : 0)) != 0;
			if (flag) {
				var num = inventoryContainer.OpenState.Value != ContainerOpenStateEnum.Locked ? 0 :
					overrideUnlockTime >= 0.0 ? 1 : 0;
		...
		var itemInContainer = GetItemInContainer(inventoryContainer, inventoryContainer.GetStorage());
	...
	private bool ItemsContainsOpenResources(
		...
		if (openResources == null || openResources.Count == 0)
			return true;
		foreach (var openResource in openResources) {
	...
	protected bool CanOpenContainer(IInventoryComponent container) {
		if (container.OpenState.Value != ContainerOpenStateEnum.Locked)
			return true;
		var flag = ItemsContainsOpenResources(Actor.Items, container.GetOpenResources());
		return (container.GetInstrument() == StorableGroup.None || GetInstrument(container.GetInstrument()) != null) &
		       flag;
	}

	protected virtual void DamageInstrument(IInventoryComponent container) {
		var instrument = GetInstrument(container.GetInstrument());
		if (instrument == null)
			return;
	...
	protected void DamageInstrument(IEntity item, IInventoryComponent container) {
		...
		if (component == null)
			return;
		var byName1 = component.GetByName<float>(ParameterNameEnum.Durability);
		if (byName1 == null)
			return;
		var instrumentDamage = container.GetInstrumentDamage();
		var byName2 = component.GetByName<float>(ParameterNameEnum.Quality);
		if (byName2 != null)
	...
	protected virtual IEntity GetInstrument(StorableGroup instrumentGroup, bool brokenEnabled = false) {
		var source = new Dictionary<IEntity, IParameter<float>>();
		foreach (var storableComponent in Actor.Items)
		...
		if (source.Count <= 0)
			return null;
		var key = source.First().Key;
		if (brokenEnabled)
			return key;
		var parameter = source.First().Value;
	...
	protected virtual bool ValidateComputeActor(IStorageComponent actor) {
		return actor == Actor;
	}
	...
	protected virtual bool ValidateContainer(
		...
		if (container.GetStorage() == Actor) {
			if (!container.Enabled.Value)
				return false;
			var limitations = container.GetLimitations();
			if (limitations.Contains(StorableGroup.Money) || limitations.Contains(StorableGroup.Key) ||
			    limitations.Contains(StorableGroup.Weapons_Hands))
				return false;
		}
	...
	protected virtual bool ItemIsInteresting(IStorableComponent item) {
		return true;
	}
	...
	protected void PlaceTo(
		...
		if (!storables.ContainsKey(storable))
			...
			var container1 = GetContainer(storage, container);
			if (container1 == null)
				...
				switch (container.GetKind()) {
					case ContainerCellKind.MultipleCellToOneStorable:
						...
						spriteByStyle = InventoryUtility.GetSpriteByStyle(storable.Placeholder,
							styleMultipleCellOneStorable.imageStyle);
						break;
					case ContainerCellKind.OneCellToOneStorable:
						style = styleOneCellOneStorable;
						spriteByStyle = InventoryUtility.GetSpriteByStyle(storable.Placeholder,
							styleOneCellOneStorable.imageStyle);
						break;
					default:
						throw new Exception();
				}
				...
				var storablePosition = InventoryUtility.CalculateStorablePosition(storable.Cell, style);
	...
	private void RegisterItemInView(IStorableComponent storable, InventoryCellSizeEnum size) {
		if (storable == null || storable.IsDisposed)
			return;
		var storableUi = StorableUI.Instantiate(storable, inventoryStorablePrefab, size);
	...
	protected bool AddItemToView(IStorableComponent storable) {
		var container = GetContainer(storable.Storage, storable.Container);
		if (container == null)
			return false;
	...
	protected virtual bool DragBegin(IStorableComponent storable) {
		if (drag.IsEnabled || storable == null || storable.IsDisposed || !storables.ContainsKey(storable) ||
		    (storable.Container != null && containerViews.ContainsKey(storable.Container) &&
		     !containerViews[storable.Container].ClickEnabled))
			return false;
		...
		if (storable.Container == null)
			return true;
		intersect = StorageUtility.GetIntersect(storable.Storage, storable.Container, (StorableComponent)storable,
			((StorableComponent)storable).Cell);
		foreach (var cell1 in intersect.Cells) {
			var container = GetContainer(drag.Storage, drag.Container);
			if (!(container == null))
				foreach (var cell2 in container.Cells)
	...
	protected virtual void Drag() {
		...
		if (!storables.ContainsKey(drag.Storable))
			Debug.LogError(
				"!storables.ContainsKey(drag.Storable), Такого быть недолжно, воспроизвести, разобраться в чем дело и пофиксить нормально");
		else {
			var storable = storables[drag.Storable];
			...
			if (storable.Transform.parent != dragAnchor.transform.transform) {
				...
				var position = selectedStorable.transform.position;
	...
	private void HoverShadow(Intersect intersect) {
		...
		if (!storables.ContainsKey(drag.Storable))
			...
			if (!storables[drag.Storable].Transform.hasChanged)
				return;
			foreach (var actionCell in drag.ActionCells)
				if (actionCell != null)
					actionCell.State = CellState.Default;
			drag.ActionCells.Clear();
			foreach (var baseCell in drag.BaseCells)
				baseCell.State = CellState.Current;
			if (!intersect.IsIntersected)
				return;
			var container = GetContainer(intersect.Storage, intersect.Container);
			foreach (var cell1 in intersect.Cells) {
					...
					if (cell1.State == CellState.Disabled)
						...
						switch (cell1.State) {
								...
								switch (intersect.Storables.Count) {
									...
									case 1:
										...
										if (storableComponent == null)
											throw new Exception();
										if (drag.Storable.Owner.TemplateId == storableComponent.Owner.TemplateId &&
										    storableComponent.Count < storableComponent.Max) {
											inventoryCellUi.State = CellState.Stack;
											break;
										}

										var num1 =
											container.InventoryContainer.GetKind() ==
											ContainerCellKind.OneCellToOneStorable ? 1 :
											intersect.Cells.Count ==
											((InventoryGridLimited)drag.Storable.Placeholder.Grid).Cells.Count ? 1 : 0;
							...
							case CellState.Stack:
								inventoryCellUi.State =
									intersect.Storables.Count <= 1 ? cell1.State : CellState.Occupied;
								break;
							default:
								var num3 =
									container.InventoryContainer.GetKind() == ContainerCellKind.OneCellToOneStorable
										?
										1
										: intersect.Cells.Count ==
										  ((InventoryGridLimited)drag.Storable.Placeholder.Grid).Cells.Count
											? 1
											: 0;
	...
	protected virtual void DragEnd(Intersect intersect, out bool isSuccess) {
		isSuccess = false;
		if (!drag.IsEnabled || !intersect.IsIntersected || (intersect.Container != null &&
		                                                    containerViews.ContainsKey(intersect.Container) &&
		                                                    !containerViews[intersect.Container].ClickEnabled))
			return;
		if (!storables.ContainsKey(drag.Storable))
			Debug.LogError(
				"!storables.ContainsKey(drag.Storable), Такого быть недолжно, воспроизвести, разобраться в чем дело и пофиксить нормально");
			...
			foreach (var cell in intersect.Cells)
			...
			switch (intersect.Storables.Count) {
				case 0:
					...
					if (!(drag.Storage == null
						    ? intersect.Storage.AddItem(intersect.Storable, intersect.Container)
						    : ((StorageComponent)drag.Storage).MoveItem(drag.Storable, intersect.Storage,
					...
					break;
				case 1:
					var storableComponent1 = intersect.Storables.FirstOrDefault();
					if (drag.Storable.Owner.TemplateId == storableComponent1.Owner.TemplateId &&
					    drag.Storable.Count + storableComponent1.Count <= storableComponent1.Max) {
					...
					if (intersect.Container.GetKind() != ContainerCellKind.OneCellToOneStorable &&
					    ((InventoryGridLimited)drag.Storable.Placeholder.Grid).Cells.Count != intersect.Cells.Count)
						return;
					var storableComponent2 = intersect.Storables.First();
					if (drag.Storable.Owner.TemplateId == storableComponent2.Owner.TemplateId &&
					    drag.Storable.Max > 1) {
						var a = storableComponent2.Max - storableComponent2.Count;
						if (a > 0) {
							var num = Mathf.Min(a, drag.Storable.Count);
					...
					if (!storables.TryGetValue(storableComponent1, out storable)) {
						Debug.LogError("Item not found, storable : " + storableComponent1.Owner.GetInfo() +
						               " , count : " + storables.Count);
						return;
					}

					var lossyScale = storable.Transform.lossyScale;
					var vector2 = inventoryCellStyle.Size * 0.5f;
	...
	protected void SaveCurrentSelectedInstance(StorableUI selectedStorable) {
		if (selectedStorable != null) {
			...
			lastStorablePosition = new Vector3(0.0f,
				currentContainer != null ? currentContainer.transform.position.y + num / 2f : 0.0f);
			lastSelectedTemplateId = new Guid();
		}
	}
	...
	private Pair<int, int> PositionToGrid(
		...
		var size = inventoryCellStyle.Size;
		var vector2 = inventoryCellStyle.Size * 0.5f;
		var offset = inventoryCellStyle.Offset;
		size.Scale(scale);
		vector2.Scale(scale);
		offset.Scale(scale);
		position -= vector2;
	...
	private Cell CellIntersect(Vector2 position, InventoryContainerUI container) {
		foreach (var cell in container.Cells) {
			var lossyScale = cell.Value.Transform.lossyScale;
			var scaledCoordinates = InventoryUtility.GetScaledCoordinates(cell.Value.Transform);
	...
	private InventoryContainerUI ContainerIntersect(Vector2 position) {
		foreach (var container in containers) {
			var scaledCoordinates1 = InventoryUtility.GetScaledCoordinates(container.Key.Transform);
			...
			if (container.Key is ComplexInventoryContainerUI) {
				var key = container.Key as ComplexInventoryContainerUI;
				var scaledCoordinates2 = InventoryUtility.GetScaledCoordinates(key.Mask);
	...
	private InventoryContainerUI ContainerIntersect(Rect rectangle) {
		foreach (var container in containers)
			if (container.Key.InventoryContainer.GetKind() == ContainerCellKind.OneCellToOneStorable) {
				var scaledCoordinates = InventoryUtility.GetScaledCoordinates(container.Key.Transform);
	...
	private Intersect GetIntersect(IStorableComponent storable) {
		...
		var storable1 = storables[storable];
		var lossyScale1 = storable1.Transform.lossyScale;
		var key = ContainerIntersect(new Rect {
		...
		if (key == null || !containers.ContainsKey(key))
			return new Intersect();
		if (!key.InventoryContainer.Available.Value ||
		    key.InventoryContainer.OpenState.Value != ContainerOpenStateEnum.Open)
			return new Intersect();
		InventoryCellStyle inventoryCellStyle;
		...
		var lossyScale2 = key.Transform.lossyScale;
		var vector2_1 = inventoryCellStyle.Size * 0.5f;
		var offset = inventoryCellStyle.Offset;
		vector2_1.Scale(lossyScale2);
		offset.Scale(lossyScale2);
		var scaledCoordinates = InventoryUtility.GetScaledCoordinates(key.Transform);
		var position = scaledCoordinates.position;
		var size = scaledCoordinates.size;
		Vector2 vector2_2 = storable1.Transform.rotation * vector2_1;
		var vector2_3 = (Vector2)storable1.Transform.position + vector2_2;
		var vector2_4 = (Vector2)key.Content.Transform.position + position;
		var grid = PositionToGrid(key.InventoryContainer, vector2_3 - vector2_4, key.Transform.lossyScale);
	...
	private Intersect GetIntersect(Vector2 position) {
		var key = ContainerIntersect(position);
		if (key == null || !containers.ContainsKey(key))
			return new Intersect();
		if (key is ComplexInventoryContainerUI) {
			var container = key as ComplexInventoryContainerUI;
			var cell = CellIntersect(position, container);
			if (cell == null)
				return new Intersect();
			var cellItem = container.GetCellItem(cell);
			return cellItem != null
				? StorageUtility.GetIntersect(cellItem.Storage, cellItem.Container, null, cell)
				: new Intersect();
		}
		...
		if (!key.InventoryContainer.Available.Value ||
			...
			return new Intersect();
		var scaledCoordinates = InventoryUtility.GetScaledCoordinates(key.Transform);
		var position1 = scaledCoordinates.position;
		var size = scaledCoordinates.size;
		var lossyScale = key.Transform.lossyScale;
		var grid = PositionToGrid(key.InventoryContainer,
			position - ((Vector2)key.Content.Transform.position + position1), lossyScale);
		var cell1 = ProxyFactory.Create<Cell>();
	...
	private void UpdateInternal() {
		...
		if (drag.IsEnabled) {
			if (intersect.Container != null) {
				...
				if (intersect.Container.GetKind() == ContainerCellKind.OneCellToOneStorable) {
					...
					storable.Image.sprite = InventoryUtility.GetSpriteByStyle(drag.Storable.Placeholder,
						styleOneCellOneStorable.imageStyle);
				} else {
		...
		if (!showInfo)
			return;
		var storable1 = !InputService.Instance.JoystickUsed || !(selectedStorable != null)
			? intersect.Storables.FirstOrDefault()
			: selectedStorable.Internal;
		if (windowContextMenu == null && !drag.IsEnabled && intersect.IsIntersected && storable1 != null &&
		    (windowSplit == null || !windowSplit.IsEnabled))
	...
	protected virtual void ShowInfoWindow(IStorableComponent storable) {
		...
		if (!flag) {
			...
			var storableByComponent = GetStorableByComponent(storable);
	...
	protected virtual float HintsBottomBorder => actorContainerWindow == null
		? 40f * transform.lossyScale.y
		: GetComponent<RectTransform>().position.y - 450f * actorContainerWindow.transform.lossyScale.y;

	protected virtual void PositionWindow(UIControl window, IStorableComponent storable) {
		StorableUI storableUi;
		if (!storables.TryGetValue(storable, out storableUi))
			return;
		var component = window.GetComponent<RectTransform>();
		var x1 = component.lossyScale.x;
		var num1 = 20f * x1;
		var num2 = 10f * x1;
		float num3;
		float num4;
		if (!InputService.Instance.JoystickUsed)
			switch (storable.Container.GetKind()) {
				case ContainerCellKind.MultipleCellToOneStorable:
					...
					Rect rect1;
					if (storableUi.Image.transform.position.x + (double)num3 + component.rect.width * (double)x1 >
					    Screen.width - (double)num1) {
					...
					var num6 = storableUi.Image.transform.position.y + (double)num4;
					rect1 = component.rect;
					var num7 = rect1.height * (double)x1;
					if (num6 - num7 < num1) {
						rect1 = storableUi.Image.rectTransform.rect;
						var num8 = rect1.height / 2.0;
						rect1 = component.rect;
						double height1 = rect1.height;
						var num9 = num8 + height1;
						rect1 = storableUi.Image.rectTransform.rect;
				...
				case ContainerCellKind.OneCellToOneStorable:
					...
					if (storableUi.Image.transform.position.x + (double)num3 + component.rect.width * (double)x1 >
					    Screen.width - (double)num1)
						num3 =
							((float)(-(double)storableUi.Image.rectTransform.rect.width / 2.0) - component.rect.width) *
							x1 - num2;
					num4 = storableUi.Image.rectTransform.rect.height / 2f * x1;
					var num10 = storableUi.Image.transform.position.y + (double)num4;
					var rect2 = component.rect;
					var num11 = rect2.height * (double)x1;
					if (num10 - num11 < num1) {
			...
			var componentInParent = storableUi.GetComponentInParent<ContainerResizableWindow>();
			if (componentInParent == null)
				return;
			var containerRect = componentInParent.GetContainerRect();
			var hintsBottomBorder = HintsBottomBorder;
			var rect3 = storableUi.Image.rectTransform.rect;
			num4 = rect3.height / 2f * x1;
			var num12 = storableUi.Image.transform.position.y + (double)num4;
			rect3 = component.rect;
			var num13 = rect3.height * (double)x1;
			if (num12 - num13 < hintsBottomBorder) {
				rect3 = component.rect;
				double height = rect3.height;
				rect3 = storableUi.Image.rectTransform.rect;
				var num14 = rect3.height / 2.0;
			...
			if (componentInParent == actorContainerWindow) {
				...
				rect3 = component.rect;
				var num15 = rect3.width * (double)x1;
				...
				var num16 = rect3.width * (double)x1;
				num3 = (float)(x4 + num16) + num1 - storableUi.Image.transform.position.x;
			}
		}

		window.Transform.position = new Vector3(storableUi.Image.transform.position.x + num3,
			storableUi.Image.transform.position.y + num4);
	}
	...
	protected bool Dress(IStorableComponent storable) {
		...
		foreach (var container2 in Actor.Containers.Where(o =>
			         o.GetGroup() == InventoryGroup.Clothes || o.GetGroup() == InventoryGroup.Weapons)) {
		...
		if (container1 == null)
			return false;
		var intersect1 = StorageUtility.GetIntersect(Actor, container1, (StorableComponent)storable, null);
		if (intersect1.IsAllowed) {
			if (((StorageComponent)storable.Storage).MoveItem(storable, intersect1.Storage, intersect1.Container,
				    intersect1.Cell.To()))
				PlaceTo(intersect1.Storage, intersect1.Container, intersect1.Storable, intersect1.Cell.To());
		} else {
			var itemInContainer = GetItemInContainer(container1, Actor);
			if (itemInContainer != null) {
				Actor.RemoveItem(itemInContainer);
				var intersect2 = StorageUtility.GetIntersect(Actor, container1, (StorableComponent)storable, null);
				if (intersect2.IsAllowed && ((StorageComponent)storable.Storage).MoveItem(storable, intersect2.Storage,
					    intersect2.Container, intersect2.Cell.To()))
	...
	protected virtual void SplitDisableHandler(BaseGraphic base2) {
		...
		var splitGraphic = (SplitGraphic)base2;
		if (splitGraphic.IsCanceled || splitGraphic.Target == null || splitGraphic.Target.IsDisposed) {
			...
			var storableComponent =
				splitGraphic.Actor != splitGraphic.Target ? splitGraphic.Target : splitGraphic.Actor;
	...
	protected void CreateSplitContainer() {
		var template = splitContainerTemplate.Value;
	...
	protected void MoveAllItems(IStorageComponent fromStorage, IStorageComponent toStorage) {
		foreach (var storableComponent in fromStorage.Items.ToList())
	...
	protected void MoveItem(
		...
		var intersect = StorageUtility.GetIntersect(toStorage, container, (StorableComponent)item, null);
		if (item.IsDisposed) {
			if (!invalidate)
				return;
			OnInvalidate();
			...
			if (!intersect.IsAllowed) {
				...
				if (!((StorageComponent)item.Storage).MoveItem(item, intersect.Storage, intersect.Container,
					    intersect.Cell.To()))
					throw new Exception();
				var storableComponent = intersect.Storables.FirstOrDefault();
	...
	protected InventoryContainerUI CreateContainerView(
		...
		key.ImageBackground.gameObject.SetActive(GetItemInContainer(container, container.GetStorage()) == null &&
		                                         key.ImageBackground.sprite != null);
		...
		key.OpenEnd += OpenEnd;
		foreach (var storable in storage.Items)
	...
	protected IStorableComponent GetItemInContainer(
		IInventoryComponent container,
		IStorageComponent storage) {
		foreach (var itemInContainer in storage.Items)
	...
	private void ResizeActorContainersWindow() {
		var containers = new List<InventoryContainerUI>();
		foreach (var container in this.containers)
			if (container.Value == Actor && container.Key.InventoryContainer.Enabled.Value &&
			    container.Key.InventoryContainer.GetGroup() == InventoryGroup.Backpack)
	...
	protected bool IsDebugMode() {
		return InstanceByRequest<EngineApplication>.Instance.IsDebug;
	}

	public virtual IEntity GetUseTarget() {
		return Actor?.Owner;
	}

	private void CreateItems() {
		for (var index1 = 0; index1 < actors.Count; ++index1) {
			var actor = actors[index1];
			if (actor != null && !actor.IsDisposed) {
				...
				foreach (var container in actor.Containers)
					if (ValidateContainer(container, actor)) {
						...
						var flag = false;
						var slotAnchor = new SlotAnchor();
						if (container.GetSlotKind() != SlotKind.None && ValidateComputeActor(actor)) {
							for (var index2 = 0; index2 < this.slotAnchor.Count; ++index2)
								if (container.GetLimitations().Contains(this.slotAnchor[index2].Group) &&
						...
						InventoryContainerUI key;
						if (container.GetGrid() is IInventoryGridLimited)
							key = LimitedInventoryContainerUI.Instantiate(container, style,
								inventoryContainerLimitedPrefab);
						else {
							if (!(container.GetGrid() is IInventoryGridInfinited))
								throw new Exception("Grid Type not found : " + (container.GetGrid() != null
									? container.GetGrid().GetType().ToString()
									: "null"));
							key = InfinitedInventoryContainerUI.Instantiate(container, style,
								inventoryContainerInfinitedPrefab);
						}
				...
				foreach (var storable in actor.Items)
					AddItemToView(storable);
			}
		}
	}
	...
	private void ClearItems() {
		...
		foreach (var keyValuePair in storables.ToList())
	...
	protected virtual void AddActionsToInfoWindow(InfoWindowNew window, IStorableComponent storable) {
		var joystickUsed = InputService.Instance.JoystickUsed;
		window.AddActionTooltip(joystickUsed ? GameActionType.Context : GameActionType.Submit,
			"{StorableTooltip.Drag}");
		window.AddActionTooltip(joystickUsed ? GameActionType.Submit : GameActionType.Context,
			"{StorableTooltip.ContextMenu}");
		if (!StorableComponentUtility.IsSplittable(storable))
			return;
	...
	public virtual bool HaveToFindSelected() {
		return true;
	}
	...
	protected void SetSelectedFrame() {
		if (selectionFrame == null)
			return;
		selectionFrame.gameObject.SetActive(InputService.Instance.JoystickUsed && _selectedStorable != null &&
		                                    !drag.IsEnabled);
		if (!(_selectedStorable != null))
			return;
		var style = _selectedStorable.Style;
		var innerSize =
			InventoryUtility.CalculateInnerSize(((StorableComponent)_selectedStorable.Internal).Placeholder.Grid,
				style);
		selectionFrame.rectTransform.SetSizeWithCurrentAnchors(RectTransform.Axis.Horizontal,
			innerSize.x + style.BackgroundImageOffset.x * 2f);
		selectionFrame.rectTransform.SetSizeWithCurrentAnchors(RectTransform.Axis.Vertical,
			innerSize.y + style.BackgroundImageOffset.x * 2f);
	...
	protected virtual void FillSelectableList(List<GameObject> list, bool block, bool addSelected = false) {
		...
		foreach (var storable in storables) {
			var container = storable.Key.Container;
			if (addSelected || !(storable.Value == selectedStorable)) {
				var position = storable.Value.transform.position;
				if (position.x >= 0.0 && position.y >= 0.0 && position.x <= (double)Screen.width &&
				    position.y <= (double)Screen.height &&
				    (!block || container == null || !(selectedStorable != null) ||
				     container != selectedStorable.Internal.Container) &&
				    AdditionalConditionOfSelectableList(storable.Value))
					list.Add(storable.Value.gameObject);
			}
		}
	}

	protected virtual bool AdditionalConditionOfSelectableList(StorableUI storable) {
		return true;
	}

	protected virtual Vector2 CurentNavigationPosition() {
		var transform = _selectedStorable != null ? _selectedStorable.transform as RectTransform : null;
	...
	protected virtual void OnSelectObject(GameObject selected) {
		var component = selected?.GetComponent<StorableUI>();
		if (!(component != null))
			return;
	...
	protected virtual void OnMoveItem(Navigation navigation) {
		...
		if (selectedStorable == null)
			FindCurrentCellInstance();
		var position = selectedStorable.transform.position;
		var storableComponent = selectedStorable.Internal as StorableComponent;
		var rows = storableComponent.Placeholder.Grid.Rows;
		var columns = storableComponent.Placeholder.Grid.Columns;
		var objects = new List<GameObject>();
		foreach (var key1 in containers.Keys) {
			var inventory = key1;
			if (inventory.InventoryContainer.Enabled.Value || inventory.InventoryContainer.GetStorage() != Actor) {
				var source = inventory.Cells.Where(c => {
					var key2 = c.Key;
					if (selectedStorable.transform.position == c.Value.transform.position ||
					    c.Value.transform.position.x < 0.0 || c.Value.transform.position.y < 0.0 ||
					    c.Value.transform.position.x > (double)Screen.width ||
					    c.Value.transform.position.y > (double)Screen.height)
						return false;
					if (inventory.InventoryContainer.GetGroup() == InventoryGroup.Clothes ||
					    inventory.InventoryContainer.GetGroup() == InventoryGroup.Weapons)
						return Vector2.Dot(
							       (c.Value.transform.position - selectedStorable.transform.position).normalized,
							       direction) >=
						       0.85000002384185791;
					return key2.Column + columns <= inventory.InventoryContainer.GetGrid().Columns &&
					       key2.Row + rows <= inventory.InventoryContainer.GetGrid().Rows &&
					       (inventory.InventoryContainer.GetGroup() == InventoryGroup.Backpack ||
					        inventory.InventoryContainer.GetGroup() == InventoryGroup.Loot) &&
					       Vector2.Dot((c.Value.transform.position - selectedStorable.transform.position).normalized,
						       direction) >= 0.85000002384185791;
				});
				if (source != null)
		...
		var gameObject = UISelectableHelper.Select(objects, selectedStorable.transform.position, direction);
		if (gameObject != null)
	...
	protected virtual void OnNavigate(Navigation navigation) {
		...
		if (selectedStorable == null && selectedContainer == null)
			...
			var origin = CurentNavigationPosition();
			switch (navigation) {
	...
	protected void FindCurrentCellInstance() {
		var list1 = storables.Values.Where(storable =>
			(storable.Internal.Owner.TemplateId == lastSelectedTemplateId || lastSelectedTemplateId == new Guid()) &&
			Vector3.Distance(storable.transform.position, lastStorablePosition) <= 500.0 &&
			ItemIsInteresting(storable.Internal)).ToList();
		StorableUI storableUi = null;
		if (list1 == null)
			return;
		var list2 = list1.Where(storable => storable.IsEnabled).OrderBy(storable =>
			Vector3.Distance(lastStorablePosition, storable.gameObject.transform.position)).ToList();
		if (list2.Count > 0)
			storableUi = list2.First();
		if (storableUi == null)
			return;
		if (_selectedStorable != null) {
			if (shouldDeselectStorable ||
			    (_selectedStorable is StorableUITrade &&
			     ((StorableUITrade)_selectedStorable).GetSelectedCount() == 0) || InputService.Instance.JoystickUsed) {
				_selectedStorable.SetSelected(false);
	...
	private void OnNavigateContainers(Navigation navigation) {
		...
		if (currentInventory == null)
			currentInventory =
				new List<ContainerResizableWindow>(GetComponentsInChildren<ContainerResizableWindow>())[0];
		var source =
			new List<LimitedInventoryContainerUI>(
		...
		var list = source.Where(container =>
			Vector3.Distance(container.transform.position, currentContainer.transform.position) <= 800.0).ToList();
		switch (navigation) {
			case Navigation.ContainerUp:
				list = list.Where(container =>
					container.transform.position.y > (double)currentContainer.transform.position.y).ToList();
				break;
			case Navigation.ContainerDown:
				list = list.Where(container =>
					container.transform.position.y < (double)currentContainer.transform.position.y).ToList();
			...
			case Navigation.ContainerRight:
				list = list.Where(container =>
					container.transform.position.x > (double)currentContainer.transform.position.x).ToList();
				break;
			case Navigation.ContainerLeft:
				list = list.Where(container =>
					container.transform.position.x < (double)currentContainer.transform.position.x).ToList();
		...
		if (list.Count <= 0)
			return;
		currentContainer = list.OrderBy(container =>
			Vector3.Distance(currentContainer.transform.position, container.gameObject.transform.position)).First();
	...
	protected virtual void SelectFirstStorableInContainer(List<StorableUI> storables) {
		...
		if (lastStorablePosition == Vector3.zero)
			lastStorablePosition = new Vector3(0.0f,
				actorContainerWindow.transform.position.y +
				actorContainerWindow.GetComponent<RectTransform>().sizeDelta.y);
	...
	protected bool SetStorableByComponent(IStorableComponent component) {
		var storableByComponent = GetStorableByComponent(component);
