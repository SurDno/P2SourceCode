...
public class CraftBreweryWindow : SelectableInventoryWindow, ICraftBreweryWindow, IWindow {
	[SerializeField] private CraftBrewingSlotAnchor[] brewingSlotAnchors;
	[SerializeField] private CraftBrewingSlot brewingSlotPrefab;
	[SerializeField] private ItemView resultPreview;
	[SerializeField] private AudioClip craftAudio;
	[SerializeField] private GameObject slotNavigation;
	private bool isCraftAvailable;
	private List<CraftBrewingSlotAnchor> activeBrewingSlots = new();
	...
	protected override void OnEnable() {
		...
		foreach (var ingredientSelector in ingredientSelectors) {
			var storableByComponent = GetStorableByComponent(ingredientSelector.Item);
		...
		foreach (var brewingSlotAnchor in brewingSlotAnchors) {
	...
	protected override void OnDisable() {
		...
		ServiceLocator.GetService<GameActionService>()
			.RemoveListener(GameActionType.CraftBrewery, WithoutJoystickCancelListener);
	...
	protected override void OnJoystick(bool joystick) {
		...
		if (!joystick) {
			ServiceLocator.GetService<GameActionService>()
				.AddListener(GameActionType.CraftBrewery, WithoutJoystickCancelListener);
			foreach (var ingredientSelector in ingredientSelectors)
	...
	private bool NavigateOnSlots(GameActionType type, bool down) {
		...
		if (currentBrewingSlotIndex != -1 && currentBrewingSlotIndex < activeBrewingSlots.Count &&
		    activeBrewingSlots[currentBrewingSlotIndex] != null)
			...
			switch (value) {
				...
				case Modes.Craft:
					if (InputService.Instance.JoystickUsed && isCraftAvailable) {
						ServiceLocator.GetService<GameActionService>()
							.AddListener(GameActionType.RStickUp, NavigateOnSlots);
						ServiceLocator.GetService<GameActionService>()
							.AddListener(GameActionType.RStickDown, NavigateOnSlots);
						if (currentBrewingSlotIndex == -1)
							currentBrewingSlotIndex = activeBrewingSlots.Count - 1;
						activeBrewingSlots[currentBrewingSlotIndex].Slot.SetSelected(true);
					}
	...
	private void SlotCraftStarted(CraftBrewingSlot slot) {
		foreach (var activeBrewingSlot in activeBrewingSlots)
	...
	private void SlotItemTaken(CraftBrewingSlot slot) {
		...
		if (activeBrewingSlots.Count == 0) {
			...
			CoroutineService.Instance.WaitFrame(1, (Action)(() => {
				if (currentBrewingSlotIndex >= activeBrewingSlots.Count ||
				    activeBrewingSlots[currentBrewingSlotIndex].Slot.IsSelected)
					return;
				activeBrewingSlots[currentBrewingSlotIndex].Slot.SetSelected(true);
			}));
	...
	private void Craft(IInventoryComponent targetContainer) {
		...
		CraftRecipe recipe;
		var craftResult = CraftHelper.GetCraftResult(ingredients, out recipe);
		if (craftResult != null) {
	...
	private void PredictCraftRecipe() {
		...
		if (craftResult != null) {
			...
			isCraftAvailable = false;
			for (var index = 0; index < activeBrewingSlots.Count; ++index)
			...
			if (CurrentActiveCrafts == 0) {
				...
				ServiceLocator.GetService<GameActionService>().RemoveListener(GameActionType.RStickUp, NavigateOnSlots);
				ServiceLocator.GetService<GameActionService>()
					.RemoveListener(GameActionType.RStickDown, NavigateOnSlots);
				slotNavigation.SetActive(false);
				if (isSlotNavigationEnabled)
					isSlotNavigationEnabled = false;
			}
	...
	private void EnableCraftButtons(float time) {
		var text1 = ServiceLocator.GetService<LocalizationService>().GetText("{UI.Craft.PrepareTime}");
		var timeSpan = TimeSpan.FromMinutes(time);
		var text2 = text1.Replace("<hours>", timeSpan.Hours.ToString("D2"))
	...
	private void SetItemTimer(IStorableComponent item, float timeInMinutes) {
		...
		var craftTimeParameter = CraftHelper.GetCraftTimeParameter(item);
	...
	public override Type GetWindowType() {
		return typeof(ICraftBreweryWindow);
	}
