...
public class AutopsyWindow : BaseInventoryWindow<AutopsyWindow>, IAutopsyWindow, IWindow {
	[SerializeField] private List<UIControl> items = new();
	[SerializeField] private ContainerTarget[] containerTargets = new ContainerTarget[0];
	[SerializeField] private IEntitySerializable bloodEntityContainerTemplate;
	[SerializeField] private IEntitySerializable bottleEntityContainerTemplate;
	[SerializeField] private ItemView bottleItemView;
	[SerializeField] private HoldableButton2 buttonDrain;
	[SerializeField] private GameObject noBloodMessage;
	[SerializeField] private GameObject noBottleMessage;
	[SerializeField] private ItemView toolItemView;
	[SerializeField] private ItemSelector toolSelector;
	[SerializeField] private GameObject tooltipArea;
	[SerializeField] private GameObject instrumentDamageItem;
	[SerializeField] private Text instrumentDamageDescriptionText;
	[SerializeField] private Text instrumentDamageValueText;
	[SerializeField] private GameObject organDamageItem;
	[SerializeField] private Text organDamageDescriptionText;
	[SerializeField] private Text organDamageValueText;
	[SerializeField] private Text instrumentTitleText;
	[SerializeField] private Text instrumentErrorText;
	[SerializeField] private AudioClip bloodOpenProgressAudio;
	[SerializeField] private AudioClip bloodOpenCompleteAudio;
	[SerializeField] private EventView organDamagedNotification;
	private IStorageComponent itemStorage;
	private IInventoryComponent selectedContainer;
	[SerializeField] private GameObject drainConsoleButton;
	...
	[SerializeField] private List<BodyPartSelectable> organs;
	...
	protected override bool ValidateContainer(
		...
		return base.ValidateContainer(container, storage) &&
		       (container.GetGroup() == InventoryGroup.Autopsy || storage == Actor);
	}

	private bool ItemIsOrgan(IStorableComponent storable) {
		return storable.Storage == Target && storable.Container.GetGroup() == InventoryGroup.Autopsy &&
		       !storable.Container.GetLimitations().Contains(StorableGroup.Autopsy_Instrument_Bottle);
	}
	...
	protected override void OnDisable() {
		ServiceLocator.GetService<GameActionService>()
			.RemoveListener(GameActionType.Autopsy, WithoutJoystickCancelListener);
	...
	private bool SwapScalpels(GameActionType type, bool down) {
		...
		if (type == GameActionType.RStickLeft) {
			ExecuteEvents.Execute(toolSelector.previousButton.gameObject, new PointerEventData(EventSystem.current),
		...
		if (!((type == GameActionType.RStickRight) & down))
			return false;
		ExecuteEvents.Execute(toolSelector.nextButton.gameObject, new PointerEventData(EventSystem.current),
			ExecuteEvents.submitHandler);
		OnInvalidate();
		HideInfoWindow();
		return true;
	...
	private bool NavigateOrgans(GameActionType type, bool down) {
		if (!InputService.Instance.JoystickUsed ||
		    (toolSelector.Item == null && organs.Count(organ => organ.OrganRemoved) == 0) || !down)
			return false;
		var dirrection = Vector2.right;
		var position = organs[currentOrganIndex].transform.position;
		switch (type) {
		...
		var gameObject = toolSelector.Item == null
			? UISelectableHelper.Select(
				organs.Where(organ => organ.OrganRemoved && !organ.OrganTaken).Select(organ => organ.gameObject),
				position, dirrection, false)
			: UISelectableHelper.Select(organs.Select(organ => organ.gameObject), position, dirrection, false);
		if (gameObject != null) {
	...
	private void OnSelectorItemChange(
		...
		if (arg3 != null) {
			...
			var storableByComponent = GetStorableByComponent(arg3);
	...
	protected override void OnJoystick(bool joystick) {
		...
		if (joystick) {
			ServiceLocator.GetService<GameActionService>()
				.RemoveListener(GameActionType.Autopsy, WithoutJoystickCancelListener);
			instrumentDamageItem.transform.localPosition =
				new Vector3(instrumentDamageItem.transform.localPosition.x, -83f);
			instrumentTitleText.transform.localPosition =
				new Vector3(instrumentTitleText.transform.localPosition.x, -115.5f);
			instrumentErrorText.transform.localPosition =
				new Vector3(instrumentErrorText.transform.localPosition.x, -285f);
			organDamageItem.transform.parent.localPosition =
				new Vector3(organDamageItem.transform.parent.localPosition.x, -325f);
			buttonDrain.transform.localPosition = new Vector3(300f, buttonDrain.transform.localPosition.y);
		} else {
			buttonDrain.GamepadEndHold();
			ServiceLocator.GetService<GameActionService>()
				.AddListener(GameActionType.Autopsy, WithoutJoystickCancelListener);
			instrumentDamageItem.transform.localPosition =
				new Vector3(instrumentDamageItem.transform.localPosition.x, -485.5f);
			instrumentTitleText.transform.localPosition =
				new Vector3(instrumentTitleText.transform.localPosition.x, -453f);
			instrumentErrorText.transform.localPosition =
				new Vector3(instrumentErrorText.transform.localPosition.x, -453f);
			organDamageItem.transform.parent.localPosition =
				new Vector3(organDamageItem.transform.parent.localPosition.x, -510f);
	...
	private void CreateContainers() {
		...
		foreach (var containerTarget in containerTargets)
			if (!(containerTarget.View == null))
				containerTarget.View.Container =
					(InventoryComponent)StorageUtility.GetContainerByTemplate(itemStorage,
						containerTarget.Template.Value);
	...
	private void CheckOrganDestroy(IInventoryComponent container) {
		if (!container.GetStorage().Items.ToList().Exists(x => x.Container == container))
			return;
		var storableComponent = container.GetStorage().Items.First(x => x.Container == container);
		var linesVision = GetLinesVision();
	...
	private float GetLinesVision() {
		var linesVision = 0.0f;
		var component1 = toolSelector.Item;
		var component2 = component1 != null ? component1.GetComponent<ParametersComponent>() : null;
		if (component2 != null) {
			var byName = component2.GetByName<float>(ParameterNameEnum.LinesVision);
			if (byName != null)
	...
	protected override void ShowClosedContainerInfo(IInventoryComponent container) {
		if (toolSelector.Item != null) {
			...
			instrumentDamageDescriptionText.text = ServiceLocator.GetService<LocalizationService>()
				.GetText("{UI.Menu.Protagonist.Autopsy.InstrumentDamage}");
			instrumentDamageValueText.text = GetInstrumentDamageString(container.GetInstrumentDamage());
			organDamageDescriptionText.text = ServiceLocator.GetService<LocalizationService>()
				.GetText("{UI.Menu.Protagonist.Autopsy.OrganDamageChance}");
			var linesVision = GetLinesVision();
	...
	public override Type GetWindowType() {
		return typeof(IAutopsyWindow);
	}

	private void DrainBegin() {
		StartOpenAudio(bloodOpenProgressAudio);
	}

	private IStorableComponent GetActorsBottle() {
		foreach (var actorsBottle in Actor.Items)
	...
	protected override IEntity GetInstrument(StorableGroup instrumentGroup, bool brokenEnabled = false) {
		var component = toolSelector.Item;
		if (component == null || !component.Groups.Contains(instrumentGroup))
			return null;
		if (!brokenEnabled) {
			var byName = component.GetComponent<ParametersComponent>()?.GetByName<float>(ParameterNameEnum.Durability);
	...
	private void Drain(bool success) {
		...
		var itemInContainer =
			GetItemInContainer(StorageUtility.GetContainerByTemplate(itemStorage, bloodEntityContainerTemplate.Value),
				itemStorage);
		if (itemInContainer != null)
			ingredients.Add(itemInContainer);
		var actorsBottle = GetActorsBottle();
		if (actorsBottle != null)
			ingredients.Add(actorsBottle);
		var craftResult = CraftHelper.GetCraftResult(ingredients, out var _);
		if (craftResult != null) {
			foreach (var storable in ingredients)
				if (storable.Count > 1)
					--storable.Count;
				else {
	...
	private void CheckDrainEnabled() {
		var containerByTemplate =
			StorageUtility.GetContainerByTemplate(itemStorage, bloodEntityContainerTemplate.Value);
		var itemInContainer = containerByTemplate != null ? GetItemInContainer(containerByTemplate, itemStorage) : null;
		var actorsBottle = GetActorsBottle();
	...
	protected override void UpdateContainerStates() {
		...
		foreach (var containerTarget in containerTargets)
			if (!(containerTarget.View == null)) {
				var container = containerTarget.View.Container;
				if (container == null)
	...
	private void UpdateViews() {
		var storableComponent = toolSelector.Item;
	...
	private string GetInstrumentDamageString(float damage) {
		var tag = damage < 0.2
			? damage < 0.15 ? "{UI.Menu.Protagonist.Autopsy.LowDamage}" : "{UI.Menu.Protagonist.Autopsy.AverageDamage}"
			: "{UI.Menu.Protagonist.Autopsy.HighDamage}";
		return ServiceLocator.GetService<LocalizationService>().GetText(tag);
	}

	private string GetOrganDamageString(float damage) {
		var tag = damage < 0.5
			? damage < 0.25 ? "{UI.Menu.Protagonist.Autopsy.LowChance}" : "{UI.Menu.Protagonist.Autopsy.AverageChance}"
			: "{UI.Menu.Protagonist.Autopsy.HighChance}";
		return ServiceLocator.GetService<LocalizationService>().GetText(tag);
	}
	...
	private void SelectItem(StorableComponent storable) {
		if (toolSelector.Item == storable || !storable.Groups.Contains(StorableGroup.Autopsy_Instrument_Stomach))
			return;
		var byName = storable.GetComponent<ParametersComponent>()?.GetByName<float>(ParameterNameEnum.Durability);
		if (byName != null && byName.Value <= 0.0)
			return;
	...
	protected override bool ItemIsInteresting(IStorableComponent item) {
		if (!item.Groups.Contains(StorableGroup.Autopsy_Instrument_Stomach))
			return false;
		var byName = item.GetComponent<ParametersComponent>()?.GetByName<float>(ParameterNameEnum.Durability);
