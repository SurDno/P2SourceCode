...
public class LootWindow :
	...
	[SerializeField] private IEntitySerializable deadNpcEntityContainer;
	[SerializeField] private ContainerResizableWindow lootContainerWindow;
	[SerializeField] private IEntitySerializable lockpickTemplate;
	[SerializeField] private LockpickTooltip lockpickTooltip;
	[SerializeField] private GameObject openContainerTooltip;
	[SerializeField] private GameObject unlockContainerTooltip;
	...
	private void CheckLivingNpc(bool subscribe = true) {
		...
		if (byName1 == null || !byName1.Value) {
			var byName2 = component?.GetByName<bool>(ParameterNameEnum.Surrender);
			if (byName2 != null) {
	...
	protected override void OnEnable() {
		...
		ServiceLocator.GetService<GameActionService>()
			.AddListener(GameActionType.BumperSelectionLeft, OnChangeInventory);
		ServiceLocator.GetService<GameActionService>()
			.AddListener(GameActionType.BumperSelectionRight, OnChangeInventory);
	...
	protected override void OnDisable() {
		...
		CheckEmpty();
		ServiceLocator.GetService<GameActionService>()
			.RemoveListener(GameActionType.Loot, WithoutJoystickCancelListener);
		ServiceLocator.GetService<GameActionService>()
		...
		var color = inventories[1].GetComponentInChildren<Image>().color with {
			a = 1f
		};
	...
	private bool MainControl(GameActionType type, bool down) {
		...
		if (_selectedContainer != null) {
			if ((type == GameActionType.Submit) & down) {
				if (_selectedContainer.InventoryContainer.OpenState.Value != ContainerOpenStateEnum.Locked ||
				    StorageUtility.GetItemAmount(Actor.Items, lockpickTemplate.Value) > 0) {
	...
	protected override void OnNavigate(
		...
		if (selectedStorable == null && selectedContainer == null)
			CurrentMode = navigation != Navigation.CellRight && navigation != Navigation.ContainerRight
				? Modes.Loot
				: Modes.Inventory;
	...
	protected override void OnJoystick(bool joystick) {
		...
		if (joystick) {
			...
			SetSelectedContainer();
			ServiceLocator.GetService<GameActionService>()
				.AddListener(GameActionType.Loot, WithoutJoystickCancelListener);
			inventories[0].SetActive(false);
			inventories[1].SetActive(false);
		}
	}
	...
	protected override bool DragBegin(IStorableComponent storable) {
		if (drag.IsEnabled || storable == null || storable.IsDisposed || !storables.ContainsKey(storable) ||
		    selectedContainer != null || (storable.Container != null &&
		                                  containerViews.ContainsKey(storable.Container) &&
	...
	protected override void DragEnd(Intersect intersect, out bool isSuccess) {
		...
		if (currentInventory != null) {
			var componentsInChildren = currentInventory.GetComponentsInChildren<StorableUI>();
	...
	protected override bool ValidateContainer(
		...
		if (!base.ValidateContainer(container, storage))
			return false;
		var flag = container.GetGroup() == InventoryGroup.Loot || storage == Actor;
		if (Target != null && Target.Owner != null && Target.Owner.GetComponent<NpcControllerComponent>() != null &&
		    container.Owner.Template.TemplateId == deadNpcEntityContainer.Value.Id) {
			var byName = Target.Owner.GetComponent<ParametersComponent>()?.GetByName<bool>(ParameterNameEnum.Dead);
			if (byName != null)
	...
	protected override void InteractItem(IStorableComponent storable) {
		...
		if (InputService.Instance.JoystickUsed) {
			...
			var list = selectableList.Where(sel => sel.GetComponent<InventoryContainerUI>() != null).ToList();
			if (list.Count > 0) {
				var num = 0;
	...
	private InventoryContainerUI FindContainerByComponent(IInventoryComponent component) {
		if (component == null)
			return null;
		var list = lootContainerWindow.GetComponentsInChildren<InventoryContainerUI>().ToList();
		if (list.Count == 0)
			return null;
		foreach (var containerByComponent in list)
	...
	protected override void OnContainerOpenEnd(IInventoryComponent container, bool complete) {
		...
		if (_selectedContainer.InventoryContainer.OpenState.Value != ContainerOpenStateEnum.Open)
			...
			var componentInChildren = _selectedContainer.GetComponentInChildren<StorableUI>();
	...
	private void ShowLockpickInfo() {
		...
		var itemAmount = StorageUtility.GetItemAmount(Actor.Items, lockpickTemplate.Value);
		lockpickTooltip.SetItem(lockpickTemplate.Value.GetComponent<StorableComponent>());
		lockpickTooltip.SetCount(itemAmount);
	}

	private void HideLockpickInfo() {
		lockpickTooltip.SetActive(false);
	}
	...
	protected override void AddActionsToInfoWindow(
		...
		if (Target.Items.Contains(storable))
			window.AddActionTooltip(joystickUsed ? GameActionType.Submit : GameActionType.Context,
				"{StorableTooltip.MoveToInventory}");
		else
			window.AddActionTooltip(joystickUsed ? GameActionType.Submit : GameActionType.Context,
	...
	protected override void FillSelectableList(List<GameObject> list, bool block, bool addSelected = false) {
		...
		foreach (var storable in storables)
			if (addSelected || !(storable.Value == selectedStorable)) {
				...
				if (!(storable.Value == selectedStorable) && container.OpenState.Value == ContainerOpenStateEnum.Open &&
	...
	protected override Vector2 CurentNavigationPosition() {
		var transform = _selectedContainer != null ? _selectedContainer.transform as RectTransform : null;
	...
	protected override void OnSelectObject(GameObject selected) {
		var componentInParent = selected?.GetComponentInParent<ContainerResizableWindow>();
		...
		var component = selected?.GetComponent<InventoryContainerUI>();
		if (component != null)
	...
	protected void SetSelectedContainer() {
		...
		if (!(_selectedContainer != null))
			return;
		var imageForeground = _selectedContainer.ImageForeground;
		selectionFrame.rectTransform.sizeDelta = imageForeground.rectTransform.rect.size;
		selectionFrame.rectTransform.position =
			imageForeground.rectTransform.TransformPoint(imageForeground.rectTransform.rect.center);
	}
	...
	protected override void GetFirstStorable() {
		...
		if (currentInventory != null) {
			...
			var component = currentInventory.GetComponent<RectTransform>();
			if (component != null)
