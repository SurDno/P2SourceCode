...
public abstract class SelectableInventoryWindow :
	...
	IWindow {
	[SerializeField] protected ItemSelector[] ingredientSelectors;
	...
	protected List<StorableUI> selectedStorables = new();
	...
	private void OnSelectorItemChange(
		...
		if (arg2 != null) {
			var storableByComponent = GetStorableByComponent(arg2);
	...
	protected override void OnJoystick(bool joystick) {
		...
		foreach (var selectedStorable in selectedStorables)
			selectedStorable?.HoldSelected(joystick);
		OnInvalidate();
	}

	protected virtual bool ItemIsSelected(IStorableComponent storable) {
		return false;
	}
	...
	public override void OnPointerDown(PointerEventData eventData) {
		if (windowContextMenu != null)
			...
			if (intersect.Storables == null)
				Debug.LogError(
					"intersect.Storables == null, Такого быть недолжно, воспроизвести, разобраться в чем дело и пофиксить нормально");
			else {
