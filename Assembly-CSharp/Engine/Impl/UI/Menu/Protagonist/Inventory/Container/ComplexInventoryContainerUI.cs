...
public class ComplexInventoryContainerUI : InventoryContainerUI {
	...
	public static ComplexInventoryContainerUI Instantiate(
		...
		var component1 = gameObject.GetComponent<ComplexInventoryContainerUI>();
		component1.content = component1;
		component1.Mask = mask;
		var num = 0;
		foreach (var storableComponent in items) {
			var inventoryCellUi = InventoryCellUI.Instantiate(new Vector2(0.0f, items.Count - 1 - num), style);
			inventoryCellUi.Transform.SetParent(component1.grid.transform, false);
			inventoryCellUi.Transform.localPosition =
				inventoryCellUi.Transform.localPosition + (Vector3)style.OutlineOffset;
			component1.cells.Add(storableComponent.Cell, inventoryCellUi);
			component1.items.Add(storableComponent.Cell, storableComponent);
			++num;
		}

		var outerSize = InventoryUtility.CalculateOuterSize(new Vector2(1f, items.Count), style);
		component1.Transform.SetSizeWithCurrentAnchors(RectTransform.Axis.Horizontal,
			(float)(outerSize.x + style.OutlineOffset.x * 2.0 + 2.0));
		component1.Transform.SetSizeWithCurrentAnchors(RectTransform.Axis.Vertical,
			(float)(outerSize.y + style.OutlineOffset.y * 2.0 + 2.0));
