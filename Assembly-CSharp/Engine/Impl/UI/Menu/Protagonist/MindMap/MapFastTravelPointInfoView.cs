...
public class MapFastTravelPointInfoView : MonoBehaviour {
	[SerializeField] private FastTravelGraph graph;
	[SerializeField] private IEntitySerializable resourceTemplate;
	[SerializeField] private StringView titleView;
	[SerializeField] private StringView textView;
	[SerializeField] private StringView timeView;
	[SerializeField] private StringView costView;
	[SerializeField] private HideableView enoughResourcesView;
	[SerializeField] private MapRouteView routeView;
	private MapFastTravelPointView targetView;
	private FastTravelComponent origin;
	private List<FastTravelPointEnum> path = new();
	private int time;
	...
	public void Show(FastTravelComponent origin, MapFastTravelPointView targetView) {
		...
		var itemAmount = StorageUtility.GetItemAmount(
			ServiceLocator.GetService<ISimulation>().Player?.GetComponent<IStorageComponent>().Items,
			resourceTemplate.Value);
		var num = origin.FastTravelPrice.Value;
	...
	private void UpdatePosition() {
		...
		transform1.pivot = new Vector2(position.x > vector2.x * 0.699999988079071 ? 1f : 0.0f,
			position.y > vector2.y * 0.30000001192092896 ? 1f : 0.0f);
