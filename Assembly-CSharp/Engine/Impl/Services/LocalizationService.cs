...
public class LocalizationService : IInitialisable {
	[Inspected] private Dictionary<LanguageEnum, Dictionary<ulong, string>> vmLanguages = new();
	[Inspected] private Dictionary<LanguageEnum, Dictionary<string, string>> languages = new();
	[Inspected] private Dictionary<LanguageEnum, Dictionary<string, string>> subTitlesLanguages = new();
	...
	[Inspected] public LanguageEnum DefaultLanguage => LanguageEnum.English;
	...
	private void TryOverrideSettings() {
		var path = PlatformUtility.GetPath("Data/Settings/DefaultLocalization.txt");
		LanguageEnum result;
		if (!File.Exists(path) || !DefaultConverter.TryParseEnum(File.ReadAllText(path), out result) ||
	...
	private void LoadSubTitlesLanguage(LanguageEnum language) {
		var fileName = "SubTitles_" + language;
	...
	private static void LoadVmLanguageImpl(
		...
		foreach (var gameDataInfo in BuildSettingsUtility.GetAllGameData())
			if (BuildSettingsUtility.IsDataExist(gameDataInfo.Name)) {
				var path = PlatformUtility.GetPath(
					"Data/VirtualMachine/{ProjectName}/Localizations/".Replace("{ProjectName}", gameDataInfo.Name) +
					fileName.ToLowerInvariant() + ".txt");
				if (!File.Exists(path))
	...
	private static string GetTextImpl(
		...
		string textImpl;
		if (languages.TryGetValue(language, out dictionary) && dictionary.TryGetValue(tag, out textImpl) &&
		    !textImpl.IsNullOrEmpty())
			return textImpl;
		string str;
		return language != defaultLanguage && languages.TryGetValue(defaultLanguage, out dictionary) &&
		       dictionary.TryGetValue(tag, out str) && !str.IsNullOrEmpty()
			? str
			: tag;
	}
	...
	private static string GetVmTextImpl(
		...
		string vmTextImpl;
		if (languages.TryGetValue(language, out dictionary) && dictionary.TryGetValue(id, out vmTextImpl) &&
		    !vmTextImpl.IsNullOrEmpty())
			return vmTextImpl;
		string str;
		return language != defaultLanguage && languages.TryGetValue(defaultLanguage, out dictionary) &&
		       dictionary.TryGetValue(id, out str) && !str.IsNullOrEmpty()
			? str
			: id.ToString();
	}
