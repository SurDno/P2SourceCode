...
public class VirtualMachineController :
	...
	[Inspected] public IVirtualMachine VirtualMachine => virtualMachine;
		...
		if (virtualMachine == null)
			...
			if (!virtualMachine.IsInitialized)
				Debug.Log(ObjectInfoUtility.GetStream().Append(TypeUtility.GetTypeName(GetType())).Append(" : ")
					.Append(MethodBase.GetCurrentMethod().Name).Append(" , is not inited"));
	...
	private IEnumerator InitializeVirtualMachine() {
		var debug = ExternalSettingsInstance<ExternalOptimizationSettings>.Instance.VirtualMachineDebug;
		yield return virtualMachine.Initialize(debug,
			ExternalSettingsInstance<ExternalOptimizationSettings>.Instance.VMEventsQueuePerFrameTimeMax);
	}

	public void ComputeUpdate() {
		if (!InstanceByRequest<EngineApplication>.Instance.ViewEnabled || virtualMachine == null ||
		    !virtualMachine.IsInitialized || !virtualMachine.IsLoaded)
			return;
	...
	public IEnumerator Load(XmlElement element, string context, IErrorLoadingHandler errorHandler) {
		Debug.Log(ObjectInfoUtility.GetStream().Append("IVirtualMachine").Append(" load, context : ").Append(context)
			.Append("\n").GetStackTrace());
		var vm = VirtualMachine;
		if (vm == null)
			errorHandler.LogError("IVirtualMachine not found");
			...
			var vmNode = element["VirtualMachine"];
			if (vmNode == null)
				errorHandler.LogError("VirtualMachine node not found in file : " + context);
			else {
				var storedDataName = element[nameof(VirtualMachineController)]?["Data"]?.InnerText;
				if (storedDataName.IsNullOrEmpty())
					...
					yield return ComputeData(errorHandler);
					if (!errorHandler.HasErrorLoading) {
						var storedDataVersionText =
							element[nameof(VirtualMachineController)]?["DataVersion"]?.InnerText;
						if (storedDataVersionText.IsNullOrEmpty()) {
							Debug.LogError("DataVersion node not found in file : " + context);
							storedDataVersionText = "0";
						}

						var saveVersion = DefaultConverter.ParseInt(storedDataVersionText);
						if (dataVersion < saveVersion)
							errorHandler.LogError("Error save version, current data version : " + dataVersion +
							                      " , save version : " + saveVersion + " , context : " + context);
						else {
							var originalExperienceSessionText =
								element[nameof(VirtualMachineController)]?["OriginalExperienceSession"]?.InnerText;
							OriginalExperienceSession = originalExperienceSessionText == null ||
							                            DefaultConverter.ParseBool(originalExperienceSessionText);
							DifficultySettings_OnApply();
							var sw = new Stopwatch();
							sw.Restart();
	...
	private IEnumerator ComputeData(IErrorLoadingHandler errorHandler) {
		var requiredProjectName = InstanceByRequest<GameDataService>.Instance.GetCurrentGameData().Name;
	...
	private static void SetFirstScene(string projectName) {
		var component = ServiceLocator.GetService<ITemplateService>().GetTemplate<IEntity>(Ids.PathologicId)
			.GetComponent<StaticModelComponent>();
		var id = (BuildSettingsUtility.GetAllGameData().FirstOrDefault(o => o.Name == projectName) ??
		          throw new Exception("Project not found : " + projectName)).Scene.Id;
		component.Connection = new Typed<IScene>(id);
		var service = ServiceLocator.GetService<HierarchyService>();
		service.CreateMainContainer();
		((IEntityHierarchy)ServiceLocator.GetService<ISimulation>().Hierarchy).HierarchyItem =
			new HierarchyItem(service.MainContainer);
	}
	...
	public IEnumerator LoadData(string projectName, IErrorLoadingHandler errorHandler) {
		...
		var dataFileName =
			PlatformUtility.GetPath("Data/VirtualMachine/{ProjectName}".Replace("{ProjectName}", projectName));
		var versionFileName = dataFileName + "/Version.xml";
		if (File.Exists(versionFileName)) {
			var data = File.ReadAllText(versionFileName);
			Debug.Log(ObjectInfoUtility.GetStream().Append("Data name : ").Append(projectName).Append(" , version : ")
				.Append("\r\n").Append(data));
			try {
				var doc = new XmlDocument();
				doc.LoadXml(data);
				var text = doc["Root"]["ProjectVersion"].InnerText;
				...
				if (formatVersion != 14) {
					errorHandler.LogError("Wrong format data version file : " + versionFileName + " , data version : " +
					                      formatVersion + " , parser version : " + 14);
					yield break;
				}
