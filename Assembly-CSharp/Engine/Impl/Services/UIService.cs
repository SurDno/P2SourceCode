...
public class UIService : IInitialisable, ISavesController {
	[Inspected] private Dictionary<Type, UIWindow> layers = new();
	[Inspected] private List<UIWindow> windows = new();
	[Inspected] private UIControl uiBehaviour;
	private UIRoot uiRoot;

	[Inspected] public bool IsTransition { get; private set; }
	...
	public UIWindow Swap(Type type) {
		Debug.Log(ObjectInfoUtility.GetStream().Append("Try swap window : ").Append(TypeUtility.GetTypeName(type))
			.Append(" , info : ").Append(TypeUtility.GetTypeName(GetType())).Append(":")
	...
	public T Swap<T>() where T : class, IWindow {
		return Swap(typeof(T)) as T;
	}

	public UIWindow Push(Type type) {
		Debug.Log(ObjectInfoUtility.GetStream().Append("Try push window : ").Append(TypeUtility.GetTypeName(type))
			.Append(" , info : ").Append(TypeUtility.GetTypeName(GetType())).Append(":")
			.Append(MethodBase.GetCurrentMethod().Name).Append("\n").GetStackTrace());
		if (!IsInitialize)
			throw new Exception(TypeUtility.GetTypeName(GetType()) + "." + MethodBase.GetCurrentMethod().Name);
	...
	public T Push<T>() where T : class, IWindow {
		return Push(typeof(T)) as T;
	}

	public void Pop() {
		Debug.Log(ObjectInfoUtility.GetStream().Append("Try pop window , info : ")
	...
	public void Initialise() {
		...
		var childCount = uiBehaviour.transform.childCount;
		for (var index = 0; index < childCount; ++index) {
			var component = uiBehaviour.transform.GetChild(index).GetComponent<UIWindow>();
			if (component != null)
				component.Initialize();
		}

		var component1 = uiRoot.GetComponent<UnityEngine.Camera>();
		var componentsInChildren = uiBehaviour.GetComponentsInChildren<Canvas>(true);
	...
	private IEnumerator TranslateRoute(UIWindow closeWindow, UIWindow openWindow) {
		IsTransition = true;
		var transition = UITransitionFactory.GetTransition(closeWindow, openWindow);
		var iterator = transition(closeWindow, openWindow);
		while (iterator.MoveNext())
			yield return iterator.Current;
		IsTransition = false;
		Debug.Log(ObjectInfoUtility.GetStream().Append("End translate window , info : ")
			.Append(TypeUtility.GetTypeName(GetType())).Append(":").Append(MethodBase.GetCurrentMethod().Name)
			.Append("\n").GetStackTrace());
	}
