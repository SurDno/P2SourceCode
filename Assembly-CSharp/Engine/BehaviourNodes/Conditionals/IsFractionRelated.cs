...
public class IsFractionRelated : Conditional, IStub, ISerializeDataWrite, ISerializeDataRead {
	...
	public override TaskStatus OnUpdate() {
		var entity1 = EntityUtility.GetEntity(gameObject);
		if (entity1 == null) {
			Debug.LogWarning(
		...
		if (Target.Value == null)
			return TaskStatus.Failure;
		var component = entity1?.GetComponent<ParametersComponent>();
		if (component == null)
			Debug.LogWarningFormat("{0}: doesn't have parameters", gameObject.name);
		var fraction = component?.GetByName<FractionEnum>(ParameterNameEnum.Fraction);
		List<FractionEnum> fractionEnumList = null;
		if (fraction != null)
			fractionEnumList = ScriptableObjectInstance<FractionsSettingsData>.Instance.Fractions
				.Find(x => x.Name == fraction.Value)?.Relations?.Find(x => x.Relation == Relation)?.Fractions;
		var entity2 = EntityUtility.GetEntity(Target.Value.gameObject);
		if (entity2 == null) {
			Debug.LogWarningFormat("{0}: entity not found", Target.Value.gameObject.name);
			return TaskStatus.Failure;
		}

		var flag = fractionEnumList != null &&
		           fractionEnumList.Contains(FractionsHelper.GetTargetFraction(entity2, entity1));
		return IsRelated.Value ? flag ? TaskStatus.Success : TaskStatus.Failure :
			flag ? TaskStatus.Failure : TaskStatus.Success;
	}
