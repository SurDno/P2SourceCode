...
public class IsOnLineOfSight : Conditional, IStub, ISerializeDataWrite, ISerializeDataRead {
	...
	public LayerMask physicsLayer =
		LayerMaskUtility.GetMask(0) | LayerMaskUtility.GetMask(9) | LayerMaskUtility.GetMask(14);
	...
	public override TaskStatus OnUpdate() {
		if (pivot == null || Target.Value == null ||
		    (Target.Value.transform.position - gameObject.transform.position).magnitude > (double)RaycastDistance)
			return TaskStatus.Failure;
		var origin = !(pivot.Head != null) ? gameObject.transform.position : pivot.Head.transform.position;
		...
		var vector3 = !(component.Head != null) ? gameObject.transform.position : component.Head.transform.position;
		return Physics.Raycast(origin, vector3 - origin, RaycastDistance, physicsLayer, QueryTriggerInteraction.Ignore)
			? TaskStatus.Failure
			: TaskStatus.Success;
	}
