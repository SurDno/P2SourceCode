...
public class CanThrowBomb : Conditional, IStub, ISerializeDataWrite, ISerializeDataRead {
	...
	public override TaskStatus OnUpdate() {
		return npcEnemy == null || Target.Value == null ? TaskStatus.Failure :
			CanThrow() ? TaskStatus.Success : TaskStatus.Failure;
	}

	private bool CanThrow() {
		var v = 15f;
		var h = 1.5f;
		var vector3 = Target.Value.position - npcEnemy.transform.position;
		float angle1;
		float angle2;
		if (BomberHelper.CalcThrowAngles(v, vector3.magnitude, h, out angle1, out angle2)) {
			var startPosition = npcEnemy.transform.position + Vector3.up * h;
