...
public class EngineUpdateBehaviour : MonoBehaviour {
	...
	private void Awake() {
		computeConsole = !Application.isEditor && ScriptableObjectInstance<BuildData>.Instance.Development &&
		                 ScriptableObjectInstance<BuildData>.Instance.Release;
		if (!computeConsole)
			return;
	...
	private void Start() {
		InstanceByRequest<UpdateService>.Instance.Start();
	}

	private void Update() {
		InstanceByRequest<UpdateService>.Instance.Update();
	}
