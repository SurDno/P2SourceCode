...
public class EngineBehavior : MonoBehaviour, IFlamable {
	...
	[SerializeField] private MovementControllerEnum movementControllerKind = MovementControllerEnum.Rootmotion45;

	[Inspected] public GaitType Gait { get; set; }
	...
	protected virtual void Awake() {
		...
		if (movementControllerKind == MovementControllerEnum.PlagueCloud)
			...
			Debug.LogError(string.Format("Deprecated movement controller type {0}", movementControllerKind),
				gameObject);
			movementController = new Rootmotion45MovementController();
		} else {
