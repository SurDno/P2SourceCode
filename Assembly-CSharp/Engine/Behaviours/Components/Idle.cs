...
public class Idle : MonoBehaviour {
	...
	[SerializeField] [FormerlySerializedAs("_Clips")]
	private Clip[] clips;
	...
	private void SetClip(MecanimKinds.MovableIdleStateKind state) {
		...
		var num2 = 0.0f;
		for (var index = 0; index < this.clips.Length; ++index) {
		...
		var clips = animatorController.clips;
		...
		var animatorStateInfoArray = new AnimatorStateInfo[animator.layerCount];
		...
		for (var layer = 0; layer < animator.layerCount; ++layer)
			animator.Play(animatorStateInfoArray[layer].fullPathHash, layer,
				animatorStateInfoArray[layer].normalizedTime);
		animator.Update(0.0f);
	}
