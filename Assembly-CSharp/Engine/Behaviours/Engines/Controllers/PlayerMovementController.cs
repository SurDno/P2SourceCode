...
public class PlayerMovementController : IMovementController {
	...
	public void Initialize(GameObject gameObject) {
		...
		if (weaponService == null)
			Debug.LogWarningFormat("{0} doesn' contain {1} unity component.", gameObject.name,
				typeof(PlayerWeaponServiceNew).Name);
		...
		if (characterController == null)
			Debug.LogWarningFormat("{0} doesn' contain {1} unity component.", gameObject.name,
				typeof(CharacterController).Name);
	...
	public void OnAnimatorMove() {
		gameObject.transform.rotation *= Quaternion.AngleAxis(
			57.29578f * animator.angularVelocity.y * (animator.updateMode == AnimatorUpdateMode.AnimatePhysics
				? Time.fixedDeltaTime
