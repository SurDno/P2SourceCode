...
public class PlagueCloudMovementController : IMovementController {
	...
	public bool Move(Vector3 direction, float remainingDistance, EngineBehavior.GaitType gait) {
		var num = gait == EngineBehavior.GaitType.Walk ? agent.speed : 2f * agent.speed;
		var vector3 = Vector3.ProjectOnPlane(gameObject.transform.InverseTransformDirection(direction), Vector3.up);
		deltaRotation *= Quaternion.Euler(0.0f,
			Mathf.Clamp(Mathf.Atan2(vector3.x, vector3.z) * 57.29578f / Time.deltaTime, -agent.angularSpeed,
