...
public class PlayerFlashlightWeaponController : IWeaponController {
	...
	private void RefreshStorageAmmo() {
		...
		storageAmmo.AddRange(storage.Items.ToList()
			.FindAll(x => x.Groups.Contains(StorableGroup.Fuel_Lamp) && x.Count > 0).Cast<StorableComponent>());
	}
	...
	public void FixedUpdate(IEntity target) {
		...
		var isOn = WorkEndTime.Value > timeService.GameTime;
		if (isOn || !IsOn)
			return;
		SwitchLight(isOn);
	}
	...
	private void FireLamp() {
		if (StorageAmmoCount() <= 0)
			return;
		var timeSpan = WorkTime.MinValue;
		var totalSeconds1 = timeSpan.TotalSeconds;
		timeSpan = WorkTime.MaxValue;
		var totalSeconds2 = timeSpan.TotalSeconds;
		WorkEndTime.Value = timeService.GameTime +
		                    TimeSpan.FromSeconds(Random.Range((float)totalSeconds1, (float)totalSeconds2));
		SwitchLight(true);
		RemoveAmmo();
		var weaponShootEvent = WeaponShootEvent;
		if (weaponShootEvent != null)
	...
	private void SwitchLight(bool isOn) {
		IsOn = isOn;
		var componentInChildren = pivot.FlashlightEffect.GetComponentInChildren<ParticleSystem>();
