...
public class PlayerRevolverWeaponController : IWeaponController {
	...
	private void EmptyBarrel() {
		var num = bullets.Value;
		for (var index = 0; index < maxAmmo; ++index) {
			var flag = index < num;
	...
	private void RefreshStorageAmmo() {
		...
		storageAmmo.AddRange(storage.Items.ToList()
			.FindAll(x => x.Groups.Contains(StorableGroup.Ammo_Revolver) && x.Count > 0).Cast<StorableComponent>());
	}
	...
	public void LateUpdate(IEntity target) {
		pivot.RevolverCylinder.localEulerAngles =
			new Vector3(0.0f, 0.0f, pivot.RevolverCylinder.localEulerAngles.z + angleDiff);
	}
