...
public class EmptyWeaponController : IWeaponController, IComparer<Collider> {
	...
	private bool FireListener(GameActionType type, bool down) {
		return down && entity == ServiceLocator.GetService<ISimulation>().Player &&
		       PlayerUtility.IsPlayerCanControlling;
	}

	public int Compare(Collider a, Collider b) {
		var num = Vector3.Dot(a.transform.forward, gameObject.transform.forward);
