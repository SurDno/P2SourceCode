...
public class PlayerShotgunWeaponController : IWeaponController {
	...
	private void ApplyBulletsVisibility() {
		var shotgunAmmos = pivot?.ShotgunAmmos;
		for (var index = 0; index < shotgunAmmos.Length; ++index)
	...
	private void CheckAmmo() {
		if (bullets == null || bullets.Value == 0) {
			...
			for (var index = 0; index < bullets.Value; ++index)
	...
	private void BeginReloading() {
		...
		var flag = durability != null && durability.Value < 0.30000001192092896;
		animatorState.ShotgunReload(true, bullets.Value);
		SetBulletVisible(true);
	}

	private void EndReloading() {
		animatorState.ShotgunReload(false);
	}
	...
	private void RefreshStorageAmmo() {
		...
		storageAmmo.AddRange(storage.Items.ToList()
			.FindAll(x => x.Groups.Contains(StorableGroup.Ammo_Shotgun) && x.Count > 0).Cast<StorableComponent>());
	}
