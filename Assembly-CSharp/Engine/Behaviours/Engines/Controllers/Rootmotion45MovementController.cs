...
public class Rootmotion45MovementController : IMovementController {
	...
	public void Initialize(GameObject gameObject) {
		...
		if (pivot == null)
			Debug.LogWarning(gameObject.name + " doesn't contain " + typeof(Pivot).Name + " unity component.",
				gameObject);
		else {
			...
			if (agent == null)
				Debug.LogError(gameObject.name + " doesn't contain " + typeof(NavMeshAgent).Name + " unity component.",
					gameObject);
			else {
	...
	private float CalculateAngleInLocalSpace(Vector3 directionInWorldSpace) {
		var vector3 = Vector3.ProjectOnPlane(gameObject.transform.InverseTransformDirection(directionInWorldSpace),
			Vector3.up);
	...
	public void StartMovement(Vector3 direction, EngineBehavior.GaitType gait) {
		if (rootmotion45 == null)
			Debug.LogError(
				"StartMovement " + gameObject.name + " doesn't contain " + typeof(Rootmotion45).Name +
				" unity component.", gameObject);
		else {
			...
			if (animatorState.MovableAngleStart > 0.0) {
				if (gait == EngineBehavior.GaitType.Walk) {
					var num = (int)(animatorState.MovableAngleStart / 45.0 + 0.5);
					...
					var num = (int)(animatorState.MovableAngleStart / 90.0 + 0.5);
				...
				var num = (int)(-(double)animatorState.MovableAngleStart / 45.0 + 0.5);
				...
				var num = (int)(-(double)animatorState.MovableAngleStart / 90.0 + 0.5);
	...
	public bool Rotate(Vector3 direction) {
		...
		var vector3 = Vector3.ProjectOnPlane(gameObject.transform.InverseTransformDirection(direction), Vector3.up);
	...
	private float CalculateRotationDeltaAngle(
		...
		var angleInLocalSpace = CalculateAngleInLocalSpace(direction);
		if (gait == EngineBehavior.GaitType.Run) {
			var num1 = Mathf.InverseLerp(0.0f, 40f, Mathf.Abs(angleInLocalSpace));
			var num2 = Mathf.Lerp(0.0f, 1f, num1 * num1);
			return Mathf.Clamp(angleInLocalSpace / dt, -num2 * rotationSpeed, rotationSpeed * num2) * dt;
		}

		var num3 = Mathf.InverseLerp(0.0f, 90f, Mathf.Abs(angleInLocalSpace));
		var num4 = Mathf.Lerp(0.0f, 1f, num3 * num3);
