...
public class PlayerWeaponServiceNew : MonoBehaviour, IEntityAttachable {
	...
	private List<PlayerWeaponSwitchCommand> switchCommandsList = new();
	...
	[Inspected(Mutable = true)] public WeaponKind KindBase => kind;
	...
	private void AddCommand(WeaponKind weapon, bool switchOn, IEntity item = null) {
		var command = new PlayerWeaponSwitchCommand();
	...
	public void OnWeaponSwitch(WeaponKind kind, bool switchOn) {
		var weaponSwitchCommand = switchCommandsList.Find(x => x.WeaponKind == kind && x.SwitchOn == switchOn);
		if (weaponSwitchCommand == null)
			return;
		weaponSwitchCommand.IsActive = false;
		var controller = controllers[weaponSwitchCommand.WeaponKind];
	...
	private void ExecuteCommand(PlayerWeaponSwitchCommand command) {
		...
		var controller1 = controllers[command.WeaponKind];
		if (command.SwitchOn) {
			currentWeaponController?.Shutdown();
			foreach (var controller2 in controllers)
	...
	public void Initialize(IEntity entity, Animator animator, bool geometryVisible) {
		...
		foreach (var controller1 in controllers) {
	...
	public IWeaponController GetCurrentWeaponController() {
		return currentWeaponController;
	}

	public bool IsWeaponOn() {
		return !(currentWeaponController is EmptyWeaponController);
	}

	public void Reaction() {
		currentWeaponController.Reaction();
	}
}