...
public class ColorGradient : IBlendable<ColorGradient> {
	...
	private void BlendColor(ColorGradient a, ColorGradient b, IBlendOperation op) {
		...
		if (a.ColorKeys.Count == 0) {
			var gradientColorKey = new GradientColorKey(Color.clear, 0.0f);
		...
		while (index1 < a.ColorKeys.Count || index2 < b.ColorKeys.Count) {
			var flag1 = index1 < a.ColorKeys.Count;
			var flag2 = index2 < b.ColorKeys.Count;
			var time1 = flag1 ? a.ColorKeys[index1].time : 0.0f;
			var time2 = flag2 ? b.ColorKeys[index2].time : 0.0f;
	...
	private void BlendAlpha(ColorGradient a, ColorGradient b, IBlendOperation op) {
		...
		if (a.AlphaKeys.Count == 0) {
			var gradientAlphaKey = new GradientAlphaKey(0.0f, 0.0f);
		...
		while (index1 < a.AlphaKeys.Count || index2 < b.AlphaKeys.Count) {
			var flag1 = index1 < a.AlphaKeys.Count;
			var flag2 = index2 < b.AlphaKeys.Count;
			var time1 = flag1 ? a.AlphaKeys[index1].time : 0.0f;
			var time2 = flag2 ? b.AlphaKeys[index2].time : 0.0f;
	...
	private static Color GetColor(List<GradientColorKey> items, int index, float time) {
		...
		var gradientColorKey1 = items[index - 1];
		var gradientColorKey2 = items[index];
		var t = Mathf.Clamp01(time - gradientColorKey1.time);
	...
	private static float GetAlpha(List<GradientAlphaKey> items, int index, float time) {
		...
		var gradientAlphaKey1 = items[index - 1];
		var gradientAlphaKey2 = items[index];
		var t = Mathf.Clamp01(time - gradientAlphaKey1.time);
