...
public class DialogCameraController : ICameraController {
	...
	public DialogCameraController(GameObject rigPrefab) {
		this.rigPrefab = rigPrefab;
	}
	...
	public void Shutdown() {
		...
		foreach (var storedLayer in storedLayers)
	...
	public void Update(IEntity target, GameObject gameObjectTarget) {
		...
		if (component == null || component.AnchorCameraFPS == null)
			return;
		if (unityRigInstance != null) {
			var transform = component.AnchorCameraFPS.transform;
			...
			var num = (float)((Mathf.Abs(localScale.x) + (double)Mathf.Abs(localScale.y) + Mathf.Abs(localScale.z)) /
			                  3.0);
			unityRigInstance.transform.localScale = new Vector3(num, num, num);
			var dialogLightingProfile = component.DialogLightingProfile;
			if (dialogLightingProfile == null)
				dialogLightingProfile =
					ScriptableObjectInstance<ResourceFromCodeData>.Instance.DefaultDialogLightingProfile;
			if (dialogLightingProfile != null) {
	...
	private void InternalInitialise(GameObject gameObjectTarget) {
		...
		if (gameObjectTarget != null)
			foreach (Component componentsInChild in gameObjectTarget.GetComponentsInChildren<Transform>()) {
				var gameObject = componentsInChild.gameObject;
		...
		var dialogCameraPrefab = ScriptableObjectInstance<ResourceFromCodeData>.Instance.DialogCameraPrefab;
		if (dialogCameraPrefab != null) {
			...
			var component = secondaryCameraObject.GetComponent<UnityEngine.Camera>();
