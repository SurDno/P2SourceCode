...
public static class BindWeatherConsoleCommands {
	...
	private static void Initialise() {
		ConsoleTargetService.AddTarget(typeof(WeatherController).Name,
			value => ServiceLocator.GetService<IWeatherController>());
		ConsoleTargetService.AddTarget("-weather",
			value => ServiceLocator.GetService<ITemplateService>().GetTemplates<IWeatherSnapshot>()
				.FirstOrDefault(o => o.Name == value));
		ConsoleTargetService.AddTarget("-weather_layer",
			value => ServiceLocator.GetService<WeatherController>().Blender.Items
				.ElementAtOrDefault(DefaultConverter.ParseInt(value)));
		SetConsoleCommand.AddBind<IWeatherLayerBlenderItem, float>("weather_opacity", true,
			(target, value) => target.SetOpacity(value));
		GetConsoleCommand.AddBind<IWeatherLayerBlenderItem, float>("weather_opacity", true, target => target.Opacity);
		SetConsoleCommand.AddBind<WeatherController, bool>("weather_enable", true,
			(target, value) => target.IsEnabled = value);
		...
		EnumConsoleCommand.AddBind("-weather", (Func<string>)(() => {
			var str = "\nWeather :\n";
			foreach (var template in ServiceLocator.GetService<ITemplateService>().GetTemplates<IWeatherSnapshot>())
				str = str + template.Name + "\n";
			return str;
		}));
		EnumConsoleCommand.AddBind("-weather_layer",
			(Func<string>)(() =>
	...
	private static string WeatherCommand(string command, ConsoleParameter[] parameters) {
		...
		if (parameters.Length == 2) {
			layerBlenderItem =
				ServiceLocator.GetService<WeatherController>().Blender.Items.FirstOrDefault() as
			...
			layerBlenderItem =
				ConsoleTargetService.GetTarget(typeof(IWeatherLayerBlenderItem), parameters[0]) as
					IWeatherLayerBlenderItem;
		...
		return "Set weather snaphot : " + target.Name + " , to layer : " + layerBlenderItem.Name + " , interval : " +
		       num;
	}
}