...
// Type: Engine.Source.Services.Consoles.Binds.SaveLoadConsoleCommands
  ...
  public class SaveLoadConsoleCommands
  {
    ...
    private static string SaveLoadGame(string command, ConsoleParameter[] parameters)
    {
      ...
      CoroutineService.Instance.WaitFrame((Action) (() =>
      {
        ...
        {
          case "save_game":
            ServiceLocator.GetService<SavesService>().Save(ProfilesUtility.SavePath(profiles.Current.Name, fileName));
            break;
          case "load_game":
            VirtualMachineController service1 = ServiceLocator.GetService<VirtualMachineController>();
            GameLauncher service2 = ServiceLocator.GetService<GameLauncher>();
            string str = ProfilesUtility.SavePath(profiles.Current.Name, fileName);
            if (File.Exists(str))
            {
