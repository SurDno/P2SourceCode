...
public class BindBlueprintConsoleCommands {
	...
	private static void Initialise() {
		ConsoleTargetService.AddTarget("-blueprint",
			value => ServiceLocator.GetService<ITemplateService>().GetTemplates<IBlueprintObject>()
				.FirstOrDefault(o => o.Name == value));
		EnumConsoleCommand.AddBind("-blueprint", (Func<string>)(() => {
			var str = "\nBlueprints :\n";
			foreach (var template in ServiceLocator.GetService<ITemplateService>().GetTemplates<IBlueprintObject>())
	...
	private static string CreateBlueprint(string command, ConsoleParameter[] parameters) {
		...
		var gameObject = ((BlueprintObject)target).GameObject;
		if (gameObject.GetComponent<FlowScriptController>() == null)
			return typeof(FlowScriptController).Name + " not found";
		var selection = UnityFactory.Instantiate(gameObject, "[Blueprints]");
	...
	private static string SendEvent(string command, ConsoleParameter[] parameters) {
		...
		if (parameters.Length != 2)
			return "Error parameters count";
		var target = ConsoleTargetService.GetTarget(typeof(GameObject), parameters[0]) as GameObject;
		var eventName = parameters[1].Value;
		if (target == null)
			return "Blueprint not found";
		var component = target.GetComponent<FlowScriptController>();
		if (component == null)
			return typeof(FlowScriptController).Name + " not found";
	...
	private static string SetValue(string command, ConsoleParameter[] parameters) {
		...
		if (parameters.Length != 3)
			return "Error parameters count";
		var target1 = ConsoleTargetService.GetTarget(typeof(GameObject), parameters[0]) as GameObject;
		var name = parameters[1].Value;
		var target2 = ConsoleTargetService.GetTarget(typeof(IObject), parameters[2]) as IObject;
		if (target1 == null)
			return "Blueprint not found";
		var component = target1.GetComponent<Blackboard>();
		if (component == null)
