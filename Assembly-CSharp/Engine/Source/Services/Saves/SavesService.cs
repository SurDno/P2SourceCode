...
public class SavesService : IErrorLoadingHandler {
	...
	public IEnumerator Load() {
		...
		if (saveLoadState)
			...
			for (var index = 0; index < serializables.Count; ++index) {
				var serializable = serializables[index];
				sw.Restart();
				yield return serializable.Load(this);
				sw.Stop();
				Debug.Log(ObjectInfoUtility.GetStream().Append(nameof(SavesService)).Append(" : ").Append(nameof(Load))
					.Append(" , type : ").Append(TypeUtility.GetTypeName(serializable.GetType()))
	...
	public IEnumerator Load(string saveName) {
		...
		if (saveLoadState)
			...
			for (var index = 0; index < serializables.Count; ++index) {
				var serializable = serializables[index];
				var typeName = TypeUtility.GetTypeName(serializable.GetType());
				var fileName = saveName + "/" + typeName + ".xml";
				sw.Restart();
				...
				sw.Stop();
				Debug.Log(ObjectInfoUtility.GetStream().Append(nameof(SavesService)).Append(" : ").Append(nameof(Load))
					.Append(" , file name : ").Append(fileName).Append(" , type : ").Append(typeName)
					.Append(" , elapsed : ").Append(sw.Elapsed));
				if (!HasErrorLoading) {
					serializable = null;
					typeName = null;
		...
		if (saveLoadState)
			...
			for (var index = 0; index < serializables.Count; ++index) {
				...
				try {
					...
					serializable.Unload();
					stopwatch.Stop();
					Debug.Log(ObjectInfoUtility.GetStream().Append(nameof(SavesService)).Append(" : ")
						.Append(nameof(Unload)).Append(" , type : ")
						.Append(TypeUtility.GetTypeName(serializable.GetType())).Append(" , elapsed : ")
						.Append(stopwatch.Elapsed));
				} catch (Exception ex) {
	...
	public void Save(string saveName) {
		...
		if (saveLoadState)
			...
			for (var index = 0; index < serializables.Count; ++index) {
				...
				var fileName = saveName + "/" + typeName + ".xml";
				stopwatch.Restart();
				SavesServiceUtility.SaveToFile("Document", serializable, fileName,
					ExternalSettingsInstance<ExternalOptimizationSettings>.Instance.SaveCompress);
				stopwatch.Stop();
				Debug.Log(ObjectInfoUtility.GetStream().Append(nameof(SavesService)).Append(" : ").Append(nameof(Save))
					.Append(" , type : ").Append(typeName).Append(" , elapsed : ").Append(stopwatch.Elapsed));
			}
