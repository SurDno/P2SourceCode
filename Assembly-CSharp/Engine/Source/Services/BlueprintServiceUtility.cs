...
public static class BlueprintServiceUtility {
	...
	public static FlowScriptController Start(
		...
		Debug.Log(ObjectInfoUtility.GetStream().Append("Start blueprint : ").Append(blueprintPrefab.name)
			.Append(" , target : ").GetInfo(target).Append(" , context : ").Append(context));
		...
		var gameObject1 = UnityFactory.Instantiate(blueprintPrefab, "[Blueprints]");
		gameObject1.name = blueprintPrefab.name;
		var component = gameObject1.GetComponent<FlowScriptController>();
	...
	public static FlowScriptController Start(
		...
		Action complete,
		string context = "") {
		var blueprintObject = (BlueprintObject)bp;
	...
	public static void Start(
		...
		IEntity target,
		string context = "") {
		var prefab = blueprint.Value;
		if (prefab == null)
			return;
		var component = UnityFactory.Instantiate(prefab, "[Blueprints]").GetComponent<FlowScriptController>();
		if (component == null)
			return;
		...
		component.SetValue("Target", target);
		var gameObject = ((IEntityView)target).GameObject;
		if (gameObject == null)
			Debug.LogError("Blueprint start failed, GameObject not found, target : " + target.GetInfo() +
			               " , context : " + context);
	...
	public static void StartAsync(
		...
		string context = "") {
		var resourcePath =
			AssetDatabaseUtility.ConvertToResourcePath(AssetDatabaseService.Instance.GetPath(blueprint.Id));
		if (resourcePath.IsNullOrEmpty())
			return;
		var key = Resources.LoadAsync(resourcePath);
		if (key == null)
			return;
	...
	private static void Request_completed(AsyncOperation operation) {
		...
		var description = asyncRequests[(ResourceRequest)operation];
		asyncRequests.Remove((ResourceRequest)operation);
		var resourcesLoaded = description.ResourcesLoaded;
		if (resourcesLoaded != null)
			resourcesLoaded();
		if (description.Target.IsDisposed)
			return;
		var gameObject = ((IEntityView)description.Target).GameObject;
		if (gameObject == null)
			Debug.LogError("Blueprint async start failed, GameObject not found, target (this is not a bug) : " +
			               description.Target.GetInfo() + " , context : " + description.Context);
		else {
			var prefab = description.Blueprint.Value;
			if (prefab == null)
				return;
			var component = UnityFactory.Instantiate(prefab, "[Blueprints]").GetComponent<FlowScriptController>();
			if (component == null)
			...
			destroy = graph => {
				graph.DestroyEvent -= destroy;
				var complete = description.Complete;
				if (complete == null)
					return;
				complete();
			};
