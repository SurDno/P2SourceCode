...
public class CombatServiceCharacterInfo : IChangeParameterListener {
	...
	public List<CombatServiceCharacterOrder> Orders = new();
	public Vector3 FightStartPosition;
	public Vector3 LastGotHitPosition;
	public List<CombatCry> HearedCries = new();
	private CombatServiceCharacterStateEnum state;
	private IEntity entity;
	private CombatServiceCharacterInfo currentEnemy;
	private List<CombatServiceCharacterInfo> personalAttackEnemies = new();
	private List<CombatServiceCharacterInfo> personalFearEnemies = new();
	private List<CombatServiceCharacterInfo> attackCharactersNearby = new();
	private List<CombatServiceCharacterInfo> fearCharactersNearby = new();
	...
	public IndividualCombatSettings CombatSettings {
		get {
			if (entity == null || combatSettings == null) {
				Entity = Character.Owner;
				var styleName = combatStyleParameter == null ? CombatStyleEnum.Default : combatStyleParameter.Value;
				combatSettings =
					ScriptableObjectInstance<FightSettingsData>.Instance.IndividualCombatSettings.Find(x =>
						x.Name == styleName);
			}
	...
	private void OnSee(IDetectableComponent detectable) {
		var interestingEntity = GetDetectorInterestingEntity(detectable);
		if (interestingEntity != null && !visibleCharacters.ContainsKey(interestingEntity)) {
			var character = GetCharacter(interestingEntity);
	...
	private void RenewDetector() {
		if (visibleCharacters.ContainsValue(null))
			foreach (var entity in new List<IEntity>(visibleCharacters.Keys))
				if (visibleCharacters[entity] == null)
					visibleCharacters[entity] = GetCharacter(entity);
		if (!hearableCharacters.ContainsValue(null))
			return;
		foreach (var entity in new List<IEntity>(hearableCharacters.Keys))
	...
	private IEntity GetDetectorInterestingEntity(IDetectableComponent detectable) {
		...
		if (owner == null)
			return null;
		var component1 = owner.GetComponent<NpcControllerComponent>();
		var component2 = owner.GetComponent<PlayerControllerComponent>();
	...
	public void RecountEnemies() {
		...
		foreach (var serviceCharacterInfo in visibleCharacters.Values)
			if (!serviceCharacterInfoList.Contains(serviceCharacterInfo))
				serviceCharacterInfoList.Add(serviceCharacterInfo);
		var fraction1 = RelatedFractionUtility.GetFraction(entity, FractionRelationEnum.FearOnSee);
		var fraction2 = RelatedFractionUtility.GetFraction(entity, FractionRelationEnum.AttackOnSee);
		var fraction3 = RelatedFractionUtility.GetFraction(entity, FractionRelationEnum.AttackOnSeeInfected);
		fearCharactersNearby = new List<CombatServiceCharacterInfo>();
		attackCharactersNearby = new List<CombatServiceCharacterInfo>();
		if (IsCombatIgnored)
			return;
		foreach (var serviceCharacterInfo in serviceCharacterInfoList)
			if (serviceCharacterInfo != null && !serviceCharacterInfo.IsDead &&
			    serviceCharacterInfo.IsIndoors == isIndoors && !serviceCharacterInfo.IsCombatIgnored) {
				if (!IsPlayer && PersonalFearEnemies != null && PersonalFearEnemies.Contains(serviceCharacterInfo) &&
				    !serviceCharacterInfo.WasBeaten)
					fearCharactersNearby.Add(serviceCharacterInfo);
				else if (!IsPlayer && PersonalAttackEnemies != null &&
				         PersonalAttackEnemies.Contains(serviceCharacterInfo))
					attackCharactersNearby.Add(serviceCharacterInfo);
				else {
					var targetFraction = FractionsHelper.GetTargetFraction(serviceCharacterInfo.entity, entity);
					if (targetFraction != FractionEnum.None) {
						if (fraction1 != null && fraction1.Contains(targetFraction) && !serviceCharacterInfo.WasBeaten)
							...
							var byName = serviceCharacterInfo.Parameters.GetByName<float>(ParameterNameEnum.Infection);
							var num = fractionSettings == null ? 0.1f : fractionSettings.InfectionThreshold;
	...
	public float GetTimeFromLastOrder() {
		return Time.time - stateSetTime;
	}
	...
	public void OnParameterChanged(IParameter parameter) {
		if (parameter.Name == ParameterNameEnum.Fraction) {
			fractionSettings =
				ScriptableObjectInstance<FractionsSettingsData>.Instance.Fractions.Find(x =>
					x.Name == ((IParameter<FractionEnum>)parameter).Value);
			needRecountEnemies = true;
