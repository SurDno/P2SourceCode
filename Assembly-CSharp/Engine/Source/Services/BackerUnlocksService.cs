...
public class BackerUnlocksService : IBackerUnlocksService {
	...
	bool IBackerUnlocksService.ItemUnlocked {
		get {
			var itemUnlocked = ItemUnlocked;
			Debug.Log(ObjectInfoUtility.GetStream().Append(nameof(BackerUnlocksService)).Append(" : ")
				.Append("ItemUnlocked").Append(" , value : ").Append(itemUnlocked));
			return itemUnlocked;
		}
	}
	...
	bool IBackerUnlocksService.QuestUnlocked {
		get {
			var questUnlocked = QuestUnlocked;
			Debug.Log(ObjectInfoUtility.GetStream().Append(nameof(BackerUnlocksService)).Append(" : ")
				.Append("QuestUnlocked").Append(" , value : ").Append(questUnlocked));
	...
	bool IBackerUnlocksService.PolyhedralRoomUnlocked {
		get {
			var polyhedralRoomUnlocked = PolyhedralRoomUnlocked;
			Debug.Log(ObjectInfoUtility.GetStream().Append(nameof(BackerUnlocksService)).Append(" : ")
				.Append("PolyhedralRoomUnlocked").Append(" , value : ").Append(polyhedralRoomUnlocked));
	...
	public bool ItemUnlocked {
		...
		private set {
			...
			var anyChangeEvent = AnyChangeEvent;
