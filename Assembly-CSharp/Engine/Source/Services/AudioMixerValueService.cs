...
public class AudioMixerValueService :
	...
	IUpdatable {
	[Inspected] private List<ItemInfo> items = new();
	[Inspected] private ReduceUpdateProxy<ItemInfo> updater;

	public void Initialise() {
		updater = new ReduceUpdateProxy<ItemInfo>(items, this,
			ExternalSettingsInstance<ExternalOptimizationSettings>.Instance.BlueprintSoundsDelay);
	...
	public void ComputeUpdateItem(ItemInfo item) {
		...
		if (nodes.Count == 0)
			return;
		var audioMixerValueNode1 = nodes[0];
		var t = audioMixerValueNode1.Value;
		for (var index = 1; index < nodes.Count; ++index) {
			var audioMixerValueNode2 = nodes[index];
			var num = audioMixerValueNode2.Value;
			if (num > (double)t) {
		...
		if (audioMixerValueNode1.Failed)
			return;
		var num1 = Mathf.LerpUnclamped(audioMixerValueNode1.MinValue, audioMixerValueNode1.MaxValue, t);
		var name = audioMixerValueNode1.Name;
	...
	public class ItemInfo {
		[Inspected(Header = true)] public string Name;
		[Inspected] public float DefaultValue;
		[Inspected] public List<AudioMixerValueNode> Nodes = new();

		[Inspected(Header = true)] public int Count => Nodes.Count;
	}
}