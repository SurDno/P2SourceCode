...
public class RuntimeTemplateLoader : ITemplateLoader {
	...
	public IEnumerator Load(Dictionary<Guid, IObject> items, Dictionary<Guid, string> names) {
		...
		sw.Stop();
		Debug.Log(ObjectInfoUtility.GetStream().Append(nameof(RuntimeTemplateLoader)).Append(" : ").Append(nameof(Load))
			.Append(" , elapsed : ").Append(sw.Elapsed).Append(" , created : ").Append(created));
	}
}