...
public class InsideIndoorListener : IInitialisable {
	...
	public bool InsideIndoor {
		...
		private set {
			if (insideIndoor == value)
				return;
			insideIndoor = value;
			var insideIndoorChanged = OnInsideIndoorChanged;
	...
	public bool IsolatedIndoor {
		...
		private set {
			...
			var isolatedIndoorChanged = OnIsolatedIndoorChanged;
	...
	private void OnBeginInteract(
		...
		if (item.Type != InteractType.Outdoor)
			return;
		var playerBeginsExit = OnPlayerBeginsExit;
	...
	private void UpdateValue() {
		...
		if (InsideIndoor) {
			var building = navigationComponent?.Building;
			IsolatedIndoor = building != null &&
