...
public class GizmoRenderBehaviour : MonoBehaviour {
	...
	public static GizmoRenderBehaviour Create(UnityEngine.Camera targetCamera) {
		if (targetCamera == null)
			return null;
		var gameObject = new GameObject("[Camera] Debug", typeof(UnityEngine.Camera), typeof(GizmoRenderBehaviour),
			typeof(DynamicResolutionCamera));
		gameObject.transform.SetParent(targetCamera.transform, false);
		var component = gameObject.GetComponent<GizmoRenderBehaviour>();
	...
	private void OnEnable() {
		gizmoService = ServiceLocator.GetService<GizmoService>();
	}

	private void OnDisable() {
		gizmoService.FireClear();
	}
	...
	private void OnPostRender() {
		gizmoService.FireDrawShapes();
	}

	public void OnGUI() {
		gizmoService.FireDrawTexts(camera);
	}
