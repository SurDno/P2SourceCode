...
public class SaveInfo {
	...
	public static SaveInfo GetSaveInfo(string saveName) {
		...
		var strArray = saveName.Split(separators, StringSplitOptions.RemoveEmptyEntries);
		...
		if (strArray.Length == 6) {
			var s = strArray[5];
			if (!DateTime.TryParseExact(s, "yyyy_MM_dd_HH_mm_ss", CultureInfo.InvariantCulture, DateTimeStyles.None,
				    out saveInfo.SaveDateTime))
	...
	public static string GetFileName(int days, int hours, int minutes, BuildingEnum building) {
		return days.ToString().PadLeft(2, '0') + "-" + hours.ToString().PadLeft(2, '0') + "-" +
		       minutes.ToString().PadLeft(2, '0') + "-" + building + "-" + Guid.NewGuid().ToString().Replace("-", "") +
