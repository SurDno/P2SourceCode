...
public static class InputUtility {
	public static string GetTagName(ActionGroup bind) {
		return "{ActionGroup." + bind.Name + "}";
	}
	...
	public static bool CheckModificators(KeyModifficator modifficators) {
		return (Input.GetKey(KeyCode.LeftControl) ? 1 : Input.GetKey(KeyCode.RightControl) ? 1 : 0) ==
		       (KeyModifficatorUtility.HasValue(modifficators, KeyModifficator.Control) ? 1 : 0) &&
		       (Input.GetKey(KeyCode.LeftShift) ? 1 : Input.GetKey(KeyCode.RightShift) ? 1 : 0) ==
		       (KeyModifficatorUtility.HasValue(modifficators, KeyModifficator.Shift) ? 1 : 0) &&
		       (Input.GetKey(KeyCode.LeftAlt) ? 1 : Input.GetKey(KeyCode.RightAlt) ? 1 : 0) ==
		       (KeyModifficatorUtility.HasValue(modifficators, KeyModifficator.Alt) ? 1 : 0);
	}
	...
	public static string GetHotKeyText(KeyCode key, KeyModifficator modifficators) {
		...
		var str3 = str2.Replace("Alpha", "");
		return str1 + str3;
	}

	public static string GetHotKeyNameByGroup(ActionGroup action, bool joystick, out bool isHold) {
		var actionGroup = ServiceLocator.GetService<GameActionService>().GetBinds()
			.FirstOrDefault(o => o.Name == action.Name);
		var hotKeyNameByGroup = "";
		isHold = false;
		if (actionGroup != null) {
			if (joystick) {
	...
	public static string GetHotKeyNameByActionWithoutHold(GameActionType action) {
		...
		var bind = service != null
			? service.GetBinds().FirstOrDefault(o => {
				if (!o.Actions.Contains(action))
					return false;
				return !InputService.Instance.JoystickUsed || o.Joystick != "";
			})
			: null;
		var actionWithoutHold = "";
		if (bind != null) {
			var actionGroup = ServiceLocator.GetService<GameActionService>().GetBinds()
				.FirstOrDefault(o => o.Name == bind.Name);
	...
	public static string GetHotKeyNameByGroup(ActionGroup action, bool joystick) {
		var bind = ServiceLocator.GetService<GameActionService>().GetBinds().FirstOrDefault(o => o.Name == action.Name);
		if (joystick && bind.Joystick == "") {
			var actionGroup = ServiceLocator.GetService<GameActionService>().GetBinds()
				.FirstOrDefault(o => o.Actions.Intersect(bind.Actions).Count() > 0 && o.Joystick != "");
			if (actionGroup != null)
				bind = actionGroup;
		}
