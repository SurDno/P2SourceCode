...
public static class TimeGroupDebug {
	...
	private static void Update() {
		...
		if (InputUtility.IsKeyDown(KeyCode.KeypadPlus)) {
			var solarTimeFactor = (int)service.SolarTimeFactor;
			var num = solarTimeFactor != 0 ? solarTimeFactor * 2 : 1;
			service.SolarTimeFactor = num;
		}

		if (InputUtility.IsKeyDown(KeyCode.KeypadMinus)) {
			var solarTimeFactor = (int)service.SolarTimeFactor;
			var num = solarTimeFactor > 1 ? solarTimeFactor / 2 : 0;
			service.SolarTimeFactor = num;
		}
		...
		if (InputUtility.IsKeyDown(KeyCode.Keypad9)) {
			var timeSpan = service.SolarTime - TimeSpan.FromHours(1.0);
		...
		if (InputUtility.IsKeyDown(KeyCode.Keypad4)) {
			var time = service.GameTime + TimeSpan.FromHours(1.0);
		...
		if (InputUtility.IsKeyDown(KeyCode.Keypad1)) {
			var gameTimeFactor = (int)service.GameTimeFactor;
			var num = gameTimeFactor != 0 ? gameTimeFactor * 2 : 1;
			service.GameTimeFactor = num;
		}

		if (InputUtility.IsKeyDown(KeyCode.Keypad2)) {
			var gameTimeFactor = (int)service.GameTimeFactor;
			var num = gameTimeFactor > 1 ? gameTimeFactor / 2 : 0;
			service.GameTimeFactor = num;
		}
		...
		var text2 = "  Is Game Paused : " + InstanceByRequest<EngineApplication>.Instance.IsPaused +
		            "     [hotkeys : pause pause]" + "\n  Solar Time : " + service.SolarTime.ToLongTimeString() +
		            "     [hotkeys : add hour 7 , set 00:00 : 8 , remove hour : 9]" + "\n  Solar Time Day : " +
		            service.SolarTime.GetTimesOfDay() + "\n  Solar Time Factor : " + service.SolarTimeFactor +
		            "     [hotkeys : great + , less - ,  default * , stop /]" + "\n  Game Time : " +
		            service.GameTime.ToLongTimeString() +
		            "     [hotkeys : add hour 4 , set 00:00 : 5 , remove hour : 6]" + "\n  Game Time Day: " +
		            service.GameTime.GetTimesOfDay() + "\n  Game Time Factor : " + service.GameTimeFactor +
		            "     [hotkeys : great 1 , less 2 ,  default 3 , stop 0]" + "\n  Default Time Factor : " +
		            service.DefaultTimeFactor;
