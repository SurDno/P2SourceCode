...
public static class GroupDebugService {
	...
	private static List<GroupInfo> groups = new();
	private static List<Action> handlers = new();
	...
	public static void RegisterGroup(Action handler) {
	...
	private static void Update() {
		...
		if (InputUtility.IsKeyDown(key))
			visible = !visible;
		var text1 = name + " (" + InputUtility.GetHotKeyText(key, modifficators) + ")";
		...
		foreach (var group in groups) {
			...
			if (visible) {
				var text2 = "  " + group.Name + " (" + InputUtility.GetHotKeyText(group.HotKey, group.Modifficators) +
				            ")";
