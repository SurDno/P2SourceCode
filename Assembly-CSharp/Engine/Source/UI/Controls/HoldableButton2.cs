...
public class HoldableButton2 : Selectable {
	...
	[SerializeField] private HideableView holdView;
	[SerializeField] private EventView cancelView;
	...
	private float CurrentHoldTime {
		...
		set {
			...
			if (!(progressView != null))
				return;
			progressView.Progress = currentHoldTime >= 0.0 && currentHoldTime <= (double)HoldTime
	...
	private void Update() {
		...
		if (!IsInteractable())
			...
			var num = CurrentHoldTime + Time.deltaTime;
	...
	public void GamepadStartHold() {
		StartHold();
	}

	public void GamepadEndHold() {
		EndHold();
	}
}