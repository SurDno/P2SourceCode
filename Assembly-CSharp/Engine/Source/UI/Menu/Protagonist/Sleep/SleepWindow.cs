...
public class SleepWindow : UIWindow, ISleepWindow, IWindow {
	...
	[SerializeField] private IntView hoursIntView;
	[SerializeField] private Button sleepButton;
	[SerializeField] private Button closeButton;
	[SerializeField] private Image fadingBackground;
	[Space] [SerializeField] private float sleepTimeFactor;
	[SerializeField] private float timeFactorBlendHours;
	[SerializeField] private float hurtingThreshold;
	[SerializeField] private float criticalFatigueThreshold;
	...
	public void SetHoursText(float value) {
		hoursIntView.IntValue = (int)value;
	}
	...
	private void Update() {
		...
		if (timeSpan >= delayTime)
			...
			var blend = Mathf.Sqrt(Mathf.Clamp01(
				Mathf.Min((float)timeSpan.TotalHours, (float)(delayTime - timeSpan).TotalHours) /
				timeFactorBlendHours));
	...
	private void BlendTimeFactor(float blend) {
		...
		var gameTimeFactor = service.GameTimeFactor;
	...
	private void UpdateSleepValue() {
		var byName = Actor?.GetComponent<ParametersComponent>()?.GetByName<bool>(ParameterNameEnum.Sleep);
		if (byName == null)
			return;
