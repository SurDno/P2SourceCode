...
public class BoundCharactersView : MonoBehaviour {
	[SerializeField] private BoundCharactersGroupView groupViewPrefab;
	[SerializeField] private RectTransform layout;
	[SerializeField] private BoundCharacterGroup[] groups;

	[SerializeField] [FormerlySerializedAs("animationTime")]
	private float singleAnimationTime = 1f;

	[SerializeField] private float allAnimationsTime = 3f;
	private List<BoundCharactersGroupView> groupViews = new();
	private List<BoundCharacterView> unseenGroupViews = new();
	private List<BoundCharacterView> unseenStateViews = new();
	...
	private void Build() {
		var service = ServiceLocator.GetService<BoundCharactersService>();
		if (service == null)
			return;
		var dictionary = new Dictionary<BoundCharacterGroup, List<BoundCharacterComponent>>();
		foreach (var characterComponent in service.Items) {
			var group = characterComponent.Group;
		...
		for (var index1 = 0; index1 < groups.Length; ++index1) {
			...
			if (dictionary.TryGetValue(groups[index1], out characterComponentList)) {
				...
				var charactersGroupView = Instantiate(groupViewPrefab, layout, false);
				groupViews.Add(charactersGroupView);
				charactersGroupView.SetGroup(groups[index1]);
				for (var index2 = 0; index2 < characterComponentList.Count; ++index2) {
					var boundCharacterView = charactersGroupView.AddCharacter(characterComponentList[index2]);
		...
		if (unseenGroupViews.Count > 0 || unseenStateViews.Count > 0) {
			animationTime = Mathf.Min(allAnimationsTime / (unseenGroupViews.Count + unseenStateViews.Count),
				singleAnimationTime);
	...
	private void UpdateScrollNavigation() {
		...
		var axis = InputService.Instance.GetAxis("LeftStickX");
		var num = (xPrev + axis) * Time.deltaTime;
		xPrev = axis;
		if (num == 0.0)
			return;
		_scrollbar.value += num * Time.deltaTime *
