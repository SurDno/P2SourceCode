...
public class LockPickingWindow :
	...
	[SerializeField] private LockPickingPin leftPin;
	[SerializeField] private LockPickingPin rightPin;
	[SerializeField] private GameObject successMessage;
	[SerializeField] private LockPickingSettingsData defaultSettings;
	...
	[SerializeField] private SwitchingItemView2 toolSwitchingView;
	[SerializeField] private HideableView actionPrompts;
	...
	private float ToolDurability {
		...
		set {
			if (toolSelector.Item == null)
				return;
			var component = toolSelector.Item.GetComponent<ParametersComponent>();
			if (component == null)
				return;
	...
	private bool BumpRight(GameActionType type, bool down) {
		if (!IsInteractive || !down)
			return false;
		var toolDurability = ToolDurability;
		var durability = toolDurability;
		var quality = ToolParameterValue(ParameterNameEnum.Quality, 1f);
	...
	public override Type GetWindowType() {
		return typeof(ILockPickingWindow);
	}
	...
	private float ToolParameterValue(ParameterNameEnum name, float defaultValue) {
		...
		if (component == null)
			return defaultValue;
		var byName = component.GetByName<float>(name);
