...
public class LockPickingPin : MonoBehaviour {
	[SerializeField] private LockPickingPinSpot[] sourSpotPrototypes;
	[SerializeField] private LockPickingPinSpot[] sweetSpotPrototypes;
	[SerializeField] private GameObject sweetIndicator;
	[SerializeField] private GameObject sourIndicator;
	...
	private float Position {
		...
		set {
			...
			transform.localEulerAngles =
				new Vector3(0.0f, 0.0f, Mathf.Lerp(rotationRange.x, rotationRange.y, position));
		}
	}
	...
	public void Build(LockPickingSettings settings) {
		...
		var pattern = settings.Patterns[Random.Range(0, settings.Patterns.Length)];
		...
		for (var index = 0; index < spots.Length; ++index) {
			var sweet = index == pattern.SweetSpotPosition;
			spots[index].Sweet = sweet;
			spots[index].View = CreateView(sweet, index);
			spots[index].Min = Random.value;
			num1 += spots[index].Min;
			spots[index].Max = !sweet
				? Random.Range(settings.SourSpotMinSize, settings.SourSpotMaxSize)
				: settings.SweetSpotSize;
			num2 += spots[index].Max;
		}

		var num3 = (1f - (float)(num2 + (double)settings.LowerDeadZone + settings.UpperDeadZone +
		                         settings.MiddleDeadZones * (double)(pattern.SpotCount - 1))) / num1;
		for (var index = 0; index < spots.Length; ++index) {
			if (index == 0) {
				spots[index].Min = settings.LowerDeadZone + spots[index].Min * num3;
				spots[index].Max += spots[index].Min;
			} else {
				spots[index].Min = (float)(spots[index - 1].Max + (double)settings.MiddleDeadZones +
				                           spots[index].Min * (double)num3);
				spots[index].Max += spots[index].Min;
			}

			if (spots[index].Sweet)
				lockPosition = (float)((spots[index].Min + (double)spots[index].Max) * 0.5);
			var view = spots[index].View;
			view.Setup((float)((spots[index].Max + (double)spots[index].Min) * 0.5),
				spots[index].Max - spots[index].Min);
	...
	public LockPickingPinSpot CreateView(bool sweet, int index) {
		...
		var view = Instantiate(original, original.transform.parent);
