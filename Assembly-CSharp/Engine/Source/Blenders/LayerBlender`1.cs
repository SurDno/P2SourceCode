...
public abstract class LayerBlender<T> : EngineObject, ILayerBlender<T> where T : class, IObject, IBlendable<T> {
	[Inspected] private List<ILayerBlenderItem<T>> items = new();
	[Inspected] private T current;
	[Inspected] private bool invalidate;
	...
	private void Compute() {
		...
		foreach (var layerBlenderItem in items) {
			var blender = layerBlenderItem.Blender;
			if (blender != null) {
