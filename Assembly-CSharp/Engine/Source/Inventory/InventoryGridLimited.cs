...
public class InventoryGridLimited :
	...
	public Cell this[int column, int row] =>
		column < 0 || row < 0 || column >= Columns || row >= Rows ? null : table[column, row];
	...
	public virtual void Add(Cell cell) {
		var length1 = Math.Max(cell.Column + 1, Columns);
		var length2 = Math.Max(cell.Row + 1, Rows);
		if (length1 >= Columns || length2 >= Rows) {
			var cellArray = new Cell[length1, length2];
			for (var index1 = 0; index1 < table.GetLength(0); ++index1) {
				for (var index2 = 0; index2 < table.GetLength(1); ++index2) {
