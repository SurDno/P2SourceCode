...
public class AudioTrack3dNode : FlowControlNode, IUpdatable {
	...
	[Port("Spatialize")] private ValueInput<bool> spatializeInput;
	...
	[Port("Loop")] private ValueInput<bool> loopInput;
			...
			if (Play) {
				if (source == null) {
					...
					if (mixer == null)
						return;
					currentVolume = 0.0f;
					source = CreateAudioSource(clip, mixer, 0.0f, spatializeInput.value, loop, targetInput.value,
