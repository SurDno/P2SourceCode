...
public class WaitingLoadingIndoorNode : FlowControlNode {
	...
	private IEnumerator WaitingLoading(FlowOutput output) {
		...
		if (player == null) {
			...
			if (locationItem == null) {
				...
				var location = locationItem.LogicLocation;
				if (location == null || ((LocationComponent)location).LocationType != LocationType.Indoor)
					output.Call();
				else {
					Debug.Log(ObjectInfoUtility.GetStream().Append(TypeUtility.GetTypeName(GetType()))
						.Append(" , location ").GetInfo(location.Owner));
					do {
						yield return null;
	...
	private bool CheckLoading(ILocationComponent location) {
		...
		foreach (var child in ((LocationComponent)location).Childs)
