...
public class VisualFlameEffect : IEffect {
	...
	public bool Prepare(float currentRealTime, float currentGameTime) {
		...
		if (single && !Name.IsNullOrEmpty() &&
		    Target.GetComponent<EffectsComponent>().Effects.FirstOrDefault(o => o.Name == Name) is VisualFlameEffect
			    visualFlameEffect) {
		...
		startTime = num;
		effect = ((IEntityView)Target).GameObject.GetComponentInChildren<SkinnedMeshRenderer>()?.gameObject
			?.AddComponent<BurnedSurfaceEffect>();
		if (effect != null) {
	...
	public bool Compute(float currentRealTime, float currentGameTime) {
		...
		var num2 = num1 - previousTime;
		previousTime = num1;
		var flag = num1 - (double)startTime > duration;
		if (effect != null) {
			var needRemove = GetNeedRemove();
			var num3 = Mathf.Max(duration - (num1 - startTime), 0.0f);
			var num4 = num3 >= (double)fadeOutTime ? 1f : num3 / fadeOutTime;
			if (needRemove) {
				effect.FireLevel = num4;
				effect.SmokeLevel = num4;
			} else if (flag) {
				effect.FireLevel =
					Mathf.MoveTowards(effect.FireLevel, fireLevelOnDisabled, num2 * fireAndSmokeChangeSpeed);
				effect.SmokeLevel = Mathf.MoveTowards(effect.SmokeLevel, smokeLevelOnDisabled,
					num2 * fireAndSmokeChangeSpeed);
				if (!Mathf.Approximately(effect.FireLevel, fireLevelOnDisabled) ||
				    !Mathf.Approximately(effect.SmokeLevel, smokeLevelOnDisabled))
					return true;
			}
		}
