...
public class IncreaseThirstEffect : IEffect {
	...
	public void ComputeEffect() {
		...
		var byName1 = component?.GetByName<bool>(runParameterName);
		if (byName1 != null && byName1.Value)
			return;
		var byName2 = component?.GetByName<float>(staminaParameterName);
		if (byName2 == null || byName2.Value >= (double)byName2.MaxValue)
			return;
		var byName3 = component?.GetByName<float>(thirstParameterName);
		if (byName3 == null || byName3.Value >= (double)byName3.MaxValue)
			return;
		var byName4 = component?.GetByName<bool>(lowStaminaParameterName);
		if (byName4 == null)
			return;
		var num1 = InstanceByRequest<DifficultySettings>.Instance.GetValue("Thirst_Speed");
		var num2 = byName3.Value;
		var num3 = !byName4.Value
			? num2 + num1 * increaseThirstStepMiddleStaminaValue
			: num2 + num1 * increaseThirstStepLowStaminaValue * byName2.MaxValue;
		byName3.Value = num3;
	}
