...
public class ReduceDamageEffect : IEffect {
	...
	private void ComputeEffect() {
		...
		if (byName1 == null)
			return;
		var byName2 = component?.GetByName<float>(adsorbtionMaxParameterName);
		if (byName2 == null)
			return;
		var num1 = byName1.Value;
		if (num1 <= 0.0)
			return;
		var num2 = InstanceByRequest<DifficultySettings>.Instance.GetValue("Clothes_Durability");
		var byName3 = AbilityItem?.Item?.GetComponent<ParametersComponent>()
			?.GetByName<float>(ParameterNameEnum.Durability);
		var num3 = byName3 == null ? 1f : byName3.Value;
		var num4 = maxArmor * num3;
		var num5 = num1 * num4;
		var num6 = (durabilityReduceByHit + num5 * durabilityReduceKoeficient) * num2;
		var num7 = byName2.Value + num5;
		byName2.Value = num7;
		if (byName3 == null)
			return;
