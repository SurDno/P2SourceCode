...
namespace Engine.Source.Effects
{
  ...
  public class ChangeItemParameterEffect : IEffect
  {
    ...
    private void ComputeEffect()
    {
      if (!enable)
        return;
      IParameter<float> byName = AbilityItem?.Item?.GetComponent<ParametersComponent>()?.GetByName<float>(itemParameterName);
      float num = string.IsNullOrEmpty(difficultyMultiplierParameterName) ? 1f : InstanceByRequest<DifficultySettings>.Instance.GetValue(difficultyMultiplierParameterName);
      if (byName == null)
        return;
