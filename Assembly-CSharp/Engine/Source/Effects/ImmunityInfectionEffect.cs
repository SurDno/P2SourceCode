...
public class ImmunityInfectionEffect : IEffect {
	...
	private void ComputeEffect() {
		...
		var byName1 = component?.GetByName<float>(infectionDamageParameterName);
		if (byName1 == null)
			return;
		var byName2 = component?.GetByName<float>(immunityParameterName);
		if (byName2 == null)
			return;
		var num1 = byName1.Value;
		if (num1 <= 0.0)
			return;
		var num2 = byName2.Value;
		if (num2 <= 0.0)
			return;
		var num3 = Mathf.Max(num1 - num2, 0.0f);
		var num4 = Mathf.Max(num2 - num1, 0.0f);
		byName1.Value = num3;
		byName2.Value = num4;
	}
}