...
public class IncreaseStaminaInBlockEffect : IEffect {
	...
	public void ComputeEffect() {
		...
		var byName1 = component?.GetByName<bool>(isFightingParameterName);
		if (byName1 != null && !byName1.Value)
			return;
		var byName2 = component?.GetByName<BlockTypeEnum>(blockTypeParameterName);
		if (byName2 != null && byName2.Value != BlockTypeEnum.Block)
			return;
		var byName3 = component?.GetByName<bool>(runParameterName);
		if (byName3 != null && byName3.Value)
			return;
		var byName4 = component?.GetByName<float>(staminaParameterName);
		if (byName4 == null)
			return;
		var num = byName4.Value;
