...
public abstract class PriorityContainer<T> : INeedSave {
	...
	protected List<PriorityItem<T>> items = new();
	...
	private PriorityParameterEnum Priority {
		get {
			if (items.Count > 0) {
				var priorityItem = items[items.Count - 1];
	...
	public bool NeedSave {
		get {
			if (items.Count == 0)
				return false;
			if (items.Count == 1) {
				var priorityItem = items[0];
				if (priorityItem == null || (priorityItem.Priority == PriorityParameterEnum.Default &&
				                             IsDefault(priorityItem.Value)))
					return false;
			}
	...
	public void SetValue(PriorityParameterEnum priority, T value) {
		...
		if (flag)
			return;
		var priorityItem1 = ProxyFactory.Create<PriorityItem<T>>();
