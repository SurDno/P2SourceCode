...
public class LabelService : InstanceByRequest<LabelService> {
	...
	public void Invalidate() {
		...
		label = ScriptableObjectInstance<BuildData>.Instance.Label.Replace("{Version}", Application.version)
			.Replace("{Branch}", ScriptableObjectInstance<BuildData>.Instance.Branch)
			.Replace("{ProjectName}", projectName).Replace("{DataVersion}", dataVersion.ToString());
