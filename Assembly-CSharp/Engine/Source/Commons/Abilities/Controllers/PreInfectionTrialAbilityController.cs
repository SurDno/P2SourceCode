...
public class PreInfectionTrialAbilityController :
	...
	private void CheckParameter() {
		if (parameterPreInfection == null)
			return;
		if (parameterPreInfection.Value > (double)thresholdValue &&
		    parameterPreInfection.Value > (double)lastPreinfectionValue)
	...
	public void ComputeUpdate() {
		if (gotPreinfectionHit && timeService.AbsoluteGameTime.TotalSeconds > nextCheckTime) {
			if (breakCoeficient1 * Mathf.Pow(parameterPreInfection.Value - breakCoeficient2, 2f) >=
			    (double)Random.value) {
				values.Clear();
