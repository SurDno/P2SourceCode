...
public class RaycastAbilityProjectile : IAbilityProjectile {
	...
	public void ComputeTargets(IEntity self, IEntity item, OutsideAbilityTargets targets) {
		...
		for (var index1 = 0; index1 < bulletsCount; ++index1) {
			var direction = parameter == null ? forward : ScatterSingleBulletDirection(forward, parameter.Value);
			...
			for (var index2 = 0; index2 < hits.Count; ++index2) {
				...
				if (!(gameObject2 == gameObject1) && !(gameObject2.tag == ignoredTag) &&
				    (!hits[index2].collider.isTrigger || gameObject2.layer ==
					    ScriptableObjectInstance<GameSettingsData>.Instance.NpcHitCollidersLayer.GetIndex())) {
					var targetBodyPartEnum = ShotTargetBodyPartEnum.Body;
					var tag = gameObject2.tag;
					if (tag == untagged) {
						var component = gameObject2.GetComponent<Pivot>();
						...
						if (tag == headTag || tag == bodyTag || tag == armTag || tag == legTag)
							do {
								if (component == null) {
									var parent = gameObject2.transform.parent;
									if (!(parent == null)) {
					...
					if (entity != null) {
						var component = entity.GetComponent<EffectsComponent>();
						if (component != null) {
	...
	private Vector3 ScatterSingleBulletDirection(Vector3 direction, float durability) {
		var num = Random.Range(0.0f,
			minimumAimingDeltaAngle +
			(float)((maximumAimingDeltaAngle - (double)minimumAimingDeltaAngle) * (1.0 - durability)));
