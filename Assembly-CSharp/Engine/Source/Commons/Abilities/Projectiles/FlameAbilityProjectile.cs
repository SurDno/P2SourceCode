...
public class FlameAbilityProjectile : IAbilityProjectile {
	...
	public void ComputeTargets(IEntity self, IEntity item, OutsideAbilityTargets targets) {
		var component1 = ((IEntityView)self).GameObject.GetComponent<PivotSanitar>();
		if (component1 == null)
			return;
		targets.Targets = component1.Targets.Select(o => o.GetComponent<EffectsComponent>()).Where(o => o != null)
			.ToList();
		if (component1.Flamethrower && component1.TargetObject != null &&
		    component1.AimingTime > (double)flameEffectiveTime) {
			var component2 = component1.TargetObject.GetComponent<EngineGameObject>();
			var component3 = component2?.Owner?.GetComponent<EffectsComponent>();
			if (component3 != null && !targets.Targets.Contains(component3)) {
				var forward = ((IEntityView)self).GameObject.transform.forward;
				var to = component1.TargetObject.position - ((IEntityView)self).GameObject.transform.position;
				if (Mathf.Abs(Vector3.Angle(forward, to)) < (double)hitAngle && to.magnitude < (double)radius) {
					var component4 = self?.GetComponent<DetectorComponent>();
					if (component4 == null)
						targets.Targets.Add(component3);
					else {
						var component5 = component2?.Owner?.GetComponent<DetectableComponent>();
	...
	private List<EffectsComponent> FilterIgnoredTargets(List<EffectsComponent> list) {
		...
		foreach (var effectsComponent in list) {
			var byName = effectsComponent.Owner.GetComponent<ParametersComponent>()
				?.GetByName<bool>(ParameterNameEnum.IsCombatIgnored);
