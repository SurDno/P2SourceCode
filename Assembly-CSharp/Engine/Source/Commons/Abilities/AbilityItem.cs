...
public class AbilityItem {
	...
	[Inspected] private HashSet<IEffect> dependEffects = new();
	private bool active;
	...
	[Inspected] public IAbilityController AbilityController => controller;

	[Inspected] public IAbilityProjectile Projectile => projectile;

	[Inspected] public AbilityTargetEnum Targets => targets;

	[Inspected] public List<IEffect> Effects => effects;

	public void AddDependEffect(IEffect effect) {
		dependEffects.Add(effect);
	}

	public void AddEffects() {
		var component = Ability.Owner.GetComponent<StorableComponent>();
		if (component != null) {
			item = Ability.Owner;
		...
		var targets = new OutsideAbilityTargets();
		if (projectile != null)
	...
	private bool CheckTargetPlayer(EffectsComponent target) {
		if (target == null || targets == AbilityTargetEnum.Item || targets == AbilityTargetEnum.Self ||
		    targets == AbilityTargetEnum.Target)
			return true;
		...
		if (owner == null)
			return true;
		var component = owner.GetComponent<PlayerControllerComponent>();
		return (targets == AbilityTargetEnum.ItemPlayer || targets == AbilityTargetEnum.SelfPlayer ||
		        targets == AbilityTargetEnum.TargetPlayer) == (component != null);
	}

	private void AddEffects(EffectsComponent target) {
		if (target == null)
			return;
		foreach (var effect1 in effects) {
			var effect2 = CloneableObjectUtility.Clone(effect1);
	...
	public void RemoveDependEffects() {
		foreach (var dependEffect in dependEffects)
