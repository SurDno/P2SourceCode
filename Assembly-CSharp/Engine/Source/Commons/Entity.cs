...
[GenerateProxy(TypeEnum.Copyable | TypeEnum.EngineCloneable | TypeEnum.DataRead | TypeEnum.DataWrite |
               TypeEnum.StateSave | TypeEnum.StateLoad)]
public class Entity :
	ComponentCollection,
	IEntity,
	IObject,
	...
	[Inspected] private EntityView view;
	private static List<MonoBehaviour> tmp = new();
	...
	public bool NeedSave => !DontSave && (NeedSaveComponents || !(Template is Entity template) ||
	                                      template.Name != Name || template.IsEnabled != IsEnabled);

	[Inspected] [StateSaveProxy] public string HierarchyPath => this.GetHierarchyPath();
	...
	public void RemoveListener(IEntityEventsListener listener) {
		var index = listeners != null ? Array.IndexOf(listeners, listener) : -1;
		if (index == -1)
			return;
	...
	[Inspected] public IEnumerable<IEntity> Childs => childs;

	[Inspected] public IEntity Parent => parent;
	...
	[Inspected] public IEntity SceneEntity => GetContainer(this);
	...
	public void OnAddedHierarchy() {
		...
		if (sceneEntity == null)
			return;
		var container = ((Entity)sceneEntity).HierarchyItem.Container;
		if (container != null) {
			...
			if (template != null)
				...
				Debug.LogError(
					"Template not found , scene : " + sceneEntity.GetInfo() + " , entity : " + this.GetInfo());
		} else
			Debug.LogError("Container not found : " + sceneEntity.GetInfo());
	}

	public void OnRemovedHierarchy() {
		hierarchyItem = null;
	}
	...
	private static IEntity GetContainer(IEntity entity) {
		...
		if (parent == null)
			return null;
		var hierarchyItem = ((Entity)parent).HierarchyItem;
	...
	public GameObject GameObject {
		...
		set {
			...
			if (View.GameObject != null) {
				...
				foreach (var monoBehaviour in tmp)
