...
public class ListKeyItems {
	...
	[Inspected] private List<KeyValuePair<string, KeyCode>> value = new();
	...
	public ListKeyItems(string name) {
		this.name = name;
		var strArray = PlayerSettings.Instance.GetString(name).Split(separators, StringSplitOptions.RemoveEmptyEntries);
