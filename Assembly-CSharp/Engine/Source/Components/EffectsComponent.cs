...
public class EffectsComponent : EngineComponent, IUpdatable {
	[Inspected] private List<IEffect> effects = new();
	private static List<IEffect> tmp = new();
	[FromThis] private ParametersComponent parameters;
	[FromThis] private LocationItemComponent locationItemComponent;
	[FromLocator] private TimeService timeService;
	...
	public void AddEffect(IEffect effect) {
		var index1 = -1;
		for (var index2 = 0; index2 < effects.Count; ++index2)
			if (effects[index2].Queue >= effect.Queue) {
				index1 = index2;
				break;
			}

		var totalSeconds1 = (float)currentRealTime.TotalSeconds;
		var totalSeconds2 = (float)currentTime.TotalSeconds;
	...
	public void ComputeUpdate() {
		if (Disabled && !forcedUpdate) {
			...
			if (InstanceByRequest<EngineApplication>.Instance.IsPaused && !forcedUpdate)
				return;
			forcedUpdate = false;
			var timeSpan1 = timeService.AbsoluteGameTime - prevTime;
			prevTime = timeService.AbsoluteGameTime;
			var timeSpan2 = timeService.RealTime - prevRealTime;
			prevRealTime = timeService.RealTime;
