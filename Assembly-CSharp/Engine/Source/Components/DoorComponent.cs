...
public class DoorComponent : EngineComponent, IDoorComponent, IComponent, INeedSave {
	...
	[CopyableProxy]
	protected PriorityContainer<List<Typed<IEntity>>> picklocks =
		ProxyFactory.Create<PriorityContainer<List<Typed<IEntity>>>>();
	...
	[Inspected(Mutable = true, Mode = ExecuteMode.Edit)]
	[CopyableProxy]
	protected PriorityContainer<List<Typed<IEntity>>> keys =
		ProxyFactory.Create<PriorityContainer<List<Typed<IEntity>>>>();
	...
	[Inspected] public IPriorityParameterValue<bool> IsFree { get; } = new PriorityParameterValue<bool>();

	[Inspected] public IPriorityParameterValue<bool> Opened { get; } = new PriorityParameterValue<bool>();

	[Inspected] public IPriorityParameterValue<bool> Bolted { get; } = new PriorityParameterValue<bool>();

	[Inspected] public IPriorityParameterValue<bool> Marked { get; } = new PriorityParameterValue<bool>();

	[Inspected]
	public IPriorityParameterValue<bool> SendEnterWithoutKnock { get; } = new PriorityParameterValue<bool>();

	[Inspected] public IPriorityParameterValue<LockState> LockState { get; } = new PriorityParameterValue<LockState>();

	[Inspected] public IParameterValue<bool> CanBeMarked { get; } = new ParameterValue<bool>();

	[Inspected] public IParameterValue<bool> Knockable { get; } = new ParameterValue<bool>();

	[Inspected] public IParameterValue<bool> Pickable { get; } = new ParameterValue<bool>();

	[Inspected] public IParameterValue<int> Difficulty { get; } = new ParameterValue<int>();
	...
	public void AddPicklock(IEntity item) {
		AddPicklock(PriorityParameterEnum.Default, item);
	}
	...
	public void AddKey(IEntity item) {
		AddKey(PriorityParameterEnum.Default, item);
	}

	public void RemoveKey(IEntity item) {
		RemoveKey(PriorityParameterEnum.Default, item);
	}
	...
	private void ChangeBoolValueEvent(bool value) {
		FireInvalidate();
	}

	private void ChangeIntValueEvent(int value) {
		FireInvalidate();
	}
}