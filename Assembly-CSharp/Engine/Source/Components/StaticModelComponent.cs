...
public class StaticModelComponent : EngineComponent, IEntityEventsListener {
	...
	[FromThis] private LocationComponent location;
	...
	[Inspected] public bool RelativePosition => relativePosition;
	...
	private void UpdateModel() {
		var reference = connection.Value;
		if (reference == null)
			return;
		var flag1 = Owner.IsEnabledInHierarchy && ((IEntityView)Owner).IsAttached &&
		            (NeedLoad || location.Player != null);
		if (flag1 && sceneAsset == null)
			...
			var flag2 = ((IEntityView)Owner).IsAttached;
	...
	private IEnumerator OnLoad(bool success) {
		if (!success)
			...
			if (!sceneAsset.IsValid)
				throw new Exception(Owner.GetInfo());
			var container = ServiceLocator.GetService<HierarchyService>().GetContainer(Connection.Id);
			if (container == null)
				throw new Exception("Container not found for connection id : " + Connection.Id + " , owner : " +
				                    Owner.GetInfo());
			var sceneContainer = SceneObjectContainer.GetContainer(sceneAsset.Scene);
			if (sceneContainer == null)
				throw new Exception(typeof(SceneObjectContainer).Name + " not found, path : " + sceneAsset.Path +
				                    " , owner : " + Owner.GetInfo());
			if (relativePosition)
				MoveToPosition();
			if (Owner.Childs != null) {
				...
				foreach (var entity in childs) {
					var child = entity;
					var item = container.GetItemByTemplateId(child.TemplateId);
					if (item != null) {
						var go = sceneContainer.GetGameObject(item.Reference.Id);
						if (go == null)
	...
	private void MoveToPosition() {
		foreach (var rootGameObject in sceneAsset.Scene.GetRootGameObjects()) {
			var matrix4x4 = Matrix4x4.TRS(rootGameObject.transform.localPosition,
				rootGameObject.transform.localRotation, rootGameObject.transform.localScale);
			Vector3 position;
			Quaternion rotation;
			EntityViewUtility.ConvertMatrix(
				Matrix4x4.TRS(((IEntityView)Owner).Position, ((IEntityView)Owner).Rotation, Vector3.one) * matrix4x4,
				out position, out rotation);
			var transform = rootGameObject.transform.transform;
