...
public class DynamicModelComponent :
	...
	public IEnumerable<IModel> Models {
		get { return models.Select(o => o.Value).Where(o => o != null); }
	}
	...
	private void UpdateEnabled() {
		if (locationItem == null || !((IEntityView)Owner).IsAttached)
			return;
		var flag = !locationItem.IsHibernation && Owner.IsEnabledInHierarchy && IsEnabled &&
	...
	private void CreateGameObject() {
		var prefab = prefabAsset.Prefab;
		if (Profiler.enabled)
			Profiler.BeginSample("Instantiate : " + prefab.name);
		var gameObject = UnityFactory.Instantiate(prefab, group);
		if (Profiler.enabled)
			Profiler.EndSample();
	...
	private void DestroyCurrentGameObject() {
		if (delaySetInstance) {
			...
			var delayInstance = this.delayInstance;
			var delayInstanceName = this.delayInstanceName;
	...
	private void SetGameObject(GameObject go, string name) {
		...
		if (Profiler.enabled)
			Profiler.EndSample();
		EntityViewUtility.SetTransformAndData(Owner, ((IEntityView)Owner).Position, ((IEntityView)Owner).Rotation,
	...
	public void ComputeUpdate() {
		var flag = !locationItem.IsHibernation && Owner.IsEnabledInHierarchy && IsEnabled &&
		           InstanceByRequest<EngineApplication>.Instance.ViewEnabled;
		if (modelInvalidate &&
		    (flag || ExternalSettingsInstance<ExternalOptimizationSettings>.Instance.CreateDisabledPrefabs)) {
			modelInvalidate = false;
			UpdateModel();
		}

		if (!flag && ExternalSettingsInstance<ExternalOptimizationSettings>.Instance.DestroyDisabledPrefabs &&
		    ((IEntityView)Owner).IsAttached && !ServiceCache.OptimizationService.FrameHasSpike) {
