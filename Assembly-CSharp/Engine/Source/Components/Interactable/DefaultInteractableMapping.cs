...
public static class DefaultInteractableMapping {
	public static InteractableWindow.IconType GetIconType(
		...
		var lockState = component.LockState;
		if (lockState.Value == LockState.Blocked)
			return InteractableWindow.IconType.Blocked;
		return lockState.Value == LockState.Locked
			? InteractableWindow.IconType.Locked
			: InteractableWindow.IconType.Normal;
	}
	...
	public static string GetText(List<InteractItemInfo> validateItems) {
		...
		foreach (var validateItem in validateItems)
			if (InstanceByRequest<EngineApplication>.Instance.IsDebug || !validateItem.Invalid) {
				bool isHold;
				var hotKeyNameByAction1 = InputUtility.GetHotKeyNameByAction(validateItem.Item.Action,
					InputService.Instance.JoystickUsed, out isHold);
				if (!hotKeyNameByAction1.IsNullOrEmpty()) {
					if (!text1.IsNullOrEmpty())
						text1 += "\n";
					if (validateItem.Invalid) {
						var hotKeyNameByAction2 = InputUtility.GetHotKeyNameByAction(validateItem.OverrideAction,
							InputService.Instance.JoystickUsed, out isHold);
						text1 = text1 + "<color=" +
						        ScriptableObjectInstance<GameSettingsData>.Instance.InteractableDebugTextColor
							        .ToRGBHex() + ">[" + hotKeyNameByAction2 + "   ( " + hotKeyNameByAction1 +
						        " ) ] : " + service.GetText(validateItem.Item.Title);
						text1 += "   [ Debug ]";
						...
						text1 = text1 + "<color=" +
						        ScriptableObjectInstance<GameSettingsData>.Instance.InteractableCrimeTextColor
							        .ToRGBHex() + ">[" + hotKeyNameByAction1 + "] : " +
						        service.GetText(validateItem.Item.Title);
						var text2 = service.GetText("{Interact.Crime}");
						text1 = text1 + "   [" + text2 + "]";
					} else if (validateItem.Item.Blueprint.Id == Guid.Empty)
						text1 = text1 + "<color=" +
						        ScriptableObjectInstance<GameSettingsData>.Instance.InteractableDisabledTextColor
							        .ToRGBHex() + ">[" + hotKeyNameByAction1 + "] : " +
						        service.GetText(validateItem.Item.Title);
					else
						text1 = text1 + "<color=" +
						        ScriptableObjectInstance<GameSettingsData>.Instance.InteractableNormalTextColor
							        .ToRGBHex() + ">[" + hotKeyNameByAction1 + "] : " +
						        service.GetText(validateItem.Item.Title);
	...
	public static string[] GetText(
		...
		foreach (var validateItem in validateItems) {
			var str1 = "";
			bool isHold;
			if ((InstanceByRequest<EngineApplication>.Instance.IsDebug || !validateItem.Invalid) && !InputUtility
				    .GetHotKeyNameByAction(validateItem.Item.Action, InputService.Instance.JoystickUsed, out isHold)
				    .IsNullOrEmpty()) {
				iconSprites.Add(new KeyValuePair<Sprite, bool>(
					ControlIconsManager.Instance.GetIconSprite(validateItem.Item.Action, out isHold), isHold));
				string str2;
				if (validateItem.Crime) {
					str2 = "<color=" +
					       ScriptableObjectInstance<GameSettingsData>.Instance.InteractableCrimeTextColor.ToRGBHex() +
					       "> : " + service.GetText(validateItem.Item.Title) + "   [" +
					       service.GetText("{Interact.Crime}") + "]";
					actions.Add(new KeyValuePair<GameActionType, bool>(validateItem.Item.Action, isHold));
				} else if (validateItem.Item.Blueprint.Id == Guid.Empty)
					str2 = str1 + "<color=" +
					       ScriptableObjectInstance<GameSettingsData>.Instance.InteractableDisabledTextColor
						       .ToRGBHex() + "> : " + service.GetText(validateItem.Item.Title);
				else {
					str2 = str1 + "<color=" +
					       ScriptableObjectInstance<GameSettingsData>.Instance.InteractableNormalTextColor.ToRGBHex() +
					       "> : " + service.GetText(validateItem.Item.Title);
					actions.Add(new KeyValuePair<GameActionType, bool>(validateItem.Item.Action, isHold));
				}
