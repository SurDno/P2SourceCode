...
public static class KeyOpeningDoorInteractValidator {
	...
	public static ValidateResult Validate(IInteractableComponent interactable, InteractItem item) {
		var door = interactable.GetComponent<IDoorComponent>();
		if (door == null)
		...
		return !ServiceLocator.GetService<ISimulation>().Player.GetComponent<StorageComponent>().Items
			.Select(o => o.Owner.TemplateId).Any(o => door.Keys.Select(p => p.Id).Any(p => p == o))
			? new ValidateResult(false, "keys not found")
