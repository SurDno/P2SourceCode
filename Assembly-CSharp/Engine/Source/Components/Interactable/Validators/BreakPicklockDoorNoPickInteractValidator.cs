	...
	public static ValidateResult Validate(IInteractableComponent interactable, InteractItem item) {
		...
		if (component1.IsOutdoor && component2.IsIndoor)
			return new ValidateResult(false,
				"IsOutdoor " + component1.IsOutdoor + " && IsIndoor " + component2.IsIndoor);
		return player.GetComponent<StorageComponent>().Items.Any(x => x.Groups.Contains(StorableGroup.Lockpick_Door))
			? new ValidateResult(false, "Lockpick_Door found")
