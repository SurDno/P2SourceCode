...
public class ControllerComponent :
	...
	IUpdatable,
	IPlayerActivated {
	[FromThis] private AttackerPlayerComponent attackerPlayer;
	[FromThis] private ParametersComponent parameters;
	[FromLocator] private InterfaceBlockingService interfaceBlockingService;
	private bool added;
	...
	[Inspected] public IParameterValue<bool> IsFighting { get; } = new ParameterValue<bool>();

	[Inspected] public IParameterValue<bool> IsStelth { get; } = new ParameterValue<bool>();

	[Inspected] public IParameterValue<bool> IsRun { get; } = new ParameterValue<bool>();

	[Inspected] public IParameterValue<bool> IsWalk { get; } = new ParameterValue<bool>();

	[Inspected] public IParameterValue<bool> IsFlashlight { get; } = new ParameterValue<bool>();

	[Inspected] public IParameterValue<float> WalkModifier { get; } = new ParameterValue<float>();

	[Inspected] public IParameterValue<bool> WalkBlock { get; } = new ParameterValue<bool>();

	[Inspected] public IParameterValue<float> RunModifier { get; } = new ParameterValue<float>();

	[Inspected] public bool IsJump { get; set; }

	[Inspected] public bool IsForward => ForwardValue != 0.0;

	[Inspected] public bool IsBackward => BackwardValue != 0.0;

	[Inspected] public bool IsLeft => LeftValue != 0.0;

	[Inspected] public bool IsRight => RightValue != 0.0;

	[Inspected] public float ForwardValue => forward ? 1f : forwardValue;

	[Inspected] public float BackwardValue => backward ? 1f : backwardValue;

	[Inspected] public float LeftValue => left ? 1f : leftValue;

	[Inspected] public float RightValue => right ? 1f : rightValue;

	[Inspected] public Vector3 PushVelocity { get; set; }
	...
	private void AttackerPlayer_HandsHolsteredEvent(WeaponKind weapon) {
		var weaponEnableChanged = OnWeaponEnableChanged;
	...
	private bool GenericPlayerMenuListener(GameActionType type, bool down) {
		if (!((type == GameActionType.GenericPlayerMenu) & down) || !PlayerUtility.IsPlayerCanControlling)
			return false;
		return SimplePlayerWindowSwapper.LastOpenedPlayerWindowType == typeof(IInventoryWindow)
			? InventoryListener(type, down)
			: SimplePlayerWindowSwapper.CallLastPlayerWindow();
	}

	private bool BoundCharactersListener(GameActionType type, bool down) {
		if (!down || (InputService.Instance.JoystickUsed && JoystickLayoutSwitcher.Instance.CurrentLayout != 0) ||
	...
	private bool InventoryListener(GameActionType type, bool down) {
		if (!down ||
		    (InputService.Instance.JoystickUsed && type == GameActionType.Inventory &&
		     JoystickLayoutSwitcher.Instance.CurrentLayout != 0) || !PlayerUtility.IsPlayerCanControlling ||
	...
	private bool UnholsterListener(GameActionType type, bool down) {
		if (InputService.Instance.JoystickUsed || !down || !PlayerUtility.IsPlayerCanControlling ||
		    attackerPlayer == null)
			return false;
