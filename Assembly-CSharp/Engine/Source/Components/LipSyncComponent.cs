...
public class LipSyncComponent : EngineComponent, ILipSyncComponent, IComponent {
	...
	[Inspected] public bool IsPlaying => audioState != null && audioState.AudioSource != null;
	...
	public void Play(ILipSyncObject lipSync, bool usePause) {
		Stop();
		var mixer = IsIndoor
			? ScriptableObjectInstance<GameSettingsData>.Instance.NpcSpeechIndoorMixer
			: ScriptableObjectInstance<GameSettingsData>.Instance.NpcSpeechOutdoorMixer;
		if (mixer == null)
			return;
	...
	public void Play(
		...
		if (mixer == null)
			return;
		var lipSyncInfo = LipSyncUtility.GetLipSyncInfo(lipSync, lastIndex, out lastIndex);
		if (lipSyncInfo == null)
			return;
		var clip = lipSyncInfo.Clip.Value;
		if (clip == null)
			return;
		audioState = SoundUtility.PlayAudioClip3D(transform, clip, mixer, 1f, minDistance, maxDistance, false, 0.0f,
			usePause, Owner.GetInfo(), (Action)(() => {
				var playCompleteEvent = PlayCompleteEvent;
				if (playCompleteEvent != null)
					playCompleteEvent();
				audioState = null;
	...
	public void Play(
		...
		if (clip == null)
			return;
		audioState = SoundUtility.PlayAudioClip3D(transform, clip, mixer, 1f, false, 0.0f, customSource, usePause,
			Owner.GetInfo(), (Action)(() => {
				var playCompleteEvent = PlayCompleteEvent;
				if (playCompleteEvent != null)
					playCompleteEvent();
				audioState = null;
	...
	private void PlayLipSync(AudioState audioState, LipSyncInfo lipsync) {
		...
		if (data == null)
			return;
		var component = gameObject.GetComponent<Lipsync>();
		if (component == null)
			return;
	...
	private void PlayTest() {
		var id = new Guid("dccf3739-64a7-c054-ba71-6e6e9925eeed");
		var template = ServiceLocator.GetService<ITemplateService>().GetTemplate<ILipSyncObject>(id);
		if (template == null)
			return;
		Play(template, true);
	}
