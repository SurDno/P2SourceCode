...
namespace Engine.Source.Components
{
    ...
    IEntityEventsListener
  {
    ...
    [Inspected]
    public IParameterValue<bool> IsAway { get; } = new ParameterValue<bool>();

    [Inspected]
    public IParameterValue<bool> CanAutopsy { get; } = new ParameterValue<bool>();

    [Inspected]
    public IParameterValue<bool> CanTrade { get; } = new ParameterValue<bool>();

    [Inspected]
    public IParameterValue<bool> ForceTrade { get; } = new ParameterValue<bool>();

    [Inspected]
    public IParameterValue<bool> CanHeal { get; } = new ParameterValue<bool>();
    ...
    [Inspected]
    public IParameterValue<float> Pain { get; } = new ParameterValue<float>();

    [Inspected]
    public IParameterValue<float> Immunity { get; } = new ParameterValue<float>();

    [Inspected]
    public IParameterValue<StammKind> StammKind { get; } = new ParameterValue<StammKind>();

    [Inspected]
    public IParameterValue<FractionEnum> Fraction { get; } = new ParameterValue<FractionEnum>();

    [Inspected]
    public IParameterValue<CombatStyleEnum> CombatStyle { get; } = new ParameterValue<CombatStyleEnum>();
    ...
    [Inspected]
    public IParameterValue<bool> ImmuneBoostAttempted { get; } = new ParameterValue<bool>();

    [Inspected]
    public IParameterValue<bool> IsCombatIgnored { get; } = new ParameterValue<bool>();

    [Inspected]
    public IParameterValue<bool> SayReplicsInIdle { get; } = new ParameterValue<bool>();
    ...
    public void StoreState(List<IParameter> states, bool indoor)
    {
      if (!indoor)
        return;
      CrowdContextUtility.Store(parameters, states, ParameterNameEnum.Dead, ParameterNameEnum.Health, ParameterNameEnum.Customization, ParameterNameEnum.Model);
    }
    ...
    public void RestoreState(List<IParameter> states, bool indoor)
    {
      ...
      if (!indoor)
        return;
      CrowdContextUtility.Restore(parameters, states, ParameterNameEnum.Dead, ParameterNameEnum.Health, ParameterNameEnum.Customization, ParameterNameEnum.Model);
    }

    private void RestoreOrGenerateParameters(List<IParameter> states)
    {
      IParameter<int> byName1 = parameters.GetByName<int>(ParameterNameEnum.Customization);
      if (byName1 != null)
      {
    ...
    private void ComputeEventsToSingleTarget(ActionEnum action, IEntity target)
    {
      if (target == null)
        return;
      Transform transform = ((IEntityView) Owner).GameObject.transform;
