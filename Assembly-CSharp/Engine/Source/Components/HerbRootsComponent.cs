...
public class HerbRootsComponent :
	...
	[FromThis] private InteractableComponent interactable;
	[Inspected] private bool spawnPointsAttached;

	[Inspected] [StateSaveProxy] [StateLoadProxy]
	protected HerbRootsComponentStateEnum state = HerbRootsComponentStateEnum.Sleeping;

	[Inspected] [StateSaveProxy] [StateLoadProxy]
	protected List<double> herbGrowTimesLeftSorted = new();
	...
	[Inspected] private List<HerbRootsPointInfo> spawnPoints = new();
	[Inspected] private HerbRootsPointInfo waitingPoint;

	[DataReadProxy] [DataWriteProxy] [Inspected(Mutable = true, Mode = ExecuteMode.Edit)] [CopyableProxy]
	protected List<HerbRootsTemplate> templates = new();
	...
	public bool NeedSave => currentHerbsCount != 0 || state != HerbRootsComponentStateEnum.Sleeping ||
	                        herbGrowTimesLeftSorted.Count != 0 || grownHerbsCount != 0;

	[Inspected]
	private IEnumerable<IEntity> Entities {
		get { return spawnPoints.Select(o => o.Entity).Where(o => o != null); }
	}

	[Inspected]
	private bool IsOperating => locationItem != null && !locationItem.IsHibernation && Owner != null &&
	                            Owner.IsEnabledInHierarchy && spawnPointsAttached;

	[Inspected] public bool IsHerbsbudgetReached => grownHerbsCount >= herbsBudget;
	...
	public void FireOnActivateStartEvent() {
		OnActivateStartEvent();
	}

	public void FireOnTriggerEnterEvent() {
		OnTriggerEnterEvent();
	}

	public void FireOnTriggerLeaveEvent() {
		OnTriggerLeaveEvent();
	}
	...
	private IEntity WeightedRandomTemplate() {
		if (templates.Count == 0)
			return null;
		var max = templates.Sum(t => t.Weight);
		if (max == 0.0)
			return null;
		var num1 = Random.Range(0.0f, max);
		var num2 = 0.0f;
	...
	public void GiveBlood() {
		...
		var gameTime = ServiceLocator.GetService<TimeService>().GameTime;
		var num1 = Random.Range(herbsCountMin, herbsCountMax + 1);
		for (var index = 0; index < num1 && grownHerbsCount < herbsBudget; ++index) {
			var num2 = 60f * Random.Range(HerbsGrowTimeInMinutesMin, herbsGrowTimeInMinutesMax);
	...
	private void Interactable_EndInteractEvent(
		...
		if (component1 == null)
			return;
		foreach (var storableComponent in component1.Items)
			if (storableComponent.Groups.Contains(StorableGroup.Craft_Organ) &&
			    !storableComponent.Groups.Contains(StorableGroup.Autopsy_Chest) &&
			    !storableComponent.Groups.Contains(StorableGroup.Autopsy_Head) &&
			    !storableComponent.Groups.Contains(StorableGroup.Autopsy_Stomach)) {
				ServiceLocator.GetService<NotificationService>()
					.AddNotify(NotificationEnum.ItemDrop, storableComponent.Owner.Template);
	...
	private void ComputeCreateEntity() {
		if (spawnPoints.Count == 0 || herbGrowTimesLeftSorted.Count == 0 || templates.Count == 0 ||
		    ServiceLocator.GetService<TimeService>().GameTime.TotalSeconds < herbGrowTimesLeftSorted[0])
			return;
		var num = Random.Range(0, spawnPoints.Count);
		herbGrowTimesLeftSorted.RemoveAt(0);
		if (state == HerbRootsComponentStateEnum.Active)
			interactable.IsEnabled = herbGrowTimesLeftSorted.Count == 0;
		for (var index1 = 0; index1 < spawnPoints.Count; ++index1) {
			var index2 = (index1 + num) % spawnPoints.Count;
			if (spawnPoints[index2].Entity == null) {
				waitingPoint = spawnPoints[index2];
				var entity = ServiceCache.Factory.Instantiate(WeightedRandomTemplate());
				((Entity)entity).DontSave = true;
				var service = ServiceLocator.GetService<Simulation>();
				service.Add(entity, service.Objects);
				AddEntity(entity);
				var onHerbSpawnEvent = OnHerbSpawnEvent;
				if (onHerbSpawnEvent != null)
					onHerbSpawnEvent();
				if (herbGrowTimesLeftSorted.Count != 0)
					break;
				var lastHerbSpawnEvent = OnLastHerbSpawnEvent;
	...
	private void EnqueueCreateEntities() {
		for (var index = 0; index < currentHerbsCount; ++index)
