...
public class RepairableComponent : EngineComponent, IRepairableComponent, IComponent {
	...
	[Inspected] public RepairableSettings Settings => settings.Value;
	...
	public RepairableLevel TargetLevel() {
		...
		if (levels == null || levels.Count == 0)
			return null;
		var byName = parameters.GetByName<float>(ParameterNameEnum.Durability);
		if (byName == null)
			return null;
		var num1 = byName.Value;
		var num2 = float.MaxValue;
		var repairableLevel1 = BaseLevel();
		for (var index = 0; index < levels.Count; ++index) {
			var repairableLevel2 = levels[index];
			if (repairableLevel2 != null) {
				var maxDurability = repairableLevel2.MaxDurability;
