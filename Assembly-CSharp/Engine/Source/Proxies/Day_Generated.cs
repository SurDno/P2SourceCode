...
public class Day_Generated : Day, ICloneable, ICopyable, ISerializeDataWrite, ISerializeDataRead {
	public object Clone() {
		var instance = Activator.CreateInstance<Day_Generated>();
		CopyTo(instance);
		return instance;
	}

	public void CopyTo(object target2) {
		var dayGenerated = (Day_Generated)target2;
	...
	public void DataRead(IDataReader reader, Type type) {
		...
		var child5 = reader.GetChild("RayColor");
		...
		var child6 = reader.GetChild("SkyColor");
		...
		if (skyColor is ISerializeDataRead serializeDataRead6)
			...
			Logger.AddError("Type : " + TypeUtility.GetTypeName(skyColor.GetType()) + " is not " +
			                typeof(ISerializeDataRead).Name);
		var child7 = reader.GetChild("SunColor");
		var sunColor = this.sunColor;
		if (sunColor is ISerializeDataRead serializeDataRead7)
			serializeDataRead7.DataRead(child7, typeof(ColorGradient));
		else
			Logger.AddError("Type : " + TypeUtility.GetTypeName(sunColor.GetType()) + " is not " +
			                typeof(ISerializeDataRead).Name);
	}
}