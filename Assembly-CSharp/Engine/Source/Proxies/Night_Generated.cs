...
public class Night_Generated :
	...
	ISerializeDataRead {
	public object Clone() {
		var instance = Activator.CreateInstance<Night_Generated>();
		CopyTo(instance);
		return instance;
	}

	public void CopyTo(object target2) {
		var nightGenerated = (Night_Generated)target2;
	...
	public void DataRead(IDataReader reader, Type type) {
		var child1 = reader.GetChild("AmbientColor");
		...
		if (ambientColor is ISerializeDataRead serializeDataRead1)
			...
			Logger.AddError("Type : " + TypeUtility.GetTypeName(ambientColor.GetType()) + " is not " +
		...
		var child2 = reader.GetChild("CloudColor");
		...
		if (cloudColor is ISerializeDataRead serializeDataRead2)
			...
			Logger.AddError("Type : " + TypeUtility.GetTypeName(cloudColor.GetType()) + " is not " +
			                typeof(ISerializeDataRead).Name);
		...
		var child3 = reader.GetChild("FogColor");
		...
		if (fogColor is ISerializeDataRead serializeDataRead3)
			...
			Logger.AddError("Type : " + TypeUtility.GetTypeName(fogColor.GetType()) + " is not " +
			                typeof(ISerializeDataRead).Name);
		fogginess = DefaultDataReadUtility.Read(reader, "Fogginess", fogginess);
		var child4 = reader.GetChild("LightColor");
		...
		if (lightColor is ISerializeDataRead serializeDataRead4)
			...
			Logger.AddError("Type : " + TypeUtility.GetTypeName(lightColor.GetType()) + " is not " +
			                typeof(ISerializeDataRead).Name);
		...
		var child5 = reader.GetChild("MoonColor");
		var moonColor = this.moonColor;
		if (moonColor is ISerializeDataRead serializeDataRead5)
			serializeDataRead5.DataRead(child5, typeof(ColorGradient));
		else
			Logger.AddError("Type : " + TypeUtility.GetTypeName(moonColor.GetType()) + " is not " +
			                typeof(ISerializeDataRead).Name);
		var child6 = reader.GetChild("RayColor");
		var rayColor = this.rayColor;
		if (rayColor is ISerializeDataRead serializeDataRead6)
			serializeDataRead6.DataRead(child6, typeof(ColorGradient));
		else
			Logger.AddError("Type : " + TypeUtility.GetTypeName(rayColor.GetType()) + " is not " +
			                typeof(ISerializeDataRead).Name);
		...
		var child7 = reader.GetChild("SkyColor");
		var skyColor = this.skyColor;
		if (skyColor is ISerializeDataRead serializeDataRead7)
			serializeDataRead7.DataRead(child7, typeof(ColorGradient));
		else
			Logger.AddError("Type : " + TypeUtility.GetTypeName(skyColor.GetType()) + " is not " +
			                typeof(ISerializeDataRead).Name);
	}
}