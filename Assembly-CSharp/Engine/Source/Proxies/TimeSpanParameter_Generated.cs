...
public class TimeSpanParameter_Generated :
	...
	public void ComputeNeedSave(object target2) {
		NeedSave = true;
		var parameterGenerated = (TimeSpanParameter_Generated)target2;
		if (parameterGenerated.name != name || parameterGenerated.value != value ||
		    parameterGenerated.minValue != minValue || parameterGenerated.maxValue != maxValue)
			return;
		NeedSave = false;
	}

	public object Clone() {
		var instance = Activator.CreateInstance<TimeSpanParameter_Generated>();
		CopyTo(instance);
		return instance;
	}
