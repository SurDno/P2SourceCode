...
public class ItemSelector : MonoBehaviour, IChangeParameterListener {
	private static List<IStorableComponent> searchBuffer = new();
	[SerializeField] private SwitchingItemView2 view;
	[SerializeField] private EntityView itemEntityView;
	[SerializeField] public Button previousButton;
	[SerializeField] public Button nextButton;
	[SerializeField] private List<StorableGroup> groups = new();
	[SerializeField] private bool avoidNull;

	[SerializeField] [FormerlySerializedAs("filterDurability")]
	private bool sortByDurability;

	[SerializeField] private bool ignoreBroken;
	...
	public void SetSelection(bool selection) {
		selectedImage?.gameObject.SetActive(selection);
	}

	public void CheckButtonsForConsole() {
		UpdateButtons();
	}
	...
	public IStorableComponent Item {
		...
		set {
			...
			if (storable != null && (sortByDurability || ignoreBroken))
				storable.GetComponent<ParametersComponent>()?.GetByName<float>(ParameterNameEnum.Durability)
					?.RemoveListener(this);
			if (value != null && (sortByDurability || ignoreBroken))
				value.GetComponent<ParametersComponent>()?.GetByName<float>(ParameterNameEnum.Durability)
					?.AddListener(this);
			var changeItemEvent = ChangeItemEvent;
	...
	private void FillSearchBuffer() {
		...
		foreach (var storableComponent in Storage.Items) {
			var flag = false;
			foreach (var group1 in storableComponent.Groups) {
			...
			if (flag && (!ignoreBroken || GetDurability(storableComponent) > 0.0) &&
			    (ValidateItemEvent == null || ValidateItemEvent(this, storableComponent)))
	...
	public void OnParameterChanged(IParameter parameter) {
		Debug.Log(ObjectInfoUtility.GetStream().Append("Item Selector : Durability changed to ")
			.Append(((IParameter<float>)parameter).Value));
