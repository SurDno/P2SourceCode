...
public class NPCWeaponService : WeaponServiceBase, IEntityAttachable {
	...
	[Inspected] public bool IsIndoor { get; private set; }
	...
	public bool IsChangingWeapon() {
		return weaponControllers[weapon].IsChangingWeapon();
	}
	...
	private void Init() {
		...
		if (!(bool)(Object)pivot)
			...
			foreach (var weaponController1 in weaponControllers) {
				var weaponController2 = weaponController1.Value;
				var weaponKind = weaponController1.Key;
	...
	private void Update() {
		...
		if (KnifeParent != null) {
			...
			var vector3_1 = KnifeParent.position - knifeParentPositionPrev;
			var vector3_2 = vector3_1.magnitude > Time.deltaTime * 0.05000000074505806
				? vector3_1 / Time.deltaTime
				: Vector3.zero;
			var num = 1f;
	...
	public void ForceUpdateLayers() {
		CountLayersWeights(true);
	}

	private void CountLayersWeights(bool immediate = false) {
		foreach (var neededLayerWeight in neededLayerWeights)
			if (neededLayerWeight.Key != -1) {
				var currentLayerWeight = currentLayerWeights[neededLayerWeight.Key];
				var num = Mathf.MoveTowards(currentLayerWeight, neededLayerWeight.Value, Time.deltaTime / 0.5f);
				if (immediate)
					num = neededLayerWeight.Value;
				if (!Mathf.Approximately(num, currentLayerWeight)) {
					currentLayerWeights[neededLayerWeight.Key] = num;
					var weight = SmoothUtility.Smooth22(num);
