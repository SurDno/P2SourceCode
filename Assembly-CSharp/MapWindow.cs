...
public class MapWindow : UIWindow, IMapWindow, IWindow, IPauseMenu {
	...
	[SerializeField] private RectTransform regionAnchorRect;
	[SerializeField] private RectTransform shadowsAnchorRect;
	[SerializeField] private RectTransform baseAnchorRect;
	[SerializeField] private RectTransform normalsAnchorRect;
	[SerializeField] private RectTransform overlayAnchorRect;
	[SerializeField] private RectTransform playerAnchorRect;
	[SerializeField] private RectTransform referenceRect;
	[SerializeField] private GameObject imagePrefab;
	[SerializeField] private GameObject overlayImagePrefab;
	[SerializeField] private GameObject mindMapImagePrefab;
	[SerializeField] private AudioClip windowOpenSound;
	[SerializeField] private AudioMixerGroup mixer;
	[Space] [SerializeField] private Vector2 worldLeftBottom;
	[SerializeField] private Vector2 worldRightTop;
	[Space] [SerializeField] private float regionResolution = 1f;
	[SerializeField] private float mainResolution = 4f;
	[SerializeField] private float normalResolution = 4f;
	[SerializeField] private float shadowResolution = 2f;
	[SerializeField] private float nodeSize = 32f;
	[Space] [SerializeField] private Material regionMaterial;
	[SerializeField] private Material buildingMaterial;
	[SerializeField] private Material normalMaterial;
	[SerializeField] private Material shadowMaterial;
	[SerializeField] private Material mindMapNodeMaterial;
	[Space] [SerializeField] private Color undiscoveredBuildingColor = Color.white;
	[SerializeField] private Color buildingColor = Color.white;
	[SerializeField] private Color shopBuildingColor = Color.white;
	[SerializeField] private Color specialBuildingColor = Color.white;
	[SerializeField] private Color npcAtRiskColor = Color.white;
	[SerializeField] private Color npcDiseasedColor = Color.white;
	[SerializeField] private Color npcDeadColor = Color.white;
	[SerializeField] private Color selectionColor = Color.white;
	[SerializeField] private RegionColor[] regionColors;
	[SerializeField] private Color regionSelectionFarColor = Color.white;
	[SerializeField] private Color regionSelectionNearColor = Color.white;
	[Space] [SerializeField] private MapItemInfoWindow infoWindow;
	[SerializeField] private MapRegionInfoWindow regionInfoWindow;
	[Space] [SerializeField] private ProgressView bull;
	[SerializeField] private Image vignette;
	[SerializeField] private Vector2 vignetteOpacityRange = new(0.0f, 1f);

	[Space] [SerializeField] [Range(0.0f, 1f)]
	private float onPlayerScalePower = 0.5f;

	[SerializeField] [Range(0.0f, 1f)] private float onNodeScalePower = 1f;
	[SerializeField] private MapNodeInfoView infoView;
	[Space] [SerializeField] private MapFastTravelPointInfoView fastTravelInfoView;
	[SerializeField] private Texture fastTravelIcon;
	[Space] [SerializeField] private HideableView hasCustomMarkerView;
	[SerializeField] private HideableView customMarkerEnabledView;
	private MapService mapService;
	[Inspected] private float desiredScroll;
	private float scrollVelocity;
	[Inspected] private float scroll;
	[Inspected] private List<MapItemView> regionObjects = new();
	[Inspected] private List<GameObject> baseObjects = new();
	[Inspected] private List<RectTransform> overlayObjects = new();
	...
	private void ApplyScale() {
		var num1 = Mathf.Max(0.0f, scroll);
		var num2 = Mathf.Pow(4f, num1);
		contentRect.localScale = new Vector3(num2, num2, num2);
		if (overlayObjects != null) {
			var num3 = (float)(1.0 / (num2 * (double)referenceRect.localScale.x));
			var vector3 = new Vector3(num3, num3, num3);
			for (var index = 0; index < overlayObjects.Count; ++index)
				overlayObjects[index].localScale = vector3;
		}

		if (regionObjects != null) {
			var num4 = num1;
			for (var index = 0; index < regionObjects.Count; ++index)
	...
	private void Clear() {
		...
		if (regionObjects != null) {
			...
			regionObjects.Clear();
			for (var index = 0; index < baseObjects.Count; ++index)
	...
	private void ColorByKind(
		...
		if (kind == ImageKind.Region) {
			...
			for (var index = 0; index < regionColors.Length; ++index)
	...
	private void CreateImage(
		...
		CreateImage(rectangle, worldPos, rotation, sprite.rect.size * resolution, kind, sprite, out instance,
			out rectTransform, out image, out normalizedPoint);
		if (mapService.FastTravelOrigin == null && mapService.FocusedItem == null && mapService.FocusedNode == null &&
		...
		if (item != null) {
			...
			if (kind == ImageKind.Region)
				mapItemView.Initialize(this, item, item.Resource.HoverSprite, baseColor, regionSelectionFarColor,
					activeColor, regionSelectionNearColor);
			else
	...
	private void CreateMindMapNodeImage(
		...
		float size,
		MMTooltip tooltip) {
		var placeholder = tooltip.node?.Content?.Placeholder as MMPlaceholder;
		var tooltipResource = tooltip.tooltipResource as MapTooltipResource;
		if (placeholder == null && tooltipResource == null)
			return;
		var kind = ImageKind.Overlay;
		var sprite = placeholder == null ? tooltipResource.Image.Value : placeholder.Image.Value;
		if (sprite == null)
			Debug.LogError("Sprite not found , holder : " + placeholder.GetInfo() + " , tooltip : " +
			               tooltipResource.GetInfo());
		else {
			var size1 = new Vector2(size, size);
			GameObject instance;
			RectTransform rectTransform;
			RawImage image;
			Vector2 normalizedPoint;
			CreateMindMapImage(rectangle, worldPos, -referenceRect.localEulerAngles.z, size1, kind, sprite,
				out instance, out rectTransform, out image, out normalizedPoint);
			...
			Color baseColor;
			ColorByKind(kind, out baseColor, out var _);
			image.color = baseColor;
			if (tooltip.node != null || tooltip.tooltipText != LocalizedText.Empty) {
				image.raycastTarget = true;
				var mapNodeView = instance.AddComponent<MapNodeView>();
	...
	private void CreateFastTravelImage(Rect rectangle, Vector2 worldPos, float size, IMapItem item) {
		...
		RectTransform rectTransform;
		RawImage image;
		CreateMindMapImage(rectangle, worldPos, -referenceRect.localEulerAngles.z, size1, kind, fastTravelIcon,
			out instance, out rectTransform, out image, out var _);
		image.material = mindMapNodeMaterial;
		Color baseColor;
		ColorByKind(kind, out baseColor, out var _);
		image.color = baseColor;
		var fastTravelPointView = instance.AddComponent<MapFastTravelPointView>();
	...
	public void ShowMapNodeInfo(MapNodeView node, MMTooltip tooltip) {
		...
		if (tooltip.node != null) {
			var text = tooltip.node.Content != null ? tooltip.node.Content.Description : LocalizedText.Empty;
			var mindMapInterface = ServiceLocator.GetService<InterfaceBlockingService>().BlockMindMapInterface;
	...
	public void CallSelectedFastTravelPoint() {
		fastTravelInfoView?.Travel();
	}
	...
	private void CreateMindMapImage(
		...
		normalizedPoint = new Vector2((worldPos.x - rectangle.x) / rectangle.width,
			(worldPos.y - rectangle.y) / rectangle.height);
	...
	private static int ItemComparison(IMapItem x, IMapItem y) {
		return (x.Resource != null ? x.Resource.Kind : MapPlaceholderKind.Region).CompareTo(
			(MapPlaceholderKind)(y.Resource != null ? (int)y.Resource.Kind : 0));
	}
	...
	private void Build2() {
		...
		startingScroll = 0.0f;
		var num1 = baseAnchorRect.sizeDelta.x / rect2.width;
		var num2 = Vector2.Angle(rect2.min - rect2.center, rectangle.min - rectangle.center);
		var source = new List<MMTooltip>();
		mapService = ServiceLocator.GetService<MapService>();
		var size = mapService.FastTravelOrigin != null ? nodeSize * 0.5f : nodeSize;
		foreach (var mapItem1 in mapService.Items) {
			var resource = mapItem1.Resource;
			var worldPosition = mapItem1.WorldPosition;
			if (resource != null) {
				if (resource.Kind == MapPlaceholderKind.Building || resource.Kind == MapPlaceholderKind.ShopBuilding ||
				    resource.Kind == MapPlaceholderKind.SpecialBuilding) {
					var shadowSprite = resource.ShadowSprite;
					if (shadowSprite != null)
						CreateImage(rectangle, worldPosition, num2 - mapItem1.Rotation, ImageKind.Shadow, shadowSprite,
							num1 / shadowResolution, false, null);
				}

				var boundHealthStateEnum = BoundHealthStateEnum.Normal;
				var component = mapItem1.BoundCharacter?.GetComponent<BoundCharacterComponent>();
				if (component != null)
				...
				if (resource.Kind != MapPlaceholderKind.Region && mapItem1.Discovered) {
					...
					if (kind != 0) {
						var hoverSprite = resource.HoverSprite;
						if (hoverSprite != null)
							CreateImage(rectangle, worldPosition, num2 - mapItem1.Rotation, kind, hoverSprite,
								num1 / shadowResolution, false, null);
					}
				}

				var mainSprite = resource.MainSprite;
				if (mainSprite != null) {
					var resolution = 1f;
					...
					var kind = ImageKind.Overlay;
					var mapItem2 = resource.Kind == MapPlaceholderKind.Region || mapItem1.Title != LocalizedText.Empty
						? mapItem1
						: null;
					switch (resource.Kind) {
						case MapPlaceholderKind.Region:
						...
						case MapPlaceholderKind.ShopBuilding:
							...
							break;
						case MapPlaceholderKind.SpecialBuilding:
							kind = mapItem1.Discovered
								? boundHealthStateEnum == BoundHealthStateEnum.Dead
									? ImageKind.SpecialBuildingDead
									: ImageKind.SpecialBuilding
								: ImageKind.UndiscoveredBuilding;
							break;
					...
					var flag = resource.Kind == MapPlaceholderKind.Marker;
					CreateImage(rectangle, worldPosition, flag ? 0.0f : num2 - mapItem1.Rotation, kind, mainSprite,
						resolution, resource.AlphaRaycast, mapItem2);
				}

				if (resource.Kind == MapPlaceholderKind.Building || resource.Kind == MapPlaceholderKind.ShopBuilding ||
				    resource.Kind == MapPlaceholderKind.SpecialBuilding) {
					var normalSprite = resource.NormalSprite;
					if (normalSprite != null)
						CreateImage(rectangle, worldPosition, num2 - mapItem1.Rotation, ImageKind.Normal, normalSprite,
							num1 / normalResolution, false, null);
				}
			}

			if (mapItem1.TooltipResource != null)
				source.Add(new MMTooltip(mapItem1.TooltipResource, mapItem1.TooltipText));
			foreach (var node in mapItem1.Nodes)
				source.Add(new MMTooltip(node));
			var num3 = source.Count();
			if (num3 == 1) {
				var tooltip = source.First();
				CreateMindMapNodeImage(rectangle, worldPosition, size, tooltip);
			} else if (num3 > 1) {
				var num4 = (float)(ExternalSettingsInstance<ExternalGraphicsSettings>.Instance.MindMapIconSize * num3 /
					2.0 * 3.1415927410125732);
				var num5 = 6.28318548f / num3;
				var num6 = 0;
				foreach (var tooltip in source) {
					var vector2 = new Vector2(Mathf.Cos(num5 * num6) * num4, Mathf.Sin(num5 * num6) * num4);
			...
			source.Clear();
			var fastTravelPointEnum = mapService.FastTravelOrigin != null
				? mapItem1.FastTravelPoint.Value
				: FastTravelPointEnum.None;
			if (fastTravelPointEnum != FastTravelPointEnum.None &&
			    fastTravelPointEnum != mapService.FastTravelOrigin.FastTravelPointIndex.Value)
				CreateFastTravelImage(rectangle, worldPosition, nodeSize, mapItem1);
			if (mapService.FocusedItem == mapItem1) {
				var normalized = Rect.PointToNormalized(rectangle, worldPosition);
				var sizeDelta = baseAnchorRect.sizeDelta;
		...
		var component1 = mapService.CustomMarker?.GetComponent<IMapItemComponent>();
	...
	protected override void OnEnable() {
		...
		var num = Mathf.Pow(4f, Mathf.Max(0.0f, startingScroll));
		var vector2_1 = (contentRect.sizeDelta * num - contentRect.sizeDelta) * 0.5f;
		if (vector2_1.x < 0.0)
			vector2_1.x = 0.0f;
		if (vector2_1.y < 0.0)
			vector2_1.y = 0.0f;
		cursorMapPosition = startingPosition;
		var vector2_2 = -num * startingPosition;
		...
		var mapNodeViewList = new List<MapNodeView>(contentRect.GetComponentsInChildren<MapNodeView>());
	...
	protected override void OnDisable() {
		...
		ServiceLocator.GetService<GameActionService>()
			.RemoveListener(GameActionType.Map, WithoutJoystickCancelListener);
	...
	private void NavigateNode(Vector3 dir) {
		...
		var collection = new List<GameObject>(contentRect.GetComponentsInChildren<MapFastTravelPointView>()
			.Select(node => node.gameObject));
		if (collection.Count > 0)
			nodes.AddRange(collection);
		if (customMarkerEnabledView.Visible)
			nodes.Add(customMarker);
		var gameObject = UISelectableHelper.Select(nodes, consoleCursor, dir);
		if (gameObject == null)
			return;
	...
	private void SelectNode() {
		...
		if (bull != null && bull.Progress != 0.0) {
			...
			foreach (var raycastResult in raycastResultList) {
	...
	private void UpdateNavigation() {
		...
		if (num1 != 0.0 || num2 != 0.0) {
			...
			var num3 = num1 * (ExternalSettingsInstance<ExternalInputSettings>.Instance.JoystickSensitivity / scale);
			var num4 = num2 * (ExternalSettingsInstance<ExternalInputSettings>.Instance.JoystickSensitivity / scale);
			cursorMapPosition.x += num3;
			cursorMapPosition.y -= num4;
			cursorMapPosition.x = Mathf.Clamp(cursorMapPosition.x, (float)(-(double)contentRect.sizeDelta.x * 0.5),
				contentRect.sizeDelta.x * 0.5f);
			cursorMapPosition.y = Mathf.Clamp(cursorMapPosition.y, (float)(-(double)contentRect.sizeDelta.y * 0.5),
				contentRect.sizeDelta.y * 0.5f);
		} else
			currentAcceleration = 0.0f;

		var transform = (RectTransform)consoleCursor.transform;
		if (transform.anchoredPosition.sqrMagnitude > 1.0) {
			var vector2_1 = contentRect.anchoredPosition - transform.anchoredPosition;
			var vector2_2 = new Vector2((float)(contentRect.sizeDelta.x * (scale - 1.0) * 0.5),
				(float)(contentRect.sizeDelta.y * (scale - 1.0) * 0.5));
			if (vector2_2.x < 0.0)
				vector2_2.x = 0.0f;
			if (vector2_2.y < 0.0)
				vector2_2.y = 0.0f;
			...
			if (vector2_1.y > (double)vector2_2.y)
				vector2_1.y = vector2_2.y;
			contentRect.anchoredPosition += (vector2_1 - contentRect.anchoredPosition) *
			                                Mathf.MoveTowards(0.0f, 1f, 5f * Time.deltaTime);
		}
	}
	...
	protected override void OnJoystick(bool joystick) {
		...
		if (joystick) {
			...
			ServiceLocator.GetService<GameActionService>()
				.AddListener(GameActionType.Map, WithoutJoystickCancelListener);
	}
	...
	private void Update() {
		...
		if (num1 < 0.0)
			desiredScroll = Scroll > 0.0 || !ServiceLocator.GetService<MapService>().BullModeAvailable
				? Mathf.Clamp01(desiredScroll + num1)
				: -1f;
		else if (num1 > 0.0)
			desiredScroll = desiredScroll >= 0.0 ? Mathf.Clamp01(desiredScroll + num1) : 0.0f;
		if (Scroll != (double)desiredScroll) {
			var scale1 = ScrollToScale(Scroll);
			Scroll = Mathf.MoveTowards(Mathf.SmoothDamp(Scroll, desiredScroll, ref scrollVelocity, 0.15f),
				desiredScroll, Time.deltaTime * 0.01f);
			var scale2 = ScrollToScale(Scroll);
			if (InputService.Instance.JoystickUsed
				    ? !InputService.Instance.GetButton("A", false)
				    : !Input.GetMouseButton(0)) {
				var num2 = scale2 / scale1;
				var anchoredPosition = contentRect.anchoredPosition;
				Vector2 vector2_1 = contentRect.parent.InverseTransformPoint(InputService.Instance.JoystickUsed
					? consoleCursor.transform.position
					: Input.mousePosition);
				var vector2_2 = (anchoredPosition - vector2_1) * num2;
				contentRect.anchoredPosition = vector2_1 + vector2_2;
			}

			if (!InputService.Instance.JoystickUsed) {
				var anchoredPosition = contentRect.anchoredPosition;
				var vector2 = new Vector2((float)((1920.0 * scale2 - 1920.0) * 0.5),
					(float)((1080.0 * scale2 - 886.0) * 0.5));
				if (vector2.x < 0.0)
					vector2.x = 0.0f;
				if (vector2.y < 0.0)
					vector2.y = 0.0f;
	...
	public Vector2 GetScreenPosition(Vector3 worldPos) {
		...
		var sizeDelta = baseAnchorRect.sizeDelta;
		var normalized = Rect.PointToNormalized(new Rect {
			min = worldLeftBottom,
			max = worldRightTop
		}, point);
		normalized.x *= sizeDelta.x;
	...
	private static float ScrollToScale(float scroll) {
		return Mathf.Pow(4f, Mathf.Clamp01(scroll));
	}
	...
	private void CreateCustomMarker() {
		var customMarker = ServiceLocator.GetService<MapService>()?.CustomMarker;
		if (customMarker == null)
			return;
		var component1 = customMarker.GetComponent<MapCustomMarkerComponent>();
		if (component1 != null)
			component1.Position = !InputService.Instance.JoystickUsed
				? GetWorldPosition(Input.mousePosition)
				: (Vector2)GetWorldPosition(consoleCursor.transform.position);
		var component2 = customMarker.GetComponent<IMapItemComponent>();
		if (component2 != null)
	...
	private void RemoveCustomMarker() {
		var component = ServiceLocator.GetService<MapService>()?.CustomMarker?.GetComponent<IMapItemComponent>();
		if (component == null)
			return;
