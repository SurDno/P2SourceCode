...
public class BreakableToolView : ItemView {
	[SerializeField] private Image storableImage;
	[SerializeField] private RectTransform fillImage;
	[SerializeField] private GameObject brokenImage;
	...
	public override StorableComponent Storable {
		...
		set {
			if (storable == value)
				return;
			storable = value;
			var sprite = storable?.Placeholder?.ImageInventorySlot.Value;
			if (sprite == null)
	...
	public static float GetDurability(IStorableComponent storable) {
		if (storable == null)
			return float.NaN;
		var component = storable.GetComponent<ParametersComponent>();
		if (component == null)
			return float.NaN;
