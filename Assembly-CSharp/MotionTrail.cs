...
public class MotionTrail : MonoBehaviour {
	...
	[SerializeField] [Range(0.0f, 1f)] [Tooltip("Approximate opacity of the previous frame at 30 fps")]
	...
	private bool FramePropertiesChanged(RenderTexture source) {
		return historyBuffer.width != source.width || historyBuffer.height != source.height ||
		       historyBuffer.format != source.format;
	}
	...
	private void CreateHistoryBuffer(RenderTexture source) {
		historyBuffer = new RenderTexture(source.width, source.height, 0, source.format,
