...
public class LodService : IInitialisable, IUpdatable, ISavesController {
	[Inspected] private List<Slot> registeredLods = new();
	[Inspected] private bool unloaded;

	[Inspected] private int RegisteredCount => registeredLods.Count();

	[Inspected]
	private int LodEnabledCount {
		get { return registeredLods.Count(e => e.Enabled); }
	}
	...
	void IUpdatable.ComputeUpdate() {
		...
		var forward = GameCamera.Instance.CameraTransform.forward;
		var angle = LodAngle(GameCamera.Instance.Camera.fieldOfView, GameCamera.Instance.Camera.aspect);
		var aiLodDistance = InstanceByRequest<GraphicsGameSettings>.Instance.AILodDistance;
		var lodDistance = aiLodDistance.Value;
		var minValue = aiLodDistance.MinValue;
		foreach (var registeredLod in registeredLods)
			if (!(registeredLod.npcState == null) &&
			    UpdaterLodState(registeredLod, lodDistance, minValue, angle, position, forward))
	...
	public void RegisterLod(NpcState npcState) {
		...
		var aiLodDistance = InstanceByRequest<GraphicsGameSettings>.Instance.AILodDistance;
		UpdaterLodState(registeredLods.Last(), aiLodDistance.Value, aiLodDistance.MinValue,
			LodAngle(GameCamera.Instance.Camera.fieldOfView, GameCamera.Instance.Camera.aspect),
			GameCamera.Instance.CameraTransform.position, GameCamera.Instance.CameraTransform.forward);
	...
	public void UnregisterLod(NpcState npcState) {
		if (unloaded)
			return;
		for (var index = 0; index < registeredLods.Count; ++index)
