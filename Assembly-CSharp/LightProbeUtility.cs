...
public static class LightProbeUtility {
	...
	public static void SetSHCoefficients(
		...
		for (var rgb = 0; rgb < 3; ++rgb)
			properties.SetVector(_idSHA[rgb],
				new Vector4(probe[rgb, 3], probe[rgb, 1], probe[rgb, 2], probe[rgb, 0] - probe[rgb, 6]));
		for (var rgb = 0; rgb < 3; ++rgb)
			properties.SetVector(_idSHB[rgb],
				new Vector4(probe[rgb, 4], probe[rgb, 6], probe[rgb, 5] * 3f, probe[rgb, 7]));
	...
	public static void SetSHCoefficients(Vector3 position, Material material, Renderer renderer = null) {
		...
		LightProbes.GetInterpolatedProbe(position, renderer, out probe);
		for (var rgb = 0; rgb < 3; ++rgb)
			material.SetVector(_idSHA[rgb],
				new Vector4(probe[rgb, 3], probe[rgb, 1], probe[rgb, 2], probe[rgb, 0] - probe[rgb, 6]));
		for (var rgb = 0; rgb < 3; ++rgb)
			material.SetVector(_idSHB[rgb],
				new Vector4(probe[rgb, 4], probe[rgb, 6], probe[rgb, 5] * 3f, probe[rgb, 7]));
