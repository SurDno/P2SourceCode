...
public class NpcStateFightFollowTarget : INpcState {
	...
	public void Update() {
		...
		if (InstanceByRequest<EngineApplication>.Instance.IsPaused)
			...
			var vector3 = target.position - enemy.transform.position;
			...
			if (enemy.IsAttacking || enemy.IsContrReacting) {
				...
				var num = 0.0f;
				if (NavMesh.Raycast(enemy.transform.position, target.position, out var _, -1)) {
					if (!agent.hasPath) {
						Status = NpcStateStatusEnum.Running;
						return;
					}
					...
					if (agent.remainingDistance > (double)stopDistance) {
						num = agent.remainingDistance > (double)runDistance ? 2f : 1f;
					...
					num = magnitude <= (double)stopDistance ? magnitude >= (double)retreatDistance ? 0.0f : -1f :
						agent.remainingDistance > (double)runDistance ? 2f : 1f;
	...
	private void UpdatePath() {
		if (Random.value < Time.deltaTime / 0.5 && NavMeshUtility.IsBrokenPath(agent)) {
			...
			if ((lastPlayerPosition - target.position).magnitude <= 0.33000001311302185)
				return;
			if (!agent.isOnNavMesh)
