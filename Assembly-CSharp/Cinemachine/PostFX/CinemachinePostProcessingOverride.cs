...
public class CinemachinePostProcessingOverride : MonoBehaviour {
	[SerializeField]
	[Tooltip(
		"If checked, then the Focus Distance will be set to the distance between the camera and the LookAt target.")]
	private bool focusTracksTarget;

	[SerializeField] [Tooltip("Offset from target distance, to be used with Focus Tracks Target.")]
	...
	internal void PostFXHandler(CinemachineBrain brain) {
		if (!enabled || !(this.brain != null) || !(postProcessingOverride != null))
			return;
		var processingOverride = GetEffectivePostFX(brain.ActiveVirtualCamera);
		...
		var currentCameraState = brain.CurrentCameraState;
		if (currentCameraState.HasLookAt && processingOverride.focusTracksTarget) {
			var depthOfFieldOverride = postProcessingOverride.DepthOfField;
			...
			if (depthOfFieldOverride != null) {
				var num = (currentCameraState.FinalPosition - currentCameraState.ReferenceLookAt).magnitude +
				          focusOffset;
	...
	private CinemachinePostProcessingOverride GetEffectivePostFX(ICinemachineCamera vcam) {
		...
		for (effectivePostFx = null; vcam != null && effectivePostFx == null; vcam = vcam.ParentCamera) {
			var virtualCameraBase = vcam as CinemachineVirtualCameraBase;
	...
	private static void StaticPostFXHandler(CinemachineBrain brain) {
		var processingComponent = brain.PostProcessingComponent as CinemachinePostProcessingOverride;
