...
public class CinemachineMixingCamera : CinemachineVirtualCameraBase {
	...
	public override bool IsLiveChild(ICinemachineCamera vcam) {
		var childCameras = ChildCameras;
		for (var index = 0; index < 8 && index < childCameras.Length; ++index)
	...
	protected void ValidateListOfChildren() {
		...
		foreach (var componentsInChild in GetComponentsInChildren<CinemachineVirtualCameraBase>(true))
			if (componentsInChild.transform.parent == transform) {
				var count = virtualCameraBaseList.Count;
	...
	public override void UpdateCameraState(Vector3 worldUp, float deltaTime) {
		...
		var num2 = 0.0f;
		for (var index = 0; index < 8 && index < childCameras.Length; ++index) {
			var virtualCameraBase = childCameras[index];
			if (virtualCameraBase.isActiveAndEnabled) {
				var num3 = Mathf.Max(0.0f, GetWeight(index));
				if (num3 > 9.9999997473787516E-05) {
					num2 += num3;
					m_State = num2 != (double)num3
						? CameraState.Lerp(m_State, virtualCameraBase.State, num3 / num2)
						: virtualCameraBase.State;
					if (num3 > (double)num1) {
						num1 = num3;
