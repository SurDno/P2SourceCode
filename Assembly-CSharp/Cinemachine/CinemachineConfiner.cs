...
public class CinemachineConfiner : CinemachineExtension {
	...
	private void OnValidate() {
		m_Damping = Mathf.Max(0.0f, m_Damping);
	}
	...
	protected override void PostPipelineStageCallback(
		...
		if (!IsValid || stage != CinemachineCore.Stage.Body)
			return;
		var vector3_1 = !m_ConfineScreenEdges || !state.Lens.Orthographic
			? ConfinePoint(state.CorrectedPosition)
			: ConfineScreenEdges(vcam, ref state);
		var extraState = GetExtraState<VcamExtraState>(vcam);
		if (m_Damping > 0.0 && deltaTime >= 0.0) {
			var vector3_2 = Damper.Damp(vector3_1 - extraState.m_previousDisplacement, m_Damping, deltaTime);
	...
	private Vector3 ConfinePoint(Vector3 camPos) {
		...
		for (var index = 0; index < m_pathCache.Count; ++index)
	...
	private Vector3 ConfineScreenEdges(CinemachineVirtualCameraBase vcam, ref CameraState state) {
		var quaternion = Quaternion.Inverse(state.CorrectedOrientation);
		var orthographicSize = state.Lens.OrthographicSize;
		var num = orthographicSize * state.Lens.Aspect;
		var vector3_1 = quaternion * Vector3.right * num;
		var vector3_2 = quaternion * Vector3.up * orthographicSize;
		var zero = Vector3.zero;
		var correctedPosition = state.CorrectedPosition;
		for (var index = 0; index < 12; ++index) {
			var v = ConfinePoint(correctedPosition - vector3_2 - vector3_1);
			if (v.AlmostZero())
