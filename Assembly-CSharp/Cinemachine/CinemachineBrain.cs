...
public class CinemachineBrain : MonoBehaviour {
	...
	[Tooltip(
		"If set, this object's Y axis will define the worldspace Up vector for all the virtual cameras.  This is useful for instance in top-down game environments.  If not set, Up is worldspace Y.  Setting this appropriately is important, because Virtual Cameras don't like looking straight up or straight down.")]
	public Transform m_WorldUpOverride;

	[Tooltip(
		"Use FixedUpdate if all your targets are animated during FixedUpdate (e.g. RigidBodies), LateUpdate if all your targets are animated during the normal Update loop, and SmartUpdate if you want Cinemachine to do the appropriate thing on a per-target basis.  SmartUpdate is the recommended setting")]
	public UpdateMethod m_UpdateMethod = UpdateMethod.SmartUpdate;

	[CinemachineBlendDefinitionProperty]
	[Tooltip(
		"The blend that is used in cases where you haven't explicitly defined a blend between two Virtual Cameras")]
	...
	private List<OverrideStackFrame> mOverrideStack = new();
	...
	private OverrideStackFrame GetOverrideFrame(int id) {
		var count = mOverrideStack.Count;
		for (var index = 0; index < count; ++index)
			if (mOverrideStack[index].id == id)
				return mOverrideStack[index];
		var overrideFrame = new OverrideStackFrame();
		overrideFrame.id = id;
	...
	internal int SetCameraOverride(
		...
		var overrideFrame = GetOverrideFrame(overrideId);
		...
		if (camA != null || camB != null) {
			if (weightB <= 9.9999997473787516E-05) {
				...
				if (camA == null) {
					var nextActiveFrame = GetNextActiveFrame(overrideId);
					camA = nextActiveFrame.blend == null
						? nextActiveFrame.camera != null ? nextActiveFrame.camera : camB
						: new BlendSourceVirtualCamera(nextActiveFrame.blend, deltaTime);
				}

				if (overrideFrame.blend == null)
					overrideFrame.blend = new CinemachineBlend(camA, camB, AnimationCurve.Linear(0.0f, 0.0f, 1f, 1f),
						1f, weightB);
	...
	private void LateUpdate() {
		var effectiveDeltaTime = GetEffectiveDeltaTime(false);
		if (m_UpdateMethod == UpdateMethod.SmartUpdate)
	...
	private float GetEffectiveDeltaTime(bool fixedDelta) {
		...
		var activeOverride = GetActiveOverride();
		if (activeOverride != null)
			return activeOverride.Expired ? -1f : activeOverride.deltaTime;
		if (!Application.isPlaying)
			return -1f;
		return m_IgnoreTimeScale ? fixedDelta ? Time.fixedDeltaTime : Time.unscaledDeltaTime :
			fixedDelta ? Time.fixedDeltaTime * Time.timeScale : Time.deltaTime;
	}
	...
	private void UpdateVirtualCameras(CinemachineCore.UpdateFilter updateFilter, float deltaTime) {
		...
		var activeVirtualCamera = ActiveVirtualCamera;
	...
	private void ProcessActiveCamera(float deltaTime) {
		if (!isActiveAndEnabled) {
			...
			if (activeVirtualCamera == null)
				...
				if (activeOverride != null)
					mActiveBlend = null;
				var cinemachineBlend = ActiveBlend;
				...
				if (mActiveCameraPreviousFrame != activeVirtualCamera) {
					if (mActiveCameraPreviousFrame != null && !mPreviousFrameWasOverride && activeOverride == null &&
					    deltaTime >= 0.0) {
						var duration = 0.0f;
						var blendCurve =
							LookupBlendCurve(mActiveCameraPreviousFrame, activeVirtualCamera, out duration);
						cinemachineBlend = CreateBlend(mActiveCameraPreviousFrame, activeVirtualCamera, blendCurve,
							duration, mActiveBlend);
					}

					if (activeVirtualCamera != mOutgoingCameraPreviousFrame) {
						activeVirtualCamera.OnTransitionFromCamera(mActiveCameraPreviousFrame, DefaultWorldUp,
							deltaTime);
						if (!activeVirtualCamera.VirtualCameraGameObject.activeInHierarchy &&
						    (cinemachineBlend == null || !cinemachineBlend.Uses(activeVirtualCamera)))
					...
					if ((cinemachineBlend == null || (cinemachineBlend.CamA != mActiveCameraPreviousFrame &&
					                                  cinemachineBlend.CamB != mActiveCameraPreviousFrame &&
					                                  cinemachineBlend.CamA != mOutgoingCameraPreviousFrame &&
					                                  cinemachineBlend.CamB != mOutgoingCameraPreviousFrame)) &&
				...
				var state = activeVirtualCamera.State;
				if (cinemachineBlend != null)
			...
			var cameraProcessedEvent = CameraProcessedEvent;
	...
	public bool IsLive(ICinemachineCamera vcam) {
		if (IsLiveItself(vcam))
			return true;
		for (var parentCamera = vcam.ParentCamera;
		     parentCamera != null && parentCamera.IsLiveChild(vcam);
	...
	private bool IsLiveItself(ICinemachineCamera vcam) {
		return mActiveCameraPreviousFrame == vcam || ActiveVirtualCamera == vcam ||
		       (IsBlending && ActiveBlend.Uses(vcam));
	}
	...
	public ICinemachineCamera ActiveVirtualCamera {
		get {
			if (SoloCamera != null)
				return SoloCamera;
			var activeOverride = GetActiveOverride();
			return activeOverride == null || activeOverride.camera == null
				? TopCameraFromPriorityQueue()
				: activeOverride.camera;
		}
	}
	...
	private ICinemachineCamera TopCameraFromPriorityQueue() {
		var outputCamera = OutputCamera;
		var num = outputCamera == null ? -1 : outputCamera.cullingMask;
		var virtualCameraCount = CinemachineCore.Instance.VirtualCameraCount;
		for (var index = 0; index < virtualCameraCount; ++index) {
			var virtualCamera = CinemachineCore.Instance.GetVirtualCamera(index);
			var cameraGameObject = virtualCamera?.VirtualCameraGameObject;
	...
	private class OverrideStackFrame {
		...
		public bool Expired => !Application.isPlaying &&
		                       Time.realtimeSinceStartup - (double)timeOfOverride > Time.maximumDeltaTime;
	}
}