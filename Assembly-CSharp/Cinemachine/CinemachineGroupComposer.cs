  ...
  public class CinemachineGroupComposer : CinemachineComposer
  {
    ...
    public override void MutateCameraState(ref CameraState curState, float deltaTime)
    {
      ...
      else
      {
        ...
        Vector3 v = TrackedPoint - curState.RawPosition;
        ...
        m_lastBoundsMatrix = Matrix4x4.TRS(boundingBox.center - forward * boundingBox.extents.magnitude, Quaternion.LookRotation(forward, curState.ReferenceUp), Vector3.one);
        m_LastBounds = targetGroup.GetViewSpaceBoundingBox(m_lastBoundsMatrix);
        float num1 = GetTargetHeight(m_LastBounds);
        Vector3 vector3 = m_lastBoundsMatrix.MultiplyPoint3x4(m_LastBounds.center);
        if (deltaTime >= 0.0)
    ...
    private float GetTargetHeight(Bounds b)
    {
      ...
      switch (m_FramingMode)
      {
        ...
        default:
          return Mathf.Max(Mathf.Max(0.0001f, b.size.x) / (num * VcamState.Lens.Aspect), Mathf.Max(0.0001f, b.size.y) / num);
      }
    }
