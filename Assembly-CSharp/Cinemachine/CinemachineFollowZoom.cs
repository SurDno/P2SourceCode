...
public class CinemachineFollowZoom : CinemachineExtension {
	...
	[Range(1f, 179f)] [Tooltip("Lower limit for the FOV that this behaviour will generate.")]
	public float m_MinFOV = 3f;

	[Range(1f, 179f)] [Tooltip("Upper limit for the FOV that this behaviour will generate.")]
	...
	protected override void PostPipelineStageCallback(
		...
		if (stage != CinemachineCore.Stage.Body)
			return;
		var num1 = Mathf.Max(m_Width, 0.0f);
		var num2 = 179f;
		var num3 = Vector3.Distance(state.CorrectedPosition, state.ReferenceLookAt);
		if (num3 > 9.9999997473787516E-05) {
			var min = num3 * 2f * Mathf.Tan((float)(m_MinFOV * (Math.PI / 180.0) / 2.0));
			var max = num3 * 2f * Mathf.Tan((float)(m_MaxFOV * (Math.PI / 180.0) / 2.0));
			var num4 = Mathf.Clamp(num1, min, max);
			if (deltaTime >= 0.0 && m_Damping > 0.0) {
				var num5 = num3 * 2f * Mathf.Tan((float)(extraState.m_previousFrameZoom * (Math.PI / 180.0) / 2.0));
				var num6 = Damper.Damp(num4 - num5, m_Damping, deltaTime);
				num4 = num5 + num6;
			}
