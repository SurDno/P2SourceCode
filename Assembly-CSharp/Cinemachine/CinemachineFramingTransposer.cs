...
public class CinemachineFramingTransposer : CinemachineComponentBase {
	...
	[Range(0.0f, 1f)] [Tooltip("Camera will not move vertically if the target is within this range of the position.")]
	...
	[Space] [Tooltip("If checked, then then soft zone will be unlimited in size.")]
	public bool m_UnlimitedSoftZone;

	[Range(0.0f, 2f)]
	[Tooltip(
		"When target is within this region, camera will gradually move horizontally to re-align towards the desired position, depending on the damping speed.")]
	public float m_SoftZoneWidth = 0.8f;

	[Range(0.0f, 2f)]
	[Tooltip(
		"When target is within this region, camera will gradually move vertically to re-align towards the desired position, depending on the damping speed.")]
	...
	public override void MutateCameraState(ref CameraState curState, float deltaTime) {
		...
		var rawOrientation = curState.RawOrientation;
		var quaternion = Quaternion.Inverse(rawOrientation);
		var vector3_1 = quaternion * previousCameraPosition;
		var targetPos2D = quaternion * TrackedPoint - vector3_1;
		var vector3_2 = Vector3.zero;
		var a = Mathf.Max(0.01f, m_CameraDistance - m_DeadZoneDepth / 2f);
		var num1 = Mathf.Max(a, m_CameraDistance + m_DeadZoneDepth / 2f);
		...
		if (targetGroup != null && m_GroupFramingMode != FramingMode.None)
			vector3_2.z +=
				AdjustCameraDepthAndLensForGroupFraming(targetGroup, targetPos2D.z - vector3_2.z, ref curState,
					deltaTime);
		targetPos2D.z -= vector3_2.z;
		var lens = curState.Lens;
		var num2 = lens.Orthographic
			? curState.Lens.OrthographicSize
			: Mathf.Tan((float)(0.5 * curState.Lens.FieldOfView * (Math.PI / 180.0))) * targetPos2D.z;
		var softGuideRect = SoftGuideRect;
		double orthoSize1 = num2;
		lens = curState.Lens;
		double aspect1 = lens.Aspect;
		var ortho1 = ScreenToOrtho(softGuideRect, (float)orthoSize1, (float)aspect1);
		if (deltaTime < 0.0) {
			var screenRect = new Rect(ortho1.center, Vector2.zero);
			vector3_2 += OrthoOffsetToScreenBounds(targetPos2D, screenRect);
		} else {
			vector3_2 += OrthoOffsetToScreenBounds(targetPos2D, ortho1);
			var vector3_3 = Vector3.zero;
			if (!m_UnlimitedSoftZone) {
				var hardGuideRect = HardGuideRect;
				double orthoSize2 = num2;
				lens = curState.Lens;
				double aspect2 = lens.Aspect;
				var ortho2 = ScreenToOrtho(hardGuideRect, (float)orthoSize2, (float)aspect2);
				var screenBounds = OrthoOffsetToScreenBounds(targetPos2D, ortho2);
				var num3 = Mathf.Max(screenBounds.x / (vector3_2.x + 0.0001f),
					screenBounds.y / (vector3_2.y + 0.0001f));
				vector3_3 = vector3_2 * num3;
			}

			vector3_2 = vector3_3 + Damper.Damp(vector3_2 - vector3_3, new Vector3(m_XDamping, m_YDamping, m_ZDamping),
				deltaTime);
		}
	...
	private float AdjustCameraDepthAndLensForGroupFraming(
		...
		float deltaTime) {
		var num1 = 0.0f;
		var boundingBox = group.BoundingBox;
		var vector3 = curState.RawOrientation * Vector3.forward;
		m_lastBoundsMatrix = Matrix4x4.TRS(boundingBox.center - vector3 * boundingBox.extents.magnitude,
			curState.RawOrientation, Vector3.one);
		m_LastBounds = group.GetViewSpaceBoundingBox(m_lastBoundsMatrix);
		var num2 = GetTargetHeight(m_LastBounds);
		if (deltaTime >= 0.0)
			num2 = m_prevTargetHeight + Damper.Damp(num2 - m_prevTargetHeight, m_ZDamping, deltaTime);
		m_prevTargetHeight = num2;
		var lens1 = curState.Lens;
		if (!lens1.Orthographic && m_AdjustmentMode != 0) {
			var num3 = Mathf.Clamp(
				Mathf.Clamp(
					num2 / (2f * Mathf.Tan((float)(curState.Lens.FieldOfView * (Math.PI / 180.0) / 2.0))) +
					m_LastBounds.extents.z, targetZ - m_MaxDollyIn, targetZ + m_MaxDollyOut), m_MinimumDistance,
				m_MaximumDistance);
			num1 += num3 - targetZ;
		}
