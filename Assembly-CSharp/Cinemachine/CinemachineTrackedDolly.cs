...
public class CinemachineTrackedDolly : CinemachineComponentBase {
	...
	[Tooltip(
		"The position along the path at which the camera will be placed.  This can be animated directly, or set automatically by the Auto-Dolly feature to get as close as possible to the Follow target.  The value is interpreted according to the Position Units setting.")]
	public float m_PathPosition;

	[Tooltip(
		"How to interpret Path Position.  If set to Path Units, values are as follows: 0 represents the first waypoint on the path, 1 is the second, and so on.  Values in-between are points on the path in between the waypoints.  If set to Distance, then Path Position represents distance along the path.")]
	public CinemachinePathBase.PositionUnits m_PositionUnits = CinemachinePathBase.PositionUnits.PathUnits;

	[Tooltip(
		"Where to put the camera relative to the path position.  X is perpendicular to the path, Y is up, and Z is parallel to the path.  This allows the camera to be offset from the path itself (as if on a tripod, for example).")]
	public Vector3 m_PathOffset = Vector3.zero;

	[Range(0.0f, 20f)]
	[Tooltip(
		"How aggressively the camera tries to maintain its position in a direction perpendicular to the path.  Small numbers are more responsive, rapidly translating the camera to keep the target's x-axis offset.  Larger numbers give a more heavy slowly responding camera. Using different settings per axis can yield a wide range of camera behaviors.")]
	public float m_XDamping;

	[Range(0.0f, 20f)]
	[Tooltip(
		"How aggressively the camera tries to maintain its position in the path-local up direction.  Small numbers are more responsive, rapidly translating the camera to keep the target's y-axis offset.  Larger numbers give a more heavy slowly responding camera. Using different settings per axis can yield a wide range of camera behaviors.")]
	public float m_YDamping;

	[Range(0.0f, 20f)]
	[Tooltip(
		"How aggressively the camera tries to maintain its position in a direction parallel to the path.  Small numbers are more responsive, rapidly translating the camera to keep the target's z-axis offset.  Larger numbers give a more heavy slowly responding camera. Using different settings per axis can yield a wide range of camera behaviors.")]
	public float m_ZDamping = 1f;

	[Tooltip(
		"How to set the virtual camera's Up vector.  This will affect the screen composition, because the camera Aim behaviours will always try to respect the Up direction.")]
	...
	public override void MutateCameraState(ref CameraState curState, float deltaTime) {
		...
		if (m_AutoDolly.m_Enabled && FollowTarget != null) {
			var num = m_PreviousPathPosition;
			if (m_PositionUnits == CinemachinePathBase.PositionUnits.Distance)
				num = m_Path.GetPathPositionFromDistance(num);
			m_PathPosition = m_Path.FindClosestPoint(FollowTarget.transform.position, Mathf.FloorToInt(num),
				deltaTime < 0.0 || m_AutoDolly.m_SearchRadius <= 0 ? -1 : m_AutoDolly.m_SearchRadius,
		...
		var pos = m_PathPosition;
		if (deltaTime >= 0.0) {
			var num1 = m_Path.MaxUnit(m_PositionUnits);
			if (num1 > 0.0) {
				var num2 = m_Path.NormalizeUnit(m_PreviousPathPosition, m_PositionUnits);
				var num3 = m_Path.NormalizeUnit(pos, m_PositionUnits);
		...
		var orientationAtUnit = m_Path.EvaluateOrientationAtUnit(pos, m_PositionUnits);
		var positionAtUnit = m_Path.EvaluatePositionAtUnit(pos, m_PositionUnits);
		var vector3_1 = orientationAtUnit * Vector3.right;
		var rhs = orientationAtUnit * Vector3.up;
		var vector3_2 = orientationAtUnit * Vector3.forward;
		var vector3_3 = positionAtUnit + m_PathOffset.x * vector3_1 + m_PathOffset.y * rhs + m_PathOffset.z * vector3_2;
		if (deltaTime >= 0.0) {
			var previousCameraPosition = m_PreviousCameraPosition;
			var lhs = previousCameraPosition - vector3_3;
			var initial = Vector3.Dot(lhs, rhs) * rhs;
			var vector3_4 = Damper.Damp(lhs - initial, m_XDamping, deltaTime);
			var vector3_5 = Damper.Damp(initial, m_YDamping, deltaTime);
			vector3_3 = previousCameraPosition - (vector3_4 + vector3_5);
		}

		curState.RawPosition = m_PreviousCameraPosition = vector3_3;
		var quaternion = GetTargetOrientationAtPathPoint(orientationAtUnit, curState.ReferenceUp);
		if (deltaTime < 0.0)
			m_PreviousOrientation = quaternion;
		else {
			if (deltaTime >= 0.0) {
				var vector3_6 = (Quaternion.Inverse(m_PreviousOrientation) * quaternion).eulerAngles;
	...
	public struct AutoDolly {
		[Tooltip(
			"If checked, will enable automatic dolly, which chooses a path position that is as close as possible to the Follow target.  Note: this can have significant performance impact")]
