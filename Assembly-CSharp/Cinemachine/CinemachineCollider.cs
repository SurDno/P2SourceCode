...
namespace Cinemachine
{
  ...
  public class CinemachineCollider : CinemachineExtension
  {
      ...
      float deltaTime)
    {
      ...
      {
        Vector3 vector3_1 = PreserveLignOfSight(ref state, ref extra);
        if (m_Damping > 0.0 && deltaTime >= 0.0)
        {
          Vector3 vector3_2 = Damper.Damp(vector3_1 - extra.m_previousDisplacement, m_Damping, deltaTime);
      ...
      if (m_OptimalTargetDistance > 0.0 && state.HasLookAt)
      {
        ...
        if (num2 <= (double) m_OptimalTargetDistance)
        {
          ...
          if (num2 >= (double) num3)
            num1 = (float) (0.20000000298023224 * (num2 - (double) num3) / (m_OptimalTargetDistance - (double) num3));
        }
        else
        {
          float num4 = num2 - m_OptimalTargetDistance;
      ...
      if (state.HasLookAt)
      {
        ...
        if (magnitude > (double) num)
        {
          ...
          if (rayLength > 9.9999997473787516E-05 && RaycastIgnoreTag(ray, out hitInfo, rayLength))
          {
            ...
            if (m_Strategy != 0)
              vector3_2 = PushCameraBack(vector3_2, vector3_3, hitInfo, referenceLookAt, new Plane(state.ReferenceUp, correctedPosition), magnitude, m_MaximumEffort, ref extra);
          }
        }
      ...
      ref VcamExtraState extra)
    {
      ...
      float pushBackDistance1 = GetPushBackDistance(ray, startPlane, targetDistance, lookAtPos);
      ...
      if (RaycastIgnoreTag(ray, out hitInfo, num))
      {
        ...
        extra.AddPointToDebugPath(vector3_2);
        if (iterations > 1)
          vector3_2 = PushCameraBack(vector3_2, zero, hitInfo, lookAtPos, startPlane, targetDistance, iterations - 1, ref extra);
        return vector3_2;
      }
      Vector3 vector3_3 = ray.GetPoint(num);
      Vector3 vector3_4 = vector3_3 - lookAtPos;
      float magnitude = vector3_4.magnitude;
      if (magnitude < 9.9999997473787516E-05 || RaycastIgnoreTag(new Ray(lookAtPos, vector3_4), out RaycastHit _, magnitude - 1f / 1000f))
        return currentPos;
      ray = new Ray(vector3_3, vector3_4);
      extra.AddPointToDebugPath(vector3_3);
      float pushBackDistance2 = GetPushBackDistance(ray, startPlane, targetDistance, lookAtPos);
      if (pushBackDistance2 > 9.9999997473787516E-05)
      {
        ...
        else
        {
          ...
          extra.AddPointToDebugPath(vector3_3);
          if (iterations > 1)
            vector3_3 = PushCameraBack(vector3_3, vector3_4, hitInfo, lookAtPos, startPlane, targetDistance, iterations - 1, ref extra);
        }
      }
      return vector3_3;
    }
      ...
      ref Vector3 outDir)
    {
      ...
      int num2 = Physics.SphereCastNonAlloc(pos, num1, pushDir.normalized, m_CornerBuffer, 0.0f, m_CollideAgainst.value, QueryTriggerInteraction.Ignore);
      if (num2 > 1)
      {
    ...
    private bool CheckForTargetObstructions(CameraState state)
    {
      if (state.HasLookAt)
      {
        ...
        if (magnitude < (double) Mathf.Max(m_MinimumDistanceFromTarget, 0.0001f) || RaycastIgnoreTag(new Ray(correctedPosition, vector3.normalized), out RaycastHit _, magnitude - m_MinimumDistanceFromTarget))
          return true;
      }
      return false;
    }
