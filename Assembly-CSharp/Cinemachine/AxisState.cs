...
public struct AxisState {
	[NoSaveDuringPlay] [Tooltip("The current value of the axis.")]
	public float Value;
	...
	[NoSaveDuringPlay] [Tooltip("If checked, then the raw value of the input axis will be inverted before it is used")]
	...
	public bool Update(float deltaTime) {
		...
		var inputAxisValue = m_InputAxisValue;
		if (m_InvertAxis)
			inputAxisValue *= -1f;
		if (m_MaxSpeed > 9.9999997473787516E-05) {
			var f = inputAxisValue * m_MaxSpeed;
			if (Mathf.Abs(f) < 9.9999997473787516E-05 || (Mathf.Sign(mCurrentSpeed) == (double)Mathf.Sign(f) &&
			                                              Mathf.Abs(f) < (double)Mathf.Abs(mCurrentSpeed))) {
				var num = Mathf.Min(Mathf.Abs(f - mCurrentSpeed) / Mathf.Max(0.0001f, m_DecelTime) * deltaTime,
					Mathf.Abs(mCurrentSpeed));
				mCurrentSpeed -= Mathf.Sign(mCurrentSpeed) * num;
			} else {
				var num = Mathf.Abs(f - mCurrentSpeed) / Mathf.Max(0.0001f, m_AccelTime);
				mCurrentSpeed += Mathf.Sign(f) * num * deltaTime;
				if (Mathf.Sign(mCurrentSpeed) == (double)Mathf.Sign(f) &&
				    Mathf.Abs(mCurrentSpeed) > (double)Mathf.Abs(f))
					mCurrentSpeed = f;
			}
		}
