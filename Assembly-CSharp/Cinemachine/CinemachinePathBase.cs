...
public abstract class CinemachinePathBase : MonoBehaviour {
	[Tooltip("Path samples per waypoint.  This is used for calculating path distances.")] [Range(1f, 100f)]
	...
	public virtual float FindClosestPoint(
		...
		var a = MaxPos;
		if (searchRadius >= 0) {
			var num2 = Mathf.FloorToInt(Mathf.Min(searchRadius, (float)((a - (double)num1) / 2.0)));
		...
		var num3 = 1f / stepsPerSegment;
		float closestPoint = startSegment;
		var num4 = float.MaxValue;
		var num5 = stepsPerSegment == 1 ? 1 : 3;
		for (var index = 0; index < num5; ++index) {
			var vector3 = EvaluatePosition(num1);
			for (var pos = num1 + num3; pos <= (double)a; pos += num3) {
				var position = EvaluatePosition(pos);
				var t = p.ClosestPointOnSegment(vector3, position);
				var num6 = Vector3.SqrMagnitude(p - Vector3.Lerp(vector3, position, t));
				if (num6 < (double)num4) {
					num4 = num6;
	...
	public bool DistanceCacheIsValid() {
		return MaxPos == (double)MinPos || (m_DistanceToPos != null && m_PosToDistance != null &&
	...
	public float GetPathPositionFromDistance(float distance) {
		...
		var f = distance / m_cachedDistanceStepSize;
	...
	private void ResamplePath(int stepsPerSegment) {
		...
		var minPos = MinPos;
		var maxPos = MaxPos;
		var num1 = 1f / Mathf.Max(1, stepsPerSegment);
		var length = Mathf.RoundToInt((maxPos - minPos) / num1) + 1;
		m_PosToDistance = new float[length];
		m_CachedSampleSteps = stepsPerSegment;
		m_cachedPosStepSize = num1;
		var a = EvaluatePosition(0.0f);
	...
	public class Appearance {
		...
		[Tooltip("The width of the railroad-tracks that are drawn to represent the path")] [Range(0.0f, 10f)]
