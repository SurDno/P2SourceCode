...
public sealed class CinemachineBlenderSettings : ScriptableObject {
	...
	public AnimationCurve GetBlendCurveForVirtualCameras(
		...
		AnimationCurve animationCurve1 = null;
		AnimationCurve animationCurve2 = null;
		if (m_CustomBlends != null)
			for (var index = 0; index < m_CustomBlends.Length; ++index) {
				var customBlend = m_CustomBlends[index];
				...
				} else if (customBlend.m_To == "**ANY CAMERA**" && !string.IsNullOrEmpty(fromCameraName) &&
				           customBlend.m_From == fromCameraName)
	...
	public struct CustomBlend {
		...
		[Tooltip("Blend curve definition")] public CinemachineBlendDefinition m_Blend;
	}
}