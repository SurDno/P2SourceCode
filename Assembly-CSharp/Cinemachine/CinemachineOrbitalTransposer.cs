...
public class CinemachineOrbitalTransposer : CinemachineTransposer {
	[Space] [Tooltip("The definition of Forward.  Camera will follow behind.")]
	public Heading m_Heading = new(Heading.HeadingDefinition.TargetForward, 4, 0.0f);

	[Tooltip(
		"Automatic heading recentering.  The settings here defines how the camera will reposition itself in the absence of player input.")]
	public Recentering m_RecenterToTargetHeading = new(true, 1f, 2f);

	[Tooltip(
		"Heading Control.  The settings here control the behaviour of the camera in response to the player's input.")]
	public AxisState m_XAxis = new(300f, 2f, 1f, 0.0f, "Mouse X", true);

	[SerializeField] [HideInInspector] [FormerlySerializedAs("m_Radius")]
	private float m_LegacyRadius = float.MaxValue;

	[SerializeField] [HideInInspector] [FormerlySerializedAs("m_HeightOffset")]
	...
	[HideInInspector] [NoSaveDuringPlay] public bool m_HeadingIsSlave;

	internal UpdateHeadingDelegate HeadingUpdater =
		(orbital, deltaTime, up) => orbital.UpdateHeading(deltaTime, up, ref orbital.m_XAxis);
	...
	protected override void OnValidate() {
		if (m_LegacyRadius != 3.4028234663852886E+38 && m_LegacyHeightOffset != 3.4028234663852886E+38 &&
		    m_LegacyHeadingBias != 3.4028234663852886E+38) {
	...
	public float UpdateHeading(float deltaTime, Vector3 up, ref AxisState axis) {
		...
		} else if (m_BindingMode != BindingMode.SimpleFollowWithWorldUp && m_RecenterToTargetHeading.m_enabled &&
		           Time.time > mLastHeadingAxisInputTime + (double)m_RecenterToTargetHeading.m_RecenterWaitTime) {
			var num1 = m_RecenterToTargetHeading.m_RecenteringTime / 3f;
			if (num1 <= (double)deltaTime)
				axis.Value = targetHeading;
			else {
				var f = Mathf.DeltaAngle(axis.Value, targetHeading);
				var a = Mathf.Abs(f);
				if (a < 9.9999997473787516E-05) {
					axis.Value = targetHeading;
					mHeadingRecenteringVelocity = 0.0f;
				} else {
					var num2 = deltaTime / num1;
					var num3 = Mathf.Sign(f) * Mathf.Min(a, a * num2);
					var num4 = num3 - mHeadingRecenteringVelocity;
	...
	public override void MutateCameraState(ref CameraState curState, float deltaTime) {
		...
		var angle = HeadingUpdater(this, deltaTime, curState.ReferenceUp);
		if (!IsValid)
			return;
		mLastTargetPosition = FollowTarget.position;
		if (m_BindingMode != BindingMode.SimpleFollowWithWorldUp)
			angle += m_Heading.m_HeadingBias;
		var quaternion1 = Quaternion.AngleAxis(angle, curState.ReferenceUp);
		var effectiveOffset = EffectiveOffset;
		Vector3 outTargetPosition;
		Quaternion outTargetOrient;
		TrackTarget(deltaTime, curState.ReferenceUp, quaternion1 * effectiveOffset, out outTargetPosition,
			out outTargetOrient);
		curState.ReferenceUp = outTargetOrient * Vector3.up;
		if (deltaTime >= 0.0) {
			var vector3_1 = quaternion1 * effectiveOffset - mHeadingPrevFrame * mOffsetPrevFrame;
			var vector3_2 = outTargetOrient * vector3_1;
			curState.PositionDampingBypass = vector3_2;
		}

		var quaternion2 = outTargetOrient * quaternion1;
	...
	public override void OnPositionDragged(Vector3 delta) {
		m_FollowOffset = m_FollowOffset +
		                 (Quaternion.Inverse(GetReferenceOrientation(VcamState.ReferenceUp)) * delta) with {
			                 x = 0.0f
		                 };
	...
	private static string GetFullName(GameObject current) {
		if (current == null)
			return "";
		return current.transform.parent == null
			? "/" + current.name
			: GetFullName(current.transform.parent.gameObject) + "/" + current.name;
	}
	...
	private float GetTargetHeading(
		...
		var filterSize = m_Heading.m_VelocityFilterStrength * 5;
		...
		var vector3_1 = targetOrientation * Vector3.up;
		var vector3_2 = vector.ProjectOntoPlane(vector3_1);
		if (!vector3_2.AlmostZero())
			mHeadingTracker.Add(vector3_2);
		var reliableHeading = mHeadingTracker.GetReliableHeading();
		return !reliableHeading.AlmostZero()
			? UnityVectorExtensions.SignedAngle(targetOrientation * Vector3.forward, reliableHeading, vector3_1)
			: currentHeading;
	}
	...
	public struct Heading {
		[Tooltip(
			"How 'forward' is defined.  The camera will be placed by default behind the target.  PositionDelta will consider 'forward' to be the direction in which the target is moving.")]
		public HeadingDefinition m_HeadingDefinition;

		[Range(0.0f, 10f)]
		[Tooltip(
			"Size of the velocity sampling window for target heading filter.  This filters out irregularities in the target's movement.  Used only if deriving heading from target's movement (PositionDelta or Velocity)")]
		public int m_VelocityFilterStrength;

		[Range(-180f, 180f)]
		[Tooltip(
			"Where the camera is placed when the X-axis value is zero.  This is a rotation in degrees around the Y axis.  When this value is 0, the camera will be placed behind the target.  Nonzero offsets will rotate the zero position around the target.")]
	...
	public struct Recentering {
		[Tooltip(
			"If checked, will enable automatic recentering of the camera based on the heading definition. If unchecked, recenting is disabled.")]
		public bool m_enabled;

		[Tooltip(
			"If no input has been detected, the camera will wait this long in seconds before moving its heading to the zero position.")]
	...
	private class HeadingTracker {
		...
		private void PopBottom() {
			if (mCount <= 0)
				return;
			var time = Time.time;
			var obj = mHistory[mBottom];
			if (++mBottom == FilterSize)
				mBottom = 0;
			--mCount;
			var num = Decay(time - obj.time);
		...
		public void DecayHistory() {
			var time = Time.time;
			var num = Decay(time - mWeightTime);
			mWeightSum *= num;
			mWeightTime = time;
