...
public class CinemachineSmoothPath : CinemachinePathBase {
	...
	private void UpdateControlPoints() {
		var length = m_Waypoints == null ? 0 : m_Waypoints.Length;
		if (length <= 1 || (Looped == m_IsLoopedCache && m_ControlPoints1 != null &&
		                    m_ControlPoints1.Length == length && m_ControlPoints2 != null &&
		                    m_ControlPoints2.Length == length))
			return;
		var ctrl1 = new Vector4[length];
		var ctrl2 = new Vector4[length];
		var knot = new Vector4[length];
	...
	public override Vector3 EvaluatePosition(float pos) {
		...
		if (m_Waypoints.Length != 0) {
			...
			position = indexA != indexB
				? SplineHelpers.Bezier3(pos - indexA, m_Waypoints[indexA].position, m_ControlPoints1[indexA].position,
					m_ControlPoints2[indexA].position, m_Waypoints[indexB].position)
				: m_Waypoints[indexA].position;
		}
	...
	public override Vector3 EvaluateTangent(float pos) {
		...
		if (m_Waypoints.Length > 1) {
			...
			direction = SplineHelpers.BezierTangent3(pos - indexA, m_Waypoints[indexA].position,
				m_ControlPoints1[indexA].position, m_ControlPoints2[indexA].position, m_Waypoints[indexB].position);
		}
	...
	public override Quaternion EvaluateOrientation(float pos) {
		...
		if (m_Waypoints.Length != 0) {
			...
			if (indexA == indexB)
				...
				UpdateControlPoints();
				angle = SplineHelpers.Bezier1(pos - indexA, m_Waypoints[indexA].roll, m_ControlPoints1[indexA].roll,
