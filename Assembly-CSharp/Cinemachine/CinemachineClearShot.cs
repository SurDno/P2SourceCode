...
public class CinemachineClearShot : CinemachineVirtualCameraBase {
	[Tooltip(
		"Default object for the camera children to look at (the aim target), if not specified in a child camera.  May be empty if all children specify targets of their own.")]
	[NoSaveDuringPlay]
	public Transform m_LookAt;

	[Tooltip(
		"Default object for the camera children wants to move with (the body target), if not specified in a child camera.  May be empty if all children specify targets of their own.")]
	[NoSaveDuringPlay]
	...
	[Tooltip(
		"If checked, camera choice will be randomized if multiple cameras are equally desirable.  Otherwise, child list order and child camera priority will be used.")]
	...
	[HideInInspector] public CinemachineBlenderSettings m_CustomBlends;
	...
	private ICinemachineCamera ChooseCurrentCamera(Vector3 worldUp, float deltaTime) {
		...
		var virtualCameraBaseArray = m_ChildCameras;
		...
		if (LiveChild != null && !LiveChild.VirtualCameraGameObject.activeSelf)
			LiveChild = null;
		var cinemachineCamera = LiveChild;
		for (var index = 0; index < virtualCameraBaseArray.Length; ++index) {
			var virtualCameraBase = virtualCameraBaseArray[index];
			if (virtualCameraBase != null && virtualCameraBase.VirtualCameraGameObject.activeInHierarchy) {
				int num;
				if (cinemachineCamera != null) {
					var state = virtualCameraBase.State;
		...
		if (mActivationTime != 0.0) {
			...
			if (deltaTime >= 0.0 && mPendingActivationTime != 0.0 && mPendingCamera == cinemachineCamera) {
				if (time - (double)mPendingActivationTime <= m_ActivateAfter ||
				    time - (double)mActivationTime <= m_MinDuration)
					return LiveChild;
				m_RandomizedChilden = null;
		...
		mPendingActivationTime = 0.0f;
		mPendingCamera = null;
		if (deltaTime >= 0.0 && mActivationTime > 0.0 &&
		    (m_ActivateAfter > 0.0 || time - (double)mActivationTime < m_MinDuration)) {
	...
	private CinemachineVirtualCameraBase[] Randomize(CinemachineVirtualCameraBase[] src) {
		...
		var virtualCameraBaseArray = new CinemachineVirtualCameraBase[src.Length];
		var array = pairList.ToArray();
		for (var index = 0; index < src.Length; ++index)
