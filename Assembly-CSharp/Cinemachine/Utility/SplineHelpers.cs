...
internal static class SplineHelpers {
	...
	public static Vector3 BezierTangent3(float t, Vector3 p0, Vector3 p1, Vector3 p2, Vector3 p3) {
		t = Mathf.Clamp01(t);
		return (-3f * p0 + 9f * p1 - 9f * p2 + 3f * p3) * t * t + (6f * p0 - 12f * p1 + 6f * p2) * t - 3f * p0 +
		       3f * p1;
	}
	...
	public static float Bezier1(float t, float p0, float p1, float p2, float p3) {
		t = Mathf.Clamp01(t);
		var num = 1f - t;
		return (float)(num * (double)num * num * p0 + 3.0 * num * num * t * p1 + 3.0 * num * t * t * p2 +
		               t * (double)t * t * p3);
	}

	public static float BezierTangent1(float t, float p0, float p1, float p2, float p3) {
		t = Mathf.Clamp01(t);
		return (float)((-3.0 * p0 + 9.0 * p1 - 9.0 * p2 + 3.0 * p3) * t * t + (6.0 * p0 - 12.0 * p1 + 6.0 * p2) * t -
			3.0 * p0 + 3.0 * p1);
	}
	...
	public static void ComputeSmoothControlPoints(
		...
		if (length <= 2) {
			...
			var numArray1 = new float[length];
			var numArray2 = new float[length];
			var numArray3 = new float[length];
			var numArray4 = new float[length];
			for (var index1 = 0; index1 < 4; ++index1) {
				...
				for (var index3 = 1; index3 < index2 - 1; ++index3) {
					numArray1[index3] = 1f;
					numArray2[index3] = 4f;
					numArray3[index3] = 1f;
				...
				for (var index4 = 1; index4 < index2; ++index4) {
					var num = numArray1[index4] / numArray2[index4 - 1];
				...
				for (var index5 = index2 - 2; index5 >= 0; --index5)
					ctrl1[index5][index1] = (numArray4[index5] - numArray3[index5] * ctrl1[index5 + 1][index1]) /
					                        numArray2[index5];
				for (var index6 = 0; index6 < index2; ++index6)
	...
	public static void ComputeSmoothControlPointsLooped(
		...
		if (length < 2) {
			...
			var num = Mathf.Min(4, length - 1);
			var knot1 = new Vector4[length + 2 * num];
			var ctrl1_1 = new Vector4[length + 2 * num];
			var ctrl2_1 = new Vector4[length + 2 * num];
