...
public class CinemachineTransposer : CinemachineComponentBase {
	...
	protected Vector3 EffectiveOffset {
		get {
			var followOffset = m_FollowOffset;
	...
	protected void TrackTarget(
		...
		var referenceOrientation = GetReferenceOrientation(up);
		var quaternion = referenceOrientation;
		if (deltaTime >= 0.0) {
			var vector3 = (Quaternion.Inverse(m_PreviousReferenceOrientation) * referenceOrientation).eulerAngles;
		...
		var previousTargetPosition = m_PreviousTargetPosition;
		var vector3_1 = position - previousTargetPosition;
		if (deltaTime >= 0.0) {
			var rotation = !desiredCameraOffset.AlmostZero()
				? Quaternion.LookRotation(quaternion * desiredCameraOffset.normalized, up)
				: VcamState.RawOrientation;
			var vector3_2 = Damper.Damp(Quaternion.Inverse(rotation) * vector3_1, Damping, deltaTime);
	...
	protected Vector3 Damping => m_BindingMode == BindingMode.SimpleFollowWithWorldUp
		? new Vector3(0.0f, m_YDamping, m_ZDamping)
		: new Vector3(m_XDamping, m_YDamping, m_ZDamping);
	...
	public Quaternion GetReferenceOrientation(Vector3 worldUp) {
		if (FollowTarget != null) {
			var rotation = FollowTarget.rotation;
			switch (m_BindingMode) {
				case BindingMode.LockToTargetOnAssign:
				...
				case BindingMode.LockToTarget:
					return rotation;
				case BindingMode.SimpleFollowWithWorldUp:
					var vector3 = FollowTarget.position - VcamState.RawPosition;
