...
public class CinemachineBlendListCamera : CinemachineVirtualCameraBase {
	...
	public override void UpdateCameraState(Vector3 worldUp, float deltaTime) {
		...
		if (virtualCameraBase != null) {
			...
			if (liveChild != null && LiveChild != null && liveChild != LiveChild && mCurrentInstruction > 0) {
				mActiveBlend = CreateBlend(liveChild, LiveChild, m_Instructions[mCurrentInstruction].m_Blend.BlendCurve,
	...
	private void AdvanceCurrentInstruction() {
		if (m_ChildCameras == null || m_ChildCameras.Length == 0 || mActivationTime < 0.0 ||
		    m_Instructions.Length == 0) {
			mActivationTime = -1f;
			mCurrentInstruction = -1;
			mActiveBlend = null;
	...
	public struct Instruction {
		...
		[CinemachineBlendDefinitionProperty] [Tooltip("How to blend to the next virtual camera in the list (if any)")]
