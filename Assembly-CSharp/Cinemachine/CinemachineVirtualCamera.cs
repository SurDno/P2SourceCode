...
public class CinemachineVirtualCamera : CinemachineVirtualCameraBase {
	...
	[SerializeField] [HideInInspector] private Transform m_ComponentOwner;
	...
	private Transform CreatePipeline(CinemachineVirtualCamera copyFrom) {
		...
		if (CreatePipelineOverride != null)
			...
			var gameObject = new GameObject("cm");
	...
	public CinemachineComponentBase GetCinemachineComponent(CinemachineCore.Stage stage) {
		var componentPipeline = GetComponentPipeline();
		if (componentPipeline != null)
			foreach (var cinemachineComponent in componentPipeline)
	...
	public T AddCinemachineComponent<T>() where T : CinemachineComponentBase {
		...
		var components = componentOwner.GetComponents<CinemachineComponentBase>();
		var obj = componentOwner.gameObject.AddComponent<T>();
	...
	public void DestroyCinemachineComponent<T>() where T : CinemachineComponentBase {
		...
		if (componentPipeline == null)
			return;
		foreach (var cinemachineComponentBase in componentPipeline)
	...
	public void OnPositionDragged(Vector3 delta) {
		...
		if (componentPipeline == null)
			return;
		for (var index = 0; index < componentPipeline.Length; ++index)
	...
	private void UpdateComponentPipeline() {
		if (m_ComponentOwner != null && m_ComponentOwner.parent != this.transform) {
			var component = m_ComponentOwner.parent != null
				? m_ComponentOwner.parent.gameObject.GetComponent<CinemachineVirtualCamera>()
				: null;
			DestroyPipeline();
		...
		if (m_ComponentOwner != null && m_ComponentPipeline != null)
			return;
		m_ComponentOwner = null;
		var cinemachineComponentBaseList = new List<CinemachineComponentBase>();
		foreach (Transform transform in this.transform)
			if (transform.GetComponent<CinemachinePipeline>() != null) {
				m_ComponentOwner = transform;
				foreach (var component in transform.GetComponents<CinemachineComponentBase>())
	...
	private CameraState CalculateNewState(Vector3 worldUp, float deltaTime) {
		...
		var curStage = CinemachineCore.Stage.Body;
		UpdateComponentPipeline();
		if (m_ComponentPipeline != null) {
			for (var index = 0; index < m_ComponentPipeline.Length; ++index)
				m_ComponentPipeline[index].PrePipelineMutateCameraState(ref newState);
			for (var index = 0; index < m_ComponentPipeline.Length; ++index) {
				curStage = AdvancePipelineStage(ref newState, deltaTime, curStage,
					(int)m_ComponentPipeline[index].Stage);
				m_ComponentPipeline[index].MutateCameraState(ref newState, deltaTime);
			}
		}

		var maxStage = 3;
		var num = (int)AdvancePipelineStage(ref newState, deltaTime, curStage, maxStage);
		return newState;
	}
