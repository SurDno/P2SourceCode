...
public abstract class CinemachineVirtualCameraBase : MonoBehaviour, ICinemachineCamera {
	[HideInInspector] [NoSaveDuringPlay] public Action CinemachineGUIDebuggerCallback = null;
	...
	protected void InvokePostPipelineStageCallback(
		...
		var parentCamera = ParentCamera as CinemachineVirtualCameraBase;
	...
	protected virtual void OnEnable() {
		var components = GetComponents<CinemachineVirtualCameraBase>();
		for (var index = 0; index < components.Length; ++index)
			if (components[index].enabled && components[index] != this) {
				Debug.LogError(Name + " has multiple CinemachineVirtualCameraBase-derived components.  Disabling " +
				               GetType().Name + ".");
				enabled = false;
			}
