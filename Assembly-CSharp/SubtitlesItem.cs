...
public class SubtitlesItem {
	...
	private void ShowPart() {
		...
		var num3 = text.IndexOf("<split=", partIndex);
		if (num3 != -1) {
			var startIndex = num3 + 7;
			var num4 = text.IndexOf('>', startIndex);
			if (num4 != -1) {
				var s = text.Substring(startIndex, num4 - startIndex);
	...
	public void Start(IEntity actor, string text, AudioState audioState, Object context) {
		...
		this.text = text;
		Actor = actor;
		var length = audioState.AudioSource.clip.length;
		progress = 0.0f;
		startTime = 0.0f;
		endTime = length;
		partIndex = 0;
	...
	public SubtitlesItem(SubtitlesView subtitlesView) {
	...
	public void Update() {
		...
		if (audioState.Complete)
			...
			if (progress >= (double)endTime)
				...
				if (!(fadeView != null))
					return;
				fadeView.Progress = subtitlesView.FadeTime > 0.0
					? Mathf.Clamp01(
						Mathf.Min(progress - partStartTime, partEndTime - progress) / subtitlesView.FadeTime)
					: 1f;
			}
		}
	}
