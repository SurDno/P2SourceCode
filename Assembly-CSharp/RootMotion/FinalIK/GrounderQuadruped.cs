...
public class GrounderQuadruped : Grounder {
	...
	[Tooltip("The weight of rotating the character root to the ground angle (range: 0 - 1).")] [Range(0.0f, 1f)]
	...
	[Tooltip("The maximum angle of rotating the quadruped upwards (going uphill, range: 0 - 90).")] [Range(0.0f, 90f)]
	...
	private bool IsReadyToInitiate() {
		return !(pelvis == null) && !(lastSpineBone == null) && legs.Length != 0 && forelegs.Length != 0 &&
		       !(characterRoot == null) && IsReadyToInitiateLegs(legs) && IsReadyToInitiateLegs(forelegs);
	}
	...
	private void Initiate() {
		...
		var feet1 = InitiateFeet(legs, ref feet, 0);
		var feet2 = InitiateFeet(forelegs, ref feet, legs.Length);
		...
		for (var index = 0; index < feet1.Length; ++index)
			feet[index].leg = solver.legs[index];
		for (var index = 0; index < feet2.Length; ++index)
	...
	private Transform[] InitiateFeet(
		...
		ref Foot[] f,
		int indexOffset) {
		var transformArray = new Transform[ikComponents.Length];
		for (var index = 0; index < ikComponents.Length; ++index) {
			var points = ikComponents[index].GetIKSolver().GetPoints();
	...
	private void RootRotation() {
		...
		var forward = characterRoot.forward;
		var normal = -gravity;
		Vector3.OrthoNormalize(ref normal, ref forward);
		var rotation = Quaternion.LookRotation(forward, -gravity);
		var vector3_1 = forelegSolver.rootHit.point - solver.rootHit.point;
		var vector3_2 = Quaternion.Inverse(rotation) * vector3_1;
		angle = Mathf.Lerp(angle,
			Mathf.Clamp(Mathf.Atan2(vector3_2.y, vector3_2.z) * 57.29578f * rootRotationWeight, minRootRotation,
				maxRootRotation), Time.deltaTime * rootRotationSpeed);
		characterRoot.rotation = Quaternion.Slerp(characterRoot.rotation,
			Quaternion.AngleAxis(-angle, characterRoot.right) * rotation, weight);
	}
	...
	private void OnSolverUpdate() {
		...
		pelvis.rotation = Quaternion.Slerp(Quaternion.identity,
			Quaternion.FromToRotation(lastSpineBone.position - pelvis.position,
				lastSpineBone.position +
				forelegSolver.root.up * Mathf.Clamp(forelegSolver.pelvis.heightOffset, float.NegativeInfinity, 0.0f) -
				solver.root.up * solver.pelvis.heightOffset - pelvis.position), weight) * pelvis.rotation;
		for (var index = 0; index < feet.Length; ++index)
	...
	private void UpdateForefeetRoot() {
		...
		for (var index = 0; index < forelegSolver.legs.Length; ++index)
			zero += forelegSolver.legs[index].transform.position;
		var vector3 = zero / forelegs.Length - transform.position;
		var up = transform.up;
		var tangent = vector3;
	...
	private void SetFootIK(Foot foot, float maxOffset) {
		var vector = foot.leg.IKPosition - foot.transform.position;
	...
	private void OnPostSolverUpdate() {
		...
		if (solvedFeet < feet.Length)
			return;
		for (var index = 0; index < feet.Length; ++index)
			feet[index].transform.rotation =
				Quaternion.Slerp(Quaternion.identity, feet[index].leg.rotationOffset, weight) * feet[index].rotation;
		if (head != null)
