...
public class InteractionEffector {
	...
	public bool ResetToDefaults(float speed) {
		...
		if (effector.isEndEffector) {
			if (pullUsed)
				interactionSystem.ik.solver.GetChain(effectorType).pull = Mathf.Lerp(defaultPull,
					interactionSystem.ik.solver.GetChain(effectorType).pull, resetTimer);
			if (reachUsed)
				interactionSystem.ik.solver.GetChain(effectorType).reach = Mathf.Lerp(defaultReach,
					interactionSystem.ik.solver.GetChain(effectorType).reach, resetTimer);
			if (pushUsed)
				interactionSystem.ik.solver.GetChain(effectorType).push = Mathf.Lerp(defaultPush,
					interactionSystem.ik.solver.GetChain(effectorType).push, resetTimer);
			if (pushParentUsed)
				interactionSystem.ik.solver.GetChain(effectorType).pushParent = Mathf.Lerp(defaultPushParent,
					interactionSystem.ik.solver.GetChain(effectorType).pushParent, resetTimer);
		}
	...
	public bool Start(
		...
		triggered.Clear();
		for (var index = 0; index < interactionObject.events.Length; ++index)
			triggered.Add(false);
		if (poser != null) {
	...
	public void Update(Transform root, float speed) {
		if (!inInteraction) {
			...
			if (isPaused) {
				...
				timer += (float)(Time.deltaTime * (double)speed *
				                 (interactionTarget != null ? interactionTarget.interactionSpeedMlp : 1.0));
				weight = Mathf.Clamp(weight + Time.deltaTime * fadeInSpeed * speed, 0.0f, 1f);
				var pickUp = false;
				var pause = false;
				TriggerUntriggeredEvents(true, out pickUp, out pause);
				var b1 = pickedUp ? pickUpPosition : target.position;
				var b2 = pickedUp ? pickUpRotation : target.rotation;
				...
				if (pause)
					Pause();
				var b3 = interactionObject.GetValue(InteractionObject.WeightCurve.Type.PoserWeight, interactionTarget,
					timer);
				if (poser != null)
					poser.weight = Mathf.Lerp(poser.weight, b3, weight);
				else if (b3 > 0.0)
					Warning.Log(
						"InteractionObject " + interactionObject.name +
						" has a curve/multipler for Poser Weight, but the bone of effector " + effectorType +
						" has no HandPoser/GenericPoser attached.", effector.bone);
	...
	private void TriggerUntriggeredEvents(bool checkTime, out bool pickUp, out bool pause) {
		pickUp = false;
		pause = false;
		for (var index = 0; index < triggered.Count; ++index)
			if (!triggered[index] && (!checkTime || interactionObject.events[index].time < (double)timer)) {
				...
				if (interactionSystem.OnInteractionEvent != null)
					interactionSystem.OnInteractionEvent(effectorType, interactionObject,
						interactionObject.events[index]);
	...
	private void PickUp(Transform root) {
		...
		pickedUp = true;
		var component = interactionObject.targetsRoot.GetComponent<Rigidbody>();
		if (component != null) {
			if (!component.isKinematic)
				component.isKinematic = true;
			if (root.GetComponent<Collider>() != null)
				foreach (var componentsInChild in interactionObject.targetsRoot.GetComponentsInChildren<Collider>())
	...
	public void OnPostFBBIK() {
		if (!inInteraction)
			return;
		var num = interactionObject.GetValue(InteractionObject.WeightCurve.Type.RotateBoneWeight, interactionTarget,
			timer) * weight;
		if (num > 0.0) {
			var quaternion = Quaternion.Slerp(effector.bone.rotation, pickedUp ? pickUpRotation : effector.rotation,
				num * num);
		...
		if (!pickUpOnPostFBBIK)
			return;
		var position = effector.bone.position;
