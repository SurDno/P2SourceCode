...
[Serializable]
public class Finger {
	[Tooltip("Master Weight for the finger.")] [Range(0.0f, 1f)]
	public float weight = 1f;

	[Tooltip("The weight of rotating the finger tip and bending the finger to the target.")] [Range(0.0f, 1f)]
	...
	public void Initiate(Transform hand, int index) {
		...
		if (!IsValid(ref empty))
			...
			var vector3 = Vector3.Cross(bone2.position - bone1.position, tip.position - bone1.position);
	...
	public void Update(float masterWeight) {
		...
		if (bone3 != null)
			bone3.rotation = num * (double)rotationWeight < 1.0
				? Quaternion.Lerp(bone3.rotation, IKRotation * bone3RelativeToTarget, num * rotationWeight)
				: IKRotation * bone3RelativeToTarget;
