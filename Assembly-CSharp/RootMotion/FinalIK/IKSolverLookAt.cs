...
public class IKSolverLookAt : IKSolver {
	...
	[Range(0.0f, 1f)] public float bodyWeight = 0.5f;
	[Range(0.0f, 1f)] public float headWeight = 0.5f;
	[Range(0.0f, 1f)] public float eyesWeight = 1f;
	[Range(0.0f, 1f)] public float clampWeight = 0.5f;
	[Range(0.0f, 1f)] public float clampWeightHead = 0.5f;
	[Range(0.0f, 1f)] public float clampWeightEyes = 0.5f;
	...
	public override Point[] GetPoints() {
		var points = new Point[spine.Length + eyes.Length + (head.transform != null ? 1 : 0)];
		for (var index = 0; index < spine.Length; ++index)
			points[index] = spine[index];
		var index1 = 0;
		for (var length = spine.Length; length < points.Length; ++length) {
	...
	protected override void OnInitiate() {
		...
		foreach (var lookAtBone in spine)
	...
	private void SolveSpine() {
		if (bodyWeight <= 0.0 || spineIsEmpty)
			return;
		GetForwards(ref spineForwards, spine[0].forward,
			(IKPosition + spineTargetOffset - spine[spine.Length - 1].transform.position).normalized, spine.Length,
			clampWeight);
		for (var index = 0; index < spine.Length; ++index)
	...
	private void SolveHead() {
		if (headWeight <= 0.0 || headIsEmpty)
			return;
		var baseForward = spine.Length == 0 || !(spine[spine.Length - 1].transform != null)
			? head.forward
			: spine[spine.Length - 1].forward;
		var a = baseForward;
		var vector3 = IKPosition - head.transform.position;
		var normalized1 = vector3.normalized;
		var t = headWeight * (double)IKPositionWeight;
	...
	private void SolveEyes() {
		if (eyesWeight <= 0.0 || eyesIsEmpty)
			return;
		for (var index = 0; index < eyes.Length; ++index) {
			GetForwards(ref eyeForward, head.transform != null ? head.forward : eyes[index].forward,
				(IKPosition - eyes[index].transform.position).normalized, 1, clampWeightEyes);
	...
	private Vector3[] GetForwards(
		...
		var num1 = (float)(1.0 - Vector3.Angle(baseForward, targetForward) / 180.0);
		var num2 = clamp > 0.0 ? Mathf.Clamp((float)(1.0 - (clamp - (double)num1) / (1.0 - num1)), 0.0f, 1f) : 1f;
		var num3 = clamp > 0.0 ? Mathf.Clamp(num1 / clamp, 0.0f, 1f) : 1f;
		...
		if (forwards.Length == 1)
			...
			var num4 = 1f / (forwards.Length - 1);
			for (var index = 0; index < forwards.Length; ++index)
				forwards[index] = Vector3.Slerp(baseForward, targetForward,
					spineWeightCurve.Evaluate(num4 * index) * num3 * num2);
		}
	...
	public class LookAtBone : Bone {
		...
		public void LookAt(Vector3 direction, float weight) {
			var rotation1 = Quaternion.FromToRotation(forward, direction);
