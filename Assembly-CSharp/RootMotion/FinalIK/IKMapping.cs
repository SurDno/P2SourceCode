...
public class IKMapping {
	...
	public class BoneMap {
		...
		public void UpdatePlane(bool rotation, bool position) {
			var animatedTargetRotation = lastAnimatedTargetRotation;
			if (rotation)
		...
		public void RotateToPlane(IKSolverFullBody solver, float weight) {
			var b = GetTargetRotation(solver) * defaultLocalTargetRotation;
			if (weight >= 1.0)
		...
		public void Swing(Vector3 pos1, Vector3 pos2, float weight) {
			var b = Quaternion.FromToRotation(transform.rotation * localSwingAxis, pos1 - pos2) * transform.rotation;
			if (weight >= 1.0)
				transform.rotation = b;
			else
				transform.rotation = Quaternion.Lerp(transform.rotation, b, weight);
		...
		public void Twist(Vector3 twistDirection, Vector3 normalDirection, float weight) {
			Vector3.OrthoNormalize(ref normalDirection, ref twistDirection);
			var b = Quaternion.FromToRotation(transform.rotation * localTwistAxis, twistDirection) * transform.rotation;
			if (weight >= 1.0)
				transform.rotation = b;
			else
				transform.rotation = Quaternion.Lerp(transform.rotation, b, weight);
		...
		public void RotateToEffector(IKSolverFullBody solver, float weight) {
			if (!isNodeBone)
				return;
			var t = weight * solver.GetNode(chainIndex, nodeIndex).effectorRotationWeight;
			if (t <= 0.0)
				return;
			if (t >= 1.0)
				...
				transform.rotation = Quaternion.Lerp(transform.rotation,
					solver.GetNode(chainIndex, nodeIndex).solverRotation, t);
		}
		...
		private Quaternion GetTargetRotation(IKSolverFullBody solver) {
			...
			return solverPosition1 == solverPosition3
				? Quaternion.identity
				: Quaternion.LookRotation(solverPosition2 - solverPosition1, solverPosition3 - solverPosition1);
		}

		private Quaternion lastAnimatedTargetRotation => planeBone1.position == planeBone3.position
			? Quaternion.identity
			: Quaternion.LookRotation(planeBone2.position - planeBone1.position,
