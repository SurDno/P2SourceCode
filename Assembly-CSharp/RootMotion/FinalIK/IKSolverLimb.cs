...
public class IKSolverLimb : IKSolverTrigonometric {
	...
	[Range(0.0f, 1f)] public float maintainRotationWeight;
	[Range(0.0f, 1f)] public float bendModifierWeight = 1f;
	...
	private Vector3 GetModifiedBendNormal() {
		var bendModifierWeight = this.bendModifierWeight;
		...
		switch (bendModifier) {
			...
			case BendModifier.Target:
				return Quaternion.Slerp(Quaternion.identity, IKRotation * Quaternion.Inverse(bone3DefaultRotation),
					bendModifierWeight) * bendNormal;
			case BendModifier.Parent:
				return bone1.transform.parent == null
					? bendNormal
					: Quaternion.Slerp(Quaternion.identity,
						bone1.transform.parent.rotation * Quaternion.Inverse(parentDefaultRotation) *
						Quaternion.Inverse(defaultRootRotation), bendModifierWeight) * bendNormal;
			case BendModifier.Arm:
				...
				var normalized = (IKPosition - bone1.transform.position).normalized;
				var rhs = Quaternion.Inverse(
					bone1.transform.parent.rotation * Quaternion.Inverse(parentDefaultRotation)) * normalized;
				if (goal == AvatarIKGoal.LeftHand)
					rhs.x = -rhs.x;
				for (var index = 1; index < axisDirections.Length; ++index) {
					axisDirections[index].dot =
						Mathf.Clamp(Vector3.Dot(axisDirections[index].direction, rhs), 0.0f, 1f);
					axisDirections[index].dot = Interp.Float(axisDirections[index].dot, InterpolationMode.InOutQuintic);
				}

				var a = axisDirections[0].axis;
				for (var index = 1; index < axisDirections.Length; ++index)
				...
				var b1 = bone1.transform.parent.rotation * Quaternion.Inverse(parentDefaultRotation) * a;
			...
			case BendModifier.Goal:
				...
				var b2 = Vector3.Cross(bendGoal.position - bone1.transform.position,
