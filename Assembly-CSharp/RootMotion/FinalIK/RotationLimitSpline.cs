...
public class RotationLimitSpline : RotationLimit {
	...
	public Quaternion LimitSwing(Quaternion rotation) {
		...
		var time = GetOrthogonalAngle(vector3, secondaryAxis, axis);
		if (Vector3.Dot(vector3, crossAxis) < 0.0)
			time = (float)(180.0 + (180.0 - time));
		var maxDegreesDelta = spline.Evaluate(time);
		var quaternion = Quaternion.RotateTowards(Quaternion.identity, Quaternion.FromToRotation(axis, vector3),
			maxDegreesDelta);
