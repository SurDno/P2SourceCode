...
public class GrounderIK : Grounder {
	...
	[Tooltip("The weight of rotating the character root to the ground normal (range: 0 - 1).")] [Range(0.0f, 1f)]
	...
	private void Update() {
		...
		if (initiated) {
			...
			if (!(characterRoot != null) || rootRotationSpeed <= 0.0 || rootRotationWeight <= 0.0)
				return;
			var vector3 = solver.GetLegsPlaneNormal();
			if (rootRotationWeight < 1.0)
				vector3 = Vector3.Slerp(Vector3.up, vector3, rootRotationWeight);
			characterRoot.rotation = Quaternion.Lerp(characterRoot.rotation,
				Quaternion.RotateTowards(Quaternion.FromToRotation(transform.up, Vector3.up) * characterRoot.rotation,
					Quaternion.FromToRotation(transform.up, vector3) * characterRoot.rotation, maxRootRotationAngle),
	...
	private void Initiate() {
		...
		for (var index = 0; index < legs.Length; ++index) {
			var points = legs[index].GetIKSolver().GetPoints();
	...
	private void SetLegIK(int index) {
		...
		if (legs[index] is LegIK) {
			(legs[index].GetIKSolver() as IKSolverLeg).IKRotation =
				Quaternion.Slerp(Quaternion.identity, solver.legs[index].rotationOffset, weight) * footRotations[index];
