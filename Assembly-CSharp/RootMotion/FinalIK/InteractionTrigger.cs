...
public class InteractionTrigger : MonoBehaviour {
	...
	public int GetBestRangeIndex(Transform character, Transform raycastFrom, RaycastHit raycastHit) {
		...
		for (var index = 0; index < ranges.Length; ++index)
			if (ranges[index].IsInRange(character, raycastFrom, raycastHit, transform, out maxError) &&
			    maxError <= (double)num) {
				num = maxError;
	...
	public class CharacterPosition {
		...
		[Tooltip("Angle offset from the default forward direction.")] [Range(-180f, 180f)]
		...
		public bool IsInRange(Transform character, Transform trigger, out float error) {
			...
			if (radius <= 0.0 || maxAngle <= 0.0)
				return false;
			var forward1 = trigger.forward;
			if (fixYAxis)
				forward1.y = 0.0f;
			if (forward1 == Vector3.zero)
				return false;
			var normal = fixYAxis ? Vector3.up : trigger.up;
			var quaternion = Quaternion.LookRotation(forward1, normal);
			var vector3_1 = trigger.position + quaternion * offset3D;
			var vector3_2 = orbit ? trigger.position : vector3_1;
			var tangent1 = character.position - vector3_2;
			Vector3.OrthoNormalize(ref normal, ref tangent1);
			var vector3_3 = tangent1 * Vector3.Project(character.position - vector3_2, tangent1).magnitude;
			if (orbit) {
				var magnitude1 = offset.magnitude;
				var magnitude2 = vector3_3.magnitude;
				if (magnitude2 < magnitude1 - (double)radius || magnitude2 > magnitude1 + (double)radius)
					return false;
			} else if (vector3_3.magnitude > (double)radius)
				return false;

			var tangent2 = quaternion * direction3D;
			Vector3.OrthoNormalize(ref normal, ref tangent2);
			if (orbit) {
				var forward2 = vector3_1 - trigger.position;
				if (forward2 == Vector3.zero)
					forward2 = Vector3.forward;
				var vector3_4 = Quaternion.Inverse(Quaternion.LookRotation(forward2, normal)) * vector3_3;
				tangent2 = Quaternion.AngleAxis(Mathf.Atan2(vector3_4.x, vector3_4.z) * 57.29578f, normal) * tangent2;
			}

			var num = Vector3.Angle(tangent2, character.forward);
	...
	public class CameraPosition {
		...
		[Tooltip("Max angle between the direction and the direction towards the camera.")] [Range(0.0f, 180f)]
		public float maxAngle = 45f;
		...
		public Quaternion GetRotation() {
			var forward = lookAtTarget.transform.forward;
			if (fixYAxis)
				forward.y = 0.0f;
			if (forward == Vector3.zero)
				return Quaternion.identity;
			var upwards = fixYAxis ? Vector3.up : lookAtTarget.transform.up;
		...
		public bool IsInRange(
			...
			if (lookAtTarget == null)
				return true;
			error = 180f;
			if (raycastFrom == null || hit.collider != lookAtTarget || hit.distance > (double)maxDistance ||
			    direction == Vector3.zero || maxDistance <= 0.0 || maxAngle <= 0.0)
				return false;
			var to = GetRotation() * direction;
			var num = Vector3.Angle(raycastFrom.position - hit.point, to);
			if (num > (double)maxAngle)
				return false;
			error = (float)(num / (double)maxAngle * 180.0);
			return true;
	...
	public class Range {
		...
		[HideInInspector] [SerializeField] public bool show = true;
		...
		public bool IsInRange(
			...
			var error1 = 0.0f;
			var error2 = 0.0f;
			if (!characterPosition.IsInRange(character, trigger, out error1) ||
			    !cameraPosition.IsInRange(raycastFrom, raycastHit, trigger, out error2))
				return false;
