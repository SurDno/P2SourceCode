...
public class HandPoser : Poser {
	...
	protected override void UpdatePoser() {
		...
		if (children.Length != poseChildren.Length)
			...
			var t1 = localRotationWeight * weight;
			var t2 = localPositionWeight * weight;
			for (var index = 0; index < children.Length; ++index)
				if (children[index] != transform) {
					children[index].localRotation = Quaternion.Lerp(children[index].localRotation,
						poseChildren[index].localRotation, t1);
					children[index].localPosition = Vector3.Lerp(children[index].localPosition,
						poseChildren[index].localPosition, t2);
				}
		}
	}
