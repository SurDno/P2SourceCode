...
public class AimController : MonoBehaviour {
	...
	[Tooltip("Master weight of the IK solver.")] [Range(0.0f, 1f)]
	...
	private void LateUpdate() {
		...
		ik.solver.IKPositionWeight = Mathf.SmoothDamp(ik.solver.IKPositionWeight, target != null ? weight : 0.0f,
			ref weightV, weightSmoothTime);
		...
		if (smoothTurnTowardsTarget) {
			var vector3 = ik.solver.IKPosition - pivot;
	...
	private void RootRotation() {
		var num1 = Mathf.Lerp(180f, maxRootAngle, ik.solver.IKPositionWeight);
		if (num1 >= 180.0)
			return;
		var vector3 = Quaternion.Inverse(ik.transform.rotation) * (ik.solver.IKPosition - pivot);
		var num2 = Mathf.Atan2(vector3.x, vector3.z) * 57.29578f;
		var angle = 0.0f;
		if (num2 > (double)num1)
			angle = num2 - num1;
		if (num2 < -(double)num1)
