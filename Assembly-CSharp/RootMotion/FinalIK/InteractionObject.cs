...
public class InteractionObject : MonoBehaviour {
	...
	public void Initiate() {
		for (var index = 0; index < weightCurves.Length; ++index)
			if (weightCurves[index].curve.length > 0)
				length = Mathf.Clamp(length, weightCurves[index].curve.keys[weightCurves[index].curve.length - 1].time,
					length);
	...
	public bool CurveUsed(WeightCurve.Type type) {
		foreach (var weightCurve in weightCurves)
	...
	public void Apply(
		...
		float weight) {
		for (var index = 0; index < weightCurves.Length; ++index) {
			var num = target == null ? 1f : target.GetValue(weightCurves[index].type);
		...
		for (var index = 0; index < multipliers.Length; ++index) {
			if (multipliers[index].curve == multipliers[index].result && !Warning.logged)
				Warning.Log(
					"InteractionObject Multiplier 'Curve' " + multipliers[index].curve + "and 'Result' are the same.",
					transform);
			var weightCurveIndex = GetWeightCurveIndex(multipliers[index].curve);
			if (weightCurveIndex != -1) {
				var num = target == null ? 1f : target.GetValue(multipliers[index].result);
				Apply(solver, effector, multipliers[index].result,
					multipliers[index].GetValue(weightCurves[weightCurveIndex], timer), weight * num);
			} else if (!Warning.logged)
				Warning.Log("InteractionObject Multiplier curve " + multipliers[index].curve + "does not exist.",
					transform);
		}
	}
	...
	public float GetValue(
		...
		InteractionTarget target,
		float timer) {
		var weightCurveIndex1 = GetWeightCurveIndex(weightCurveType);
		if (weightCurveIndex1 != -1) {
			var num = target == null ? 1f : target.GetValue(weightCurveType);
			return weightCurves[weightCurveIndex1].GetValue(timer) * num;
		}

		for (var index = 0; index < multipliers.Length; ++index)
			if (multipliers[index].result == weightCurveType) {
				var weightCurveIndex2 = GetWeightCurveIndex(multipliers[index].curve);
		...
		switch (type) {
			...
			case WeightCurve.Type.PositionOffsetX:
				solver.GetEffector(effector).position +=
					(positionOffsetSpace != null ? positionOffsetSpace.rotation : solver.GetRoot().rotation) *
	...
	private int GetWeightCurveIndex(WeightCurve.Type weightCurveType) {
		for (var weightCurveIndex = 0; weightCurveIndex < weightCurves.Length; ++weightCurveIndex)
	...
	private int GetMultiplierIndex(WeightCurve.Type weightCurveType) {
		for (var multiplierIndex = 0; multiplierIndex < multipliers.Length; ++multiplierIndex)
