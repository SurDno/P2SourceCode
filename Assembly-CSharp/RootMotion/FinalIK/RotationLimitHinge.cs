...
public class RotationLimitHinge : RotationLimit {
	...
	private Quaternion LimitHinge(Quaternion rotation) {
		...
		var quaternion = Limit1DOF(rotation, axis);
		if (!useLimits)
			return quaternion;
		var b = quaternion * Quaternion.Inverse(lastRotation);
		var num = Quaternion.Angle(Quaternion.identity, b);
		var lhs = new Vector3(axis.z, axis.x, axis.y);
		var rhs = Vector3.Cross(lhs, axis);
