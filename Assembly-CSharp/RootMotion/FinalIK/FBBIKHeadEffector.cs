...
public class FBBIKHeadEffector : MonoBehaviour {
	...
	[LargeHeader("Position")] [Tooltip("Master weight for positioning the head.")] [Range(0.0f, 1f)]
	public float positionWeight = 1f;

	[Tooltip("The weight of moving the body along with the head")] [Range(0.0f, 1f)]
	public float bodyWeight = 0.8f;

	[Tooltip("The weight of moving the thighs along with the head")] [Range(0.0f, 1f)]
	...
	[LargeHeader("Rotation")] [Tooltip("The weight of rotating the head bone after solving")] [Range(0.0f, 1f)]
	public float rotationWeight;

	[Tooltip("Clamping the rotation of the body")] [Range(0.0f, 1f)]
	public float bodyClampWeight = 0.5f;

	[Tooltip("Clamping the rotation of the head")] [Range(0.0f, 1f)]
	...
	[Tooltip("The weight of rolling the bones in towards the target")] [Range(0.0f, 1f)]
	public float roll;

	[Tooltip("Smoothing the CCD effect.")] [Range(0.0f, 1000f)]
	...
	[Range(0.0f, 1f)] public float chestDirectionWeight = 1f;
	...
	private void OnStoreDefaultLocalState() {
		...
		for (var index = 0; index < CCDBones.Length; ++index)
		...
		for (var index = 0; index < stretchBones.Length; ++index)
		...
		for (var index = 0; index < chestBones.Length; ++index)
	...
	private void OnPreRead() {
		...
		if (bendBonesCount != bendBones.Length || ccdBonesCount != CCDBones.Length ||
		    stretchBonesCount != stretchBones.Length || chestBonesCount != chestBones.Length)
			OnStoreDefaultLocalState();
		ChestDirection();
		SpineBend();
		CCDPass();
	...
	private void SpineBend() {
		var num1 = bendWeight * ik.solver.IKPositionWeight;
		if (num1 <= 0.0 || bendBones.Length == 0)
			return;
		var b = QuaTools.ClampRotation(
			transform.rotation * Quaternion.Inverse(ik.references.root.rotation * headRotationRelativeToRoot),
			bodyClampWeight, 2);
		var num2 = 1f / bendBones.Length;
		for (var index = 0; index < bendBones.Length; ++index)
			if (bendBones[index].transform != null)
				bendBones[index].transform.rotation =
					Quaternion.Lerp(Quaternion.identity, b, num2 * bendBones[index].weight * num1) *
					bendBones[index].transform.rotation;
	}

	private void CCDPass() {
		var num1 = CCDWeight * ik.solver.IKPositionWeight;
		if (num1 <= 0.0)
			return;
		for (var index = CCDBones.Length - 1; index > -1; --index) {
			var quaternion = Quaternion.FromToRotation(ik.references.head.position - CCDBones[index].position,
				transform.position - CCDBones[index].position) * CCDBones[index].rotation;
			var num2 = Mathf.Lerp((CCDBones.Length - index) / CCDBones.Length, 1f, roll);
			var b = Quaternion.Angle(Quaternion.identity, quaternion);
			var num3 = Mathf.Lerp(0.0f, b, (damper - b) / damper);
			CCDBones[index].rotation =
				Quaternion.RotateTowards(CCDBones[index].rotation, quaternion, num3 * num1 * num2);
		}
	}
	...
	private void Iterate(int iteration) {
		...
		LerpSolverPosition(ik.solver.leftShoulderEffector, leftShoulderPos, positionWeight * ik.solver.IKPositionWeight,
			ik.solver.leftShoulderEffector.positionOffset);
		...
		var rotation = QuaTools.FromToRotation(chestRotation,
			Quaternion.LookRotation(transform.position - leftShoulderPos, rightShoulderPos - leftShoulderPos));
		LerpSolverPosition(ik.solver.bodyEffector, transform.position + rotation * headToBody,
			positionWeight * ik.solver.IKPositionWeight,
			ik.solver.bodyEffector.positionOffset - ik.solver.pullBodyOffset);
		var quaternion = Quaternion.Lerp(Quaternion.identity, rotation, thighWeight);
		var vector3_1 = quaternion * headToLeftThigh;
		var vector3_2 = quaternion * headToRightThigh;
		LerpSolverPosition(ik.solver.leftThighEffector, transform.position + vector3_1,
			positionWeight * ik.solver.IKPositionWeight,
			ik.solver.bodyEffector.positionOffset - ik.solver.pullBodyOffset +
			ik.solver.leftThighEffector.positionOffset);
		LerpSolverPosition(ik.solver.rightThighEffector, transform.position + vector3_2,
			positionWeight * ik.solver.IKPositionWeight,
			ik.solver.bodyEffector.positionOffset - ik.solver.pullBodyOffset +
	...
	private void OnPostUpdate() {
		if (!enabled || !gameObject.activeInHierarchy)
			return;
		PostStretching();
		ik.references.head.rotation = Quaternion.Lerp(Quaternion.identity,
			QuaTools.ClampRotation(QuaTools.FromToRotation(ik.references.head.rotation, transform.rotation),
				headClampWeight, 2), rotationWeight * ik.solver.IKPositionWeight) * ik.references.head.rotation;
	}

	private void ChestDirection() {
		var num = chestDirectionWeight * ik.solver.IKPositionWeight;
		if (num <= 0.0)
			return;
		var changed = false;
		...
		var quaternion = Quaternion.Lerp(Quaternion.identity,
			Quaternion.FromToRotation(ik.references.root.forward, chestDirection), num * (1f / chestBones.Length));
		foreach (var chestBone in chestBones)
			chestBone.rotation = quaternion * chestBone.rotation;
	}

	private void PostStretching() {
		var num = postStretchWeight * ik.solver.IKPositionWeight;
		if (num > 0.0) {
			var vector3 = Vector3.ClampMagnitude(transform.position - ik.references.head.position, maxStretch) * num;
	...
	private void Solve(ref Vector3 pos1, ref Vector3 pos2, float nominalDistance) {
		var vector3_1 = pos2 - pos1;
		var magnitude = vector3_1.magnitude;
		if (magnitude == (double)nominalDistance || magnitude == 0.0)
			return;
		var num = 1f * (float)(1.0 - nominalDistance / (double)magnitude);
		var vector3_2 = vector3_1 * num * 0.5f;
		pos1 += vector3_2;
		pos2 -= vector3_2;
	}
	...
	public class BendBone {
		...
		[Tooltip("The weight of rotating this bone.")] [Range(0.0f, 1f)]
