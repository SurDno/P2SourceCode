...
public class FullBodyBipedIK : IK {
	...
	public bool ReferencesError(ref string errorMessage) {
		...
		if (solver.rootNode != references.pelvis) {
			...
			for (var index = 0; index < references.spine.Length; ++index)
				if (solver.rootNode == references.spine[index]) {
					flag = true;
					break;
				}
	...
	public bool ReferencesWarning(ref string warningMessage) {
		if (BipedReferences.SetupWarning(references, ref warningMessage))
			return true;
		if (Vector3.Dot((references.rightUpperArm.position - references.leftUpperArm.position).normalized,
			    (solver.rootNode.position - references.leftUpperArm.position).normalized) > 0.949999988079071) {
			warningMessage =
		...
		if (Vector3.Dot((references.rightThigh.position - references.leftThigh.position).normalized,
			    (solver.rootNode.position - references.leftThigh.position).normalized) <= 0.949999988079071)
			return false;
		warningMessage =
	...
	private void AutoDetectReferences() {
		...
		BipedReferences.AutoDetectReferences(ref references, transform,
