...
public class IKSolverFullBodyBiped : IKSolverFullBody {
	...
	[Range(0.0f, 1f)] public float spineStiffness = 0.5f;
	[Range(-1f, 1f)] public float pullBodyVertical = 0.5f;
	[Range(-1f, 1f)] public float pullBodyHorizontal;
	...
	public override bool IsValid(ref string message) {
		...
		if (chain.Length == 5 && chain[0].nodes.Length == 1 && chain[1].nodes.Length == 3 &&
		    chain[2].nodes.Length == 3 && chain[3].nodes.Length == 3 && chain[4].nodes.Length == 3 &&
		    effectors.Length == 9 && limbMappings.Length == 4)
			return true;
	...
	public void SetToReferences(BipedReferences references, Transform rootNode = null) {
		...
		var spineBones = new Transform[references.spine.Length + 1];
		...
		spineMapping.SetBones(spineBones, references.leftUpperArm, references.rightUpperArm, references.leftThigh,
			references.rightThigh);
		var length = references.head != null ? 1 : 0;
		if (boneMappings.Length != length) {
			boneMappings = new IKMappingBone[length];
			for (var index = 0; index < boneMappings.Length; ++index)
		...
		limbMappings[0].SetBones(references.leftUpperArm, references.leftForearm, references.leftHand,
			GetLeftClavicle(references));
		limbMappings[1].SetBones(references.rightUpperArm, references.rightForearm, references.rightHand,
			GetRightClavicle(references));
	...
	public static Transform DetectRootNodeBone(BipedReferences references) {
		if (!references.isFilled || references.spine.Length < 1)
			return null;
		var length = references.spine.Length;
		if (length == 1)
			return references.spine[0];
		var vector3_1 = Vector3.Lerp(references.leftThigh.position, references.rightThigh.position, 0.5f);
		var onNormal = Vector3.Lerp(references.leftUpperArm.position, references.rightUpperArm.position, 0.5f) -
		               vector3_1;
		var magnitude = onNormal.magnitude;
		if (references.spine.Length < 2)
			return references.spine[0];
		var index1 = 0;
		for (var index2 = 1; index2 < length; ++index2) {
			var vector3_2 = Vector3.Project(references.spine[index2].position - vector3_1, onNormal);
	...
	private void SetLimbOrientation(
		...
		if (chain == FullBodyBipedChain.LeftArm || chain == FullBodyBipedChain.RightArm) {
			GetBendConstraint(chain).SetLimbOrientation(-limbOrientation.upperBoneForwardAxis,
				-limbOrientation.lowerBoneForwardAxis, -limbOrientation.lastBoneLeftAxis);
			GetLimbMapping(chain).SetLimbOrientation(-limbOrientation.upperBoneForwardAxis,
				-limbOrientation.lowerBoneForwardAxis);
		} else {
			GetBendConstraint(chain).SetLimbOrientation(limbOrientation.upperBoneForwardAxis,
				limbOrientation.lowerBoneForwardAxis, limbOrientation.lastBoneLeftAxis);
			GetLimbMapping(chain)
				.SetLimbOrientation(limbOrientation.upperBoneForwardAxis, limbOrientation.lowerBoneForwardAxis);
		}
	}

	private static Transform GetLeftClavicle(BipedReferences references) {
		return references.leftUpperArm == null || Contains(references.spine, references.leftUpperArm.parent)
			? null
			: references.leftUpperArm.parent;
	}

	private static Transform GetRightClavicle(BipedReferences references) {
		return references.rightUpperArm == null || Contains(references.spine, references.rightUpperArm.parent)
			? null
			: references.rightUpperArm.parent;
	}
	...
	protected override void ReadPose() {
		for (var index = 0; index < effectors.Length; ++index)
			effectors[index].SetToTarget();
		PullBody();
		var num = Mathf.Clamp(1f - spineStiffness, 0.0f, 1f);
	...
	private void PullBody() {
		...
		pullBodyOffset = V3Tools.ExtractVertical(bodyOffset, root.up, pullBodyVertical) +
		                 V3Tools.ExtractHorizontal(bodyOffset, root.up, pullBodyHorizontal);
		bodyEffector.positionOffset += pullBodyOffset;
	}

	private Vector3 GetBodyOffset() {
		var offset = Vector3.zero + GetHandBodyPull(leftHandEffector, leftArmChain, Vector3.zero) *
			Mathf.Clamp(leftHandEffector.positionWeight, 0.0f, 1f);
		return offset + GetHandBodyPull(rightHandEffector, rightArmChain, offset) *
			Mathf.Clamp(rightHandEffector.positionWeight, 0.0f, 1f);
	}

	private Vector3 GetHandBodyPull(IKEffector effector, FBIKChain arm, Vector3 offset) {
		var vector3 = effector.position - (arm.nodes[0].transform.position + offset);
		var num1 = arm.nodes[0].length + arm.nodes[1].length;
		var magnitude = vector3.magnitude;
		if (magnitude < (double)num1)
			return Vector3.zero;
		var num2 = magnitude - num1;
	...
	protected override void ApplyBendConstraints() {
		if (iterations > 0) {
			...
			offset = Vector3.Lerp(effectors[0].positionOffset,
				effectors[0].position - (effectors[0].bone.position + effectors[0].positionOffset),
				effectors[0].positionWeight);
			for (var index = 0; index < 5; ++index) {
				var node = effectors[index].GetNode(this);
