...
public class TwistRelaxer : MonoBehaviour {
	...
	[Tooltip("The weight of relaxing the twist of this Transform")] [Range(0.0f, 1f)]
	...
	public void Relax() {
		...
		var quaternion1 = Quaternion.AngleAxis(twistAngleOffset, rotation1 * twistAxis);
		var quaternion2 = quaternion1 * rotation1;
		var vector3_1 = Vector3.Slerp(quaternion1 * parent.rotation * axisRelativeToParentDefault,
			quaternion1 * child.rotation * axisRelativeToChildDefault, parentChildCrossfade);
		var vector3_2 = Quaternion.Inverse(Quaternion.LookRotation(quaternion2 * axis, quaternion2 * twistAxis)) *
		                vector3_1;
		var num = Mathf.Atan2(vector3_2.x, vector3_2.z) * 57.29578f;
		var rotation2 = child.rotation;
	...
	private void Start() {
		...
		if (transform.childCount == 0) {
			var componentsInChildren = parent.GetComponentsInChildren<Transform>();
		...
		var vector3 = transform.rotation * axis;
