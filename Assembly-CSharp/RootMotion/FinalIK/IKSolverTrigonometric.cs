...
public class IKSolverTrigonometric : IKSolver {
	...
	[Range(0.0f, 1f)] public float IKRotationWeight = 1f;
	...
	public void SetBendGoalPosition(Vector3 goalPosition, float weight) {
		if (!initiated || weight <= 0.0)
			return;
		var b = Vector3.Cross(goalPosition - bone1.transform.position, IKPosition - bone1.transform.position);
		if (!(b != Vector3.zero))
			return;
		if (weight >= 1.0)
			bendNormal = b;
	...
	public void SetBendPlaneToCurrent() {
		if (!initiated)
			return;
		var vector3 = Vector3.Cross(bone2.transform.position - bone1.transform.position,
	...
	public static void Solve(
		...
		var vector3_1 = targetPosition - bone1.position;
		var magnitude = vector3_1.magnitude;
		if (magnitude == 0.0)
			return;
		var vector3_2 = bone2.position - bone1.position;
		var sqrMagnitude1 = vector3_2.sqrMagnitude;
		vector3_2 = bone3.position - bone2.position;
		var sqrMagnitude2 = vector3_2.sqrMagnitude;
		var bendDirection = Vector3.Cross(vector3_1, bendNormal);
		var directionToBendPoint =
			GetDirectionToBendPoint(vector3_1, magnitude, bendDirection, sqrMagnitude1, sqrMagnitude2);
		var b1 = Quaternion.FromToRotation(bone2.position - bone1.position, directionToBendPoint);
		if (weight < 1.0)
			b1 = Quaternion.Lerp(Quaternion.identity, b1, weight);
		bone1.rotation = b1 * bone1.rotation;
		var b2 = Quaternion.FromToRotation(bone3.position - bone2.position, targetPosition - bone2.position);
		if (weight < 1.0)
	...
	protected override void OnUpdate() {
		...
		if (IKPositionWeight > 0.0) {
			...
			var bendNormal = Vector3.Lerp(bone1.GetBendNormalFromCurrentRotation(), this.bendNormal, IKPositionWeight);
			var direction = Vector3.Lerp(bone2.transform.position - bone1.transform.position,
				GetBendDirection(weightIKPosition, bendNormal), IKPositionWeight);
	...
	protected Vector3 GetBendDirection(Vector3 IKPosition, Vector3 bendNormal) {
		var vector3 = IKPosition - bone1.transform.position;
		if (vector3 == Vector3.zero)
			return Vector3.zero;
		var sqrMagnitude = vector3.sqrMagnitude;
		var num = (float)Math.Sqrt(sqrMagnitude);
		var z = (float)((sqrMagnitude + (double)bone1.sqrMag - bone2.sqrMag) / 2.0) / num;
		var y = (float)Math.Sqrt(Mathf.Clamp(bone1.sqrMag - z * z, 0.0f, float.PositiveInfinity));
		var upwards = Vector3.Cross(vector3, bendNormal);
	...
	public class TrigonometricBone : Bone {
		...
		public void Initiate(Vector3 childPosition, Vector3 bendNormal) {
			targetToLocalSpace = QuaTools.RotationToLocalSpace(transform.rotation,
				Quaternion.LookRotation(childPosition - transform.position, bendNormal));
