...
public class RagdollUtility : MonoBehaviour {
	...
	public void Start() {
		...
		var componentsInChildren1 = GetComponentsInChildren<Rigidbody>();
		var num = componentsInChildren1[0].gameObject == gameObject ? 1 : 0;
		rigidbones = new Rigidbone[num == 0 ? componentsInChildren1.Length : componentsInChildren1.Length - 1];
		for (var index = 0; index < rigidbones.Length; ++index)
			rigidbones[index] = new Rigidbone(componentsInChildren1[index + num]);
		var componentsInChildren2 = GetComponentsInChildren<Transform>();
	...
	private IEnumerator DisableRagdollSmooth() {
		...
		for (var i = 0; i < allIKComponents.Length; ++i) {
	...
	private void Update() {
		...
		if (!applyIkOnRagdoll) {
			var flag = false;
			for (var index = 0; index < allIKComponents.Length; ++index)
	...
	private void RagdollEnabler() {
		...
		for (var index = 0; index < fixTransforms.Length; ++index) {
	...
	private void RecordVelocities() {
		foreach (var rigidbone in rigidbones)
	...
	private bool ikUsed {
		get {
			...
			if (ik.enabled && ik.GetIKSolver().IKPositionWeight > 0.0)
				return true;
			foreach (var allIkComponent in allIKComponents)
