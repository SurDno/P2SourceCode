...
public class GrounderFBBIK : Grounder {
	...
	private void OnSolverUpdate() {
		...
		if (spineBend != 0.0) {
			...
			var vector3 = ik.references.root.up * spineOffset.magnitude;
			for (var index = 0; index < spine.Length; ++index)
				ik.solver.GetEffector(spine[index].effectorType).positionOffset +=
					spineOffset * spine[index].horizontalWeight + vector3 * spine[index].verticalWeight;
		}
	...
	private void SetLegIK(IKEffector effector, Grounding.Leg leg) {
		...
		effector.bone.rotation =
			Quaternion.Slerp(Quaternion.identity, leg.rotationOffset, weight) * effector.bone.rotation;
	}
