...
public class IKEffector {
	...
	[Range(0.0f, 1f)] public float positionWeight;
	[Range(0.0f, 1f)] public float rotationWeight;
	public Vector3 position = Vector3.zero;
	public Quaternion rotation = Quaternion.identity;
	public Vector3 positionOffset;
	public bool effectChildNodes = true;
	[Range(0.0f, 1f)] public float maintainRelativePositionWeight;
	...
	public bool IsValid(IKSolver solver, ref string message) {
		...
		foreach (var childBone in childBones)
			if (solver.GetPoint(childBone) == null) {
				message = "IK Effector is referencing to a bone '" + childBone.name +
				          "' that does not excist in the Node Chain.";
				return false;
			}
	...
	public void Initiate(IKSolverFullBody solver) {
		...
		for (var index = 0; index < childBones.Length; ++index)
	...
	public void ResetOffset(IKSolverFullBody solver) {
		...
		for (var index = 0; index < childChainIndexes.Length; ++index)
	...
	public void OnPreSolve(IKSolverFullBody solver) {
		...
		if (effectChildNodes && solver.iterations > 0)
			for (var index = 0; index < childBones.Length; ++index) {
				...
				solver.GetNode(childChainIndexes[index], childNodeIndexes[index]).offset +=
	...
	private Quaternion GetPlaneRotation(IKSolverFullBody solver) {
		var solverPosition1 = solver.GetNode(plane1ChainIndex, plane1NodeIndex).solverPosition;
		var solverPosition2 = solver.GetNode(plane2ChainIndex, plane2NodeIndex).solverPosition;
		var solverPosition3 = solver.GetNode(plane3ChainIndex, plane3NodeIndex).solverPosition;
		var forward = solverPosition2 - solverPosition1;
		var upwards = solverPosition3 - solverPosition1;
	...
	public void Update(IKSolverFullBody solver) {
		...
		if (!effectChildNodes)
			return;
		for (var index = 0; index < childBones.Length; ++index)
			solver.GetNode(childChainIndexes[index], childNodeIndexes[index]).solverPosition = Vector3.Lerp(
				solver.GetNode(childChainIndexes[index], childNodeIndexes[index]).solverPosition,
				solver.GetNode(chainIndex, nodeIndex).solverPosition + localPositions[index], posW);
	}
	...
	private Vector3 GetPosition(IKSolverFullBody solver, out Quaternion planeRotationOffset) {
		...
		var vector3_1 = bone.position - planeBone1.position;
		planeRotationOffset = GetPlaneRotation(solver) * Quaternion.Inverse(animatedPlaneRotation);
		var vector3_2 = solver.GetNode(plane1ChainIndex, plane1NodeIndex).solverPosition +
		                planeRotationOffset * vector3_1;
		planeRotationOffset = Quaternion.Lerp(Quaternion.identity, planeRotationOffset, maintainRelativePositionWeight);
		return Vector3.Lerp(animatedPosition, vector3_2 + solver.GetNode(chainIndex, nodeIndex).offset,
