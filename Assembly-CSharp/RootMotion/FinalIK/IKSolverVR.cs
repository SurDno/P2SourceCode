...
public class IKSolverVR : IKSolver {
	...
	[Tooltip("The spine solver.")] public Spine spine = new();
	[Tooltip("The left arm solver.")] public Arm leftArm = new();
	[Tooltip("The right arm solver.")] public Arm rightArm = new();
	[Tooltip("The left leg solver.")] public Leg leftLeg = new();
	[Tooltip("The right leg solver.")] public Leg rightLeg = new();
	...
	public void GuessHandOrientations(VRIK.References references, bool onlyIfZero) {
		if (!references.isFilled)
			Debug.LogWarning(
				"VRIK References are not filled in, can not guess hand orientations. Right-click on VRIK header and slect 'Guess Hand Orientations' when you have filled in the References.");
		else {
	...
	public override void StoreDefaultLocalState() {
		for (var index = 1; index < solverTransforms.Length; ++index)
	...
	public override bool IsValid(ref string message) {
		...
		if (leftArm.wristToPalmAxis == Vector3.zero) {
			message =
				"Left arm 'Wrist To Palm Axis' needs to be set in VRIK. Please select the hand bone, set it to the axis that points from the wrist towards the palm. If the arrow points away from the palm, axis must be negative.";
			return false;
		}

		if (rightArm.wristToPalmAxis == Vector3.zero) {
			message =
				"Right arm 'Wrist To Palm Axis' needs to be set in VRIK. Please select the hand bone, set it to the axis that points from the wrist towards the palm. If the arrow points away from the palm, axis must be negative.";
			return false;
		}

		if (leftArm.palmToThumbAxis == Vector3.zero) {
			message =
				"Left arm 'Palm To Thumb Axis' needs to be set in VRIK. Please select the hand bone, set it to the axis that points from the palm towards the thumb. If the arrow points away from the thumb, axis must be negative.";
			return false;
		}
		...
		if (!(rightArm.palmToThumbAxis == Vector3.zero))
			return true;
		message =
			"Right arm 'Palm To Thumb Axis' needs to be set in VRIK. Please select the hand bone, set it to the axis that points from the palm towards the thumb. If the arrow points away from the thumb, axis must be negative.";
		return false;
	}
	...
	private Vector3 GetNormal(Transform[] transforms) {
		...
		var vector3 = zero2 / transforms.Length;
		for (var index = 0; index < transforms.Length - 1; ++index)
			zero1 += Vector3.Cross(transforms[index].position - vector3, transforms[index + 1].position - vector3)
				.normalized;
		return zero1;
	}

	private Vector3 GuessWristToPalmAxis(Transform hand, Transform forearm) {
		var vector3 = forearm.position - hand.position;
		var palmAxis = AxisTools.ToVector3(AxisTools.GetAxisToDirection(hand, vector3));
	...
	private Vector3 GuessPalmToThumbAxis(Transform hand, Transform forearm) {
		if (hand.childCount == 0) {
			Debug.LogWarning(
				"Hand " + hand.name +
				" does not have any fingers, VRIK can not guess the hand bone's orientation. Please assign 'Wrist To Palm Axis' and 'Palm To Thumb Axis' manually for both arms in VRIK settings.",
				hand);
			return Vector3.zero;
		}
		...
		var index1 = 0;
		for (var index2 = 0; index2 < hand.childCount; ++index2) {
			var num2 = Vector3.SqrMagnitude(hand.GetChild(index2).position - hand.position);
			if (num2 < (double)num1) {
				num1 = num2;
				index1 = index2;
			}
		...
		var vector3 =
			Vector3.Cross(
				Vector3.Cross(hand.position - forearm.position, hand.GetChild(index1).position - hand.position),
				hand.position - forearm.position);
		var thumbAxis = AxisTools.ToVector3(AxisTools.GetAxisToDirection(hand, vector3));
	...
	private static Keyframe[] GetSineKeyframes(float mag) {
		var sineKeyframes = new Keyframe[3];
	...
	private void UpdateSolverTransforms() {
		for (var index = 0; index < solverTransforms.Length; ++index)
			if (solverTransforms[index] != null) {
		...
		for (var index = 0; index < solverTransforms.Length; ++index)
			if (solverTransforms[index] != null) {
				if ((index < 2) | (index > 5 && index < 14))
					solverTransforms[index].position = V3Tools.Lerp(solverTransforms[index].position,
						GetPosition(index), IKPositionWeight);
				solverTransforms[index].rotation = QuaTools.Lerp(solverTransforms[index].rotation, GetRotation(index),
					IKPositionWeight);
			}
	}
	...
	private void Solve() {
		...
		if (spine.pelvisPositionWeight > 0.0 && plantFeet)
			Warning.Log(
				"If VRIK 'Pelvis Position Weight' is > 0, 'Plant Feet' should be disabled to improve performance and stability.",
				root);
		if (locomotion.weight > 0.0) {
			var leftFootPosition = Vector3.zero;
			var rightFootPosition = Vector3.zero;
			var leftFootRotation = Quaternion.identity;
			var rightFootRotation = Quaternion.identity;
			var leftFootOffset = 0.0f;
			var rightFootOffset = 0.0f;
			var leftHeelOffset = 0.0f;
			var rightHeelOffset = 0.0f;
			locomotion.Solve(rootBone, spine, leftLeg, rightLeg, leftArm, rightArm, supportLegIndex,
				out leftFootPosition, out rightFootPosition, out leftFootRotation, out rightFootRotation,
				out leftFootOffset, out rightFootOffset, out leftHeelOffset, out rightHeelOffset);
			leftFootPosition += root.up * leftFootOffset;
			rightFootPosition += root.up * rightFootOffset;
			leftLeg.footPositionOffset += (leftFootPosition - leftLeg.lastBone.solverPosition) * IKPositionWeight *
			                              (1f - leftLeg.positionWeight) * locomotion.weight;
			rightLeg.footPositionOffset += (rightFootPosition - rightLeg.lastBone.solverPosition) * IKPositionWeight *
			                               (1f - rightLeg.positionWeight) * locomotion.weight;
			...
			var rotation1 = QuaTools.FromToRotation(leftLeg.lastBone.solverRotation, leftFootRotation);
			var rotation2 = QuaTools.FromToRotation(rightLeg.lastBone.solverRotation, rightFootRotation);
			var quaternion1 = Quaternion.Lerp(Quaternion.identity, rotation1,
				IKPositionWeight * (1f - leftLeg.rotationWeight) * locomotion.weight);
			var quaternion2 = Quaternion.Lerp(Quaternion.identity, rotation2,
				IKPositionWeight * (1f - rightLeg.rotationWeight) * locomotion.weight);
			...
			var plane = V3Tools.PointToPlane(
				Vector3.Lerp(leftLeg.position + leftLeg.footPositionOffset,
					rightLeg.position + rightLeg.footPositionOffset, 0.5f), rootBone.solverPosition, root.up);
			rootBone.solverPosition =
				Vector3.Lerp(rootBone.solverPosition + rootVelocity * Time.deltaTime * 2f * locomotion.weight, plane,
					Time.deltaTime * locomotion.rootSpeed * locomotion.weight);
		...
		for (var index = 0; index < legs.Length; ++index) {
			var num2 = Vector3.SqrMagnitude(legs[index].lastBone.solverPosition - legs[index].bones[0].solverPosition);
			if (num2 < (double)num1) {
	...
	private Vector3 GetPelvisOffset() {
		if (locomotion.weight <= 0.0 || locomotion.blockingLayers == -1)
			return Vector3.zero;
		var raycastOriginPelvis = this.raycastOriginPelvis with {
			y = spine.pelvis.solverPosition.y
		};
		var origin = spine.pelvis.readPosition with {
			y = spine.pelvis.solverPosition.y
		};
		var direction1 = origin - raycastOriginPelvis;
		RaycastHit hitInfo;
		if (locomotion.raycastRadius <= 0.0) {
			if (Physics.Raycast(raycastOriginPelvis, direction1, out hitInfo, direction1.magnitude * 1.1f,
				    locomotion.blockingLayers))
				origin = hitInfo.point;
		} else if (Physics.SphereCast(raycastOriginPelvis, locomotion.raycastRadius * 1.1f, direction1, out hitInfo,
			           direction1.magnitude, locomotion.blockingLayers))
			origin = raycastOriginPelvis + direction1.normalized * hitInfo.distance / 1.1f;

		var vector3 = spine.pelvis.solverPosition;
		var direction2 = vector3 - origin;
		if (locomotion.raycastRadius <= 0.0) {
			if (Physics.Raycast(origin, direction2, out hitInfo, direction2.magnitude, locomotion.blockingLayers))
				vector3 = hitInfo.point;
		} else if (Physics.SphereCast(origin, locomotion.raycastRadius, direction2, out hitInfo, direction2.magnitude,
			           locomotion.blockingLayers))
	...
	public class Arm : BodyPart {
		[Tooltip("The hand target")] public Transform target;

		[Tooltip("The elbow will be bent towards this Transform if 'Bend Goal Weight' > 0.")]
		public Transform bendGoal;

		[Tooltip("Positional weight of the hand target.")] [Range(0.0f, 1f)]
		public float positionWeight = 1f;

		[Tooltip("Rotational weight of the hand target")] [Range(0.0f, 1f)]
		public float rotationWeight = 1f;

		[Tooltip("Different techniques for shoulder bone rotation.")]
		public ShoulderRotationMode shoulderRotationMode = ShoulderRotationMode.YawPitch;

		[Tooltip("The weight of shoulder rotation")] [Range(0.0f, 1f)]
		public float shoulderRotationWeight = 1f;

		[Tooltip("If greater than 0, will bend the elbow towards the 'Bend Goal' Transform.")] [Range(0.0f, 1f)]
		public float bendGoalWeight;

		[Tooltip("Angular offset of the elbow bending direction.")] [Range(-180f, 180f)]
		...
		[Tooltip("Use this to make the arm shorter/longer.")] [Range(0.01f, 2f)]
		...
		[HideInInspector] [NonSerialized] public Vector3 IKPosition;
		[HideInInspector] [NonSerialized] public Quaternion IKRotation = Quaternion.identity;
		[HideInInspector] [NonSerialized] public Vector3 bendDirection = Vector3.back;
		[HideInInspector] [NonSerialized] public Vector3 handPositionOffset;
		...
		protected override void OnRead(
			...
			var rotation1 = rotations[index];
			var position2 = positions[index + 1];
			var rotation2 = rotations[index + 1];
			var position3 = positions[index + 2];
			var rotation3 = rotations[index + 2];
			var position4 = positions[index + 3];
			var rotation4 = rotations[index + 3];
			if (!initiated) {
		...
		private void Stretching() {
			var num1 = upperArm.length + forearm.length;
			var zero1 = Vector3.zero;
			var zero2 = Vector3.zero;
			if (armLengthMlp != 1.0) {
				num1 *= armLengthMlp;
				var vector3_1 = (forearm.solverPosition - upperArm.solverPosition) * (armLengthMlp - 1f);
				var vector3_2 = (hand.solverPosition - forearm.solverPosition) * (armLengthMlp - 1f);
				forearm.solverPosition += vector3_1;
				hand.solverPosition += vector3_1 + vector3_2;
			}

			var num2 = stretchCurve.Evaluate(Vector3.Distance(upperArm.solverPosition, position) / num1) *
			           positionWeight;
			var vector3_3 = (forearm.solverPosition - upperArm.solverPosition) * num2;
			var vector3_4 = (hand.solverPosition - forearm.solverPosition) * num2;
		...
		public void Solve(bool isLeft) {
			...
			if (hasShoulder && shoulderRotationWeight > 0.0)
				switch (shoulderRotationMode) {
					case ShoulderRotationMode.YawPitch:
						var vector3_1 = position - shoulder.solverPosition;
						vector3_1 = vector3_1.normalized;
						var num1 = isLeft ? 45f : -45f;
						var rotation1 = Quaternion.AngleAxis((isLeft ? -90f : 90f) + num1, chestUp) * chestRotation;
						var lhs = Quaternion.Inverse(rotation1) * vector3_1;
						var num2 = Mathf.Atan2(lhs.x, lhs.z) * 57.29578f *
							(1f - Mathf.Abs(Vector3.Dot(lhs, Vector3.up))) - num1;
						var num3 = isLeft ? -20f : -50f;
						var num4 = isLeft ? 50f : 20f;
						var angle1 = DamperValue(num2, num3 - num1, num4 - num1, 0.7f);
						var rotation2 = Quaternion.FromToRotation(shoulder.solverRotation * shoulder.axis,
							rotation1 * (Quaternion.AngleAxis(angle1, Vector3.up) * Vector3.forward));
						var quaternion1 = Quaternion.AngleAxis(isLeft ? -90f : 90f, chestUp) * chestRotation;
						var rotation3 = Quaternion.AngleAxis(isLeft ? -30f : 30f, chestForward) * quaternion1;
						vector3_1 = position - (shoulder.solverPosition +
						                        chestRotation * (isLeft ? Vector3.right : Vector3.left) * mag);
						var vector3_2 = Quaternion.Inverse(rotation3) * vector3_1;
						var num5 = DamperValue(Mathf.Atan2(vector3_2.y, vector3_2.z) * 57.29578f - -30f, -15f, 75f);
						var quaternion2 = Quaternion.AngleAxis(-num5, rotation3 * Vector3.right) * rotation2;
						if (shoulderRotationWeight * (double)positionWeight < 1.0)
							quaternion2 = Quaternion.Lerp(Quaternion.identity, quaternion2,
								shoulderRotationWeight * positionWeight);
						...
						var angle2 = Mathf.Clamp(num5 * 2f * positionWeight, 0.0f, 180f);
						shoulder.solverRotation =
							Quaternion.AngleAxis(angle2,
					...
					case ShoulderRotationMode.FromTo:
						var solverRotation = shoulder.solverRotation;
						...
						VirtualBone.SolveTrigonometric(bones, 1, 2, 3, position,
							GetBendNormal(position - upperArm.solverPosition), positionWeight);
						var quaternion3 = Quaternion.Inverse(Quaternion.LookRotation(chestUp, chestForward));
						var vector3_3 = quaternion3 * (solverRotation * shoulder.axis);
						var vector3_4 = quaternion3 * (shoulder.solverRotation * shoulder.axis);
						var num6 = Mathf.DeltaAngle(Mathf.Atan2(vector3_3.x, vector3_3.z) * 57.29578f,
							Mathf.Atan2(vector3_4.x, vector3_4.z) * 57.29578f);
						if (isLeft)
							num6 = -num6;
						var angle3 = Mathf.Clamp(num6 * 2f * positionWeight, 0.0f, 180f);
						shoulder.solverRotation =
							Quaternion.AngleAxis(angle3,
								shoulder.solverRotation * (isLeft ? shoulder.axis : -shoulder.axis)) *
							shoulder.solverRotation;
						upperArm.solverRotation =
							Quaternion.AngleAxis(angle3,
			...
			var quaternion4 = upperArm.solverRotation * forearmRelToUpperArm;
			RotateTo(forearm,
				Quaternion.FromToRotation(quaternion4 * forearm.axis, hand.solverPosition - forearm.solverPosition) *
				quaternion4, positionWeight);
		...
		private float DamperValue(float value, float min, float max, float weight = 1f) {
			var num1 = max - min;
			if (weight < 1.0) {
				var num2 = max - num1 * 0.5f;
				var num3 = (value - num2) * 0.5f;
				value = num2 + num3;
			}

			value -= min;
			var t = Interp.Float(Mathf.Clamp(value / num1, 0.0f, 1f), InterpolationMode.InOutQuintic);
		...
		private Vector3 GetBendNormal(Vector3 dir) {
			...
			var vector3_1 = bones[0].solverRotation * bones[0].axis;
			var vector3_2 =
				Quaternion.FromToRotation(Vector3.down,
					Quaternion.Inverse(chestRotation) * dir.normalized + Vector3.forward) * Vector3.back;
			var vector3_3 =
				chestRotation * (Quaternion.FromToRotation(Quaternion.Inverse(chestRotation) * vector3_1,
					Quaternion.Inverse(chestRotation) * dir) * vector3_2) + vector3_1 - rotation * wristToPalmAxis -
				rotation * palmToThumbAxis * 0.5f;
	...
	public abstract class BodyPart {
		...
		public void MovePosition(Vector3 position) {
			var vector3 = position - bones[0].solverPosition;
			foreach (var bone in bones)
				bone.solverPosition += vector3;
		}

		public void MoveRotation(Quaternion rotation) {
			VirtualBone.RotateAroundPoint(bones, 0, bones[0].solverPosition,
				QuaTools.FromToRotation(bones[0].solverRotation, rotation));
		}
		...
		public void TranslateRoot(Vector3 newRootPos, Quaternion newRootRot) {
			var vector3 = newRootPos - rootPosition;
			rootPosition = newRootPos;
			foreach (var bone in bones)
				bone.solverPosition += vector3;
			var rotation = QuaTools.FromToRotation(rootRotation, newRootRot);
		...
		public void RotateTo(VirtualBone bone, Quaternion rotation, float weight = 1f) {
			if (weight <= 0.0)
				return;
			var quaternion = QuaTools.FromToRotation(bone.solverRotation, rotation);
			if (weight < 1.0)
	...
	public class Footstep {
		...
		public void UpdateStanding(Quaternion rootRotation, float minAngle, float speed) {
			if (speed <= 0.0 || minAngle >= 180.0)
				return;
			var quaternion = rootRotation * footRelativeToRoot;
			var num = Quaternion.Angle(rotation, quaternion);
			if (num <= (double)minAngle)
				return;
			rotation = Quaternion.RotateTowards(rotation, quaternion,
				Mathf.Min((float)(Time.deltaTime * (double)speed * (1.0 - supportLegW)), num - minAngle));
		}
		...
		public void Update(InterpolationMode interpolation, UnityEvent onStep) {
			...
			if (stepProgress >= 1.0)
				onStep.Invoke();
			var t = Interp.Float(stepProgress, interpolation);
	...
	public class Leg : BodyPart {
		[Tooltip("The toe/foot target.")] public Transform target;

		[Tooltip("The knee will be bent towards this Transform if 'Bend Goal Weight' > 0.")]
		public Transform bendGoal;

		[Tooltip("Positional weight of the toe/foot target.")] [Range(0.0f, 1f)]
		public float positionWeight;

		[Tooltip("Rotational weight of the toe/foot target.")] [Range(0.0f, 1f)]
		public float rotationWeight;

		[Tooltip("If greater than 0, will bend the knee towards the 'Bend Goal' Transform.")] [Range(0.0f, 1f)]
		public float bendGoalWeight;

		[Tooltip("Angular offset of the knee bending direction.")] [Range(-180f, 180f)]
		public float swivelOffset;

		[HideInInspector] [NonSerialized] public Vector3 IKPosition;
		[HideInInspector] [NonSerialized] public Quaternion IKRotation = Quaternion.identity;
		[HideInInspector] [NonSerialized] public Vector3 footPositionOffset;
		[HideInInspector] [NonSerialized] public Vector3 heelPositionOffset;
		[HideInInspector] [NonSerialized] public Quaternion footRotationOffset = Quaternion.identity;
		[HideInInspector] [NonSerialized] public float currentMag;
		...
		public override void PreSolve() {
			...
			bendNormal = Vector3.Cross(calf.solverPosition - thigh.solverPosition,
				foot.solverPosition - calf.solverPosition);
		}
		...
		public override void ApplyOffsets() {
			...
			var rotation =
				Quaternion.FromToRotation(footPosition - position, footPosition + heelPositionOffset - position);
			...
			if (bendGoal != null && bendGoalWeight > 0.0) {
				var vector3_1 = Vector3.Cross(bendGoal.position - thigh.solverPosition,
					position - thigh.solverPosition);
				var vector3_2 =
					Quaternion.Inverse(Quaternion.LookRotation(bendNormal,
						thigh.solverPosition - foot.solverPosition)) * vector3_1;
				num = Mathf.Atan2(vector3_2.x, vector3_2.z) * 57.29578f * bendGoalWeight;
			}

			var angle = swivelOffset + num;
			if (angle == 0.0)
				return;
			bendNormal = Quaternion.AngleAxis(angle, thigh.solverPosition - lastBone.solverPosition) * bendNormal;
			thigh.solverRotation =
				Quaternion.AngleAxis(-angle, thigh.solverRotation * thigh.axis) * thigh.solverRotation;
		}
		...
		public void Solve() {
			...
			if (!hasToes)
				return;
			VirtualBone.SolveTrigonometric(bones, 0, 2, 3, position,
				Vector3.Cross(foot.solverPosition - thigh.solverPosition, toes.solverPosition - foot.solverPosition),
				1f);
			var quaternion = thigh.solverRotation * calfRelToThigh;
			calf.solverRotation =
				Quaternion.FromToRotation(quaternion * calf.axis, foot.solverPosition - calf.solverPosition) *
				quaternion;
	...
	public class Locomotion {
		[Tooltip("Used for blending in/out of procedural locomotion.")] [Range(0.0f, 1f)]
		...
		[Tooltip("The speed of steps.")] public float stepSpeed = 3f;
		...
		public void AddDeltaRotation(Quaternion delta, Vector3 pivot) {
			var vector3_1 = lastComPosition - pivot;
			...
			foreach (var footstep in footsteps) {
				...
				var vector3_2 = footstep.position - pivot;
				footstep.position = pivot + delta * vector3_2;
				var vector3_3 = footstep.stepFrom - pivot;
				footstep.stepFrom = pivot + delta * vector3_3;
				var vector3_4 = footstep.stepTo - pivot;
		...
		public void Solve(
			...
			if (weight <= 0.0) {
				...
				var vector3_1 = rootBone.solverRotation * Vector3.up;
				var vector3_2 = spine.pelvis.solverPosition +
				                spine.pelvis.solverRotation * leftLeg.thighRelativeToPelvis;
				var vector3_3 = spine.pelvis.solverPosition +
				...
				var faceDirection = spine.faceDirection;
				var vertical = V3Tools.ExtractVertical(faceDirection, vector3_1, 1f);
				var quaternion = Quaternion.LookRotation(faceDirection - vertical, vector3_1);
				var num1 = 1f;
				var num2 = 1f;
				var num3 = 0.2f;
				var num4 = (float)(num1 + (double)num2 + 2.0 * num3);
				...
				var point = centerOfMass + comVelocity;
				var plane1 = V3Tools.PointToPlane(spine.pelvis.solverPosition, rootBone.solverPosition, vector3_1);
				var plane2 = V3Tools.PointToPlane(point, rootBone.solverPosition, vector3_1);
				var vector3_4 = Vector3.Lerp(footsteps[0].position, footsteps[1].position, 0.5f);
				var num5 = Vector3.Angle(point - vector3_4, rootBone.solverRotation * Vector3.up) * comAngleMlp;
				for (var index = 0; index < footsteps.Length; ++index)
					footsteps[index].isSupportLeg = supportLegIndex == index;
				for (var index = 0; index < footsteps.Length; ++index)
					if (footsteps[index].isStepping) {
						var vector3_5 = plane2 + rootBone.solverRotation * footsteps[index].characterSpaceOffset;
				...
				if (CanStep()) {
					var index1 = -1;
					var num6 = float.NegativeInfinity;
					for (var index2 = 0; index2 < footsteps.Length; ++index2)
						if (!footsteps[index2].isStepping) {
							var vector3_6 = plane2 + rootBone.solverRotation * footsteps[index2].characterSpaceOffset;
							var num7 = index2 == 0 ? leftLeg.mag : rightLeg.mag;
							var b = index2 == 0 ? vector3_2 : vector3_3;
							var num8 = Vector3.Distance(footsteps[index2].position, b);
							var flag1 = false;
							...
							var flag2 = false;
							for (var index3 = 0; index3 < footsteps.Length; ++index3)
								if (index3 != index2 && !flag1) {
									if (Vector3.Distance(footsteps[index2].position, footsteps[index3].position) >=
									    0.25 || (footsteps[index2].position - vector3_6).sqrMagnitude >=
									    (double)(footsteps[index3].position - vector3_6).sqrMagnitude)
										flag2 = GetLineSphereCollision(footsteps[index2].position, vector3_6,
											footsteps[index3].position, 0.25f);
									if (flag2)
										break;
								}

							var num9 = Quaternion.Angle(quaternion, footsteps[index2].stepToRootRot);
							if (!flag2 || num9 > (double)angleThreshold) {
								var num10 = Vector3.Distance(footsteps[index2].position, vector3_6);
								var num11 = Mathf.Lerp(stepThreshold, stepThreshold * 0.1f, num5 * 0.015f);
								if (flag1)
									num11 *= 0.5f;
								if (index2 == 0)
									num11 *= 0.9f;
								if (!StepBlocked(footsteps[index2].position, vector3_6, rootBone.solverPosition) &&
								    (num10 > (double)num11 || num9 > (double)angleThreshold)) {
									var num12 = 0.0f - num10;
					...
					if (index1 != -1) {
						var p = plane2 + rootBone.solverRotation * footsteps[index1].characterSpaceOffset;
		...
		private bool StepBlocked(Vector3 fromPosition, Vector3 toPosition, Vector3 rootPosition) {
			if (blockingLayers == -1 || !blockingEnabled)
				return false;
			var origin = fromPosition with {
				y = rootPosition.y + raycastHeight + raycastRadius
			};
			var direction = (toPosition - origin) with {
				y = 0.0f
			};
			RaycastHit hitInfo;
			return raycastRadius <= 0.0
				? Physics.Raycast(origin, direction, out hitInfo, direction.magnitude, blockingLayers)
				: Physics.SphereCast(origin, raycastRadius, direction, out hitInfo, direction.magnitude,
					blockingLayers);
		}
		...
		private static bool GetLineSphereCollision(
			...
			float sphereRadius) {
			var forward = lineEnd - lineStart;
			var upwards = sphereCenter - lineStart;
			var num = upwards.magnitude - sphereRadius;
			if (num > (double)forward.magnitude)
				return false;
			var vector3 = Quaternion.Inverse(Quaternion.LookRotation(forward, upwards)) * upwards;
	...
	public class Spine : BodyPart {
		[Tooltip("The head target.")] public Transform headTarget;

		[Tooltip("The pelvis target, useful with seated rigs.")]
		public Transform pelvisTarget;

		[Tooltip("Positional weight of the head target.")] [Range(0.0f, 1f)]
		public float positionWeight = 1f;

		[Tooltip("Rotational weight of the head target.")] [Range(0.0f, 1f)]
		public float rotationWeight = 1f;

		[Tooltip("Positional weight of the pelvis target.")] [Range(0.0f, 1f)]
		public float pelvisPositionWeight;

		[Tooltip("Rotational weight of the pelvis target.")] [Range(0.0f, 1f)]
		public float pelvisRotationWeight;

		[Tooltip("If 'Chest Goal Weight' is greater than 0, the chest will be turned towards this Transform.")]
		public Transform chestGoal;

		[Tooltip("Rotational weight of the chest target.")] [Range(0.0f, 1f)]
		public float chestGoalWeight;

		[Tooltip("Minimum height of the head from the root of the character.")]
		public float minHeadHeight = 0.8f;

		[Tooltip("Determines how much the body will follow the position of the head.")] [Range(0.0f, 1f)]
		public float bodyPosStiffness = 0.55f;

		[Tooltip("Determines how much the body will follow the rotation of the head.")] [Range(0.0f, 1f)]
		...
		[Tooltip("The amount of rotation applied to the chest based on hand positions.")] [Range(0.0f, 1f)]
		public float rotateChestByHands = 1f;

		[Tooltip("Clamps chest rotation.")] [Range(0.0f, 1f)]
		public float chestClampWeight = 0.5f;

		[Tooltip("Clamps head rotation.")] [Range(0.0f, 1f)]
		...
		[Tooltip("How much will the pelvis maintain it's animated position?")] [Range(0.0f, 1f)]
		...
		[HideInInspector] [NonSerialized] public Vector3 IKPositionHead;
		[HideInInspector] [NonSerialized] public Quaternion IKRotationHead = Quaternion.identity;
		[HideInInspector] [NonSerialized] public Vector3 IKPositionPelvis;
		[HideInInspector] [NonSerialized] public Quaternion IKRotationPelvis = Quaternion.identity;
		[HideInInspector] [NonSerialized] public Vector3 goalPositionChest;
		[HideInInspector] [NonSerialized] public Vector3 pelvisPositionOffset;
		[HideInInspector] [NonSerialized] public Vector3 chestPositionOffset;
		[HideInInspector] [NonSerialized] public Vector3 headPositionOffset;
		[HideInInspector] [NonSerialized] public Quaternion pelvisRotationOffset = Quaternion.identity;
		[HideInInspector] [NonSerialized] public Quaternion chestRotationOffset = Quaternion.identity;
		[HideInInspector] [NonSerialized] public Quaternion headRotationOffset = Quaternion.identity;
		[HideInInspector] [NonSerialized] public Vector3 faceDirection;
		[HideInInspector] [NonSerialized] public Vector3 locomotionHeadPositionOffset;
		[HideInInspector] [NonSerialized] public Vector3 headPosition;
		...
		protected override void OnRead(
			...
			var position4 = positions[index + 3];
			var rotation4 = rotations[index + 3];
			var position5 = positions[index + 4];
			var rotation5 = rotations[index + 4];
			if (!hasChest) {
				position3 = position2;
				rotation3 = rotation2;
			}
		...
		public override void ApplyOffsets() {
			...
			var vector3 = rootRotation * Vector3.up;
			if (vector3 == Vector3.up)
				headPosition.y = Math.Max(rootPosition.y + minHeadHeight, headPosition.y);
			else {
				var v = headPosition - rootPosition;
				var horizontal = V3Tools.ExtractHorizontal(v, vector3, 1f);
				var lhs = v - horizontal;
		...
		public void Solve(
			...
			if (this.maxRootAngle < 180.0) {
				var vector3 = Quaternion.Inverse(rootBone.solverRotation) * faceDirection;
				var num = Mathf.Atan2(vector3.x, vector3.z) * 57.29578f;
				var angle = 0.0f;
				var maxRootAngle = this.maxRootAngle;
				if (num > (double)maxRootAngle)
					angle = num - maxRootAngle;
				if (num < -(double)maxRootAngle)
					angle = num + maxRootAngle;
				rootBone.solverRotation = Quaternion.AngleAxis(angle, rootBone.readRotation * Vector3.up) *
				                          rootBone.solverRotation;
			}

			var solverPosition = pelvis.solverPosition;
			...
			if (chestGoalWeight > 0.0)
				Bend(bones, pelvisIndex, chestIndex,
					Quaternion.FromToRotation(bones[chestIndex].solverRotation * chestForward,
						goalPositionChest - bones[chestIndex].solverPosition) * bones[chestIndex].solverRotation,
					chestRotationOffset, chestClampWeight, false, chestGoalWeight);
		...
		private void FABRIKPass(Vector3 animatedPelvisPos, Vector3 rootUp) {
			VirtualBone.SolveFABRIK(bones,
				Vector3.Lerp(pelvis.solverPosition, animatedPelvisPos, maintainPelvisPosition) + pelvisPositionOffset -
				chestPositionOffset, headPosition - chestPositionOffset, 1f, 1f, 1, mag,
				rootUp * (bones[bones.Length - 1].solverPosition - bones[0].solverPosition).magnitude);
		}

		private void SolvePelvis() {
			if (pelvisPositionWeight <= 0.0)
				return;
			var solverRotation = head.solverRotation;
			var vector3 = (IKPositionPelvis + pelvisPositionOffset - pelvis.solverPosition) * pelvisPositionWeight;
			foreach (var bone in bones)
				bone.solverPosition += vector3;
			var bendNormal = anchorRotation * Vector3.right;
			if (hasChest && hasNeck) {
				VirtualBone.SolveTrigonometric(bones, pelvisIndex, spineIndex, headIndex, headPosition, bendNormal,
					pelvisPositionWeight * 0.6f);
				VirtualBone.SolveTrigonometric(bones, spineIndex, chestIndex, headIndex, headPosition, bendNormal,
					pelvisPositionWeight * 0.6f);
				VirtualBone.SolveTrigonometric(bones, chestIndex, neckIndex, headIndex, headPosition, bendNormal,
					pelvisPositionWeight * 1f);
			} else if (hasChest && !hasNeck) {
				VirtualBone.SolveTrigonometric(bones, pelvisIndex, spineIndex, headIndex, headPosition, bendNormal,
					pelvisPositionWeight * 0.75f);
				VirtualBone.SolveTrigonometric(bones, spineIndex, chestIndex, headIndex, headPosition, bendNormal,
					pelvisPositionWeight * 1f);
			} else if (!hasChest && hasNeck) {
				VirtualBone.SolveTrigonometric(bones, pelvisIndex, spineIndex, headIndex, headPosition, bendNormal,
					pelvisPositionWeight * 0.75f);
				VirtualBone.SolveTrigonometric(bones, spineIndex, neckIndex, headIndex, headPosition, bendNormal,
					pelvisPositionWeight * 1f);
		...
		private void AdjustChestByHands(ref Quaternion chestTargetRotation, Arm[] arms) {
			var quaternion1 = Quaternion.Inverse(anchorRotation);
			var vector3_1 = quaternion1 * (arms[0].position - headPosition) / sizeMlp;
			var vector3_2 = quaternion1 * (arms[1].position - headPosition) / sizeMlp;
			...
			var quaternion2 =
				Quaternion.AngleAxis(Mathf.Atan2(forward.x, forward.z) * 57.29578f, rootRotation * Vector3.up);
			chestTargetRotation = quaternion2 * chestTargetRotation;
			var up = Vector3.up;
			up.x += vector3_1.y;
			up.x -= vector3_2.y;
			up.x *= 0.5f * rotateChestByHands;
			var quaternion3 = Quaternion.AngleAxis(Mathf.Atan2(up.x, up.y) * 57.29578f, rootRotation * Vector3.back);
		...
		public void InverseTranslateToHead(
			...
			Vector3 offset,
			float w) {
			var pelvisPosition = pelvis.solverPosition +
			                     (headPosition + offset - head.solverPosition) * w * (1f - pelvisPositionWeight);
		...
		private void TranslatePelvis(
			...
			Vector3 deltaPosition,
			Quaternion deltaRotation) {
			var solverPosition = head.solverPosition;
			deltaRotation = QuaTools.ClampRotation(deltaRotation, chestClampWeight, 2);
			VirtualBone.RotateAroundPoint(bones, 0, pelvis.solverPosition,
				pelvisRotationOffset *
				Quaternion.Slerp(Quaternion.Slerp(Quaternion.identity, deltaRotation, bodyRotStiffness),
					QuaTools.FromToRotation(pelvis.solverRotation, IKRotationPelvis), pelvisRotationWeight));
			deltaPosition -= head.solverPosition - solverPosition;
			var vector3 = rootRotation * Vector3.forward;
			var num = V3Tools.ExtractVertical(deltaPosition, rootRotation * Vector3.up, 1f).magnitude *
			          -moveBodyBackWhenCrouching * headHeight;
		...
		private Vector3 LimitPelvisPosition(
			...
			for (var index = 0; index < it; ++index)
				foreach (var leg in legs) {
					var vector3_1 = pelvisPosition - pelvis.solverPosition;
					var vector3_2 = leg.thigh.solverPosition + vector3_1;
					var vector = vector3_2 - leg.position;
					var maxLength = useCurrentLegMag ? leg.currentMag : leg.mag;
					var vector3_3 = leg.position + Vector3.ClampMagnitude(vector, maxLength);
		...
		private void Bend(
			...
			if (w <= 0.0 || bones.Length == 0)
				return;
			var num1 = lastIndex + 1 - firstIndex;
			if (num1 < 1)
				return;
			var b = QuaTools.ClampRotation(QuaTools.FromToRotation(bones[lastIndex].solverRotation, targetRotation),
				clampWeight, 2);
			var num2 = uniformWeight ? 1f / num1 : 0.0f;
			for (var index = firstIndex; index < lastIndex + 1; ++index) {
				if (!uniformWeight)
					num2 = Mathf.Clamp((index - firstIndex + 1) / num1, 0.0f, 1f);
				VirtualBone.RotateAroundPoint(bones, index, bones[index].solverPosition,
					Quaternion.Slerp(Quaternion.identity, b, num2 * w));
			}
		}
		...
		private void Bend(
			...
			if (w <= 0.0 || bones.Length == 0)
				return;
			var num = lastIndex + 1 - firstIndex;
			if (num < 1)
				return;
			var b = QuaTools.ClampRotation(QuaTools.FromToRotation(bones[lastIndex].solverRotation, targetRotation),
				clampWeight, 2);
			var t = uniformWeight ? 1f / num : 0.0f;
			for (var index = firstIndex; index < lastIndex + 1; ++index) {
				if (!uniformWeight)
					t = Mathf.Clamp((index - firstIndex + 1) / num, 0.0f, 1f);
				VirtualBone.RotateAroundPoint(bones, index, bones[index].solverPosition,
					Quaternion.Slerp(Quaternion.Slerp(Quaternion.identity, rotationOffset, t), b, t * w));
			}
		}
	}
	...
	public class VirtualBone {
		...
		public static void SwingRotation(
			...
			if (weight <= 0.0)
				return;
			var b = Quaternion.FromToRotation(bones[index].solverRotation * bones[index].axis,
		...
		public static void RotateAroundPoint(
			...
			for (var index1 = index; index1 < bones.Length; ++index1)
				if (bones[index1] != null) {
					var vector3 = bones[index1].solverPosition - point;
		...
		public static void RotateBy(VirtualBone[] bones, Quaternion rotation) {
			for (var index = 0; index < bones.Length; ++index)
				if (bones[index] != null) {
					if (index > 0) {
						var vector3 = bones[index].solverPosition - bones[0].solverPosition;
		...
		public static void RotateTo(VirtualBone[] bones, int index, Quaternion rotation) {
			var rotation1 = QuaTools.FromToRotation(bones[index].solverRotation, rotation);
		...
		public static void SolveTrigonometric(
			...
			var vector3 = targetPosition - bones[first].solverPosition;
			var sqrMagnitude1 = vector3.sqrMagnitude;
			if (sqrMagnitude1 == 0.0)
				return;
			var directionMag = Mathf.Sqrt(sqrMagnitude1);
			var sqrMagnitude2 = (bones[second].solverPosition - bones[first].solverPosition).sqrMagnitude;
			var sqrMagnitude3 = (bones[third].solverPosition - bones[second].solverPosition).sqrMagnitude;
			var bendDirection = Vector3.Cross(vector3, bendNormal);
			var directionToBendPoint =
				GetDirectionToBendPoint(vector3, directionMag, bendDirection, sqrMagnitude2, sqrMagnitude3);
			var quaternion1 = Quaternion.FromToRotation(bones[second].solverPosition - bones[first].solverPosition,
				directionToBendPoint);
			if (weight < 1.0)
			...
			var quaternion2 = Quaternion.FromToRotation(bones[third].solverPosition - bones[second].solverPosition,
		...
		private static Vector3 GetDirectionToBendPoint(
			...
			float sqrMag1,
			float sqrMag2) {
			var z = (float)((directionMag * (double)directionMag + (sqrMag1 - (double)sqrMag2)) / 2.0) / directionMag;
			var y = (float)Math.Sqrt(Mathf.Clamp(sqrMag1 - z * z, 0.0f, float.PositiveInfinity));
			return direction == Vector3.zero
				? Vector3.zero
				: Quaternion.LookRotation(direction, bendDirection) * new Vector3(0.0f, y, z);
		}
		...
		public static void SolveFABRIK(
			...
			if (minNormalizedTargetDistance > 0.0) {
				var vector3 = targetPosition - startPosition;
				var magnitude = vector3.magnitude;
				targetPosition = startPosition +
				                 vector3 / magnitude * Mathf.Max(length * minNormalizedTargetDistance, magnitude);
			}
			...
			for (var index1 = 0; index1 < iterations; ++index1) {
				bones[bones.Length - 1].solverPosition =
					Vector3.Lerp(bones[bones.Length - 1].solverPosition, targetPosition, weight);
				for (var index2 = bones.Length - 2; index2 > -1; --index2)
					bones[index2].solverPosition = SolveFABRIKJoint(bones[index2].solverPosition,
						bones[index2 + 1].solverPosition, bones[index2].length);
				bones[0].solverPosition = startPosition;
				for (var index3 = 1; index3 < bones.Length; ++index3)
					bones[index3].solverPosition = SolveFABRIKJoint(bones[index3].solverPosition,
						bones[index3 - 1].solverPosition, bones[index3 - 1].length);
			}
			...
			for (var index1 = 0; index1 < iterations; ++index1) {
				for (var index2 = bones.Length - 2; index2 > -1; --index2) {
					var rotation = Quaternion.FromToRotation(
						bones[bones.Length - 1].solverPosition - bones[index2].solverPosition,
						targetPosition - bones[index2].solverPosition);
					if (weight >= 1.0)
