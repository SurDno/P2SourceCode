...
public class Grounding {
	...
	[Tooltip("Offset of the foot center along character forward axis.")] [HideInInspector]
	public float footCenterOffset;

	[Tooltip("Amount of velocity based prediction of the foot positions.")]
	public float prediction = 0.05f;

	[Tooltip("Weight of rotating the feet to the ground normal offset.")] [Range(0.0f, 1f)]
	public float footRotationWeight = 1f;

	[Tooltip("Speed of slerping the feet to their grounded rotations.")]
	public float footRotationSpeed = 7f;

	[Tooltip("Max Foot Rotation Angle. Max angular offset from the foot's rotation.")] [Range(0.0f, 90f)]
	...
	[Tooltip("Used for smoothing out vertical pelvis movement (range 0 - 1).")] [Range(0.0f, 1f)]
	...
	public RaycastHit GetRootHit(float maxDistanceMlp = 10f) {
		...
		var vector3 = zero / legs.Length;
		hitInfo.point = vector3 - up * maxStep * 10f;
		var num = maxDistanceMlp + 1f;
		...
		if (quality != Quality.Best)
			...
			Physics.SphereCast(vector3 + up * maxStep, rootSphereCastRadius, -this.up, out hitInfo, maxStep * num,
				layers);
		return hitInfo;
	}
	...
	public Vector3 GetLegsPlaneNormal() {
		if (!initiated)
			return Vector3.up;
		var up = this.up;
		var legsPlaneNormal = up;
		for (var index = 0; index < legs.Length; ++index) {
			var toDirection = legs[index].IKPosition - root.position;
			var normal = up;
			var tangent = toDirection;
	...
	public class Leg {
		...
		public void Process() {
			...
			lastPosition = transform.position;
			var offsetFromHeel = velocity * grounding.prediction;
			if (grounding.footRadius <= 0.0)
				grounding.quality = Quality.Fastest;
			switch (grounding.quality) {
				case Quality.Fastest:
					var raycastHit = GetRaycastHit(offsetFromHeel);
					SetFootToPoint(raycastHit.normal, raycastHit.point);
					break;
				case Quality.Simple:
					heelHit = GetRaycastHit(Vector3.zero);
					var vector3_1 = grounding.GetFootCenterOffset();
					if (invertFootCenter)
						vector3_1 = -vector3_1;
					var vector3_2 = Vector3.Cross(GetRaycastHit(vector3_1 + offsetFromHeel).point - heelHit.point,
							GetRaycastHit(grounding.root.right * grounding.footRadius * 0.5f).point - heelHit.point)
						.normalized;
			...
			IKOffset = Mathf.Clamp(IKOffset,
				-Mathf.Clamp(
					grounding.maxStep - grounding.GetVerticalOffset(transform.position, grounding.root.position), 0.0f,
					grounding.maxStep), IKOffset);
			RotateFoot();
			IKPosition = transform.position - up * IKOffset;
			var footRotationWeight = grounding.footRotationWeight;
			rotationOffset = footRotationWeight >= 1.0
				? r
		...
		private RaycastHit GetCapsuleHit(Vector3 offsetFromHeel) {
			...
			if (invertFootCenter)
				vector3_1 = -vector3_1;
			var vector3_2 = transform.position + vector3_1;
			hitInfo.point = vector3_2 - up * grounding.maxStep * 2f;
			hitInfo.normal = up;
			var point1 = vector3_2 + grounding.maxStep * up;
			var point2 = point1 + offsetFromHeel;
			if (Physics.CapsuleCast(point1, point2, grounding.footRadius, -up, out hitInfo, grounding.maxStep * 3f,
				    grounding.layers) && float.IsNaN(hitInfo.point.x)) {
				hitInfo.point = vector3_2 - up * grounding.maxStep * 2f;
				hitInfo.normal = up;
			}
		...
		private RaycastHit GetRaycastHit(Vector3 offsetFromHeel) {
			var hitInfo = new RaycastHit();
			var vector3 = transform.position + offsetFromHeel;
			hitInfo.point = vector3 - up * grounding.maxStep * 2f;
			hitInfo.normal = up;
			if (grounding.maxStep <= 0.0)
				return hitInfo;
			Physics.Raycast(vector3 + grounding.maxStep * up, -up, out hitInfo, grounding.maxStep * 3f,
				grounding.layers);
			return hitInfo;
		}

		private Vector3 RotateNormal(Vector3 normal) {
			return grounding.quality == Quality.Best
				? normal
				: Vector3.RotateTowards(up, normal, grounding.maxFootRotationAngle * ((float)Math.PI / 180f),
					deltaTime);
		}
		...
		private void SetFootToPlane(Vector3 planeNormal, Vector3 planePoint, Vector3 heelHitPoint) {
			...
			heightFromGround = GetHeightFromGround(V3Tools.LineToPlane(transform.position + up * grounding.maxStep, -up,
		...
		private Quaternion GetRotationOffsetTarget() {
			...
			return grounding.maxFootRotationAngle >= 180.0
				? toHitNormal
				: Quaternion.RotateTowards(Quaternion.identity, toHitNormal, grounding.maxFootRotationAngle);
		}

		private float rootYOffset =>
			grounding.GetVerticalOffset(transform.position, grounding.root.position - up * grounding.heightOffset);
	}
	...
	public class Pelvis {
		...
		public void Process(float lowestOffset, float highestOffset, bool isGrounded) {
			...
			if (num <= 0.0)
				return;
			var b = lowestOffset + highestOffset;
			if (!grounding.rootGrounded)
				b = 0.0f;
			heightOffset = Mathf.Lerp(heightOffset, b, num * grounding.pelvisSpeed);
			var p1 = grounding.root.position - lastRootPosition;
