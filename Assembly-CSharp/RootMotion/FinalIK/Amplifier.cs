	...
	public class Body {
		...
		public void Update(IKSolverFullBodyBiped solver, float w, float deltaTime) {
			if (transform == null || relativeTo == null)
				return;
			var vector3_1 = relativeTo.InverseTransformDirection(transform.position - relativeTo.position);
			if (firstUpdate) {
				lastRelativePos = vector3_1;
				firstUpdate = false;
			}

			var b = (vector3_1 - lastRelativePos) / deltaTime;
			smoothDelta = speed <= 0.0 ? b : Vector3.Lerp(smoothDelta, b, deltaTime * speed);
			var v = relativeTo.TransformDirection(smoothDelta);
			var vector3_2 = V3Tools.ExtractVertical(v, solver.GetRoot().up, verticalWeight) +
			                V3Tools.ExtractHorizontal(v, solver.GetRoot().up, horizontalWeight);
			for (var index = 0; index < effectorLinks.Length; ++index)
				solver.GetEffector(effectorLinks[index].effector).positionOffset +=
					vector3_2 * w * effectorLinks[index].weight;
			lastRelativePos = vector3_1;
		}
