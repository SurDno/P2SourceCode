	...
	private void LateUpdate() {
		...
		if (ik.solver.leftHandEffector.target != null) {
			ik.solver.leftArmChain.bendConstraint.direction =
				ik.solver.leftHandEffector.target.rotation * Vector3.left +
		...
		if (!(ik.solver.rightHandEffector.target != null))
			return;
		ik.solver.rightArmChain.bendConstraint.direction = ik.solver.rightHandEffector.target.rotation * Vector3.right +
		                                                   ik.solver.rightHandEffector.target.rotation *
		                                                   bendDirectionOffsetRight + ik.transform.rotation *
		                                                   characterSpaceBendOffsetRight;
