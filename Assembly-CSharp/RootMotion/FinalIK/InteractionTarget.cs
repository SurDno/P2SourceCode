...
public class InteractionTarget : MonoBehaviour {
	...
	public void RotateTo(Vector3 position) {
		...
		pivot.localRotation = defaultLocalRotation;
		if (twistWeight > 0.0) {
			var tangent1 = transform.position - pivot.position;
			var axis = pivot.rotation * twistAxis;
			var normal1 = axis;
			Vector3.OrthoNormalize(ref normal1, ref tangent1);
			var normal2 = axis;
			var tangent2 = position - pivot.position;
			Vector3.OrthoNormalize(ref normal2, ref tangent2);
			pivot.rotation =
				Quaternion.Lerp(Quaternion.identity, QuaTools.FromToAroundAxis(tangent1, tangent2, axis), twistWeight) *
				pivot.rotation;
		}

		if (swingWeight <= 0.0)
			return;
		pivot.rotation = Quaternion.Lerp(Quaternion.identity,
			                 Quaternion.FromToRotation(transform.position - pivot.position, position - pivot.position),
			                 swingWeight) *
		                 pivot.rotation;
	}
