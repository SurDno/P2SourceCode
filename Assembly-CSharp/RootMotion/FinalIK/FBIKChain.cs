...
public class FBIKChain {
	[Range(0.0f, 1f)] public float pin;
	[Range(0.0f, 1f)] public float pull = 1f;
	[Range(0.0f, 1f)] public float push;
	[Range(-1f, 1f)] public float pushParent;
	[Range(0.0f, 1f)] public float reach = 0.1f;
	...
	public void ReadPose(IKSolverFullBody solver, bool fullBody) {
		...
		if (!fullBody)
			return;
		for (var index = 0; index < childConstraints.Length; ++index)
			childConstraints[index].OnPreSolve(solver);
		if (children.Length != 0) {
			var effectorPositionWeight = nodes[nodes.Length - 1].effectorPositionWeight;
			for (var index = 0; index < children.Length; ++index)
				effectorPositionWeight += solver.chain[children[index]].nodes[0].effectorPositionWeight *
				                          solver.chain[children[index]].pull;
			var num = Mathf.Clamp(effectorPositionWeight, 1f, float.PositiveInfinity);
			for (var index = 0; index < children.Length; ++index)
				crossFades[index] = solver.chain[children[index]].nodes[0].effectorPositionWeight *
					solver.chain[children[index]].pull / num;
		}
	...
	private void CalculateBoneLengths(IKSolverFullBody solver) {
		...
		for (var index = 0; index < nodes.Length - 1; ++index) {
			nodes[index].length =
				Vector3.Distance(nodes[index].transform.position, nodes[index + 1].transform.position);
			length += nodes[index].length;
			if (nodes[index].length == 0.0) {
				Warning.Log(
					"Bone " + nodes[index].transform.name + " - " + nodes[index + 1].transform.name +
					" length is zero, can not solve.", nodes[index].transform);
				return;
			}
		}

		for (var index = 0; index < children.Length; ++index) {
			solver.chain[children[index]].rootLength = (solver.chain[children[index]].nodes[0].transform.position -
	...
	public void Reach(IKSolverFullBody solver) {
		...
		if (reachForce <= 0.0)
			return;
		var vector3_1 = nodes[2].solverPosition - nodes[0].solverPosition;
		if (vector3_1 == Vector3.zero)
			return;
		var magnitude = vector3_1.magnitude;
		var vector3_2 = vector3_1 / magnitude * length;
		var num = Mathf.Clamp(Mathf.Clamp(magnitude / length, 1f - reachForce, 1f + reachForce) - 1f + reachForce, -1f,
			1f);
		...
		var vector3_3 = vector3_2 * Mathf.Clamp(num, 0.0f, magnitude);
		var node1 = nodes[0];
	...
	public Vector3 Push(IKSolverFullBody solver) {
		...
		if (magnitude == 0.0)
			return Vector3.zero;
		var num = (float)(1.0 - magnitude / (double)distance);
		if (num <= 0.0)
			return Vector3.zero;
		...
		var vector3_2 = -vector3_1 * num * push;
		var node2 = nodes[0];
	...
	public void SolveTrigonometric(IKSolverFullBody solver, bool calculateBendDirection = false) {
		...
		if (nodes.Length != 3)
			return;
		var vector3 = nodes[2].solverPosition - nodes[0].solverPosition;
		var magnitude = vector3.magnitude;
		if (magnitude == 0.0)
			return;
		var directionMagnitude = Mathf.Clamp(magnitude, 0.0f, length * 0.99999f);
		nodes[1].solverPosition = nodes[0].solverPosition + GetDirToBendPoint(vector3 / magnitude * directionMagnitude,
			!calculateBendDirection || !bendConstraint.initiated
				? nodes[1].solverPosition - nodes[0].solverPosition
				: bendConstraint.GetDir(solver), directionMagnitude);
	}
	...
	public void Stage1(IKSolverFullBody solver) {
		...
		if (children.Length == 0)
			...
			var solverPosition = nodes[nodes.Length - 1].solverPosition;
			SolveChildConstraints(solver);
			for (var index = 0; index < children.Length; ++index) {
				var vector3 = solver.chain[children[index]].nodes[0].solverPosition;
				if (solver.chain[children[index]].rootLength > 0.0)
					vector3 = SolveFABRIKJoint(nodes[nodes.Length - 1].solverPosition,
						solver.chain[children[index]].nodes[0].solverPosition,
						solver.chain[children[index]].rootLength);
				if (pullParentSum > 0.0)
					solverPosition += (vector3 - nodes[nodes.Length - 1].solverPosition) *
					                  (solver.chain[children[index]].pull / pullParentSum);
			}
	...
	public void Stage2(IKSolverFullBody solver, Vector3 position) {
		...
		var num = Mathf.Clamp(solver.iterations, 2, 4);
	...
	public void SolveConstraintSystems(IKSolverFullBody solver) {
		SolveChildConstraints(solver);
		for (var index = 0; index < children.Length; ++index)
			SolveLinearConstraint(nodes[nodes.Length - 1], solver.chain[children[index]].nodes[0], crossFades[index],
				solver.chain[children[index]].rootLength);
	}
	...
	protected Vector3 GetDirToBendPoint(
		...
		var z = (float)((directionMagnitude * (double)directionMagnitude + sqrMagDif) / 2.0) / directionMagnitude;
	...
	private void SolveLinearConstraint(
		...
		float crossFade,
		float distance) {
		var vector3_1 = node2.solverPosition - node1.solverPosition;
		var magnitude = vector3_1.magnitude;
		if (distance == (double)magnitude || magnitude == 0.0)
			return;
		var vector3_2 = vector3_1 * (float)(1.0 - distance / (double)magnitude);
		var node3 = node1;
	...
	public void ForwardReach(Vector3 position) {
		...
		for (var index = nodes.Length - 2; index > -1; --index)
			nodes[index].solverPosition = SolveFABRIKJoint(nodes[index].solverPosition, nodes[index + 1].solverPosition,
				nodes[index].length);
	}
	...
	public class ChildConstraint {
		...
		public void OnPreSolve(IKSolverFullBody solver) {
			...
			crossFade = !isRigid
				? 0.5f
				: (float)(1.0 - (0.5 + (solver.chain[chain1Index].pull - solver.chain[chain2Index].pull) * 0.5));
		...
		public void Solve(IKSolverFullBody solver) {
			if (pushElasticity >= 1.0 && pullElasticity >= 1.0)
				return;
			var vector3_1 = solver.chain[chain2Index].nodes[0].solverPosition -
			...
			var num2 = num1 * (float)(1.0 - nominalDistance / (double)magnitude);
			var vector3_2 = vector3_1 * num2;
			var node1 = solver.chain[chain1Index].nodes[0];
			node1.solverPosition = node1.solverPosition + vector3_2 * crossFade;
			var node2 = solver.chain[chain2Index].nodes[0];
