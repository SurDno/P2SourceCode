...
public class SubBehaviourBalancer : SubBehaviourBase {
	...
	private void Solve() {
		if (copPoints.Length == 0)
			...
			cop = Vector3.zero;
			foreach (var copPoint in copPoints)
		...
		var v = (PhysXTools.GetFromToAcceleration(dirVel, -Physics.gravity) - Ibody.angularVelocity) /
		        Time.fixedDeltaTime;
		PhysXTools.ScaleByInertia(ref v, Ibody.rotation, Ibody.inertiaTensor * settings.IMlp);
		var vector3 = Vector3.ClampMagnitude(v, settings.maxTorqueMag);
		if (pressureSensor == null || !pressureSensor.enabled || pressureSensor.inContact) {
			...
			joint.targetAngularVelocity = Quaternion.Inverse(toJointSpace) *
			                              Quaternion.Inverse(joint.transform.rotation) * vector3;
		} else
