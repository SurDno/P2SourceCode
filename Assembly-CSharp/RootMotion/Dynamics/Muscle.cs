...
public class Muscle {
	...
	public bool IsValid(bool log) {
		...
		if (target == null) {
			if (log)
				Debug.LogError("Muscle " + joint.name +
				               "target is null, please remove the muscle from PuppetMaster or disable PuppetMaster before destroying a muscle's target.");
			return false;
		}
	...
	public virtual void Initiate(Muscle[] colleagues) {
		...
		if (joint.connectedBody != null)
			for (var index = 0; index < colleagues.Length; ++index)
		...
		var normalized1 = Vector3.Cross(joint.axis, joint.secondaryAxis).normalized;
		var normalized2 = Vector3.Cross(normalized1, joint.axis).normalized;
		if (normalized1 == normalized2)
			Debug.LogError("Joint " + joint.name +
			               " secondaryAxis is in the exact same direction as it's axis. Please make sure they are not aligned.");
		else {
			rotationRelativeToTarget = Quaternion.Inverse(target.rotation) * transform.rotation;
			var rotation = Quaternion.LookRotation(normalized1, normalized2);
			...
			if (!joint.gameObject.activeInHierarchy)
				...
				if (joint.anchor != Vector3.zero)
					Debug.LogError(
						"PuppetMaster joint anchors need to be Vector3.zero. Joint axis on " + transform.name + " is " +
						joint.anchor, transform);
				else {
	...
	public void UpdateColliders() {
		...
		var childCount = joint.transform.childCount;
	...
	private void AddColliders(Transform t, ref Collider[] C, bool includeMeshColliders) {
		var components = t.GetComponents<Collider>();
		var num1 = 0;
		...
		for (var index = 0; index < components.Length; ++index) {
			var flag = components[index] is MeshCollider;
	...
	public void IgnoreCollisions(Muscle m, bool ignore) {
		foreach (var collider1 in colliders) {
			foreach (var collider2 in m.colliders)
				if (collider1 != null && collider2 != null && collider1.enabled && collider2.enabled &&
				    collider1.gameObject.activeInHierarchy && collider2.gameObject.activeInHierarchy)
	...
	public void Read() {
		var num = Time.time - lastReadTime;
		lastReadTime = Time.time;
		if (num > 0.0) {
			targetVelocity = (target.position - targetAnimatedPosition) / num;
			targetAngularVelocity =
				QuaTools.FromToRotation(targetAnimatedWorldRotation, target.rotation).eulerAngles / num;
		}
	...
	public void UpdateAnchor(bool supportTranslationAnimation) {
		if (joint.connectedBody == null || connectedBodyTarget == null ||
		    (directTargetParent && !supportTranslationAnimation))
			return;
		joint.connectedAnchor =
			(joint.connectedAnchor = InverseTransformPointUnscaled(connectedBodyTarget.position,
				connectedBodyTarget.rotation * toParentSpace, target.position)) *
			(1f / connectedBodyTransform.lossyScale.x);
	}
	...
	public void Map(float mappingWeightMaster) {
		var t = props.mappingWeight * mappingWeightMaster * state.mappingWeightMlp;
		if (t <= 0.0)
			return;
		...
		if (t >= 1.0) {
			...
			if (!props.mapPosition)
				return;
			target.position = !(connectedBodyTransform != null)
				? position
				: connectedBodyTarget.TransformPoint(connectedBodyTransform.InverseTransformPoint(position));
		} else {
			target.rotation = Quaternion.Lerp(target.rotation, rotation * targetRotationRelative, t);
			if (!props.mapPosition)
				return;
			target.position = !(connectedBodyTransform != null)
				? Vector3.Lerp(target.position, position, t)
				: Vector3.Lerp(target.position,
					connectedBodyTarget.TransformPoint(connectedBodyTransform.InverseTransformPoint(position)), t);
		}
	}

	public void CalculateMappedVelocity() {
		var num = Time.time - lastWriteTime;
		if (num > 0.0) {
	...
	private void Pin(float pinWeightMaster, float pinPow, float pinDistanceFalloff) {
		...
		var f = pinWeightMaster * props.pinWeight * state.pinWeightMlp;
		if (f <= 0.0)
			return;
		var vector3 = (-rigidbody.velocity + targetVelocity + positionOffset / Time.fixedDeltaTime) *
		              Mathf.Pow(f, pinPow);
	...
	private void MuscleRotation(float muscleWeightMaster, float muscleSpring, float muscleDamper) {
		var a = (float)(muscleWeightMaster * (double)props.muscleWeight * muscleSpring * state.muscleWeightMlp * 10.0);
		if (joint.connectedBody == null)
			a = 0.0f;
		else if (a > 0.0)
			joint.targetRotation = LocalToJointSpace(targetAnimatedRotation);
		var b = props.muscleDamper * muscleDamper * state.muscleDamperMlp + state.muscleDamperAdd;
	...
	private Quaternion targetLocalRotation =>
		Quaternion.Inverse(targetParentRotation * toParentSpace) * target.rotation;
	...
	private Vector3 CalculateInertiaTensorCuboid(Vector3 size, float mass) {
		var num1 = Mathf.Pow(size.x, 2f);
		var num2 = Mathf.Pow(size.y, 2f);
		var num3 = Mathf.Pow(size.z, 2f);
		var num4 = 0.0833333358f * mass;
	...
	public class Props {
		...
		[Tooltip("The weight (multiplier) of mapping this muscle's target to the muscle.")] [Range(0.0f, 1f)]
		...
		[Tooltip("The muscle strength (multiplier).")] [Range(0.0f, 1f)]
		public float muscleWeight = 1f;

		[Tooltip("Multiplier of the positionDamper of the ConfigurableJoints' Slerp Drive.")] [Range(0.0f, 1f)]
