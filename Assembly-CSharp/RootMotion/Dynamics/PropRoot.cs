...
public class PropRoot : MonoBehaviour {
	...
	private void OpenScriptReference() {
		Application.OpenURL(
			"http://root-motion.com/puppetmasterdox/html/class_root_motion_1_1_dynamics_1_1_prop_root.html");
	}
	...
	private void Update() {
		if (!fixedUpdateCalled || !(currentProp != null) || !(lastProp == currentProp) ||
		    !(currentProp.muscle.connectedBody == null))
			return;
		currentProp.Drop();
		currentProp = null;
		lastProp = null;
	...
	private void AttachProp(Prop prop) {
		...
		puppetMaster.AddMuscle(prop.muscle, prop.transform, connectTo, transform, prop.muscleProps,
			forceLayers: prop.forceLayers);
		if (!(prop.additionalPin != null) || !(prop.additionalPinTarget != null))
			return;
		puppetMaster.AddMuscle(prop.additionalPin, prop.additionalPinTarget, prop.muscle.GetComponent<Rigidbody>(),
			prop.transform, new Muscle.Props(prop.additionalPinWeight, 0.0f, 0.0f, 0.0f, false, Muscle.Group.Prop),
			true, prop.forceLayers);
	}
}