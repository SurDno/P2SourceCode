...
public class MuscleCollisionBroadcaster : MonoBehaviour {
	...
	private void OnCollisionStay(Collision collision) {
		if (!enabled || puppetMaster == null ||
		    (Singleton<PuppetMasterSettings>.instance != null &&
		     !Singleton<PuppetMasterSettings>.instance.collisionStayMessages) ||
	...
	private void OnCollisionExit(Collision collision) {
		if (!enabled || puppetMaster == null ||
		    (Singleton<PuppetMasterSettings>.instance != null &&
		     !Singleton<PuppetMasterSettings>.instance.collisionExitMessages) ||
		    collision.collider.transform.root == transform.root)
			return;
		foreach (var behaviour in puppetMaster.behaviours)
