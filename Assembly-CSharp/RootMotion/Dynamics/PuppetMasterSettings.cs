...
public class PuppetMasterSettings : Singleton<PuppetMasterSettings> {
	[Header("Optimizations")] public PuppetUpdateLimit kinematicCollidersUpdateLimit = new();
	...
	private List<PuppetMaster> _puppets = new();
	...
	public class PuppetUpdateLimit {
		[Range(1f, 100f)] public int puppetsPerFrame;
		private int index;
		...
		public bool Update(List<PuppetMaster> puppets, PuppetMaster puppetMaster) {
			...
			if (index >= puppets.Count)
				return false;
			for (var index1 = 0; index1 < puppetsPerFrame; ++index1) {
