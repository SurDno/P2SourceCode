...
public class BehaviourAnimatedStagger : BehaviourBase {
	[Header("Master Properties")] public LayerMask groundLayers;
	...
	[Header("Muscle Group Properties")] public FallParams defaults;
	public FallParamsGroup[] groupOverrides;
	[Header("Events")] public PuppetEvent onUngrounded;
	...
	private IEnumerator LoseBalance() {
		...
		var done = false;
		while (!done) {
			var vel = Quaternion.Inverse(puppetMaster.targetRoot.rotation) * centerOfMass.direction * animationMag;
			...
			for (var index = 0; index < muscleArray2.Length; ++index) {
				...
				var fallParams = GetFallParams(m.props.group);
				m.state.pinWeightMlp =
					Mathf.MoveTowards(m.state.pinWeightMlp, 0.0f, Time.deltaTime * fallParams.losePinSpeed);
				m.state.mappingWeightMlp =
					Mathf.MoveTowards(m.state.mappingWeightMlp, 1f, Time.deltaTime * animationBlendSpeed);
				m = null;
			}

			muscleArray2 = null;
			done = true;
			var muscleArray3 = puppetMaster.muscles;
			...
			muscleArray3 = null;
			if (puppetMaster.muscles[0].rigidbody.position.y - (double)puppetMaster.targetRoot.position.y <
			    minHipHeight)
				done = true;
			yield return null;
			vel = new Vector3();
		...
		if (!isGrounded) {
			var muscleArray4 = puppetMaster.muscles;
			for (var index = 0; index < muscleArray4.Length; ++index) {
				var m = muscleArray4[index];
		...
		var muscleArray5 = puppetMaster.muscles;
		for (var index = 0; index < muscleArray5.Length; ++index) {
			var m = muscleArray5[index];
			m.state.pinWeightMlp = 0.0f;
