...
public class PuppetMaster : MonoBehaviour {
	...
	[Tooltip("The normalized strength of the muscles.")] [Range(0.0f, 1f)]
	public float muscleWeight = 1f;

	[LargeHeader("Joint and Muscle Settings")] [Tooltip("The positionSpring of the ConfigurableJoints' Slerp Drive.")]
	...
	[LargeHeader("Individual Muscle Settings")] [Tooltip("The Muscles managed by this PuppetMaster.")]
	...
	[HideInInspector] public List<SolverManager> solvers = new();
	...
	private void OpenSetupTutorial() {
		Application.OpenURL(
			"https://www.youtube.com/watch?v=mIN9bxJgfOU&index=2&list=PLVxSIA1OaTOuE2SB9NUbckQ9r2hTg4mvL");
	}
	...
	public UpdateMode updateMode => targetUpdateMode == AnimatorUpdateMode.AnimatePhysics
		? isLegacy ? UpdateMode.AnimatePhysics : UpdateMode.FixedUpdate
		: UpdateMode.Normal;
		...
		for (var index = 0; index < muscles.Length; ++index) {
			...
			if (behaviours.Length != 0) {
				...
				if (muscles[index].broadcaster == null)
					muscles[index].broadcaster =
						muscles[index].joint.gameObject.AddComponent<MuscleCollisionBroadcaster>();
			...
			if (double.PositiveInfinity != muscles[index].joint.breakForce) {
				muscles[index].jointBreakBroadcaster =
					muscles[index].joint.gameObject.GetComponent<JointBreakBroadcaster>();
				if (muscles[index].jointBreakBroadcaster == null)
					muscles[index].jointBreakBroadcaster =
						muscles[index].joint.gameObject.AddComponent<JointBreakBroadcaster>();
	...
	private void ActivateBehaviour(BehaviourBase behaviour) {
		foreach (var behaviour1 in behaviours) {
	...
	protected virtual void FixedUpdate() {
		...
		if (!isFrozen) {
			...
			for (var index = 0; index < muscles.Length; ++index)
				muscles[index].Update(pinWeight, muscleWeight, muscleSpring, muscleDamper, pinPow, pinDistanceFalloff,
					true);
		}
	...
	protected virtual void OnLateUpdate() {
		...
		if (!isFrozen) {
			...
			var mappingWeightMaster = mappingWeight * mappingBlend;
	...
	private void MoveToTarget() {
		if (!(Singleton<PuppetMasterSettings>.instance == null) &&
		    (!(Singleton<PuppetMasterSettings>.instance != null) ||
		     !Singleton<PuppetMasterSettings>.instance.UpdateMoveToTarget(this)))
			return;
		foreach (var muscle in muscles)
			muscle.MoveToTarget();
	}
	...
	private void Read() {
		if (teleport) {
			...
			var parent2 = targetRoot.parent;
			transform.parent = gameObject.transform;
			targetRoot.parent = gameObject.transform;
			var position = transform.parent.position;
			var rotation = QuaTools.FromToRotation(targetRoot.rotation, teleportRotation);
			transform.parent.rotation = rotation * transform.parent.rotation;
			var deltaPosition = teleportPosition - targetRoot.position;
	...
	private AnimatorUpdateMode targetUpdateMode {
		get {
			...
			return targetAnimation != null
				? targetAnimation.animatePhysics ? AnimatorUpdateMode.AnimatePhysics : AnimatorUpdateMode.Normal
	...
	private void SetInternalCollisions(bool collide) {
		if (internalCollisionsEnabled == collide)
			return;
		for (var index1 = 0; index1 < muscles.Length; ++index1) {
			for (var index2 = index1; index2 < muscles.Length; ++index2)
				if (index1 != index2)
	...
	public void AddMuscle(
		...
		if (!initiated)
			...
			m.joint = joint;
			m.target = target;
			m.joint.transform.parent = (!hierarchyIsFlat && !(connectTo == null)) || forceTreeHierarchy
				? connectTo.transform
				: transform;
			if (forceLayers) {
			...
			if (connectTo != null) {
				...
				var position = GetMuscle(connectTo).transform.InverseTransformPoint(m.target.position);
				var quaternion = Quaternion.Inverse(GetMuscle(connectTo).transform.rotation) * m.target.rotation;
	...
	public void RemoveMuscleRecursive(
		...
		if (joint == null)
			...
			var muscleIndex = GetMuscleIndex(joint);
			var muscleArray = new Muscle[muscles.Length - (muscles[muscleIndex].childIndexes.Length + 1)];
			var index1 = 0;
			for (var index2 = 0; index2 < muscles.Length; ++index2)
			...
			switch (removeMode) {
				case MuscleRemoveMode.Sever:
					...
					for (var index3 = 0; index3 < muscles[muscleIndex].childIndexes.Length; ++index3)
						KillJoint(muscles[muscles[muscleIndex].childIndexes[index3]].joint);
					break;
				case MuscleRemoveMode.Explode:
					DisconnectJoint(muscles[muscleIndex].joint);
					for (var index4 = 0; index4 < muscles[muscleIndex].childIndexes.Length; ++index4)
				...
				case MuscleRemoveMode.Numb:
					...
					for (var index5 = 0; index5 < muscles[muscleIndex].childIndexes.Length; ++index5)
						KillJoint(muscles[muscles[muscleIndex].childIndexes[index5]].joint);
					break;
			}

			muscles[muscleIndex].transform.parent = null;
			for (var index6 = 0; index6 < muscles[muscleIndex].childIndexes.Length; ++index6)
				if (removeMode == MuscleRemoveMode.Explode ||
				    muscles[muscles[muscleIndex].childIndexes[index6]].transform.parent == transform)
					muscles[muscles[muscleIndex].childIndexes[index6]].transform.parent = null;
			foreach (var behaviour in behaviours) {
				behaviour.OnMuscleRemoved(muscles[muscleIndex]);
				for (var index7 = 0; index7 < muscles[muscleIndex].childIndexes.Length; ++index7) {
					var muscle = muscles[muscles[muscleIndex].childIndexes[index7]];
			...
			if (attachTarget) {
				...
				muscles[muscleIndex].target.rotation = muscles[muscleIndex].transform.rotation *
				                                       muscles[muscleIndex].targetRotationRelative;
				for (var index8 = 0; index8 < muscles[muscleIndex].childIndexes.Length; ++index8) {
					var muscle = muscles[muscles[muscleIndex].childIndexes[index8]];
			...
			if (blockTargetAnimation) {
				...
				for (var index9 = 0; index9 < muscles[muscleIndex].childIndexes.Length; ++index9)
					muscles[muscles[muscleIndex].childIndexes[index9]].target.gameObject
						.AddComponent<AnimationBlocker>();
			}

			if (OnMuscleRemoved != null)
				OnMuscleRemoved(muscles[muscleIndex]);
			for (var index10 = 0; index10 < muscles[muscleIndex].childIndexes.Length; ++index10) {
				var muscle = muscles[muscles[muscleIndex].childIndexes[index10]];
				if (OnMuscleRemoved != null)
					OnMuscleRemoved(muscle);
			}

			if (!internalCollisionsEnabled)
				foreach (var muscle in muscleArray) {
					foreach (var collider1 in muscle.colliders) {
						foreach (var collider2 in muscles[muscleIndex].colliders)
							Physics.IgnoreCollision(collider1, collider2, false);
						for (var index11 = 0; index11 < muscles[muscleIndex].childIndexes.Length; ++index11)
							foreach (var collider3 in muscles[index11].colliders)
	...
	public void TreeHierarchy() {
		foreach (var muscle in muscles)
			if (muscle.joint != null)
				muscle.joint.transform.parent = muscle.joint.connectedBody != null
					? muscle.joint.connectedBody.transform
					: transform;
	...
	private void AddIndexesRecursive(int index, ref int[] indexes) {
		...
		if (muscles[index].childIndexes.Length == 0)
			return;
		for (var index1 = 0; index1 < muscles[index].childIndexes.Length; ++index1)
	...
	private IEnumerator DisabledToActive() {
		...
		SetInternalCollisions(internalCollisions);
		Read();
		var muscleArray3 = muscles;
		for (var index = 0; index < muscleArray3.Length; ++index) {
			var m = muscleArray3[index];
			m.MoveToTarget();
			m = null;
		}
	...
	private IEnumerator ActiveToDisabled() {
		...
		var muscleArray = muscles;
		for (var index = 0; index < muscleArray.Length; ++index) {
			var m = muscleArray[index];
	...
	public void SetMuscleWeights(
		...
		if (!CheckIfInitiated())
			return;
		var muscleIndex = GetMuscleIndex(target);
		if (muscleIndex == -1)
			return;
	...
	public void SetMuscleWeights(
		...
		if (!CheckIfInitiated())
			return;
		var muscleIndex = GetMuscleIndex(humanBodyBone);
		if (muscleIndex == -1)
			return;
		SetMuscleWeights(muscleIndex, muscleWeight, pinWeight, mappingWeight, muscleDamper);
	}
	...
	public void SetMuscleWeightsRecursive(
		...
		if (!CheckIfInitiated())
			return;
		for (var muscleIndex = 0; muscleIndex < muscles.Length; ++muscleIndex)
	...
	public void SetMuscleWeightsRecursive(
		...
		if (!CheckIfInitiated())
			return;
		SetMuscleWeights(muscleIndex, muscleWeight, pinWeight, mappingWeight, muscleDamper);
		for (var index = 0; index < muscles[muscleIndex].childIndexes.Length; ++index)
			SetMuscleWeights(muscles[muscleIndex].childIndexes[index], muscleWeight, pinWeight, mappingWeight,
				muscleDamper);
	}
	...
	public Muscle GetMuscle(Rigidbody rigidbody) {
		var muscleIndex = GetMuscleIndex(rigidbody);
		return muscleIndex == -1 ? null : muscles[muscleIndex];
	}
	...
	public int GetMuscleIndex(HumanBodyBones humanBodyBone) {
		if (!CheckIfInitiated())
			return -1;
		if (targetAnimator == null) {
			Debug.LogWarning("PuppetMaster 'Target Root' has no Animator component on it nor on it's children.",
				transform);
			return -1;
		}
		...
		if (!targetAnimator.isHuman) {
			Debug.LogWarning(
				"PuppetMaster target's Animator does not belong to a Humanoid, can hot get human muscle index.",
				transform);
			return -1;
		}

		var boneTransform = targetAnimator.GetBoneTransform(humanBodyBone);
		if (!(boneTransform == null))
			return GetMuscleIndex(boneTransform);
		Debug.LogWarning("PuppetMaster target's Avatar does not contain a bone Transform for " + humanBodyBone,
			transform);
		return -1;
	}
	...
	public static PuppetMaster SetUp(
		...
		var puppetMaster = ragdoll.gameObject.AddComponent<PuppetMaster>();
	...
	public static PuppetMaster SetUp(
		...
		int characterControllerLayer,
		int ragdollLayer) {
		var puppetMaster = Instantiate(target.gameObject, target.position, target.rotation).transform.gameObject
			.AddComponent<PuppetMaster>();
		puppetMaster.SetUpTo(target, characterControllerLayer, ragdollLayer);
	...
	public void SetUpTo(Transform setUpTo, int characterControllerLayer, int ragdollLayer) {
		if (setUpTo == null)
			...
			var componentsInChildren = GetComponentsInChildren<Component>();
			for (var index = 0; index < componentsInChildren.Length; ++index)
				if (!(componentsInChildren[index] is PuppetMaster) && !(componentsInChildren[index] is Transform) &&
				    !(componentsInChildren[index] is Rigidbody) && !(componentsInChildren[index] is BoxCollider) &&
				    !(componentsInChildren[index] is CapsuleCollider) &&
				    !(componentsInChildren[index] is SphereCollider) &&
				    !(componentsInChildren[index] is MeshCollider) && !(componentsInChildren[index] is Joint) &&
				    !(componentsInChildren[index] is Animator))
			...
			foreach (var componentsInChild in transform.GetComponentsInChildren<Rigidbody>())
				if (componentsInChild.transform != transform &&
				    componentsInChild.GetComponent<ConfigurableJoint>() == null)
					componentsInChild.gameObject.AddComponent<ConfigurableJoint>();
			targetRoot = setUpTo;
			SetUpMuscles(setUpTo);
			name = nameof(PuppetMaster);
			var transform1 =
				setUpTo.parent == null || setUpTo.parent != transform.parent ||
				setUpTo.parent.name != setUpTo.name + " Root"
					? new GameObject(setUpTo.name + " Root").transform
					: setUpTo.parent;
			transform1.parent = transform.parent;
			var transform2 = new GameObject("Behaviours").transform;
			var comments = transform2.gameObject.GetComponent<Comments>();
			if (comments == null)
				comments = transform2.gameObject.AddComponent<Comments>();
			comments.text =
				"All Puppet Behaviours should be parented to this GameObject, the PuppetMaster will automatically find them from here. All Puppet Behaviours have been designed so that they could be simply copied from one character to another without changing any references. It is important because they contain a lot of parameters and would be otherwise tedious to set up and tweak.";
	...
	public static void RemoveRagdollComponents(Transform target, int characterControllerLayer) {
		if (target == null)
			return;
		var componentsInChildren1 = target.GetComponentsInChildren<Rigidbody>();
		var componentsInChildren2 = target.GetComponentsInChildren<Cloth>();
		for (var index = 0; index < componentsInChildren1.Length; ++index)
			if (componentsInChildren1[index].gameObject != target.gameObject) {
				var component1 = componentsInChildren1[index].GetComponent<Joint>();
				var component2 = componentsInChildren1[index].GetComponent<Collider>();
				if (component1 != null)
		...
		var componentsInChildren3 = target.GetComponentsInChildren<Collider>();
		for (var index = 0; index < componentsInChildren3.Length; ++index)
			if (componentsInChildren3[index].transform != target &&
			    !IsClothCollider(componentsInChildren3[index], componentsInChildren2))
				DestroyImmediate(componentsInChildren3[index]);
		var component = target.GetComponent<PuppetMaster>();
		if (!(component != null))
			return;
	...
	private void SetUpMuscles(Transform setUpTo) {
		var componentsInChildren1 = this.transform.GetComponentsInChildren<ConfigurableJoint>();
		if (componentsInChildren1.Length == 0)
			Debug.LogWarning(
				"No ConfigurableJoints found, can not build PuppetMaster. Please create ConfigurableJoints to connect the ragdoll bones together.",
				transform);
		else {
			var componentInChildren = targetRoot.GetComponentInChildren<Animator>();
			var componentsInChildren2 = setUpTo.GetComponentsInChildren<Transform>();
			muscles = new Muscle[componentsInChildren1.Length];
			var index1 = -1;
			for (var index2 = 0; index2 < componentsInChildren1.Length; ++index2) {
				...
				if (muscles[index2].joint.connectedBody == null && index1 == -1)
					index1 = index2;
				foreach (var transform in componentsInChildren2)
					if (transform.name == componentsInChildren1[index2].name) {
						...
						if (componentInChildren != null) {
							...
							if (muscles[index2].props.group == Muscle.Group.Hips ||
							    muscles[index2].props.group == Muscle.Group.Leg ||
							    muscles[index2].props.group == Muscle.Group.Foot)
			...
			if (index1 != 0) {
				var muscle1 = muscles[0];
				var muscle2 = muscles[index1];
			...
			foreach (var muscle in muscles) {
				if (muscle.target == null)
					Debug.LogWarning(
						"No target Transform found for PuppetMaster muscle " + muscle.joint.name +
						". Please assign manually.", transform);
	...
	private void RemoveUnnecessaryBones() {
		...
		for (var index1 = 1; index1 < componentsInChildren.Length; ++index1) {
			var flag = false;
			if (componentsInChildren[index1].GetComponent<Rigidbody>() != null ||
			    componentsInChildren[index1].GetComponent<ConfigurableJoint>() != null)
				flag = true;
			if (componentsInChildren[index1].GetComponent<Collider>() != null &&
			    componentsInChildren[index1].GetComponent<Rigidbody>() == null)
				flag = true;
			if (componentsInChildren[index1].GetComponent<CharacterController>() != null)
				flag = false;
			if (!flag) {
				var transformArray = new Transform[componentsInChildren[index1].childCount];
				for (var index2 = 0; index2 < transformArray.Length; ++index2)
					transformArray[index2] = componentsInChildren[index1].GetChild(index2);
				for (var index3 = 0; index3 < transformArray.Length; ++index3)
	...
	private static bool IsClothCollider(Collider collider, Cloth[] cloths) {
		...
		foreach (var cloth in cloths) {
			if (cloth == null)
				return false;
			foreach (var capsuleCollider in cloth.capsuleColliders)
			...
			foreach (var sphereCollider in cloth.sphereColliders)
				if ((sphereCollider.first != null && sphereCollider.first.gameObject == collider.gameObject) ||
				    (sphereCollider.second != null && sphereCollider.second.gameObject == collider.gameObject))
					return true;
		}
	...
	private IEnumerator AliveToDead(bool freeze) {
		...
		muscleArray1 = null;
		var range = muscles[0].state.muscleWeightMlp - stateSettings.deadMuscleWeight;
		var behaviourBaseArray1 = behaviours;
		for (var index = 0; index < behaviourBaseArray1.Length; ++index) {
			var behaviour = behaviourBaseArray1[index];
			behaviour.KillStart();
			behaviour = null;
		}
		...
		if (stateSettings.killDuration > 0.0 && range > 0.0) {
			var mW = muscles[0].state.muscleWeightMlp;
			while (mW > (double)stateSettings.deadMuscleWeight) {
				mW = Mathf.Max(mW - Time.deltaTime * (range / stateSettings.killDuration),
					stateSettings.deadMuscleWeight);
				var muscleArray2 = muscles;
				for (var index = 0; index < muscleArray2.Length; ++index) {
					var m = muscleArray2[index];
		...
		if (freeze)
			freezeFlag = true;
		var behaviourBaseArray2 = behaviours;
		for (var index = 0; index < behaviourBaseArray2.Length; ++index) {
			var behaviour = behaviourBaseArray2[index];
			behaviour.KillEnd();
			behaviour = null;
		}
	...
	public void FixTargetToSampledState(float weight) {
		...
		for (var index = 0; index < targetChildren.Length; ++index) {
			if (targetChildren[index] == null) {
				Debug.LogWarning(
					"PuppetMaster.UpdateTargetHierarchy() needs to be called when any of the child Transforms of the targetRoot are unparented or removed.",
					transform);
				return;
			}

			if (index == 0) {
				targetChildren[index].position =
					Vector3.Lerp(targetChildren[index].position, targetSampledPositions[index], weight);
				targetChildren[index].rotation = Quaternion.Lerp(targetChildren[index].rotation,
					targetSampledRotations[index], weight);
			} else {
				targetChildren[index].position = Vector3.Lerp(targetChildren[index].position,
					targetSampledPositions[0] + targetSampledRotations[0] * targetSampledPositions[index], weight);
				targetChildren[index].rotation = Quaternion.Lerp(targetChildren[index].rotation,
					targetSampledRotations[0] * targetSampledRotations[index], weight);
			}
		}
	...
	public void StoreTargetMappedState() {
		...
		for (var index = 0; index < targetChildren.Length; ++index)
			if (index == 0) {
				...
				targetMappedPositions[index] = Quaternion.Inverse(targetChildren[0].rotation) *
				                               (targetChildren[index].position - targetChildren[0].position);
				targetMappedRotations[index] =
					Quaternion.Inverse(targetChildren[0].rotation) * targetChildren[index].rotation;
			}
	...
	private void AssignParentAndChildIndexes() {
		for (var index1 = 0; index1 < muscles.Length; ++index1) {
			...
			if (muscles[index1].joint.connectedBody != null)
				AddToParentsRecursive(muscles[index1].joint.connectedBody.GetComponent<ConfigurableJoint>(),
					ref muscles[index1].parentIndexes);
			...
			for (var index2 = 0; index2 < muscles.Length; ++index2)
				if (index1 != index2 && muscles[index2].joint.connectedBody == muscles[index1].rigidbody)
					AddToChildrenRecursive(muscles[index2].joint, ref muscles[index1].childIndexes,
						ref muscles[index1].childFlags);
		}
	}
	...
	private void AddToParentsRecursive(ConfigurableJoint joint, ref int[] indexes) {
		if (joint == null)
			return;
		var muscleIndexLowLevel = GetMuscleIndexLowLevel(joint);
			...
			if (index1 != index && muscles[index1].rigidbody == muscles[index].joint.connectedBody) {
				...
				for (var index2 = 0; index2 < muscles.Length; ++index2)
					if (index2 != index1 && index2 != index &&
					    muscles[index2].joint.connectedBody == muscles[index1].rigidbody) {
	...
	private int GetMuscleIndexLowLevel(ConfigurableJoint joint) {
		for (var muscleIndexLowLevel = 0; muscleIndexLowLevel < muscles.Length; ++muscleIndexLowLevel)
	...
	public bool IsValid(bool log) {
		...
		if (targetRoot.position != transform.position) {
			if (log)
				Debug.LogWarning(
					"The position of the animated character (Target) must match with the position of the PuppetMaster when initiating PuppetMaster. If you are creating the Puppet in runtime, make sure you don't move the Target to another position immediatelly after instantiation. Move the Root Transform instead.");
			return false;
		}
		...
		for (var index1 = 0; index1 < muscles.Length; ++index1) {
			for (var index2 = 0; index2 < muscles.Length; ++index2)
				if (index1 != index2 &&
				    (muscles[index1] == muscles[index2] || muscles[index1].joint == muscles[index2].joint)) {
					if (log)
						Debug.LogWarning(
							"Joint " + muscles[index1].joint.name + " is used by multiple muscles (indexes " + index1 +
							" and " + index2 + "), PuppetMaster muscle setup is invalid.", transform);
					return false;
				}
		}

		if (muscles[0].joint.connectedBody != null && muscles.Length > 1)
			for (var index = 1; index < muscles.Length; ++index)
				if (muscles[index].joint.GetComponent<Rigidbody>() == muscles[0].joint.connectedBody) {
					if (log)
						Debug.LogWarning(
							"The first muscle needs to be the one that all the others are connected to (the hips).",
							transform);
					return false;
				}

		for (var index = 0; index < muscles.Length; ++index)
			if (Vector3.SqrMagnitude(muscles[index].joint.transform.position - muscles[index].target.position) >
			    1.0 / 1000.0) {
				if (log)
					Debug.LogWarning(
						"The position of each muscle needs to match with the position of it's target. Muscle '" +
						muscles[index].joint.name +
						"' position does not match with it's target. Right-click on the PuppetMaster component's header and select 'Fix Muscle Positions' from the context menu.",
						muscles[index].joint.transform);
				return false;
			}
	...
	private bool CheckMassVariation(float threshold, bool log) {
		...
		var num2 = 0.0f;
		for (var index = 0; index < muscles.Length; ++index) {
			var mass = muscles[index].joint.GetComponent<Rigidbody>().mass;
			if (mass < (double)num1)
				num1 = mass;
			if (mass > (double)num2)
				num2 = mass;
		...
		if (log)
			Debug.LogWarning(
				"Mass variation between the Rigidbodies in the ragdoll is more than " + threshold +
				" times. This might cause instability and unwanted results with Rigidbodies connected by Joints. Min mass: " +
				num1 + ", max mass: " + num2, transform);
		return false;
	}
	...
	public struct StateSettings {
		...
		[Tooltip(
			"If true, PuppetMaster, all it's behaviours and the ragdoll will be destroyed when the puppet is frozen.")]
