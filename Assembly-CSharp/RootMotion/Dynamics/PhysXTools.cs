...
public static class PhysXTools {
	...
	public static Vector3 GetFromToAcceleration(Vector3 fromV, Vector3 toV) {
		var rotation = Quaternion.FromToRotation(fromV, toV);
		var angle = 0.0f;
	...
	public static Vector3 GetAngularAcceleration(Quaternion fromR, Quaternion toR) {
		var vector3_1 = Vector3.Cross(fromR * Vector3.forward, toR * Vector3.forward);
		var vector3_2 = Vector3.Cross(fromR * Vector3.up, toR * Vector3.up);
		var num = Quaternion.Angle(fromR, toR);
	...
	public static void AddFromToTorque(Rigidbody r, Quaternion toR, ForceMode forceMode) {
		var torque = GetAngularAcceleration(r.rotation, toR) - r.angularVelocity;
		switch (forceMode) {
			case ForceMode.Force:
				var v1 = torque / Time.fixedDeltaTime;
	...
	public static void AddFromToTorque(
		...
		Vector3 toV,
		ForceMode forceMode) {
		var torque = GetFromToAcceleration(fromV, toV) - r.angularVelocity;
		switch (forceMode) {
			case ForceMode.Force:
				var v1 = torque / Time.fixedDeltaTime;
				ScaleByInertia(ref v1, r.rotation, r.inertiaTensor);
	...
	public static void AddFromToForce(
		...
		Vector3 toV,
		ForceMode forceMode) {
		var force1 = GetLinearAcceleration(fromV, toV) - r.velocity;
		switch (forceMode) {
			case ForceMode.Force:
				var force2 = force1 / Time.fixedDeltaTime * r.mass;
				r.AddForce(force2, forceMode);
				break;
			case ForceMode.Impulse:
				var force3 = force1 * r.mass;
	...
	public static Quaternion ToJointSpace(ConfigurableJoint joint) {
		var vector3 = Vector3.Cross(joint.axis, joint.secondaryAxis);
		var upwards = Vector3.Cross(vector3, joint.axis);
		return Quaternion.LookRotation(vector3, upwards);
	}

	public static Vector3 CalculateInertiaTensorCuboid(Vector3 size, float mass) {
		var num1 = size.x * size.x;
		var num2 = size.y * size.y;
		var num3 = size.z * size.z;
