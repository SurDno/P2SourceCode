...
public abstract class BehaviourBase : MonoBehaviour {
	...
	public void Activate() {
		foreach (var behaviour in puppetMaster.behaviours)
	...
	protected void RotateTargetToRootMuscle() {
		puppetMaster.targetRoot.rotation = Quaternion.LookRotation(
			(puppetMaster.muscles[0].rigidbody.rotation * (Quaternion.Inverse(puppetMaster.muscles[0].target.rotation) *
			                                               puppetMaster.targetRoot.forward)) with {
				y = 0.0f
			});
	}

	protected void TranslateTargetToRootMuscle(float maintainY) {
		puppetMaster.muscles[0].target.position = new Vector3(puppetMaster.muscles[0].transform.position.x,
			Mathf.Lerp(puppetMaster.muscles[0].transform.position.y, puppetMaster.muscles[0].target.position.y,
				maintainY), puppetMaster.muscles[0].transform.position.z);
	}
	...
	protected virtual void GroundTarget(LayerMask layers) {
		RaycastHit hitInfo;
		if (!Physics.Raycast(
			    new Ray(puppetMaster.targetRoot.position + puppetMaster.targetRoot.up, -puppetMaster.targetRoot.up),
			    out hitInfo, 4f, layers))
			return;
	...
	public struct PuppetEvent {
		...
		public void Trigger(PuppetMaster puppetMaster, bool switchBehaviourEnabled = true) {
			...
			if (!flag)
				Debug.LogWarning("No Puppet Behaviour of type '" + switchToBehaviour +
				                 "' was found. Can not switch to the behaviour, please check the spelling (also for empty spaces).");
		}
	}
