...
public class BehaviourPuppet : BehaviourBase {
	...
	public Weight collisionResistance = new(3f,
	...
	[Tooltip("An optimisation. Will only process up to this number of collisions per physics step.")] [Range(1f, 30f)]
	public int maxCollisions = 30;

	[Tooltip("How fast will the muscles of this group regain their pin weight?")] [Range(0.001f, 10f)]
	...
	[Tooltip("Smaller value makes the muscles weaker when the puppet is knocked out.")] [Range(0.0f, 1f)]
	...
	[LargeHeader("Events")] [Tooltip("Called when the character starts getting up from a prone pose (facing down).")]
	...
	protected override void OnInitiate() {
		foreach (var resistanceMultiplier in collisionResistanceMultipliers)
			if (resistanceMultiplier.layers == 0)
				Debug.LogWarning(
					"BehaviourPuppet has a Collision Resistance Multiplier that's layers is set to Nothing. Please add some layers.",
					transform);
		foreach (var muscle in puppetMaster.muscles) {
			if (muscle.joint.gameObject.layer == puppetMaster.targetRoot.gameObject.layer)
				Debug.LogWarning(
					"One of the ragdoll bones is on the same layer as the animated character. This might make the ragdoll collide with the character controller.");
			if (!Physics.GetIgnoreLayerCollision(muscle.joint.gameObject.layer,
				    puppetMaster.targetRoot.gameObject.layer))
				Debug.LogWarning("The ragdoll layer (" + muscle.joint.gameObject.layer +
				                 ") and the character controller layer (" + puppetMaster.targetRoot.gameObject.layer +
	...
	protected override void OnFixedUpdate() {
		...
		if (!puppetMaster.isActive)
			...
			foreach (var muscle in puppetMaster.muscles) {
				muscle.state.pinWeightMlp = 0.0f;
				muscle.state.mappingWeightMlp =
					Mathf.MoveTowards(muscle.state.mappingWeightMlp, 1f, Time.deltaTime * 5f);
			}
		else {
			if (hasBoosted)
				foreach (var muscle in puppetMaster.muscles) {
					muscle.state.immunity =
						Mathf.MoveTowards(muscle.state.immunity, 0.0f, Time.deltaTime * boostFalloff);
					muscle.state.impulseMlp = Mathf.Lerp(muscle.state.impulseMlp, 1f, Time.deltaTime * boostFalloff);
				}

			if (state == State.Unpinned) {
				unpinnedTimer += Time.deltaTime;
				if (unpinnedTimer >= (double)getUpDelay && canGetUp && !getupDisabled &&
				    puppetMaster.muscles[0].rigidbody.velocity.magnitude < (double)maxGetUpVelocity) {
				...
				foreach (var muscle in puppetMaster.muscles) {
					muscle.state.pinWeightMlp = 0.0f;
					muscle.state.mappingWeightMlp = Mathf.MoveTowards(muscle.state.mappingWeightMlp, 1f,
						Time.deltaTime * masterProps.mappingBlendSpeed);
				}
			}
			...
			if (state != State.Unpinned) {
				...
				foreach (var muscle in puppetMaster.muscles) {
					...
					var num1 = unpinnedMuscleKnockout
						? muscle.positionOffset.sqrMagnitude
						: muscle.positionOffset.sqrMagnitude * muscle.props.pinWeight;
					if (hasCollidedSinceGetUp && !puppetMaster.isBlending && num1 > 0.0 &&
					    muscle.state.pinWeightMlp < (double)pinWeightThreshold &&
					    num1 > props.knockOutDistance * (double)knockOutDistanceSqr * b) {
				...
				var num = 1f;
				foreach (var muscle in puppetMaster.muscles)
					if ((muscle.props.group == Muscle.Group.Leg || muscle.props.group == Muscle.Group.Foot) &&
					    muscle.state.pinWeightMlp < (double)num)
		...
		switch (masterProps.normalMode) {
			case NormalMode.Unmapped:
				if (!puppetMaster.isActive)
					break;
				var to = false;
				for (var muscleIndex = 0; muscleIndex < puppetMaster.muscles.Length; ++muscleIndex)
	...
	private bool SetKinematic() {
		if (!puppetMaster.isActive || state != 0 || puppetMaster.isBlending || getupAnimationBlendWeight > 0.0 ||
		    !puppetMaster.isAlive)
			return false;
		foreach (var muscle in puppetMaster.muscles)
	...
	protected override void OnReadBehaviour() {
		...
		if (getupAnimationBlendWeight > 0.0) {
			getUpPosition += Vector3.Project(puppetMaster.targetRoot.position - getUpPosition,
				puppetMaster.targetRoot.up);
			MoveTarget(getUpPosition);
			getupAnimationBlendWeight = Mathf.SmoothDamp(getupAnimationBlendWeight, 0.0f,
				ref getupAnimationBlendWeightV, blendToAnimationTime);
	...
	private void BlendMuscleMapping(int muscleIndex, ref bool to) {
		...
		var target = to ? state == State.Puppet ? props.maxMappingWeight : 1f : props.minMappingWeight;
		puppetMaster.muscles[muscleIndex].state.mappingWeightMlp = Mathf.MoveTowards(
			puppetMaster.muscles[muscleIndex].state.mappingWeightMlp, target,
			Time.deltaTime * masterProps.mappingBlendSpeed);
	}
	...
	private void OnDrawGizmosSelected() {
		for (var i = 0; i < groupOverrides.Length; i++) {
			...
			if (groupOverrides[i].groups.Length != 0)
				for (var j = 0; j < groupOverrides[i].groups.Length; j++) {
					if (j > 0) {
	...
	public void Boost(
		...
		if (boostParents <= 0.0 && boostChildren <= 0.0)
			...
			for (var index = 0; index < puppetMaster.muscles.Length; ++index) {
				var falloff = GetFalloff(index, muscleIndex, boostParents, boostChildren);
	...
	public void BoostImmunity(int muscleIndex, float immunity) {
		hasBoosted = true;
		puppetMaster.muscles[muscleIndex].state.immunity =
			Mathf.Clamp(immunity, puppetMaster.muscles[muscleIndex].state.immunity, 1f);
	}
	...
	public void BoostImpulseMlp(int muscleIndex, float impulseMlp) {
		hasBoosted = true;
		puppetMaster.muscles[muscleIndex].state.impulseMlp =
			Mathf.Max(impulseMlp, puppetMaster.muscles[muscleIndex].state.impulseMlp);
	}
	...
	protected override void OnMuscleCollisionBehaviour(MuscleCollision m) {
		if (!enabled || state == State.Unpinned || collisions > maxCollisions ||
		    !LayerMaskExtensions.Contains(collisionLayers, m.collision.gameObject.layer) ||
		    (masterProps.normalMode == NormalMode.Kinematic && !puppetMaster.isActive &&
		     !masterProps.activateOnStaticCollisions && m.collision.gameObject.isStatic))
			return;
		var collisionThreshold = this.collisionThreshold;
		var impulse = GetImpulse(m, ref collisionThreshold);
		if (OnCollisionImpulse != null)
			OnCollisionImpulse(m, impulse);
		var num1 = Singleton<PuppetMasterSettings>.instance != null
			? (float)(1.0 + Singleton<PuppetMasterSettings>.instance.currentlyActivePuppets *
				(double)Singleton<PuppetMasterSettings>.instance.activePuppetCollisionThresholdMlp)
			: 1f;
		var num2 = collisionThreshold * num1;
	...
	private float GetImpulse(MuscleCollision m, ref float layerThreshold) {
		var impulse = m.collision.impulse.sqrMagnitude / puppetMaster.muscles[m.muscleIndex].rigidbody.mass * 0.3f;
		foreach (var resistanceMultiplier in collisionResistanceMultipliers)
	...
	private void UnPin(int muscleIndex, float unpin) {
		...
		for (var index = 0; index < puppetMaster.muscles.Length; ++index)
			UnPinMuscle(index,
				unpin * GetFalloff(index, muscleIndex, props.unpinParents, props.unpinChildren, props.unpinGroup));
	...
	private void UnPinMuscle(int muscleIndex, float unpin) {
		...
		var num2 = collisionResistance.mode == Weight.Mode.Float
			? collisionResistance.floatValue
			: collisionResistance.GetValue(puppetMaster.muscles[muscleIndex].targetVelocity.magnitude);
		var num3 = unpin / (props.collisionResistance * num2 * num1) *
		           (1f - puppetMaster.muscles[muscleIndex].state.immunity);
	...
	private bool Activate(Collision collision, float impulse) {
		if (masterProps.normalMode != NormalMode.Kinematic || puppetMaster.mode != PuppetMaster.Mode.Kinematic ||
		    impulse < (double)masterProps.activateOnImpulse)
			return false;
	...
	private float GetFalloff(int i, int muscleIndex, float falloffParents, float falloffChildren) {
		if (i == muscleIndex)
			return 1f;
		var childFlag = puppetMaster.muscles[muscleIndex].childFlags[i];
		var kinshipDegree = puppetMaster.muscles[muscleIndex].kinshipDegrees[i];
	...
	private float GetFalloff(
		...
		float falloffGroup) {
		var a = GetFalloff(i, muscleIndex, falloffParents, falloffChildren);
		if (falloffGroup > 0.0 && i != muscleIndex && InGroup(puppetMaster.muscles[i].props.group,
			    puppetMaster.muscles[muscleIndex].props.group))
	...
	private bool InGroup(Muscle.Group group1, Muscle.Group group2) {
		...
		foreach (var groupOverride in groupOverrides) {
			foreach (var group3 in groupOverride.groups)
				if (group3 == group1)
					foreach (var group4 in groupOverride.groups)
						if (group4 == group2)
							return true;
		}
	...
	private MuscleProps GetProps(Muscle.Group group) {
		foreach (var groupOverride in groupOverrides) {
			foreach (var group1 in groupOverride.groups)
				if (group1 == group)
		...
		switch (newState) {
			...
			case State.Unpinned:
				...
				foreach (var muscle in puppetMaster.muscles) {
					...
					if (maxRigidbodyVelocity != double.PositiveInfinity)
						muscle.rigidbody.velocity =
							Vector3.ClampMagnitude(muscle.rigidbody.velocity, maxRigidbodyVelocity);
					SetColliders(muscle, true);
				}
				...
				if (dropProps)
					dropPropFlag = true;
				foreach (var muscle in puppetMaster.muscles)
					muscle.state.muscleWeightMlp = puppetMaster.isAlive
						? unpinnedMuscleWeightMlp
						: puppetMaster.stateSettings.deadMuscleWeight;
			...
			case State.GetUp:
				...
				var tangent = puppetMaster.muscles[0].rigidbody.rotation * hipsUp;
				var up = puppetMaster.targetRoot.up;
				...
				puppetMaster.SampleTargetMappedState();
				MoveTarget(puppetMaster.muscles[0].rigidbody.position +
				           puppetMaster.targetRoot.rotation * (flag ? getUpOffsetProne : getUpOffsetSupine));
				GroundTarget(groundLayers);
				getUpPosition = puppetMaster.targetRoot.position;
	...
	private void SetColliders(Muscle m, bool unpinned) {
		...
		if (unpinned)
			foreach (var collider in m.colliders) {
	...
	public struct MuscleProps {
		[Tooltip("How much will collisions with muscles of this group unpin parent muscles?")] [Range(0.0f, 1f)]
		public float unpinParents;

		[Tooltip("How much will collisions with muscles of this group unpin child muscles?")] [Range(0.0f, 1f)]
		...
		[Tooltip("If 1, muscles of this group will always be mapped to the ragdoll.")] [Range(0.0f, 1f)]
