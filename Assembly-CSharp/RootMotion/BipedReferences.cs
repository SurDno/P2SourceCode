...
public class BipedReferences {
	...
	public virtual bool isFilled {
		get {
			if (root == null || pelvis == null || leftThigh == null || leftCalf == null || leftFoot == null ||
			    rightThigh == null || rightCalf == null || rightFoot == null || leftUpperArm == null ||
			    leftForearm == null || leftHand == null || rightUpperArm == null || rightForearm == null ||
			    rightHand == null)
				return false;
	...
	public virtual bool IsEmpty(bool includeRoot) {
		if ((includeRoot && root != null) || pelvis != null || head != null || leftThigh != null || leftCalf != null ||
		    leftFoot != null || rightThigh != null || rightCalf != null || rightFoot != null || leftUpperArm != null ||
		    leftForearm != null || leftHand != null || rightUpperArm != null || rightForearm != null ||
		    rightHand != null)
			return false;
		foreach (Object @object in spine)
	...
	public virtual bool Contains(Transform t, bool ignoreRoot = false) {
		if ((!ignoreRoot && root == t) || pelvis == t || leftThigh == t || leftCalf == t || leftFoot == t ||
		    rightThigh == t || rightCalf == t || rightFoot == t || leftUpperArm == t || leftForearm == t ||
		    leftHand == t || rightUpperArm == t || rightForearm == t || rightHand == t || head == t)
			return true;
		foreach (Object @object in spine)
			if (@object == t)
				return true;
	...
	public static bool AutoDetectReferences(
		...
		var component = root.GetComponent<Animator>();
	...
	public static void DetectReferencesByNaming(
		...
		if (references == null)
			references = new BipedReferences();
		var componentsInChildren = root.GetComponentsInChildren<Transform>();
		...
		if (references.leftUpperArm != null && references.rightUpperArm != null && references.pelvis != null &&
			...
			if (firstCommonAncestor != null) {
				...
				if (references.head == null)
					for (var index = 0; index < firstCommonAncestor.childCount; ++index) {
						var child = firstCommonAncestor.GetChild(index);
		...
		var bonesOfType = BipedNaming.GetBonesOfType(BipedNaming.BoneType.Eye, componentsInChildren);
		references.eyes = new Transform[0];
		if (!autoDetectParams.includeEyes)
			return;
		for (var index = 0; index < bonesOfType.Length; ++index)
	...
	public static void AssignHumanoidReferences(
		...
		if (references.leftUpperArm != null &&
		    !IsNeckBone(animator.GetBoneTransform(HumanBodyBones.Neck), references.leftUpperArm))
	...
	public static bool SetupError(BipedReferences references, ref string errorMessage) {
		...
		return LimbError(references.leftThigh, references.leftCalf, references.leftFoot, ref errorMessage) ||
		       LimbError(references.rightThigh, references.rightCalf, references.rightFoot, ref errorMessage) ||
		       LimbError(references.leftUpperArm, references.leftForearm, references.leftHand, ref errorMessage) ||
		       LimbError(references.rightUpperArm, references.rightForearm, references.rightHand, ref errorMessage) ||
		       SpineError(references, ref errorMessage) || EyesError(references, ref errorMessage);
	}

	public static bool SetupWarning(BipedReferences references, ref string warningMessage) {
		return LimbWarning(references.leftThigh, references.leftCalf, references.leftFoot, ref warningMessage) ||
		       LimbWarning(references.rightThigh, references.rightCalf, references.rightFoot, ref warningMessage) ||
		       LimbWarning(references.leftUpperArm, references.leftForearm, references.leftHand, ref warningMessage) ||
		       LimbWarning(references.rightUpperArm, references.rightForearm, references.rightHand,
			       ref warningMessage) || SpineWarning(references, ref warningMessage) ||
		       EyesWarning(references, ref warningMessage) || RootHeightWarning(references, ref warningMessage) ||
		       FacingAxisWarning(references, ref warningMessage);
	}

	private static bool IsNeckBone(Transform bone, Transform leftUpperArm) {
		return (!(leftUpperArm.parent != null) || !(leftUpperArm.parent == bone)) &&
		       !Hierarchy.IsAncestor(leftUpperArm, bone);
	}
	...
	private static bool AddBoneToEyes(
		...
		ref BipedReferences references,
		AutoDetectParams autoDetectParams) {
		return (!(references.head != null) || Hierarchy.IsAncestor(bone, references.head)) &&
		       !(bone.GetComponent<SkinnedMeshRenderer>() != null);
	}
	...
	private static bool AddBoneToSpine(
		...
		ref BipedReferences references,
		AutoDetectParams autoDetectParams) {
		return !(bone == references.root) &&
		       (!(bone == references.leftThigh.parent) || autoDetectParams.legsParentInSpine) &&
		       (!(references.pelvis != null) ||
		        (!(bone == references.pelvis) && !Hierarchy.IsAncestor(references.pelvis, bone)));
	}
	...
	private static void DetectLimb(
		...
		var bonesOfTypeAndSide = BipedNaming.GetBonesOfTypeAndSide(boneType, boneSide, transforms);
	...
	private static bool LimbError(
		...
		var transform = (Transform)Hierarchy.ContainsDuplicate(new Transform[3] {
			bone1,
			bone2,
			bone3
		});
	...
	private static bool SpineError(BipedReferences references, ref string errorMessage) {
		if (references.spine.Length == 0)
			return false;
		for (var index = 0; index < references.spine.Length; ++index)
			if (references.spine[index] == null) {
				errorMessage = "BipedReferences spine bone at index " + index + " is null.";
				return true;
			}

		var transform = (Transform)Hierarchy.ContainsDuplicate(references.spine);
		if (transform != null) {
		...
		for (var index = 0; index < references.spine.Length; ++index) {
			...
			if (index == 0 && references.spine[index].position == references.pelvis.position)
				flag = true;
			if (index != 0 && references.spine.Length > 1 &&
			    references.spine[index].position == references.spine[index - 1].position)
				flag = true;
			if (flag) {
				errorMessage = "Biped's spine bone nr " + index +
	...
	private static bool EyesError(BipedReferences references, ref string errorMessage) {
		if (references.eyes.Length == 0)
			return false;
		for (var index = 0; index < references.eyes.Length; ++index)
			if (references.eyes[index] == null) {
				errorMessage = "BipedReferences eye bone at index " + index + " is null.";
				return true;
			}

		var transform = (Transform)Hierarchy.ContainsDuplicate(references.eyes);
		if (!(transform != null))
			return false;
	...
	private static bool RootHeightWarning(BipedReferences references, ref string warningMessage) {
		if (references.head == null)
			return false;
		var verticalOffset = GetVerticalOffset(references.head.position, references.leftFoot.position,
			references.root.rotation);
		if (GetVerticalOffset(references.root.position, references.leftFoot.position, references.root.rotation) /
		    (double)verticalOffset <= 0.20000000298023224)
			return false;
		warningMessage =
	...
	private static bool FacingAxisWarning(BipedReferences references, ref string warningMessage) {
		var vector3_1 = references.rightHand.position - references.leftHand.position;
		var vector3_2 = references.rightFoot.position - references.leftFoot.position;
		var num1 = Vector3.Dot(vector3_1.normalized, references.root.right);
		var num2 = Vector3.Dot(vector3_2.normalized, references.root.right);
