...
public class Interp {
	...
	public static float LerpValue(
		...
		return value < (double)target
			? Mathf.Clamp(value + Time.deltaTime * increaseSpeed, float.NegativeInfinity, target)
			: Mathf.Clamp(value - Time.deltaTime * decreaseSpeed, target, float.PositiveInfinity);
	}
	...
	private static float InElastic(float t, float b, float c) {
		...
		return (float)-(num3 * (double)Mathf.Pow(2f, 10f * --t) *
		                Mathf.Sin((float)((t * (double)num1 - num4) * 6.2831854820251465) / num2)) + b;
	}
	...
	private static float OutElastic(float t, float b, float c) {
		...
		return num3 * Mathf.Pow(2f, -10f * t) *
			Mathf.Sin((float)((t * (double)num1 - num4) * 6.2831854820251465) / num2) + c + b;
	}
