...
public class KeyboardShortcutListenerService : SRServiceBase<KeyboardShortcutListenerService> {
	...
	private void Update() {
		if (Settings.Instance.KeyboardEscapeClose && Input.GetKeyUp(KeyCode.Escape) && Service.Panel.IsVisible)
			SRDebug.Instance.HideDebugPanel();
		var flag1 = Input.GetKey(KeyCode.LeftControl) || Input.GetKey(KeyCode.RightControl);
		var flag2 = Input.GetKey(KeyCode.LeftAlt) || Input.GetKey(KeyCode.RightAlt);
		var flag3 = Input.GetKey(KeyCode.LeftShift) || Input.GetKey(KeyCode.RightShift);
		for (var index = 0; index < shortcuts.Count; ++index) {
			var shortcut = shortcuts[index];
			if ((!shortcut.Control || flag1) && (!shortcut.Shift || flag3) && (!shortcut.Alt || flag2) &&
