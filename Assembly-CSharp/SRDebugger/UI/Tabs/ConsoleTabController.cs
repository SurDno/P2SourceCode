...
public class ConsoleTabController : SRMonoBehaviour {
	...
	[Inspected] private List<string> commands = new();
	[Inspected] private int commandIndex = -1;
	...
	private void ComputeHackInput() {
		...
		if (Input.GetKeyDown(KeyCode.Backspace)) {
			...
			var flag = Input.GetKey(KeyCode.LeftShift) || Input.GetKey(KeyCode.RightShift);
				...
				if (Input.GetKeyDown(keyCode)) {
					...
					switch (keyCode) {
						...
						default:
							...
							if (!flag) str = str.ToLowerInvariant();
							break;
					}
	...
	private void ComputeEnter() {
		if (!Input.GetKeyDown(KeyCode.Return))
			return;
		var text = StackTraceInputField.text;
	...
	private void CreateItem(string data) {
		var consoleEntryView = SRInstantiate.Instantiate(ItemPrefab);
	...
	private static string HistoryCommand(string command, ConsoleParameter[] parameters) {
		var str = "\nCommand history :\n";
		for (var index = 0; index < instance.commands.Count; ++index)
