...
public class Puddle : MonoBehaviour {
	...
	public bool GetWetness(RaycastHit Hit, out float Wetness) {
		...
		if (mainTexture == null)
			return false;
		var pixelBilinear = mainTexture.GetPixelBilinear(Hit.textureCoord.x, Hit.textureCoord.y);
		var color1 = material.GetColor("_MainMask");
		var component = GetComponent<PuddleCutter>();
		if (component != null)
			color1 *= component.ColorMask;
		var color2 = material.GetColor("_WorldMask");
		var num1 = material.GetFloat("_MaxSize");
		var num2 = material.GetFloat("_AuraSize");
		var num3 = 1f -
		           (float)((1.0 - pixelBilinear.r) * color1.r + (1.0 - pixelBilinear.g) * color1.g +
		                   (1.0 - pixelBilinear.b) * color1.b + (1.0 - pixelBilinear.a) * color1.a) +
		           (float)((color2.r + (double)color2.g + color2.b + color2.a) * 0.5);
	...
	public static Hit Raycast(Ray ray, float maxDistance) {
		var instance = RainManager.Instance;
		RaycastHit hitInfo;
		if (instance == null || instance.puddleWetness <= 0.0 ||
		    !Physics.Raycast(ray, out hitInfo, maxDistance, instance.puddleLayers))
			return null;
		var component1 = hitInfo.collider.GetComponent<Puddle>();
		if (component1 == null)
			return null;
		var material = component1.material;
		if (material == null)
			return null;
		var pixelBilinear =
			(material.mainTexture as Texture2D).GetPixelBilinear(hitInfo.textureCoord.x, hitInfo.textureCoord.y);
		var color1 = material.GetColor("_MainMask");
		var component2 = component1.GetComponent<PuddleCutter>();
		if (component2 != null)
		...
		var num3 = 1f -
		           (float)((1.0 - pixelBilinear.r) * color1.r + (1.0 - pixelBilinear.g) * color1.g +
		                   (1.0 - pixelBilinear.b) * color1.b + (1.0 - pixelBilinear.a) * color1.a) +
		           (float)((color2.r + (double)color2.g + color2.b + color2.a) * 0.5);
		var wetness = (num1 * instance.puddleWetness - num3) / num2;
	...
	private CommandBuffer CreateBuffer(bool hdr) {
		...
		var colors = new RenderTargetIdentifier[4] {
		...
		if (rippleListRoot != null) {
			...
			for (var ripple = rippleListRoot; ripple != null; ripple = ripple.nextNode)
	...
	private void OnWillRenderObject() {
		if (!enabled || RainManager.Instance == null || RainManager.Instance.puddleWetness <= 0.0 ||
		    Camera.current.actualRenderingPath != RenderingPath.DeferredShading)
			return;
