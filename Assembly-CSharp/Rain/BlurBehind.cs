...
public class BlurBehind : MonoBehaviour {
	...
	public static void SetViewport() {
		var rect = Camera.current.rect;
		if (!(rect != new Rect(0.0f, 0.0f, 1f, 1f)))
			return;
		var vector2 = !(Camera.current.targetTexture == null)
		...
		rect.width -= rect.x;
		rect.height -= rect.y;
		Shader.SetGlobalVector("_BlurBehindRect",
			new Vector4((storedRect.x - rect.x) / rect.width, storedRect.y / rect.height + rect.y,
				storedRect.width / rect.width, storedRect.height / rect.height));
	}

	public static void ResetViewport() {
		Shader.SetGlobalVector("_BlurBehindRect",
			new Vector4(storedRect.x, storedRect.y, storedRect.width, storedRect.height));
	}
	...
	private void CheckSettings(int sourceSize) {
		...
		if (settings == Settings.Manual)
			return;
		var p = settings == Settings.Standard ? 36f : 6f;
	...
	private RenderTexture CropSource(RenderTexture source) {
		var rect1 = new Rect(cropRect.x * source.width + pixelOffset.x, cropRect.y * source.height + pixelOffset.y,
			cropRect.width * source.width + pixelOffset.width, cropRect.height * source.height + pixelOffset.height);
		...
		if (rect1 != new Rect(0.0f, 0.0f, 1f, 1f)) {
			dest = RenderTexture.GetTemporary(Mathf.RoundToInt(rect1.width * source.width),
				Mathf.RoundToInt(rect1.height * source.height), 0, source.format);
		...
		if (rect2 != new Rect(0.0f, 0.0f, 1f, 1f)) {
			...
			storedRect = new Rect(rect2.x + storedRect.x * rect2.width, rect2.y + storedRect.y * rect2.height,
				rect2.width * storedRect.width, rect2.height * storedRect.height);
		}
	...
	private void Downsample(RenderTexture source, RenderTexture dest) {
		...
		if (source.width > source.height)
			for (var width = source.width; width > dest.width; width >>= 1)
				++num;
		else
			for (var height = source.height; height > dest.height; height >>= 1)
				++num;
		if (num > 1) {
			var renderTexture = source;
			for (; num > 2; num -= 2) {
				...
				var temporary = RenderTexture.GetTemporary(width, height, 0, renderTexture.format);
				blurMaterial.SetVector("_Parameter",
					new Vector4(renderTexture.texelSize.x, renderTexture.texelSize.y, -renderTexture.texelSize.x,
						-renderTexture.texelSize.y));
	...
	private void OnRenderImage(RenderTexture source, RenderTexture destination) {
		if (!CheckSupport() || !CheckResources()) {
			...
			var renderTexture = CropSource(source);
			var sourceSize = renderTexture.width > renderTexture.height ? renderTexture.width : renderTexture.height;
			...
			if (iterations > 0 && radius > 0.0) {
				...
				for (var p = 0; p < iterations; ++p) {
					var num = radius / 300f * Mathf.Pow(3f, p) / Mathf.Pow(3f, iterations - 1);
					if (mode == Mode.Absolute)
						num *= 100f / sourceSize;
					else if (renderTexture != source)
						num *= (source.width > source.height ? source.width : (float)source.height) / sourceSize;
					var f = p * 0.7853982f / iterations;
					var vector2_1 = new Vector2(Mathf.Sin(f), Mathf.Cos(f)) * num;
					var vector2_2 = width > height
						? new Vector2(1f, 1f / height * width)
						: new Vector2(1f / width * height, 1f);
					var vector4 = new Vector4(vector2_1.x * vector2_2.x, vector2_1.y * vector2_2.y, 0.0f, 0.0f);
					vector4.z = -vector4.x;
					vector4.w = -vector4.y;
	...
	private void SetOutputSize(
		...
		out int width,
		out int height) {
		var num = mode != Mode.Absolute
			? source.width <= source.height ? source.height <= (double)downsample ? 1f : downsample / source.height :
			source.width <= (double)downsample ? 1f : downsample / source.width
			: 1f / downsample;
