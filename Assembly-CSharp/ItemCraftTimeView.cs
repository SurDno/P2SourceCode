...
public class ItemCraftTimeView : ItemView {
	...
	[SerializeField] private HideableView busyStatus;
	...
	private string GetTimerString(StorableComponent item) {
		...
		if (item != null) {
			...
			if (craftTimeParameter != null) {
				craftTimeParameter.Value =
					craftTimeParameter.MaxValue - ServiceLocator.GetService<TimeService>().GameTime;
				if (craftTimeParameter.Value > TimeSpan.Zero) {
					var timeSpan = craftTimeParameter.Value;
