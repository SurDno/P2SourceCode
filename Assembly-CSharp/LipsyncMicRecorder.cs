...
public class LipsyncMicRecorder : MonoBehaviour {
	...
	private List<float[]> m_RecordBuffers = new();
	...
	public float[] PCM {
		get {
			var length = 0;
			foreach (var recordBuffer in m_RecordBuffers)
				length += recordBuffer.Length;
			var pcm = new float[length];
			var num = 0;
			foreach (var recordBuffer in m_RecordBuffers) {
				for (var index = 0; index < recordBuffer.Length; ++index)
	...
	public void LipsyncAudioClip(AudioClip theClip) {
		Stop(false);
		var numArray = new float[theClip.samples * theClip.channels];
	...
	private void CreateClipFromBuffers() {
		if (src.clip == null)
			return;
		var pcm = PCM;
		src.clip = AudioClip.Create("Microphone", pcm.Length / src.clip.channels, src.clip.channels, src.clip.frequency,
			false, false);
	...
	private void Update() {
		if (state == State.rec_speech || state == State.rec_speech_realtime) {
			...
			if (position < m_cursor) {
				var audioBuffer1 = GetAudioBuffer(m_cursor, src.clip.samples);
				var audioBuffer2 = GetAudioBuffer(0, position);
	...
	private float[] GetAudioBuffer(int startPos, int endPos) {
		var data = new float[(endPos - startPos) * src.clip.channels];
