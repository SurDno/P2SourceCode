...
public class Controls : MonoBehaviour {
	...
	private void OnGUI() {
		...
		GUILayout.Space(5f);
		occlusion.ApplyMethod =
			GUILayout.Toggle(occlusion.ApplyMethod == AmplifyOcclusionBase.ApplicationMethod.PostEffect,
				" Standard Post-effect")
				? AmplifyOcclusionBase.ApplicationMethod.PostEffect
				: occlusion.ApplyMethod;
		occlusion.ApplyMethod =
			GUILayout.Toggle(occlusion.ApplyMethod == AmplifyOcclusionBase.ApplicationMethod.Deferred,
				" Deferred Injection")
				? AmplifyOcclusionBase.ApplicationMethod.Deferred
				: occlusion.ApplyMethod;
		occlusion.ApplyMethod =
			GUILayout.Toggle(occlusion.ApplyMethod == AmplifyOcclusionBase.ApplicationMethod.Debug, " Debug Mode")
				? AmplifyOcclusionBase.ApplicationMethod.Debug
				: occlusion.ApplyMethod;
		...
		GUILayout.EndVertical();
		occlusion.PowerExponent =
			GUILayout.HorizontalSlider(occlusion.PowerExponent, 0.0001f, 6f, GUILayout.Width(100f));
		GUILayout.Space(5f);
		GUILayout.BeginVertical();
		GUILayout.Space(-3f);
		...
		occlusion.SampleCount =
			(AmplifyOcclusionBase.SampleCountLevel)GUILayout.HorizontalSlider((float)occlusion.SampleCount, 0.0f, 3f,
