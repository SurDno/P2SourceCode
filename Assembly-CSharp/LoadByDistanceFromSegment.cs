...
public class LoadByDistanceFromSegment : BaseLoadByDistance, IEntityAttachable, IUpdatable {
	...
	[SerializeField] private float loadIndoorDistance;
	[SerializeField] private Vector3 segmentStart;
	[SerializeField] private Vector3 segmentEnd;
	...
	private bool IsLoadCondition(Vector3 position) {
		return DistanceToSegment(position - transform.position, segmentStart, segmentEnd) <
		       (insideIndoor ? loadIndoorDistance : (double)loadDistance);
	}
	...
	public float DistanceToSegment(Vector3 p, Vector3 s0, Vector3 s1) {
		var vector3 = s1 - s0;
		var num1 = Vector3.SqrMagnitude(vector3);
		var num2 = 0.0f;
		if (num1 > 1.0 / 1000.0)
	...
	private void DrawGizmoCapsule(float radius) {
		var normalized = (segmentEnd - segmentStart).normalized;
		var vector3_1 = Vector3.Cross(Vector3.up, normalized);
		var position = transform.position;
		for (var index = 0; index < 20; ++index) {
			var f1 = 0.157079637f * index;
			var f2 = 0.157079637f * (index + 1);
			var vector3_2 = radius * (vector3_1 * Mathf.Cos(f1) + normalized * Mathf.Sin(f1));
			var vector3_3 = radius * (vector3_1 * Mathf.Cos(f2) + normalized * Mathf.Sin(f2));
