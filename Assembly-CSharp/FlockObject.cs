...
public class FlockObject : MonoBehaviour {
	...
	[SerializeField] private FlockChildTypeEnum flockType;
	[SerializeField] private FlockController flockController;
	private HashSet<LandingSpotObject> landingZones = new();
	...
	private void Update() {
		...
		if (!audioPositionSet || !(audioSource != null))
			return;
		audioSource.transform.position = Vector3.MoveTowards(audioSource.transform.position, audioPosition,
			audioMoveSpeed * Time.deltaTime);
	}
	...
	private void SetAudioSourcePosition() {
		...
		if (FlockController == null)
			...
			var num = 0;
			foreach (var child in FlockController.childs)
	...
	public void OnDrawGizmosSelected() {
		...
		Gizmos.DrawLine(FlockController.transform.position + FlockController.targetPosition,
