...
public class TOD_Time : MonoBehaviour {
	[Tooltip("Length of one day in minutes.")] [TOD_Min(0.0f)]
	public float DayLengthInMinutes = 30f;

	[Tooltip("Progress time at runtime.")] public bool ProgressTime = true;
	...
	[Tooltip("Time progression curve.")] public AnimationCurve TimeCurve = AnimationCurve.Linear(0.0f, 0.0f, 24f, 24f);
	private TOD_Sky sky;
	...
	public float ApplyTimeCurve(float deltaTime) {
		var time = timeCurveInverse.Evaluate(sky.Cycle.Hour) + deltaTime;
	...
	public void AddHours(float hours, bool adjust = true) {
		...
		var dateTime1 = sky.Cycle.DateTime;
		var dateTime2 = dateTime1.AddHours(hours);
		...
		var totalHours1 = dateTime1.TimeOfDay.TotalHours;
		var totalHours2 = dateTime2.TimeOfDay.TotalHours;
		if (totalHours1 < sky.SunriseTime && totalHours2 >= sky.SunriseTime) {
			var onSunrise = OnSunrise;
			if (onSunrise != null)
				onSunrise();
		}
	...
	private void ApproximateCurve(
		...
		var keys1 = new Keyframe[25];
		var keys2 = new Keyframe[25];
		var time1 = -0.01f;
		for (var time2 = 0; time2 < 25; ++time2) {
