...
public class FightCamera : MonoBehaviour {
	...
	public void PushFOV(float strengthInDegrees) {
		currentFOVOffset = strengthInDegrees;
	}
	...
	public void PushKnifeStab(
		...
		float strength) {
		knifePostProcess.AddScar(speedInWorldSpace, positionInWorldSpace, strength * 2f,
			(float)(0.75 + strength * 1.25));
	}
	...
	private void FixedUpdate() {
		...
		camera.fieldOfView =
			(float)(initialFOV + (double)currentFOVOffset - (playerPunchFOV > 0.0 ? playerPunchFOV : 0.0));
	}

	[Inspected]
	public void TestVignettePush() {
		PushVignette(0.5f);
	}

	[Inspected]
	public void TestFOVPush() {
		PushFOV(5f);
	}

	[Inspected]
	public void TestDirectionalPush() {
		PushDirectional(new Vector3(1f, 1f, 0.0f), 30f);
	}
	...
	private void UpdateCameraVignette() {
		vignetteVelocity +=
			(float)(-(double)currentVignetteOffset * vignetteK - vignetteVelocity * (double)vignetteDump) *
			Time.fixedDeltaTime;
	...
	private void UpdateCameraBlackAndWhite() {
		BlackAndWhiteVelocity +=
			(float)(-(double)currentBlackAndWhiteOffset * blackAndWhiteK -
			        BlackAndWhiteVelocity * (double)blackAndWhiteDump) * Time.fixedDeltaTime;
		currentBlackAndWhiteOffset += BlackAndWhiteVelocity * Time.fixedDeltaTime;
		var settings = postProcessing.profile.colorGrading.settings;
	...
	private void UpdateCameraAngles() {
		velocityEulerY += (float)(-(double)currentEulerYOffset * EulerK - velocityEulerY * (double)EulerDump) *
		                  Time.fixedDeltaTime;
		currentEulerYOffset += velocityEulerY * Time.fixedDeltaTime;
		velocityEulerX += (float)(-(double)currentEulerXOffset * EulerK - velocityEulerX * (double)EulerDump) *
		                  Time.fixedDeltaTime;
