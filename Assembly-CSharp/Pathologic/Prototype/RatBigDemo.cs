...
public class RatBigDemo : MonoBehaviour {
	...
	[Tooltip("Probability to go to point of interest (0-1)")] [Range(0.0f, 1f)]
	...
	private bool IsPlayerNear() {
		...
		return !(gameObject == null) &&
		       (gameObject.transform.position - transform.position).magnitude < (double)CullDistance;
	}
	...
	private Vector3 PointOfInterest(float distance) {
		...
		if (gameObjectsWithTag.Length == 0)
			return RandomNavSphere(gameObject.transform.position, distance, _agent.areaMask);
		var index1 = Random.Range(0, gameObjectsWithTag.Length);
		for (var index2 = 0; index2 < gameObjectsWithTag.Length; ++index2) {
			var index3 = (index2 + index1) % gameObjectsWithTag.Length;
			if ((gameObjectsWithTag[index3].transform.position - gameObject.transform.position).magnitude <
			    2.0 * WanderRadius)
	...
	private Vector3 RandomNavSphere(Vector3 origin, float distance, int layermask) {
		var vector3 = Random.insideUnitSphere * distance;
	...
	private void Start() {
		...
		if (NavMesh.SamplePosition(gameObject.transform.position, out hit, maxDistance, -1))
			...
			Debug.LogWarningFormat(
				"Can't find NavMesh for rat in radius of {0} meter ({1}), x = {2}, y = {3}, z = {4}. The rat will be deleted.",
				maxDistance, name, transform.position.x, transform.position.y, transform.position.z);
			Destroy(gameObject);
		}
	...
	private void Update() {
		...
		if (_state == State.WANDERING_CORNERS) {
			...
			Vector3 forward;
			while (true) {
				var vector3 = _pathCorners[_cornerPathIndex + 1] - _pathCorners[_cornerPathIndex];
			...
			return;
			label_7:
			var vector3_1 = _pathCorners[_cornerPathIndex] + forward * _cornerPathDistance;
			...
			if (!_agent.isOnNavMesh) {
				...
				if (_agent.path == null || (_agent.pathStatus != NavMeshPathStatus.PathComplete &&
				                            _agent.pathStatus != NavMeshPathStatus.PathPartial))
					return;
				if (!NavMeshUtility.HasPathNoGarbage(_agent) || (Random.value < Time.deltaTime / 0.5 &&
				                                                 !NavMeshUtility.HasPathWithGarbage(_agent)))
					_agent.destination = PointOfInterest(WanderRadius);
				else {
