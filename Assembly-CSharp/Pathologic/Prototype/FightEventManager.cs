...
public class FightEventManager : MonoBehaviour {
	private List<SimpleEvent> eventHandlersOnAttackOrReactionEnds = new();
	private List<SimpleEvent> eventHandlersOnAttackOrReactionStarts = new();
	private List<SimpleEvent> eventHandlersOnKlubokFalldownEnd = new();
	private List<SimpleEvent> eventHandlersOnKlubokFalldownStart = new();
	private List<SimpleEvent> eventHandlersOnKlubokHitEnd = new();
	private List<SimpleEvent> eventHandlersOnKlubokIdleEnter = new();
	private List<SimpleEvent> eventHandlersOnKlubokIdleExit = new();
	private List<SimpleEvent> eventHandlersOnKlubokJumpBackTransitionEnd = new();
	private List<SimpleEvent> eventHandlersOnKlubokJumpTransitionEnd = new();
	private List<SimpleEvent> eventHandlersOnLocomotionStepLastCycleEnter = new();
	private List<SimpleEvent> eventHandlersOnLocomotionStepLastCycleExit = new();
	private List<SimpleEvent> eventHandlersOnLocomotionStepsCycleEnter = new();
	private List<SimpleEvent> eventHandlersOnLocomotionStepsCycleExit = new();
	...
	public void OnLocomotionStepsCycleEnter() {
		foreach (var simpleEvent in new List<SimpleEvent>(eventHandlersOnLocomotionStepsCycleEnter))
			simpleEvent();
	}

	public void OnLocomotionStepsCycleExit() {
		foreach (var simpleEvent in new List<SimpleEvent>(eventHandlersOnLocomotionStepsCycleExit))
			simpleEvent();
	}

	public void OnLocomotionStepLastCycleEnter() {
		foreach (var simpleEvent in new List<SimpleEvent>(eventHandlersOnLocomotionStepLastCycleEnter))
			simpleEvent();
	}

	public void OnLocomotionStepLastCycleExit() {
		foreach (var simpleEvent in new List<SimpleEvent>(eventHandlersOnLocomotionStepLastCycleExit))
			simpleEvent();
	}

	public void OnAttackOrReactionStarts() {
		foreach (var simpleEvent in new List<SimpleEvent>(eventHandlersOnAttackOrReactionStarts))
			simpleEvent();
	}

	public void OnAttackOrReactionEnds() {
		foreach (var simpleEvent in new List<SimpleEvent>(eventHandlersOnAttackOrReactionEnds))
			simpleEvent();
	}

	public void OnKlubokIdleEnter() {
		foreach (var simpleEvent in new List<SimpleEvent>(eventHandlersOnKlubokIdleEnter))
			simpleEvent();
	}

	public void OnKlubokIdleExit() {
		foreach (var simpleEvent in new List<SimpleEvent>(eventHandlersOnKlubokIdleExit))
			simpleEvent();
	}

	public void OnKlubokJumpTransitionEnd() {
		foreach (var simpleEvent in new List<SimpleEvent>(eventHandlersOnKlubokJumpTransitionEnd))
			simpleEvent();
	}

	public void OnKlubokJumpBackTransitionEnd() {
		foreach (var simpleEvent in new List<SimpleEvent>(eventHandlersOnKlubokJumpBackTransitionEnd))
			simpleEvent();
	}

	public void OnKlubokHitEnd() {
		foreach (var simpleEvent in new List<SimpleEvent>(eventHandlersOnKlubokHitEnd))
			simpleEvent();
	}

	public void OnKlubokFalldownStart() {
		foreach (var simpleEvent in new List<SimpleEvent>(eventHandlersOnKlubokFalldownStart))
			simpleEvent();
	}

	public void OnKlubokFalldownEnd() {
		foreach (var simpleEvent in new List<SimpleEvent>(eventHandlersOnKlubokFalldownEnd))
			simpleEvent();
	}
