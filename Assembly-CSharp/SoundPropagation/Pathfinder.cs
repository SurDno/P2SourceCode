...
public class Pathfinder {
	...
	private void Enqueue(
		...
		if (portal == null)
			...
			return;
		var vector1 = output - prevPosition;
		var num1 = Math.Normalize(ref vector1);
		if (prevNodeIndex == -1)
		...
		var num2 = Vector3.Distance(prevDirection, vector1) * LossPerTurn;
		if (cell != null && cell.Profile != null)
			num2 += cell.LossPerMeter * num1;
		if (portal != null)
			num2 += portal.Loss;
		var loss = num2 + prevLoss;
		var distance = num1 + prevDistance;
		var vector2 = destPosition - output;
		var num3 = Math.Normalize(ref vector2);
		var vector3 = Vector3.MoveTowards(vector1, vector2, num3 * MaxTurnPerDistance);
		var num4 = Vector3.Distance(vector1, vector3) * LossPerTurn;
		var direction = Math.DirectionalityToDirection(destDirectionality, vector3);
		var num5 = num4 + Vector3.Distance(vector3, direction) * LossPerTurn;
		var estimation = (num3 + distance) * Mathf.Pow(2f, num5 + loss);
	...
	private void FillPath(
		...
		for (var index = output.Count - 2; index >= 0; --index) {
			...
			pathPoint4.Direction = Vector3.MoveTowards(pathPoint3.Direction, pathPoint4.Direction,
				pathPoint4.StepLength * MaxTurnPerDistance);
	...
	public bool GetReversedPath(
		...
		while (frontier.Count > 0) {
			...
			var nextCell = GetNextCell(node.Cell, node.Portal);
			if (nextCell == destCell)
				Enqueue(indexInPool, node.Position, node.Direction, node.Distance, node.Loss, nextCell, null);
			if (nextCell != null) {
				var portals = nextCell.Portals;
				for (var index = 0; index < portals.Length; ++index)
					if (portals[index] != node.Portal)
						Enqueue(indexInPool, node.Position, node.Direction, node.Distance, node.Loss, nextCell,
							portals[index]);
			} else if (destCell != null && destCell.Group != null) {
				var outerPortals = destCell.Group.OuterPortals;
				for (var index = 0; index < outerPortals.Length; ++index)
	...
	private void Reset() {
		dictionary.Clear();
		for (var index = 0; index < poolUsage; ++index)
	...
	private void SortInFrontier(int frontierIndex) {
		for (;
		     frontierIndex > 0 && pool[frontier[frontierIndex]].Estimation >
		     (double)pool[frontier[frontierIndex - 1]].Estimation;
		     --frontierIndex)
			SwapInFrontier(frontierIndex, frontierIndex - 1);
		for (;
		     frontierIndex < frontier.Count - 1 && pool[frontier[frontierIndex]].Estimation <
		     (double)pool[frontier[frontierIndex + 1]].Estimation;
		     ++frontierIndex)
			SwapInFrontier(frontierIndex, frontierIndex + 1);
	}
