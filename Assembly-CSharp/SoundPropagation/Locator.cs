...
public class Locator {
	...
	private Location LocationFromPath(List<PathPoint> path, Vector3 destDirectionality) {
		...
		var pathPoint1 = path[0];
		var rhs = location.NearestCorner - pathPoint1.Position;
		rhs.Normalize();
		for (var index = path.Count - 2; index >= 0; --index) {
			var pathPoint2 = path[index];
			var pathPoint3 = path[index + 1];
			...
			var vector3 = pathPoint2.Direction - pathPoint3.Direction;
			location.Filtering.AddOcclusion(vector3.magnitude * OcclusionPerTurn);
			if (index > 0) {
				var lhs = pathPoint2.Position - pathPoint1.Position;
				lhs.Normalize();
		...
		var occlusion = Math.Normalize(ref destDirectionality) *
		                (Vector3.Distance(destDirectionality, pathPoint1.Direction) * OcclusionPerTurn);
	...
	public Location GetLocation(
		...
		path.Clear();
		var activePathfinder = ActivePathfinder;
		activePathfinder.MaxTurnPerDistance = MaxTurnPerDistance;
		activePathfinder.LossPerTurn = OcclusionPerTurn;
		if (activePathfinder.GetReversedPath(originCell, originPosition, originDirectionality, destCell, destPosition,
			    destDirectionality, path, roughMaxCost, logPathfinding))
