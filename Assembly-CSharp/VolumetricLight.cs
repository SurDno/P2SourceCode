...
public class VolumetricLight : MonoBehaviour {
	...
	[Range(0.0f, 1f)] public float ScatteringCoef = 0.5f;
	[Range(0.0f, 0.1f)] public float ExtinctionCoef = 0.01f;
	[Range(0.0f, 1f)] public float SkyboxExtinctionCoef = 0.9f;
	[Range(0.0f, 0.999f)] public float MieG = 0.1f;
	public bool HeightFog;
	[Range(0.0f, 0.5f)] public float HeightScale = 0.1f;
	...
	[Tooltip("")] public float MaxRayLength = 400f;
	...
	private void Start() {
		...
		_cascadeShadowCommandBuffer.SetGlobalTexture("_CascadeShadowMapTexture",
	...
	private void VolumetricLightRenderer_PostRenderEvent() {
	...
	private void VolumetricLightRenderer_PreRenderEvent(
		...
		if (_light == null || _light.gameObject == null) {
			...
			_material.SetVector("_MieG",
				new Vector4((float)(1.0 - MieG * (double)MieG), (float)(1.0 + MieG * (double)MieG), 2f * MieG,
					0.07957747f));
			_material.SetVector("_VolumetricLight",
				new Vector4(ScatteringCoef, ExtinctionCoef, _light.range, 1f - SkyboxExtinctionCoef));
	...
	private void SetupPointLight(VolumetricLightRenderer renderer, Matrix4x4 viewProj) {
		...
		var pointLightMesh = VolumetricLightRenderer.GetPointLightMesh();
		var num2 = _light.range * 2f;
		var matrix = Matrix4x4.TRS(transform.position, _light.transform.rotation, new Vector3(num2, num2, num2));
		...
		_material.SetVector("_LightPos",
			new Vector4(_light.transform.position.x, _light.transform.position.y, _light.transform.position.z,
				(float)(1.0 / (_light.range * (double)_light.range))));
		...
		if (_light.cookie == null) {
			...
			_material.SetMatrix("_MyLightMatrix0",
				Matrix4x4.TRS(_light.transform.position, _light.transform.rotation, Vector3.one).inverse);
		...
		var flag = false;
		if ((_light.transform.position - Camera.current.transform.position).magnitude >=
		    (double)QualitySettings.shadowDistance)
			flag = true;
		if (_light.shadows != LightShadows.None && !flag) {
			...
			var customRenderEvent = CustomRenderEvent;
	...
	private void SetupSpotLight(VolumetricLightRenderer renderer, Matrix4x4 viewProj) {
		...
		if (!IsCameraInSpotLightBounds())
			shaderPass = 3;
		var spotLightMesh = VolumetricLightRenderer.GetSpotLightMesh();
		var range = _light.range;
		var num = Mathf.Tan((float)((_light.spotAngle + 1.0) * 0.5 * (Math.PI / 180.0))) * _light.range;
		var matrix = Matrix4x4.TRS(transform.position, transform.rotation, new Vector3(num, num, range));
		var inverse = Matrix4x4.TRS(_light.transform.position, _light.transform.rotation, Vector3.one).inverse;
		_material.SetMatrix("_MyLightMatrix0",
			Matrix4x4.TRS(new Vector3(0.5f, 0.5f, 0.0f), Quaternion.identity, new Vector3(-0.5f, -0.5f, 1f)) *
			Matrix4x4.Perspective(_light.spotAngle, 1f, 0.0f, 1f) * inverse);
		_material.SetMatrix("_WorldViewProj", viewProj * matrix);
		_material.SetVector("_LightPos",
			new Vector4(_light.transform.position.x, _light.transform.position.y, _light.transform.position.z,
				(float)(1.0 / (_light.range * (double)_light.range))));
		...
		if (_light.cookie == null)
			...
			_material.SetTexture("_LightTexture0", _light.cookie);
		var flag = false;
		var b = position + forward * _light.range;
	...
	private void SetupDirectionalLight(VolumetricLightRenderer renderer, Matrix4x4 viewProj) {
		...
		if (Noise)
			...
			_material.DisableKeyword("NOISE");
		_material.SetVector("_LightDir",
			new Vector4(_light.transform.forward.x, _light.transform.forward.y, _light.transform.forward.z,
				(float)(1.0 / (_light.range * (double)_light.range))));
		_material.SetVector("_LightColor", _light.color * _light.intensity);
		...
		if (_light.shadows != 0) {
			_material.EnableKeyword("SHADOWS_DEPTH");
			_commandBuffer.Blit(source, (RenderTargetIdentifier)(Texture)renderer.GetVolumeLightBuffer(), _material,
				pass);
			var customRenderEvent = CustomRenderEvent;
			if (customRenderEvent == null)
				return;
			...
			_material.DisableKeyword("SHADOWS_DEPTH");
			renderer.GlobalCommandBuffer.Blit(source, (RenderTargetIdentifier)(Texture)renderer.GetVolumeLightBuffer(),
				_material, pass);
			var customRenderEvent = CustomRenderEvent;
			if (customRenderEvent != null)
				customRenderEvent(renderer, this, renderer.GlobalCommandBuffer, viewProj);
	...
	private bool IsCameraInPointLightBounds() {
		var sqrMagnitude = (_light.transform.position - Camera.current.transform.position).sqrMagnitude;
		var num = _light.range + 1f;
		return sqrMagnitude < num * (double)num;
	}

	private bool IsCameraInSpotLightBounds() {
		return Vector3.Dot(_light.transform.forward, Camera.current.transform.position - _light.transform.position) <=
		       (double)(_light.range + 1f) &&
		       Mathf.Acos(Vector3.Dot(transform.forward,
			       (Camera.current.transform.position - _light.transform.position).normalized)) * 57.295780181884766 <=
		       (_light.spotAngle + 3.0) * 0.5;
	}
	...
	private static float DistanceToSegment(Vector3 v, Vector3 a, Vector3 b) {
		...
		var lhs = v - b;
		return Vector3.Dot(lhs, vector3_1) >= 0.0
			? lhs.magnitude
			: Vector3.Cross(vector3_1, vector3_2).magnitude / vector3_1.magnitude;
	}
}