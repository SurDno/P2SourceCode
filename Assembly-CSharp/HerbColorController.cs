...
public class HerbColorController : MonoBehaviour {
	...
	private void Update() {
		...
		currentAlpha = Mathf.MoveTowards(currentAlpha,
			(transform.position - (player != null ? ((IEntityView)player).Position : Vector3.zero)).magnitude <
			(double)maxDistance
				? 1f
				: 0.0f, colorChangeSpeed * Time.deltaTime);
		CountColor();
