...
public class ProtagonistShadersSettings : MonoBehaviourInstance<ProtagonistShadersSettings> {
	...
	[Range(0.0f, 1f)] [SerializeField] private float flatness = 0.9f;
	...
	private void OnDisable() {
		Shader.SetGlobalFloat("_ProtagonistFlatness", 0.0f);
	}
	...
	public Vector3 ProtagonistToWorld(Vector3 position) {
		var vector4_1 = Camera.worldToCameraMatrix * new Vector4(position.x, position.y, position.z, 1f);
		var vector4_2 = ProtagonistProjection() * vector4_1;
		vector4_2.z = Mathf.Lerp(vector4_2.z, -1f * vector4_2.w, flatness);
		var vector4_3 = Camera.cameraToWorldMatrix * (Camera.nonJitteredProjectionMatrix.inverse * vector4_2);
		return vector4_3 / vector4_3.w;
	}

	private void UpdateShaders() {
		Shader.SetGlobalFloat("_ProtagonistFlatness", flatness);
		Shader.SetGlobalMatrix("_ProtagonistMatrixVP",
			GL.GetGPUProjectionMatrix(ProtagonistProjection(), true) * Camera.worldToCameraMatrix);
	}
}