  ...
  public class SRServiceManager : SRAutoSingleton<SRServiceManager>
  {
    ...
    private static object GetServiceInternal(Type t)
    {
      if (_hasQuit || !Application.isPlaying)
        return null;
      SRList<Service> services = Instance._services;
    ...
    public static bool HasService<T>() where T : class => HasService(typeof (T));
    ...
    protected object AutoCreateService(Type t)
    {
      ...
      foreach (ServiceStub serviceStub in _serviceStubs)
      {
        if (!(serviceStub.InterfaceType != t))
        {
          ...
          {
            Type implType = serviceStub.Type;
            if (implType == null)
    ...
    protected void OnApplicationQuit() => _hasQuit = true;
    ...
    {
      foreach (MethodInfo staticMethod in GetStaticMethods(t))
      {
        ServiceSelectorAttribute attrib = SRReflection.GetAttribute<ServiceSelectorAttribute>(staticMethod);
        if (attrib != null)
        {
          ...
          {
            ServiceStub serviceStub = stubs.FirstOrDefault(p => p.InterfaceType == attrib.ServiceType);
            if (serviceStub == null)
            {
    ...
    private static void ScanTypeForConstructors(Type t, List<ServiceStub> stubs)
    {
      foreach (MethodInfo staticMethod in GetStaticMethods(t))
      {
        ...
        ServiceConstructorAttribute attrib = SRReflection.GetAttribute<ServiceConstructorAttribute>(method);
        if (attrib != null)
        {
