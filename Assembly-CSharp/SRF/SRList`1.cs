...
public class SRList<T> :
	...
	public void Insert(int index, T item) {
		if (Buffer == null || Count == Buffer.Length)
			Expand();
		if (index < Count) {
			for (var count = Count; count > index; --count)
	...
	public void RemoveAt(int index) {
		...
		for (var index1 = index; index1 < Count; ++index1)
	...
	public void Clean() {
		if (Buffer == null)
			return;
		for (var count = Count; count < _buffer.Length; ++count)
	...
	private void Expand() {
		var objArray = Buffer != null ? new T[Mathf.Max(Buffer.Length << 1, 32)] : new T[32];
