...
public class VirtualVerticalLayoutGroup : LayoutGroup, IPointerClickHandler, IEventSystemHandler {
	private readonly SRList<object> _itemList = new();
	private readonly SRList<int> _visibleItemList = new();
	private bool _isDirty;
	private SRList<Row> _rowCache = new();
	private ScrollRect _scrollRect;
	private int _selectedIndex;
	private object _selectedItem;
	[SerializeField] private SelectedItemChangedEvent _selectedItemChanged;
	private int _visibleItemCount;
	private SRList<Row> _visibleRows = new();
	...
	public object SelectedItem {
		...
		set {
			if (_selectedItem == value || !EnableSelection)
				return;
			var num = value == null ? -1 : _itemList.IndexOf(value);
			if (value != null && num < 0)
	...
	public override float minHeight => (float)(_itemList.Count * (double)ItemHeight + padding.top + padding.bottom +
	                                           Spacing * (double)_itemList.Count);

	public void OnPointerClick(PointerEventData eventData) {
		if (!EnableSelection)
			return;
		var gameObject = eventData.pointerPressRaycast.gameObject;
		if (gameObject == null)
			return;
		var index = Mathf.FloorToInt(Mathf.Abs(rectTransform.InverseTransformPoint(gameObject.transform.position).y) /
		                             ItemHeight);
	...
	protected void ScrollUpdate() {
		...
		var height = ((RectTransform)ScrollRect.transform).rect.height;
		var num1 = Mathf.FloorToInt(y / (ItemHeight + Spacing));
		var num2 = Mathf.CeilToInt((float)((y + (double)height) / (ItemHeight + (double)Spacing)));
		var b1 = num1 - RowPadding;
		var b2 = num2 + RowPadding;
		var num3 = Mathf.Max(0, b1);
		var num4 = Mathf.Min(_itemList.Count, b2);
		var flag = false;
		for (var index = 0; index < _visibleRows.Count; ++index) {
			var visibleRow = _visibleRows[index];
		...
		for (var forIndex = num3; forIndex < num4 && forIndex < _itemList.Count; ++forIndex)
	...
	public override void SetLayoutHorizontal() {
		var size = rectTransform.rect.width - padding.left - padding.right;
		for (var index = 0; index < _visibleRows.Count; ++index)
			SetChildAlongAxis(_visibleRows[index].Rect, 0, padding.left, size);
		for (var index = 0; index < _rowCache.Count; ++index)
	...
	public override void SetLayoutVertical() {
		...
		for (var index = 0; index < _visibleRows.Count; ++index) {
			var visibleRow = _visibleRows[index];
			SetChildAlongAxis(visibleRow.Rect, 1,
				(float)(visibleRow.Index * (double)ItemHeight + padding.top + Spacing * (double)visibleRow.Index),
				ItemHeight);
		}
	}
	...
	public void ClearItems() {
		for (var index = _visibleRows.Count - 1; index >= 0; --index)
	...
	private bool AlignBottom => childAlignment == TextAnchor.LowerRight || childAlignment == TextAnchor.LowerCenter ||
	                            childAlignment == TextAnchor.LowerLeft;

	private bool AlignTop => childAlignment == TextAnchor.UpperLeft || childAlignment == TextAnchor.UpperCenter ||
	                         childAlignment == TextAnchor.UpperRight;

	private float ItemHeight {
		get {
			if (_itemHeight <= 0.0) {
				_itemHeight = !(ItemPrefab.GetComponent(typeof(ILayoutElement)) is ILayoutElement component)
					? ItemPrefab.rect.height
	...
	private Row CreateRow() {
		var row = new Row();
		var rectTransform = SRInstantiate.Instantiate(ItemPrefab);
