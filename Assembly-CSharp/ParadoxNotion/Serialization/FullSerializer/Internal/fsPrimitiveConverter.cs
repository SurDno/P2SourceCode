...
public class fsPrimitiveConverter : fsConverter {
	...
	public override fsResult TrySerialize(object instance, out fsData serialized, Type storageType) {
		...
		if (UseDouble(type)) {
			if (instance.GetType() == typeof(float) && (float)instance != -3.4028234663852886E+38 &&
			    (float)instance != 3.4028234663852886E+38 && !float.IsInfinity((float)instance) &&
	...
	public override fsResult TryDeserialize(fsData storage, ref object instance, Type storageType) {
		...
		if (UseDouble(storageType) || UseInt64(storageType)) {
			if (storage.IsDouble)
				instance = !(storageType == typeof(float))
					? Convert.ChangeType(storage.AsDouble, storageType)
					: (float)storage.AsDouble;
			else if (storage.IsInt64)
				instance = !(storageType == typeof(int))
					? Convert.ChangeType(storage.AsInt64, storageType)
					: (int)storage.AsInt64;
