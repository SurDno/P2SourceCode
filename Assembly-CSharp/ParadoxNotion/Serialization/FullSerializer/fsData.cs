...
public sealed class fsData {
	...
	public static fsData CreateDictionary() {
		return new fsData(new Dictionary<string, fsData>(fsGlobalConfig.IsCaseSensitive
	...
	public bool Equals(fsData other) {
		...
		switch (Type) {
			case fsDataType.Array:
				var asList1 = AsList;
				var asList2 = other.AsList;
				if (asList1.Count != asList2.Count)
					return false;
				for (var index = 0; index < asList1.Count; ++index)
					if (!asList1[index].Equals(asList2[index]))
						return false;
				return true;
			case fsDataType.Object:
				var asDictionary1 = AsDictionary;
				var asDictionary2 = other.AsDictionary;
				if (asDictionary1.Count != asDictionary2.Count)
					return false;
				foreach (var key in asDictionary1.Keys)
