...
public abstract class fsBaseConverter {
	...
	public virtual object CreateInstance(fsData data, Type storageType) {
		return !RequestCycleSupport(storageType)
			? (object)storageType
	...
	protected fsResult FailExpectedType(fsData data, params fsDataType[] types) {
		return fsResult.Fail(GetType().Name + " expected one of " +
		                     string.Join(", ", types.Select(t => t.ToString()).ToArray()) + " but got " + data.Type +
		                     " in " + data);
	}
	...
	protected fsResult SerializeMember<T>(
		...
		T value) {
		fsData data1;
		var fsResult = Serializer.TrySerialize(typeof(T), overrideConverterType, value, out data1);
	...
	protected fsResult DeserializeMember<T>(
		...
		object result = null;
		var success = Serializer.TryDeserialize(data1, typeof(T), overrideConverterType, ref result);
		value = (T)result;
		return success;
	}
}