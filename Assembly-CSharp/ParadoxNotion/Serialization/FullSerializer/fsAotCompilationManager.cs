...
public class fsAotCompilationManager {
	...
	private static List<AotCompilation> _uncomputedAotCompilations = new();

	public static Dictionary<Type, string> AvailableAotCompilations {
		get {
			for (var index = 0; index < _uncomputedAotCompilations.Count; ++index) {
				var uncomputedAotCompilation = _uncomputedAotCompilations[index];
				_computedAotCompilations[uncomputedAotCompilation.Type] = GenerateDirectConverterForTypeInCSharp(
					uncomputedAotCompilation.Type, uncomputedAotCompilation.Members,
					uncomputedAotCompilation.IsConstructorPublic);
			}
	...
	private static string GetConverterString(fsMetaProperty member) {
		return member.OverrideConverterType == null
			? "null"
			: string.Format("typeof({0})", member.OverrideConverterType.CSharpName(true));
	}
	...
	private static string GenerateDirectConverterForTypeInCSharp(
		...
		var str1 = type.CSharpName(true);
		var str2 = type.CSharpName(true, true);
		...
		stringBuilder.AppendLine("        protected override fsResult DoSerialize(" + str1 +
		                         " model, Dictionary<string, fsData> serialized) {");
		stringBuilder.AppendLine("            var result = fsResult.Success;");
		stringBuilder.AppendLine();
		foreach (var member in members)
			stringBuilder.AppendLine("            result += SerializeMember(serialized, " + GetConverterString(member) +
		...
		stringBuilder.AppendLine();
		stringBuilder.AppendLine(
			"        protected override fsResult DoDeserialize(Dictionary<string, fsData> data, ref " + str1 +
			" model) {");
		stringBuilder.AppendLine("            var result = fsResult.Success;");
		stringBuilder.AppendLine();
		for (var index = 0; index < members.Length; ++index) {
			...
			stringBuilder.AppendLine("            result += DeserializeMember(data, " + GetConverterString(member) +
			                         ", \"" + member.JsonName + "\", out t" + index + ");");
		...
		if (isConstructorPublic)
			...
			stringBuilder.AppendLine("            return Activator.CreateInstance(typeof(" + str1 +
			                         "), /*nonPublic:*/true);");
		stringBuilder.AppendLine("        }");
		stringBuilder.AppendLine("    }");
		stringBuilder.AppendLine("}");
