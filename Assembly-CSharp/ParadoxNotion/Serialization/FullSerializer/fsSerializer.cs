...
public class fsSerializer {
	private static HashSet<string> _reservedKeywords = new() {
		"$ref",
		"$id",
		"$type",
		"$version",
	...
	private static void Invoke_OnAfterSerialize(
		...
		object instance,
		ref fsData data) {
		for (var index = processors.Count - 1; index >= 0; --index)
	...
	public fsSerializer() {
		...
		Config = new fsConfig();
		foreach (var converter in fsConverterRegistrar.Converters)
	...
	private List<fsObjectProcessor> GetProcessors(Type type) {
		...
		if (attribute != null && attribute.Processor != null) {
			var instance = (fsObjectProcessor)Activator.CreateInstance(attribute.Processor);
	...
	public void AddConverter(fsBaseConverter converter) {
		if (converter.Serializer != null)
			throw new InvalidOperationException(
				"Cannot add a single converter instance to multiple fsConverters -- please construct a new instance for " +
				converter);
		switch (converter) {
			case fsDirectConverter _:
				var fsDirectConverter = (fsDirectConverter)converter;
			...
			default:
				throw new InvalidOperationException("Unable to add converter " + converter +
				                                    "; the type association strategy is unknown. Please use either fsDirectConverter or fsConverter as your base type.");
		}
	...
	private fsBaseConverter GetConverter(Type type, Type overrideConverterType) {
		...
		if (_cachedConverters.TryGetValue(type, out converter))
			return converter;
		var attribute1 = fsPortableReflection.GetAttribute<fsObjectAttribute>(type);
		if (attribute1 != null && attribute1.Converter != null) {
			var instance = (fsBaseConverter)Activator.CreateInstance(attribute1.Converter);
			instance.Serializer = this;
			return _cachedConverters[type] = instance;
		}

		var attribute2 = fsPortableReflection.GetAttribute<fsForwardAttribute>(type);
		if (attribute2 != null) {
		...
		if (!_cachedConverters.TryGetValue(type, out converter)) {
			...
			for (var index = 0; index < _availableConverters.Count; ++index)
	...
	public fsResult TryDeserialize<T>(fsData data, ref T instance) {
		...
		var fsResult = TryDeserialize(data, typeof(T), ref result);
	...
	public fsResult TrySerialize(
		...
		var fsResult2 = InternalSerialize_1_ProcessCycles(storageType, overrideConverterType, instance, out data);
		Invoke_OnAfterSerialize(processors, storageType, instance, ref data);
		return fsResult2;
	}
	...
	private fsResult InternalSerialize_1_ProcessCycles(
		...
		try {
			...
			var fsResult2 = InternalSerialize_2_Inheritance(storageType, overrideConverterType, instance, out data);
			if (fsResult2.Failed)
				return fsResult2;
	...
	private fsResult InternalSerialize_2_Inheritance(
		...
		object instance,
		out fsData data) {
		var fsResult = InternalSerialize_4_Converter(overrideConverterType, instance, out data);
		if (fsResult.Failed || !(storageType != instance.GetType()) || !GetConverter(storageType, overrideConverterType)
			    .RequestInheritanceSupport(storageType))
			return fsResult;
		EnsureDictionary(data);
		var safeType = SafeTypeUtility.GetSafeType(instance);
	...
	public fsResult TryDeserialize(
		...
		try {
			_references.Enter();
			List<fsObjectProcessor> processors;
			var fsResult = InternalDeserialize_1_CycleReference(overrideConverterType, data, storageType, ref result,
				out processors);
			if (fsResult.Succeeded)
				Invoke_OnAfterDeserialize(processors, storageType, result);
			return fsResult;
		} catch (Exception ex) {
			var str = string.Format("<b>(Deserialization Error)</b>: {0}\n{1}", ex.Message, ex.StackTrace);
			Debug.LogError(str);
	...
	private fsResult InternalDeserialize_1_CycleReference(
		...
		if (!IsObjectReference(data))
			return InternalDeserialize_3_Inheritance(overrideConverterType, data, storageType, ref result,
				out processors);
		var id = int.Parse(data.AsDictionary["$ref"].AsString);
	...
	private fsResult InternalDeserialize_3_Inheritance(
		...
		Invoke_OnBeforeDeserialize(processors, storageType, ref data);
		var type1 = storageType;
		if (IsTypeSpecified(data)) {
			var fsData = data.AsDictionary["$type"];
			if (!fsData.IsString) {
				var message = "$type value must be a string (in " + data + ")";
				success.AddMessage(message);
			} else {
				var asString = fsData.AsString;
				var type2 = fsTypeCache.GetType(asString, storageType);
				if (type2 == null) {
					var message = "Unable to locate specified type \"" + asString + "\" , " +
					              ReflectionTools.GetContext();
					success.AddMessage(message);
					Debug.LogWarning(message);
				} else if (!storageType.IsAssignableFrom(type2)) {
					var message = "Ignoring type specifier; a field/property of type " + storageType +
					              " cannot hold an instance of " + type2;
					success.AddMessage(message);
					Debug.LogWarning(message);
				} else
					type1 = type2;
		...
		if (result == null || result.GetType() != type1) {
			var converter = GetConverter(type1, overrideConverterType);
	...
	internal class fsLazyCycleDefinitionWriter {
		...
		private HashSet<int> _references = new();
		...
		public void WriteReference(int id, Dictionary<string, fsData> dict) {
			if (_pendingDefinitions.ContainsKey(id)) {
				var pendingDefinition = _pendingDefinitions[id];
