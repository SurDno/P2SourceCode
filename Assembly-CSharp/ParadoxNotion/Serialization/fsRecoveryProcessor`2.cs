...
public class fsRecoveryProcessor<TCanProcess, TMissing> : fsObjectProcessor
	...
	public override void OnBeforeDeserialize(Type storageType, ref fsData data) {
		...
		if (!json.TryGetValue("$type", out fsData))
			return;
		var type1 = fsTypeCache.GetType(fsData.AsString, storageType);
		if (type1 == null) {
		...
		if (type1 == typeof(TMissing)) {
			var type2 = fsTypeCache.GetType(json["missingType"].AsString, storageType);
			if (type2 != null) {
				var asDictionary = fsJsonParser.Parse(json["recoveryState"].AsString).AsDictionary;
				json = json.Concat(asDictionary.Where(kvp => !json.ContainsKey(kvp.Key)))
