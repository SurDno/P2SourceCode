...
public class MessageRouter :
	...
	private void OnBecameInvisible() {
		Dispatch(nameof(OnBecameInvisible), null);
	}

	private void OnBecameVisible() {
		Dispatch(nameof(OnBecameVisible), null);
	}
	...
	private void OnMouseDown() {
		Dispatch(nameof(OnMouseDown), null);
	}

	private void OnMouseDrag() {
		Dispatch(nameof(OnMouseDrag), null);
	}

	private void OnMouseEnter() {
		Dispatch(nameof(OnMouseEnter), null);
	}

	private void OnMouseExit() {
		Dispatch(nameof(OnMouseExit), null);
	}

	private void OnMouseOver() {
		Dispatch(nameof(OnMouseOver), null);
	}

	private void OnMouseUp() {
		Dispatch(nameof(OnMouseUp), null);
	}
		...
		for (var index = 0; index < messages.Length; ++index)
			if (target.GetType().RTGetMethod(messages[index]) == null)
				Debug.LogError(string.Format(
					"Type '{0}' does not implement a method named '{1}', for the registered event to use.",
					target.GetType().FriendlyName(), messages[index]));
			else {
	...
	public void UnRegister(object target) {
		...
		foreach (var key in listeners.Keys) {
			foreach (var obj in listeners[key].ToArray())
				if (obj == target)
	...
	public void UnRegister(object target, params string[] messages) {
		...
		for (var index = 0; index < messages.Length; ++index) {
			...
			if (listeners.ContainsKey(message))
				foreach (var obj in listeners[message].ToArray())
					if (obj == target)
	...
	public void Dispatch(string message, object arg) {
		...
		for (var index = 0; index < objectList.Count; ++index) {
			var del = objectList[index];
			if (del != null) {
				var methodInfo = !(del is Delegate)
					? del.GetType().RTGetMethod(message)
					: (del as Delegate).RTGetDelegateMethodInfo();
				if (!(methodInfo == null)) {
					object[] objArray;
