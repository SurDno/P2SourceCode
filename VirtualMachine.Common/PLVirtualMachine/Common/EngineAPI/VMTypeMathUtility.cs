...
public static class VMTypeMathUtility {
	public static bool IsValueLarger(object firstValue, object secondValue, bool bEqual = false) {
		...
		try {
			var num1 = !(firstValue.GetType() == typeof(float))
				? !(firstValue.GetType() == typeof(GameTime))
					? Convert.ToSingle(firstValue)
					: ((GameTime)firstValue).TotalSeconds
				: (float)firstValue;
			var num2 = !(secondValue.GetType() == typeof(float))
				? !(secondValue.GetType() == typeof(GameTime))
					? Convert.ToSingle(secondValue)
					: ((GameTime)secondValue).TotalSeconds
				: (float)secondValue;
			return !bEqual ? num1 > (double)num2 : num1 > num2 - 9.9999997473787516E-05;
		} catch (Exception ex) {
			firstValue.GetType();
			secondValue.GetType();
			Logger.AddError(string.Format("Error value comparing ' IsValueLarger': {0} at {1})", ex,
	...
	public static bool IsValueLess(object firstValue, object secondValue, bool bEqual = false) {
		...
		try {
			var num1 = !(firstValue.GetType() == typeof(GameTime))
				? Convert.ToSingle(firstValue)
				: ((GameTime)firstValue).TotalSeconds;
			var num2 = !(secondValue.GetType() == typeof(GameTime))
				? Convert.ToSingle(secondValue)
				: ((GameTime)secondValue).TotalSeconds;
			return !bEqual ? num1 < (double)num2 : num1 < num2 + 9.9999997473787516E-05;
		} catch (Exception ex) {
			firstValue.GetType();
			secondValue.GetType();
			Logger.AddError(string.Format("Error value comparing ' IsValueLess': {0} at {1})", ex,
				EngineAPIManager.Instance.CurrentFSMStateInfo));
			return false;
		}
	}
	...
	public static object DoMathAdd(object firstValue, object secondValue) {
		if (firstValue.GetType() != secondValue.GetType())
			Logger.AddError(MethodBase.GetCurrentMethod().Name + " , type is different, first : " +
			                firstValue.GetType() + " , second : " + secondValue.GetType());
		...
		Logger.AddError(MethodBase.GetCurrentMethod().Name + " , type is unknown, first : " + firstValue.GetType() +
		                " , second : " + secondValue.GetType());
	...
	public static bool IsValuesEqual(object oldVal, object newVal) {
		...
		if ((oldVal == null && newVal != null) || (oldVal != null && newVal == null))
			return false;
		return typeof(IEntity).IsAssignableFrom(newVal.GetType()) && typeof(IObjRef).IsAssignableFrom(oldVal.GetType())
			? ((Engine.Common.IObject)newVal).Id == ((IEngineInstanced)oldVal).EngineGuid
			: IsValueEqual(oldVal, newVal);
	}
	...
	public static bool IsValueEqual(object firstValue, object secondValue) {
		...
		var type1 = firstValue.GetType();
		var type2 = secondValue.GetType();
		...
		return typeof(IRef).IsAssignableFrom(type2) && typeof(Engine.Common.IObject).IsAssignableFrom(type1)
			? CompareVMRefWithEngineObject((IRef)secondValue, (Engine.Common.IObject)firstValue)
	...
	private static bool CompareVMRefWithEngineObject(IRef refValue, Engine.Common.IObject engineObjValue) {
		...
		return typeof(ISampleRef).IsAssignableFrom(refValue.GetType()) &&
		       ((IEngineTemplated)refValue).EngineTemplateGuid == engineObjValue.Id;
	}
