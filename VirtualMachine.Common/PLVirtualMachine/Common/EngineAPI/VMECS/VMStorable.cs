...
public class VMStorable : VMEngineComponent<IStorableComponent> {
	...
	public static IStorableComponent MakeStorableByTemplate(
		...
		if (template.GetComponent<IStorableComponent>() == null) {
			Logger.AddError(string.Format("Invalid storable object template {0}: there is no storable component!",
				template.Name));
			return null;
		}
		...
		try {
			...
			var component = EngineAPIManager.Instance
				.CreateWorldTemplateDynamicChildInstance(template, null, ServiceCache.Simulation.Storables).Instance
	...
	public static void ClearAll() {
		StorableTemplateNamesDict.Clear();
	}
}