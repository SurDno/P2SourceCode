...
public class VMGameComponent : VMComponent {
	...
	[Event("Cutscene finish event", "")] public event Action OnEndCutsceneEvent;
	...
	[Event("Game mode change event", "")] public event Action OnGameModeChanged;
	...
	[Event("Common logic event", "value")] public event Action<string> OnCommonConsoleEvent;
	...
	public void AddDropBagEntity(IEntity entity) {
		if (entity == null)
			Logger.AddError(string.Format("Cannot add drop bag entity, because entity is null at {0}",
				EngineAPIManager.Instance.CurrentFSMStateInfo));
		else {
			Logger.AddWarning(string.Format("Process create drop bag entity {0} at {1}", entity.Name,
				EngineAPIManager.Instance.CurrentFSMStateInfo));
			try {
				ServiceLocator.GetService<IDropBagService>().AddEntity(entity);
			} catch (Exception ex) {
				Logger.AddError(string.Format("Cannot add drop bag entity, error: {0} at {1}", ex,
					EngineAPIManager.Instance.CurrentFSMStateInfo));
			}
		}
	}
	...
	public void ResetDropBags() {
		ServiceLocator.GetService<IDropBagService>().Reset();
	}
	...
	public virtual EVMGameLocalizationName GetGameLocalization() {
		return EVMGameLocalizationName.english;
	}

	[Method("Create object", "Template", "")]
	public virtual IObjRef CreateObject(IBlueprintRef staticObj) {
		return null;
	}
	...
	public virtual bool IsObjectExist(IObjRef remObj) {
		return false;
	}
	...
	public virtual ulong StartTimer(float fInterval) {
		return 0;
	}

	[Method("SleepState", "Game time length", "")]
	public virtual ulong SleepState(GameTime interval) {
		return 0;
	}

	[Method("SleepState", "Seconds", "")]
	public virtual ulong SleepStateRT(float interval) {
		return 0;
	}

	[Method("Start game timer", "Game time interval", "")]
	public virtual ulong StartGameTimer(GameTime interval) {
		return 0;
	}

	[Method("Start loop timer", "Interval in seconds", "")]
	public virtual ulong StartLoopTimer(float interval) {
		return 0;
	}

	[Method("Start loop game timer", "Game time interval", "")]
	public virtual ulong StartLoopGameTimer(GameTime interval) {
		return 0;
	}

	[Method("Start game timer at context", "Game time interval,context", "")]
	public virtual ulong StartGameTimerAtContext(GameTime interval, IGameModeRef gameMode) {
		return 0;
	}

	[Method("Start loop game timer at context", "Game time interval,context", "")]
	public virtual ulong StartLoopGameTimerAtContext(GameTime interval, IGameModeRef gameMode) {
		return 0;
	}
	...
	public virtual IBlueprintRef GetObjectClass(IObjRef objRef) {
		return null;
	}

	[Method("Is object derived from class", "Object,class", "")]
	public virtual bool IsObjectDerivedFromClass(IObjRef objRef, IBlueprintRef classRef) {
		return false;
	}
	...
	public virtual bool IsObjectCompatible(IObjRef objRef, VMType type) {
		return false;
	}
	...
	public virtual IObjRef GetObjectWithTag(IObjRef objRef, string tag) {
		return null;
	}
	...
	public void AddNotification(NotificationEnum notification, VMCommonList textList) {
		try {
			...
			if (service == null)
				...
				var localizedTextList = new List<LocalizedText>();
				for (var objIndex = 0; objIndex < textList.ObjectsCount; ++objIndex) {
					var obj = textList.GetObject(objIndex);
					if (obj != null && typeof(ITextRef).IsAssignableFrom(obj.GetType())) {
						var engineTextInstance = EngineAPIManager.CreateEngineTextInstance((ITextRef)obj);
	...
	public void ResetSpreading() {
		ServiceLocator.GetService<ISpreadingService>().Reset();
	}
	...
	public void JerboaSyncronize() {
		ServiceLocator.GetService<IJerboaService>().Syncronize();
	}

	[Method("Herb Reset", "", "")]
	public void HerbReset() {
		ServiceLocator.GetService<ISteppeHerbService>().Reset();
	}
	...
	public virtual int ReadIntParamValue(string valueStr) {
		return 0;
	}

	public virtual float ReadFloatParamValue(string valueStr) {
		return 0.0f;
	}

	public void OnGameModeChange() {
		var onGameModeChanged = OnGameModeChanged;
	...
	private void FireEntityEvent(string name, IEntity entity) {
		if (entity != null) {
			if (entity.IsTemplate) {
				var entityLogicEvent = OnTemplateEntityLogicEvent;
				if (entityLogicEvent == null)
					return;
				entityLogicEvent(name, entity);
			} else {
				var entityLogicEvent = OnEntityLogicEvent;
				if (entityLogicEvent == null)
					return;
				entityLogicEvent(name, entity);
			}
		} else
			Logger.AddError(string.Format("Fire entity not defined for FireEntityEvent calling at {0}!",
				EngineAPIManager.Instance.CurrentFSMStateInfo));
	}

	public override void OnCreate() {
		var service1 = ServiceLocator.GetService<ILogicEventService>();
		service1.OnCommonEvent += FireCommonEvent;
		service1.OnValueEvent += FireValueEvent;
		service1.OnEntityEvent += FireEntityEvent;
		var service2 = ServiceLocator.GetService<ISpreadingService>();
		...
		var service3 = ServiceLocator.GetService<IDropBagService>();
	...
	private void FireRegionLoadedOnce(IEntity region) {
		var regionLoadedOnce = OnRegionLoadedOnce;
	...
	private void FireRegionLoaded(IEntity region) {
		var onRegionLoaded = OnRegionLoaded;
	...
	private void FireFurnitureLoadedOnce(
		...
		var furnitureLoadedOnce = OnFurnitureLoadedOnce;
	...
	private void FireFurnitureLoaded(
		...
		BuildingEnum building,
		DiseasedStateEnum diseased) {
		var onFurnitureLoaded = OnFurnitureLoaded;
	...
	private void FireValueEvent(string name, string value) {
		var onValueLogicEvent = OnValueLogicEvent;
	...
	private void FireCommonEvent(string name) {
		var commonConsoleEvent = OnCommonConsoleEvent;
	...
	private void DropBagService_OnDeleteEntity(IEntity entity) {
		var deleteDropBagEvent = NeedDeleteDropBagEvent;
	...
	private void DropBagService_OnCreateEntity(IEntity entity) {
		var createDropBagEvent = NeedCreateDropBagEvent;
