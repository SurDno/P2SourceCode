...
public class VMBaseEntity :
	...
	private List<VMComponent> components = new();
	...
	public virtual void Initialize(ILogicObject templateObj, Guid engineGuid = default) {
		if (templateObj == null)
			return;
		editorTemplate = templateObj;
		var flag1 = false;
		...
		if (flag1)
			...
			engineEntity = !(engineGuid != Guid.Empty)
				? ServiceCache.Factory.Create<IEntity>()
				: ServiceCache.Factory.Create<IEntity>(engineGuid);
			Instantiated = true;
			LoadComponentsFromVMTemplate(templateObj.Blueprint);
		}
	...
	public bool IsHierarchy {
		get {
			if (IsSimple)
				return true;
			return editorTemplate != null && typeof(IHierarchyObject).IsAssignableFrom(editorTemplate.GetType()) &&
			       ((IHierarchyObject)editorTemplate).HierarchyGuid.IsHierarchy;
		}
	}

	public HierarchyGuid HierarchyGuid =>
		editorTemplate != null && typeof(IHierarchyObject).IsAssignableFrom(editorTemplate.GetType())
			? ((IHierarchyObject)editorTemplate).HierarchyGuid
			: HierarchyGuid.Empty;
	...
	public static string GetComponentName(IComponent engComponent) {
		foreach (var type in engComponent.GetType().GetInterfaces()) {
			var componentName = InfoAttribute.GetComponentName(type);
	...
	public void AddNewComponent(string componentName) {
		try {
			var componentTypeByName = GetComponentTypeByName(componentName);
			if (componentTypeByName != null) {
				var instance = (VMComponent)Activator.CreateInstance(componentTypeByName);
	...
	private void AddNewComponentFromEngine(IComponent component) {
		...
		if (null == componentTypeByName)
			return;
		try {
			var newComponent = (VMComponent)ProxyFactory.Create(componentTypeByName);
	...
	public string EngineBaseTemplateGuid {
		get {
			var engineEntityTemplate = this.engineEntityTemplate;
			if (engineEntityTemplate == null)
				return "";
			var template = engineEntityTemplate.Template;
	...
	public virtual void Remove() {
		try {
			if (idDisposed)
				Logger.AddWarning(string.Format("You try to remove already removed object: {0} at {1}", Name,
				...
				if (EngineAPIManager.ObjectCreationExtraDebugInfoMode)
					Logger.AddWarning(string.Format("Extra debug info: removing object: {0} name = {1}", Instance.Id,
						Name));
				DisposeInstance();
			}
		} catch (Exception ex) {
	...
	public virtual void DisposeInstance(bool clear = false) {
		try {
			...
			if (clear) {
				...
				for (var index = 0; index < dynamicChilds.Count; ++index)
	...
	public void LoadComponentsFromVMTemplate(IBlueprint template) {
		var functionalComponents = template.FunctionalComponents;
	...
	public List<VMBaseEntity> GetAllChildEntities() {
		return allChilds;
	}

	public List<VMBaseEntity> GetChildEntities() {
		return childs;
	}
	...
	public string GetCustomTag() {
		var componentByName = GetComponentByName("Common");
	...
	protected void InstantiateEngineEntity(Guid engDynGuid = default) {
		if (editorTemplate == null)
			return;
		if (typeof(IWorldObject).IsAssignableFrom(editorTemplate.GetType()))
			try {
				var id = ((IEngineTemplated)editorTemplate).EngineTemplateGuid;
				if (id == Guid.Empty)
				...
				if (engineEntityTemplate == null)
					Logger.AddError(string.Format(
						"Object by template {0} creation failed!, Template with engine guid {1} not found in engine",
						editorTemplate.Name, id));
				else {
					engineEntity = !(engDynGuid == Guid.Empty)
						? ServiceCache.Factory.Instantiate(engineEntityTemplate, engDynGuid)
						: ServiceCache.Factory.Instantiate(engineEntityTemplate);
					idDisposed = false;
				}
			} catch (Exception ex) {
				Logger.AddError(string.Format("Object {0} with id {1} engine instancing error: {2}", Name, engDynGuid,
					ex));
			}
		else
			try {
