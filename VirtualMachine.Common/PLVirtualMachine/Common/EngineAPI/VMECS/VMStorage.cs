...
public class VMStorage : VMEngineComponent<IStorageComponent> {
	...
	protected List<string> innerContainerTags = new();
	protected List<IInventoryComponent> innerContainersList = new();
	...
	public void PickUpByTemplate([Template] IEntity template) {
		if (template == null)
			...
			var storableComponent = VMStorable.MakeStorableByTemplate(Parent, template);
			if (storableComponent == null)
				Logger.AddError(string.Format("Cannot crteate storable entity by template {0} !", template.Name));
			else {
				var component = storableComponent.Owner.GetComponent<IStorableComponent>();
	...
	public void PickUpByTemplate_v1([Template] IEntity template, int thingsCount) {
		try {
			if (template == null)
				...
				do {
					component = VMStorable.MakeStorableByTemplate(Parent, template).Owner
						.GetComponent<IStorableComponent>();
					var num2 = component.Max;
			...
			Logger.AddError(string.Format("Cannot pick up items {0} to {1} storage: {2}", template.Name, Parent.Name,
				ex));
		}
	}
	...
	public void AddItemOrDropByTemplate([Template] IEntity template, [Template] IEntity containerTemplate) {
		try {
			if (template == null)
				...
				var containerByTemplate = GetContainerByTemplate(containerTemplate);
				if (containerByTemplate == null)
					Logger.AddError(string.Format("PickUp target container name={0} id ={1} not found !",
						containerTemplate.Name, containerTemplate.Id));
				else
		...
		if (template == null)
			...
			foreach (var storableComponent in Component.Items)
				if (storableComponent != null && !storableComponent.IsDisposed &&
				    storableComponent.Owner.Name == template.Name) {
	...
	public void ReceiveByTemplate([Template] IEntity template, IObjRef newPlace) {
		if (template == null)
			...
			var storableByTemplate = GetStorableByTemplate(template);
			if (storableByTemplate == null)
				return;
			if (newPlace == null)
				Logger.AddError(string.Format("New place for storable receiving in {0} not defined !", Parent.Name));
			else {
				var entity = ServiceCache.Simulation.Get(newPlace.EngineGuid);
				if (entity == null)
					Logger.AddError(string.Format("New place entity guid={0} for storable receiving in {1} not found !",
						newPlace.EngineGuid, Parent.Name));
				else {
					var component = entity.GetComponent<IStorageComponent>();
					if (component == null)
						Logger.AddError(string.Format(
							"New place entity {0} for storable receiving in {1} hasn't storage component !",
	...
	public void ReceiveItems(IObjRef newPlace, int itemsCount) {
		if (newPlace == null)
			...
			if (entity == null)
				...
				if (component == null)
					Logger.AddError(string.Format(
						"New place entity {0} for storable receiving in {1} hasn't storage component !", entity.Name,
						Parent.Name));
				else {
					var storableComponentList = new List<IStorableComponent>();
					if (itemsCount > Component.Items.Count())
						itemsCount = Component.Items.Count();
					var num = (int)Math.Floor(Component.Items.Count() * VMMath.GetRandomDouble());
					for (var index1 = 0; index1 < itemsCount; ++index1) {
						var index2 = index1 + num;
						if (index2 >= Component.Items.Count())
							index2 -= Component.Items.Count();
						var storableComponent = Component.Items.ElementAt(index2);
						if (storableComponent != null && !storableComponent.IsDisposed)
							storableComponentList.Add(storableComponent);
					}

					OnModify();
					for (var index = 0; index < storableComponentList.Count; ++index)
	...
	public virtual bool CheckExistCombinationElement(IBlueprintRef combinationTemplate) {
		return false;
	}
	...
	public bool CheckItemInPlace([Template] IEntity template, [Template] IEntity containerTemplate) {
		...
		foreach (var storableComponent in Component.Items)
			if (storableComponent != null && !storableComponent.IsDisposed &&
			    storableComponent.Container.Owner.TemplateId == containerTemplate.Id &&
	...
	public IStorableComponent GetStorableByTemplate(Engine.Common.IObject template) {
		...
		foreach (var storableByTemplate in Component.Items)
			if (storableByTemplate != null && !storableByTemplate.IsDisposed &&
			    storableByTemplate.Owner.Name == template.Name)
	...
	public void FreeContainer([Template] IEntity containerTemplate) {
		try {
			...
			var component = containerTemplate.GetComponent<IInventoryComponent>();
			if (component != null)
				Component.ClearItems(component);
			else
				Logger.AddError(string.Format("Container {0} not found in storage {1} at {2} !", containerTemplate.Name,
					Parent.Name, EngineAPIManager.Instance.CurrentFSMStateInfo));
		} catch (Exception ex) {
			var str1 = "Not defined";
			var str2 = "Not defined";
			if (Component != null && Component.Owner != null)
	...
	public IInventoryComponent GetInnerContainer(int containerIndex) {
		...
		Logger.AddError(string.Format("Invalid inner container index {0} int {1} storage !", containerIndex,
			Parent.Name));
		return null;
	}
	...
	public List<IInventoryComponent> GetInnerContainersByTagsList(OperationMultiTagsInfo tagsList) {
		...
		var containersByTagsList = new List<IInventoryComponent>();
		for (var index = 0; index < innerContainersList.Count; ++index)
	...
	public string GetInnerContainerTag(int containerIndex) {
		...
		Logger.AddError(
			string.Format("Invalid inner container index {0} in {1} storage !", containerIndex, Parent.Name));
		return "";
	}
	...
	public void SetInnerContainerTag(int containerIndex, string sTag) {
		if (containerIndex >= 0 && containerIndex < innerContainerTags.Count)
			...
			Logger.AddError(string.Format("Invalid inner container index {0} in {1} storage !", containerIndex,
				Parent.Name));
		OnModify();
	}

	public void ResetInnerContainersTags() {
		for (var index = 0; index < innerContainerTags.Count; ++index)
	...
	public override void AfterCreate() {
		if (Component == null)
			...
			try {
				...
				var childEntities = Parent.GetChildEntities();
				if (childEntities != null)
					for (var index = 0; index < childEntities.Count; ++index)
						if (childEntities[index].Instance != null) {
							var component = childEntities[index].Instance.GetComponent<IInventoryComponent>();
							if (component != null) {
							...
							Logger.AddError(string.Format(
								"Invalid inventory container creation: Storage {0} inner inventory {1} engine instance not defined !",
								Parent.Name, childEntities[index].Name));

				if (innerContainersList.Count != 0)
					return;
				foreach (var inventoryTemplate in Component.InventoryTemplates) {
					var inventoryComponent = MakeStorageInventoryByTemplate(Parent, inventoryTemplate);
				...
				Logger.AddError(string.Format("Entity {0} storage component aftercreation error: {1} !", Parent.Name,
					ex));
			}
	}
	...
	public void CheckInventoryTagsConsistensy() {
		...
		if (innerContainerTags.Count < innerContainersList.Count)
			for (var index = 0; index < innerContainersList.Count - innerContainerTags.Count; ++index)
				innerContainerTags.Add("");
		else
			innerContainerTags.RemoveRange(innerContainersList.Count,
				innerContainerTags.Count - innerContainersList.Count);
	}
	...
	public static IInventoryComponent MakeStorageInventoryByTemplate(
		...
		if (template.GetComponent<IInventoryComponent>() == null) {
			Logger.AddError(string.Format("Invalid inventory object template {0}: there is no inventory component!",
				template.Name));
			return null;
		}

		try {
			var dynamicChildInstance =
				EngineAPIManager.Instance.CreateWorldTemplateDynamicChildInstance(template, storageEntity,
					storageEntity.Instance);
			if (dynamicChildInstance != null)
				return dynamicChildInstance.Instance.GetComponent<IInventoryComponent>();
			Logger.AddError(string.Format(
				"Cannot create storage inventory entity in {0}, probably editor template with engine id = {1} not found",
				storageEntity.Name, template.Id));
			return null;
		} catch (Exception ex) {
			Logger.AddError(string.Format("Making inventory instance in storage {0} by template {1}  error: {2}",
				storageEntity.Name, template.Name, ex));
		}
	...
	public static bool DoAddItemToStorage(
		...
		if (storable.Count <= 0) {
			Logger.AddWarning(string.Format("Add item to storage warning: adding items '{0}' count is 0",
				storable.Owner.Name));
			return false;
		}
		...
		try {
			...
			Logger.AddError(string.Format("Cannot add item {0} to storage {1}, error: {2}", storage.Owner,
				storable.Owner, ex));
		}
	...
	private void RaiseAddItemEvent(IStorableComponent item, IInventoryComponent container) {
		try {
			var id = item.Owner.Id;
			if (EngineAPIManager.Instance.GetEditorTemplateByEngineGuid(id) == null) {
				var template = (IEntity)item.Owner.Template ?? VMStorable.GetEngTemplateByStorableName(item.Owner.Name);
				if (template == null)
					Logger.AddError(string.Format("New added to {0} storable entity engine template not defined !",
						Parent.Name));
				else
					VMStorable.MakeStorableByInstance(this, item, template);
				if (EngineAPIManager.Instance.GetEditorTemplateByEngineGuid(id) == null) {
					Logger.AddWarning(string.Format("New added to {0} storable entity engine template not defined !",
						Parent.Name));
					return;
				}
			}
	...
	private void RaiseRemoveItemEvent(IStorableComponent item, IInventoryComponent container) {
		try {
			...
			if (EngineAPIManager.Instance.GetEditorTemplateByEngineGuid(id) == null) {
				var template = (IEntity)item.Owner.Template ?? VMStorable.GetEngTemplateByStorableName(item.Owner.Name);
				if (template == null) {
					Logger.AddError(string.Format("Removing from {0} storable entity engine template not defined !",
						Parent.Name));
					return;
				}
				...
				VMStorable.MakeStorableByInstance(this, item, template);
				if (EngineAPIManager.Instance.GetEditorTemplateByEngineGuid(id) == null) {
					Logger.AddWarning(string.Format("Removing from  {0} storable entity engine template not defined !",
						Parent.Name));
					return;
				}
			}
			...
			Logger.AddError(string.Format("Removing item at {0} storage event raising error: {1}", Parent.Name,
				ex.Message));
		}
	}
	...
	private void RaiseChangeItemEvent(IStorableComponent item, IInventoryComponent container) {
		...
		if (EngineAPIManager.Instance.GetEditorTemplateByEngineGuid(id) == null) {
			var template = (IEntity)item.Owner.Template ?? VMStorable.GetEngTemplateByStorableName(item.Owner.Name);
			if (template == null) {
				Logger.AddError(string.Format("Change from {0} storable entity engine template not defined !",
					Parent.Name));
				return;
			}
			...
			VMStorable.MakeStorableByInstance(this, item, template);
			if (EngineAPIManager.Instance.GetEditorTemplateByEngineGuid(id) == null) {
				Logger.AddWarning(string.Format("Change from  {0} storable entity engine template not defined !",
					Parent.Name));
				return;
			}
		}
