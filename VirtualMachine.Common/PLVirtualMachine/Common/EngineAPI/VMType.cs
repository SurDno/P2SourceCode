...
public class VMType : IVMStringSerializable {
	...
	public VMType(Type baseType, string specialType) {
		...
		else if (typeof(ISampleRef) == this.baseType || typeof(IStateRef) == this.baseType ||
		         typeof(ILogicMapNodeRef) == this.baseType)
	...
	public static VMType CreateListTypeByElementType(VMType elemType) {
		var typeByElementType = new VMType {
	...
	public static VMType CreateBlueprintSpecialType(IBlueprint blueprint) {
		var blueprintSpecialType = new VMType(typeof(IObjRef));
	...
	public static VMType CreateStateSpecialType(IGraphObject state) {
		...
		var str = state.GetType().ToString();
		if (str.Contains('.')) {
	...
	private void MakeSpecial(IBlueprint blueprint) {
		if (blueprint == null)
			return;
		if (blueprint.Static) {
			Logger.AddError(string.Format("Cannot specialize type by static object {0} at {1}", blueprint.Name,
	...
	public bool IsFunctional =>
		IsList
			? listElementType != null && listElementType.IsFunctional
			: typeof(IObjRef) == BaseType || typeof(IBlueprintRef) == BaseType;

	public bool IsFunctionalSpecial => IsList
		? listElementType != null && listElementType.IsFunctionalSpecial
		: functionalPartsList != null && functionalPartsList.Count > 0;
	...
	public IEnumerable<string> GetFunctionalParts(bool withDepended = false) {
		if (IsList) {
			if (listElementType == null) yield break;

			foreach (var functionalPart in listElementType.GetFunctionalParts(withDepended))
				yield return functionalPart;
		}

		if (withDepended && functionalPartsList != null)
			for (var i = 0; i < functionalPartsList.Count; ++i) {
				yield return functionalPartsList[i];
				var dependedFunctional = EngineAPIManager.GetDependedFunctional(functionalPartsList[i]);
				if ("" != dependedFunctional)
					yield return dependedFunctional;
			}

		var isFunctionalParts = false;
		if (functionalPartsList != null)
			isFunctionalParts = functionalPartsList.Count > 0;
		if (SpecialTypeBlueprint != null && !isFunctionalParts)
			foreach (var componentName in SpecialTypeBlueprint.GetComponentNames())
				yield return componentName;
		if (isFunctionalParts)
			foreach (var functionalParts in functionalPartsList)
	...
	public bool IsIntegerNumber => baseType == typeof(byte) || baseType == typeof(ushort) ||
	                               baseType == typeof(short) || baseType == typeof(int) || baseType == typeof(uint) ||
	...
	public void Read(string data) {
		switch (data) {
			case null:
				Logger.AddError(string.Format("Attempt to read null vmtype data at {0}",
					EngineAPIManager.Instance.CurrentFSMStateInfo));
				break;
			case "":
				break;
	...
	public string Write() {
		if (null == baseType) {
			Logger.AddError(string.Format("Type serialization error: base type is null at {0}",
				EngineAPIManager.Instance.CurrentFSMStateInfo));
			return "";
		}
		...
		try {
			...
			if (!isEngineStorageMode) {
				var objectTypeNameByType = EngineAPIManager.GetObjectTypeNameByType(baseType);
			...
			Logger.AddError(string.Format("Type serialization error: {0} at {1}", ex,
				EngineAPIManager.Instance.CurrentFSMStateInfo));
			return "";
		}
	}
	...
	public IBlueprint SpecialTypeBlueprint {
		get {
			if (specialTypeBlueprintId == 0UL)
				return null;
			var specialTypeBlueprint =
				IStaticDataContainer.StaticDataContainer.GameRoot.GetBlueprintByGuid(specialTypeBlueprintId);
			if (specialTypeBlueprint == null) {
				var objectByGuid = IStaticDataContainer.StaticDataContainer.GetObjectByGuid(specialTypeBlueprintId);
	...
	private void ReadFunctionalPart(string sFuncSpecialPart) {
		if (functionalPartsList != null)
			functionalPartsList.Clear();
		var strArray = sFuncSpecialPart.Split('&');
		for (var index = 0; index < strArray.Length; ++index)
			if (!InfoAttribute.TryGetValue(strArray[index], out var _)) {
				var uint64 = StringUtility.ToUInt64(strArray[index]);
				if (uint64 != 0UL)
