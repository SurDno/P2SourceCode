...
public class EngineAPIManager {
	protected static Dictionary<string, Dictionary<EContextVariableCategory, List<IVariable>>>
		componentsAbstractVarsDict = new();
	...
	public virtual IBlueprint GetEditorTemplateByEngineGuid(Guid guid) {
		return null;
	}
	...
	public static ComponentInfo GetFunctionalComponentByName(string componentName) {
		...
		Logger.AddError(string.Format("Functional component with name {0} not found at {1}", componentName,
			Instance.CurrentFSMStateInfo));
		return null;
	}
	...
	public static APIEventInfo GetAPIEventInfoByName(string componentName, string sEventName) {
		...
		if (componentsInfoDict.TryGetValue(componentName, out componentInfo))
			for (var index = 0; index < componentInfo.Events.Count; ++index)
	...
	public static VMType GetEditorTypeByEngType(Type engType, string specTypeName, bool isTemplate) {
		if (typeof(IObjRef) == engType || typeof(VMBaseEntity).IsAssignableFrom(engType) ||
		    (typeof(IEntity) == engType && !isTemplate))
		...
		return typeof(ICommonList).IsAssignableFrom(engType)
			? new VMType(typeof(ICommonList), specTypeName, false)
			: new VMType(engType);
	}
	...
	public static string GetSpecialFunctionName(
		...
		var specialFunctionInfo = GetSpecialFunctionInfo(specFuncName, ownerClassType);
		return bWithComponent
			? GetComponentNameByType(ownerClassType) + "." + specialFunctionInfo
			: specialFunctionInfo;
	}
	...
	public static string GetSpecialPropertyName(
		...
		Type ownerClassType,
		bool bWithComponent = false) {
		var specialPropertyInfo = GetSpecialPropertyInfo(specPropertyName, ownerClassType);
		return bWithComponent
			? GetComponentNameByType(ownerClassType) + "." + specialPropertyInfo
			: specialPropertyInfo;
	}
	...
	public static string GetSpecialEventName(
		...
		Type ownerClassType,
		bool bWithComponent = false) {
		var specialEventInfo = GetSpecialEventInfo(specEventName, ownerClassType);
	...
	public static string GetSpecialFunctionInfo(
		...
		var methods = ownerClassType.GetMethods();
		for (var index = 0; index < methods.Length; ++index)
			foreach (SpecialFunctionAttribute customAttribute in methods[index]
				         .GetCustomAttributes(typeof(SpecialFunctionAttribute), true))
				if (customAttribute.Name == specFuncName) {
					var name = methods[index].Name;
					specMethodInfoDict.Add(key, name);
					return name;
				}

		Logger.AddError(string.Format("No special method info found for special function name {0} for class type {1}",
			specFuncName, ownerClassType));
		return null;
	}
	...
	public static string GetSpecialPropertyInfo(
		...
		var properties = ownerClassType.GetProperties();
		for (var index = 0; index < properties.Length; ++index)
			foreach (SpecialPropertyAttribute customAttribute in properties[index]
				         .GetCustomAttributes(typeof(SpecialPropertyAttribute), true))
		...
		Logger.AddError(string.Format("No special property info found for special property name {0} for class type {1}",
			specPropertyName, ownerClassType));
		return null;
	}
	...
	public static string GetSpecialEventInfo(ESpecialEventName specEventName, Type ownerClassType) {
		...
		var events = ownerClassType.GetEvents();
		for (var index = 0; index < events.Length; ++index) {
			var specialEventAttribute = GetSpecialEventAttribute(events[index]);
		...
		Logger.AddError(string.Format("No special event info found for special event name {0} for class type {1}",
			specEventName, ownerClassType));
		return null;
	}

	private static SpecialEventAttribute GetSpecialEventAttribute(System.Reflection.EventInfo evntInfo) {
		var customAttributes = evntInfo.GetCustomAttributes(typeof(SpecialEventAttribute), true);
	...
	public static VMComponent GetObjectComponentByType(
		...
		if (targetObj == null || baseComponentType == null)
			return null;
		foreach (var component in targetObj.Components)
	...
	public static Type GetObjectTypeByName(string typeName) {
		Type result;
		if (VMTypeAttribute.TryGetValue(typeName, out result) || SampleAttribute.TryGetValue(typeName, out result) ||
		    EnumTypeAttribute.TryGetValue(typeName, out result))
			return result;
		Logger.AddError("Cannot get object Type for typename " + typeName + " from ECS at " +
		                Instance.CurrentFSMStateInfo);
		return null;
	}
	...
	public static IEnumerable<IVariable> GetAbstractVariablesByFunctionalName(
		...
		if (componentsAbstractVarsDict.ContainsKey(functionalName))
			foreach (var variable in GetAbstractVariablesByRegisteredFunctionalName(functionalName, varCategory))
			...
			if (functionalName.StartsWith("I")) {
				...
				if (componentsAbstractVarsDict.ContainsKey(functionalName))
					foreach (var variable in
					         GetAbstractVariablesByRegisteredFunctionalName(functionalName, varCategory))
						yield return variable;
				else
					Logger.AddError(string.Format("Functional with name {0} not found in engine api at {1}",
						functionalName, Instance.CurrentFSMStateInfo));
			}

			Logger.AddError(string.Format("Functional with name {0} not found in engine api at {1}", functionalName,
				Instance.CurrentFSMStateInfo));
		}
	}
	...
	public static IEnumerable<IVariable> GetAbstractParametricFunctionsByFunctionalName(
		...
		if (componentsAbstractVarsDict.ContainsKey(functionalName))
			variables = GetAbstractVariablesByRegisteredFunctionalName(functionalName,
		...
		if (variables != null)
			foreach (var variable in variables)
				if (variable is BaseFunction baseFunction &&
				    baseFunction.InputParams.Count == functionParamList.Count) {
					var flag = true;
					for (var index = 0; index < baseFunction.InputParams.Count; ++index)
						if (functionParamList[index].BaseType == typeof(object)) {
							if ((baseFunction.InputParams[index].Type.BaseType.IsValueType ||
							     baseFunction.InputParams[index].Type.BaseType.IsEnum ? 1 :
								    baseFunction.InputParams[index].Type.BaseType == typeof(bool) ? 1 : 0) == 0)
								flag = false;
	...
	protected static IEnumerable<IVariable> GetAbstractVariablesByRegisteredFunctionalName(
		string functionalName,
		EContextVariableCategory varCategory) {
		var abstractVariablesCategoryDict = componentsAbstractVarsDict[functionalName];
		if (abstractVariablesCategoryDict.ContainsKey(varCategory))
			foreach (var variable in abstractVariablesCategoryDict[varCategory])
				yield return variable;
		else if (varCategory == EContextVariableCategory.CONTEXT_VARIABLE_CATEGORY_ALL)
			foreach (var keyValuePair in abstractVariablesCategoryDict) {
	...
	private static void LoadEngineAPIComponents() {
		foreach (var component in InfoAttribute.Components) {
			...
			componentsAbstractVarsDict[component.Name]
				.Add(EContextVariableCategory.CONTEXT_VARIABLE_CATEGORY_APIFUNCTION,
					GetAbstractFunctionsList(component, componentInfo.Methods));
			componentsAbstractVarsDict[component.Name].Add(EContextVariableCategory.CONTEXT_VARIABLE_CATEGORY_PARAM,
				GetAbstractParamsList(component, componentInfo.Properties));
		}
	}

	private static List<APIPropertyInfo> ComputeProperties(ComponentReplectionInfo component) {
		var properties = new List<APIPropertyInfo>();
		foreach (var property in component.Properties) {
			var propertyInfo = property.Value;
			var propertyAttribInfo = GetPropertyAttribInfo(component.Type, propertyInfo);
			if (propertyAttribInfo != null) {
				var editorTypeByEngType = GetEditorTypeByEngType(propertyInfo.PropertyType,
					propertyAttribInfo.SpecialTypeInfo, propertyInfo.IsDefined(typeof(TemplateAttribute), false));
				var apiPropertyInfo = new APIPropertyInfo(propertyInfo, editorTypeByEngType);
				apiPropertyInfo.PropertyDefValue = propertyAttribInfo.DefValue;
				if (apiPropertyInfo.PropertyDefValue != null && apiPropertyInfo.PropertyType != null &&
				    apiPropertyInfo.PropertyDefValue.GetType() != apiPropertyInfo.PropertyType.BaseType)
					Logger.AddError("Type : " + propertyInfo.DeclaringType.Name + " , property name : " +
					                propertyInfo.Name + " , need type : " + apiPropertyInfo.PropertyType.BaseType.Name +
					                " , has name : " + apiPropertyInfo.PropertyDefValue.GetType().Name);
	...
	private static List<APIEventInfo> ComputeEvents(ComponentReplectionInfo component) {
		var events = new List<APIEventInfo>();
		foreach (var keyValuePair in component.Events) {
			var reflEventInfo = keyValuePair.Value;
			var eventAttribInfo = GetEventAttribInfo(component.Type, reflEventInfo);
			if (eventAttribInfo != null) {
				var apiEventInfo = new APIEventInfo(reflEventInfo.Name);
				apiEventInfo.AtOnce = eventAttribInfo.AtOnce;
				if (reflEventInfo.EventHandlerType.IsGenericType) {
					var genericArguments = reflEventInfo.EventHandlerType.GetGenericArguments();
					if (genericArguments != null && eventAttribInfo.InputTypesDesc != "") {
						var strArray1 = eventAttribInfo.InputTypesDesc.Split(',');
						if (strArray1.Length != genericArguments.Length)
							Logger.AddError(string.Format(
								"{0}: attribute event desc params count not corresponds to real params count",
						...
						for (var index = 0; index < genericArguments.Length; ++index) {
							...
							try {
								var apiParamInfo =
									new APIParamInfo(
										GetEditorTypeByEngType(genericArguments[index], specTypeName, isTemplate),
										name);
					...
					foreach (var method in reflEventInfo.EventHandlerType.GetMethods())
					...
					if (eventAttribInfo.InputTypesDesc != "") {
						var strArray3 = eventAttribInfo.InputTypesDesc.Split(',');
						if (strArray3.Length != parameterInfoList.Count)
							Logger.AddError(string.Format(
								"{0}: attribute event desc params count not corresponds to real params count",
								apiEventInfo.EventName));
						for (var index = 0; index < parameterInfoList.Count; ++index) {
							var name = "param_" + index;
							var specTypeName = "";
							...
							try {
								var isTemplate = parameterInfoList[index].IsDefined(typeof(TemplateAttribute), false);
								var apiParamInfo =
									new APIParamInfo(
										GetEditorTypeByEngType(parameterInfoList[index].ParameterType, specTypeName,
											isTemplate), name);
								apiEventInfo.MessageParams.Add(apiParamInfo);
							} catch (Exception ex) {
								Logger.AddError(ex.ToString());
	...
	private static List<APIMethodInfo> ComputeMethods(ComponentReplectionInfo component) {
		...
		foreach (var method in component.Methods) {
			var key = method.Key;
			var reflMethodInfo = method.Value;
			var methodAttribInfo = GetMethodAttribInfo(component.Type, reflMethodInfo);
			if (methodAttribInfo != null) {
				var apiMethodInfo = new APIMethodInfo(key);
				apiMethodInfo.ReturnParam = new APIParamInfo(GetEditorTypeByEngType(reflMethodInfo.ReturnType,
					methodAttribInfo.OutputTypesSpecialInfo, false));
				var parameters = reflMethodInfo.GetParameters();
				...
				if (strArray1 != null && strArray1.Length != parameters.Length)
					Logger.AddError(string.Format(
						"Method {0} attribute params types info length don't match params count",
						apiMethodInfo.MethodName));
				for (var index = 0; index < parameters.Length; ++index) {
					...
					var apiParamInfo =
						new APIParamInfo(
							GetEditorTypeByEngType(parameters[index].ParameterType, specTypeName,
								parameters[index].IsDefined(typeof(TemplateAttribute), false)), name);
	...
	private static List<IVariable> GetAbstractFunctionsList(
		...
		var abstractFunctionsList = new List<IVariable>();
		for (var index = 0; index < methodList.Count; ++index) {
			var baseFunction = new BaseFunction(methodList[index].MethodName, componentInfo.Name);
	...
	private static List<IVariable> GetAbstractParamsList(
		...
		var abstractParamsList = new List<IVariable>();
		for (var index = 0; index < propertiesList.Count; ++index) {
			var propertyName = propertiesList[index].PropertyName;
			var name = componentInfo.Name;
			var propertyDefValue = propertiesList[index].PropertyDefValue;
			var componentName = name;
			var propertyType = propertiesList[index].PropertyType;
			var defValue = propertyDefValue;
			var abstractParameter = new AbstractParameter(propertyName, componentName, propertyType, defValue);
	...
	private static void RegistrObjectDefaultComponent(
		...
		var functionalComponentByName = GetFunctionalComponentByName(engineComponentName);
	...
	private static MethodAttribute GetMethodAttribInfo(
		...
		var customAttributes = reflMethodInfo.GetCustomAttributes(typeof(MethodAttribute), true);
	...
	private static EventAttribute GetEventAttribInfo(Type componentType, System.Reflection.EventInfo reflEventInfo) {
		var customAttributes = reflEventInfo.GetCustomAttributes(typeof(EventAttribute), true);
	...
	private static PropertyAttribute GetPropertyAttribInfo(
		...
		var customAttributes = reflPropertyInfo.GetCustomAttributes(typeof(PropertyAttribute), true);
