...
public class ObjectCombinationElement {
	private List<ObjectCombinationVariant> combinationVariants = new();
	...
	public ObjectCombinationElement(ObjectCombinationElement cloneElement) {
		for (var index = 0; index < cloneElement.combinationVariants.Count; ++index)
			combinationVariants.Add(new ObjectCombinationVariant(cloneElement.combinationVariants[index].ObjectGuid,
				cloneElement.combinationVariants[index].MinCount, cloneElement.combinationVariants[index].MaxCount) {
	...
	public ObjectCombinationElement(string dataStr) {
		Read(dataStr);
	}

	public bool ContainsItem(IBlueprint item) {
		for (var index = 0; index < combinationVariants.Count; ++index)
	...
	public int GetVariantsCount() {
		return combinationVariants.Count;
	}
	...
	public ObjectCombinationVariant GetRandomVariantByWeight(float randVal) {
		if (combinationVariants.Count == 0)
			return null;
		var num1 = variantsSumWeight * randVal;
		var num2 = 0.0f;
		for (var index = 0; index < combinationVariants.Count; ++index) {
			double num3 = num2;
	...
	public void Read(string dataStr) {
		if (dataStr.Length == 0)
			return;
		if (dataStr.Length < 8)
			Logger.AddError(string.Format("Cannot convert {0} to combibation element data at {1}", dataStr,
				EngineAPIManager.Instance.CurrentFSMStateInfo));
		else {
			var separator = new string[1] { "END&VAR" };
			var strArray = dataStr.Split(separator, StringSplitOptions.RemoveEmptyEntries);
			combinationVariants.Clear();
			for (var index = 0; index < strArray.Length; ++index) {
				var dataStr1 = strArray[index];
				if (dataStr1.StartsWith("Probability_")) {
					var text = dataStr1.Substring("Probability_".Length);
					try {
						spawnProbability = StringUtility.ToInt32(text);
					} catch (Exception ex) {
						Logger.AddError(string.Format(
							"Cannot read spawn probability value: {0}  isn't correct probability value. Error: {1} at {2}",
							text, ex.Message, EngineAPIManager.Instance.CurrentFSMStateInfo));
					}
				} else
