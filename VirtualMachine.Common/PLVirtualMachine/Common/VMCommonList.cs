...
public class VMCommonList : ICommonList, IVMStringSerializable {
	protected List<ListObjectInfo> objectsList = new();

	public VMCommonList() { }

	public VMCommonList(VMCommonList copyList) {
		for (var index = 0; index < copyList.objectsList.Count; ++index)
	...
	public virtual object GetObject(int objIndex) {
		...
		if (objIndex >= objectsList.Count) {
			Logger.AddWarning(
				string.Format("Invalid list index {0} in list of {1} values", objIndex, objectsList.Count));
			objIndex = objectsList.Count - 1;
		}

		if (objectsList[objIndex].ObjectValue == null)
			BindListElementVarInstance(objIndex);
		return objectsList[objIndex].ObjectValue != null
			? objectsList[objIndex].ObjectValue
			: objectsList[objIndex].ObjectInfoStr;
	}
	...
	public void SetObject(int objIndex, object obj) {
		if (objIndex < 0 || objIndex >= objectsList.Count)
			return;
		var listObjectInfo = new ListObjectInfo(obj);
	...
	public virtual int RemoveObjectInstanceByGuid(Guid objGuid) {
		var index1 = -1;
		for (var index2 = 0; index2 < objectsList.Count; ++index2)
			if (objectsList[index2].ObjectValue != null &&
			    typeof(IEngineInstanced).IsAssignableFrom(objectsList[index2].ObjectValue.GetType()) &&
			    ((IEngineInstanced)objectsList[index2].ObjectValue).EngineGuid == objGuid) {
				index1 = index2;
				break;
			}
	...
	public virtual void Merge(ICommonList mergeList) {
		var vmCommonList = (VMCommonList)mergeList;
		for (var index = 0; index < vmCommonList.objectsList.Count; ++index)
			objectsList.Add(new ListObjectInfo(vmCommonList.objectsList[index]));
	}

	public bool CheckObjectExist(object obj) {
		return GetObjectIndex(obj) >= 0;
	}

	public bool RemoveObjectFromList(object obj) {
		var objectIndex = GetObjectIndex(obj);
	...
	public int GetObjectIndex(object obj) {
		for (var objIndex = 0; objIndex < ObjectsCount; ++objIndex) {
			var otherVar = GetObject(objIndex);
	...
	public virtual void Read(string text) {
		...
		if (text.Length <= 1)
			return;
		var separator = new string[1] { "LIST&ELEM" };
		foreach (var data in text.Split(separator, StringSplitOptions.RemoveEmptyEntries)) {
			var listObjectInfo = new ListObjectInfo();
	...
	public int GetListIndexOfMaxValue() {
		...
		for (var index = 0; index < objectsList.Count; ++index)
			if (objectsList[index].ObjectValue != null && objectsList[index].ObjectValue.GetType().IsValueType) {
				var single = Convert.ToSingle(objectsList[index].ObjectValue);
	...
	private void BindListElementVarInstance(int objIndex) {
		var objectInfoStr = objectsList[objIndex].ObjectInfoStr;
		...
		if (commonVariable.IsBinded && commonVariable.Variable != null) {
			...
			var type = commonVariable.Variable.GetType();
			...
			Logger.AddError(string.Format("List element binding error: cannot find variable for variable info {0}",
				objectsList[objIndex].ObjectInfoStr));
	}
}