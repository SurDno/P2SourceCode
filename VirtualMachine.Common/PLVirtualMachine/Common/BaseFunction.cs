...
public class BaseFunction : IVariable, INamed {
	...
	public bool HasOutput => OutputParam != null && OutputParam.Type != null && null != OutputParam.Type.BaseType &&
	                         OutputParam.Type.BaseType != typeof(void);

	public virtual bool IsEqual(IVariable other) {
		if (!typeof(BaseFunction).IsAssignableFrom(other.GetType()))
			return false;
		var baseFunction = (BaseFunction)other;
		return (parentComponentName == null || !("" != parentComponentName) ||
		        !(parentComponentName != baseFunction.parentComponentName)) && Name == baseFunction.Name;
	}
