...
public class AbstractActionInfo {
	...
	public AbstractActionInfo(IAbstractAction actionInstance) {
	...
	public void Update() {
		isValid = true;
		var gameRoot = IStaticDataContainer.StaticDataContainer.GameRoot;
		if (instance.ActionType == EActionType.ACTION_TYPE_NONE)
			return;
		var targetObject = instance.TargetObject;
		if (targetObject == null)
			...
			var ownerContext1 = (IGameObjectContext)gameRoot;
			...
			if (targetObject.IsBinded) {
				if (targetObject.VariableContext != null) {
					var variableContext = targetObject.VariableContext;
					if (typeof(IBlueprint).IsAssignableFrom(variableContext.GetType()) &&
					    !((ILogicObject)variableContext).Static && ownerContext1 != null &&
					    typeof(IBlueprint).IsAssignableFrom(ownerContext1.GetType()) &&
					    !((IBlueprint)ownerContext1).IsDerivedFrom(((IEditorBaseTemplate)variableContext).BaseGuid,
						    true)) {
						isValid = false;
						return;
					}
				...
				var vmTypeList = new List<VMType>();
				if (instance.ActionType == EActionType.ACTION_TYPE_DO_FUNCTION) {
					targetFunction = null;
					var contextVariable = targetObject.GetContextVariable(instance.TargetFunction);
					...
					if (targetFunction != null) {
						for (var index = 0; index < targetFunction.InputParams.Count; ++index)
						...
						if (instance.TargetParam != null && !instance.TargetParam.IsNull && targetFunction.HasOutput) {
							var localContext = instance.LocalContext;
							if (localContext != null && localContext.Owner != null) {
								instance.TargetParam.Bind((IContext)localContext.Owner, targetFunction.OutputParam.Type,
									localContext);
				...
				} else if (instance.ActionType == EActionType.ACTION_TYPE_SET_PARAM ||
				           instance.ActionType == EActionType.ACTION_TYPE_MATH ||
				           instance.ActionType == EActionType.ACTION_TYPE_SET_EXPRESSION) {
					targetFunction = null;
					var localContext = instance.LocalContext;
					var ownerContext2 = (IContext)localContext.Owner;
					if (targetObject != null && targetObject.VariableContext != null)
						ownerContext2 = targetObject.VariableContext;
					instance.TargetParam.Bind(ownerContext2, localContext: localContext,
						contextElement: contextElement);
					if (instance.TargetParam.IsBinded) {
						var type = instance.TargetParam.Type;
						if (instance.ActionType == EActionType.ACTION_TYPE_MATH && !type.IsNumber) {
							isValid = false;
							return;
						}

						if (instance.ActionType == EActionType.ACTION_TYPE_MATH && instance.MathOperationType ==
						    EMathOperationType.ACTION_OPERATION_TYPE_NONE) {
							isValid = false;
							return;
						}
					...
					for (var index = 0; index < targetEvent.Event.ReturnMessages.Count; ++index)
						vmTypeList.Add(targetEvent.Event.ReturnMessages[index].Type);
				}

				if (vmTypeList.Count <= 0)
					return;
				if (instance.ActionType == EActionType.ACTION_TYPE_SET_EXPRESSION &&
				    typeof(ISingleAction).IsAssignableFrom(instance.GetType())) {
					if (((ISingleAction)instance).SourceExpression == null)
						...
						if (VMTypeUtility.IsTypesCompatible(instance.TargetParam.Type,
							    ((ISingleAction)instance).SourceExpression.ResultType))
							return;
						isValid = false;
					}
					...
					for (var index = 0; index < vmTypeList.Count; ++index)
						if (index >= instance.SourceParams.Count)
							isValid = false;
						else {
							var localContext = instance.LocalContext;
							var sourceParam = instance.SourceParams[index];
							sourceParam.Bind((IContext)localContext.Owner, vmTypeList[index], localContext,
								contextElement);
