...
public static class SaveManagerUtility {
	...
	public static void SaveEnum<T>(IDataWriter writer, string name, T value)
	...
	public static void SaveDynamicSerializableList<T>(
		...
		foreach (var obj in value)
			if (obj != null) {
				if (obj is ISerializeStateSave serializeStateSave) {
					if (name == "HierarchyObjects" &&
					    typeof(VMBaseEntity).IsAssignableFrom(serializeStateSave.GetType())) {
						var vmBaseEntity = (VMBaseEntity)serializeStateSave;
						if (!vmBaseEntity.IsHierarchy && !vmBaseEntity.IsEngineRoot)
							EngineAPIManager.Instance.SetFatalError(string.Format(
								"UNUSUAL HIERARCHY SYSTEM ERROR: INCORRECT OBJECT {0} WORLD ADDING !!!",
								vmBaseEntity.Name));
					}
	...
	public static void SaveCommon(IDataWriter writer, string name, object value) {
		switch (value) {
			...
			case IVMStringSerializable stringSerializable:
				...
				break;
			default:
				var str = StringSerializer.WriteValue(value);
				Save(writer, name, str);
				break;
		}
	}
