...
public struct HierarchyGuid {
	...
	public HierarchyGuid(string data) {
		...
		if (!TryParse(data, out result))
			Logger.AddError(
				"Invalid hierarchy guid format: no hierarchy splitter or incorrect symbols count in string, data : '" +
				data + "' , state : " + EngineAPIManager.Instance.CurrentFSMStateInfo);
		else
			guids = result.guids;
	}
	...
	private HierarchyGuid(ulong[] guids) {
		this.guids = guids;
	}
	...
	public static bool operator ==(HierarchyGuid a, HierarchyGuid b) {
		...
		if (a.guids == null || b.guids == null || a.guids.Length != b.guids.Length)
			return false;
		for (var index = 0; index < b.guids.Length; ++index)
