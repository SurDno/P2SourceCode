...
public class CommonVariable : ContextVariable, IVMStringSerializable {
	...
	public override EContextVariableCategory Category =>
		commonVariableType == ECommonVariableType.CV_TYPE_CONTEXT_VARIABLE
			? ((IVariable)value).Category
			: EContextVariableCategory.CONTEXT_VARIABLE_CATEGORY_ALL;
	...
	public override IEnumerable<IVariable> GetContextVariables(
		...
		return IsSelf
			? contextObj.GetContextVariables(contextVarCategory)
			: base.GetContextVariables(contextVarCategory);
	}
	...
	public void Bind(
		...
		try {
			if (contextData == "" && variableData == "" && needType != null &&
			    typeof(IRef).IsAssignableFrom(needType.BaseType)) {
				value = BaseSerializer.GetDefaultValue(needType.BaseType);
				binded = true;
				commonVariableType = ECommonVariableType.CV_TYPE_CONSTANT;
			}

			var globalContext = (IGameObjectContext)IStaticDataContainer.StaticDataContainer.GameRoot;
			...
			if ("" != contextData) {
				contextObj =
					IVariableService.Instance.GetContextByData(globalContext, contextData, localContext,
						contextElement);
				if (contextObj != null) {
					if (contextData == variableData && typeof(ILogicObject).IsAssignableFrom(contextObj.GetType())) {
						...
						if ("" != variableData)
							value = IVariableService.Instance.GetContextVariableByData(contextObj, variableData,
								localContext, contextElement);
						else if (typeof(ILogicObject).IsAssignableFrom(contextObj.GetType()))
							value = ((ILogicObject)contextObj).GetSelf();
						if (typeof(ILogicObject).IsAssignableFrom(contextObj.GetType()) &&
						    !((ILogicObject)contextObj).Static) {
							var flag = false;
							if (ownerContext != null &&
							    ((ILogicObject)ownerContext).Blueprint.IsDerivedFrom(
								    ((ILogicObject)contextObj).Blueprint.BaseGuid, true))
								flag = true;
							if (!flag) {
								binded = false;
								Logger.AddError(string.Format("Context {0} must be static at {1}", contextObj.Name,
									EngineAPIManager.Instance.CurrentFSMStateInfo));
								return;
							}
						}

						if (variableData == "none" && needType != null &&
						    typeof(IRef).IsAssignableFrom(needType.BaseType)) {
							value = BaseSerializer.GetDefaultValue(needType.BaseType);
							binded = true;
							commonVariableType = ECommonVariableType.CV_TYPE_CONSTANT;
						...
						if (value == null && typeof(IParam).IsAssignableFrom(contextObj.GetType()) &&
						    contextData == variableData) {
							value = contextObj;
				...
				var staticContextByData = GetStaticContextByData(variableData);
				var flag = false;
				...
				if (!flag) {
					...
					if (variableData.StartsWith("group_")) {
						...
						((ContextVariable)value).Initialize(variableData,
							new VMType(typeof(IObjRef), variableData.Substring("group_".Length)));
						commonVariableType = ECommonVariableType.CV_TYPE_CONTEXT_VARIABLE;
						binded = true;
					} else
						value = IVariableService.Instance.GetContextVariableByData(ownerContext, variableData,
							localContext, contextElement);
					...
					if (value == null) {
						contextObj = globalContext;
						var contextByData = IVariableService.Instance.GetContextByData(globalContext, variableData);
						if (contextByData != null) {
							if (typeof(ILogicObject).IsAssignableFrom(contextByData.GetType())) {
								...
								if (value == null && ((IParam)contextByData).OwnerContext != null) {
									var ownerContext1 = ((IParam)contextByData).OwnerContext;
									if (ownerContext1 != null &&
									    typeof(ILogicObject).IsAssignableFrom(ownerContext1.GetType()) &&
									    ownerContext != null &&
									    typeof(ILogicObject).IsAssignableFrom(ownerContext.GetType()) &&
									    ((ILogicObject)ownerContext).Blueprint.IsDerivedFrom(
										    ownerContext1.Blueprint.BaseGuid, true)) {
			...
			if ((!binded || commonVariableType == ECommonVariableType.CV_TYPE_CONSTANT) && value == null &&
			    needType != null && !CommonVariableUtility.IsLocalVariableData(variableData) &&
			    !TryGetConstantValue(needType)) {
				binded = false;
				return;
			}
			...
			if (value != null)
				if (variableData != "none") {
					if (commonVariableType != ECommonVariableType.CV_TYPE_CONSTANT)
						commonVariableType = !IsSelf
							? ECommonVariableType.CV_TYPE_CONTEXT_VARIABLE
							: ECommonVariableType.CV_TYPE_CONTEXT_SELF;
					var secondType = !typeof(IVariable).IsAssignableFrom(value.GetType())
						? new VMType(value.GetType())
						: ((IVariable)value).Type;
					if (secondType == null) {
						binded = false;
						Logger.AddError(string.Format("Variable {0} value type is undefined at {1}", ToString(),
							EngineAPIManager.Instance.CurrentFSMStateInfo));
					}

					if (needType != null) {
						var isWeak = variableData == "none";
	...
	private ILogicObject GetStaticContextByData(string data = "") {
		if (data == "")
			data = contextData;
		var guidFormat = GuidUtility.GetGuidFormat(data);
		if (guidFormat == EGuidFormat.GT_BASE)
			return GetTemplateByGuid(DefaultConverter.ParseUlong(data));
		return EGuidFormat.GT_HIERARCHY == guidFormat
			? IStaticDataContainer.StaticDataContainer.GameRoot.GetWorldHierarhyObjectByGuid(new HierarchyGuid(data))
			: (ILogicObject)null;
	}
	...
	private void CalculateSelf() {
		...
		if (value != null && contextObj != null && typeof(IGameObjectContext).IsAssignableFrom(contextObj.GetType())) {
			if (typeof(IRef).IsAssignableFrom(value.GetType())) {
				if (((IRef)value).StaticInstance != null && (long)((IRef)value).StaticInstance.BaseGuid ==
				    (long)((IEditorBaseTemplate)contextObj).BaseGuid)
					isSelf = true;
	...
	private void MakeVariableContext() {
		...
		if (typeof(IContext).IsAssignableFrom(value.GetType()))
			...
			if (!typeof(IRef).IsAssignableFrom(value.GetType()))
				return;
			var staticInstance = ((IRef)value).StaticInstance;
	...
	private bool IsDataNull(string data) {
		return data == "0" || variableData == "none";
	}
