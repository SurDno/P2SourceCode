...
// Type: PLVirtualMachine.ExpressionUtility
...
namespace PLVirtualMachine
{
  public static class ExpressionUtility
  {
    ...
    private static EntityMethodExecuteData lastActionMethodExecuteData = (EntityMethodExecuteData) null;
    ...
    {
      ExpressionUtility.inputParamsList.Clear();
      ExpressionUtility.inputTypesList.Clear();
      ExpressionUtility.lastActionMethodExecuteData = (EntityMethodExecuteData) null;
    }
    ...
    public static bool CalculateConditionResult(
      ...
      INamed actor = null)
    {
      return ExpressionUtility.DoCalculateConditionResult(condition, dynamicObjContext, actor);
    }
      ...
      INamed actor = null)
    {
      if (condition == null)
      {
        Logger.AddError(string.Format("Condition not defined at {0}!", (object) DynamicFSM.CurrentStateInfo));
        return false;
      }
      ...
      try
      {
        if (!condition.IsPart())
        {
          ...
          if (vmCondition.Operation == EConditionOperation.COP_XOR)
          {
            ...
            {
              if (ExpressionUtility.DoCalculateConditionResult(vmCondition.Predicates[index], dynamicObjContext, actor))
                flag2 = true;
              else
                flag1 = false;
            }
        ...
        if (vmPartCondition.ConditionType == EConditionType.CONDITION_TYPE_VALUE_EXPRESSION)
        {
          ...
          {
            Logger.AddError(string.Format("Expression not definied at {0}!", (object) DynamicFSM.CurrentStateInfo));
            return false;
          }
          if (vmPartCondition.FirstExpression.ResultType.BaseType != typeof (bool))
          {
            Logger.AddError(string.Format("This expression type must be boolean at {0} !", (object) DynamicFSM.CurrentStateInfo));
            return false;
          }
          object expressionResult = ExpressionUtility.CalculateExpressionResult(vmPartCondition.FirstExpression, dynamicObjContext);
          if (expressionResult.GetType() == typeof (bool))
            return (bool) expressionResult;
          Logger.AddError(string.Format("This expression return value must be boolean at {0}!", (object) DynamicFSM.CurrentStateInfo));
          return false;
        }
        if (vmPartCondition.FirstExpression == null)
        {
          Logger.AddError(string.Format("Condition {0} first expression not defined at {1}!", (object) vmPartCondition.BaseGuid, (object) DynamicFSM.CurrentStateInfo));
          return false;
        }
        if (vmPartCondition.SecondExpression == null)
        {
          Logger.AddError(string.Format("Condition {0} second expression not defined at {1}!", (object) vmPartCondition.BaseGuid, (object) DynamicFSM.CurrentStateInfo));
          return false;
        }
        VMType resultType1 = vmPartCondition.FirstExpression.ResultType;
        if (resultType1 == null)
        {
          Logger.AddError(string.Format("Condition {0} first expression {1} is invalid at {2}!", (object) vmPartCondition.BaseGuid, (object) "unknown", (object) DynamicFSM.CurrentStateInfo));
          return false;
        }
        ...
        {
          if (actor != null)
            Logger.AddError(string.Format("Condition expressions types are not compatible at {0}!", (object) actor.Name));
          else
            Logger.AddError(string.Format("Condition expressions types are not compatible at {0}!", (object) DynamicFSM.CurrentStateInfo));
          return false;
        }
        object expressionResult1 = ExpressionUtility.CalculateExpressionResult(vmPartCondition.FirstExpression, dynamicObjContext);
        object expressionResult2 = ExpressionUtility.CalculateExpressionResult(vmPartCondition.SecondExpression, dynamicObjContext);
      ...
      {
        Logger.AddError(string.Format("Calculating condition result error: {0} at {1}", (object) ex.ToString(), (object) DynamicFSM.CurrentStateInfo));
      }
      return false;
    }
    ...
    {
      return ExpressionUtility.lastActionMethodExecuteData;
    }
    ...
    public static object CalculateExpressionResult(
      ...
      IDynamicGameObjectContext dynamicObjContext)
    {
      if (!ExpressionUtility.StaticInited)
        ExpressionUtility.InitStatic();
      ...
      if (expression.Type == ExpressionType.EXPRESSION_SRC_PARAM)
      {
        ...
        if (targetParam == null)
        {
          Logger.AddError(string.Format("Expression {0}: target param is null at {1} !!!", (object) expression.BaseGuid, (object) DynamicFSM.CurrentStateInfo));
          return (object) null;
        }
        if (!targetParam.IsBinded)
        {
          Logger.AddError(string.Format("Expression {0}: target param {1} not defined at {2}", (object) expression.BaseGuid, (object) targetParam.ToString(), (object) DynamicFSM.CurrentStateInfo));
          return (object) null;
        }
        ...
        expressionResult2.Initialize(((Engine.Common.IObject) expressionResult1).Id);
        return (object) expressionResult2;
      }
      if (expression.Type == ExpressionType.EXPRESSION_SRC_FUNCTION)
      {
        ...
        if (dynamicContext == null)
        {
          ...
          Logger.AddError(string.Format("No target entity found for method {0} at {1}", (object) expression.TargetFunction, (object) DynamicFSM.CurrentStateInfo));
        }
        else
        {
          ...
          {
            Logger.AddError(string.Format("Expression function {0} not found at {1}", (object) expression.TargetFunction, (object) DynamicFSM.CurrentStateInfo));
            return (object) null;
          }
          if (contextFunction.Name == ExpressionUtility.GetDayTimeFunctionName || contextFunction.Name == ExpressionUtility.GetObjectClassFunctionName || contextFunction.Name == ExpressionUtility.GetItemsCountByTemplateFunctionName || contextFunction.Name == ExpressionUtility.SetMultiStorablesFixedPricesFunctionName)
            return ExpressionUtility.DoFastFunctionCall(contextFunction, expression.SourceParams, dynamicObjContext);
          ExpressionUtility.inputParamsList.Clear();
          ExpressionUtility.inputTypesList.Clear();
          for (int index = 0; index < contextFunction.InputParams.Count; ++index)
          {
            ...
            ExpressionUtility.inputTypesList.Add(type);
            ExpressionUtility.inputParamsList.Add(dynamicVariableValue);
          }
          Type componentApiType = contextFunction.ParentComponentAPIType;
          object expressionResult = (object) null;
          if (contextFunction.Entity != null)
          {
            expressionResult = VMEngineAPIManager.ExecMethod(dynamicObjContext, contextFunction.Entity.EngineGuid, componentApiType, contextFunction.ParentComponentAPIName, contextFunction.APIName, ExpressionUtility.inputParamsList, ExpressionUtility.inputTypesList, contextFunction.Type);
            if (expression.Inversion)
              expressionResult = VMTypeMathUtility.MakeInversion(expressionResult);
          }
          else
            Logger.AddError(string.Format("No target entity found for method {0} at {1}", (object) contextFunction.Name, (object) DynamicFSM.CurrentStateInfo));
      ...
      else if (expression.Type == ExpressionType.EXPRESSION_SRC_COMPLEX)
      {
        ...
        {
          Logger.AddError(string.Format("Complex expression {0} result type must be number at {1}", (object) expression.BaseGuid, (object) DynamicFSM.CurrentStateInfo));
          return (object) 0;
        }
        ...
        for (int childIndex = 0; childIndex < expressionsCount; ++childIndex)
        {
          ...
          if (dOperations[0] == FormulaOperation.FORMULA_OP_NONE)
          {
            ...
            {
              num += prevValue;
              prevValue = ExpressionUtility.CalculateExpressionValue(0.0, childExpression, dOperations, dynamicObjContext);
            }
            else
              Logger.AddError(string.Format("Operation on child expression index = {0} not defined in complex expression guid={1} at {2}", (object) childIndex, (object) expression.BaseGuid, (object) DynamicFSM.CurrentStateInfo));
          }
          else if (dOperations[0] == FormulaOperation.FORMULA_OP_PLUS || dOperations[0] == FormulaOperation.FORMULA_OP_MINUS)
          {
            ...
            prevValue = ExpressionUtility.CalculateExpressionValue(prevValue, childExpression, dOperations, dynamicObjContext);
          else if (childIndex == 0 && (dOperations[0] == FormulaOperation.FORMULA_OP_COS || dOperations[0] == FormulaOperation.FORMULA_OP_SIN || dOperations[0] == FormulaOperation.FORMULA_OP_EXP || dOperations[0] == FormulaOperation.FORMULA_OP_LOG || dOperations[0] == FormulaOperation.FORMULA_OP_LOG10))
          {
            ...
            Logger.AddError(string.Format("Base operation on child expression index = {0} not defined in complex expression guid={1} at {2}", (object) childIndex, (object) expression.BaseGuid, (object) DynamicFSM.CurrentStateInfo));
        }
        object expressionResult = Convert.ChangeType((object) (num + prevValue), expression.ResultType.BaseType);
        if (expression.Inversion)
          expressionResult = VMTypeMathUtility.MakeInversion(expressionResult);
        return expressionResult;
      }
    ...
    public static double CalculateExpressionValue(
      ...
      IDynamicGameObjectContext dynamicObjContext)
    {
      object expressionResult = ExpressionUtility.CalculateExpressionResult((IExpression) expression, dynamicObjContext);
      if (!VMTypeUtility.IsTypeNumber(expressionResult.GetType()) && !VMTypeUtility.IsTypeNumber(expression.ResultType.BaseType))
      {
        Logger.AddError(string.Format("Child expression {0} result type in complex expression must be number at {1}", (object) expression.BaseGuid, (object) DynamicFSM.CurrentStateInfo));
        return 0.0;
      }
      ...
      if (dOperations.Count > 1)
      {
        ...
        switch (dOperation)
        {
          ...
          case FormulaOperation.FORMULA_OP_COS:
            ...
            break;
          default:
            Logger.AddError(string.Format("Invalid child expression {0} secondary operation: {1} at {2}", (object) expression.BaseGuid, (object) dOperation.ToString(), (object) DynamicFSM.CurrentStateInfo));
            break;
        }
      }
      ...
      switch (dOperation1)
      {
        ...
        case FormulaOperation.FORMULA_OP_RDIVIDE:
          ...
          expressionValue = (double) ((int) num1 % (int) Math.Floor(expressionValue)) + num2;
          goto case FormulaOperation.FORMULA_OP_NONE;
        case FormulaOperation.FORMULA_OP_POWER:
          expressionValue = Math.Pow(prevValue, expressionValue);
          goto case FormulaOperation.FORMULA_OP_NONE;
        default:
          Logger.AddError(string.Format("Invalid child expression {0} base operation: {1} at {2}", (object) expression.BaseGuid, (object) dOperation1.ToString(), (object) DynamicFSM.CurrentStateInfo));
          goto case FormulaOperation.FORMULA_OP_NONE;
      }
    }
      ...
      ILocalContext localContext = null)
    {
      ...
      if (localContext == null)
        localContext = (ILocalContext) dynamicObjContext.CurrentState;
      variable.Bind((IContext) dynamicObjContext.FSMStaticObject, paramType, localContext);
      ...
      DynamicFSM contextObjFSM = null)
    {
      ...
      try
      {
        ...
        if (!action.IsValid)
        {
          ...
          if (!action.IsValid)
          {
            Logger.AddError(string.Format("Action {0} has errors {1} at {2}:", (object) str, (object) "unknown", (object) DynamicFSM.CurrentStateInfo));
            action.Update();
          }
        }
        if (action.ActionType == EActionType.ACTION_TYPE_SET_PARAM || action.ActionType == EActionType.ACTION_TYPE_MATH)
        {
          ...
          try
          {
            ...
            if (targetParam == null)
              Logger.AddError(string.Format("Action {0}: target param is null at {1} !!!", (object) str, (object) DynamicFSM.CurrentStateInfo));
            else if (!targetParam.IsBinded)
              Logger.AddError(string.Format("Action {0}: target param {1} not defined at {2}", (object) str, (object) targetParam.ToString(), (object) DynamicFSM.CurrentStateInfo));
            else if (action.SourceParams.Count < 1)
            {
              Logger.AddError(string.Format("Action {0}: source param not defined at {1}", (object) str, (object) DynamicFSM.CurrentStateInfo));
            }
            else
            {
              CommonVariable sourceParam = action.SourceParams[0];
              if (sourceParam == null)
                Logger.AddError(string.Format("Action {0}: source param is null at {1} !!!", (object) str, (object) DynamicFSM.CurrentStateInfo));
              else if (!sourceParam.IsBinded)
                Logger.AddError(string.Format("Action {0}: source param {1} not defined at {2}", (object) str, (object) sourceParam.ToString(), (object) DynamicFSM.CurrentStateInfo));
              else if (targetParam.CommonVariableType != ECommonVariableType.CV_TYPE_CONTEXT_VARIABLE)
              {
                Logger.AddError(string.Format("Action {0}: target param {1} must be parameter at {2}", (object) str, (object) targetParam.ToString(), (object) DynamicFSM.CurrentStateInfo));
              }
              else
              {
                ...
                if (obj != null)
                {
                  ...
                  else
                  {
                    ...
                    if (dynamicVariableValue != null)
                    {
                      ...
                      Logger.AddError(string.Format("Action {0}: source math operation param value not defined at {1}", (object) str, (object) DynamicFSM.CurrentStateInfo));
                  }
                }
                else
                  Logger.AddError(string.Format("Action {0}: target param {1} not found at {2}", (object) str, (object) targetParam.ToString(), (object) DynamicFSM.CurrentStateInfo));
              }
            }
          }
          catch (Exception ex)
          {
            Logger.AddError(string.Format("Action {0}: set param error: {1} at {2}", (object) str, (object) ex, (object) DynamicFSM.CurrentStateInfo));
          }
        }
        else if (action.ActionType == EActionType.ACTION_TYPE_DO_FUNCTION)
        {
          ...
          {
            Logger.AddError(string.Format("No target entity found for method {0} at {1}", (object) action.TargetFunction, (object) DynamicFSM.CurrentStateInfo));
          }
          else
          {
            ...
            if (contextFunction == null)
            {
              Logger.AddError(string.Format("Action function not found at {0}", (object) DynamicFSM.CurrentStateInfo));
            }
            else
            {
              if (contextFunction.Name == ExpressionUtility.GetDayTimeFunctionName || contextFunction.Name == ExpressionUtility.GetObjectClassFunctionName || contextFunction.Name == ExpressionUtility.GetItemsCountByTemplateFunctionName || contextFunction.Name == ExpressionUtility.SetMultiStorablesFixedPricesFunctionName)
              {
                ExpressionUtility.DoFastFunctionCall(contextFunction, action.SourceParams, dynamicObjContext);
                if (action.TargetParam != null && contextFunction.HasOutput)
                {
                  if (action.TargetParam.IsBinded)
                  {
                    ...
                    if (dynamicParam != null)
                      dynamicParam.Value = ExpressionUtility.DoFastFunctionCall(contextFunction, action.SourceParams, dynamicObjContext);
                  }
                }
                else
                  ExpressionUtility.DoFastFunctionCall(contextFunction, action.SourceParams, dynamicObjContext);
              ...
              else
              {
                ...
                if (contextFunction.Entity != null)
                {
                  obj = VMEngineAPIManager.ExecMethod(dynamicObjContext, contextFunction.Entity.EngineGuid, componentApiType, contextFunction.ParentComponentAPIName, contextFunction.APIName, ExpressionUtility.inputParamsList, ExpressionUtility.inputTypesList, contextFunction.Type);
                }
                else
                {
                  Logger.AddError(string.Format("No target entity found for method {0} at {1}", (object) contextFunction.Name, (object) DynamicFSM.CurrentStateInfo));
              ...
              ExpressionUtility.lastActionMethodExecuteData = VMEngineAPIManager.EntityMethodExecuteData;
            }
          }
        }
        else if (action.ActionType == EActionType.ACTION_TYPE_SET_EXPRESSION)
        {
          ...
          {
            Logger.AddError(string.Format("Source expression not defined in action with guid = {0} at {1}", (object) "unknown", (object) DynamicFSM.CurrentStateInfo));
          }
          else
          {
            object expressionResult = ExpressionUtility.CalculateExpressionResult(((ISingleAction) action).SourceExpression, dynamicObjContext);
        ...
        else
        {
          ...
          if (contextObjFSM != null)
            ownerContext = (IDynamicGameObjectContext) contextObjFSM;
          IDynamicGameObjectContext dynamicContext = instance.GetDynamicContext(action.TargetObject.VariableContext, ownerContext);
          if (dynamicContext == null)
          {
            Logger.AddError(string.Format("Action event target object not found at {0}", (object) DynamicFSM.CurrentStateInfo));
          }
          else
          {
            ...
            {
              Logger.AddError(string.Format("Action raising event not found at {0}", (object) DynamicFSM.CurrentStateInfo));
            }
            else
            {
              ...
              {
                Logger.AddError(string.Format("Event raising error: action {0} source params count not equal to event {1} messages count at {2}", (object) str, (object) contextEvent.StaticEvent.Name, (object) DynamicFSM.CurrentStateInfo));
                if (action.SourceParams.Count < contextEvent.StaticEvent.ReturnMessages.Count)
              ...
              for (int index = 0; index < count; ++index)
              {
                ...
                object contextParamValue = ExpressionUtility.GetContextParamValue(dynamicObjContext, sourceParam, type);
      ...
      double elapsedTime)
    {
      ...
      if (!ExpressionUtility.execFunctionsTimingDict.ContainsKey(key))
      {
        ExpressionUtility.execFunctionsTimingDict.Add(key, 0.0);
        ExpressionUtility.execFunctionsCountDict.Add(key, 0);
      }
      ExpressionUtility.execFunctionsTimingDict[key] += elapsedTime;
      ++ExpressionUtility.execFunctionsCountDict[key];
    }
    ...
    {
      VMVariableService instance = (VMVariableService) IVariableService.Instance;
      if (dynamicObjFunction.Name == ExpressionUtility.GetDayTimeFunctionName)
        return (object) ((VMGameComponent) dynamicObjFunction.Entity.GetComponentByName("GameComponent")).GetCurrDayTime();
      if (dynamicObjFunction.Name == ExpressionUtility.GetObjectClassFunctionName)
        return (object) ((VMGameComponent) dynamicObjFunction.Entity.GetComponentByName("GameComponent")).GetObjectClass((IObjRef) instance.GetDynamicVariableValue(sourceParams[0], dynamicObjFunction.InputParams[0].Type, dynamicObjContext));
      if (dynamicObjFunction.Name == ExpressionUtility.GetItemsCountByTemplateFunctionName)
      {
        ...
        if (Guid.Empty != engineTemplateGuid)
        {
          ...
          if (template != null)
            return (object) componentByName.GetItemsCountByTemplate(template);
        }
        else
      ...
      else if (dynamicObjFunction.Name == ExpressionUtility.SetMultiStorablesFixedPricesFunctionName)
      {
    ...
    private static bool StaticInited { get; set; } = false;

    private static void InitStatic()
    {
      ExpressionUtility.GetDayTimeFunctionName = EngineAPIManager.GetSpecialFunctionName(ESpecialFunctionName.SFN_GET_DAY_TIME, typeof (VMGameComponent), true);
      ExpressionUtility.GetObjectClassFunctionName = EngineAPIManager.GetSpecialFunctionName(ESpecialFunctionName.SFN_GET_OBJECT_CLASS, typeof (VMGameComponent), true);
      ExpressionUtility.GetItemsCountByTemplateFunctionName = EngineAPIManager.GetSpecialFunctionName(ESpecialFunctionName.SFN_GET_ITEMS_COUNT_BY_TEMPLATE, typeof (VMStorage), true);
      ExpressionUtility.SetMultiStorablesFixedPricesFunctionName = EngineAPIManager.GetSpecialFunctionName(ESpecialFunctionName.SFN_SET_MULTI_STORABLES_FIXED_PRICES, typeof (VMMarket), true);
      ExpressionUtility.StaticInited = true;
    }
  }
}
