...
public class VMActionLine :
	...
	private List<IGameAction> actionsList = new();

	[FieldData("LocalContext", DataFieldType.Reference)]
	private ILocalContext localContext;

	[FieldData("ActionLineType")] private EActionLineType actionLineType;
	[FieldData("OrderIndex")] private int orderIndex;
	[FieldData("ActionLoopInfo")] private ActionLoopInfoData actionLoopInfo;
	...
	private List<IVariable> loopLocalVariables = new();
		...
		while (xml.Read()) {
			if (xml.NodeType == XmlNodeType.Element)
				switch (xml.Name) {
					...
					case "ActionLoopInfo":
						actionLoopInfo =
							EditorDataReadUtility.ReadEditorDataSerializable<ActionLoopInfoData>(xml, creator,
								typeContext);
						continue;
					default:
	...
	public VMActionLine(ulong guid) {
		this.guid = guid;
	}
	...
	public IVariable GetLocalContextVariable(string name) {
		foreach (var loopLocalVariable in loopLocalVariables)
		...
		get {
			...
			if (loopListParamInstance == null) {
				...
				if (localContext != null && localContext.Owner != null &&
				    typeof(IContext).IsAssignableFrom(localContext.Owner.GetType()))
					ownerContext = (IContext)localContext.Owner;
				loopListParam.Bind(ownerContext, localContext: localContext);
				if (loopListParam.Variable != null &&
				    typeof(IVariable).IsAssignableFrom(loopListParam.Variable.GetType()))
	...
	public bool IsEqual(IObject other) {
		return other != null && typeof(VMActionLine) == other.GetType() &&
		       (long)BaseGuid == (long)((VMActionLine)other).BaseGuid;
	}
	...
	public void Update() {
		...
		if (ActionLineType == EActionLineType.ACTION_LINE_TYPE_LOOP) {
			...
			if (actionLoopInfo != null) {
				var start = actionLoopInfo.Start;
				var end = actionLoopInfo.End;
	...
	private void MakeLoopLocalVariables() {
		...
		var listParamInstance = LoopListParamInstance;
		if (listParamInstance != null) {
			var name = "local_" + BaseGuid + "_Loop_List_" + listParamInstance.Name + "_Element";
			var listElementType = listParamInstance.Type.GetListElementType();
			var localVariable = new LocalVariable();
			localVariable.Initialize(name, listElementType);
			loopLocalVariables.Add(localVariable);
		}

		var name1 = "local_" + BaseGuid + "_Loop_Index";
		var localVariable1 = new LocalVariable();
