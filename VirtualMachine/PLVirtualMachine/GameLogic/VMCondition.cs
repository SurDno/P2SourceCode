...
public class VMCondition : VMPartCondition, IStub, IEditorDataReader {
	...
	private List<ICondition> predicatesList = new();

	[FieldData("Operation")] private EConditionOperation operation = EConditionOperation.COP_NONE;
	...
	public override List<GameTime> GetCheckRaisingTimePoints() {
		var raisingTimePoints1 = new List<GameTime>();
		var num = predicatesList.Count;
		if (Operation == EConditionOperation.COP_ROOT)
			num = 1;
		for (var index = 0; index < num; ++index) {
			var raisingTimePoints2 = ((VMPartCondition)predicatesList[index]).GetCheckRaisingTimePoints();
			if (raisingTimePoints2.Count == 0 &&
			    (Operation == EConditionOperation.COP_OR || Operation == EConditionOperation.COP_XOR))
	...
	public override List<VMParameter> GetCheckRaisingParams() {
		var checkRaisingParams1 = new List<VMParameter>();
		var num = predicatesList.Count;
		if (Operation == EConditionOperation.COP_ROOT)
			num = 1;
		for (var index = 0; index < num; ++index) {
			var checkRaisingParams2 = ((VMPartCondition)predicatesList[index]).GetCheckRaisingParams();
	...
	public override List<BaseFunction> GetCheckRaisingFunctions() {
		var raisingFunctions1 = new List<BaseFunction>();
		var num = predicatesList.Count;
		if (Operation == EConditionOperation.COP_ROOT)
			num = 1;
		for (var index = 0; index < num; ++index) {
			var raisingFunctions2 = ((VMPartCondition)predicatesList[index]).GetCheckRaisingFunctions();
	...
	public override void Update() {
		for (var index = 0; index < predicatesList.Count; ++index)
