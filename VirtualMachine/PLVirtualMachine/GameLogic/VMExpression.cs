...
public class VMExpression :
	...
	IStaticUpdateable {
	private ulong guid;
	[FieldData("ExpressionType")] private ExpressionType expressionType;
	[FieldData("TargetFunctionName")] private string targetFunctionName;
	...
	[FieldData("FormulaOperations")] private List<FormulaOperation> formulaOperations;
	[FieldData("Inversion")] private bool inversion;
	...
	public VMExpression(ulong guid) {
		this.guid = guid;
	}
	...
	public VMType ResultType {
		get {
			...
			if (expressionType == ExpressionType.EXPRESSION_SRC_FUNCTION)
				return functionInstance == null || functionInstance.OutputParam == null
					? VMType.Empty
					: functionInstance.OutputParam.Type;
			if (expressionType == ExpressionType.EXPRESSION_SRC_COMPLEX) {
				if (formulaChilds.Count >= 1) {
					var resultType = formulaChilds[0].ResultType;
					for (var index = 1; index < formulaChilds.Count; ++index)
						...
						else if (formulaChilds[index].ResultType.BaseType == typeof(double) &&
						         resultType.BaseType != typeof(double))
							resultType = formulaChilds[index].ResultType;
					return resultType;
				}
			...
			return expressionType == ExpressionType.EXPRESSION_SRC_CONST && constInstance != null
				? constInstance.Type
				: VMType.Empty;
		}
	}
	...
	public void Update() {
		var baseGuid = (long)BaseGuid;
		if (isUpdated && isValid)
			return;
		if (expressionType == ExpressionType.EXPRESSION_SRC_COMPLEX) {
			var expressionsCount = ChildExpressionsCount;
			isValid = true;
			for (var childIndex = 0; childIndex < expressionsCount; ++childIndex) {
				var childExpression = GetChildExpression(childIndex);
			...
			if (!targetObject.IsBinded) {
				Logger.AddError(string.Format("Expression {0} target object variable {1} not binded", BaseGuid,
					targetObject));
				isValid = false;
				return;
			}
			...
			if (expressionType == ExpressionType.EXPRESSION_SRC_FUNCTION) {
				var contextVariable = targetObject.GetContextVariable(TargetFunction);
				...
				for (var index = 0; index < functionInstance.InputParams.Count; ++index)
			...
			if (vmTypeList.Count > 0)
				for (var index = 0; index < vmTypeList.Count; ++index) {
					...
					var sourceParam = sourceParams[index];
	...
	public bool IsEqual(IObject other) {
		return other != null && typeof(VMExpression) == other.GetType() &&
		       (long)BaseGuid == (long)((VMExpression)other).BaseGuid;
	}
