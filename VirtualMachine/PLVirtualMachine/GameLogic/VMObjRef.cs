...
public class VMObjRef :
	...
	public void Initialize(IBlueprint obj) {
		LoadStaticInstance(obj);
	}
	...
	public override EContextVariableCategory Category =>
		Object == null || Object.GetCategory() != EObjectCategory.OBJECT_CATEGORY_GAME
			? EContextVariableCategory.CONTEXT_VARIABLE_CATEGORY_OBJECT
			: EContextVariableCategory.CONTEXT_VARIABLE_CATEGORY_GAME;

	public Guid EngineTemplateGuid {
		get {
			if (engineInstance != null)
				return engineInstance.EngineTemplateGuid;
			return Object != null && typeof(IWorldObject).IsAssignableFrom(Object.GetType())
				? ((IEngineTemplated)Object).EngineTemplateGuid
				: Guid.Empty;
		}
	}
	...
	public override VMType Type => StaticInstance == null
		? new VMType(typeof(IObjRef))
		: new VMType(typeof(IObjRef), StaticInstance.BaseGuid.ToString());
	...
	protected override void Load() {
		if (!hierarchyGuid.IsEmpty) {
			ILogicObject hierarhyObjectByGuid =
				((VMGameRoot)IStaticDataContainer.StaticDataContainer.GameRoot).GetWorldHierarhyObjectByGuid(
					hierarchyGuid);
			...
			var templateByEngineGuid = WorldEntityUtility.GetEditorTemplateByEngineGuid(engineGuid);
