...
public static class VMSaveLoadManager {
	...
	public static void LoadDynamiSerializableList<T>(XmlElement listNode, List<T> list)
		where T : IDynamicLoadSerializable, new() {
		if (listNode == null)
			return;
		if (list == null)
			list = new List<T>();
	...
	public static int ReadInt(XmlNode valueNode) {
		return DefaultConverter.ParseInt(valueNode.InnerText);
	}
	...
	public static object ReadValue(XmlNode valueNode, Type objType) {
		...
		var type = BaseSerializer.GetRealRefType(objType);
		if (type == null)
			type = objType;
		var instance = (IDynamicLoadSerializable)Activator.CreateInstance(type);
