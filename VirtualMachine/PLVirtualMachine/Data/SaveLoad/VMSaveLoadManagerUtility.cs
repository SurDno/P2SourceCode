...
public static class VMSaveLoadManagerUtility {
	...
	public static void LoadEntities(XmlElement rootNode) {
		...
		foreach (var preloadedNode in preloadedNodes) {
			var key = preloadedNode.Key;
			var xmlNode = preloadedNode.Value;
			var objectEntityByUniName = WorldEntityUtility.GetDynamicObjectEntityByUniName(key);
			if (objectEntityByUniName != null)
				...
				var guid = StringUtility.ToGuid(firstChild.InnerText);
				var uint64 = StringUtility.ToUInt64(firstChild.NextSibling.InnerText);
				var objectByGuid = (IBlueprint)IStaticDataContainer.StaticDataContainer.GetObjectByGuid(uint64);
