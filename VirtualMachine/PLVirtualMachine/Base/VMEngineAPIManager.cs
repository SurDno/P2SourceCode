...
public class VMEngineAPIManager : EngineAPIManager {
	...
	public static object ExecMethod(
		...
		if (componentBaseType == null) {
			Logger.AddError(string.Format("Component base type not defined for method {0}, component {1} at {2}",
				methodName, componentName, DynamicFSM.CurrentStateInfo));
			return null;
		}

		if (objGuid == Guid.Empty) {
			Logger.AddError(string.Format("Cannot exec engine method {0} for entity with id=0 at {1}",
				componentBaseType.Name + "." + methodName, DynamicFSM.CurrentStateInfo));
			return null;
		}

		var entityByEngineGuid = WorldEntityUtility.GetDynamicObjectEntityByEngineGuid(objGuid);
		if (entityByEngineGuid == null) {
			Logger.AddError(string.Format("Object with id={0} not found in virtual machine at {1}", objGuid,
				DynamicFSM.CurrentStateInfo));
			return null;
		}

		if (!entityByEngineGuid.Instantiated && methodName != "Instantiate") {
			Logger.AddError(string.Format("Cannot call method {0} at removed or not instantiated object {1} at {2}",
				methodName, entityByEngineGuid.Name, DynamicFSM.CurrentStateInfo));
			return null;
		}

		if (methodName == GetSpecialFunctionName(ESpecialFunctionName.SFN_OBJECT_INIT, typeof(VMCommon))) {
			CreateEntityUtility.InitializeObject(entityByEngineGuid, (VMLogicObject)entityByEngineGuid.EditorTemplate,
				entityByEngineGuid.GetFSM());
			return null;
		}

		lastMethodExecInitiator = initiator;
		var objectComponentByType = GetObjectComponentByType(entityByEngineGuid, componentBaseType);
		var methodExecuteContext = GetMethodExecuteContext(objectComponentByType, methodName, dParams.Count);
		for (var index = 0; index < methodExecuteContext.InputParamsInfo.Length; ++index)
			try {
				methodExecuteContext.InputParams[index] = CreateHierarchyHelper.ConvertEditorTypeToEngineType(
					dParams[index], methodExecuteContext.InputParamsInfo[index].ParameterType, dParamTypes[index]);
			} catch (Exception ex) {
				Logger.AddError(string.Format("Exec method {0} params processing error: {1} at {2}", methodName, ex,
					DynamicFSM.CurrentStateInfo));
			}

		try {
			var engValue =
				methodExecuteContext.ExecMethodInfo.Invoke(objectComponentByType, methodExecuteContext.InputParams);
			lastEntityMethodExecData.Initialize(entityByEngineGuid, objectComponentByType,
				methodExecuteContext.ExecMethodInfo, methodExecuteContext.InputParams);
			var vmType = outputType;
			return ConvertEngineTypeToEditorType(engValue, vmType);
		} catch (Exception ex) {
			Logger.AddError(string.Format("Call method {0} error in object {1}: {2} at {3}", methodName,
				entityByEngineGuid.Name, ex, DynamicFSM.CurrentStateInfo));
			return null;
		}
	}
	...
	public static object ExecMethod(EntityMethodExecuteData executeData) {
		if (executeData == null)
			return null;
		var targetComponent = executeData.TargetComponent;
		var execMethodInfo = executeData.ExecMethodInfo;
		var inputParams = executeData.InputParams;
		var vmComponent = targetComponent;
		var parameters = inputParams;
	...
	public static object ExecMethodOnComponentDirect(
		...
		string methodName,
		object[] dFactParams) {
		var name = funcComponent.Name;
		var componentMethodInfo = InfoAttribute.GetComponentMethodInfo(name, methodName);
		try {
			return componentMethodInfo.Invoke(funcComponent, dFactParams);
		} catch (Exception ex) {
			Logger.AddError(string.Format("Call direct on component method {0} error in {1}.{2}, error: {3} at {4}",
				methodName, funcComponent.Parent.Name, name, ex, DynamicFSM.CurrentStateInfo));
			return null;
		}
	}
	...
	public override void InstantiateObject(
		...
		if (obj == null)
			...
			var templateByEngineGuid = Instance.GetEditorTemplateByEngineGuid(obj.EngineGuid);
			if (typeof(VMWorldObject).IsAssignableFrom(templateByEngineGuid.GetType())) {
				var vmWorldObject = (VMWorldObject)templateByEngineGuid;
				if (forceSpawnPos == null) {
					var entityByPositionId =
						CreateHierarchyHelper.GetEngineMilestoneEntityByPositionID(vmWorldObject.WorldPositionGuid);
					if (entityByPositionId != null) {
						...
						Logger.AddError(string.Format(
							"Cannot instantiate object {0}, because it's spawn position not defined at {1}",
							templateByEngineGuid.Name, DynamicFSM.CurrentStateInfo));
				} else
					VirtualMachine.Instance.SpawnObject((VMEntity)obj, forceSpawnPos, areaType);
			} else
				Logger.AddError(string.Format("Cannot instantiate not world object {0} at {1}",
					templateByEngineGuid.Name, DynamicFSM.CurrentStateInfo));
		}
	}

	public static VMObjRef ConvertEngineEntityInstanceToVMReference(IEntity engineEntity) {
		var vmReference = new VMObjRef();
	...
	public static object ConvertEngineTypeToEditorType(object engValue, VMType vmType = null) {
		...
		if (flag1 | flag4) {
			...
			if (flag1) {
				Logger.AddError(string.Format("Cannot convert value with type {0} to IObjRef at {1}",
					engValue.GetType(), DynamicFSM.CurrentStateInfo));
				return null;
			}
		}
		...
		if (flag2 | flag4) {
			...
			if (engValue is IEntity entity && ServiceCache.TemplateService.GetTemplate<IEntity>(entity.Id) != null) {
				var editorType = new VMBlueprintRef();
		...
		if (flag3 | flag4) {
			...
			if (typeof(IObject).IsAssignableFrom(engValue.GetType())) {
				var editorType = new VMSampleRef();
				editorType.Initialize(((IObject)engValue).Id);
				return editorType;
			}
			...
			if (typeof(ISampleRef).IsAssignableFrom(engValue.GetType()))
				return engValue;
			if (flag3) {
				Logger.AddError(string.Format("Cannot convert value with type {0} to ISampleRef at {1}",
					engValue.GetType(), DynamicFSM.CurrentStateInfo));
				return null;
			}
		}
		...
		if (engValue.GetType().IsEnum) {
			...
			var num = vmType.BaseType.IsEnum ? 1 : 0;
			return engValue;
		}

		if (typeof(ICommonList).IsAssignableFrom(engValue.GetType())) {
			for (var objIndex = 0; objIndex < ((ICommonList)engValue).ObjectsCount; ++objIndex) {
				var editorType = ConvertEngineTypeToEditorType(((ICommonList)engValue).GetObject(objIndex));
	...
	public static EntityMethodExecuteData EntityMethodExecuteData =>
		lastEntityMethodExecData.TargetEntity != null ? lastEntityMethodExecData : null;

	public static IDynamicGameObjectContext LastMethodExecInitiator => lastMethodExecInitiator;

	public static void OnRestart() {
		lastEntityMethodExecData.Clear();
	}
	...
	public static void Clear() {
		...
		foreach (var keyValuePair1 in componentsAbstractVarsDict) {
			foreach (var keyValuePair2 in keyValuePair1.Value) {
				var variableList = keyValuePair2.Value;
	...
	private static MethodExecuteContext GetMethodExecuteContext(
		...
		string methodName,
		int paramsCount) {
		var str = funcComponent.Name + methodName + paramsCount;
		...
		var componentMethodInfo = InfoAttribute.GetComponentMethodInfo(funcComponent.Name, methodName);
		var methodExecuteContext2 = new MethodExecuteContext();
