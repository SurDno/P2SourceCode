...
public class GameTimeContext : ISerializeStateSave, IDynamicLoadSerializable {
	...
	private List<GameTimer> gameTimerList = new();
	...
	public GameTimeContext(IGameMode gameMode) {
		if (gameMode == null)
			Logger.AddError(" Invalid game time context creation: game mode not defined! : " +
			                DynamicFSM.CurrentStateInfo);
		else {
			if (gameMode.StartGameTime == null)
				Logger.AddError(" Invalid game time context creation: start time not defined! : " +
				                DynamicFSM.CurrentStateInfo);
	...
	public void Update(double fDtime, bool bForceEvents = true) {
		...
		foreach (var timerPriority in timerPriorityList) {
			var index = timerPriority.Value;
	...
	public GameTimer StartTimer(
		...
		if (freeTimersIndexesStack.Count > 0) {
			var index = freeTimersIndexesStack.Pop();
			if (index < 0 || index >= gameTimerList.Count)
				Logger.AddError(string.Format("Invalid free timer index {0}, timers list count = {1} at {2}", index,
					gameTimerList.Count, DynamicFSM.CurrentStateInfo));
			else {
		...
		var count = gameTimerList.Count;
		var gameTimer = new GameTimer(count);
		gameTimer.Start(timerType, initiatorFSMGuid, stateId, targetTime, bIsRepeat);
		gameTimerList.Add(gameTimer);
		OnNewTimer(count);
		var dynamicFsmObjectsCount = VirtualMachine.Instance.DynamicFSMObjectsCount;
		if (VirtualMachine.Instance.IsLoaded && gameTimerList.Count > dynamicFsmObjectsCount * 4)
			Logger.AddError("Warning: on start game timer too many timers created!!! at " +
			                DynamicFSM.CurrentStateInfo);
		return gameTimer;
	}

	public void StopTimer(ulong timerID) {
		var timerIndexById = GetTimerIndexByID(timerID);
		if (timerIndexById < 0 || timerIndexById >= gameTimerList.Count)
			Logger.AddWarning(string.Format("Invalid stopping game timer index {0} at {1}", timerIndexById,
				DynamicFSM.CurrentStateInfo));
		else {
	...
	private int GetTimerIndexByID(ulong timerID) {
		...
		return loadingTimersIdIndexesDict.ContainsKey(timerID)
			? loadingTimersIdIndexesDict[timerID]
			: (int)(timerID % uint.MaxValue);
	}

	public void RevertEventsCutTime(GameTime newTime) {
		for (var index = 0; index < gameTimerList.Count; ++index)
	...
	private void SaveTimers(IDataWriter writer) {
		var gameTimerList = new List<GameTimer>();
		for (var index = 0; index < this.gameTimerList.Count; ++index)
			if (this.gameTimerList[index].GTType != EGameTimerType.GAME_TIMER_TYPE_ABSOLUTE &&
			    this.gameTimerList[index].Active)
		...
		foreach (XmlElement childNode in xmlNode.ChildNodes) {
			...
			if (gameTimer.GTType != EGameTimerType.GAME_TIMER_TYPE_ABSOLUTE && gameTimer.Active) {
				...
				if (loadingTimersIdIndexesDict.ContainsKey(gameTimer.TimerGuid))
					Logger.AddError(string.Format("SaveLoad error: loading timer id {0} dublication",
						gameTimer.TimerGuid));
				else
	...
	private void OnNewTimer(int timerIndex) {
		if (TIMERS_LIST_PROCESSIN_ON)
			Logger.AddError("Invalid timer creation: timers are being processed now at " + gameMode.Name +
			                " game mode! Probably incorrect non-queue event rising");
		else
