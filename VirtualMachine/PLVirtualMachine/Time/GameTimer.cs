...
public class GameTimer : ISerializeStateSave, IDynamicLoadSerializable {
	...
	public void Start(
		...
		if (timerType == EGameTimerType.GAME_TIMER_TYPE_ABSOLUTE)
			return;
		Logger.AddWarning(string.Format("Test timer started, timer id={0}, hours={1}, minutes={2}, seconds={3} at {4}",
			TimerGuid, targetTime.Hours, targetTime.Minutes, targetTime.Seconds, DynamicFSM.CurrentStateInfo));
	}
	...
	public bool IsElapsed(GameTime currTime) {
		return currTime.TotalValue > targetTime.TotalValue;
	}
	...
	private void OnGameTime(double timeElapsed, bool needForceEvents = true) {
		...
		if (((!IsRepeat ? 0 : gtType != 0 ? 1 : 0) & (needForceEvents ? 1 : 0)) != 0) {
			var totalValue = targetTime.TotalValue;
			var d = remainder + timeElapsed / totalValue;
			iCount = (int)Math.Floor(d);
			remainder = d - iCount;
		}

		if (needForceEvents) {
			if (gtType == EGameTimerType.GAME_TIMER_TYPE_ABSOLUTE &&
			    targetTime.TotalSeconds >= eventsCutTime.TotalSeconds) {
				var onEvent = OnEvent;
				if (onEvent != null)
					onEvent(this);
	...
	public ulong CalculateTimerPriority() {
		return GTType == EGameTimerType.GAME_TIMER_TYPE_ABSOLUTE
			? TargetTime.TotalSeconds * 1000000UL
			: (TargetTime.TotalSeconds - currentTime.TotalSeconds) * 1000000UL;
	}
}