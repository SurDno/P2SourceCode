...
public class VMDebugger : DebugServer {
	...
	private List<Guid> objectsSendedToClient = new();
			...
			case EDebugIPCApplicationWorkMode.IPC_APPLICATION_WORK_MODE_DEBUG:
				lock (fsmDebugObjectsLocker) {
					foreach (var fsmObject in fsmObjects) {
						var debugFsmInfo = fsmObject.Value;
				...
				using (var enumerator = fsmObjects.GetEnumerator()) {
					while (enumerator.MoveNext()) {
						var debugFsmInfo = enumerator.Current.Value;
						if (!objectsSendedToClient.Contains(debugFsmInfo.FSM.Entity.EngineGuid))
							AddMessageToClient(
								new SendIpcMessage(EDebugIPCMessageType.IPC_MESSAGE_SERVER_EVENT_ADD_DEBUG_OBJECT) {
	...
	public void OnAddObject(DynamicFSM fsm) {
		var stateGraph = fsm.FSMStaticObject.StateGraph;
		if (stateGraph == null)
			return;
		try {
			var debugFsmInfo = new DebugFSMInfo(fsm);
			lock (fsmDebugObjectsLocker) {
				if (!fsmObjects.ContainsKey(fsm.Entity.EngineGuid))
					fsmObjects.Add(fsm.Entity.EngineGuid, debugFsmInfo);
				else
					OnError(string.Format("Dynamic object {0} guid {1} dublication at debugger registration",
						fsm.Entity.Name, fsm.Entity.EngineGuid));
			}

			var messageToClient = new SendIpcMessage(EDebugIPCMessageType.IPC_MESSAGE_SERVER_EVENT_ADD_DEBUG_OBJECT);
	...
	public bool OnStateInput(DynamicFSM fsm, IState state) {
		var engineGuid = fsm.Entity.EngineGuid;
		var flag1 = false;
		lock (fsmDebugObjectsLocker) {
			if (fsmObjects.ContainsKey(engineGuid)) {
				var fsmObject = fsmObjects[engineGuid];
				if (fsmObject.CurrentStatus == EDebugFSMStatus.DEBUG_FSM_STATUS_PLAY) {
					var parent = (IFiniteStateMachine)((VMBaseObject)state).Parent;
					var num1 = IsAllDebugging(parent.BaseGuid) ? 1 : 0;
					var flag2 = IsObjectUnderDebug(parent.BaseGuid, engineGuid);
					var num2 = IsObjectUnderDebug(parent.BaseGuid, engineGuid, true) ? 1 : 0;
					...
					OnError(string.Format("Invalid state input: fsm {0} currently stepbystep blocked",
						fsm.Entity.Name));
				else {
	...
	public void OnParamValueChange(VMParameter param, object newValue, Guid objGuid) {
		if (newValue == null)
			return;
		var paramVal = newValue;
		var type = paramVal.GetType();
		...
		if (!currentDebuggingObjects.Contains(objGuid))
			return;
		var messageToClient = new SendIpcMessage(EDebugIPCMessageType.IPC_MESSAGE_SERVER_EVENT_PARAM_VALUE_CHANGE);
		messageToClient.AddParamChange(param.BaseGuid, paramVal);
		messageToClient.DebugObjectEngGuid = objGuid;
		AddMessageToClient(messageToClient);
	}

	public bool OnStateExec(DynamicFSM fsm, IState state) {
		return false;
	}
	...
	private void ProcessDebug() {
		lock (fsmDebugObjectsLocker) {
			foreach (var fsmObject in fsmObjects) {
				var debugFsmInfo = fsmObject.Value;
				if (debugFsmInfo.CurrentStatus != EDebugFSMStatus.DEBUG_FSM_STATUS_PLAY &&
				    debugFsmInfo.CurrentStatus != EDebugFSMStatus.DEBUG_FSM_STATUS_STEPBYSTEP_STEP_INTO &&
				    debugFsmInfo.CurrentStatus != EDebugFSMStatus.DEBUG_FSM_STATUS_STEPBYSTEP_STEP_OVER) {
					var currentStatus = (int)debugFsmInfo.CurrentStatus;
				}
			}
		}
		...
		lock (raisingEventsLocker) {
			while (debugRaisingEventQueue.Count != 0) {
				...
				var objectEntityByUniName =
					WorldEntityUtility.GetDynamicObjectEntityByUniName(raisingEventInfo.EventOwnerUniName);
				if (objectEntityByUniName == null) {
					OnError(string.Format("Cannot raise debug event: object with dynamic id={0} not found",
						raisingEventInfo.EventOwnerUniName));
					break;
				}

				var eventByStaticGuid = objectEntityByUniName.GetFSM().GetEventByStaticGuid(raisingEventInfo.EventGuid);
				if (eventByStaticGuid == null) {
					OnError(string.Format("Cannot raise debug event on object {0}: event with id={1} not found",
						objectEntityByUniName.EditorTemplate.Name, raisingEventInfo.EventGuid));
					break;
				}
	...
	private void ProcessErrors() {
		var lastErrors = LastErrors;
		ResetErrors();
		for (var index = 0; index < lastErrors.Count; ++index)
	...
	private void SetDebugObject(ulong graphId, Guid setObGuid) {
		...
		currentDebuggingObjects.Clear();
		foreach (var keyValuePair in debuggingObjectsByStaticFSM)
		...
		lock (fsmDebugObjectsLocker) {
			if (!fsmObjects.ContainsKey(setObGuid))
				...
				if (fsmObjects[setObGuid].CurrentPosition.Parent == null)
					OnError(string.Format("Current position state {0} not created in debugger",
						fsmObjects[setObGuid].CurrentPosition.Name));
				else {
					var parent = (IFiniteStateMachine)fsmObjects[setObGuid].CurrentPosition.Parent;
					var fsmDynamicParams = fsmObjects[setObGuid].FSM.FSMDynamicParams;
					AddInputStateMessageToClient(setObGuid, fsmObjects[setObGuid].CurrentPosition.BaseGuid,
						fsmDynamicParams);
				}
			}
		}
	...
	private void SetBreakpoint(Guid objGuid, ulong stateId) {
		lock (fsmDebugObjectsLocker) {
			if (fsmObjects.ContainsKey(objGuid)) {
				var fsmObject = fsmObjects[objGuid];
				var stateByGuid =
					((FiniteStateMachine)fsmObject.FSM.FSMStaticObject.StateGraph).GetStateByGuid(stateId, false);
				if (stateByGuid == null)
		...
		lock (fsmDebugObjectsLocker) {
			if (fsmObjects.ContainsKey(objGuid)) {
				...
				if (!fsmObject.Breakpoints.ContainsKey(stateId))
					OnError(
						string.Format("Breakpoint state id={0} not found in object {1} debug fsm", stateId, objGuid));
				else
	...
	private void AddRaisingEvent(string sObjUniName, ulong stateGuid) {
		var raisingEventInfo = new DebugRaisingEventInfo(sObjUniName, stateGuid);
		lock (raisingEventsLocker) {
	...
	private void AddInputStateMessageToClient(
		...
		var messageToClient = new SendIpcMessage(EDebugIPCMessageType.IPC_MESSAGE_SERVER_EVENT_INPUT_STATE);
		messageToClient.DebugObjectEngGuid = objGuid;
		messageToClient.StateGuid = stateGuid;
		if (currentParams != null)
			foreach (var currentParam in currentParams)
				if ((!currentParam.Implicit || !currentParam.Name.Contains("_Self")) &&
				    (currentParam.Entity == null || !currentParam.Entity.IsDisposed)) {
					var name = currentParam.Value;
					if (name != null) {
	...
	private bool IsObjectUnderDebug(ulong staticFSMGuid, Guid objGuid, bool allGraphes = false) {
		if (debuggingObjectsByStaticFSM.ContainsKey(staticFSMGuid) &&
		    debuggingObjectsByStaticFSM[staticFSMGuid] == objGuid)
			return true;
		return allGraphes && debuggingObjectsByStaticFSM.ContainsValue(objGuid);
	}

	private bool IsAllDebugging(ulong staticFSMGuid) {
		return !debuggingObjectsByStaticFSM.ContainsKey(staticFSMGuid) ||
		       debuggingObjectsByStaticFSM[staticFSMGuid] == Guid.Empty;
	}
}