...
public class VMBranch :
	...
	private List<ICondition> branchesConditions = new();

	[FieldData("BranchType")] private EStateType branchType;
	[FieldData("BranchVariantInfo")] private List<NameTypeData> branchVariantInfos;
			...
			if (xml.NodeType == XmlNodeType.Element)
				switch (xml.Name) {
					...
					case "BranchVariantInfo":
						branchVariantInfos =
							EditorDataReadUtility.ReadEditorDataSerializableList(xml, creator, branchVariantInfos);
						continue;
					case "EntryPoints":
	...
	public override int GetExitPointsCount() {
		return branchesConditions.Count + 1;
	}
	...
	public IVariable MakeCastedVariable(IVariable prevVariable, int varIndex) {
		...
		var variable = prevVariable;
		if (branchType == EStateType.STATE_TYPE_MESSAGE_CAST_BRANCH && varIndex < messageCastInfo.Count()) {
			var branchVariantCastInfo = GetBranchVariantCastInfo(varIndex);
			if (branchVariantCastInfo.Message != null && branchVariantCastInfo.CastType != null &&
			    branchVariantCastInfo.Message.Name == prevVariable.Name) {
				var name = prevVariable.Name;
				var type = prevVariable.GetType();
	...
	private void LoadMessageCastInfo() {
		...
		if (branchVariantInfos == null)
			return;
		foreach (var branchVariantInfo in branchVariantInfos) {
			...
			for (var index = 0; index < contextVariables.Count; ++index)
				if (contextVariables[index] is ContextVariable contextVariable &&
				    contextVariable.Name == branchVariantInfo.Name) {
