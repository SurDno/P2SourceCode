...
public class VMState :
	...
	protected List<IEntryPoint> entryPoints = new();

	[FieldData("Owner", DataFieldType.Reference)]
	protected IContainer owner;

	[FieldData("InputLinks", DataFieldType.Reference)]
	protected List<VMEventLink> inputLinks = new();

	[FieldData("OutputLinks", DataFieldType.Reference)]
	protected List<ILink> outputLinks = new();

	[FieldData("Initial")] protected bool initial;
	[FieldData("IgnoreBlock")] protected bool ignoreBlock;
	...
	public virtual int GetExitPointsCount() {
		return 0;
	}
	...
	public List<IVariable> GetLocalContextVariables(
		...
		int counter = 0) {
		var contextVariables1 = new List<IVariable>();
		++counter;
		var num = MAX_CIRCLE_STATE_ITERATIONS_COUNT;
		...
		if (contextVarCategory == EContextVariableCategory.CONTEXT_VARIABLE_CATEGORY_MESSAGE ||
		    contextVarCategory == EContextVariableCategory.CONTEXT_VARIABLE_CATEGORY_ALL) {
			var flag1 = Initial;
			if (inputLinks.Count == 1 && inputLinks[0].SourceState == null)
				flag1 = true;
			if (flag1 && ((IFiniteStateMachine)Parent).IsSubGraph) {
				var contextVariables2 =
					((VMState)Parent).GetLocalContextVariables(
						EContextVariableCategory.CONTEXT_VARIABLE_CATEGORY_MESSAGE, null);
			...
			for (var index1 = 0; index1 < inputLinks.Count; ++index1) {
				if (inputLinks[index1].SourceState != null) {
					...
					if (typeof(VMTalkingGraph) == Parent.GetType())
						flag2 = false;
					var flag3 = typeof(IBranch).IsAssignableFrom(inputLinks[index1].SourceState.GetType());
					if (flag3 | flag2) {
						var contextVariables3 =
							((VMState)inputLinks[index1].SourceState).GetLocalContextVariables(contextVarCategory,
								currentElement, counter);
						if (flag3) {
							var sourceState = (VMBranch)inputLinks[index1].SourceState;
							if (sourceState.StateType == EStateType.STATE_TYPE_MESSAGE_CAST_BRANCH)
								for (var index2 = 0; index2 < contextVariables3.Count; ++index2)
									contextVariables3[index2] =
										sourceState.MakeCastedVariable(contextVariables3[index2],
											inputLinks[index1].SourceExitPoint);
						}
				...
				var destEntryPoint = inputLinks[index1].DestEntryPoint;
				if (destEntryPoint < 0 || destEntryPoint >= entryPoints.Count)
					Logger.AddError("Wrong entry point index");
				else {
					if (currentElement != null && localContextDependencyDict != null &&
					    localContextDependencyDict.ContainsKey(currentElement.BaseGuid)) {
						var actionLine = (IActionLine)localContextDependencyDict[currentElement.BaseGuid][0];
						if (actionLine != null && (entryPoints[destEntryPoint] == null ||
						                           entryPoints[destEntryPoint].ActionLine == null ||
						                           (long)entryPoints[destEntryPoint].ActionLine.BaseGuid !=
						                           (long)actionLine.BaseGuid))
							continue;
					}

					if (inputLinks[index1].Event.EventInstance != null) {
						var returnMessages = inputLinks[index1].Event.EventInstance.ReturnMessages;
						for (var index3 = 0; index3 < returnMessages.Count; ++index3)
							contextVariables1.Add(returnMessages[index3]);
					}
				}
			}

			if (Parent != null && Parent.GetCategory() == EObjectCategory.OBJECT_CATEGORY_GRAPH &&
			    ((VMState)Parent).IsProcedure) {
				var contextVariables4 =
					((VMState)Parent).GetLocalContextVariables(
						EContextVariableCategory.CONTEXT_VARIABLE_CATEGORY_MESSAGE, null, counter);
				contextVariables1.AddRange(contextVariables4);
			}
		}

		if ((contextVarCategory == EContextVariableCategory.CONTEXT_VARIABLE_CATEGORY_LOCAL_VAR ||
		     contextVarCategory == EContextVariableCategory.CONTEXT_VARIABLE_CATEGORY_ALL) && currentElement != null &&
		    !typeof(IBranch).IsAssignableFrom(GetType()) && localContextDependencyDict != null &&
		    localContextDependencyDict.ContainsKey(currentElement.BaseGuid)) {
			var contextElementList = localContextDependencyDict[currentElement.BaseGuid];
			for (var index = 0; index < contextElementList.Count; ++index)
	...
	public IVariable GetLocalContextVariable(string variableUniName, IContextElement currentElement = null) {
		if (!localVarsCacheLoaded) {
			var contextVariables1 =
				GetLocalContextVariables(EContextVariableCategory.CONTEXT_VARIABLE_CATEGORY_LOCAL_VAR, null);
			localVarsCache.Clear();
			for (var index = 0; index < contextVariables1.Count; ++index)
				localVarsCache.Add(contextVariables1[index]);
			var contextVariables2 =
				GetLocalContextVariables(EContextVariableCategory.CONTEXT_VARIABLE_CATEGORY_MESSAGE, null);
			for (var index = 0; index < contextVariables2.Count; ++index)
				localMessagesCache.Add(contextVariables2[index]);
			localVarsCacheLoaded = true;
		}

		IVariable result;
		if (localVarsCache.TryGetValue(variableUniName, out result) ||
		    localMessagesCache.TryGetValue(variableUniName, out result))
			return result;
		if (currentElement != null && !typeof(IBranch).IsAssignableFrom(GetType()) &&
		    localContextDependencyDict != null && localContextDependencyDict.ContainsKey(currentElement.BaseGuid)) {
			var contextElementList = localContextDependencyDict[currentElement.BaseGuid];
			for (var index = 0; index < contextElementList.Count; ++index) {
				var localContextVariable = contextElementList[index].GetLocalContextVariable(variableUniName);
	...
	public override void Clear() {
		...
		if (localContextDependencyDict != null) {
			foreach (var keyValuePair in localContextDependencyDict)
		...
		if (outputLinksByEventGuid != null) {
			foreach (var keyValuePair in outputLinksByEventGuid)
				keyValuePair.Value.Clear();
		...
		if (outputLinksByEventName == null)
			return;
		foreach (var keyValuePair in outputLinksByEventName)
			keyValuePair.Value.Clear();
	...
	protected void MakeLocalContextElementsDependencys(
		...
		foreach (var action in ((IActionLine)contextElement).Actions) {
	...
	protected void UpdateLinks() {
		for (var index = 0; index < inputLinks.Count; ++index)
	...
	protected IContainer CalculateOwner() {
		if (Parent == null)
			return null;
		return Parent.GetCategory() == EObjectCategory.OBJECT_CATEGORY_GRAPH ||
		       Parent.GetCategory() == EObjectCategory.OBJECT_CATEGORY_GRAPH_ELEMENT
			? ((VMState)Parent).CalculateOwner()
			: Parent;
	}
		...
		if (owner == null)
			...
			try {
				...
				for (var index = 0; index < entryPoints.Count; ++index) {
					var actionLine = entryPoints[index].ActionLine;
					if (actionLine != null)
