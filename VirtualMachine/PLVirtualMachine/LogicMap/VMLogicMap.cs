...
public class VMLogicMap :
	...
	private List<IGraphObject> mapNodes = new();

	[FieldData("Links", DataFieldType.Reference)]
	private List<ILink> mapLinks = new();

	[FieldData("LogicMapType")] private ELogicMapType logicMapType;
	...
	public List<ILink> GetLinksByDestState(IGraphObject state) {
		...
		for (var index = 0; index < mapLinks.Count; ++index)
			if (mapLinks[index].Destination != null &&
			    (long)mapLinks[index].Destination.BaseGuid == (long)state.BaseGuid)
	...
	public virtual void OnAfterLoad() {
		for (var index = 0; index < mapNodes.Count; ++index)
	...
	public override EObjectCategory GetCategory() {
		return EObjectCategory.OBJECT_CATEGORY_LOGIC_MAP;
	}
