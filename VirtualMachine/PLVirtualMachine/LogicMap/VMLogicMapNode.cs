...
public class VMLogicMapNode :
	...
	private List<ILink> inputLinks = new();

	[FieldData("OutputLinks", DataFieldType.Reference)]
	private List<ILink> outputLinks = new();

	[FieldData("LogicMapNodeType")] private ELogicMapNodeType logicMapNodeType = ELogicMapNodeType.LM_NODE_TYPE_COMMON;

	[FieldData("NodeContent", DataFieldType.Reference)]
	private List<VMLogicMapNodeContent> nodeContentList = new();

	[FieldData("GameScreenPosX")] private float gameScreenPosX;
	[FieldData("GameScreenPosY")] private float gameScreenPosY;
	...
	public VMLogicMapNodeContent GetContentByGuid(ulong contentId) {
		foreach (var nodeContent in nodeContentList)
			if ((long)nodeContent.BaseGuid == (long)contentId)
				return nodeContent;
		Logger.AddError(string.Format("Logic map node content with id {0} not found in mindmap node {1}", contentId,
			Name));
		return null;
	}
