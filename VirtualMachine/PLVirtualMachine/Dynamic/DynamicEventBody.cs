...
public class DynamicEventBody : IDependedEventRef, ISerializeStateSave, IDynamicLoadSerializable {
	...
	private List<GameTimer> checkRaisingTimePointEvents = new();
	private List<VMParameter> checkRaisingParams = new();
	...
	private List<ulong> saveLoadingCheckRaisingObjectsID = new();
	...
	public void Think() {
		if (eventCondition != null) {
			var conditionResult = CalculateConditionResult();
			var flag = false;
	...
	public bool CalculateConditionResult() {
		var dynamicObjContext = VirtualMachine.Instance.GameRootFsm;
		if (parentEntity != null)
			dynamicObjContext = parentEntity.GetFSM();
		return eventCondition != null &&
		       ExpressionUtility.CalculateConditionResult(eventCondition, dynamicObjContext, parent);
	}

	public void ClearSubscribtions() {
		for (var index = 0; index < checkRaisingTimePointEvents.Count; ++index)
			checkRaisingTimePointEvents[index].OnEvent -= OnOutTimer;
		checkRaisingTimePointEvents.Clear();
		for (var index = 0; index < checkRaisingParams.Count; ++index)
	...
	public void StateSave(IDataWriter writer) {
		...
		if (RaisingType != EEventRaisingType.EVENT_RAISING_TYPE_CONDITION &&
		    RaisingType != EEventRaisingType.EVENT_RAISING_TYPE_TIME)
			return;
		SaveManagerUtility.SaveList(writer, "DependedObjectsInfo",
			checkRaisingTimePointEvents.Select(o => o.TimerGuid));
	}
			...
			if (childNode1.Name == "CurrentConditionResult")
				...
				try {
					...
					Logger.AddError(string.Format(
						"SaveLoad error: cannot load raising timers guids list from node {0}  in event {1}, error: {2}",
						childNode1.InnerText, parent.Name, ex));
				}
		}
	}
	...
	public void AfterSaveLoading() {
		try {
			var gameTimeContext = GameTimeManager.CurrentGameTimeContext;
			if (this.gameTimeContext != null && "" != this.gameTimeContext.Name) {
				if (GameTimeManager.GameTimeContexts.ContainsKey(this.gameTimeContext.Name))
					...
					Logger.AddError(string.Format(
						"SaveLoad error at {0} event afterloading: event game time context with name {1} not found",
						parent.Name, this.gameTimeContext.Name));
					return;
				}
			}

			var count = checkRaisingTimePointEvents.Count;
			if (count > 0)
				for (var index = count - 1; index >= 0; --index) {
					var raisingTimePointEvent = checkRaisingTimePointEvents[index];
		...
		try {
			...
			if (RaisingType == EEventRaisingType.EVENT_RAISING_TYPE_CONDITION) {
				if (eventCondition == null)
					...
					foreach (var raisingTimePoint in eventCondition.GetCheckRaisingTimePoints()) {
						var gameTimer = GameTimeManager.StartTimer(EGameTimerType.GAME_TIMER_TYPE_ABSOLUTE,
							parentEntity.EngineGuid, 0UL, raisingTimePoint, repeated, gameTimeContext.Name);
					...
					if (eventCondition.GetCheckRaisingFunctions().Count > checkRaisingTimePointEvents.Count)
						...
						if (checkRaisingParams.Count <= 0)
							return;
						eventUpdateMode = eventUpdateMode != EEventUpdateMode.EVENT_UPDATE_MODE_OUTER_EVENT
							? EEventUpdateMode.EVENT_UPDATE_MODE_PARAM_CHANGE
							: EEventUpdateMode.EVENT_UPDATE_MODE_COMBINE;
						for (var index = 0; index < checkRaisingParams.Count; ++index)
				...
				if (RaisingType != EEventRaisingType.EVENT_RAISING_TYPE_TIME)
					return;
				var gameTimer = GameTimeManager.StartTimer(EGameTimerType.GAME_TIMER_TYPE_ABSOLUTE,
					parentEntity.EngineGuid, 0UL, eventTime, repeated, gameTimeContext.Name);
				gameTimer.OnEvent += OnOutTimer;
				checkRaisingTimePointEvents.Add(gameTimer);
				eventUpdateMode = EEventUpdateMode.EVENT_UPDATE_MODE_OUTER_EVENT;
			}
