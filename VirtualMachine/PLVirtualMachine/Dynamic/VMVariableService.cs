...
public class VMVariableService : IVariableService {
	public bool IsContextBinded(CommonVariable variable, IDynamicGameObjectContext activeContext) {
		if (variable.Context == null)
			return false;
		return !typeof(ILogicObject).IsAssignableFrom(variable.Context.GetType()) ||
		       ((ILogicObject)variable.Context).Static || activeContext.IsStaticDerived((IBlueprint)variable.Context);
	}
	...
	public override IContext GetContextByData(
		...
		switch (GuidUtility.GetGuidFormat(data)) {
			case EGuidFormat.GT_BASE:
				var id = DefaultConverter.ParseUlong(data);
				if (id > 0UL) {
					ulong num = 0;
					...
					if ((long)num == (long)id)
						return (IContext)IStaticDataContainer.StaticDataContainer.GameRoot.GetWorldHierarhyObjectByGuid(
							new HierarchyGuid(data)) ?? null;
			...
			case EGuidFormat.GT_ENGINE:
				if (globalContext != null && typeof(IWorldObject).IsAssignableFrom(globalContext.GetType())) {
					var guid = DefaultConverter.ParseGuid(data);
			...
			default:
				try {
					if (localContext != null)
						if (localContext.Owner != null)
							if (typeof(IBlueprint).IsAssignableFrom(localContext.Owner.GetType())) {
								var contextByData = ((IContext)localContext.Owner).GetContextVariable(data);
					...
					Logger.AddError(string.Format("Context getting error from data {0} at {1}, error: {2} ", data,
						globalContext.Name, ex));
				}
	...
	public IParam GetDynamicParam(
		...
		if (paramVariable.CommonVariableType == ECommonVariableType.CV_TYPE_CONTEXT_VARIABLE) {
			var dynamicContext = GetDynamicContext(contextVariable.VariableContext, activeContext);
			if (dynamicContext != null)
				return GetDynamicContextParamByVariable(dynamicContext, (IVariable)paramVariable.Variable);
		}

		Logger.AddError(string.Format("Dynamic param by variable {0} in context {1} not found at {2}", paramVariable,
			contextVariable, DynamicFSM.CurrentStateInfo));
		return null;
	}
	...
	public IParam GetDynamicParam(
		...
		if (variable.CommonVariableType == ECommonVariableType.CV_TYPE_CONTEXT_VARIABLE) {
			var ownerContext = activeContext;
			if (ownerFSM != null)
				ownerContext = ownerFSM;
			var dynamicContext = GetDynamicContext(variable.Context, ownerContext);
			if (dynamicContext != null) {
				...
				Logger.AddError(string.Format("Cannot get dynamic param by variable, variable {0} not found at {1}",
					variable, DynamicFSM.CurrentStateInfo));
				return null;
			}
		}

		Logger.AddError(string.Format("Cannot get dynamic param by variable, variable {0} isn't param at {1}", variable,
	...
	public object GetDynamicVariableValue(
		...
		if (variable.CommonVariableType == ECommonVariableType.CV_TYPE_CONTEXT_VARIABLE) {
			if (variable.Category == EContextVariableCategory.CONTEXT_VARIABLE_CATEGORY_PARAM) {
				if (variable.IsSelf)
					return GetSelfObjctRef(variable, activeContext);
				var dynamicParam = GetDynamicParam(variable, activeContext, ownerFSM);
				if (dynamicParam != null)
					return dynamicParam.Value;
				var str = "none";
				if (activeContext != null)
					str = activeContext.Entity.EditorTemplate.Name;
				Logger.AddError(string.Format("Cannot get dynamic param by variable {0} in context {1} at {2}",
					variable, str, DynamicFSM.CurrentStateInfo));
				return null;
			}
			...
			if (variable.Context != null && variable.Variable != null) {
				...
				if (typeof(IVariable).IsAssignableFrom(variable.Variable.GetType())) {
					var variable1 = (IVariable)variable.Variable;
					if (variable.Category == EContextVariableCategory.CONTEXT_VARIABLE_CATEGORY_MESSAGE) {
						var contextMessage = ownerContext.GetContextMessage(variable1.Name);
						return contextMessage != null
							? contextMessage.Value
							: ownerContext.GetLocalVariableValue(variable1.Name);
					}
					...
					if (variable.Category == EContextVariableCategory.CONTEXT_VARIABLE_CATEGORY_PARAM) {
						var contextParamByVariable = GetDynamicContextParamByVariable(dynamicContext, variable1);
						...
						if (variable.Category == EContextVariableCategory.CONTEXT_VARIABLE_CATEGORY_GLOBAL_VAR) {
							var globalVarName = variable1.Name;
						...
						if (variable.Category == EContextVariableCategory.CONTEXT_VARIABLE_CATEGORY_OBJECT &&
						    variable.IsSelf) {
							var dynamicVariableValue = new VMObjRef();
							if (dynamicContext != null) {
			...
			Logger.AddError(string.Format("Cannot get dynamic variable value, variable {0} isn't binded at {1}",
				variable, DynamicFSM.CurrentStateInfo));
			return null;
		}

		Logger.AddError(string.Format("Cannot get dynamic variable value, variable {0} isn't binded at {1}", variable,
			DynamicFSM.CurrentStateInfo));
		return null;
	}
	...
	public IDynamicGameObjectContext GetDynamicContext(
		...
		if (staticContext == null) {
			Logger.AddError(string.Format("Cannot bind dynamic context: static context not defined at {0}",
				DynamicFSM.CurrentStateInfo));
			return null;
		}
		...
		Logger.AddError(string.Format("Cannot get dynamic context by variable {0} at {1}", staticContext.Name,
			DynamicFSM.CurrentStateInfo));
		return null;
	}
	...
	private IDynamicGameObjectContext GetDynamicContextByBlueprintContext(
		...
		if (ownerContext != null && ownerContext.IsStaticDerived(blueprintContext.Blueprint))
			return ownerContext;
		Logger.AddError(string.Format("Cannot get dynamic context by unbinded template context {0} at {1}",
			blueprintContext.Name, DynamicFSM.CurrentStateInfo));
		return null;
	}

	private IDynamicGameObjectContext GetDynamicContextByStaticObject(ILogicObject staticObject) {
		if (typeof(IWorldHierarchyObject).IsAssignableFrom(staticObject.GetType()) &&
		    (((IHierarchyObject)staticObject).HierarchyGuid.IsHierarchy || ((IWorldObject)staticObject).IsEngineRoot)) {
			var entityByHierarchyGuid =
				WorldEntityUtility.GetDynamicObjectEntityByHierarchyGuid(((IHierarchyObject)staticObject)
					.HierarchyGuid);
			if (entityByHierarchyGuid != null)
				return entityByHierarchyGuid.GetFSM();
			Logger.AddError(string.Format("Dynamic entity by static object {0} with id={1} not found at {2}",
				staticObject.Name, ((IHierarchyObject)staticObject).HierarchyGuid, DynamicFSM.CurrentStateInfo));
			return null;
		}

		var entityByStaticGuid = WorldEntityUtility.GetDynamicObjectEntityByStaticGuid(staticObject.Blueprint.BaseGuid);
		if (entityByStaticGuid != null)
			return entityByStaticGuid.GetFSM();
		Logger.AddError(string.Format("Dynamic entity by static object {0} with id={1} not found at {2}",
			staticObject.Name, staticObject.Blueprint.BaseGuid, DynamicFSM.CurrentStateInfo));
		return null;
	}
	...
	private IDynamicGameObjectContext GetDynamicContextByVariable(
		...
		IVariable variable) {
		if (typeof(IObjRef).IsAssignableFrom(variable.GetType()) ||
		    typeof(IBlueprintRef).IsAssignableFrom(variable.GetType())) {
			var staticInstance = (IBlueprint)((IRef)variable).StaticInstance;
			if (staticInstance != null)
				return GetDynamicContextByBlueprintContext(staticInstance, dynContext);
		}

		if (variable.Category == EContextVariableCategory.CONTEXT_VARIABLE_CATEGORY_PARAM) {
			var contextParamByVariable = GetDynamicContextParamByVariable(dynContext, variable);
			if (contextParamByVariable != null && contextParamByVariable.Value != null &&
			    typeof(IObjRef).IsAssignableFrom(contextParamByVariable.Value.GetType()))
				return GetDynamicContextByObjRef((IObjRef)contextParamByVariable.Value);
			if (typeof(VMParameter) == variable.GetType()) {
				var ownerContext = ((VMParameter)variable).OwnerContext;
				if (ownerContext != null && typeof(IBlueprint).IsAssignableFrom(ownerContext.GetType()) &&
				    ownerContext.Static)
					return GetDynamicContextByVariable(GetDynamicContextByBlueprintContext(ownerContext, dynContext),
						variable);
			}
		} else if (variable.Category == EContextVariableCategory.CONTEXT_VARIABLE_CATEGORY_MESSAGE) {
			var contextMessage = dynContext.GetContextMessage(variable.Name);
			if (contextMessage != null) {
				...
				var localVariableValue = dynContext.GetLocalVariableValue(variable.Name);
			...
			if (localVariableValue != null) {
				...
				Logger.AddError(string.Format("Cannot get value by local variable {0} in {1} at {1}", variable.Name,
					dynContext.Entity.EditorTemplate.Name, DynamicFSM.CurrentStateInfo));
		}

		Logger.AddError(string.Format("Cannot get dynamic context by variable {0} at {1}", variable.Name,
			DynamicFSM.CurrentStateInfo));
		return null;
	}
	...
	private IDynamicGameObjectContext GetDynamicContextByObjRef(IObjRef objRef) {
		...
		return vmObjRef.EngineInstance != null && typeof(VMEntity) == vmObjRef.EngineInstance.GetType()
			? ((VMEntity)vmObjRef.EngineInstance).GetFSM()
			: (IDynamicGameObjectContext)null;
	}
	...
	private VMObjRef GetSelfObjctRef(
		...
		if (variable.Context != null) {
			...
			if (typeof(ILogicObject).IsAssignableFrom(context.GetType())) {
				var blueprintContext = GetDynamicContextByBlueprintContext((ILogicObject)context, activeContext);
				var selfObjctRef = new VMObjRef();
				if (blueprintContext != null)
					selfObjctRef.InitializeInstance(blueprintContext.Entity);
				else
					Logger.AddError(string.Format("Self dynamic context by static object {0} not found at {1}",
						context.Name, DynamicFSM.CurrentStateInfo));
				return selfObjctRef;
			}
		}
