...
public class FSMGraphManager {
	...
	public virtual void StateSave(IDataWriter writer) {
		var flag = IsSaveStateValid();
		if (!flag)
			Logger.AddError(string.Format("SaveLoad error: saving entity {0} is in invalid state {1}", fsm.Entity.Name,
				CurrentState.Name));
		...
		SaveManagerUtility.Save(writer, "LockingFSM",
			lockingOwnerFSM != null ? lockingOwnerFSM.EngineGuid : Guid.Empty);
	...
	public void OnSaveLoad() {
		if (CurrentState != null)
			try {
				if (subgraphLocalVariableValuesDict != null) {
					var list = subgraphLocalVariableValuesDict.Keys.ToList();
					for (var index = 0; index < list.Count; ++index) {
						var str = list[index];
						var localContextVariable = CurrentState.GetLocalContextVariable(str);
						if (localContextVariable != null) {
							var obj = StringSerializer.ReadValue((string)subgraphLocalVariableValuesDict[str],
								localContextVariable.Type.BaseType);
				...
				Logger.AddError(string.Format(
					"SaveLoad error: cannot load local variables dict for fsm in entity {0}, error: {1}",
					fsm.Entity.Name, ex));
			}

		if (isStateValid)
			return;
		var str1 = "not defined";
		if (CurrentState != null)
			str1 = CurrentState.Name;
		Logger.AddError(string.Format("SaveLoad error: entity {0} has saved in invalid state {1}", fsm.Entity.Name,
			str1));
	}
		...
		if (fsm.Entity.SaveLoadInstantiated)
			try {
				if (CurrentState != null) {
					...
					var list = CurrentStateStack.Stack.ToList();
					for (var index = list.Count - 1; index >= 0; --index) {
						var currentState = list[index].currentState;
						if (currentState != null) {
							if (currentState.Parent != null &&
							    typeof(IFiniteStateMachine).IsAssignableFrom(currentState.Parent.GetType()))
							...
							if (typeof(VMState).IsAssignableFrom(currentState.GetType())) {
								if (currentState.IsStable ||
								    typeof(IFiniteStateMachine).IsAssignableFrom(currentState.GetType()))
				...
				Logger.AddError(string.Format("SaveLoad error at {0} fsm afterloading, error: {1}",
					fsm.FSMStaticObject.Name, ex));
			}

		if (!(Guid.Empty != lockingOwnerGuid))
			return;
		var entityByEngineGuid = WorldEntityUtility.GetDynamicObjectEntityByEngineGuid(lockingOwnerGuid);
	...
	public virtual void OnProcessEvent(RaisedEventInfo evntInfo) {
		var instance = evntInfo.Instance;
	...
	public void DoProcessEvent(RaisedEventInfo evntInfo) {
		...
		if (DynamicTalkingFSM.IsTalking) {
			var flag = instance.Name ==
			           EngineAPIManager.GetSpecialEventName(ESpecialEventName.SEN_ON_GLOBAL_TIMER,
				           typeof(VMGameComponent));
			if (!flag && instance.OwnerFSM != null && instance.OwnerFSM.Active &&
			    instance.OwnerFSM.Entity.EngineGuid == GameTimeManager.CurrentPlayCharacterEntity.EngineGuid &&
			    CurrentState != null && CurrentState.Parent != null &&
			    ((IFiniteStateMachine)CurrentState.Parent).GraphType != EGraphType.GRAPH_TYPE_TALKING &&
			    ((IFiniteStateMachine)CurrentState.Parent).GraphType != EGraphType.GRAPH_TYPE_TRADE)
				flag = true;
			if (!flag)
				return;
		}
		...
		if (lockingOwnerFSM != null && lockingOwnerFSM.IsActualEvent(evntInfo))
			return;
		ulong num = 0;
		var messages = evntInfo.Messages;
		if (contextMessages == null) {
			if (messages.Count > 0)
		...
		var ownEvent = IsEventOwn(instance);
		var fsm = this.fsm;
		if (instance.OwnerFSM != null) {
			var ownerFsm = instance.OwnerFSM;
		}

		if (num != 0UL && (CurrentState == null || (long)CurrentState.BaseGuid != (long)num))
			return;
		if (CurrentState != null && CurrentState.Parent != null &&
		    ((FiniteStateMachine)CurrentState.Parent).IsSubGraph) {
			var outputLinkByEvent = GetOuterOutputLinkByEvent(instance);
		...
		if (moveLink == null) {
			var parentGraph = this.fsm.FSMStaticObject.StateGraph;
			if (CurrentState != null)
		...
		if (moveLink != null)
			ProcessLink(moveLink);
		         ...
		         ((FiniteStateMachine)CurrentState.Parent).IsSubGraph) {
			var enterLinkByEvent = GetOuterEnterLinkByEvent(instance, evntInfo.Messages);
		...
		if (((IFiniteStateMachine)CurrentState.Parent).GraphType == EGraphType.GRAPH_TYPE_PROCEDURE)
			Logger.AddError(string.Format(
				" {0} FSM  must return from procedure '{1}' after event '{2}' processing end!",
				this.fsm.FSMStaticObject.Name, CurrentState.Parent.Name, instance.Name));
		if (CurrentStateStack.StackType != EStateStackType.STATESTACK_TYPE_LOCAL)
			return;
		Logger.AddError(string.Format(" {0} FSM  must return from procedure '{1}' after event '{2}' processing end!",
			this.fsm.FSMStaticObject.Name, CurrentState.Parent.Name, instance.Name));
	}
	...
	public void SubscribeToEvents(VMState currState) {
		...
		if (typeof(ISpeech).IsAssignableFrom(currState.GetType())) {
			var contextEvent = VirtualMachine.Instance.GameRootFsm.GetContextEvent(
				EngineAPIManager.GetSpecialEventName(ESpecialEventName.SEN_SPEECH_REPLY, typeof(VMSpeaking)));
			if (contextEvent == null)
				Logger.AddError(string.Format("Speech reply event not created in this FSM at {0}",
					DynamicFSM.CurrentStateInfo));
			else
				contextEvent.Subscribe(fsm);
		} else
			for (var index = 0; index < currState.OutputLinks.Count; ++index) {
				var outputLink = (VMEventLink)currState.OutputLinks[index];
				if (outputLink.Event != null && outputLink.Event.EventInstance != null) {
					var ownerDynamicContext = DynamicFsmUtility.GetEventOwnerDynamicContext(fsm,
						outputLink.Event.EventOwner, (IBlueprint)outputLink.Owner);
					if (ownerDynamicContext != null) {
						if (!ownerDynamicContext.Entity.IsDisposed) {
							var eventByStaticGuid =
								((DynamicFSM)ownerDynamicContext).GetEventByStaticGuid(outputLink.Event.EventInstance
									.BaseGuid);
							if (eventByStaticGuid == null) {
								Logger.AddError(string.Format(
									"Event subscribing error: event with name {0} and static guid={1} not found in object {2} at {3} fsm",
									outputLink.Event.EventInstance.Name, outputLink.Event.EventInstance.BaseGuid,
									outputLink.Event.EventOwner.Name, ownerDynamicContext.FSMStaticObject.Name));
								((DynamicFSM)ownerDynamicContext).GetEventByStaticGuid(outputLink.Event.EventInstance
									.BaseGuid);
							} else
						...
						Logger.AddError(string.Format(
							"Event subscribing error: event owner by variable {0} not found in {1} FSM at {2}",
							outputLink.Event.EventOwner.Name, fsm.FSMStaticObject.Name, DynamicFSM.CurrentStateInfo));
				}
			}
	}
	...
	public EGraphType CurrentFSMGraphType =>
		CurrentState != null && CurrentState.Parent != null &&
		typeof(IFiniteStateMachine).IsAssignableFrom(CurrentState.Parent.GetType())
			? ((IFiniteStateMachine)CurrentState.Parent).GraphType
			: EGraphType.GRAPH_TYPE_EVENTGRAPH;

	public EventMessage GetContextMessage(string paramName) {
		EventMessage eventMessage;
		return contextMessages != null && contextMessages.TryGetValue(paramName, out eventMessage)
			? eventMessage
			: null;
	}
	...
	public object GetLocalVariableValue(string varName) {
		...
		if (loopLocalVariableValuesDict != null &&
		    loopLocalVariableValuesDict.TryGetValue(varName, out localVariableValue1))
			return localVariableValue1;
		object localVariableValue2;
		if (subgraphLocalVariableValuesDict != null &&
		    subgraphLocalVariableValuesDict.TryGetValue(varName, out localVariableValue2))
			return localVariableValue2;
		Logger.AddError(string.Format("Local variable with name {0} not found in {1} FSM at {2}", varName,
			fsm.FSMStaticObject.Name, DynamicFSM.CurrentStateInfo));
		return null;
	}
	...
	private bool IsSaveStateValid() {
		return CurrentState == null ||
		       (((IFiniteStateMachine)CurrentState.Parent).GraphType != EGraphType.GRAPH_TYPE_PROCEDURE &&
		        CurrentStateStack.StackType != EStateStackType.STATESTACK_TYPE_LOCAL &&
		        ((VMState)CurrentState).IsStable);
	}
	...
	protected bool ProcessLink(VMEventLink moveLink) {
		...
		if (moveLink.Event != null && moveLink.Event.EventInstance != null && IsProcedureLink(moveLink)) {
			if (moveLink.DestState == null) {
				Logger.AddError(string.Format("After procedural event {0} in graph {1} must be procedure",
					moveLink.Event.EventInstance.Name, moveLink.Parent.Name));
				return false;
			}
			...
			var destState = (FiniteStateMachine)moveLink.DestState;
		...
		if (CurrentState != null && !((VMBaseObject)moveLink.Parent).IsEqual(CurrentState.Parent)) {
			if (typeof(VMBlueprint) == ((VMBaseObject)moveLink.Parent).Parent.GetType()) {
				if (fsm.FSMStaticObject.IsDerivedFrom(((VMBaseObject)((VMBaseObject)moveLink.Parent).Parent).BaseGuid,
					    true)) {
					lastSubGraphStateStack = new StateStack(CurrentStateStack);
					var destState = moveLink.DestState;
					var destEntryPoint = moveLink.DestEntryPoint;
		...
		var destState1 = moveLink.DestState;
		var destEntryPoint1 = moveLink.DestEntryPoint;
		var flag = false;
		if (CurrentState == null && destState1 != null)
			flag = destState1.Initial;
		if (destState1 != null) {
			if (((lockingOwnerFSM == null ? 1 : destState1.IgnoreBlock ? 1 : 0) | (flag ? 1 : 0)) == 0 &&
			    !IsThisTalking)
				return false;
	...
	public bool Lock(DynamicFSM lockingFSM) {
		...
		Logger.AddWarning(string.Format("This fsm already locked by {0} FSM of static object id={0}",
			lockingOwnerFSM.StaticObject.BaseGuid));
		return false;
	}
	...
	public bool UnLock(DynamicFSM lockingFSM) {
		...
		Logger.AddWarning(string.Format("This fsm is not locked by FSM of static object id={0}",
			lockingFSM.StaticObject.BaseGuid));
		return false;
	}
	...
	public void ProcessState(IState state, int destEntryPoint) {
		try {
			...
			if (typeof(ISpeech).IsAssignableFrom(state.GetType()))
				return;
			var afterExitLink = ((VMState)state).GetAfterExitLink();
	...
	public bool IsActualEvent(RaisedEventInfo evntInfo) {
		var ownEvent = IsEventOwn(evntInfo.Instance);
		...
		if (vmEventLink != null)
			return true;
		var parentGraph = fsm.FSMStaticObject.StateGraph;
		if (CurrentState != null)
			parentGraph = (IFiniteStateMachine)CurrentState.Parent;
		var enterLinkByEvent =
			GetGraphEnterLinkByEvent(parentGraph, fsm, evntInfo.Instance, evntInfo.Messages, ownEvent);
		if (enterLinkByEvent != null)
			return true;
		if (CurrentState != null && CurrentState.Parent != null && ((FiniteStateMachine)CurrentState.Parent).IsSubGraph)
	...
	private void LoadFlipFlopBranchCurrentIndexesData(XmlElement rootNode) {
		...
		for (var i = 0; i < rootNode.ChildNodes.Count; ++i) {
			var firstChild = rootNode.ChildNodes[i].FirstChild;
			flipFlopBranchCurrentIndexes.Add(VMSaveLoadManager.ReadUlong(firstChild),
				VMSaveLoadManager.ReadInt(firstChild.NextSibling));
		}
	}
	...
	protected virtual void ReturnToOuterGraph(VMEventLink prevLink = null) {
		...
		if (prevLink != null && prevLink.LinkExitType == ELinkExitType.LINK_EXIT_TYPE_OUTER_EVENT_EXECUTION &&
		    currentGraphExecEvent != null) {
			outerGraphLink = GetOuterEnterLinkByEvent(currentGraphExecEvent.Instance, currentGraphExecEvent.Messages,
				(IFiniteStateMachine)prevLink.Parent);
			if (outerGraphLink != null)
				outherGraph = (IFiniteStateMachine)outerGraphLink.Parent;
		}

		if (DebugUtility.IsDebug) {
			var state = CurrentState ?? fsm.FSMStaticObject.StateGraph;
			if (!eventProcessedStateSequence.ContainsKey(state.BaseGuid)) {
				...
				var num = eventProcessedStateSequenceCount[state.BaseGuid] + 1;
				eventProcessedStateSequenceCount[state.BaseGuid] = num;
				if (num > CIRCULARING_CHECK_MAX_ITERATIONS_COUNT) {
					circularingAlarm = true;
					Logger.AddError(string.Format("State {0} process is cycling!!! at {1}", state.Name,
						DynamicFSM.CurrentStateInfo));
					return;
				}
			}
	...
	private void ReturnToOuterGraph(IFiniteStateMachine outherGraph, VMEventLink outerGraphLink = null) {
		if ((CurrentStateStack.StackType == EStateStackType.STATESTACK_TYPE_LOCAL) & IsCurrentProcedure &&
		    CurrentStateStack.Count == 1)
			SetCurrentStateStack(EStateStackType.STATESTACK_TYPE_MAIN);
		else {
			lastSubGraphStateStack =
				outherGraph == null || outerGraphLink == null ||
				CurrentStateStack.StackType != EStateStackType.STATESTACK_TYPE_MAIN
					? null
					: new StateStack(CurrentStateStack);
			...
			if (outherGraph != null) {
				while (CurrentState != null) {
					if (CurrentState.Parent == null)
						Logger.AddError(string.Format("Current state {0} id={1} has null parent", CurrentState.Name,
							CurrentState.BaseGuid));
					if (!CurrentState.Parent.IsEqual(outherGraph)) {
						PopState();
						if (CurrentState == null) {
							Logger.AddError(string.Format(
								"Invalid outher graph returning: return graph id={0} not found in state stack",
								outherGraph.BaseGuid));
							goto label_15;
						}
					} else
						goto label_15;
			...
			if (CurrentState == null)
				Logger.AddError(string.Format(
					"Invalid outher graph returning: CurrentState is not defined in graph {0} after returning from state {1}, graph {2}",
					str3, str1, str2));
			else if (CurrentState.Parent == null)
				Logger.AddError(string.Format(
					"Invalid outher graph returning: CurrentState.Parent is not defined in graph {0}", str3));
			else {
	...
	protected void ReturnToPreviousState(bool prevIsCurrent = false) {
		...
		if (state != null && DebugUtility.IsDebug && eventProcessedStateSequence.ContainsKey(state.BaseGuid) &&
		    PreviousState != null && (long)PreviousState.BaseGuid != (long)state.BaseGuid)
			state = PreviousState;
		if (state != null) {
			...
			var previousStack = PreviousStack;
	...
	protected void MoveIntoSubGraph(
		...
		if ((!CurrentState.IsProcedure ? 0 : subGraph.IsProcedure ? 1 : 0) == 0) {
			...
			if (subGraph.Parent == null || CurrentState == null)
				Logger.AddError(string.Format("Invalid subgraph {0} enter in {1} fsm", subGraph.Name,
		...
		var initState = subGraph.InitState;
	...
	protected virtual void MoveIntoState(IState newState, int destEntryPoint, bool bNextLevel = false) {
		if (newState == null)
			...
			if (destEntryPoint < 0 || destEntryPoint >= newState.EntryPoints.Count)
				Logger.AddError(string.Format("Invalid entry point index at move to {0} state in {1} object",
					newState.Name, fsm.StaticGuid));
	...
	protected void PushState(IState prevState, IState newState) {
		if (newState == null)
			...
			if (!((VMState)CurrentState).Initial || ((FiniteStateMachine)CurrentState.Parent).IsSubGraph)
				return;
			var parent = (FiniteStateMachine)CurrentState.Parent;
			...
			if (!typeof(IBlueprint).IsAssignableFrom(parent.Owner.GetType()))
				return;
			try {
				var baseBlueprints = ((IBlueprint)parent.Owner).BaseBlueprints;
	...
	protected IState PopState() {
		if (CurrentStateStack.Count == 0)
			return null;
		var currentState = CurrentStateStack.Pop().currentState;
		if (!IsCurrentProcedure) {
	...
	protected bool OnStateIn(IState state, bool bCanBlock = true) {
		...
		if (DebugUtility.IsDebug) {
			...
			var str2 = str1 + state.Name;
			debugCurrentState = state;
			if (fsm.Entity != null)
				fsm.Entity.DebugContextInfo = str2;
			if ((long)debugCurrStepStateGuid != (long)state.BaseGuid &&
			    DebugUtility.Debugger.OnStateInput(fsm, state) && bCanBlock) {
	...
	private void OnStateExec(IState state) {
		DebugUtility.OnStateExec(fsm, state);
	}
	...
	private void ExecuteState(IState state, int destEntryPoint) {
		if (DebugUtility.IsDebug) {
			if (!eventProcessedStateSequence.ContainsKey(((VMBaseObject)state).BaseGuid)) {
				...
				var num = eventProcessedStateSequenceCount[((VMBaseObject)state).BaseGuid] + 1;
	...
	private bool IsReturnToPrevios() {
		var num = previosStateReturningMode ? 1 : 0;
		previosStateReturningMode = false;
		return num != 0;
	}
	...
	protected void ProcessActionLine(IActionLine actionLine, bool bFromSpeech = false) {
		...
		if (actionLine.ActionLineType == EActionLineType.ACTION_LINE_TYPE_LOOP) {
			if (!typeof(IActionLoop).IsAssignableFrom(actionLine.GetType()))
				return;
			var actionLoop = (IActionLoop)actionLine;
			var loopStartIndexValue = GetLoopStartIndexValue(actionLoop);
			var num = GetLoopEndIndexValue(actionLoop);
			var loopListValue = GetLoopListValue(actionLoop);
			...
			if (actionLoop.LoopRandomIndexing) {
				var randomMinIndex = loopStartIndexValue;
				var randomMaxIndex = num;
				var indexesCount = num - loopStartIndexValue;
				if (loopListValue != null && randomMinIndex == 0)
					randomMaxIndex = loopListValue.ObjectsCount;
				var randomIndexes = VMMath.GetRandomIndexes(randomMinIndex, randomMaxIndex, indexesCount);
				for (var index = 0; index < randomIndexes.Count; ++index) {
				...
				if (num - loopStartIndexValue >= ACTION_LOOP_CHECK_MAX_ITERATIONS_COUNT) {
					...
					Logger.AddError(string.Format(
						"Invalid loop indexing (infinity loop) in loop action line id={0} at {1}", actionLine.BaseGuid,
						DynamicFSM.CurrentStateInfo));
				}

				for (var currentLoopIndex = loopStartIndexValue; currentLoopIndex < num; ++currentLoopIndex) {
	...
	private void ReturnToMemoryStack(StateStack prevMemoryStack) {
		...
		var source = new Stack<StatePoint>(prevMemoryStack.Stack);
		do {
	...
	protected void ProcessBranch(IBranch branch) {
		...
		if (branch.StateType == EStateType.STATE_TYPE_MINVALUE_BRANCH ||
		    branch.StateType == EStateType.STATE_TYPE_MAXVALUE_BRANCH)
			...
			for (var index = 0; index < branch.GetExitPointsCount(); ++index) {
				bool flag;
				if (num1 >= 0)
					flag = index == num1;
				else {
					var branchCondition = branch.GetBranchCondition(index);
					flag = index >= branch.GetExitPointsCount() - 1 ||
					       ExpressionUtility.CalculateConditionResult(branchCondition, fsm);
				}

				if (flag) {
					var sourceExitPointIndex = ((VMState)branch).GetOutputLinkBySourceExitPointIndex(index);
					if (sourceExitPointIndex != null) {
						var num2 = IgnoreBranchCase(sourceExitPointIndex.DestState) ? 1 : 0;
	...
	protected void ProcessMinMaxBranch(IBranch branch) {
		...
		for (var exitPntIndex = 0; exitPntIndex < branch.GetExitPointsCount(); ++exitPntIndex) {
			var branchCondition = (VMPartCondition)branch.GetBranchCondition(exitPntIndex);
			if (branchCondition != null) {
				var firstExpression = (VMExpression)branchCondition.FirstExpression;
				if (firstExpression != null) {
					var expressionResult = ExpressionUtility.CalculateExpressionResult(firstExpression, fsm);
					if (branch.StateType == EStateType.STATE_TYPE_MINVALUE_BRANCH) {
		...
		if (iSrcPntIndex < 0)
			return;
		var sourceExitPointIndex = ((VMState)branch).GetOutputLinkBySourceExitPointIndex(iSrcPntIndex);
		if (sourceExitPointIndex != null) {
			var destState = sourceExitPointIndex.DestState;
			if (sourceExitPointIndex.DestState != null)
				ProcessLink(sourceExitPointIndex);
			else if (sourceExitPointIndex.ExitFromSubGraph || IsThisTalking)
				ReturnToOuterGraph(sourceExitPointIndex);
	...
	private void SubscribeToOnLoadEvent(VMState state) {
		var contextEvent = VirtualMachine.Instance.GameRootFsm.GetContextEvent(
			EngineAPIManager.GetSpecialEventName(ESpecialEventName.SEN_LOAD_GAME, typeof(VMGameComponent)));
		if (contextEvent == null)
	...
	private void SubscribeToNullSourceGraphEvents(FiniteStateMachine eventGraph) {
		for (var index = 0; index < eventGraph.EnterLinks.Count; ++index) {
			var enterLink = (VMEventLink)eventGraph.EnterLinks[index];
			if (!enterLink.IsInitial() && enterLink.Event != null && enterLink.Event.EventInstance != null) {
				var ownerDynamicContext =
					DynamicFsmUtility.GetEventOwnerDynamicContext(fsm, enterLink.Event.EventOwner,
						(IBlueprint)enterLink.Owner);
				if (ownerDynamicContext != null) {
					if (!ownerDynamicContext.Entity.IsDisposed) {
						var eventByStaticGuid =
							((DynamicFSM)ownerDynamicContext).GetEventByStaticGuid(enterLink.Event.EventInstance
								.BaseGuid);
						if (eventByStaticGuid == null) {
							Logger.AddError(string.Format(
								"Event subscribing error: event with name {0} and static guid={1} not found in object {2} at {3} fsm",
								enterLink.Event.EventInstance.Name, enterLink.Event.EventInstance.BaseGuid,
								enterLink.Event.EventOwner.Name, ownerDynamicContext.FSMStaticObject.Name));
							((DynamicFSM)ownerDynamicContext).GetEventByStaticGuid(enterLink.Event.EventInstance
								.BaseGuid);
						} else
							eventByStaticGuid.Subscribe(fsm);
					...
					Logger.AddError(string.Format(
						"Event subscribing error: event owner by variable {0} not found in {1} FSM at {2}",
						enterLink.Event.EventOwner.Name, fsm.FSMStaticObject.Name, DynamicFSM.CurrentStateInfo));
			}
		}

		if (!typeof(VMBlueprint).IsAssignableFrom(eventGraph.Parent.GetType()))
			return;
		var baseBlueprints = ((VMBlueprint)eventGraph.Parent).BaseBlueprints;
		if (baseBlueprints == null)
			return;
		for (var index = 0; index < baseBlueprints.Count; ++index)
			SubscribeToNullSourceGraphEvents((FiniteStateMachine)baseBlueprints[index].StateGraph);
				...
				if (ownerDynamicContext != null) {
					if (!ownerDynamicContext.Entity.IsDisposed) {
						...
						if (eventByStaticGuid != null)
							...
							Logger.AddError(string.Format(
								"Event desubscribing error: event with static guid={0} not found in {1} FSM at {2}",
								enterLink.Event.EventInstance.BaseGuid, fsm.FSMStaticObject.Name,
								DynamicFSM.CurrentStateInfo));
					}
				} else
					Logger.AddWarning(string.Format(
						"Event desubscribing error: event owner by variable {0} not found in {1} FSM at {2}",
						enterLink.Event.EventOwner.Name, fsm.FSMStaticObject.Name, DynamicFSM.CurrentStateInfo));
			}
		}
	}
	...
	private void ResetEvents(VMState remState) {
		...
		for (var index = 0; index < remState.OutputLinks.Count; ++index) {
			var outputLink = (VMEventLink)remState.OutputLinks[index];
			if (outputLink.Event != null && outputLink.Event.EventInstance != null) {
				var ownerDynamicContext = DynamicFsmUtility.GetEventOwnerDynamicContext(fsm,
					outputLink.Event.EventOwner, (IBlueprint)outputLink.Owner);
				if (ownerDynamicContext != null) {
					if (!ownerDynamicContext.Entity.IsDisposed) {
						...
						if (eventByStaticGuid != null)
							...
							Logger.AddError(string.Format(
								"Event desubscribing error: event with static guid={0} not found in {1} FSM at {2}",
								outputLink.Event.EventInstance.BaseGuid, fsm.FSMStaticObject.Name,
								DynamicFSM.CurrentStateInfo));
					}
				} else
					Logger.AddWarning(string.Format(
	...
	private bool IsProcedureLink(IEventLink moveLink) {
		if (moveLink.SourceState == null && moveLink.DestState != null &&
		    typeof(IFiniteStateMachine).IsAssignableFrom(moveLink.DestState.GetType()) &&
		    moveLink.DestState.IsProcedure) {
			var outputLink = (IEventLink)moveLink.DestState.OutputLinks[0];
	...
	private void LoadSubgraphLocalVariablesData(XmlElement rootNode) {
		...
		for (var i = 0; i < rootNode.ChildNodes.Count; ++i) {
			...
			if (!subgraphLocalVariableValuesDict.ContainsKey(innerText))
				...
				Logger.AddError(string.Format("Save load error: FSM {0} subgraph local variable {1} dublicated",
					fsm.FSMStaticObject.Name, innerText));
		}
	}
	...
	private VMEventLink GetOuterOutputLinkByEvent(DynamicEvent evnt) {
		...
		for (var index = 0; index < CurrentStateStack.Stack.Count; ++index) {
			var outputLinkByEvent2 =
				GetStateOutputLinkByEvent((VMState)CurrentStateStack.Stack.ElementAt(index).currentState, fsm, evnt,
					ownEvent);
	...
	private VMEventLink GetOuterEnterLinkByEvent(
		...
		if (evnt.Entity != null && fsm.Entity != null)
			ownEvent = evnt.Entity.EngineGuid == fsm.Entity.EngineGuid;
		for (var index = 0; index < CurrentStateStack.Stack.Count; ++index) {
			var statePoint = CurrentStateStack.Stack.ElementAt(index);
			if (statePoint.currentState.Parent != null) {
				var flag = false;
				var parent = (FiniteStateMachine)statePoint.currentState.Parent;
				if (currentGraph != null && (long)currentGraph.BaseGuid == (long)parent.BaseGuid)
					flag = true;
				if (!flag) {
					var enterLinkByEvent2 = GetGraphEnterLinkByEvent(parent, fsm, evnt, messages, ownEvent);
	...
	private VMEventLink GetOuterAfterExitLink() {
		var statePoint1 = CurrentStateStack.Stack.ElementAt(0);
		var afterExitLink = ((VMState)statePoint1.currentState).GetAfterExitLink();
		if (CurrentStateStack.Stack.Count < 2)
			return afterExitLink;
		var statePoint2 = CurrentStateStack.Stack.ElementAt(1);
		...
		return statePoint2.currentState.GetCategory() == EObjectCategory.OBJECT_CATEGORY_GRAPH &&
		       ((FiniteStateMachine)statePoint2.currentState).SubstituteGraph != null &&
		       (long)((FiniteStateMachine)statePoint2.currentState).SubstituteGraph.BaseGuid ==
		       (long)statePoint1.currentState.Parent.BaseGuid
			? ((VMState)((FiniteStateMachine)statePoint2.currentState).SubstituteGraph).GetAfterExitLink()
			: afterExitLink;
	}

	private bool IsCurrentProcedure => CurrentState != null && CurrentState.Parent != null &&
	                                   ((VMState)CurrentState.Parent).IsProcedure;

	private void PushLoopToStack(IContextElement ownerContextElement, ICommonList loopList) {
		try {
			var loopInfo = new LoopInfo(ownerContextElement, loopList);
			var contextVariables = ownerContextElement.LocalContextVariables;
			if (loopLocalVariableValuesDict == null)
				loopLocalVariableValuesDict = new Dictionary<string, object>();
			for (var index = 0; index < contextVariables.Count; ++index)
			...
			Logger.AddError(string.Format("Push loop stack error at action {0}: {1} at {2}",
				ownerContextElement.BaseGuid, ex.Message, DynamicFSM.CurrentStateInfo));
		}
	}
	...
	protected void CheckLoopStackClear() {
		if (loopStack != null && loopStack.Count > 0) {
			Logger.AddError(string.Format(
				"Loop processing error: loop stack not clear after entry action line processing in {0} at {0}!",
				fsm.FSMStaticObject.Name, DynamicFSM.CurrentStateInfo));
			loopStack.Clear();
		}
	...
	protected virtual bool IgnoreBranchCase(IState newState) {
		return false;
	}
	...
	private void PopLoopFromStack() {
		if (loopStack != null)
			...
			Logger.AddError(string.Format("Loop processing error: attempt to pop not inited loop stack in {0} at {0}!",
				fsm.FSMStaticObject.Name, DynamicFSM.CurrentStateInfo));
	}
	...
	private void SetCurrentLoopIndexValue(int currentLoopIndex) {
		if (loopStack != null && loopStack.Count != 0)
			...
			Logger.AddError(string.Format("Cannot get current loop index because of loop stack is empty in {0} at {1}",
				fsm.FSMStaticObject.Name, DynamicFSM.CurrentStateInfo));
	}

	private int GetLoopStartIndexValue(IActionLoop actionLoop) {
		var startIndexParam = actionLoop.StartIndexParam;
		if (startIndexParam != null) {
			...
			if (startIndexParam is CommonVariable) {
				var variable = (CommonVariable)startIndexParam;
				var vmType = new VMType(typeof(int));
				if (!variable.IsBinded)
					variable.Bind(fsm.FSMStaticObject, vmType, ((VMActionLine)actionLoop).LocalContext);
				return (int)((VMVariableService)IVariableService.Instance).GetDynamicVariableValue(variable, vmType,
					fsm);
			}
		}
	...
	private ICommonList GetLoopListValue(IActionLoop actionLoop) {
		var listParamInstance = actionLoop.LoopListParamInstance;
		if (listParamInstance != null)
			try {
				return (ICommonList)ExpressionUtility.GetContextParamValue(fsm, actionLoop.LoopListParam,
					listParamInstance.Type);
			} catch (Exception ex) {
	...
	private int GetCurrentFlipFlopVariantIndex(IBranch branch) {
		...
		var branchCurrentIndex = flipFlopBranchCurrentIndexes[branch.BaseGuid];
	...
	private void AddSubgraphLocalVariable(string localVarName, object localVarValue) {
		if (localVarValue != null && typeof(IEntity).IsAssignableFrom(localVarValue.GetType())) {
			...
			var entity = (IEntity)localVarValue;
	...
	private void MakeSubgraphInputParams(
		...
		if (inputParamSource == null)
			return;
		for (var index = 0; index < subGraph.InputParams.Count; ++index)
			if (index < inputParamSource.SourceParams.Count)
				AddSubgraphLocalVariable(subGraph.InputParams[index].Name,
					ExpressionUtility.GetContextParamValue(fsm, inputParamSource.SourceParams[index],
						subGraph.InputParams[index].Type, (ILocalContext)inputParamSource));
	}
}