...
public class FSMEventManager {
	...
	public DynamicEvent GetContextEvent(string eventName) {
		...
		return fsmEventsByName != null && fsmEventsByName.TryGetValue(eventName, out dynamicEvent)
			? dynamicEvent
			: null;
	}
	...
	public void AfterSaveLoading() {
		foreach (var fsmEvent in fsmEvents) {
			var dynamicEvent = fsmEvent.Value;
			var flag = false;
	...
	private void LoadEvents() {
		var count = fsm.FSMStaticObject.Events.Count;
		var num = 0;
		...
		if (fsm.FSMStaticObject.Events == null)
			return;
		foreach (var staticEvent in fsm.FSMStaticObject.Events)
	...
	private void LoadEventsFromEngineDirect() {
		...
		foreach (var component in fsm.Entity.Components) {
			...
			if (functionalComponentByName == null)
				...
				for (var index = 0; index < functionalComponentByName.Events.Count; ++index) {
					var apiEventInfo = functionalComponentByName.Events[index];
