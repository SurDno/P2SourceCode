...
public class VMEntity :
	...
	public override void Initialize(
		...
		if (templateObj == null)
			...
			if (typeof(IWorldObject).IsAssignableFrom(templateObj.GetType())) {
				var worldObject = (IWorldObject)templateObj;
	...
	public override void Initialize(ILogicObject templateObj, Guid engineGuid = default) {
	...
	public virtual void LoadFromXML(XmlElement xmlNode) {
		...
		for (var i = 0; i < xmlNode.ChildNodes.Count; ++i) {
			...
			if (childNode.Name == "Instantiated")
				...
				engineEntityTemplate =
					ServiceCache.TemplateService.GetTemplate<IEntity>(VMSaveLoadManager.ReadGuid(childNode));
			else if (childNode.Name == "InstanceId") {
				Instantiated = true;
				isDynamicChild = true;
				engineEntity =
					ServiceCache.Factory.Instantiate(engineEntityTemplate, VMSaveLoadManager.ReadGuid(childNode));
			}
		}
		...
		if (typeof(IWorldObject).IsAssignableFrom(editorTemplate.GetType()))
			try {
				var objects = ServiceCache.Simulation.Objects;
				if (!typeof(IWorldHierarchyObject).IsAssignableFrom(editorTemplate.GetType()))
					ServiceCache.Simulation.Add(Instance, objects);
			} catch (Exception ex) {
				Logger.AddError(string.Format("Saveload error: adding engine instance of {0} to simulation failed: {1}",
					Name, ex));
			}
		...
		if (listNode == null || listNode.ChildNodes.Count <= 0)
			return;
		var list = new List<VMEntity>();
		VMSaveLoadManager.LoadDynamiSerializableList(listNode, list);
		var componentByName = (VMStorage)GetComponentByName("Storage");
	...
	public void StateSave(IDataWriter writer) {
		...
		if (Instance == null)
			Logger.AddError(
				string.Format("Saveload error: cannot save entity {0}, because it's engine instance not created!",
					Name));
			...
			var saveLoadKeyGuid = GetSaveLoadKeyGuid();
			SaveManagerUtility.Save(writer, "Id", saveLoadKeyGuid);
			var baseGuid = editorTemplate.Blueprint.BaseGuid;
	...
	public override void OnCreate(bool afterLoad = false) {
		if (!afterLoad && typeof(IWorldHierarchyObject).IsAssignableFrom(editorTemplate.GetType()))
			foreach (var simpleChild in ((IWorldHierarchyObject)editorTemplate).SimpleChilds)
	...
	public void InitFSM(DynamicFSM fsm) {
		FSM = fsm;
	}
	...
	public bool NeedCreateFSM => VirtualMachine.Instance.ForceCreateFSM ||
	                             editorTemplate.Blueprint.StateGraph != null ||
	                             (typeof(IWorldObject).IsAssignableFrom(editorTemplate.GetType()) &&
	                              !((IWorldObject)editorTemplate).DirectEngineCreated);
	...
	public void RegistrAsEngineRoot() {
		...
		if (VMEntityUtility.RegisteredEngineRoot == null)
			...
			Logger.AddError(string.Format("Engine root {0} already registered !",
				VMEntityUtility.RegisteredEngineRoot.Name));
	}

	public override void SetPosition(IEntity positionMileStoneEngEntity,
		AreaEnum NOT_USED_areaType = AreaEnum.Unknown) {
		try {
			var component = Instance.GetComponent<INavigationComponent>();
			if (component != null)
	...
	public static IEnumerable<FreeEntityInfo> GetFreeEntities() {
		var storables = ServiceCache.Simulation.Storables;
		if (storables != null && storables.Childs != null)
			foreach (var child in storables.Childs)
				if (!child.DontSave)
	...
	public static void LoadFreeEntities(List<FreeEntityInfo> freeEntities) {
		for (var index = 0; index < freeEntities.Count; ++index)
			ServiceCache.Simulation.Add(
				ServiceCache.Factory.Instantiate(
					ServiceCache.TemplateService.GetTemplate<IEntity>(freeEntities[index].TemplateId),
					freeEntities[index].InstanceId), ServiceCache.Simulation.Storables);
	}
	...
	public bool IsPlayerControllable(bool bMain = false) {
		if (GetComponentByName("PlayerControllerComponent") != null) {
			...
			if (editorTemplate != null)
				return GameTimeManager.MainContextPlayingCharacter == null || (long)editorTemplate.Blueprint.BaseGuid ==
					(long)GameTimeManager.MainContextPlayingCharacter.BaseGuid;
		}
	...
	public void CreateSimpleChild(IHierarchyObject simpleChildTemplate) {
		if (Instance == null)
			Logger.AddError(string.Format("Cannot create simple child entity: owner engine instance not defined in {0}",
				Name));
		else {
			var childEntity = new VMEntity();
			childEntity.InitSimpleChild(simpleChildTemplate);
			if (childEntity.Instance == null)
				Logger.AddError(string.Format(
					"Cannot create simple child entity by template id={0}: engine instance not created",
					simpleChildTemplate.EngineTemplateGuid));
			else {
	...
	public void TestHierarchyConsistency() {
		if (Instance != null && editorTemplate != null && !IsSimple && editorTemplate.Name != Instance.Name)
			VirtualMachine.SetFatalError(string.Format(
				"SAVE LOAD CRITICAL ERROR: HIERARCHY INCONSISTENCY AT OBJECT {0}, ENGINE_GUID={1}, ENGINE_NAME={2} !!!",
				editorTemplate.Name, Instance.Id, Instance.Name));
		if (childs == null)
			return;
		for (var index = 0; index < childs.Count; ++index)
	...
	protected override void AddAggregateChild(VMBaseEntity childEntity) {
		if (EngineGuid == VirtualMachine.Instance.WorldHierarchyRootEntity.EngineGuid && !childEntity.IsHierarchy)
			VirtualMachine.SetFatalError(
				string.Format("UNUSUAL HIERARCHY SYSTEM ERROR: INCORRECT OBJECT {0} WORLD ADDING !!!",
					childEntity.Name));
		base.AddAggregateChild(childEntity);
	}

	protected override Type GetComponentTypeByName(string componentName) {
		var componentTypeByName = EngineAPIManager.GetComponentTypeByName(componentName);
	...
	protected override void MakeEngineData() {
		...
		if (engineEntityTemplate == null) {
			if (Instance != null)
				...
				Logger.AddError(string.Format(
					"Cannot get engine data for entity with editor template id = {0}: Engine entity and template are not defined",
					EngineBaseTemplateGuid));
				return;
			}
		}
	...
	private void OnLoadDynamicChildEntity(VMEntity dynamicChild, VMStorage storageContainer) {
		var engineTemplateByGuid =
			((VMGameRoot)IStaticDataContainer.StaticDataContainer.GameRoot).GetEngineTemplateByGuid(dynamicChild
				.EngineTemplateGuid);
		if (engineTemplateByGuid == null)
			Logger.AddError(string.Format(
				"Saveload error: dynamic child template with engine guid = {0} not registered",
				GuidUtility.GetGuidString(dynamicChild.EngineTemplateGuid)));
		else if (dynamicChild.Instance == null)
			Logger.AddError(string.Format(
				"SaveLoad error: Cannot load dynamic child with template {0}, instance not created",
				dynamicChild.EngineTemplateGuid));
		else {
			var instance1 = dynamicChild.Instance;
