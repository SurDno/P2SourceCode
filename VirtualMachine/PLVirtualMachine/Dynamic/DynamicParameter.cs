...
public class DynamicParameter :
	...
	public DynamicParameter(VMEntity entity, DynamicFSM parentFSM, VMParameter stParam)
		...
		try {
			...
			if (typeof(ICommonList).IsAssignableFrom(paramType.BaseType) && stParam.Value != null &&
			    typeof(ICommonList).IsAssignableFrom(stParam.Value.GetType()))
	...
	public virtual bool IsEqual(IVariable other) {
		return (!typeof(DynamicParameter).IsAssignableFrom(other.GetType()) ||
		        parentFSM.Equals(((DynamicParameter)other).parentFSM)) && other.Name == Name;
	}
	...
	public void LoadFromXML(XmlElement xmlNode) {
		for (var i = 0; i < xmlNode.ChildNodes.Count; ++i) {
			...
			if (childNode.Name == "Value") {
				var realParamType = GetRealParamType(paramType.BaseType);
	...
	public void InitStandartParam(VMComponent component) {
		if (IsCustom)
			...
			var componentPropertyInfo = InfoAttribute.GetComponentPropertyInfo(component.GetComponentTypeName(), name);
	...
	public bool Self => StaticObject != null && ((VMParameter)StaticObject).Implicit &&
	                    ((VMParameter)StaticObject).Name.EndsWith(nameof(Self));
	...
	public void UpdateStandartValueByPhysicalComponent() {
		if (objComponent == null) {
			if (IsCustom)
				return;
			Logger.AddError(string.Format("Invalid standart parameter {0} in entity {1} at {2}", Name, Entity.Name,
				DynamicFSM.CurrentStateInfo));
		} else {
			var vmReference = objPropertyInfo.GetValue(objComponent, null);
	...
	public object Value {
		...
		set {
			if (Entity.IsDisposed)
				Logger.AddError(string.Format("Attempt to removed object {0} parameter accessing at  {1}", Entity.Name,
					DynamicFSM.CurrentStateInfo));
			else {
				var num = DynamicCompatibleType(value) ? 1 : 0;
				var flag = false;
				if (num == 0) {
					if (value == null)
						...
						var obj = value;
						try {
							if (typeof(IObject).IsAssignableFrom(obj.GetType()) &&
							    typeof(IRef).IsAssignableFrom(paramType.BaseType))
							...
							Logger.AddError(string.Format("Cannot direct convert type {0} to {1}", value.GetType(),
								paramType));
						}

						if (!DynamicCompatibleType(obj)) {
							Logger.AddError(string.Format(
								"Receiving value type {0} not compatible with param type {1} on setting param {2} at {3}",
								value.GetType(), paramType, Name, DynamicFSM.CurrentStateInfo));
	...
	private void DoSetValue(object newValue) {
		if (newValue != null && typeof(ContextVariable).IsAssignableFrom(newValue.GetType()))
			Logger.AddError(string.Format(
				"Invalid param {0} value setting: attempt to use context variable {1} as param value", Name, newValue));
		if (this.currValue != null && typeof(IObjRef).IsAssignableFrom(this.currValue.GetType()))
			WorldEntityUtility.GetDynamicObjectEntityByEngineGuid(((IEngineInstanced)this.currValue).EngineGuid)
				?.GetFSM().RemoveRefParam(this);
		if (firstSet) {
		...
		if (this.currValue == null || !typeof(IObjRef).IsAssignableFrom(this.currValue.GetType()) ||
		    StaticObject == null)
			return;
		WorldEntityUtility.GetDynamicObjectEntityByEngineGuid(((IEngineInstanced)this.currValue).EngineGuid)?.GetFSM()
			.AddRefParam(this);
	}
	...
	private void DoSetPhysicalStandartPropertyValue(object value) {
		if (null != objPropertyInfo) {
			...
			try {
				...
				Logger.AddError(string.Format("Parameter {0} at component {1} vaue set error!: {2}", Name,
					objComponent.Name, ex));
			}
		} else
	...
	public void OnUpdateParam(object prevVal, object newVal) {
		if (StaticObject == null)
			return;
		var eventsByStaticGuid = DynamicParameterUtility.GetParameterDependedEventsByStaticGuid(StaticObject.BaseGuid);
		if (eventsByStaticGuid == null)
			return;
		var count = eventsByStaticGuid.Count;
		if (count <= 0)
			return;
		for (var index = 0; index < count; ++index) {
			var dependedEventRef = eventsByStaticGuid[index];
			if (dependedEventRef != null) {
				var flag = VMTypeMathUtility.IsValueEqual(prevVal, newVal);
	...
	public void OnUpdateParam() {
		...
		if (eventsByStaticGuid == null)
			return;
		for (var index = 0; index < eventsByStaticGuid.Count; ++index)
	...
	private bool DynamicCompatibleType(object value) {
		return DynamicCompatibleTypeImpl(value);
	}
	...
	private bool DynamicCompatibleTypeImpl(object value) {
		if (typeof(IObjRef).IsAssignableFrom(paramType.BaseType)) {
			...
			if (!typeof(IObjRef).IsAssignableFrom(value.GetType()))
				return false;
			var vmObjRef = (VMObjRef)value;
			if ((currValue != null && typeof(IObjRef).IsAssignableFrom(currValue.GetType()) &&
			     ((((IRef)currValue).StaticInstance != null && vmObjRef.StaticInstance != null &&
			       vmObjRef.StaticInstance.IsEqual(((IRef)currValue).StaticInstance)) ||
			      (((VMObjRef)currValue).TypeTemplate != null && vmObjRef.TypeTemplate != null &&
			       DynamicTemplatesCompatible(((VMObjRef)currValue).TypeTemplate, vmObjRef.TypeTemplate)))) ||
			    (vmObjRef.Object == null && !WorldEntityUtility.IsDynamicGuidExist(vmObjRef.EngineGuid)) ||
			    !vmObjRef.IsDynamic)
				return true;
			...
			if (entityByEngineGuid.EditorTemplate != null) {
				var staticObject = (VMParameter)StaticObject;
				if (staticObject != null && staticObject.Value != null &&
				    ((VMObjRef)staticObject.Value).Object != null &&
				    !((VMLogicObject)((VMObjRef)staticObject.Value).Object).Static)
					return ((VMObjRef)staticObject.Value).Object.GetCategory() != EObjectCategory.OBJECT_CATEGORY_CLASS
						? entityByEngineGuid.EditorTemplate.IsEqual(((BaseRef)staticObject.Value).StaticInstance)
						: entityByEngineGuid.EditorTemplate.IsDerivedFrom(
							((BaseRef)staticObject.Value).StaticInstance.BaseGuid, true);
				if (paramType.SpecialType != "") {
					var flag = true;
					foreach (var functionalPart in paramType.GetFunctionalParts())
						if (!(!((VMLogicObject)entityByEngineGuid.EditorTemplate).DirectEngineCreated
							    ? entityByEngineGuid.EditorTemplate.IsFunctionalSupport(functionalPart)
							    : entityByEngineGuid.IsFunctionalSupport(functionalPart))) {
							flag = false;
							break;
						}
		...
		if (typeof(IVariable).IsAssignableFrom(value.GetType()) &&
		    ((IVariable)value).Type.BaseType == paramType.BaseType)
			return VMTypeUtility.IsTypesCompatible(paramType, ((IVariable)value).Type);
		var secondType = new VMType(value.GetType());
		return (typeof(ICommonList).IsAssignableFrom(paramType.BaseType) &&
		        secondType.BaseType == typeof(VMDynamicCommonList)) ||
		       VMTypeUtility.IsTypesCompatible(paramType, secondType);
	}

	private bool DynamicTemplatesCompatible(IBlueprint needTemplate, IBlueprint sourceTemplate) {
		foreach (var functionalComponent in needTemplate.FunctionalComponents)
	...
	private Type GetRealParamType(Type type) {
		var realRefType = BaseSerializer.GetRealRefType(type);
