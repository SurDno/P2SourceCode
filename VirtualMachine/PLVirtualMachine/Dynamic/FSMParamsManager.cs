...
public class FSMParamsManager {
	...
	public void Clear() {
		if (fsmDynamicParams != null)
			for (var index = 0; index < fsmDynamicParams.Count; ++index)
				if (typeof(IObjRef).IsAssignableFrom(fsmDynamicParams[index].Type.BaseType) &&
				    fsmDynamicParams[index].Value != null &&
				    typeof(IObjRef).IsAssignableFrom(fsmDynamicParams[index].Value.GetType())) {
					var entityByEngineGuid =
						WorldEntityUtility.GetDynamicObjectEntityByEngineGuid(
							((IEngineInstanced)fsmDynamicParams[index].Value).EngineGuid);
	...
	public IEnumerable<DynamicParameter> FSMDynamicParams {
		get {
			foreach (var fsmDynamicParam in fsmDynamicParams)
	...
	public IParam GetContextParam(string paramName) {
		IParam contextParam;
		if (GuidUtility.GetGuidFormat(paramName) == EGuidFormat.GT_BASE &&
		    contextParamsByStaticGuid.TryGetValue(StringUtility.ToUInt64(paramName), out contextParam))
			return contextParam;
		var strArray = paramName.Split('.');
	...
	public void StateSave(IDataWriter writer) {
		SaveParameters(writer, "Parameters");
	}
	...
	private void LoadParams() {
		var contextVariables =
			fsm.FSMStaticObject.GetContextVariables(EContextVariableCategory.CONTEXT_VARIABLE_CATEGORY_PARAM);
		var capacity = fsm.FSMStaticObject.StandartParamsCount + fsm.FSMStaticObject.CustomParamsCount + 1;
		...
		foreach (var variable in contextVariables)
			try {
				var stParam = (VMParameter)variable;
				var dynamicParameter = new DynamicParameter(fsm.Entity, fsm, stParam);
				if (stParam.Implicit) {
					if (!contextParamsByStaticGuid.ContainsKey(stParam.BaseGuid))
						contextParamsByStaticGuid.Add(stParam.BaseGuid, dynamicParameter);
					else
						Logger.AddError(string.Format("FSM {0} dynamic state param {1} guid {2} dublicated",
							fsm.FSMStaticObject.Name, stParam.Name, stParam.BaseGuid));
				} else {
					if (!contextParamsByStaticGuid.ContainsKey(stParam.BaseGuid))
						contextParamsByStaticGuid.Add(stParam.BaseGuid, dynamicParameter);
					else
						Logger.AddError(string.Format("FSM {0} dynamic context param {1} guid {2} dublicated",
							fsm.FSMStaticObject.Name, stParam.Name, stParam.BaseGuid));
					if (!stParam.IsCustom) {
						var componentByName = fsm.Entity.GetComponentByName(stParam.ComponentName);
						if (componentByName == null) {
							Logger.AddError(string.Format(
								"Standart parameter {0} not loaded in {1}, because component {2} not found in entity",
								stParam.Name, fsm.FSMStaticObject.Name, stParam.ComponentName));
							continue;
						}
			...
			if (functionalComponentByName == null)
				...
				for (var index = 0; index < functionalComponentByName.Properties.Count; ++index) {
					...
					if (contextParamsByName.ContainsKey(property.PropertyName))
						Logger.AddError(string.Format("FSM {0} dynamic context param {1} dublicated",
							fsm.FSMStaticObject.Name, property.PropertyName));
					else {
						var dynamicParameter = new DynamicParameter(fsm.Entity, fsm, component, property);
	...
	private void SaveParameters(IDataWriter writer, string name) {
		...
		for (var index = 0; index < fsmDynamicParams.Count; ++index) {
			var fsmDynamicParam = fsmDynamicParams[index];
	...
	private void LoadParameters(XmlElement rootNode) {
		for (var i = 0; i < rootNode.ChildNodes.Count; ++i) {
			var childNode = (XmlElement)rootNode.ChildNodes[i];
			var innerText = childNode.FirstChild.InnerText;
			var uint64 = StringUtility.ToUInt64(innerText);
			if (contextParamsByStaticGuid.ContainsKey(uint64))
				((DynamicParameter)contextParamsByStaticGuid[uint64]).LoadFromXML(childNode);
			else
				Logger.AddError(string.Format("SaveLoad error: saved parameter by key {0} not found in fsm {1}",
					innerText, fsm.FSMStaticObject.Name));
		}
	}
}