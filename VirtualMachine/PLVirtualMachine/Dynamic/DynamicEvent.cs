...
public class DynamicEvent :
	...
	public DynamicEvent(VMEntity entity, IEvent staticEvent, DynamicFSM parentFSM)
		...
		try {
			...
			if (!isManual) {
				var name = Name;
				var componentType = ((VMFunctionalComponent)StaticEvent.Parent).ComponentType;
				var objectComponentByType = EngineAPIManager.GetObjectComponentByType(entity, componentType);
	...
	private void RegisterEngineEvent(VMComponent eventEngComponent) {
		...
		if (parentFSM.FSMStaticObject.GetCategory() == EObjectCategory.OBJECT_CATEGORY_GAME ||
		    !(eventEngComponent.Name != "Common"))
			return;
		isLazyEvent = true;
	}
	...
	private void InitCustomEvent() {
		...
		if (StaticEvent.EventRaisingType == EEventRaisingType.EVENT_RAISING_TYPE_CONDITION)
			dynamicEventBody = new DynamicEventBody((VMPartCondition)StaticEvent.Condition, this, GameTimeContext,
				OnCheckRise, Repeated, Entity);
		else if (StaticEvent.EventRaisingType == EEventRaisingType.EVENT_RAISING_TYPE_PARAM_CHANGE)
			dynamicEventBody = new DynamicEventBody((VMParameter)StaticEvent.EventParameter, this, GameTimeContext,
				OnCheckRise, Repeated, Entity);
		else {
			if (StaticEvent.EventRaisingType != EEventRaisingType.EVENT_RAISING_TYPE_TIME)
				return;
			dynamicEventBody = new DynamicEventBody(StaticEvent.EventTime, this, GameTimeContext, OnCheckRise, Repeated,
				Entity);
		}
	}

	public IGameMode GameTimeContext =>
		StaticEvent.GameTimeContext != null ? StaticEvent.GameTimeContext : parentFSM.GameTimeContext;

	public IEvent StaticEvent => (IEvent)StaticObject;

	public override void Think() {
		if (!Active || !isManual ||
		    (GameTimeContext != null && GameTimeManager.CurrentGameTimeContext != null &&
		     GameTimeContext.Name != GameTimeManager.CurrentGameTimeContext.Name) || !dynamicEventBody.NeedUpdate())
			return;
	...
	public void RaiseFromEngineImpl(params object[] parameters) {
		...
		for (var index = 0; index < parameters.Length; ++index) {
			var type = ReturnMessages[index].Type;
			var name = ReturnMessages[index].Name;
			var editorType = VMEngineAPIManager.ConvertEngineTypeToEditorType(parameters[index], type);
			var eventMessage = new EventMessage();
			eventMessage.Initialize(name, type, editorType);
			raisingEventMessageList.Add(eventMessage);
		}

		var raisingMode = EEventRaisingMode.ERM_ADD_TO_QUEUE;
			...
			if (raisingEventMessageList != null) {
				if (StaticEvent.ReturnMessages.Count != raisingEventMessageList.Count)
					Logger.AddError(string.Format(
						"Dynamic event messages count {0} don't match static event messages count: {1}",
						raisingEventMessageList.Count, StaticEvent.ReturnMessages.Count));
				for (var index = 0; index < StaticEvent.ReturnMessages.Count; ++index)
					if (index < raisingEventMessageList.Count) {
						if (!VMTypeUtility.IsTypesCompatible(StaticEvent.ReturnMessages[index].Type,
							    raisingEventMessageList[index].Type))
							Logger.AddError(string.Format(
								"Dynamic event message {0} type {1} is incompatible with static event message {2} type {3}",
								raisingEventMessageList[index].Name, raisingEventMessageList[index].Type,
								StaticEvent.ReturnMessages[index].Name, StaticEvent.ReturnMessages[index].Type));
						raisingMessages.Add(raisingEventMessageList[index]);
					}
			}
		} else
			for (var index = 0; index < raisingEventMessageList.Count; ++index)
				raisingMessages.Add(raisingEventMessageList[index]);
	...
	public void Execute(RaisedEventInfo evntInfo) {
		...
		for (var index = 0; index < currSubscribeFSMList.Count; ++index)
			if (evntInfo.SendingFSMGuid == Guid.Empty ||
			    currSubscribeFSMList[index].DynamicGuid == evntInfo.SendingFSMGuid)
				try {
					currSubscribeFSMList[index].OnProcessEvent(evntInfo);
				} catch (Exception ex) {
					Logger.AddError(string.Format("Error during event {0} subscribtion by fsm {1} processing: {2}",
						Name, currSubscribeFSMList[index].FSMStaticObject.Name, ex));
				}
	...
	public EObjectCategory GetCategory() {
		return StaticEvent.GetCategory();
	}
	...
	public bool IsEqual(IObject other) {
		return !(typeof(DynamicEvent) != other.GetType()) &&
		       (long)((DynamicObject)other).StaticGuid == (long)StaticGuid &&
		       !(((DynamicEvent)other).OwnerFSM.EngineGuid == OwnerFSM.EngineGuid);
	}
	...
	public List<BaseMessage> ReturnMessages =>
		StaticObject != null ? ((IEvent)StaticObject).ReturnMessages : apiEventMessages;
	...
	private void LoadApiMessages(APIEventInfo apiEventInfo) {
		...
		if (apiEventInfo == null)
			...
			for (var index = 0; index < apiEventInfo.MessageParams.Count; ++index) {
				var type = apiEventInfo.MessageParams[index].Type;
				var name = Name + "_message_" + apiEventInfo.MessageParams[index].Name;
	...
	private static void RegisterInComponent(VMComponent component, DynamicEvent target) {
		if (component is IInitialiseEvents initialiseEvents)
			...
			Logger.AddError("Event not found, need generate code, component : " + component.Name + " , event name : " +
			                target.Name);
	}
