...
public class DynamicTalkingFSM : DynamicFSM {
	...
	private List<VMEntity> speechAuthorsList = new();
	private List<ILink> curentTalkingEventLinks = new();
	...
	public DynamicTalkingFSM(VMEntity entity, VMLogicObject templateObj)
		...
		if (entity == null)
			Logger.AddWarning(string.Format("Cannot init talking fsm: speaking component not found in {0}",
				templateObj.Name));
		speechAuthorsList.Add(Entity);
		if (graphManager != null)
			return;
		Logger.AddError(string.Format("Invalid talking fsm creation at {0}, graph manager not created!",
			templateObj.Name));
	}
	...
	public VMTalkingGraph CurrentTalkingGraph =>
		CurrentState != null && CurrentState.Parent != null && typeof(VMTalkingGraph) == CurrentState.Parent.GetType()
			? (VMTalkingGraph)CurrentState.Parent
			: null;
	...
	public override void OnStart() {
		base.OnStart();
		beginTalkingEvent =
			GetContextEvent(EngineAPIManager.GetSpecialEventName(ESpecialEventName.SEN_BEGIN_TALKING,
	...
	public override void OnProcessEvent(RaisedEventInfo evntInfo) {
		...
		if (CurrentTalkingGraph != null) {
			...
			if (!Speaking.IsActiveTalking)
				return;
			var activeTalking = Speaking.ActiveTalking;
			if (CheckTalkingActual(activeTalking))
				...
				Logger.AddError(string.Format(
					"Talking {0} won't cause any speeches by inner logic. Talking will be close to avoid 'Chicken Bug' at {1}",
					activeTalking.Name, CurrentStateInfo));
			}
		}
	}
	...
	public void OnChangeState(IState state) {
		...
		try {
			...
			if (state != null && state.Parent != null) {
				for (var index = 0; index < state.OutputLinks.Count; ++index) {
					var outputLink = (VMEventLink)state.OutputLinks[index];
					if (outputLink.Event != null && outputLink.Event.EventInstance != null &&
					    outputLink.Event.EventInstance.Name == VMSpeaking.BeginTalkingEventName)
						curentTalkingEventLinks.Add(outputLink);
				}

				var parent = (FiniteStateMachine)state.Parent;
				for (var index = 0; index < parent.EnterLinks.Count; ++index) {
					var enterLink = (VMEventLink)parent.EnterLinks[index];
					if (!enterLink.IsInitial() && enterLink.Event != null && enterLink.Event.EventInstance != null &&
					    enterLink.Event.EventInstance.Name == VMSpeaking.BeginTalkingEventName)
	...
	protected bool CheckTalkingActual(IFiniteStateMachine talkingGraph, int depth = 0) {
		...
		if (depth > 10) {
			Logger.AddError(string.Format(
				"Unusual talking content error in talking {0}: probably invalid talking data loaded",
				talkingGraph.Name));
			return false;
		}

		return talkingGraph.Abstract
			? CheckTalkingActual((IFiniteStateMachine)Speaking.CurrentTalking.State, depth + 1)
			: TalkingManager.CheckTalkingStateActual(talkingGraph.InitState).Key;
	}
	...
	public void CheckContextTalkingAvailable() {
		if (!Entity.Instantiated) {
			...
			Logger.AddError(string.Format("Attempt to removed object fsm processing at {0}, state {1}",
				FSMStaticObject.Name, str));
		} else {
			if (Speaking == null || (Speaking.CurrentTalking != null && Speaking.CurrentTalking.State != null))
				return;
			var isTalkingAvailable = false;
			for (var index = 0; index < curentTalkingEventLinks.Count; ++index) {
				isTalkingAvailable = TalkingManager
					.CheckTalkingStateActual(((VMEventLink)curentTalkingEventLinks[index]).DestState).Key;
