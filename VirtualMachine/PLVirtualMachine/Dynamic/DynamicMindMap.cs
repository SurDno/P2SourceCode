...
public class DynamicMindMap : IDynamicMindMapObject, ISerializeStateSave, IDynamicLoadSerializable {
	...
	private List<DynamicMindMapNode> nodes = new();
	private static Dictionary<ulong, DynamicMindMap> mindMaps = new(UlongComparer.Instance);
	private static List<ulong> mindMapOrderList = new();
	...
	public DynamicMindMap(VMLogicMap staticMindMap, IMMPage page) {
		...
		if (staticMindMap.LogicMapType == ELogicMapType.LOGIC_MAP_TYPE_GLOBAL_MINDMAP)
			...
			Logger.AddError(string.Format("Invalid logic map {0} type {0} for mind map creation", staticMindMap.Name,
				staticMindMap.LogicMapType));
			return;
		}
	...
	public static void RemoveMindMap(ILogicMap mindMap) {
		if (!mindMaps.ContainsKey(mindMap.BaseGuid))
			Logger.AddError(string.Format(
				"Cannot remove mind map {0}: this mind map not added previously, or has been removed", mindMap.Name));
		else {
	...
	public static void Update() {
		foreach (var mindMap in mindMaps) {
			var dynamicMindMap = mindMap.Value;
	...
	public static Guid RegistrDynamicMMObject(IDynamicMindMapObject mmObj) {
		try {
			...
			Logger.AddError(string.Format("SaveLoad error: cannot register mindmap object guid={0}, error: {1}",
				mmObj.StaticGuid, ex));
			return new Guid();
		}
	}
	...
	public static IMMNode GetEngineNodeByStaticGuid(ulong stGuid) {
		var objectByStaticguid = GetDynamicMMObjectByStaticguid(stGuid);
	...
	public static void SaveMindMapsToXML(IDataWriter writer) {
		...
		for (var index = 0; index < mindMapOrderList.Count; ++index) {
			var mindMapOrder = mindMapOrderList[index];
			if (mindMaps.ContainsKey(mindMapOrder))
				SaveManagerUtility.SaveDynamicSerializable(writer, "Element", mindMaps[mindMapOrder]);
			else
				Logger.AddError(string.Format("SaveLoad error: invalid mind map guid in indexed list: {0}",
					mindMapOrder));
		}
	...
	public static void LoadMindMapsFromXML(XmlElement xmlNode) {
		var firstChild1 = (XmlElement)xmlNode.FirstChild;
		for (var i = 0; i < firstChild1.ChildNodes.Count; ++i) {
			var childNode = (XmlElement)firstChild1.ChildNodes[i];
			var firstChild2 = childNode.FirstChild;
			var nextSibling = firstChild2.NextSibling.NextSibling;
			var key = VMSaveLoadManager.ReadUlong(firstChild2);
			var num = VMSaveLoadManager.ReadBool(nextSibling) ? 1 : 0;
			var logicMap = ((VMGameRoot)IStaticDataContainer.StaticDataContainer.GameRoot).LogicMaps[key];
	...
	private void LoadNodes() {
		for (var index = 0; index < staticMindMap.Nodes.Count; ++index)
	...
	private DynamicMindMapNode AddMindMapNode(VMLogicMapNode staticNode) {
		var dynamicMindMapNode = new DynamicMindMapNode(this, staticNode);
		var nodeKindByNodeType = GetEngineNodeKindByNodeType(dynamicMindMapNode.StaticNode.NodeType);
		var position = new Position(dynamicMindMapNode.StaticNode.GameScreenPositionX,
			dynamicMindMapNode.StaticNode.GameScreenPositionY);
		var node = ServiceCache.Factory.Create<IMMNode>(dynamicMindMapNode.DynamicGuid);
	...
	private void LoadLinks() {
		for (var index = 0; index < staticMindMap.Links.Count; ++index) {
			var link1 = staticMindMap.Links[index];
			if (link1.Source == null)
				Logger.AddError(string.Format("Mind map link {0} source not defined at mind map {1}", link1.BaseGuid,
					Name));
			else if (!mmObjectsDict.ContainsKey(link1.Source.BaseGuid))
				Logger.AddError(string.Format("invalid mm link source id={0} at mind map {1}", link1.Source.BaseGuid,
					Name));
			else if (link1.Destination == null)
				Logger.AddError(string.Format("Mind map link {0} destination not defined at mind map {1}",
					link1.BaseGuid, Name));
			else if (!mmObjectsDict.ContainsKey(link1.Destination.BaseGuid))
				Logger.AddError(string.Format("invalid mm link destination id={0} at mind map {1}",
					link1.Destination.BaseGuid, Name));
			else {
				var dynamicMindMapNode1 = (DynamicMindMapNode)mmObjectsDict[link1.Source.BaseGuid];
				var dynamicMindMapNode2 = (DynamicMindMapNode)mmObjectsDict[link1.Destination.BaseGuid];
				var link2 = ServiceCache.Factory.Create<IMMLink>();
