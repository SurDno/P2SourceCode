...
public class VMDynamicCommonList : VMCommonList, ISerializeStateSave, IDynamicLoadSerializable {
	private List<object> realObjectsList = new();
	...
	public override object GetObject(int objIndex) {
		...
		if (objIndex >= realObjectsList.Count) {
			Logger.AddWarning(string.Format("Invalid list index {0} in list of {1} values", objIndex,
				realObjectsList.Count));
			objIndex = realObjectsList.Count - 1;
		}

		if (realObjectsList[objIndex] != null)
			return typeof(IParam).IsAssignableFrom(realObjectsList[objIndex].GetType())
				? ((IParam)realObjectsList[objIndex]).Value
				: realObjectsList[objIndex];
		var data = base.GetObject(objIndex);
		...
		if (variable != null) {
			...
			if (variable.IsBinded) {
				var dynamicParam =
					((VMVariableService)IVariableService.Instance).GetDynamicParam(variable,
						VMEngineAPIManager.LastMethodExecInitiator);
				realObjectsList[objIndex] = !(typeof(DynamicParameter) == dynamicParam.GetType())
					? dynamicParam.Value
					: (DynamicParameter)dynamicParam;
	...
	public override int RemoveObjectInstanceByGuid(Guid objGuid) {
		var num = -1;
		for (var index = 0; index < realObjectsList.Count; ++index) {
			...
			if (realObjectsList[index] != null) {
				if (typeof(IParam).IsAssignableFrom(realObjectsList[index].GetType())) {
					if (((IParam)realObjectsList[index]).Value != null) {
						var obj = ((IParam)realObjectsList[index]).Value;
	...
	public override void Merge(ICommonList mergeList) {
		var count = realObjectsList.Count;
		base.Merge(mergeList);
		for (var index = count; index < ObjectsCount; ++index)
			realObjectsList.Add(null);
	}
	...
	public void LoadFromXML(XmlElement objNode) {
		...
		for (var i = 0; i < objNode.ChildNodes.Count; ++i)
			if (objNode.ChildNodes[i].Name == "CommonListElementsList") {
				var childNode1 = (XmlElement)objNode.ChildNodes[i];
