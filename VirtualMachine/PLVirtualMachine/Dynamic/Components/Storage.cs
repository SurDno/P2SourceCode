...
public class Storage : VMStorage, IInitialiseComponentFromHierarchy, IInitialiseEvents {
		...
		try {
			if (template == null)
				...
				if (containerByTemplate == null)
					...
					var crowdStorageCommand = new CrowdStorageCommand();
					crowdStorageCommand.Initialize(EStorageCommandType.StorageCommandTypeAddItem, this,
						containerByTemplate.Owner, template, thingsCount);
					CrowdStorageCommandProcessor.MakeStorageCommand(crowdStorageCommand, true);
				} else {
					TestSimplePickUpMode = true;
					var num = 0;
					do {
						var iNeedCount = thingsCount - num;
						var inventoryByTemplate =
							DoAddItemsToInventoryByTemplate(containerByTemplate.Owner, template, iNeedCount);
	...
	public int DoAddItemsToInventoryByTemplate(
		...
		var component = owner.GetComponent<IStorableComponent>();
		var inventoryByTemplate = component.Max;
	...
	private void DoPickUpCombinationByTemplate(IBlueprintRef combinationObject, bool dropIfBusy = true) {
		if (combinationObject == null)
			Logger.AddError(
				string.Format("Combination object for adding items to storage {0} not defined", Parent.Name));
		else {
			var blueprint = combinationObject.Blueprint;
			if (blueprint != null) {
				if (blueprint.IsFunctionalSupport("Combination")) {
					var property = blueprint.GetProperty("Combination",
					...
					if (property != null) {
						if (property.Value != null) {
							if (typeof(ObjectCombinationDataStruct) == property.Value.GetType()) {
								var combinationData = (ObjectCombinationDataStruct)property.Value;
								if (combinationData.GetElementsCount() > 0)
									((GlobalStorageManager)VMGlobalStorageManager.Instance).AddCombinationToStorage(
										combinationData, this, null, null, dropIfBusy);
								OnModify();
							} else
								Logger.AddError(string.Format(
									"Combination object {0} for adding items to storage {1} combination component ObjectCombinationDataStruct param value is invalid",
									blueprint.Name, Parent.Name));
						} else
							Logger.AddError(string.Format(
								"Combination object {0} for adding items to storage {1} combination component not contains ObjectCombinationDataStruct param value",
								blueprint.Name, Parent.Name));
					} else
						Logger.AddError(string.Format(
							"Combination object {0} for adding items to storage {1} combination component not contains ObjectCombinationDataStruct param",
							blueprint.Name, Parent.Name));
				} else
					Logger.AddError(string.Format(
						"Combination object {0} for adding items to storage {1} not contains Combination component",
						blueprint.Name, Parent.Name));
			} else
				Logger.AddError(string.Format(
					"Combination object with guid={0} for adding items to storage {1} not found",
					combinationObject.Name, Parent.Name));
		}
	}
	...
	public override bool CheckExistCombinationElement(IBlueprintRef combinationTemplate) {
		...
		if (blueprint == null) {
			Logger.AddError(string.Format(
				"Combination for checking element containing in storage {0} not defined at {1} !", Parent.Name,
		...
		if (blueprint.IsFunctionalSupport("Combination")) {
			...
			if (property != null) {
						...
						if (combinationDataStruct.GetElementsCount() > 0)
							foreach (var storableComponent in Component.Items)
								if (storableComponent != null && !storableComponent.IsDisposed) {
									var templateId = storableComponent.Owner.TemplateId;
									var engineTemplateByGuid =
										((VMGameRoot)IStaticDataContainer.StaticDataContainer.GameRoot)
										.GetEngineTemplateByGuid(templateId);
									if (engineTemplateByGuid == null)
										Logger.AddError(string.Format(
											"Combination element checking in storage {0} error: template {1} id = {2} not registered in vm",
											Parent.Name, storableComponent.Owner.Name, templateId));
									else if (combinationDataStruct.ContainsItem(engineTemplateByGuid))
										return true;
								}
					} else
						Logger.AddError(string.Format(
							"Combination object {0} for checking element containing in storage {1}  not contains Combination data at",
							blueprint.Name, Parent.Name, DynamicFSM.CurrentStateInfo));
				} else
					Logger.AddError(string.Format(
						"Combination object {0} for checking element containing in storage {1}  not contains Combination data at",
						blueprint.Name, Parent.Name, DynamicFSM.CurrentStateInfo));
				...
				Logger.AddError(string.Format(
					"Combination object {0} for checking element containing in storage {1}  not contains ObjectCombinationDataStruct param",
					blueprint.Name, Parent.Name, DynamicFSM.CurrentStateInfo));
		} else
			Logger.AddError(string.Format(
				"Combination object {0} for checking element containing in storage {1}  not contains Combination component at {2}",
				blueprint.Name, Parent.Name, DynamicFSM.CurrentStateInfo));
