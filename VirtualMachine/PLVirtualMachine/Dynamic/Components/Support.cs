...
public class Support : VMSupport, IInitialiseComponentFromHierarchy, IInitialiseEvents {
	...
	public override float Random() {
		return (float)VMMath.GetRandomDouble();
	}

	public override float Rand(float maxVal) {
		return maxVal * (float)VMMath.GetRandomDouble();
	}
	...
	public override int MinMaxIntNextRand(int minVal, int maxVal, int prevVal) {
		var num = prevVal + 1 + VMMath.GetRandomInt(maxVal - minVal - 1);
		if (num >= maxVal)
			num -= maxVal - minVal;
		return num;
	}
	...
	public override GameTime PoissonRandTime(float flowPerSecond) {
		...
		var d = 1E-06 + VMMath.GetRandomDouble();
		if (d > 1.0)
			d = 1.0;
	...
	public override int GetListObjectsCount(VMCommonList objList) {
		...
		Logger.AddError(string.Format("GetListObjectsCount error: list instance is null !!! at {0}",
	...
	public override object GetRandomListObject(VMCommonList objList) {
		if (objList == null) {
			Logger.AddError(string.Format("Cannot get random object from list: list instance not defined at {0}",
				DynamicFSM.CurrentStateInfo));
			return null;
		}

		var index = VMMath.GetRandomInt(objList.ObjectsCount);
		if (index >= objList.ObjectsCount) {
			Logger.AddError(string.Format("Random index generating error: probably list is empty at {0}",
				DynamicFSM.CurrentStateInfo));
