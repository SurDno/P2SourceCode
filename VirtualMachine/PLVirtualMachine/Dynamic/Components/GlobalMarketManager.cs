...
public class GlobalMarketManager :
	...
	public override string GetComponentTypeName() {
		return nameof(GlobalMarketManager);
	}
	...
	public override void SetBaseItemTradePrices(
		...
		if (tradePricesTable == null)
			...
			var strArray2 = itemTemplatesPrices.Split(';');
			var strArray3 = itemTemplatesBuyPrices.Split(';');
			if (strArray1.Length != strArray2.Length)
				Logger.AddError(string.Format("Set prices items count not corresponded to prices count at {0}",
			...
			if ("" != strArray2[0])
				...
				Logger.AddError(string.Format("Cannot set base price for {0}, price not defined ",
					DynamicFSM.CurrentStateInfo));
			var num2 = 0.0f;
			if ("" != strArray3[0])
				num2 = GameComponent.ReadContextFloatParamValue(methodExecInitiator, strArray3[0]);
			else
				Logger.AddError(string.Format("Cannot set base buy price for {0}, buy price not defined ",
					DynamicFSM.CurrentStateInfo));
			for (var index = 0; index < strArray1.Length; ++index) {
				var key = strArray1[index];
				if (!tradePricesTable.ContainsKey(key))
					Logger.AddError(string.Format("Item template with name {0} not found in prices table at {1}", key,
						DynamicFSM.CurrentStateInfo));
				else {
	...
	public override void SetBaseItemTradePriceFactors(
		...
		if (tradePricesTable == null)
			...
			var strArray1 = itemTemplatesNames.Split(';');
			var strArray2 = itemTemplatesFactors.Split(';');
			var strArray3 = itemTemplatesBuyFactors.Split(';');
			if (strArray1.Length != strArray2.Length)
				Logger.AddError(string.Format("Set prices items count not corresponded to prices count at {0}",
					DynamicFSM.CurrentStateInfo));
			if (strArray1.Length == 0)
				return;
			var methodExecInitiator = VMEngineAPIManager.LastMethodExecInitiator;
			var num1 = GameComponent.ReadContextFloatParamValue(methodExecInitiator, strArray2[0]);
			var num2 = GameComponent.ReadContextFloatParamValue(methodExecInitiator, strArray3[0]);
			for (var index = 0; index < strArray1.Length; ++index) {
				var key = strArray1[index];
				if (!tradePricesTable.ContainsKey(key))
					Logger.AddError(string.Format("Item template with name {0} not found in prices table at {1}", key,
	...
	public override void SetBaseItemTradePrice(IBlueprintRef template, float price) {
		if (tradePricesTable == null)
			...
			var name = template.Blueprint.Name;
			if (!tradePricesTable.ContainsKey(name))
				Logger.AddError(string.Format("Item template with name {0} not found in prices table at {1}", name,
					DynamicFSM.CurrentStateInfo));
			else {
	...
	public override float GetCurrentItemTradeGlobalPrice(string templateName, bool buyPrice = false) {
		...
		if (!tradePricesTable.ContainsKey(key))
			key = "Pathologic";
		if (!tradePricesTable.ContainsKey(key)) {
			Logger.AddError(string.Format("Item template with name {0} not found in prices table at {1}", templateName,
				DynamicFSM.CurrentStateInfo));
			return 0.0f;
		}
	...
	public void InitTradePricesTable(IEnumerable<IBlueprint> allTemplates) {
		...
		foreach (var allTemplate in allTemplates)
			if ((allTemplate.GetCategory() == EObjectCategory.OBJECT_CATEGORY_ITEM ||
			     allTemplate.GetCategory() == EObjectCategory.OBJECT_CATEGORY_OTHERS) &&
			    allTemplate.IsFunctionalSupport("Storable")) {
				var itemTradePriceInfo = new ItemTradePriceInfo((IWorldBlueprint)allTemplate);
		...
		for (var i1 = 0; i1 < xmlNode.ChildNodes.Count; ++i1) {
			...
			if (childNode1.Name == "TradePricesTable") {
				...
				for (var i2 = 0; i2 < childNode1.ChildNodes.Count; ++i2) {
					...
					var itemTradePriceInfo = new ItemTradePriceInfo();
