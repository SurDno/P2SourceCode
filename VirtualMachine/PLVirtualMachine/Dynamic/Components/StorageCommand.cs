...
public class StorageCommand :
	...
	public void AfterSaveLoading() {
		if (!(targetStorageEntityLoadedId != Guid.Empty))
			return;
		var entityByEngineGuid = WorldEntityUtility.GetDynamicObjectEntityByEngineGuid(targetStorageEntityLoadedId);
		if (entityByEngineGuid != null) {
			var componentByName = entityByEngineGuid.GetComponentByName("Storage");
			if (componentByName != null)
				TargetStorage = (VMStorage)componentByName;
			else
				Logger.AddError(string.Format(
					"SaveLoad error: storage component not found in target storage entity {0} at storage group command loading",
					entityByEngineGuid.Name));
		} else
			Logger.AddError(string.Format(
				"SaveLoad error: target storage entity id={0} not found at storage group command loading",
				targetStorageEntityLoadedId));
	}
