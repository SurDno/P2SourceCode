...
public class DynamicFSM :
		...
		if (!passive) {
			...
			if (templateObj.GetCategory() == EObjectCategory.OBJECT_CATEGORY_QUEST) {
				if (((VMQuest)templateObj).StartEvent == null)
					Logger.AddError(string.Format("Starting event for quest {0} not dfined. Quest won't be started",
						templateObj.Name));
				else {
					Active = false;
					var eventByStaticGuid =
						VirtualMachine.Instance.GameRootFsm.GetEventByStaticGuid(((VMQuest)templateObj).StartEvent
							.BaseGuid);
					if (eventByStaticGuid == null)
						Logger.AddError(string.Format(
							"Starting event {0} for quest {1} not found. Quest won't be started",
							((VMQuest)templateObj).StartEvent.Name, templateObj.Name));
			...
			if (templateObj.GetCategory() == EObjectCategory.OBJECT_CATEGORY_GAME)
				eventManager.LoadFSMEvent(
					new DynamicEvent(this,
	...
	public virtual FSMGraphManager CreateGraphManager() {
		return new FSMGraphManager(this);
	}

	public FSMEventManager CreateEventManager() {
		return new FSMEventManager(this);
	}

	public FSMFunctionManager CreateFunctionManager() {
		return new FSMFunctionManager(this);
	}

	public FSMParamsManager CreateParamsManager() {
		return new FSMParamsManager(this);
	}
	...
	private void SaveSubgraphRefParamsData(IDataWriter writer, string name) {
		...
		for (var index = 0; index < refParams.Count; ++index) {
			var refParam = refParams[index];
	...
	private void LoadSubgraphRefParamsData(XmlElement rootNode) {
		...
		for (var i = 0; i < rootNode.ChildNodes.Count; ++i) {
			...
			refParamsData.Add(new KeyValuePair<ulong, Guid>(VMSaveLoadManager.ReadUlong(firstChild),
				VMSaveLoadManager.ReadGuid(firstChild.NextSibling)));
		}
	}
	...
	public virtual void AfterSaveLoading() {
		...
		if (refParamsData != null)
			for (var index = 0; index < refParamsData.Count; ++index) {
				var keyValuePair = refParamsData[index];
				var key = keyValuePair.Key;
				keyValuePair = refParamsData[index];
				var engGuid = keyValuePair.Value;
				var entityByEngineGuid = WorldEntityUtility.GetDynamicObjectEntityByEngineGuid(engGuid);
				if (entityByEngineGuid != null) {
					var contextParam = entityByEngineGuid.GetFSM().GetContextParam(key.ToString());
					if (contextParam != null) {
						if (typeof(DynamicParameter) == contextParam.GetType()) {
							...
							Logger.AddError(string.Format(
								"SaveLoad error: strange ref param with id={0} in entity {1}: ref param type is {2}",
								key, entityByEngineGuid.Name, contextParam.GetType()));
					} else
						Logger.AddError(string.Format("SaveLoad error: ref param with id={0} in entity {1} not found",
							key, entityByEngineGuid.Name));
				} else
					Logger.AddError(string.Format("SaveLoad error: ref params owner entity with id={0} not found",
						engGuid));
			}
		...
		set {
			if (!base.Active & value) {
				if (Entity.Instance != null && Entity.Instance.IsDisposed)
					Logger.AddError(string.Format("Object fsm start error: object {0} is dead !!!",
						FSMStaticObject.Name));
				OnStart();
				if (FSMStaticObject.StateGraph != null) {
					var startEventFuncName = FSMStaticObject.GetStartEventFuncName();
					if (startEventFuncName == null) {
						Logger.AddError(
							string.Format("Null start event name at {0} received !!!", FSMStaticObject.Name));
						return;
					}

					if ("" == startEventFuncName) {
						Logger.AddError(string.Format("Empty start event name at {0} received !",
							FSMStaticObject.Name));
						return;
					}
	...
	public override void Think() {
		...
		var startEvent = FSMStaticObject.GetStartEvent();
		try {
			if (eventManager != null) {
				var currentState = graphManager.CurrentState;
				foreach (var fsmEvent in eventManager.FSMEvents)
					if ((currentState != null || (long)fsmEvent.BaseGuid == (long)startEvent.BaseGuid) &&
					    fsmEvent.NeedUpdate())
						fsmEvent.Think();
			}
		} catch (Exception ex) {
	...
	public void ProcessEvent(RaisedEventInfo eventInfo) {
	...
	public IParam GetContextParam(string paramName) {
		if (Entity.IsDisposed) {
			Logger.AddError(string.Format("Attempt to removed object {0} param accessing at  {1}", FSMStaticObject.Name,
				CurrentStateInfo));
			return null;
		}

		if (graphManager != null) {
			var contextMessage = graphManager.GetContextMessage(paramName);
	...
	public DynamicEvent GetContextEvent(string eventName) {
		if (Entity.IsDisposed) {
			Logger.AddError(string.Format("Attempt to removed object {0} event rising at  {1}", FSMStaticObject.Name,
				CurrentStateInfo));
			return null;
		}
	...
	public DynamicEvent GetEventByStaticGuid(ulong stEventGuid) {
		try {
			var eventByStaticGuid = FindEventByStaticGuid(stEventGuid);
			if (eventByStaticGuid == null && FSMStaticObject.GetCategory() != EObjectCategory.OBJECT_CATEGORY_GAME &&
			    typeof(IBlueprint).IsAssignableFrom(FSMStaticObject.GetType())) {
	...
	public virtual void OnProcessEvent(RaisedEventInfo evntInfo) {
		...
		if ("OnTimer" == evntInfo.Instance.Name) {
			...
			if (evntInfo.Messages.Count > 0) {
				var message = evntInfo.Messages[0];
				if (message != null)
	...
	private void SubscribeToEvents(VMState currState) {
		if (currState != null)
			...
			if (startEventFuncName == null)
				...
				if (eventManager == null)
					eventManager = CreateEventManager();
				var contextEvent = eventManager.GetContextEvent(startEventFuncName);
	...
	public void RaiseEventByName(string eventName, EEventRaisingMode raisingMode = EEventRaisingMode.ERM_ADD_TO_QUEUE) {
		if (eventName == null)
			Logger.AddError(string.Format("Event by name raising error at {0}: event name is null !!!",
				StaticObject.BaseGuid));
		else
			try {
				if (eventManager == null)
					eventManager = CreateEventManager();
				var contextEvent = eventManager.GetContextEvent(eventName);
				if (contextEvent != null)
					RaiseEvent(new RaisedEventInfo(contextEvent), raisingMode);
				else
					Logger.AddError(string.Format("Event with name {0} not found in object {1}", eventName,
						StaticObject.BaseGuid));
			} catch (Exception ex) {
	...
	public void RaiseEvent(RaisedEventInfo evntInfo, EEventRaisingMode raisingMode) {
		var flag = raisingMode == EEventRaisingMode.ERM_ATONCE;
		if (flag && DynamicTalkingFSM.IsTalking && evntInfo.Instance.Name !=
		    EngineAPIManager.GetSpecialEventName(ESpecialEventName.SEN_SPEECH_REPLY, typeof(VMSpeaking)))
			flag = false;
		if (flag)
	...
	public void RaiseTimerEvent(GameTimer gameTimer, int iCount = 1) {
		...
		if (gameTimer.GTType == EGameTimerType.GAME_TIMER_TYPE_RELATIVE_GLOBAL)
			eventName = EngineAPIManager.GetSpecialEventName(ESpecialEventName.SEN_ON_GLOBAL_TIMER,
				typeof(VMGameComponent), true);
		else if (gameTimer.GTType == EGameTimerType.GAME_TIMER_TYPE_RELATIVE_LOCAL)
			eventName = EngineAPIManager.GetSpecialEventName(ESpecialEventName.SEN_ON_LOCAL_TIMER,
				typeof(VMGameComponent), true);
		if (eventManager == null)
			eventManager = CreateEventManager();
		var contextEvent = eventManager.GetContextEvent(eventName);
		if (contextEvent != null) {
			...
			var returnMessages = contextEvent.StaticEvent.ReturnMessages;
			...
			Logger.AddError(string.Format("Timer event with guid={0} not found in object at {1}", StaticObject.BaseGuid,
				CurrentStateInfo));
	}
	...
	public void OnProcessActionLine() {
		var methodExecuteData = ExpressionUtility.GetLastActionMethodExecuteData();
	...
	public static DynamicFSM CreateEntityFSM(VMEntity newObjEntity) {
		...
		var editorTemplate = newObjEntity.EditorTemplate;
		return editorTemplate.GetCategory() != EObjectCategory.OBJECT_CATEGORY_QUEST
			? newObjEntity.GetComponentByName("Speaking") == null
				? new DynamicFSM(newObjEntity, (VMLogicObject)editorTemplate)
				: new DynamicTalkingFSM(newObjEntity, (VMLogicObject)editorTemplate)
			: new QuestFSM(newObjEntity, (VMLogicObject)editorTemplate);
	}
	...
	private void ExecuteEvent(RaisedEventInfo evntInfo) {
		evntInfo.Instance.Execute(evntInfo);
	}
