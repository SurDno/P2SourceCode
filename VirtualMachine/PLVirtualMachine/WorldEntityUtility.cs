...
public static class WorldEntityUtility {
	...
	private static List<VMWorldObject> staticWorldTemplatesList = new();
	private static Dictionary<Guid, VMEntity> dynamicEntityByEngineGuidDict = new(GuidComparer.Instance);

	private static Dictionary<HierarchyGuid, VMEntity> hierarchyGuidsToDynamicEntityDict =
	...
	public static VMEntity GetDynamicObjectEntityByUniName(string uniName) {
		...
		switch (GuidUtility.GetGuidFormat(uniName)) {
			case EGuidFormat.GT_BASE:
				var stguid = DefaultConverter.ParseUlong(uniName);
				if (stguid > 0UL) objectEntityByUniName = GetDynamicObjectEntityByStaticGuid(stguid);
				break;
			...
			case EGuidFormat.GT_ENGINE:
				var guid = DefaultConverter.ParseGuid(uniName);
				if (guid != Guid.Empty) objectEntityByUniName = GetDynamicObjectEntityByEngineGuid(guid);
				break;
		}
	...
	public static void AddDynamicObjectEntityByHierarchyGuid(
		...
		if (!hierarchyGuidsToDynamicEntityDict.ContainsKey(hierarchyGuid))
			...
			Logger.AddError(string.Format("Duplicate hierarchy guid {0} during registration dynamic object by template",
				hierarchyGuid));
	}
	...
	public static VMBaseEntity CreateWorldTemplateDynamicChildInstance(
		...
		if (template == null) {
			Logger.AddWarning(string.Format("Template for creating dynamic instance in {0} not defined",
				parentEntity.Name));
			return null;
		}
		...
		var engineTemplateByGuid =
			((VMGameRoot)IStaticDataContainer.StaticDataContainer.GameRoot).GetEngineTemplateByGuid(template.Id);
		if (engineTemplateByGuid == null) {
			Logger.AddWarning(string.Format("Template with engine guid = {0} not registered",
				GuidUtility.GetGuidString(template.Id)));
			return null;
		}

		var id = entity.Id;
		var dynamicChildInstance = new VMEntity();
		...
		var editorTemplate = engineTemplateByGuid;
	...
	public static VMBaseEntity CreateWorldTemplateDynamicChildInstance(
		...
		if (template == null) {
			Logger.AddError(string.Format("Template of created instance {0} not defined at {1}", instance.Id,
		...
		var engineTemplateByGuid =
			((VMGameRoot)IStaticDataContainer.StaticDataContainer.GameRoot).GetEngineTemplateByGuid(template.Id);
		if (engineTemplateByGuid == null) {
			Logger.AddError(string.Format("Template with engine guid = {0} not registered at {1}",
				GuidUtility.GetGuidString(template.Id), DynamicFSM.CurrentStateInfo));
			return null;
		}
	...
	public static void Clear() {
		foreach (var keyValuePair in dynamicEntityByEngineGuidDict)
