...
public class VMBlueprint :
	...
	IContext,
	ILogicObject {
	[FieldData("Static")] protected bool isStatic;
	...
	public override EObjectCategory GetCategory() {
		return EObjectCategory.OBJECT_CATEGORY_CLASS;
	}
	...
	public override List<IEvent> CustomEvents {
		get {
			var mergedEventsTo = new List<IEvent>();
			if (baseBlueprints != null)
				for (var index = 0; index < baseBlueprints.Count; ++index)
					VMBlueprintUtility.MergeCustomEvents(mergedEventsTo,
						((VMLogicObject)baseBlueprints[index]).CustomEvents);
	...
	public override IEnumerable<IVariable> GetContextVariables(
		...
		switch (contextVarCategory) {
			case EContextVariableCategory.CONTEXT_VARIABLE_CATEGORY_PARAM:
				var mergedParamsTo = new List<IVariable>();
				if (baseBlueprints != null)
					for (var index = 0; index < baseBlueprints.Count; ++index)
						VMBlueprintUtility.MergeContextParams(mergedParamsTo,
							baseBlueprints[index].GetContextVariables(contextVarCategory));
				VMBlueprintUtility.MergeContextParams(mergedParamsTo, base.GetContextVariables(contextVarCategory));
				foreach (var contextVariable in mergedParamsTo)
					yield return contextVariable;
				break;
			case EContextVariableCategory.CONTEXT_VARIABLE_CATEGORY_STATUS:
				break;
			default:
				foreach (var contextVariable in base.GetContextVariables(contextVarCategory))
					yield return contextVariable;
				break;
		}
	}
	...
	public override bool IsDerivedFrom(ulong iBlueprintGuid, bool bWithSelf = false) {
		...
		if (baseBlueprints != null)
			for (var index = 0; index < baseBlueprints.Count; ++index)
				if ((long)baseBlueprints[index].BaseGuid == (long)iBlueprintGuid ||
				    baseBlueprints[index].IsDerivedFrom(iBlueprintGuid))
					return true;
		return false;
	}

	public override IEnumerable<IStateRef> GetObjectStates() {
		foreach (var objectState in base.GetObjectStates())
			yield return objectState;
		if (baseBlueprints != null)
			for (var i = 0; i < baseBlueprints.Count; ++i)
				foreach (var objectState in baseBlueprints[i].GetObjectStates())
					yield return objectState;
	}
	...
	public virtual ulong GetInheritanceMappedEventGuid(ulong stEventGuid) {
		...
		return inheritancyMappedEventGuids != null && inheritancyMappedEventGuids.ContainsKey(stEventGuid)
			? inheritancyMappedEventGuids[stEventGuid]
			: stEventGuid;
	}
	...
	protected override void UpdateContextVariablesTotalCache() {
		...
		foreach (var contextVariable in GetContextVariables(EContextVariableCategory.CONTEXT_VARIABLE_CATEGORY_PARAM))
	...
	private Dictionary<string, IFunctionalComponent> GetAllFunctionalComponents() {
		var mergeTo = new Dictionary<string, IFunctionalComponent>();
		if (baseBlueprints != null)
			for (var index = 0; index < baseBlueprints.Count; ++index) {
				var functionalComponents = ((VMBlueprint)baseBlueprints[index]).GetAllFunctionalComponents();
	...
	protected void MakeInheritanceMapping(IEnumerable<IBlueprint> classHierarchyList) {
		if (events != null)
			for (var index = 0; index < events.Count; ++index) {
				var key = events[index].Parent.Name + "." + events[index].Name;
				foreach (VMBlueprint classHierarchy in classHierarchyList) {
					var functionalMappedEvents = classHierarchy.FunctionalMappedEvents;
					if (functionalMappedEvents != null && functionalMappedEvents.ContainsKey(key)) {
						var @event = functionalMappedEvents[key];
						...
						if (!inheritancyMappedEventGuids.ContainsKey(@event.BaseGuid))
							...
							Logger.AddError(string.Format("Inheritance mapped event with id={0} is dublicated in {1}",
								@event.BaseGuid, Name));
					}
				}
			}
	...
	protected IEnumerable<IBlueprint> GetClassHierarchy() {
		if (baseBlueprints != null)
			for (var i = 0; i < baseBlueprints.Count; ++i) {
				...
				foreach (var blueprint in ((VMBlueprint)baseBlueprints[i]).GetClassHierarchy())
	...
	private void LoadFunctionalMappedEvents() {
		...
		for (var index1 = 0; index1 < functionalComponents.Count; ++index1) {
			var engineEvents = functionalComponents[index1].EngineEvents;
			for (var index2 = 0; index2 < engineEvents.Count; ++index2) {
				var @event = engineEvents[index2];
				var key = functionalComponents[index1].Name + "." + @event.Name;
				if (functionalMappedEventsDict == null)
