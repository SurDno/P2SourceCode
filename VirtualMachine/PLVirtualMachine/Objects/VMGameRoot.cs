...
public class VMGameRoot :
	...
	private List<ISample> samplesList = new();

	[FieldData("GameModes", DataFieldType.Reference)]
	private List<IGameMode> gameModesList = new();

	[FieldData("LogicMaps", DataFieldType.Reference)]
	private List<ILogicMap> logicMapsList = new();

	[FieldData("BaseToEngineGuidsTable")] private BaseToEngineGuidsTableData baseToEngineGuidsTable;
			...
			if (xml.NodeType == XmlNodeType.Element)
				switch (xml.Name) {
					case "BaseToEngineGuidsTable":
						baseToEngineGuidsTable =
							EditorDataReadUtility.ReadEditorDataSerializable<BaseToEngineGuidsTableData>(xml, creator,
								typeContext);
						continue;
					case "ChildObjects":
					...
					case "CustomParams":
						customParamsDict =
							EditorDataReadUtility.ReadStringReferenceDictionary(xml, creator, customParamsDict);
						continue;
					case "EventGraph":
					...
					case "FunctionalComponents":
						functionalComponents =
							EditorDataReadUtility.ReadReferenceList(xml, creator, functionalComponents);
						continue;
					case "GameModes":
					...
					case "HierarchyScenesStructure":
						hierarchyScenesStructure =
							EditorDataReadUtility.ReadUlongEditorDataSerializableDictionary(xml, creator,
								hierarchyScenesStructure);
						continue;
					case "LogicMaps":
					...
					case "StandartParams":
						standartParamsDict =
							EditorDataReadUtility.ReadStringReferenceDictionary(xml, creator, standartParamsDict);
						continue;
					case "WorldObjectSaveOptimizeMode":
						worldObjectSaveOptimizedMode =
							EditorDataReadUtility.ReadValue(xml, worldObjectSaveOptimizedMode);
						continue;
					default:
	...
	public Dictionary<string, IGameMode> GameModes {
		get {
			...
			for (var index = 0; index < gameModesList.Count; ++index)
	...
	protected override void UpdateContextVariablesTotalCache() {
		...
		foreach (var staticObject in ((VMLogicObject)IStaticDataContainer.StaticDataContainer.GameRoot)
		         .GetStaticObjects())
			if (((BaseRef)staticObject).Exist)
				LoadContextVaraiblesTotalCache(staticObject);
		foreach (IVariable blueprint in
		         ((VMLogicObject)IStaticDataContainer.StaticDataContainer.GameRoot).GetBlueprints())
			LoadContextVaraiblesTotalCache(blueprint);
		var templatesTalkings = GetAllTemplatesTalkings();
		var variableList1 = new List<IVariable>();
		foreach (var vmTalkingGraph in templatesTalkings) {
			var vmStateRef = new VMStateRef();
			vmStateRef.Initialize(vmTalkingGraph);
			variableList1.Add(vmStateRef);
		}

		var allLogicMapNodes = GetAllLogicMapNodes();
		var variableList2 = new List<IVariable>();
		foreach (var lmNode in allLogicMapNodes) {
			var vmLogicMapNodeRef = new VMLogicMapNodeRef();
			vmLogicMapNodeRef.Initialize(lmNode);
			variableList2.Add(vmLogicMapNodeRef);
		}

		foreach (var contextVariable in GetContextVariables(EContextVariableCategory.CONTEXT_VARIABLE_CATEGORY_CLASS))
			LoadContextVaraiblesTotalCache(contextVariable);
		foreach (var contextVariable in GetContextVariables(EContextVariableCategory
			         .CONTEXT_VARIABLE_CATEGORY_GLOBAL_VAR))
			LoadContextVaraiblesTotalCache(contextVariable);
		foreach (var contextVariable in GetContextVariables(
			         EContextVariableCategory.CONTEXT_VARIABLE_CATEGORY_LOGIC_MAP))
			LoadContextVaraiblesTotalCache(contextVariable);
		var contextVariables = GetContextVariables(EContextVariableCategory.CONTEXT_VARIABLE_CATEGORY_GAMEMODE);
		foreach (var variable in variableList1)
			LoadContextVaraiblesTotalCache(variable);
		foreach (var variable in variableList2)
			LoadContextVaraiblesTotalCache(variable);
		foreach (var variable in contextVariables)
			LoadContextVaraiblesTotalCache(variable);
		foreach (var sample in ((VMGameRoot)IStaticDataContainer.StaticDataContainer.GameRoot).Samples) {
			var vmSampleRef = new VMSampleRef();
	...
	public IBlueprint GetBlueprintByGuid(ulong bpGuid) {
		...
		return allBlueprints.TryGetValue(bpGuid, out blueprint) || allClasses.TryGetValue(bpGuid, out blueprint)
			? blueprint
			: null;
	}

	public HierarchyGuid GetHierarchyGuidByHierarchyPath(string sPath) {
		return HierarchyGuid.Empty;
	}

	public List<IObjRef> GetNotHierarchyStaticObjects() {
		var hierarchyStaticObjects = new List<IObjRef>(staticObjectsBaseGuidsDict.Count);
		foreach (var keyValuePair in staticObjectsBaseGuidsDict) {
			var vmLogicObject = keyValuePair.Value;
	...
	public string GetStartGameEventFuncName() {
		return startGameEventFuncName;
	}
	...
	public IEnumerable<VMLogicMapNode> GetAllLogicMapNodes() {
		for (var i = 0; i < logicMapsList.Count; ++i) {
			for (var j = 0; j < logicMapsList[i].Nodes.Count; ++j)
	...
	public override void OnAfterLoad() {
		...
		var allLogicObjects = GetAllLogicObjects();
	...
	public override void Clear() {
		...
		if (hierarchyScenesStructure != null) {
			foreach (var keyValuePair in hierarchyScenesStructure)
		...
		if (allFunctionalComponents != null) {
			foreach (var functionalComponent in allFunctionalComponents)
	...
	private void ComputeStartEvents() {
		...
		foreach (var @event in events)
			if (!@event.IsManual) {
				var parent = (VMFunctionalComponent)@event.Parent;
				if (parent.Main && (!(startGameEventFuncName != "") || parent.BaseGuid <= num) &&
				    @event.Name ==
				    EngineAPIManager.GetSpecialEventName(ESpecialEventName.SEN_START_GAME, typeof(VMGameComponent)) &&
				    startGameEventFuncName == "") {
	...
	private void ComputeVariables() {
		...
		foreach (var allBlueprint in allBlueprints) {
			var blueprint = allBlueprint.Value;
			var instancesListVariable = GlobalVariableUtility.CreateGlobalTemplateInstancesListVariable(blueprint);
	...
	private void ComputeMaps() {
		...
		foreach (var logicMaps in logicMapsList) {
			var vmLogicMapRef = new VMLogicMapRef();
	...
	private void ComputeModes() {
		...
		foreach (var gameModes in gameModesList) {
			if (gameModes.IsMain)
				mainGameMode = gameModes;
			var vmGameModeRef = new VMGameModeRef();
	...
	private void ComputeBlueprints(IEnumerable<VMLogicObject> logicObjects) {
		...
		foreach (var allClass in allClasses) {
	...
	private void ComputeTemplates(IEnumerable<VMLogicObject> logicObjects) {
		...
		engineTemplateObjectGuidsDict.Clear();
		foreach (var logicObject in logicObjects)
			if ((logicObject.GetCategory() & EObjectCategory.OBJECT_CATEGORY_TEMPLATES) >
			    EObjectCategory.OBJECT_CATEGORY_NONE) {
				var vmWorldObject = (VMWorldObject)logicObject;
				if (!vmWorldObject.IsVirtual) {
					...
					if (vmWorldObject.EngineTemplateGuid != Guid.Empty) {
						if (!engineTemplateObjectGuidsDict.ContainsKey(vmWorldObject.EngineTemplateGuid))
							...
							Logger.AddError(string.Format("Engine template guid={0} is dublicated",
								vmWorldObject.EngineTemplateGuid));
					}
				}
			}
	}
	...
	private void PreloadTalkings() {
		...
		for (var index = 0; index < blueprints.Count; ++index)
			if (blueprints[index].Blueprint is VMBlueprint blueprint && blueprint.IsFunctionalSupport("Speaking") &&
			    blueprint.StateGraph != null)
				foreach (var finiteStateMachine in blueprint.StateGraph.GetSubGraphStructure(
					         EGraphType.GRAPH_TYPE_TALKING, false))
