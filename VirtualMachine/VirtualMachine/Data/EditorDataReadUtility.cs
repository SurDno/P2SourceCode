...
public static class EditorDataReadUtility {
	public static T ReadReference<T>(XmlReader xml, IDataCreator creator) where T : class {
		var id = DefaultConverter.ParseUlong(XmlReaderUtility.ReadContent(xml));
		if (id == 0UL)
			return default;
		var objectThreadSave = creator.GetOrCreateObjectThreadSave(id);
		...
		if (!xml.IsEmptyElement)
			while (xml.Read())
				if (xml.NodeType == XmlNodeType.Element) {
					var obj = ReadReference<T>(xml, creator);
					source.Add(obj);
	...
	public static List<T> ReadEnumList<T>(XmlReader xml, List<T> source)
	...
	public static List<string> ReadValueList(XmlReader xml, List<string> source) {
		...
		if (!xml.IsEmptyElement)
			while (xml.Read())
				if (xml.NodeType == XmlNodeType.Element) {
					var str = XmlReaderUtility.ReadContent(xml);
					source.Add(str);
				} else if (xml.NodeType == XmlNodeType.EndElement)
					break;
	...
	public static List<T> ReadEditorDataSerializableList<T>(
		...
		if (!xml.IsEmptyElement)
			while (xml.Read())
				if (xml.NodeType == XmlNodeType.Element) {
					var obj = ReadEditorDataSerializable<T>(xml, creator, "");
					source.Add(obj);
				} else if (xml.NodeType == XmlNodeType.EndElement)
					break;
	...
	public static List<T> ReadSerializableList<T>(XmlReader xml, List<T> source)
		where T : class, IVMStringSerializable, new() {
		var capacity = 0;
		if (!xml.IsEmptyElement) {
			xml.MoveToAttribute("count");
			capacity = DefaultConverter.ParseInt(xml.Value);
		...
		if (!xml.IsEmptyElement)
			while (xml.Read())
				if (xml.NodeType == XmlNodeType.Element) {
					var obj = ReadSerializable<T>(xml);
					source.Add(obj);
				} else if (xml.NodeType == XmlNodeType.EndElement)
					break;
	...
	public static string ReadValue(XmlReader xml, string tmp) {
	...
	public static T ReadSerializable<T>(XmlReader xml) where T : class, IVMStringSerializable, new() {
		var data = XmlReaderUtility.ReadContent(xml);
		var obj = new T();
		obj.Read(data);
		return obj;
	}
